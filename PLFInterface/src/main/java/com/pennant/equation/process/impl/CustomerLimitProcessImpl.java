...
public class CustomerLimitProcessImpl extends GenericProcess implements CustomerLimitProcess {
	...
	public Map<String, Object> fetchCustLimitEnqList(int pageNo, int pageSize) throws InterfaceException {
		...
		try {
			...
			if ("0000".equals(pcmlDoc.getValue(pcml + ".@ERCOD").toString())) {
				...
				custLimitMap.put("PageNumber",
						Integer.parseInt(String.valueOf(pcmlDoc.getValue(pcml + ".@RSPDTA.PageNumber"))));
				custLimitMap.put("TotalSize",
						Integer.parseInt(String.valueOf(pcmlDoc.getValue(pcml + ".@RSPDTA.TotalSize"))));
				...
				for (indices[0] = 0; indices[0] < dsRspCount; indices[0]++) {
					...
					strDate = StringUtils
							.trimToEmpty(String.valueOf(pcmlDoc.getValue(pcml + ".@RSPDTA.DETDTA.LmtExpiry", indices)));
					...
					item.setRepeatThousands(
							pcmlDoc.getValue(pcml + ".@RSPDTA.DETDTA.RptThousands", indices).toString());
		...
		try {
			...
			if ("0000".equals(pcmlDoc.getValue(pcml + ".@ERCOD").toString())) {
				...
				for (indices[0] = 0; indices[0] < dsRspCount; indices[0]++) {
					...
					item.setLimitCategory(pcmlDoc.getValue(pcml + ".@RSPDTA.DETDTA.LmtCategory", indices).toString());
					item.setLimitCategoryDesc(
							pcmlDoc.getValue(pcml + ".@RSPDTA.DETDTA.LmtCtgDesc", indices).toString());
					item.setLimitCurrency(pcmlDoc.getValue(pcml + ".@RSPDTA.DETDTA.LmtCurrency", indices).toString());

					strDate = StringUtils
							.trimToEmpty(pcmlDoc.getValue(pcml + ".@RSPDTA.DETDTA.LmtExpiry", indices).toString());
					item.setLimitExpiry(DateUtility.convertDateFromAS400(new BigDecimal(strDate)));

					item.setLimitAmount(
							new BigDecimal(pcmlDoc.getValue(pcml + ".@RSPDTA.DETDTA.LmtAmount", indices).toString()));
					item.setAvailAmount(
							new BigDecimal(pcmlDoc.getValue(pcml + ".@RSPDTA.DETDTA.AvailAmount", indices).toString()));
					item.setRiskAmount(
							new BigDecimal(pcmlDoc.getValue(pcml + ".@RSPDTA.DETDTA.RiskAmount", indices).toString()));
					/*
	...
	@Override
	public CustomerLimitUtilization doReserveUtilization(CustomerLimitUtilization limitUtilReq)
			throws InterfaceException {
		// TODO Auto-generated method stub
		return null;
	}
	...
	@Override
	public CustomerLimitUtilization doOverrideAndReserveUtil(CustomerLimitUtilization limitUtilReq)
			throws InterfaceException {
		// TODO Auto-generated method stub
		return null;
	}
	...
	@Override
	public CustomerLimitUtilization doConfirmReservation(CustomerLimitUtilization limitUtilReq)
			throws InterfaceException {
		// TODO Auto-generated method stub
		return null;
	}
	...
	@Override
	public CustomerLimitUtilization doCancelReservation(CustomerLimitUtilization limitUtilReq)
			throws InterfaceException {
		// TODO Auto-generated method stub
		return null;
	}
	...
	@Override
	public CustomerLimitUtilization doCancelUtilization(CustomerLimitUtilization limitUtilReq)
			throws InterfaceException {
		// TODO Auto-generated method stub
		return null;
	}
