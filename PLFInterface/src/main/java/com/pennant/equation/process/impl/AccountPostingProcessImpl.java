...
public class AccountPostingProcessImpl extends GenericProcess implements AccountPostingProcess {
	...
	public List<CoreBankAccountPosting> doPostings(List<CoreBankAccountPosting> accountPostings, String postBranch,
		...
		List<CoreBankAccountPosting> bankAccountPostings = new ArrayList<CoreBankAccountPosting>(
				accountPostings.size());
		...
		try {
			...
			pcmlDoc.setValue(pcml + ".@REQDTA.LinkTranId",
					StringUtils.rightPad(StringUtils.trimToEmpty(accountPostings.get(0).getLinkedTranId()), 10)); //Linked Transaction Id
			pcmlDoc.setValue(pcml + ".@REQDTA.ValueDate",
					DateUtility.formatDate(accountPostings.get(0).getValueDate(), "ddMMyyyy")); //Value Date
			pcmlDoc.setValue(pcml + ".@REQDTA.FinType",
					StringUtils.rightPad(StringUtils.trimToEmpty(accountPostings.get(0).getFinType()), 8)); //Finance Type
			...
			for (indices[0] = 0; indices[0] < accountPostings.size(); indices[0]++) {
				...
				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.AcType", indices,
						StringUtils.rightPad(posting.getAcType(), 8));// Account Type
				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.Branch", indices,
						StringUtils.trimToEmpty(posting.getAcBranch())); // Account Branch
				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.AccountNumber", indices,
						StringUtils.rightPad(StringUtils.trimToEmpty(posting.getAccount()), 13));// Account Number
				...
				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.TranCode", indices,
						StringUtils.rightPad(StringUtils.trimToEmpty(posting.getTranCode()), 8)); //Transaction Code
				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.RevTranCode", indices,
						StringUtils.rightPad(StringUtils.trimToEmpty(posting.getRevTranCode()), 8));//Rev Transaction Code
				...
				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.FinReference", indices,
						accountPostings.get(indices[0]).getFinReference()); //Finance Reference
			}
			...
			for (indices[0] = 0; indices[0] < dsRspCount; indices[0]++) {
				...
				account.setValueDate(
						DateUtility.getUtilDate(
								StringUtils.leftPad(StringUtils
										.trimToEmpty(pcmlDoc.getValue(pcml + ".@RSPDTA.ValueDate").toString()), 8, "0"),
								"ddMMyyyy")); //Value Date
				...
				account.setPostAmount(
						new BigDecimal(pcmlDoc.getValue(pcml + ".@RSPDTA.DETDTA.Amount", indices).toString())); //Post Amount
