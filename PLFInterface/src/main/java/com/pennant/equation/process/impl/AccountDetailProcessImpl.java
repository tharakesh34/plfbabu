...
public class AccountDetailProcessImpl extends GenericProcess implements AccountDetailProcess{
	...
	public List<CoreBankAccountDetail> fetchAccount(List<CoreBankAccountDetail> bankAccountDetails,
		...
		try {
			...
			logger.debug(pcmlDoc.getValue(pcml + ".@ERCOD").toString());
			logger.debug(pcmlDoc.getValue(pcml + ".@ERPRM").toString());
			...
			if (!("0000".equals(pcmlDoc.getValue(pcml + ".@ERCOD").toString()) || "".equals(pcmlDoc.getValue(pcml + ".@ERCOD").toString()))) {	
				logger.warn(pcmlDoc.getValue(pcml + ".@ERPRM").toString());				
			}
	 ...
	 */
	@Override
	public List<CoreBankAccountDetail> fetchAccountsListAvailableBal(List<CoreBankAccountDetail> coreAcctList, boolean isCcyCheck) throws PFFInterfaceException {
		logger.debug("Entering");
	...
	public List<CoreBankAccountDetail> fetchCustomerAccounts(CoreBankAccountDetail coreAcct) throws PFFInterfaceException {
		...
		try {
			...
			pcmlDoc.setValue(pcml + ".@REQDTA.DSREQACTYPES", coreAcct.getAcType().replaceAll(",", "")); 	// Account Types
