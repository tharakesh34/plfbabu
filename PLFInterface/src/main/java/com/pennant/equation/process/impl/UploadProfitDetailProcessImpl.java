	...
	public void doUploadPftDetails(List<EodFinProfitDetail> profitDetails, boolean isItFirstCall)
		...
		try {
			...
			for (indices[0] = 0; indices[0] < profitDetails.size(); indices[0]++) {
				...
				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.FinPurpose", indices,
						StringUtils.trimToEmpty(pftDetails.getFinPurpose()));
				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.FinContractDate", indices,
						DateUtility.formatDate(pftDetails.getFinContractDate(), "ddMMyyyy"));
				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.FinApprovedDate", indices,
						DateUtility.formatDate(pftDetails.getFinApprovedDate(), "ddMMyyyy"));
				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.FinStartDate", indices,
						DateUtility.formatDate(pftDetails.getFinStartDate(), "ddMMyyyy"));
				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.MaturityDate", indices,
						DateUtility.formatDate(pftDetails.getMaturityDate(), "ddMMyyyy"));
				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.FullPaidDate", indices,
						DateUtility.formatDate(pftDetails.getFullPaidDate(), "ddMMyyyy"));
				...
				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.PftAccrueTsfd", indices,
						pftDetails.getPftAccrueTsfd() == null ? BigDecimal.ZERO : pftDetails.getPftAccrueTsfd());
				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.FinStatus", indices,
						StringUtils.trimToEmpty(pftDetails.getFinStatus()));
				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.FinStsReason", indices,
						StringUtils.trimToEmpty(pftDetails.getFinStsReason()));
				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.FinWorstStatus", indices,
						StringUtils.trimToEmpty(pftDetails.getFinWorstStatus()));
				...
				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.FinAccount", indices,
						StringUtils.trimToEmpty(pftDetails.getFinAccount()));
				...
				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.FinCustPftAccount", indices,
						StringUtils.trimToEmpty(pftDetails.getFinCustPftAccount()));
				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.IncomeAccount", indices, pftDetails.getIncomeAccount());
				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.UEIncomeSuspAccount", indices,
						pftDetails.getUEIncomeSuspAccount());
				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.FinCommitmentRef", indices,
						StringUtils.trimToEmpty(pftDetails.getFinCommitmentRef()));
				...
				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.FirstRepay", indices,
						DateUtility.formatDate(pftDetails.getFirstRepayDate(), "ddMMyyyy"));
				...
				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.ClosingSts", indices,
						StringUtils.trimToEmpty(pftDetails.getClosingStatus()));
				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.ProductCtg", indices,
						StringUtils.trimToEmpty(pftDetails.getFinCategory()));
				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.PrvRepayDate", indices, pftDetails.getLastRpySchDate() != null
						? DateUtility.formatDate(pftDetails.getLastRpySchDate(), "ddMMyyyy") : 0);
				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.NextRepayDate", indices, pftDetails.getNextRpySchDate() != null
						? DateUtility.formatDate(pftDetails.getNextRpySchDate(), "ddMMyyyy") : 0);
				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.PreviousRepayPri", indices,
						pftDetails.getLastRpySchPri() != null ? pftDetails.getLastRpySchPri() : 0);
				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.PreviousRepayPft", indices,
						pftDetails.getLastRpySchPft() != null ? pftDetails.getLastRpySchPft() : 0);

				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.LtstPayDate", indices, pftDetails.getLatestRpyDate() != null
						? DateUtility.formatDate(pftDetails.getLatestRpyDate(), "ddMMyyyy") : 0);
				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.LtstPriPay", indices,
						pftDetails.getLatestRpyPri() != null ? pftDetails.getLatestRpyPri() : 0);
				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.LtstPftPay", indices,
						pftDetails.getLatestRpyPft() != null ? pftDetails.getLatestRpyPft() : 0);
				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.WODate", indices, pftDetails.getLatestWriteOffDate() != null
						? DateUtility.formatDate(pftDetails.getLatestWriteOffDate(), "ddMMyyyy") : 0);
				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.WOAmt", indices,
						pftDetails.getTotalWriteoff() != null ? pftDetails.getTotalWriteoff() : 0);

				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.SalariedCustomer", indices,
						pftDetails.isSalariedCustomer() ? "Y" : "N");

				if (pftDetails.getFirstODDate() != null) {
					pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.FirstODDate", indices,
							DateUtility.formatDate(pftDetails.getFirstODDate(), "ddMMyyyy"));
				} else {
					pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.FirstODDate", indices, "0");
				}

				if (pftDetails.getLastODDate() != null) {
					pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.LastODDate", indices,
							DateUtility.formatDate(pftDetails.getLastODDate(), "ddMMyyyy"));
				} else {
					pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.LastODDate", indices, "0");
				}
				if (pftDetails.getCRBFirstODDate() != null) {
					pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.CRBFirstODDate", indices,
							DateUtility.formatDate(pftDetails.getCRBFirstODDate(), "ddMMyyyy"));
				} else {
					pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.CRBFirstODDate", indices, "0");
				}

				if (pftDetails.getCRBLastODDate() != null) {
					pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.CRBLastODDate", indices,
							DateUtility.formatDate(pftDetails.getCRBLastODDate(), "ddMMyyyy"));
				} else {
				...
				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.CRBODPrincipal", indices,
						pftDetails.getCRBODPrincipal() == null ? BigDecimal.ZERO : pftDetails.getCRBODPrincipal());
				pcmlDoc.setValue(pcml + ".@REQDTA.DEFDTA.CRBODProfit", indices,
						pftDetails.getCRBODProfit() == null ? BigDecimal.ZERO : pftDetails.getCRBODProfit());
