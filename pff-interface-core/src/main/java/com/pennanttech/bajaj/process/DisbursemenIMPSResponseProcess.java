...
public class DisbursemenIMPSResponseProcess extends DatabaseDataEngine {
	...
	private int updateDisbursements(List<Long> disbIdList, long batchId) {
		...
		try {
			sql.append(
					"  UPDATE DISBURSEMENT_REQUESTS DR SET (DR.TRANSACTIONREF, DR.STATUS, DR.REMARKS, DR.REJECT_REASON, RESP_BATCH_ID) =");
			sql.append(
					" (SELECT IDR.TRANSACTIONID, CASE WHEN IDR.STATUS = 0 THEN :E ELSE :R END, IDR.DESCRIPTION, IDR.DESCRIPTION, :RESP_BATCH_ID ");
			sql.append(
					" FROM INT_DSBIMPS_RESPONSE IDR WHERE IDR.CHANNELPARTNERREFNO = DR.ID AND IDR.CHANNELPARTNERREFNO IN (:RESP_IDS))");
			sql.append(
					" WHERE EXISTS(SELECT IDR.TRANSACTIONID, CASE WHEN IDR.STATUS = 0 THEN :E ELSE :R END, IDR.DESCRIPTION,");
			sql.append(
					" :RESP_BATCH_ID FROM INT_DSBIMPS_RESPONSE IDR WHERE IDR.CHANNELPARTNERREFNO = DR.ID AND IDR.CHANNELPARTNERREFNO IN (:RESP_IDS))");
