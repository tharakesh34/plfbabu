...
public class ExtendedMapFromPropertiesTest {
	...
	public void getExtendedMapObjectFromJson() throws IOException {
		...
		try {
			...
			Map<String, Object> responseMapvalues = objectMapper.readValue(jsonString,
			...
			for (final Entry<Object, Object> entry : properties.entrySet()) {
				/*
				 * NameValuePair valuePair = new NameValuePair(); valuePair.setName(entry.getKey());
	...
	public void getExtendedMapObjectFromReflection() throws IOException {
		/*
		 * ObjectMapper objectMapper = new ObjectMapper(); try { String jsonString = getJSONResponse();
		 * assertNotNull(jsonString);
		 * 
		 * Map<String, Object> responseMapvalues = objectMapper.readValue(jsonString, new TypeReference<Map<String,
		 * Object>>() {}); assertNotNull(responseMapvalues, "responseMap should not be null");
		 * 
		 * Map<String, Object> extendedMap = new HashMap<>(1); Map<String, Object> keyValues = new HashMap<>(1);
		 * 
		 * List<NameValuePair> keyValues1 = new ArrayList<>(1);
		 * 
		 * Properties properties = new Properties(); InputStream inputStream =
		 * this.getClass().getResourceAsStream("/experianDedup.properties"); properties.load(inputStream);
		 * 
		 * for (final Entry<Object, Object> entry : properties.entrySet()) { NameValuePair valuePair = new
		 * NameValuePair(); keyValues.put(String.valueOf(entry.getKey()), entry.getValue()); }
		 * 
		 * keyValues1.stream().map(p -> { keyValues1.remove(p); return p; });
		 * 
		 * 
		 * for (int i = 0; i < keyValues.size(); i++) { NameValuePair pair = keyValues.get(i);
		 * if(responseMapvalues.get(pair.getValue()) instanceof Map) { if(responseMapvalues.get(pair.getValue()) !=
		 * null) {
		 * 
		 * @SuppressWarnings("unchecked") Map<String, Object> resultMap = (Map<String, Object>)
		 * responseMapvalues.get(pair.getValue()); if(responseMapvalues.containsKey(pair.getValue())) {
		 * extendedMap.put(pair.getName(), resultMap.get(pair.getValue())); keyValues.remove(i); i--; }
		 * doSetExtendedMapObject(extendedMap, resultMap, keyValues); } } else if(responseMapvalues.get(pair.getValue())
		 * instanceof List) { if(responseMapvalues.containsKey(pair.getValue())) { extendedMap.put(pair.getName(),
		 * responseMapvalues.get(pair.getValue())); keyValues.remove(i); i--; } } else {
		 * if(responseMapvalues.containsKey(pair.getValue())) { extendedMap.put(pair.getName(),
		 * responseMapvalues.get(pair.getValue())); keyValues.remove(i); i--; } } } for(String key:extendedMap.keySet())
		 * { System.out.println("Key Value:"+key +" And Value is:"+extendedMap.get(key)); } responseMapvalues = null;
		 * extendedMap = null; keyValues = null; inputStream = null; } catch (IOException e) { e.printStackTrace(); }
		 */}
	...
	private void doSetExtendedMapObject(Map<String, Object> extendedMap, Map<String, Object> resultMap,
			List<NameValuePair> keyValues) {
		for (int i = 0; i < keyValues.size(); i++) {
