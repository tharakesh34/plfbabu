...
public class ShowReturnedCheques extends Window implements Serializable {
	...
	private static final Logger logger = Logger.getLogger(ShowReturnedCheques.class);
	 ...
	 */
	public static Object show(Component parent, List<ReturnedCheques> returnChequeList,
			String dedupFields, ReturnedCheques returnedCheque) {
		
		return new ShowReturnedCheques(parent, returnChequeList, dedupFields,returnedCheque);
	}


	public ShowReturnedCheques(Component parent, List<ReturnedCheques> dedupList,
			String dedupFields, ReturnedCheques returnedCheque) {
		super();
		...
		for(int i=0;i<dedupListSize.size();i++){
			if(date.before(dedupListSize.get(i).getReturnDate()) || date.equals(dedupListSize.get(i).getReturnDate())){
				count++;
			}
	...
	private void createBox(ReturnedCheques returnedCheque) {
			...
			rows.appendChild(prepareRow(new Row(),
					Labels.getLabel("label_ReturnedCheque_NoOfCheques.value"),returnedCheque.getCount(),
					Labels.getLabel("label_ReturnedCheque_ReturnedCheque.value"),returnedCheque.getCounting()));
			...
			for (int j = 0; j < fieldString.length; j++) {
				...
				if (data.getClass().getMethod(fieldMethod).getReturnType()
					...
					lc = new Listcell(fieldValue);
						...
						.getReturnType().equals(Date.class)) {
					...
					if(currency!=null){
						lc = new Listcell(PennantAppUtil.amountFormate(
								decfieldValue,currency.getCcyEditField()));
	...
	private Row prepareRow(Row row, String label1, int value1,
