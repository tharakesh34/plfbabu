...
public class FinApprovalStsInquiryDialogCtrl extends GFCBaseCtrl<FinanceMain> {
	...
	public void onCreate$window_FinApprovalStsInquiryDialog(ForwardEvent event) throws Exception {
		...
		try {
			if (arguments.containsKey("customerFinanceDetail")) {
				this.customerFinanceDetail = (CustomerFinanceDetail) arguments
						.get("customerFinanceDetail");
			} else {
	...
	private String fetchFinCurrentUser(String finReference){
		...
		try {
			...
			jdbcSearchObject.addFilters(new Filter[]{new Filter("PROCESSED", 0, Filter.OP_EQUAL),new Filter("REFERENCE",finReference, Filter.OP_EQUAL)});
			List<TaskOwners> finTaskOwnerList = getPagedListWrapper().getPagedListService().getBySearchObject(jdbcSearchObject);
