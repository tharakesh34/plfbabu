...
public class FeePaymentDetailDialogCtrl extends GFCBaseCtrl<FeePaymentDetail> {
	 ...
	 * zul-file is called with a parameter for a selected FeePaymentDetailDetail object in a
	 * Map.
	 * 
	 * @param event
	 * @throws Exception
	...
	public void onCreate$window_FeePaymentDetailDialog(Event event) throws Exception {
		...
		try {
			...
			doLoadWorkFlow(this.feePaymentDetail.isWorkflow(),this.feePaymentDetail.getWorkflowId(),this.feePaymentDetail.getNextTaskId());
	...
	public void doWriteBeanToComponents(FeePaymentDetail aFinAdvnancePayments) {
		...
		fillComboBox(this.paymentMethod, aFinAdvnancePayments.getPaymentMethod(), PennantStaticListUtil.getRemFeeSchdMethods(), "");
		this.paymentAmount.setValue(PennantApplicationUtil.formateAmount(aFinAdvnancePayments.getPaymentAmount(),ccyFormatter));
	...
	public void doWriteComponentsToBean(FeePaymentDetail aFeePaymentDetail) throws InterruptedException {
		...
		try {
			aFeePaymentDetail.setPaymentAmount(PennantAppUtil.unFormateAmount(
					this.paymentAmount.isReadonly() ? this.paymentAmount.getActualValue() : this.paymentAmount
	...
	private void doSetValidation() {
		...
		if(!this.paymentMethod.isDisabled()){
			this.paymentMethod.setConstraint(new PTStringValidator(Labels.getLabel("label_FeePaymentDetailDialog_PaymentMethod.value"), null, true));
		}
		if (!this.paymentAmount.isDisabled()) {
			this.paymentAmount.setConstraint(new PTDecimalValidator(Labels.getLabel("label_FeePaymentDetailDialog_PaymentAmount.value"), ccyFormatter, true, false));
		}
		if (!this.valueDate.isDisabled()){
			this.valueDate.setConstraint(new PTDateValidator(Labels.getLabel("label_FeePaymentDetailDialog_ValueDate.value"),true));
		}
		...
		if (MessageUtil.confirm(msg) == MessageUtil.YES) {
			if (StringUtils.isBlank(aFeePaymentDetail.getRecordType())){
				...
				if (isWorkFlowEnabled()){
					...
					getWorkFlowDetails(userAction.getSelectedItem().getLabel(), aFeePaymentDetail.getNextTaskId(), aFeePaymentDetail);
				}else{
				...
				if(StringUtils.equals(afeePaymentDetail.getPaymentReference(), loanDetail.getPaymentReference())){ // Both Current and Existing list rating same
					...
					if(PennantConstants.TRAN_DEL.equals(tranType)){
						if(PennantConstants.RECORD_TYPE_UPD.equals(afeePaymentDetail.getRecordType())){
							...
							listPaymentsApproved = getFinFeeDetailListCtrl().getFinanceDetail().getFeePaymentDetailList();
							for (int j = 0; j < listPaymentsApproved.size(); j++) {
								...
								if (detail.getFinReference() == afeePaymentDetail.getFinReference() && 
										StringUtils.equals(detail.getPaymentReference(), afeePaymentDetail.getPaymentReference())) {
	...
	private AuditHeader getAuditHeader(FeePaymentDetail aFeePaymentDetail, String tranType){
		...
		return new AuditHeader(aFeePaymentDetail.getFinReference(),null,null,null,auditDetail,aFeePaymentDetail.getUserDetails(),getOverideMap());
	}
