...
public class CostOfFundCodeServiceaImpl extends GenericService<CostOfFundCode> implements CostOfFundCodeService {
	 ...
	 * RMTCostOfFundCodes/RMTCostOfFundCodes_Temp by using CostOfFundCodeDAO's save
	 * method b) Update the Record in the table. based on the module workFlow
	 * Configuration. by using CostOfFundCodeDAO's update method 3) Audit the
	 * record in to AuditHeader and AdtRMTCostOfFundCodes by using
	 ...
	 * table RMTCostOfFundCodes by using CostOfFundCodeDAO's delete method with type
	 * as Blank 3) Audit the record in to AuditHeader and AdtRMTCostOfFundCodes by
	...
	/**
	 * getCostOfFundCodeById fetch the details by using CostOfFundCodeDAO's
	 * getCostOfFundCodeById method.
	 * 
	 * @param id
	 *            (String)
	 * @param type
	...
	/**
	 * getApprovedCostOfFundCodeById fetch the details by using CostOfFundCodeDAO's
	 * getCostOfFundCodeById method . with parameter id and type as blank. it
	 * fetches the approved records from the RMTCostOfFundCodes.
	 * 
	 * @param id
	 *            (String)
	 * @return CostOfFundCode
	 ...
	 * using getCostOfFundCodeDAO().delete with parameters costOfFundCode,"" b) NEW
	 * Add new record in to main table by using getCostOfFundCodeDAO().save with
	 * parameters costOfFundCode,"" c) EDIT Update record in the main table by
	 * using getCostOfFundCodeDAO().update with parameters costOfFundCode,"" 3)
	 * Delete the record from the workFlow table by using
	 * getCostOfFundCodeDAO().delete with parameters costOfFundCode,"_Temp" 4) Audit
	 * the record in to AuditHeader and AdtRMTCostOfFundCodes by using
	 * auditHeaderDAO.addAudit(auditHeader) for Work flow 5) Audit the record in
	 * to AuditHeader and AdtRMTCostOfFundCodes by using
	...
	public AuditHeader doApprove(AuditHeader auditHeader) {
		...
		if (!PennantConstants.RECORD_TYPE_NEW.equals(costOfFundCode.getRecordType())) {
			auditHeader.getAuditDetail().setBefImage(costOfFundCodeDAO.getCostOfFundCodeById(costOfFundCode.getCofCode(), ""));
		}
	 ...
	 * the workFlow table by using getCostOfFundCodeDAO().delete with parameters
	 * costOfFundCode,"_Temp" 3) Audit the record in to AuditHeader and
	 * AdtRMTCostOfFundCodes by using auditHeaderDAO.addAudit(auditHeader) for
	 * Work flow
	 * 
	 * @param AuditHeader
	 *            (auditHeader)
	 ...
	 * getCostOfFundCodeDAO().getErrorDetail with Error ID and language as parameters.
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage){
		...
		if (costOfFundCode != null && costOfFundCode.isNew()
				...
				&& costOfFundCodeDAO.isDuplicateKey(costOfFundCode.getCofCode(), costOfFundCode.isWorkflow() ? TableType.BOTH_TAB : TableType.MAIN_TAB)) {
