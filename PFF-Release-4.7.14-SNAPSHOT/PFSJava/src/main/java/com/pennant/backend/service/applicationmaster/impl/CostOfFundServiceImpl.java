...
public class CostOfFundServiceImpl extends GenericService<CostOfFund> implements CostOfFundService {
	 ...
	 * RMTCostOfFunds/RMTCostOfFunds_Temp by using CostOfFundDAO's save method b)
	 * Update the Record in the table. based on the module workFlow
	 * Configuration. by using CostOfFundDAO's update method 3) Audit the record
	 * in to AuditHeader and AdtRMTCostOfFunds by using
	 ...
	 * table RMTCostOfFunds by using CostOfFundDAO's delete method with type as
	 * Blank 3) Audit the record in to AuditHeader and AdtRMTCostOfFunds by using
	 * auditHeaderDAO.addAudit(auditHeader)
	 * 
	 * @param AuditHeader
	 *            (auditHeader)
	...
	/**
	 * getCostOfFundById fetch the details by using CostOfFundDAO's getCostOfFundById
	 * method.
	 * 
	 * @param id
	 *            (String)
	...
	/**
	 * getApprovedCostOfFundById fetch the details by using CostOfFundDAO's
	 * getCostOfFundById method . with parameter id and type as blank. it fetches
	 * the approved records from the RMTCostOfFunds.
	 * 
	 * @param id
	 *            (String)
	 * @return CostOfFund
	...
	/**
	 * getCostOfFundDelById fetch the details by using CostOfFundDAO's getCostOfFundDelById
	 * method.
	 * 
	 * @param id
	 *            (String)
	 ...
	 * costOfFund,"" c) EDIT Update record in the main table by using getCostOfFundDAO().update with parameters costOfFund,""
	 * 3) Delete the record from the workFlow table by using getCostOfFundDAO().delete with parameters costOfFund,"_Temp" 4)
	 * Audit the record in to AuditHeader and AdtRMTCostOfFunds by using auditHeaderDAO.addAudit(auditHeader) for Work
	 * flow 5) Audit the record in to AuditHeader and AdtRMTCostOfFunds by using auditHeaderDAO.addAudit(auditHeader)
	...
	public AuditHeader doApprove(AuditHeader auditHeader) {
		...
		if (!PennantConstants.RECORD_TYPE_NEW.equals(costOfFund.getRecordType())) {
			auditHeader.getAuditDetail().setBefImage(costOfFundDAO.getCostOfFundById(costOfFund.getCofCode(), costOfFund.getCurrency(), costOfFund.getCofEffDate(), ""));
		}
	 ...
	 * the workFlow table by using getCostOfFundDAO().delete with parameters
	 * costOfFund,"_Temp" 3) Audit the record in to AuditHeader and
	 * AdtRMTCostOfFunds by using auditHeaderDAO.addAudit(auditHeader) for Work
	 * flow
	 * 
	 * @param AuditHeader
	 *            (auditHeader)
	 ...
	 * getCostOfFundDAO().getErrorDetail with Error ID and language as parameters.
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage){
		...
		if (costOfFund != null && costOfFund.isNew()
				&& PennantConstants.RECORD_TYPE_NEW.equals(costOfFund.getRecordType())
				&& costOfFundDAO.isDuplicateKey(costOfFund.getCofCode(),costOfFund.getCofEffDate(),costOfFund.getCurrency(), costOfFund.isWorkflow() ? TableType.BOTH_TAB : TableType.MAIN_TAB)) {
