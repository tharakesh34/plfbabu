...
public class DSRCalculationReportData implements Serializable{
	...
	public DSRCalculationReportData getDSRCalculationReportData(FinanceDetail detail){
		...
		reportData.setDownPay(PennantApplicationUtil.amountFormate(financeMain.getDownPayBank().add(financeMain.getDownPaySupl()), format));
		...
		reportData.setTotalFinAmount(PennantApplicationUtil.amountFormate(financeMain.getFinAmount().add(
				...
				subtract(financeMain.getDownPayment()),format));
		reportData.setTotalGrcPft(PennantApplicationUtil.amountFormate(financeMain.getTotalGracePft(),format));
		reportData.setTotalRepayPft(PennantApplicationUtil.amountFormate(financeMain.getTotalProfit().subtract(financeMain.getTotalGracePft()),format));
		...
		if(financeMain.getRepayRateBasis().equals(CalculationConstants.RATE_BASIS_R)){
			...
			BigDecimal totalFinAmt = financeMain.getFinAmount().add(financeMain.getFeeChargeAmt() == null ? BigDecimal.ZERO : 
				financeMain.getFeeChargeAmt()).subtract(financeMain.getDownPayment()).add(financeMain.getInsuranceAmt() == null ? BigDecimal.ZERO : 
		...
		if(feeRules != null && !feeRules.isEmpty()){
			for (FeeRule feeRule : feeRules) {
	            ...
	            fee.setFeeAmount(PennantApplicationUtil.amountFormate(feeRule.getFeeAmount().subtract(
	            		feeRule.getWaiverAmount()).subtract(feeRule.getPaidAmount()), format));
