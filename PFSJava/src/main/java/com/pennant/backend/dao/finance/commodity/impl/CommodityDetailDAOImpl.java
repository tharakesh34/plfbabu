...
public class CommodityDetailDAOImpl extends BasicDao<CommodityDetail> implements CommodityDetailDAO {
	private static Logger logger = LogManager.getLogger(CommodityDetailDAOImpl.class);
	...
	public CommodityDetail getCommodityDetailById(final String id, String commodityUnitCode, String type) {
		...
		RowMapper<CommodityDetail> typeRowMapper = ParameterizedBeanPropertyRowMapper
				.newInstance(CommodityDetail.class);
	...
	/**
	 * This method Deletes the Record from the FCMTCommodityDetail or FCMTCommodityDetail_Temp. if Record not deleted
	 * then throws DataAccessException with error 41003. delete Commodity Details by key CommodityCode
	 * 
	 * @param commodityDetail
	 *            (CommodityDetail)
	 * @param type
	...
	/**
	 * This method updates the Record FCMTCommodityDetail or FCMTCommodityDetail_Temp. if Record not updated then throws
	 * DataAccessException with error 41004. update Commodity Details by key CommodityCode and Version
	 * 
	 * @param commodityDetail
	 *            (CommodityDetail)
	 * @param type
	...
	public boolean getBrokerCommodityDetails(String code) {
		...
		logger.debug("Leaving");
		List<BrokerCommodityDetail> commList = this.jdbcTemplate.query(selectSql.toString(), beanParameters,
				typeRowMapper);
		if (commList.size() > 0) {
			return true;
		} else {
