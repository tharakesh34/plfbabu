...
public class LimitGroupDAOImpl extends BasisCodeDAO<LimitGroup> implements LimitGroupDAO {
	...
	public void delete(LimitGroup limitGroup,String type) {
		...
		try {
			...
			if (recordCount <= 0) {
				ErrorDetails errorDetails = getError("41003",limitGroup.getId(), limitGroup.getUserDetails().getUsrLanguage());
				throw new DataAccessException(errorDetails.getError()) {};
			}
		} catch (DataAccessException e) {
			logger.error("Exception: ", e);
			ErrorDetails errorDetails = getError("41006",limitGroup.getId(), limitGroup.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {};
		}
		logger.debug("Leaving");
	}
	...
	public void update(LimitGroup limitGroup,String type) {
		...
		if (recordCount <= 0) {
			...
			ErrorDetails errorDetails = getError("41004", limitGroup.getId() , limitGroup.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {};
		}
		logger.debug("Leaving");
	}
