...
public class BeneficiaryDAOImpl extends BasisNextidDaoImpl<Beneficiary> implements BeneficiaryDAO {
	...
	public void delete(Beneficiary beneficiary, String type) {
		...
		try {
			...
			if (recordCount <= 0) {
				ErrorDetails errorDetails = getError("41003", beneficiary.getBeneficiaryId(), beneficiary
			...
			ErrorDetails errorDetails = getError("41006", beneficiary.getBeneficiaryId(), beneficiary.getUserDetails()
					.getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {
			};
		}
	...
	public void update(Beneficiary beneficiary, String type) {
		...
		if (recordCount <= 0) {
			...
			ErrorDetails errorDetails = getError("41004", beneficiary.getBeneficiaryId(), beneficiary.getUserDetails()
					.getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {
			};
		}
	...
	private ErrorDetails getError(String errorId, long beneficiaryId, String userLanguage) {
		String[][] parms = new String[2][1];
		parms[1][0] = String.valueOf(beneficiaryId);
		parms[0][0] = PennantJavaUtil.getLabel("label_BeneficiaryId") + ":" + parms[1][0];
