...
public class CollateralStructureDAOImpl extends BasisCodeDAO<CollateralStructure> implements CollateralStructureDAO {
	...
	public void delete(CollateralStructure collateralStructure, String type) {
		...
		try {
			if (this.jdbcTemplate.update(sql.toString(), beanParameters) <= 0) {
				ErrorDetails errorDetails = getError("41003", collateralStructure.getId(), collateralStructure .getUserDetails().getUsrLanguage());
				throw new DataAccessException(errorDetails.getError()) {
				};
			}
		} catch (DataAccessException e) {
			logger.error(e);
			ErrorDetails errorDetails = getError("41006", collateralStructure.getId(), collateralStructure
	...
	public void update(CollateralStructure collateralStructure, String type) {
		...
		if (recordCount <= 0) {
			...
			ErrorDetails errorDetails = getError("41004", collateralStructure.getId(), collateralStructure.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {
			};
		}
		logger.debug("Leaving");
	...
	private ErrorDetails getError(String errorId, String collateralType, String userLanguage) {
		String[][] parms = new String[2][1];
		parms[1][0] = collateralType;
		parms[0][0] = PennantJavaUtil.getLabel("label_CollateralType") + ":" 	+ parms[1][0];
