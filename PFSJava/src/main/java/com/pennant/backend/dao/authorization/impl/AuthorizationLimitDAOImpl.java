 ...
 * FILE HEADER *
 ********************************************************************************************
 * * FileName : AuthorizationLimitDAOImpl.java * * Author : PENNANT TECHONOLOGIES * * Creation Date : 06-04-2018 * *
...
public class AuthorizationLimitDAOImpl extends SequenceDao<AuthorizationLimit> implements AuthorizationLimitDAO {
	...
	@Override
	public boolean isDuplicateKey(long id, int limitType, long userID, long roleId, String module,
			TableType tableType) {
		logger.debug(Literal.ENTERING);
	...
	public String save(AuthorizationLimit authorizationLimit, TableType tableType) {
		...
		if (authorizationLimit.getId() == Long.MIN_VALUE) {
			authorizationLimit.setId(getNextValue("SeqAuth_Limits"));
