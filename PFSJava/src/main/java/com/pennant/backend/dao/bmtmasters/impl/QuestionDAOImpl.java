...
public class QuestionDAOImpl extends BasisNextidDaoImpl<Question> implements QuestionDAO {
	...
	public void delete(Question question,String type) {
		...
		try{
			...
			if (recordCount <= 0) {
				ErrorDetails errorDetails= getError("41003",question.getId() ,question.getUserDetails().getUsrLanguage());
				throw new DataAccessException(errorDetails.getError()) {};
			}
		}catch(DataAccessException e){
			logger.error("Exception: ", e);
			ErrorDetails errorDetails= getError("41006",question.getId() ,question.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {};
		}
		logger.debug("Leaving");
	}
	...
	public void update(Question question,String type) {
		...
		updateSql.append(" Set QuestionId = :QuestionId, QuestionDesc = :QuestionDesc, AnswerA = :AnswerA, AnswerB = :AnswerB, AnswerC = :AnswerC, AnswerD = :AnswerD, CorrectAnswer = :CorrectAnswer, QuestionIsActive = :QuestionIsActive");
		...
		if (recordCount <= 0) {
			...
			ErrorDetails errorDetails= getError("41004",question.getId() ,question.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {};
		}
		logger.debug("Leaving");
	}
	
	private ErrorDetails  getError(String errorId, long questionId, String userLanguage){
		String[][] parms= new String[2][1];
		parms[1][0] = String.valueOf(questionId);
		parms[0][0] = PennantJavaUtil.getLabel("label_QuestionId")+ ":" + parms[1][0];
