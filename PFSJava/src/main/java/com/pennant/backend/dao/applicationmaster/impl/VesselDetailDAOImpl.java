...
public class VesselDetailDAOImpl extends BasicDao<VesselDetail> implements VesselDetailDAO {
	private static Logger logger = LogManager.getLogger(VesselDetailDAOImpl.class);
	...
	public VesselDetail getVesselDetailById(final String id, String type) {
		...
		RowMapper<VesselDetail> typeRowMapper = BeanPropertyRowMapper.newInstance(VesselDetail.class);
	...
	public VesselDetail getVesselDetailByType(VesselDetail vesselDetail, String type) {
		...
		selectSql.append(StringUtils.trimToEmpty(type));
		selectSql.append(
				" Where VesselTypeID != :VesselTypeID AND VesselType = :VesselType AND VesselSubType = :VesselSubType");
	...
	/**
	 * This method Deletes the Record from the VesselDetails or VesselDetails_Temp. if Record not deleted then throws
	 * DataAccessException with error 41003. delete Vessel Details by key VesselTypeID
	 * 
	 * @param Vessel
	...
	/**
	 * This method insert new Records into VesselDetails or VesselDetails_Temp. it fetches the available Sequence form
	 * SeqVesselDetails by using getNextidviewDAO().getNextId() method.
	 *
	 * save Vessel Details
	 * 
	 * @param Vessel
	...
	/**
	 * This method updates the Record VesselDetails or VesselDetails_Temp. if Record not updated then throws
	 * DataAccessException with error 41004. update Vessel Details by key VesselTypeID and Version
	 * 
	 * @param Vessel
	 *            Details (vesselDetail)
	 * @param type
