...
public class ProjectedFeeRefundProcessDAOImpl extends BasicDao<FeeRefundProcessQueuing>
	...
	@Override
	public int prepareFeeRefundProcessQueue(long feeRefundId) {
		...
		frpQueuing.setFeeRefundHeaderId(feeRefundId);
		...
		sql.append(
				" SELECT Reference,FeeRefundId,JSONOBJECT, :EodDate, :ThreadId, :Progress, :StartTime, :EodProcess From FEEREFUNDHEADER T WHERE T.FEEREFUNDID = :FeeRefundId");
		sql.append(" AND T.STATUS != 'Approved'  order by T.Reference,T.FeeRefundId ");
	...
	public int[] prepareFeeRefundProcessQueue(List<FeeRefundProcessQueuing> frdQueuings) {
		...
		sql.append(" SELECT FINREFERENCE, FeeRefundId");
		sql.append(", :EodDate, :ThreadId, :Progress, :StartTime, :EodProcess");
		sql.append(" From FeeRefundHeader_view T WHERE T.FeeRefundId = :FeeRefundHeaderId");
		sql.append(" AND T.APPROVALSTATUS in ('2', '3')  order by T.FinReference,T.FeeRefundId ");
