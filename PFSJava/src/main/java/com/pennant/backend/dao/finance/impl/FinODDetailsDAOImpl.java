...
public class FinODDetailsDAOImpl extends BasicDao<FinODDetails> implements FinODDetailsDAO {
	...
	public void update(FinODDetails finOdDetails) {
		logger.debug("Entering");
		StringBuilder updateSql = updateFODQuery();
	...
	private StringBuilder updateFODQuery() {
		...
		updateSql.append(" LpCpz = :LpCpz, LpCpzAmount = :LpCpzAmount, LpCurCpzBal = :LpCurCpzBal ");
	...
	public void save(FinODDetails finOdDetails) {
		...
		insertSql.append(" LpCpz, LpCpzAmount, LpCurCpzBal,  ");
	...
	public Long checkCustPastDue(long custID) {
		...
		try {
			overdueCount = this.jdbcTemplate.queryForObject(selectSql.toString(), beanParameters, Long.class);
		} catch (EmptyResultDataAccessException e) {
			logger.warn("Exception: ", e);
			overdueCount = 0;
		}
	...
	public FinODDetails getMaxDaysFinODDetails(String finReference) {
		...
		selectSql.append(
				" ON T1.FinReference = T2.FinReference AND T1.FinCurODDays = T2.MaxODDays AND T1.FinCurODAmt > 0 ");
	...
	public List<FinODDetails> getFinODPenalityByFinRef(String finReference, boolean ispft, boolean isrender) {
		...
		selectSql.append(" LPIAmt, LPIPaid, LPIBal, LPIWaived, ApplyODPenalty, ODIncGrcDays, ODChargeType, ");
		selectSql.append(" ODGraceDays, ODChargeCalOn, ODChargeAmtOrPerc, ODAllowWaiver, ODMaxWaiverPerc,  ");
		selectSql.append(" LpCpz, LpCpzAmount, LpCurCpzBal, ");
