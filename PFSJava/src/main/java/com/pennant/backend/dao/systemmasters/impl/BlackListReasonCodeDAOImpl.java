...
public class BlackListReasonCodeDAOImpl extends BasisCodeDAO<BlackListReasonCode> implements BlackListReasonCodeDAO {
	...
	public void delete(BlackListReasonCode blackListReasonCode, String type) {
		...
		try {
			...
			if (recordCount <= 0) {
				ErrorDetails errorDetails= getError("41004", blackListReasonCode.getBLRsnCode(), 
						blackListReasonCode.getUserDetails().getUsrLanguage());
			...
			logger.error("Exception: ", e);
			ErrorDetails errorDetails= getError("41006", blackListReasonCode.getBLRsnCode(), 
					blackListReasonCode.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {
			};
		}
	...
	public void update(BlackListReasonCode blackListReasonCode, String type) {
		...
		if (recordCount <= 0) {
			...
			ErrorDetails errorDetails= getError("41003", blackListReasonCode.getBLRsnCode(), 
					blackListReasonCode.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {
			};
		}
	 ...
	 * @return ErrorDetails
	 */
	private ErrorDetails  getError(String errorId, String bLRsnCode,String userLanguage){
		String[][] parms= new String[2][2]; 
		parms[1][0] = bLRsnCode;

		parms[0][0] = PennantJavaUtil.getLabel("label_BLRsnCode")+ ":" + parms[1][0];
