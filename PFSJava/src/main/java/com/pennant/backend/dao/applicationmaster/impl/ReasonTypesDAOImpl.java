 ...
 */
public class ReasonTypesDAOImpl extends BasisNextidDaoImpl<ReasonTypes> implements ReasonTypesDAO {
	...
	public ReasonTypes getReasonTypes(long id,String type) {
		...
		try {
			reasonTypes = namedParameterJdbcTemplate.queryForObject(sql.toString(), paramSource, rowMapper);
	...
	public String save(ReasonTypes reasonTypes,TableType tableType) {
		...
		if (reasonTypes.getId() <= 0) {
			reasonTypes.setId(getNextidviewDAO().getNextId("SeqReasonTypes"));
		}
