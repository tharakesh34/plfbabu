...
public class FinAdvancePaymentsDAOImpl extends BasisNextidDaoImpl<FinAdvancePayments> implements FinAdvancePaymentsDAO {
	...
	public List<FinAdvancePayments> getFinAdvancePaymentsByFinRef(final String id, String type) {
		...
		selectSql.append(" PhoneNumber, ClearingDate, Status, Active, InputDate, DisbCCy,POIssued,PartnerBankID,LinkedTranId,");
		selectSql.append(" Version , LastMntBy, LastMntOn, RecordStatus, RoleCode, NextRoleCode, TaskId,");
		selectSql.append(" NextTaskId, RecordType, WorkflowId");
		if (StringUtils.trimToEmpty(type).contains("View")) {
			selectSql.append(",BranchCode,BranchBankCode,BranchBankName,BranchDesc,BankName,City,IFSC,partnerbankCode,PartnerBankName,AcType");
		}
		selectSql.append(" From FinAdvancePayments");
		selectSql.append(StringUtils.trimToEmpty(type));
	...
	public void delete(FinAdvancePayments finAdvancePayments, String type) {
		...
		try {
			...
			ErrorDetails errorDetails = getError("41006", finAdvancePayments.getFinReference(), finAdvancePayments
	...
	public String save(FinAdvancePayments finAdvancePayments, String type) {
		...
		insertSql.append(" PhoneNumber, ClearingDate, Status, Active, InputDate, DisbCCy,POIssued,PartnerBankID,LinkedTranId,");
	...
	public void update(FinAdvancePayments finAdvancePayments, String type) {
		...
		if (recordCount <= 0) {
			...
			ErrorDetails errorDetails = getError("41004", finAdvancePayments.getFinReference(), finAdvancePayments
					.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {
			};
		}
