...
public class FinServiceInstrutionDAOImpl extends BasisNextidDaoImpl<FinServiceInstruction> implements FinServiceInstrutionDAO {
	...
	public void saveList(List<FinServiceInstruction> finServiceInstructionList, String type) {
		...
		for (FinServiceInstruction finSerList:finServiceInstructionList) {
			if (finSerList.getServiceSeqId()==Long.MIN_VALUE){
				finSerList.setServiceSeqId(getNextidviewDAO().getNextId("SeqFinInstruction"));
		...
		insertSql.append(" PftDaysBasis, SchdMethod, ActualRate," );
		insertSql.append(" BaseRate, SplRate, Margin, GrcPeriodEndDate," );
	...
	public List<FinServiceInstruction> getFinServiceInstructions(final String finReference, String type, String finEvent) {
		...
		selectSql.append(",PftDaysBasis,SchdMethod, ActualRate, BaseRate, SplRate, Margin, GrcPeriodEndDate,NextGrcRepayDate");
		selectSql.append(",RepayFrq, NextRepayDate, Amount, RecalType, RecalFromDate, RecalToDate, PftIntact, Terms ,ServiceReqNo,Remarks");
		...
		SqlParameterSource beanParameters = new BeanPropertySqlParameterSource(finServiceInstruction);
		RowMapper<FinServiceInstruction> typeRowMapper = ParameterizedBeanPropertyRowMapper.newInstance(FinServiceInstruction.class);
