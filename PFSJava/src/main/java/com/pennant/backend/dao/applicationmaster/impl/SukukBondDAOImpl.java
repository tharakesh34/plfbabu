...
public class SukukBondDAOImpl extends BasisCodeDAO<SukukBond> implements SukukBondDAO {
	...
	public void delete(SukukBond sukukBond, String type) {
		...
		try {
			...
			if (recordCount <= 0) {
				ErrorDetails errorDetails= getError("41003",sukukBond.getId() ,sukukBond.getUserDetails().getUsrLanguage());
				throw new DataAccessException(errorDetails.getError()) {
				};
			}
		} catch (DataAccessException e) {
			logger.error("Exception: ", e);
			ErrorDetails errorDetails= getError("41006",sukukBond.getId() ,sukukBond.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {
			};
		}
		logger.debug("Leaving");
	...
	public void update(SukukBond sukukBond, String type) {
		...
		if (recordCount <= 0) {
			...
			ErrorDetails errorDetails= getError("41004",sukukBond.getId() ,sukukBond.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {
			};
		}
		logger.debug("Leaving");
	...
	private ErrorDetails  getError(String errorId, String bondCode, String userLanguage){
		String[][] parms= new String[2][1];
		parms[1][0] = bondCode;
		parms[0][0] = PennantJavaUtil.getLabel("label_BondCode")+ ":" + parms[1][0];
