...
public class MandateDAOImpl extends BasisNextidDaoImpl<Mandate> implements MandateDAO {
	...
	public void delete(Mandate mandate, String type) {
		...
		try {
			...
			if (recordCount <= 0) {
				ErrorDetails errorDetails = getError("41003", mandate.getMandateID(), mandate.getUserDetails()
						.getUsrLanguage());
			...
			logger.error(e);
			ErrorDetails errorDetails = getError("41006", mandate.getMandateID(), mandate.getUserDetails()
					.getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {
			};
		}
	...
	public void update(Mandate mandate, String type) {
		...
		if (recordCount <= 0) {
			...
			ErrorDetails errorDetails = getError("41004", mandate.getMandateID(), mandate.getUserDetails()
					.getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {
			};
		}
