...
public class FinanceScoreHeaderDAOImpl extends BasisNextidDaoImpl<FinanceScoreHeader> implements FinanceScoreHeaderDAO {
    ...
    public void deleteDetailList(List<Long> headerIdList, String type) {
		...
		logger.debug("deleteSql: " + deleteSql.toString());
		logger.debug("Leaving");
		this.namedParameterJdbcTemplate.update(deleteSql.toString(), parameterSource);
    ...
    public boolean deleteHeader(FinanceScoreHeader scoreHeader, String type) {
		...
		SqlParameterSource beanParameters = new BeanPropertySqlParameterSource(scoreHeader);
		int recordCount = this.namedParameterJdbcTemplate.update(deleteSql.toString(), beanParameters);
