...
public class VASConfigurationDAOImpl extends BasisCodeDAO<VASConfiguration> implements VASConfigurationDAO {
	...
	public void delete(VASConfiguration vASConfiguration,String type) {
		...
		try{
			recordCount = this.namedParameterJdbcTemplate.update(deleteSql.toString(), beanParameters);
				if (recordCount <= 0) {
				ErrorDetails errorDetails= getError("41003",vASConfiguration.getId() , vASConfiguration.getUserDetails().getUsrLanguage());
				throw new DataAccessException(errorDetails.getError()) {};
			}
		}catch(DataAccessException e){
			logger.error("Exception: ", e);
			ErrorDetails errorDetails= getError("41006",vASConfiguration.getId(), vASConfiguration.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {};
		}
		logger.debug("Leaving");
	}
	...
	public void update(VASConfiguration vASConfiguration,String type) {
		...
		if (recordCount <= 0) {
			...
			ErrorDetails errorDetails= getError("41004",vASConfiguration.getId() ,vASConfiguration.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {};
		}
		logger.debug("Leaving");
	}
