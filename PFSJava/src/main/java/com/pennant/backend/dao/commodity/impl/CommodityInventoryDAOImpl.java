...
public class CommodityInventoryDAOImpl extends BasisNextidDaoImpl<CommodityInventory> implements CommodityInventoryDAO {
	...
	public void delete(CommodityInventory commodityInventory, String type) {
		...
		try {
			...
			if (recordCount <= 0) {
				ErrorDetails errorDetails = getError("41003", String.valueOf(commodityInventory.getCommodityInvId()), 
						commodityInventory.getUserDetails().getUsrLanguage());
			...
			ErrorDetails errorDetails = getError("41006", String.valueOf(commodityInventory.getCommodityInvId()), 
					commodityInventory.getUserDetails().getUsrLanguage());
	...
	public void update(CommodityInventory commodityInventory, String type) {
		...
		if (recordCount <= 0) {
			...
			ErrorDetails errorDetails = getError("41004", String.valueOf(commodityInventory.getCommodityInvId()),
					commodityInventory.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {
			};
		}
	...
	private ErrorDetails getError(String errorId, String commodityInvId, String userLanguage) {
		String[][] parms = new String[2][1];
		parms[1][0] = commodityInvId;
		parms[0][0] = PennantJavaUtil.getLabel("label_CommodityInvId") + ":" + parms[1][0];
