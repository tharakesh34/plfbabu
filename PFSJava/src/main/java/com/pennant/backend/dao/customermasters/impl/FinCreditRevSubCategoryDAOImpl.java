...
public class FinCreditRevSubCategoryDAOImpl extends BasisCodeDAO<FinCreditRevSubCategory> implements FinCreditRevSubCategoryDAO {
	...
	/**
	 * This method get the module from method getFinCreditRevSubCategory() and set the new record flag as true and return FinCreditRevSubCategory()   
	 * @return FinCreditRevSubCategory
	 */
	...
	public FinCreditRevSubCategory getFinCreditRevSubCategoryById(final String id, String type) {
		...
		StringBuilder selectSql = new StringBuilder("Select SubCategoryCode, SubCategorySeque, CategoryId, SubCategoryDesc, SubCategoryItemType, ItemsToCal, ItemRule, isCreditCCY, mainSubCategoryCode, CalcSeque, format, percentCategory, grand");
	 ...
	 * delete Finance Credit Review Sub Category by key SubCategoryCode
	 * 
	...
	public void delete(FinCreditRevSubCategory finCreditRevSubCategory,String type) {
		...
		try{
			...
			if (recordCount <= 0) {
				ErrorDetails errorDetails= getError("41003",finCreditRevSubCategory.getId() ,StringUtils.trimToEmpty(finCreditRevSubCategory.getUserDetails().getUsrLanguage()));
				throw new DataAccessException(errorDetails.getError()) {};
			}
		}catch(DataAccessException e){
			logger.error("Exception: ", e);
			ErrorDetails errorDetails= getError("41006",finCreditRevSubCategory.getId() ,finCreditRevSubCategory.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {};
		}
		logger.debug("Leaving");
	}
	...
	public String save(FinCreditRevSubCategory finCreditRevSubCategory,String type) {
		...
		insertSql.append(" (SubCategoryCode, SubCategorySeque, CategoryId, SubCategoryDesc, SubCategoryItemType, ItemsToCal, ItemRule, isCreditCCY, mainSubCategoryCode, CalcSeque, format, percentCategory, grand");
		insertSql.append(", Version , LastMntBy, LastMntOn, RecordStatus, RoleCode, NextRoleCode, TaskId, NextTaskId, RecordType, WorkflowId)");
		insertSql.append(" Values(:SubCategoryCode, :SubCategorySeque, :CategoryId, :SubCategoryDesc, :SubCategoryItemType, :ItemsToCal, :ItemRule, :isCreditCCY, :mainSubCategoryCode, :CalcSeque, :format, :percentCategory, :grand");
		insertSql.append(", :Version , :LastMntBy, :LastMntOn, :RecordStatus, :RoleCode, :NextRoleCode, :TaskId, :NextTaskId, :RecordType, :WorkflowId)");
	...
	/**
	 * This method updates the Record FinCreditRevSubCategory or FinCreditRevSubCategory_Temp.
	 * if Record not updated then throws DataAccessException with  error  41004.
	 * update Finance Credit Review Sub Category by key SubCategoryCode and Version
	 * 
	 * @param Finance Credit Review Sub Category (finCreditRevSubCategory)
	 * @param  type (String)
	 * 			""/_Temp/_View          
	...
	public void update(FinCreditRevSubCategory finCreditRevSubCategory,String type) {
		...
		if (recordCount <= 0) {
			...
			ErrorDetails errorDetails= getError("41004",finCreditRevSubCategory.getId() ,finCreditRevSubCategory.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {};
		}
		logger.debug("Leaving");
	}
