...
public class ExpenseTypeDAOImpl extends BasisNextidDaoImpl<ExpenseType> implements ExpenseTypeDAO {
	...
	public void delete(ExpenseType expenseType, String type) {
		...
		try {
			...
			if (recordCount <= 0) {
				ErrorDetails errorDetails = getError("41003", expenseType.getExpenceTypeId(), expenseType
			...
			ErrorDetails errorDetails = getError("41006", expenseType.getExpenceTypeId(), expenseType.getUserDetails()
					.getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {
			};
		}
	...
	public void update(ExpenseType expenseType, String type) {
		...
		if (recordCount <= 0) {
			...
			ErrorDetails errorDetails = getError("41004", expenseType.getExpenceTypeId(), expenseType.getUserDetails()
					.getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {
			};
		}
