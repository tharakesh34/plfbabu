 ...
 */
public class PaymentHeaderDAOImpl extends BasisNextidDaoImpl<PaymentHeader> implements PaymentHeaderDAO {
	...
	public PaymentHeader getPaymentHeader(long paymentId, String type) {
		...
		try {
			paymentHeader = namedParameterJdbcTemplate.queryForObject(sql.toString(), paramSource, rowMapper);
	...
	public String save(PaymentHeader paymentHeader, TableType tableType) {
		...
		if (paymentHeader.getPaymentId() <= 0) {
			paymentHeader.setPaymentId(getNextidviewDAO().getNextId("SeqPaymentHeader"));
		}
		// Execute the SQL, binding the arguments.
		logger.trace(Literal.SQL + sql.toString());
		SqlParameterSource paramSource = new BeanPropertySqlParameterSource(paymentHeader);
	...
	public FinanceMain getFinanceDetails(String finReference) {
		...
		sql.append("  SELECT FM.FinReference, FT.FinType, FT.FINTYPEDESC LovDescFinTypeName, CU.CUSTCIF LovDescCustCif");
		sql.append(" ,CU.CUSTSHRTNAME LovDescCustShrtName, CURR.CCYCODE finCcy");
	...
	public List<FinExcessAmount> getfinExcessAmount(String finReference) {
		...
		try {
			return namedParameterJdbcTemplate.query(sql.toString(), source, rowMapper);
		} catch (EmptyResultDataAccessException e) {
			logger.error("Exception: ", e);
		} finally {
			source = null;
