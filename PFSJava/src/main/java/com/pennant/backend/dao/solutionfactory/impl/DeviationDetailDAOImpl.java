...
public class DeviationDetailDAOImpl extends BasisNextidDaoImpl<DeviationDetail> implements DeviationDetailDAO {
	...
	public void delete(DeviationDetail deviationDetail,String type) {
		...
		try{
			...
			if (recordCount <= 0) {
				ErrorDetails errorDetails= getError("41003",deviationDetail.getDeviationID() ,deviationDetail.getUserDetails().getUsrLanguage());
				throw new DataAccessException(errorDetails.getError()) {};
			}
		}catch(DataAccessException e){
			logger.error("Exception: ", e);
			ErrorDetails errorDetails= getError("41006",deviationDetail.getDeviationID() ,deviationDetail.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {};
		}
		logger.debug("Leaving");
	}
	...
	public void update(DeviationDetail deviationDetail,String type) {
		...
		if (recordCount <= 0) {
			...
			ErrorDetails errorDetails= getError("41004",deviationDetail.getDeviationID() ,deviationDetail.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {};
		}
		logger.debug("Leaving");
	}
