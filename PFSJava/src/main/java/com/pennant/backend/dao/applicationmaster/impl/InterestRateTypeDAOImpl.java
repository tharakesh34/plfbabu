...
public class InterestRateTypeDAOImpl extends BasisCodeDAO<InterestRateType> implements InterestRateTypeDAO {
	...
	public void delete(InterestRateType interestRateType, String type) {
		...
		try {
			...
			if (recordCount <= 0) {
				ErrorDetails errorDetails = getError("41004",interestRateType.getIntRateTypeCode(), 
					interestRateType.getUserDetails().getUsrLanguage());
			...
			logger.error("Exception: ", e);
			ErrorDetails errorDetails = getError("41006",interestRateType.getIntRateTypeCode(), 
					interestRateType.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {
			};
		}
	...
	public void update(InterestRateType interestRateType, String type) {
		...
		if (recordCount <= 0) {
			...
			ErrorDetails errorDetails = getError("41003",interestRateType.getIntRateTypeCode(),
					interestRateType.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {
			};
		}
	 ...
	 * @return ErrorDetails
	 */
	private ErrorDetails  getError(String errorId, String intRatetypeCode, String userLanguage){
		String[][] parms= new String[2][1]; 
		parms[1][0] = intRatetypeCode;
		parms[0][0] = PennantJavaUtil.getLabel("label_IntRateTypeCode")+ ":" + parms[1][0];
