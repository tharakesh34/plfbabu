...
public class AuthorizationServiceImpl extends GenericService<Authorization> implements AuthorizationService {
	 ...
	 * AMTAuthorization/AMTAuthorization_Temp by using AuthorizationDAO's save method b) Update the Record in the table.
	 ...
	 * following actions a) DELETE Delete the record from the main table by using getAuthorizationDAO().delete with
	 * parameters authorization,"" b) NEW Add new record in to main table by using getAuthorizationDAO().save with
	 * parameters authorization,"" c) EDIT Update record in the main table by using getAuthorizationDAO().update with
	 * parameters authorization,"" 3) Delete the record from the workFlow table by using getAuthorizationDAO().delete
	 * with parameters authorization,"_Temp" 4) Audit the record in to AuditHeader and AdtAMTAuthorization by using
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage, String method, boolean onlineRequest) {
		...
		if (authorization.isWorkflow()) {
			tempAuthorization = getAuthorizationDAO().getAuthorization(authorization.getAuthUserId(),
					authorization.getAuthType(), "_Temp");
		}
		Authorization befAuthorization = getAuthorizationDAO().getAuthorization(authorization.getAuthUserId(),
				authorization.getAuthType(), "");
			...
			if (!authorization.isWorkflow()) { // With out Work flow for update and delete
				...
				if (befAuthorization == null) { // if records not exists in the main table
					...
					if (oldAuthorization != null
							&& !oldAuthorization.getLastMntOn().equals(befAuthorization.getLastMntOn())) {
				...
				if (tempAuthorization != null && oldAuthorization != null
						&& !oldAuthorization.getLastMntOn().equals(tempAuthorization.getLastMntOn())) {
					auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(
							new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, valueParm), usrLanguage));
				}
			}
