...
public class ProfitCenterServiceImpl extends GenericService<ProfitCenter> implements ProfitCenterService {
	private static final Logger logger = LogManager.getLogger(ProfitCenterServiceImpl.class);
	 ...
	 * Do Add or Update the Record a) Add new Record for the new record in the DB table ProfitCenters/ProfitCenters_Temp
	 * by using ProfitCentersDAO's save method b) Update the Record in the table. based on the module workFlow
	 ...
	 * following actions a) DELETE Delete the record from the main table by using getProfitCenterDAO().delete with
	 * parameters profitCenter,"" b) NEW Add new record in to main table by using getProfitCenterDAO().save with
	 * parameters profitCenter,"" c) EDIT Update record in the main table by using getProfitCenterDAO().update with
	 * parameters profitCenter,"" 3) Delete the record from the workFlow table by using getProfitCenterDAO().delete with
	 * parameters profitCenter,"_Temp" 4) Audit the record in to AuditHeader and AdtProfitCenters by using
	...
	public AuditHeader doApprove(AuditHeader auditHeader) {
		...
		if (!PennantConstants.RECORD_TYPE_NEW.equals(profitCenter.getRecordType())) {
			auditHeader.getAuditDetail()
					.setBefImage(profitCenterDAO.getProfitCenter(profitCenter.getProfitCenterID(), ""));
		}
		...
		if (profitCenter.getRecordType().equals(PennantConstants.RECORD_TYPE_DEL)) {
			...
			if (profitCenter.getRecordType().equals(PennantConstants.RECORD_TYPE_NEW)) {
	 ...
	 * from getProfitCenterDAO().getErrorDetail with Error ID and language as parameters. if any error/Warnings then
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage) {
		...
		if (profitCenter.isNew()
			...
			parameters[0] = PennantJavaUtil.getLabel("label_ProfitCenterCode") + ": "
					+ profitCenter.getProfitCenterCode();
