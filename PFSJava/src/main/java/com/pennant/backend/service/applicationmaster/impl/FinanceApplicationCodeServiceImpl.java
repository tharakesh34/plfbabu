...
/**
 * Service implementation for methods that depends on <b>FinanceApplicationCode</b>.<br>
 * 
 */
public class FinanceApplicationCodeServiceImpl extends GenericService<AddressType>
		implements FinanceApplicationCodeService {

	private static Logger logger = LogManager.getLogger(FinanceApplicationCodeServiceImpl.class);
	...
	public void setFinanceApplicationCodeDAO(FinanceApplicationCodeDAO financeApplicationCodeDAO) {
	 ...
	 * BMTFinAppCodes/BMTFinAppCodes_Temp by using FinanceApplicationCodeDAO's save method b) Update the Record in the
	...
	public AuditHeader saveOrUpdate(AuditHeader auditHeader) {
		...
		String tableType = "";
		FinanceApplicationCode financeApplicationCode = (FinanceApplicationCode) auditHeader.getAuditDetail()
				.getModelData();
		...
		if (financeApplicationCode.isNew()) {
			...
			getFinanceApplicationCodeDAO().update(financeApplicationCode, tableType);
		}
	...
	public FinanceApplicationCode getFinanceApplicationCodeById(String id) {
		return getFinanceApplicationCodeDAO().getFinanceApplicationCodeById(id, "_View");
	}

	/**
	 * getApprovedFinanceApplicationCodeById fetch the details by using FinanceApplicationCodeDAO's
	...
	public FinanceApplicationCode getApprovedFinanceApplicationCodeById(String id) {
	 ...
	 * following actions a) DELETE Delete the record from the main table by using getFinanceApplicationCodeDAO().delete
	 * with parameters financeApplicationCode,"" b) NEW Add new record in to main table by using
	 * getFinanceApplicationCodeDAO().save with parameters financeApplicationCode,"" c) EDIT Update record in the main
	 * table by using getFinanceApplicationCodeDAO().update with parameters financeApplicationCode,"" 3) Delete the
	 * record from the workFlow table by using getFinanceApplicationCodeDAO().delete with parameters
	 * financeApplicationCode,"_Temp" 4) Audit the record in to AuditHeader and AdtBMTFinAppCodes by using
	...
	public AuditHeader doApprove(AuditHeader auditHeader) {
		...
		BeanUtils.copyProperties((FinanceApplicationCode) auditHeader.getAuditDetail().getModelData(),
				financeApplicationCode);

		if (financeApplicationCode.getRecordType().equals(PennantConstants.RECORD_TYPE_DEL)) {
			...
			if (financeApplicationCode.getRecordType().equals(PennantConstants.RECORD_TYPE_NEW)) {
	 ...
	 * workFlow table by using getFinanceApplicationCodeDAO().delete with parameters financeApplicationCode,"_Temp" 3)
	 ...
	 * from getFinanceApplicationCodeDAO().getErrorDetail with Error ID and language as parameters. if any
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage, String method) {
		...
		FinanceApplicationCode financeApplicationCode = (FinanceApplicationCode) auditDetail.getModelData();
		FinanceApplicationCode tempFinanceApplicationCode = null;

		if (financeApplicationCode.isWorkflow()) {
			tempFinanceApplicationCode = getFinanceApplicationCodeDAO()
					.getFinanceApplicationCodeById(financeApplicationCode.getId(), "_Temp");
		}
		...
		FinanceApplicationCode oldFinanceApplicationCode = financeApplicationCode.getBefImage();
		...
		if (financeApplicationCode.isNew()) { // for New record or new record
			...
			if (!financeApplicationCode.isWorkflow()) {// With out Work flow
				...
				if (financeApplicationCode.getRecordType().equals(PennantConstants.RECORD_TYPE_NEW)) { // if records type
					...
					if (befFinanceApplicationCode == null || tempFinanceApplicationCode != null) {
			...
			if (!financeApplicationCode.isWorkflow()) { // With out Work flow
				...
				if (befFinanceApplicationCode == null) { // if records not
					...
					if (oldFinanceApplicationCode != null && !oldFinanceApplicationCode.getLastMntOn()
							.equals(befFinanceApplicationCode.getLastMntOn())) {
				...
				if (tempFinanceApplicationCode != null && oldFinanceApplicationCode != null
						&& !oldFinanceApplicationCode.getLastMntOn()
								.equals(tempFinanceApplicationCode.getLastMntOn())) {
					auditDetail.setErrorDetail(new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, null));
				}
			}
		}
		...
		if ("doApprove".equals(StringUtils.trimToEmpty(method)) || !financeApplicationCode.isWorkflow()) {
