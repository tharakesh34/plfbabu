...
public class VesselDetailServiceImpl extends GenericService<VesselDetail> implements VesselDetailService {
	private static final Logger logger = LogManager.getLogger(VesselDetailServiceImpl.class);
	...
	/**
	 * @param vesselDetailDAO
	 *            the vesselDetailDAO to set
	 */
	 ...
	 * Do Add or Update the Record a) Add new Record for the new record in the DB table VesselDetails/VesselDetails_Temp
	 * by using VesselDetailDAO's save method b) Update the Record in the table. based on the module workFlow
	 * Configuration. by using VesselDetailDAO's update method 3) Audit the record in to AuditHeader and
	 * AdtVesselDetails by using auditHeaderDAO.addAudit(auditHeader)
	 * 
	 * @param AuditHeader
	 *            (auditHeader)
	 * @return auditHeader
	 ...
	 * VesselDetails by using VesselDetailDAO's delete method with type as Blank 3) Audit the record in to AuditHeader
	 * and AdtVesselDetails by using auditHeaderDAO.addAudit(auditHeader)
	 * 
	 * @param AuditHeader
	 *            (auditHeader)
	 * @return auditHeader
	...
	/**
	 * getApprovedVesselDetailById fetch the details by using VesselDetailDAO's getVesselDetailById method . with
	 * parameter id and type as blank. it fetches the approved records from the VesselDetails.
	 * 
	 * @param id
	 *            (int)
	 * @return VesselDetail
	 ...
	 * workFlow table by using getVesselDetailDAO().delete with parameters vesselDetail,"_Temp" 3) Audit the record in
	 * to AuditHeader and AdtVesselDetails by using auditHeaderDAO.addAudit(auditHeader) for Work flow
	 * 
	 * @param AuditHeader
	 *            (auditHeader)
	 * @return auditHeader
	 ...
	 * mismatch conditions Fetch the error details from getVesselDetailDAO().getErrorDetail with Error ID and language
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage, String method, boolean onlineRequest) {
		...
		if (vesselDetail.isNew()) { // for New record or new record into work flow
			...
			if (!vesselDetail.isWorkflow()) { // With out Work flow for update and delete
				...
				if (befVesselDetail == null) { // if records not exists in the main table
					...
					if (oldVesselDetail != null
							&& !oldVesselDetail.getLastMntOn().equals(befVesselDetail.getLastMntOn())) {
				...
				if (tempVesselDetail != null && oldVesselDetail != null
						&& !oldVesselDetail.getLastMntOn().equals(tempVesselDetail.getLastMntOn())) {
					auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(
							new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, valueParm), usrLanguage));
				}
			}
		...
		if (vesselType != null) {
			...
			errParm1[0] = PennantJavaUtil.getLabel("label_VesselType") + ":" + valueParm1[0] + " "
					+ PennantJavaUtil.getLabel("label_VesselSubType") + ":" + valueParm1[1];
