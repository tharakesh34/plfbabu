...
public class FinanceFlagsServiceImpl extends GenericService<FinanceDetail> implements FinanceFlagsService {
	 ...
	 * Do Add or Update the Record a) Add new Record for the new record in the DB table FinanceFlags/FinanceFlags_Temp
	 * by using FinFlagsHeaderDAO's save method b) Update the Record in the table. based on the module workFlow
	...
	private List<AuditDetail> processingFinFlags(FinanceFlag financeFlag, String type) {
		...
		for (AuditDetail auditDetail : auditDetails) {
			...
			if (saveRecord) {
				finFlagDetailsDAO.delete(financeFlag.getFinReference(), finFlagsDetail.getFlagCode(),
	...
	private List<AuditDetail> setFinFlagsDetailsAuditData(FinanceFlag financeFlag, String method) {
		logger.debug("Entering");
		...
		String[] fields = PennantJavaUtil.getFieldDetails(new FinFlagsDetail(),
				new FinFlagsDetail().getExcludeFields());
		int count = 0;
		for (FinFlagsDetail finFlagsDetail : financeFlag.getFinFlagDetailList()) {
			...
			auditDetails.add(new AuditDetail(auditTranType, count++, fields[0], fields[1], finFlagsDetail.getBefImage(),
					finFlagsDetail));
		}
		logger.debug("Leaving");
		return auditDetails;
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage, String method, boolean onlineRequest) {
			...
			if (!financeFlag.isWorkflow()) { // With out Work flow for update and delete
				...
				if (befFinanceFlag == null) { // if records not exists in the main table
					...
					if (oldFinanceFlag != null
							&& !oldFinanceFlag.getLastMntOn().equals(befFinanceFlag.getLastMntOn())) {
						if (StringUtils.trimToEmpty(auditDetail.getAuditTranType())
								.equalsIgnoreCase(PennantConstants.TRAN_DEL)) {
							auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(
				...
				if (oldFinanceFlag != null && tempFinanceFlag != null
						&& !oldFinanceFlag.getLastMntOn().equals(tempFinanceFlag.getLastMntOn())) {
					auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(
							new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, valueParm), usrLanguage));
				}
			}
		...
		if ("doApprove".equals(StringUtils.trimToEmpty(method)) || !financeFlag.isWorkflow()) {
	...
	public AuditHeader doApprove(AuditHeader auditHeader) {
		...
		BeanUtils.copyProperties((FinanceFlag) auditHeader.getAuditDetail().getModelData(), financeFlag);
