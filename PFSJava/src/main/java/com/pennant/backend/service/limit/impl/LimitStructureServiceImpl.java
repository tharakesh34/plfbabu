...
public class LimitStructureServiceImpl extends GenericService<LimitStructure> implements LimitStructureService {
	 ...
	 * LIMIT_STRUCTURE/LIMIT_STRUCTURE_Temp by using LimitStructureDAO's save method b) Update the Record in the table.
	...
	public LimitStructure getLimitStructureById(String id) {
		...
		if (limitStructure != null) {
			limitStructure.setLimitStructureDetailItemsList(
					getLimitStructureDetailDAO().getLimitStructureDetailById(id, "_View"));
		}
		logger.debug("Leaving");
		return limitStructure;
	}
	...
	public LimitStructure getApprovedLimitStructureById(String id) {
		...
		if (limitStructure != null) {
			limitStructure.setLimitStructureDetailItemsList(
					getLimitStructureDetailDAO().getLimitStructureDetailById(id, "_AView"));
		}
		logger.debug("Leaving");
		return limitStructure;
	}
	...
	public AuditHeader doApprove(AuditHeader auditHeader) {
		...
		/*
		 * if(!limitStructure.isScheduled()){ tableType = "_STAGING"; }
		 */
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage, String method, boolean onlineRequest) {
		 ...
		 * if(!limitStructure.isScheduled()){ tableType="_STAGING"; }
		 */
		LimitStructure befLimitStructure = getLimitStructureDAO().getLimitStructureById(limitStructure.getId(),
				tableType);
		...
		if (limitStructure.isNew()) { // for New record or new record into work flow
			...
			if (!limitStructure.isWorkflow()) { // With out Work flow for update and delete
				if (befLimitStructure == null) { // if records not exists in the main table
					...
					if (oldLimitStructure != null
							&& !oldLimitStructure.getLastMntOn().equals(befLimitStructure.getLastMntOn())) {
				...
				if (tempLimitStructure == null) { // if records not exists in the Work flow table 
					auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(
						...
						&& !oldLimitStructure.getLastMntOn().equals(tempLimitStructure.getLastMntOn())) {
					auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(
							new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, valueParm), usrLanguage));
				}
			}
	...
	private AuditHeader getAuditDetails(AuditHeader auditHeader, String method) {
		...
		if (limitStructure.getLimitStructureDetailItemsList() != null
				&& limitStructure.getLimitStructureDetailItemsList().size() > 0) {
	 ...
	 * @return
	 */
	private List<AuditDetail> setLimitStructureItemsAuditData(LimitStructure limitStructure, String auditTranType,
			String method) {
		logger.debug("Entering ");
		List<AuditDetail> auditDetails = new ArrayList<AuditDetail>();
		boolean delete = false;

		if ((PennantConstants.RECORD_TYPE_DEL.equals(limitStructure.getRecordType())
		...
		for (int i = 0; i < limitStructure.getLimitStructureDetailItemsList().size(); i++) {
			...
			if (!auditTranType.equals(PennantConstants.TRAN_WF)) {
				...
				} else if (limitStructureDetails.getRecordType().equalsIgnoreCase(PennantConstants.RECORD_TYPE_DEL)
						|| limitStructureDetails.getRecordType().equalsIgnoreCase(PennantConstants.RECORD_TYPE_CAN)) {
			...
			if (StringUtils.isNotEmpty(limitStructureDetails.getRecordType())) {
				auditDetails.add(new AuditDetail(auditTranType, i + 1, fields[0], fields[1],
						limitStructureDetails.getBefImage(), limitStructureDetails));
			}
		}
		logger.debug("Leaving ");
		return auditDetails;
	 ...
	 * @return
	 */
	private List<AuditDetail> processingLimitStructureDetailList(List<AuditDetail> auditDetails, String type,
	...
	@Override
	public List<LimitStructureDetail> getStructuredetailsByLimitGroup(String category, String groupCode,
			boolean isLine) {
