 ...
 * FILE HEADER *
 ********************************************************************************************
 * * FileName : CustomerIdentityServiceImpl.java * * Author : PENNANT TECHONOLOGIES * * Creation Date : 06-05-2011 * *
...
public class CustomerIdentityServiceImpl extends GenericService<CustomerIdentity> implements CustomerIdentityService {
	...
	private static Logger logger = LogManager.getLogger(CustomerIdentityServiceImpl.class);
	 ...
	 * CustIdentities/CustIdentities_Temp by using CustomerIdentityDAO's save method b) Update the Record in the table.
	 * based on the module workFlow Configuration. by using CustomerIdentityDAO's update method 3) Audit the record in
	 * to AuditHeader and AdtCustIdentities by using auditHeaderDAO.addAudit(auditHeader)
	 * 
	 ...
	 * CustIdentities by using CustomerIdentityDAO's delete method with type as Blank 3) Audit the record in to
	 * AuditHeader and AdtCustIdentities by using auditHeaderDAO.addAudit(auditHeader)
	 * 
	 * @param AuditHeader (auditHeader)
	 * @return auditHeader
	 */
	 ...
	 * following actions a) DELETE Delete the record from the main table by using getCustomerIdentityDAO().delete with
	 * parameters customerIdentity,"" b) NEW Add new record in to main table by using getCustomerIdentityDAO().save with
	 * parameters customerIdentity,"" c) EDIT Update record in the main table by using getCustomerIdentityDAO().update
	 * with parameters customerIdentity,"" 3) Delete the record from the workFlow table by using
	 * getCustomerIdentityDAO().delete with parameters customerIdentity,"_Temp" 4) Audit the record in to AuditHeader
	 * and AdtCustIdentities by using auditHeaderDAO.addAudit(auditHeader) for Work flow 5) Audit the record in to
	 * AuditHeader and AdtCustIdentities by using auditHeaderDAO.addAudit(auditHeader) based on the transaction Type.
	 * 
	 * @param AuditHeader (auditHeader)
	 * @return auditHeader
	 */
	 ...
	 * workFlow table by using getCustomerIdentityDAO().delete with parameters customerIdentity,"_Temp" 3) Audit the
	 * record in to AuditHeader and AdtCustIdentities by using auditHeaderDAO.addAudit(auditHeader) for Work flow
	 * 
	 * @param AuditHeader (auditHeader)
	 * @return auditHeader
	 */
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage, String method) {
		...
		if (customerIdentity.isWorkflow()) {
			tempCustomerIdentity = getCustomerIdentityDAO().getCustomerIdentityByID(customerIdentity.getId(),
					customerIdentity.getIdType(), "_Temp");
		}
		CustomerIdentity befCustomerIdentity = getCustomerIdentityDAO()
				.getCustomerIdentityByID(customerIdentity.getId(), customerIdentity.getIdType(), "");
		...
		if (customerIdentity.isNewRecord()) { // for New record or new record into
			...
			if (!customerIdentity.isWorkflow()) {// With out Work flow only new
				...
				if (customerIdentity.getRecordType().equals(PennantConstants.RECORD_TYPE_NEW)) { // if records type
																									// is new
					if (befCustomerIdentity != null || tempCustomerIdentity != null) { // if records
																						// already
																						// exists in the
																						// main table
						auditDetail.setErrorDetail(new ErrorDetail(PennantConstants.KEY_FIELD, "41001", errParm, null));
					...
					if (befCustomerIdentity == null || tempCustomerIdentity != null) {
			...
			if (!customerIdentity.isWorkflow()) { // With out Work flow for
				...
				if (befCustomerIdentity == null) { // if records not exists in
					...
					if (oldCustomerIdentity != null
							&& !oldCustomerIdentity.getLastMntOn().equals(befCustomerIdentity.getLastMntOn())) {
				...
				if (tempCustomerIdentity != null && oldCustomerIdentity != null
						&& !oldCustomerIdentity.getLastMntOn().equals(tempCustomerIdentity.getLastMntOn())) {
					auditDetail.setErrorDetail(new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, null));
				}
		...
		if ("doApprove".equals(StringUtils.trimToEmpty(method)) || !customerIdentity.isWorkflow()) {
