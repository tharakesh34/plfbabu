...
public class ErrorDetailServiceImpl extends GenericService<ErrorDetail> implements ErrorDetailService {
	 ...
	 * Do Add or Update the Record a) Add new Record for the new record in the DB table ErrorDetails/ErrorDetails_Temp
	 * by using ErrorDetailDAO's save method b) Update the Record in the table. based on the module workFlow
			...
			if (!errorDetail.isWorkflow()) { // With out Work flow for update and delete
				...
				if (befErrorDetail == null) { // if records not exists in the main table
					...
					if (oldErrorDetail != null
							&& !oldErrorDetail.getLastMntOn().equals(befErrorDetail.getLastMntOn())) {
				...
				if (tempErrorDetail != null && oldErrorDetail != null
						&& !oldErrorDetail.getLastMntOn().equals(tempErrorDetail.getLastMntOn())) {
					auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(
							new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, valueParm), usrLanguage));
				}
			}
