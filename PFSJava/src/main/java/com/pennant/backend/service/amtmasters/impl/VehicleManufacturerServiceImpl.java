...
public class VehicleManufacturerServiceImpl extends GenericService<VehicleManufacturer>
	 ...
	 * AMTVehicleManufacturer/AMTVehicleManufacturer_Temp by using VehicleManufacturerDAO's save method b) Update the
	 * Record in the table. based on the module workFlow Configuration. by using VehicleManufacturerDAO's update method
	 ...
	 * AMTVehicleManufacturer by using VehicleManufacturerDAO's delete method with type as Blank 3) Audit the record in
	 * to AuditHeader and AdtAMTVehicleManufacturer by using auditHeaderDAO.addAudit(auditHeader)
	 * 
	 * @param AuditHeader
	 *            (auditHeader)
	 * @return auditHeader
	...
	/**
	 * getApprovedVehicleManufacturerById fetch the details by using VehicleManufacturerDAO's getVehicleManufacturerById
	 * method . with parameter id and type as blank. it fetches the approved records from the AMTVehicleManufacturer.
	 * 
	 * @param id
	 *            (int)
	 * @return VehicleManufacturer
	 ...
	 * following actions a) DELETE Delete the record from the main table by using getVehicleManufacturerDAO().delete
	 * with parameters vehicleManufacturer,"" b) NEW Add new record in to main table by using
	 * getVehicleManufacturerDAO().save with parameters vehicleManufacturer,"" c) EDIT Update record in the main table
	 * by using getVehicleManufacturerDAO().update with parameters vehicleManufacturer,"" 3) Delete the record from the
	 * workFlow table by using getVehicleManufacturerDAO().delete with parameters vehicleManufacturer,"_Temp" 4) Audit
	 * the record in to AuditHeader and AdtAMTVehicleManufacturer by using auditHeaderDAO.addAudit(auditHeader) for Work
	 * flow 5) Audit the record in to AuditHeader and AdtAMTVehicleManufacturer by using
	...
	public AuditHeader doApprove(AuditHeader auditHeader) {
		...
		BeanUtils.copyProperties((VehicleManufacturer) auditHeader.getAuditDetail().getModelData(),
				vehicleManufacturer);
	 ...
	 * workFlow table by using getVehicleManufacturerDAO().delete with parameters vehicleManufacturer,"_Temp" 3) Audit
	 * the record in to AuditHeader and AdtAMTVehicleManufacturer by using auditHeaderDAO.addAudit(auditHeader) for Work
	 * flow
	 * 
	 * @param AuditHeader
	 ...
	 * from getVehicleManufacturerDAO().getErrorDetail with Error ID and language as parameters. if any error/Warnings
		...
		if (vehicleManufacturer.isNew()) { // for New record or new record into work flow
			...
			if (!vehicleManufacturer.isWorkflow()) { // With out Work flow for update and delete
				...
				if (befVehicleManufacturer == null) { // if records not exists in the main table
					...
					if (oldVehicleManufacturer != null
							&& !oldVehicleManufacturer.getLastMntOn().equals(befVehicleManufacturer.getLastMntOn())) {
				...
				if (tempVehicleManufacturer != null && oldVehicleManufacturer != null
						&& !oldVehicleManufacturer.getLastMntOn().equals(tempVehicleManufacturer.getLastMntOn())) {
					auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(
							new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, valueParm), usrLanguage));
				}
			}
