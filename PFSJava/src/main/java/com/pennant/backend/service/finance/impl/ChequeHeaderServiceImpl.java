...
public class ChequeHeaderServiceImpl extends GenericService<ChequeHeader> implements ChequeHeaderService {
	 ...
	 * ChequeHeaders/ChequeHeaders_Temp by using ChequeHeadersDAO's save method b)
	 * Update the Record in the table. based on the module workFlow
	 * Configuration. by using ChequeHeadersDAO's update method 3) Audit the record
	 * in to AuditHeader and AdtChequeHeaders by using
	 ...
	 */
	@Override
	public List<AuditDetail> processingChequeDetailList(List<AuditDetail> auditDetails, TableType type, long headerID,String finCcy) {
	...
	private List<AuditDetail> setChequeDetailAuditData(ChequeHeader chequeHeader, String auditTranType, String method) {
		...
		for (int i = 0; i < chequeHeader.getChequeDetailList().size(); i++) {
			...
			auditDetails.add(
					new AuditDetail(auditTranType, i + 1, fields[0], fields[1], detail.getBefImage(), detail));
		}
	 ...
	 * table ChequeHeaders by using ChequeHeadersDAO's delete method with type as
	 * Blank 3) Audit the record in to AuditHeader and AdtChequeHeaders by using
	 * auditHeaderDAO.addAudit(auditHeader)
	 * 
	 * @param AuditHeader
	 *            (auditHeader)
	...
	/**
	 * getChequeHeaders fetch the details by using ChequeHeadersDAO's getChequeHeadersById
	 * method.
	 * 
	 * @param productID
	...
	/**
	 * getApprovedChequeHeadersById fetch the details by using ChequeHeadersDAO's
	 * getChequeHeadersById method . with parameter id and type as blank. it fetches
	 * the approved records from the ChequeHeaders.
	 * 
	 * @param productID
	 *            productID of the ChequeHeader.
	 *            (String)
	 ...
	 * using getChequeHeaderDAO().delete with parameters product,"" b) NEW Add new
	 * record in to main table by using getChequeHeaderDAO().save with parameters
	 * product,"" c) EDIT Update record in the main table by using
	 * getChequeHeaderDAO().update with parameters product,"" 3) Delete the record
	 * from the workFlow table by using getChequeHeaderDAO().delete with parameters
	 * product,"_Temp" 4) Audit the record in to AuditHeader and
	 * AdtChequeHeaders by using auditHeaderDAO.addAudit(auditHeader) for Work
	 * flow 5) Audit the record in to AuditHeader and AdtChequeHeaders by using
	 ...
	 */
	@Override
	public AuditHeader doApprove(AuditHeader auditHeader,TableType tableType) {
		logger.info(Literal.ENTERING);
		...
		if (chequeHeader.getRecordType().equals(PennantConstants.RECORD_TYPE_DEL)) {
			...
			if (chequeHeader.getRecordType().equals(
		...
		auditHeader.setAuditDetails(getListAuditDetails(listDeletion(chequeHeader, TableType.TEMP_TAB, auditHeader.getAuditTranType())));//FIXME
		...
		if (chequeHeader.getChequeDetailList() != null && chequeHeader.getChequeDetailList().size() > 0) {
			...
			for (int i = 0; i < chequeHeader.getChequeDetailList().size(); i++) {
				...
				if(!StringUtils.isEmpty(chequeDetail.getRecordType()) || StringUtils.isEmpty(tableType.getSuffix())){
					auditList.add(new AuditDetail(auditTranType, i+1, fields[0], fields[1],chequeDetail.getBefImage(), chequeDetail));
				}
			}
			ChequeDetail detail =  chequeHeader.getChequeDetailList().get(0);
	 ...
	 * the workFlow table by using getChequeHeaderDAO().delete with parameters
	 * product,"_Temp" 3) Audit the record in to AuditHeader and
	 * AdtChequeHeaders by using auditHeaderDAO.addAudit(auditHeader) for Work
	 * flow
	 * 
	 * @param AuditHeader
	...
	public AuditHeader  doReject(AuditHeader auditHeader) {
		...
		ChequeHeader chequeHeader = (ChequeHeader) auditHeader.getAuditDetail().getModelData();
		auditHeader.setAuditTranType(PennantConstants.TRAN_WF);
		auditHeader.setAuditDetails(getListAuditDetails(listDeletion(chequeHeader, TableType.TEMP_TAB, PennantConstants.TRAN_WF)));
		getChequeHeaderDAO().delete(chequeHeader,TableType.TEMP_TAB);
		//getAuditHeaderDAO().addAudit(auditHeader);
	 ...
	 * from getChequeHeaderDAO().getErrorDetail with Error ID and language as parameters. if any error/Warnings then assign
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage) {
		...
		if (chequeHeader.isNew() && chequeHeaderDAO.isDuplicateKey(chequeHeader.getHeaderID(),financeDetail.getFinReference(),
