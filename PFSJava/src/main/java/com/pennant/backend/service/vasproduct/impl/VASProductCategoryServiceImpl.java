 ...
 * 
 */
public class VASProductCategoryServiceImpl extends GenericService<VASProductCategory>
		implements VASProductCategoryService {
	private static final Logger logger = LogManager.getLogger(VASProductCategoryServiceImpl.class);
	 ...
	 * VasProductCategory/VasProductCategory_Temp by using VASProductCategoryDAO's save method b) Update the Record in
	...
	public AuditHeader saveOrUpdate(AuditHeader auditHeader) {
		...
		if (vASProductCategory.isNewRecord()) {
	 ...
	 * following actions a) DELETE Delete the record from the main table by using getVASProductCategoryDAO().delete with
	 * parameters vASProductCategory,"" b) NEW Add new record in to main table by using getVASProductCategoryDAO().save
	 * with parameters vASProductCategory,"" c) EDIT Update record in the main table by using
	 * getVASProductCategoryDAO().update with parameters vASProductCategory,"" 3) Delete the record from the workFlow
	 * table by using getVASProductCategoryDAO().delete with parameters vASProductCategory,"_Temp" 4) Audit the record
	 * in to AuditHeader and AdtVasProductCategory by using auditHeaderDAO.addAudit(auditHeader) for Work flow 5) Audit
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage, String method) {
		...
		if (vASProductCategory.isWorkflow()) {
			tempVASProductCategory = getVASProductCategoryDAO().getVASProductCategoryById(vASProductCategory.getId(),
					"_Temp");
		}
		VASProductCategory befVASProductCategory = getVASProductCategoryDAO()
				.getVASProductCategoryById(vASProductCategory.getId(), "");
		...
		if (vASProductCategory.isNewRecord()) { // for New record or new record into work flow
			...
			if (!vASProductCategory.isWorkflow()) { // With out Work flow for update and delete
				...
				if (befVASProductCategory == null) { // if records not exists in the main table
					...
					if (old_VASProductCategory != null
							&& !old_VASProductCategory.getLastMntOn().equals(befVASProductCategory.getLastMntOn())) {
				...
				if (tempVASProductCategory != null && old_VASProductCategory != null
						&& !old_VASProductCategory.getLastMntOn().equals(tempVASProductCategory.getLastMntOn())) {
					auditDetail
							.setErrorDetail(new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, valueParm));
				}
			}
