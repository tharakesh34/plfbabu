...
public class DeviationParamServiceImpl extends GenericService<DeviationParam> implements DeviationParamService {
	private static final Logger logger = LogManager.getLogger(DeviationParamServiceImpl.class);
	 ...
	 * DeviationParams/DeviationParams_Temp by using DeviationParamDAO's save method b) Update the Record in the table.
		...
		if (deviationParam.isNewRecord()) { // for New record or new record into work flow
			...
			if (!deviationParam.isWorkflow()) { // With out Work flow for update and delete
				...
				if (befDeviationParam == null) { // if records not exists in the main table
					...
					if (oldDeviationParam != null
							&& !oldDeviationParam.getLastMntOn().equals(befDeviationParam.getLastMntOn())) {
				...
				if (tempDeviationParam != null && oldDeviationParam != null
						&& !oldDeviationParam.getLastMntOn().equals(tempDeviationParam.getLastMntOn())) {
					auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(
							new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, valueParm), usrLanguage));
				}
			}
