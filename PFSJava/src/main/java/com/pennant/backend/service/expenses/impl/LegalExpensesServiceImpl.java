...
public class LegalExpensesServiceImpl extends GenericService<LegalExpenses> implements LegalExpensesService {
	 ...
	 * FinLegalExpenses/FinLegalExpenses_Temp by using LegalExpensesDAO's save method b) Update the Record in the table.
		...
		if (legalExpenses.isNewRecord()) { // for New record or new record into work flow
			...
			if (!legalExpenses.isWorkflow()) { // With out Work flow for update and delete
				...
				if (befLegalExpenses == null) { // if records not exists in the main table
					...
					if (oldLegalExpenses != null
							&& !oldLegalExpenses.getLastMntOn().equals(befLegalExpenses.getLastMntOn())) {
				...
				if (tempLegalExpenses != null && oldLegalExpenses != null
						&& !oldLegalExpenses.getLastMntOn().equals(tempLegalExpenses.getLastMntOn())) {
					auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(
							new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, valueParm), usrLanguage));
				}
			}
