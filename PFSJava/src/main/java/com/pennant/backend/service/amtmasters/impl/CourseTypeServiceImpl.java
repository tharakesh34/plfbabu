...
public class CourseTypeServiceImpl extends GenericService<CourseType> implements CourseTypeService {
	private static final Logger logger = LogManager.getLogger(CourseTypeServiceImpl.class);
	 ...
	 * Do Add or Update the Record a) Add new Record for the new record in the DB table AMTCourseType/AMTCourseType_Temp
	 * by using CourseTypeDAO's save method b) Update the Record in the table. based on the module workFlow
	 ...
	 * following actions a) DELETE Delete the record from the main table by using getCourseTypeDAO().delete with
	 * parameters courseType,"" b) NEW Add new record in to main table by using getCourseTypeDAO().save with parameters
	 * courseType,"" c) EDIT Update record in the main table by using getCourseTypeDAO().update with parameters
	 * courseType,"" 3) Delete the record from the workFlow table by using getCourseTypeDAO().delete with parameters
	 * courseType,"_Temp" 4) Audit the record in to AuditHeader and AdtAMTCourseType by using
	 ...
	 * from getCourseTypeDAO().getErrorDetail with Error ID and language as parameters. if any error/Warnings then
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage, String method) {
		...
		if (courseType.isNewRecord()) { // for New record or new record into work flow
			...
			if (!courseType.isWorkflow()) { // With out Work flow for update and delete
				...
				if (tempCourseType != null && oldCourseType != null
						&& !oldCourseType.getLastMntOn().equals(tempCourseType.getLastMntOn())) {
					auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(
							new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, valueParm), usrLanguage));
				}
			}
