...
public class BuilderCompanyServiceImpl extends GenericService<BuilderCompany> implements BuilderCompanyService {
	 ...
	 * BuilderCompany/BuilderCompany_Temp by using BuilderCompanyDAO's save method b) Update the Record in the table.
	 ...
	 * following actions a) DELETE Delete the record from the main table by using getBuilderCompanyDAO().delete with
	 * parameters builderCompany,"" b) NEW Add new record in to main table by using getBuilderCompanyDAO().save with
	 * parameters builderCompany,"" c) EDIT Update record in the main table by using getBuilderCompanyDAO().update with
	 * parameters builderCompany,"" 3) Delete the record from the workFlow table by using getBuilderCompanyDAO().delete
	 * with parameters builderCompany,"_Temp" 4) Audit the record in to AuditHeader and AdtBuilderCompany by using
	...
	public AuditHeader doApprove(AuditHeader auditHeader) {
		...
		if (builderCompany.getRecordType().equals(PennantConstants.RECORD_TYPE_DEL)) {
			...
			if (builderCompany.getRecordType().equals(PennantConstants.RECORD_TYPE_NEW)) {
	 ...
	 * from getBuilderCompanyDAO().getErrorDetail with Error ID and language as parameters. if any error/Warnings then
