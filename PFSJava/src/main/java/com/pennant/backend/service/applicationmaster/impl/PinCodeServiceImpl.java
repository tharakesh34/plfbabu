...
public class PinCodeServiceImpl extends GenericService<PinCode> implements PinCodeService {
	 ...
	 * PinCodesDAO's save method b) Update the Record in the table. based on the module workFlow Configuration. by using
	 ...
	 * following actions a) DELETE Delete the record from the main table by using getPinCodeDAO().delete with parameters
	 * pinCode,"" b) NEW Add new record in to main table by using getPinCodeDAO().save with parameters pinCode,"" c)
	 * EDIT Update record in the main table by using getPinCodeDAO().update with parameters pinCode,"" 3) Delete the
	 * record from the workFlow table by using getPinCodeDAO().delete with parameters pinCode,"_Temp" 4) Audit the
	 * record in to AuditHeader and AdtPinCodes by using auditHeaderDAO.addAudit(auditHeader) for Work flow 5) Audit the
