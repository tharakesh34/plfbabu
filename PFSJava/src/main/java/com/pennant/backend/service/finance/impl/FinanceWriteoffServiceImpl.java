...
public class FinanceWriteoffServiceImpl  extends GenericFinanceDetailService  implements FinanceWriteoffService {
	 ...
	 */
	@Override
	public FinanceWriteoffHeader getFinanceWriteoffDetailById(String finReference, String type, String userRole, String procEdtEvent) {
		...
		if (scheduleData.getFinanceMain() != null) {
			...
			scheduleData.setDisbursementDetails(getFinanceDisbursementDAO().getFinanceDisbursementDetails(finReference, "", false));

			scheduleData.setFeeRules(getFinFeeChargesDAO().getFeeChargesByFinRef(finReference,FinanceConstants.FINSER_EVENT_WRITEOFF, false, ""));
			...
			//=======================================
			Long accSetId = getFinTypeAccountingDAO().getAccountSetID(scheduleData.getFinanceType().getFinType(), AccountEventConstants.ACCEVENT_WRITEOFF);
	...
	public AuditHeader saveOrUpdate(AuditHeader aAuditHeader) throws PFFInterfaceException, IllegalAccessException, InvocationTargetException {
		...
		//=======================================
		if (header.getFinanceDetail().getStageAccountingList() != null && header.getFinanceDetail().getStageAccountingList().size() > 0) {
		...
		if(!financeMain.isWorkflow()){
			...
			DataSet dataSet = AEAmounts.createDataSet(financeMain, AccountEventConstants.ACCEVENT_WRITEOFF, curBDay, financeMain.getMaturityDate());		
			amountCodes = AEAmounts.procAEAmounts(financeMain, scheduleData.getFinanceScheduleDetails(),profitDetail, curBDay);
	...
	public AuditHeader doReject(AuditHeader auditHeader) throws PFFInterfaceException {
		...
		// Save Document Details
		if (header.getFinanceDetail().getDocumentDetailsList() != null && header.getFinanceDetail().getDocumentDetailsList().size() > 0) {
		...
		getFinFeeChargesDAO().deleteChargesBatch(financeMain.getFinReference(), header.getFinanceDetail().getModuleDefiner() ,false, "_Temp");

		// Checklist Details delete
		//=======================================
		auditHeader.getAuditDetails().addAll(getCheckListDetailService().delete(header.getFinanceDetail(), "_Temp", tranType));
	...
	public AuditHeader doApprove(AuditHeader aAuditHeader) throws PFFInterfaceException, IllegalAccessException, InvocationTargetException {
		...
		profitDetail = AEAmounts.calProfitDetails(financeMain, scheduleData.getFinanceScheduleDetails(), profitDetail, curBDay);
		
		AEAmountCodes amountCodes =  AEAmounts.procCalAEAmounts(financeMain, profitDetail, curBDay);
		DataSet dataSet = AEAmounts.createDataSet(financeMain, AccountEventConstants.ACCEVENT_WRITEOFF, curBDay, financeMain.getMaturityDate());		
		...
		// Save Fee Charges List
		//=======================================
		saveFeeChargeList(header.getFinanceDetail().getFinScheduleData(),header.getFinanceDetail().getModuleDefiner(), false,"");
		...
		getFinStageAccountingLogDAO().update(financeMain.getFinReference(), header.getFinanceDetail().getModuleDefiner(), false);
