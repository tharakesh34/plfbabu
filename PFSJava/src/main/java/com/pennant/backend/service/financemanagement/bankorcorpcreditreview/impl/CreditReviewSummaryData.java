...
public class CreditReviewSummaryData {
	...
	public Map<String, String> setDataMap(long custID, Set<Long> custIds, int year, int noOfYears, String custCtgType,
		...
		if (isEnquiry) {
			detailedMap = this.creditApplicationReviewService.getListCreditReviewSummaryByCustId(custID, noOfYears + 1,
					year, "");
			coAppDetailedMap.put(custID, detailedMap);
			for (Long coAppcustID : custIds) {

				Map<String, List<FinCreditReviewSummary>> codetailedMap = this.creditApplicationReviewService
						.getListCreditReviewSummaryByCustId(coAppcustID, noOfYears + 1, year, "");
				coAppDetailedMap.put(coAppcustID, codetailedMap);
		...
		// This data is expected to come from Finance Main Dialog (Mainly Loan Details,
		// banking details and obligations)
		// If there is no external data, no need to load the data to Map
		// If this screen is not loaded from Finance Main tabs data will be empty and if
		// any ratios/cells using these variables will not work
			...
			if (externalDataMap != null && externalDataMap.size() > 0) {
				for (Entry<String, String> entry : externalDataMap.entrySet()) {
					if (entry.getKey().startsWith("EXT_")) {
						scriptDataMap.put(entry.getKey(), externalDataMap.get(entry.getKey()));
			...
			if (detailsMap.size() > 0) {
				for (FinCreditRevCategory fcrcy : listOfFinCreditRevCategory) {
					...
					if (detailedMap.get(String.valueOf(dataYear)) != null) {
						List<FinCreditReviewSummary> listOfCreditReviewSummary = this.detailsMap
								.get(String.valueOf(dataYear));
						...
						for (FinCreditReviewSummary finCreditReviewSummary : listOfCreditReviewSummary) {
							scriptDataMap.put("YM" + finCreditReviewSummary.getSubCategoryCode(),
						...
						for (FinCreditRevSubCategory finCreditRevSubCategory : listOfFinCreditRevSubCategory) {
							scriptDataMap.put("YM" + finCreditRevSubCategory.getSubCategoryCode(), BigDecimal.ZERO);
						}
					}
						...
						if (listOfCreditReviewSummary.size() > 0) {
							for (int k = 0; k < listOfCreditReviewSummary.size(); k++) {
								...
								if (!dataMap.containsKey("lovDescCcyEditField")) {
									dataMap.put("lovDescCcyEditField",
											String.valueOf(creditReviewSummary.getLovDescCcyEditField()));
								}

								scriptDataMap.put("EXCHANGE", creditReviewSummary.getLovDescConversionRate());
								scriptDataMap.put("NoOfShares", creditReviewSummary.getLovDescNoOfShares());
								scriptDataMap.put("MarketPrice", creditReviewSummary.getLovDescMarketPrice());
								String value = "--";
								if (!itemTotCalMap.keySet().contains(subCategoryCode)) {
									...
									scriptDataMap.put("Y" + (years) + subCategoryCode,
											!("--").equals(value) ? formateAmount(new BigDecimal(value), 2)
													: BigDecimal.ZERO);
									dataMap.put("Y" + (years) + "_" + subCategoryCode, value);
									if (noOfYears == j) {
										scriptDataMap.put("Y" + (years) + "DIVCOUNT", BigDecimal.ONE);
									} else {
										scriptDataMap.put("Y" + (years) + "DIVCOUNT", new BigDecimal(2));
									}
								}
							}
						} else {
							for (int r = 0; r < listOfFinCreditRevSubCategoryRatio.size(); r++) {
								...
								String subCategoryCode = finCreditRevSubCategory.getSubCategoryCode();
								if (!itemTotCalMap.keySet().contains(subCategoryCode)) {
									scriptDataMap.put("Y" + (years) + subCategoryCode, BigDecimal.ZERO);
							...
							if (itemTotCalMap.keySet().contains(subCategoryCode)) {
								...
								try {
									if (StringUtils.isBlank(itemTotCalMap.get(subCategoryCode))) {
										...
										value = scriptEngine.getResultAsString(
						...
						for (int p = 0; p < listOfFinCreditRevSubCategoryRatio.size(); p++) {
							...
							if (itemRuleMap.keySet().contains(subCategoryCode)) {
								...
								if (this.ratioFlag) {
									scriptDataMap.put("Y" + (years) + subCategoryCode,
											!value.contains("--") ? new BigDecimal(value) : BigDecimal.ZERO);
									dataMap.put("Y" + (noOfYears - l) + "_" + subCategoryCode, String
											.valueOf(!value.contains("--") ? new BigDecimal(value) : BigDecimal.ZERO));
								} else if (required) {
									dataMap.put("RY" + (noOfYears - l) + "_" + subCategoryCode, String
											.valueOf(!value.contains("--") ? new BigDecimal(value) : BigDecimal.ZERO));
								}
							}
						}
							...
							for (int p = 0; p < listOfFinCreditRevSubCategoryRatio.size(); p++) {
								...
								try {
									if (dataMap.get("Y" + (years - 1) + "_" + subCategoryCode) != null) {
										ratioCalValPrev = new BigDecimal(
												dataMap.get("Y" + (years - 1) + "_" + subCategoryCode));
									} else {
										ratioCalValPrev = BigDecimal.ZERO;
									}
									if (dataMap.get("Y" + (years) + "_" + subCategoryCode) != null) {
										ratioCalValCurr = new BigDecimal(
												dataMap.get("Y" + (years) + "_" + subCategoryCode));
									} else {
										ratioCalValCurr = BigDecimal.ZERO;
									}
									scriptDataMap.put("ratioCalValPrev", ratioCalValPrev);
									scriptDataMap.put("ratioCalValCurr", ratioCalValCurr);
									value = scriptEngine.getResultAsString(
											"Result=((ratioCalValCurr-ratioCalValPrev)/ratioCalValPrev)*100;",
											scriptDataMap);
									if ("NaN".equals(value) || value.contains("Infinity")) {
										value = "--";
									}
	...
	public Map<String, String> setDataMap1(Long custId, int year, int noOfYears, boolean required,
			Map<String, String> externalDataMap, List<FinCreditRevCategory> listOfFinCreditRevCategory) {
		logger.debug("Entering");

		Map<String, Object> scriptDataMap = new HashMap<String, Object>();
		Map<String, List<FinCreditReviewSummary>> detailedMap = new HashMap<>();
		List<FinCreditRevSubCategory> listOfFinCreditRevSubCategoryRatio = null;
		dataMap = new HashMap<String, String>();
		itemTotCalMap = new HashMap<String, String>();
					...
					for (FinCreditRevCategory fcrcy : listOfFinCreditRevCategory) {
						...
						for (int j = noOfYears; j >= 1; j--) {
							...
							if (listOfCreditReviewSummary.size() > 0) {
								...
								for (int r = 0; r < listOfFinCreditRevSubCategoryRatio.size(); r++) {
									FinCreditRevSubCategory finCreditRevSubCategory = listOfFinCreditRevSubCategory
											.get(r);
									String subCategoryCode = finCreditRevSubCategory.getSubCategoryCode();
									if (!itemTotCalMap.keySet().contains(subCategoryCode)) {
										scriptDataMap.put("Y" + (years) + subCategoryCode, BigDecimal.ZERO);
										dataMap.put("Y" + (years) + "_" + subCategoryCode,
								...
								if (itemTotCalMap.keySet().contains(subCategoryCode)) {
									...
									try {
										if (StringUtils.isBlank(itemTotCalMap.get(subCategoryCode))) {
											...
											value = scriptEngine
													.getResultAsString(replaceYear(itemTotCalMap.get(subCategoryCode),
															years, noOfYearsToShow), scriptDataMap);
											if ("NaN".equals(value) || value.contains("Infinity")) {
												value = "--";
											}
										}
						...
						for (int l = noOfYears; l >= 1; l--) {
							...
							for (int p = 0; p < listOfFinCreditRevSubCategoryRatio.size(); p++) {
								...
								if (itemRuleMap.keySet().contains(subCategoryCode)) {
									...
									try {
										if (StringUtils.isNotEmpty(itemRuleMap.get(subCategoryCode))) {
											value = scriptEngine
													.getResultAsString(replaceYear(itemRuleMap.get(subCategoryCode),
															years, noOfYearsToShow), scriptDataMap);
											if ("NaN".equals(value) || value.contains("Infinity")) {
												value = "--";
											}
									...
									if (this.ratioFlag) {
										scriptDataMap.put("Y" + (years) + subCategoryCode,
												!value.contains("--") ? new BigDecimal(value) : BigDecimal.ZERO);
										dataMap.put("Y" + (noOfYears - l) + "_" + subCategoryCode, String.valueOf(
												!value.contains("--") ? new BigDecimal(value) : BigDecimal.ZERO));
									} else if (required) {
										dataMap.put("RY" + (noOfYears - l) + "_" + subCategoryCode, String.valueOf(
												!value.contains("--") ? new BigDecimal(value) : BigDecimal.ZERO));
									}
								}
							}
							...
							if (noOfYears != l) {
								...
								for (int p = 0; p < listOfFinCreditRevSubCategoryRatio.size(); p++) {
									...
									if (required) {
										dataMap.put("CY" + (years) + "_" + subCategoryCode, String.valueOf(
												!value.contains("--") ? new BigDecimal(value) : BigDecimal.ZERO));
									}
								}
							}
