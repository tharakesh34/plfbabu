...
public class VehicleModelServiceImpl extends GenericService<VehicleModel> implements VehicleModelService {
	private static final Logger logger = LogManager.getLogger(VehicleModelServiceImpl.class);
	 ...
	 * AMTVehicleModel/AMTVehicleModel_Temp by using VehicleModelDAO's save method b) Update the Record in the table.
	 * based on the module workFlow Configuration. by using VehicleModelDAO's update method 3) Audit the record in to
	 * AuditHeader and AdtAMTVehicleModel by using auditHeaderDAO.addAudit(auditHeader)
	 * 
	 * @param AuditHeader
	 *            (auditHeader)
	 * @return auditHeader
	 ...
	 * AMTVehicleModel by using VehicleModelDAO's delete method with type as Blank 3) Audit the record in to AuditHeader
	 * and AdtAMTVehicleModel by using auditHeaderDAO.addAudit(auditHeader)
	 * 
	 * @param AuditHeader
	 *            (auditHeader)
	 * @return auditHeader
	...
	/**
	 * getApprovedVehicleModelById fetch the details by using VehicleModelDAO's getVehicleModelById method . with
	 * parameter id and type as blank. it fetches the approved records from the AMTVehicleModel.
	 * 
	 * @param id
	 *            (int)
	 * @return VehicleModel
	 ...
	 * following actions a) DELETE Delete the record from the main table by using getVehicleModelDAO().delete with
	 * parameters vehicleModel,"" b) NEW Add new record in to main table by using getVehicleModelDAO().save with
	 * parameters vehicleModel,"" c) EDIT Update record in the main table by using getVehicleModelDAO().update with
	 * parameters vehicleModel,"" 3) Delete the record from the workFlow table by using getVehicleModelDAO().delete with
	 * parameters vehicleModel,"_Temp" 4) Audit the record in to AuditHeader and AdtAMTVehicleModel by using
	 * auditHeaderDAO.addAudit(auditHeader) for Work flow 5) Audit the record in to AuditHeader and AdtAMTVehicleModel
	 ...
	 * workFlow table by using getVehicleModelDAO().delete with parameters vehicleModel,"_Temp" 3) Audit the record in
	 * to AuditHeader and AdtAMTVehicleModel by using auditHeaderDAO.addAudit(auditHeader) for Work flow
	 * 
	 * @param AuditHeader
	 *            (auditHeader)
	 * @return auditHeader
	 ...
	 * from getVehicleModelDAO().getErrorDetail with Error ID and language as parameters. if any error/Warnings then
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage, String method) {
		...
		if (vehicleModel.isWorkflow()) {
			tempVehicleModel = getVehicleModelDAO().getVehicleModelById(vehicleModel.getVehicleModelId(),
					vehicleModel.getVehicleManufacturerId(), "_Temp");
		}
		VehicleModel befVehicleModel = getVehicleModelDAO().getVehicleModelById(vehicleModel.getVehicleModelId(),
				vehicleModel.getVehicleManufacturerId(), "");
			...
			if (!vehicleModel.isWorkflow()) { // With out Work flow for update and delete
				...
				if (befVehicleModel == null) { // if records not exists in the main table
					...
					if (oldVehicleModel != null
							&& !oldVehicleModel.getLastMntOn().equals(befVehicleModel.getLastMntOn())) {
				...
				if (tempVehicleModel != null && oldVehicleModel != null
						&& !oldVehicleModel.getLastMntOn().equals(tempVehicleModel.getLastMntOn())) {
					auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(
							new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, valueParm), usrLanguage));
				}
			}
		...
		if (vehiclemodel != null) {
			...
			errParm1[0] = PennantJavaUtil.getLabel("label_ManufacturerId") + ":" + valueParm1[0] + ","
					+ PennantJavaUtil.getLabel("label_ModelDesc") + ":" + valueParm1[1];
