...
public class CustomerPRelationValidation {
	...
	public List<AuditDetail> pRelationListValidation(List<AuditDetail> auditDetails, String method,
			String usrLanguage) {
	...
	private AuditDetail validate(AuditDetail auditDetail, String method, String usrLanguage) {
		...
		if (customerPRelation.isWorkflow()) {
			tempCustomerPRelation = getCustomerPRelationDAO().getCustomerPRelationByID(customerPRelation.getId(),
					customerPRelation.getPRCustPRSNo(), "_Temp");
		}

		CustomerPRelation befCustomerPRelation = getCustomerPRelationDAO()
				.getCustomerPRelationByID(customerPRelation.getId(), customerPRelation.getPRCustPRSNo(), "");
		...
		if (customerPRelation.isNewRecord()) { // for New record or new record into work flow
			...
			if (!customerPRelation.isWorkflow()) {// With out Work flow only new records
				...
				if (customerPRelation.getRecordType().equals(PennantConstants.RECORD_TYPE_NEW)) { // if records type is
																									// new
					if (befCustomerPRelation != null || tempCustomerPRelation != null) { // if records already exists in
																							// the main table
						auditDetail.setErrorDetail(new ErrorDetail(PennantConstants.KEY_FIELD, "41001", errParm, null));
					}
			...
			if (!customerPRelation.isWorkflow()) { // With out Work flow for update and delete
				...
				if (befCustomerPRelation == null) { // if records not exists in the main table
					...
					if (oldCustomerPRelation != null
							&& !oldCustomerPRelation.getLastMntOn().equals(befCustomerPRelation.getLastMntOn())) {
