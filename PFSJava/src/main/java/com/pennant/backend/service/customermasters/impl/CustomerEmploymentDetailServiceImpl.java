...
public class CustomerEmploymentDetailServiceImpl extends GenericService<CustomerEmploymentDetail> implements CustomerEmploymentDetailService {
	...
	*/
	 ...
	 */
	@Override
	public AuditDetail doValidations(CustomerEmploymentDetail custEmpDetails) {
		...
		if (custEmpDetails != null) {
			...
			if (custEmpDetails.getCustEmpFrom() != null && custEmpDetails.getCustEmpFrom().compareTo(DateUtility.getAppDate()) != -1 || SysParamUtil.getValueAsDate("APP_DFT_START_DATE").compareTo(custEmpDetails.getCustEmpFrom()) >= 0) {
				ErrorDetails errorDetail = new ErrorDetails();
				String[] valueParm = new String[2];
				valueParm[0] = "employment startDate" + DateUtility.formatDate(custEmpDetails.getCustEmpFrom(), PennantConstants.XMLDateFormat);
				errorDetail = ErrorUtil.getErrorDetail(new ErrorDetails("90319", "", valueParm), "EN");
				auditDetail.setErrorDetail(errorDetail);
			}
		}
