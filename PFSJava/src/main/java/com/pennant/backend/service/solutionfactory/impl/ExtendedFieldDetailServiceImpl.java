...
public class ExtendedFieldDetailServiceImpl extends GenericService<ExtendedFieldDetail>
	 ...
	 * ExtendedFieldDetail/ExtendedFieldDetail_Temp by using ExtendedFieldDetailDAO's save method b) Update the Record
	...
	private AuditHeader getAuditDetails(AuditHeader auditHeader, String method) {
		...
		if (extendedFieldHeader.getExtendedFieldDetails() != null
				&& extendedFieldHeader.getExtendedFieldDetails().size() > 0) {
			auditDetailMap.put("ExtendedFieldHeader",
	 ...
	 * @return
	 */
	private List<AuditDetail> setExtendedFieldsAuditData(ExtendedFieldHeader extendedFldHeader, String auditTranType,
			String method) {
		logger.debug("Entering");
	...
	public ExtendedFieldHeader getExtendedFieldHeaderById(ExtendedFieldHeader extendedFieldHeader) {
		extendedFieldHeader.setExtendedFieldDetails(
				getExtendedFieldDetailDAO().getExtendedFieldDetailById(extendedFieldHeader.getId(), "_View"));
	 ...
	 * following actions a) DELETE Delete the record from the main table by using getExtendedFieldDetailDAO().delete
	 * with parameters extendedFieldDetail,"" b) NEW Add new record in to main table by using
	 * getExtendedFieldDetailDAO().save with parameters extendedFieldDetail,"" c) EDIT Update record in the main table
	 * by using getExtendedFieldDetailDAO().update with parameters extendedFieldDetail,"" 3) Delete the record from the
	 * workFlow table by using getExtendedFieldDetailDAO().delete with parameters extendedFieldDetail,"_Temp" 4) Audit
	...
	public AuditHeader doApprove(AuditHeader auditHeader) {
		...
		auditDetailList.addAll(
				getListAuditDetails(listDeletion(extendedFieldHeader, "_Temp", auditHeader.getAuditTranType())));
	 ...
	 * workFlow table by using getExtendedFieldDetailDAO().delete with parameters extendedFieldDetail,"_Temp" 3) Audit
	 * the record in to AuditHeader and AdtExtendedFieldDetail by using auditHeaderDAO.addAudit(auditHeader) for Work
	 * flow
	 * 
	 * @param AuditHeader
	...
	private AuditDetail validate(AuditDetail auditDetail, String usrLanguage, String method) {
		...
		if (details.isNew()) { // for New record or new record into work flow
			...
			if (!details.isWorkflow()) { // With out Work flow for update and delete
				...
				if (tempExtendedFieldDetail != null && oldExtendedFieldDetail != null
		...
		if (StringUtils.isBlank(extendedFieldHeader.getModuleName())) {
			...
			if (StringUtils.equals(extendedFieldHeader.getModuleName(), ExtendedFieldConstants.MODULE_CUSTOMER)) {
				if (!StringUtils.equals(extendedFieldHeader.getSubModuleName(), PennantConstants.PFF_CUSTCTG_INDIV)
						&& !StringUtils.equals(extendedFieldHeader.getSubModuleName(),
	 ...
	 */
	@Override
	public ExtendedFieldHeader getExtendedFieldHeaderByModuleName(String moduleName, String subModuleName,
			String type) {
