...
public class CustomerChequeInfoValidation {
	...
	public List<AuditDetail> chequeInfoListValidation(List<AuditDetail> auditDetails, String method,
			String usrLanguage) {
	...
	private AuditDetail validate(AuditDetail auditDetail, String method, String usrLanguage) {
		...
		if (customerChequeInfo.isWorkflow()) {
			tempCustomerChequeInfo = getCustomerChequeInfoDAO().getCustomerChequeInfoById(customerChequeInfo.getId(),
					customerChequeInfo.getChequeSeq(), "_Temp");
		}

		CustomerChequeInfo befCustomerChequeInfo = getCustomerChequeInfoDAO()
				.getCustomerChequeInfoById(customerChequeInfo.getId(), customerChequeInfo.getChequeSeq(), "");
		...
		errParm[0] = PennantJavaUtil.getLabel("CustomerChequeInfo") + " , " + PennantJavaUtil.getLabel("label_CustCIF")
				+ ":" + valueParm[0] + " and ";
		...
		if (customerChequeInfo.isNewRecord()) { // for New record or new record into work flow
			...
			if (!customerChequeInfo.isWorkflow()) {// With out Work flow only new records
				...
				if (customerChequeInfo.getRecordType().equals(PennantConstants.RECORD_TYPE_NEW)) { // if records type is
																									// new
					if (befCustomerChequeInfo != null || tempCustomerChequeInfo != null) { // if records already exists
																							// in the main table
						auditDetail.setErrorDetail(new ErrorDetail(PennantConstants.KEY_FIELD, "41001", errParm, null));
					}
			...
			if (!customerChequeInfo.isWorkflow()) { // With out Work flow for update and delete
				...
				if (befCustomerChequeInfo == null) { // if records not exists in the main table
					...
					if (oldCustomerChequeInfo != null
							&& !oldCustomerChequeInfo.getLastMntOn().equals(befCustomerChequeInfo.getLastMntOn())) {
				...
				if (tempCustomerChequeInfo != null && oldCustomerChequeInfo != null
						&& !oldCustomerChequeInfo.getLastMntOn().equals(tempCustomerChequeInfo.getLastMntOn())) {
					auditDetail.setErrorDetail(new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, null));
				}
