...
public class OverdueChargeDetailValidation {
	...
	public List<AuditDetail> overDueChargeDetailsListValidation(List<AuditDetail> auditDetails, String method,
			String usrLanguage) {
	...
	private AuditDetail validate(AuditDetail auditDetail, String method, String usrLanguage) {
		...
		if (overdueChargeDetail.isWorkflow()) {
			tempChargeDetail = getDetailsDAO().getOverdueChargeDetailById(overdueChargeDetail.getoDCRuleCode(),
					overdueChargeDetail.getoDCCustCtg(), "_Temp");
		}

		OverdueChargeDetail befChargeDetail = getDetailsDAO().getOverdueChargeDetailById(
				overdueChargeDetail.getoDCRuleCode(), overdueChargeDetail.getoDCCustCtg(), "");
		...
		if (overdueChargeDetail.isNewRecord()) { // for New record or new record into work flow
			...
			if (!overdueChargeDetail.isWorkflow()) { // With out Work flow for update and delete
				...
				if (befChargeDetail == null) { // if records not exists in the main table
					...
					if (oldChargeDetail != null
							&& !oldChargeDetail.getLastMntOn().equals(befChargeDetail.getLastMntOn())) {
				...
				if (tempChargeDetail != null && oldChargeDetail != null
						&& !oldChargeDetail.getLastMntOn().equals(tempChargeDetail.getLastMntOn())) {
					auditDetail.setErrorDetail(new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, null));
				}
