...
public class EntityServiceImpl extends GenericService<Entity> implements EntityService {
	 ...
	 * Do Add or Update the Record a) Add new Record for the new record in the DB table Entities/Entities_Temp by using
	 * EntitiesDAO's save method b) Update the Record in the table. based on the module workFlow Configuration. by using
	 ...
	 * following actions a) DELETE Delete the record from the main table by using getEntityDAO().delete with parameters
	 * entity,"" b) NEW Add new record in to main table by using getEntityDAO().save with parameters entity,"" c) EDIT
	 * Update record in the main table by using getEntityDAO().update with parameters entity,"" 3) Delete the record
	 * from the workFlow table by using getEntityDAO().delete with parameters entity,"_Temp" 4) Audit the record in to
	...
	public AuditHeader doApprove(AuditHeader auditHeader) {
		...
		if (entity.getRecordType().equals(PennantConstants.RECORD_TYPE_DEL)) {
			...
			if (entity.getRecordType().equals(PennantConstants.RECORD_TYPE_NEW)) {
