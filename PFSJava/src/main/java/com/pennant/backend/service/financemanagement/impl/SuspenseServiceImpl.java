...
public class SuspenseServiceImpl extends GenericFinanceDetailService implements SuspenseService {
	...
	public FinanceSuspHead getFinanceSuspHeadById(String finRef,boolean isEnquiry,String userRole, String procEdtEvent) {
		...
		if(isEnquiry){
			...
			suspHead.setSuspPostingsList(getPostingsDAO().getPostingsByFinRefAndEvent(
					suspHead.getFinReference(), "'PIS_NORM','NORM_PIS'",true,""));
		}
		...
		if (scheduleData.getFinanceMain() != null) {
			...
			scheduleData.setFinanceScheduleDetails(getFinanceScheduleDetailDAO().getFinScheduleDetails(finRef, "", false));
			scheduleData.setDisbursementDetails(getFinanceDisbursementDAO().getFinanceDisbursementDetails(finRef, "", false));
	...
	public AuditHeader saveOrUpdate(AuditHeader auditHeader) throws PFFInterfaceException, IllegalAccessException, InvocationTargetException {
		...
		//=======================================
		if (financeSuspHead.getFinanceDetail().getStageAccountingList() != null && financeSuspHead.getFinanceDetail().getStageAccountingList().size() > 0) {
		...
		if(!financeSuspHead.isWorkflow()){
			...
			amountCodes = AEAmounts.procAEAmounts(financeMain, financeSuspHead.getFinanceDetail().getFinScheduleData().getFinanceScheduleDetails(),
		...
		if (StringUtils.isNotBlank(tableType)) {
			getFinFeeChargesDAO().deleteChargesBatch(financeMain.getFinReference(), financeSuspHead.getFinanceDetail().getModuleDefiner() ,false, tableType);
		}
		saveFeeChargeList(financeSuspHead.getFinanceDetail().getFinScheduleData(),financeSuspHead.getFinanceDetail().getModuleDefiner(), false,tableType);
		...
		if (financeSuspHead.getFinanceDetail().getDocumentDetailsList() != null
			...
			details = processingDocumentDetailsList(details, tableType,
					financeSuspHead.getFinanceDetail().getFinScheduleData().getFinanceMain(),financeSuspHead.getFinanceDetail().getModuleDefiner());
			auditDetails.addAll(details);
		}

		// set Finance Check List audit details to auditDetails
		//=======================================
		if (financeSuspHead.getFinanceDetail().getFinanceCheckList() != null && !financeSuspHead.getFinanceDetail().getFinanceCheckList().isEmpty()) {
	...
	public AuditHeader doApprove(AuditHeader auditHeader) throws PFFInterfaceException, IllegalAccessException, InvocationTargetException {
		...
		// Save Fee Charges List
		//=======================================
		saveFeeChargeList(financeSuspHead.getFinanceDetail().getFinScheduleData(),financeSuspHead.getFinanceDetail().getModuleDefiner(), false,"");
		...
		getFinFeeChargesDAO().deleteChargesBatch(financeMain.getFinReference(), financeSuspHead.getFinanceDetail().getModuleDefiner() ,false, "_Temp");

		finPftDetail = AEAmounts.calProfitDetails(financeMain, scheduleDetailList,finPftDetail, curBussDate);
		...
		getFinStageAccountingLogDAO().update(financeMain.getFinReference(),financeSuspHead.getFinanceDetail().getModuleDefiner(), false);
	...
	public AuditHeader  doReject(AuditHeader auditHeader) throws PFFInterfaceException {
		...
		// Save Document Details
		if (financeSuspHead.getFinanceDetail().getDocumentDetailsList() != null && financeSuspHead.getFinanceDetail().getDocumentDetailsList().size() > 0) {
	...
	private AuditDetail validation(AuditDetail auditDetail,String usrLanguage,String method){
		...
		if (financeSuspHead.isNew()){ // for New record or new record into work flow
			...
			if (!financeSuspHead.isWorkflow()){	// With out Work flow for update and delete
				...
				if (oldFinanceSuspHead!=null && tempFinanceSuspHead!=null && !oldFinanceSuspHead.getLastMntOn().equals(tempFinanceSuspHead.getLastMntOn())){ 
					auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(new ErrorDetails(PennantConstants.KEY_FIELD, "41005", errParm,valueParm), usrLanguage));
				}
			}
		}
