	...
	/*private AuditDetail validation(AuditDetail auditDetail,String usrLanguage,String method){
		...
		if (lovFieldDetail.isWorkflow()){
			tempLovFieldDetail = getLovFieldDetailDAO().getLovFieldDetailById(
					lovFieldDetail.getFieldCode(), lovFieldDetail.getFieldCodeValue(),"_Temp");
		}
		LovFieldDetail befLovFieldDetail= getLovFieldDetailDAO().getLovFieldDetailById(
				lovFieldDetail.getFieldCode(),lovFieldDetail.getFieldCodeValue(), "");
		...
		valueParm[0]=lovFieldDetail.getFieldCode();
		valueParm[1]=lovFieldDetail.getFieldCodeValue();
		errParm[0]=PennantJavaUtil.getLabel("label_FieldCode")+":"+valueParm[0];
		errParm[1]=PennantJavaUtil.getLabel("label_FieldCodeValue")+":"+valueParm[1];
		...
		if (lovFieldDetail.isNew()){ // for New record or new record into work flow
			...
			if (!lovFieldDetail.isWorkflow()){	// With out Work flow for update and delete
				...
				if (tempLovFieldDetail!=null  && oldLovFieldDetail!=null && !oldLovFieldDetail.getLastMntOn().equals(
		...
		if("doApprove".equals(StringUtils.trimToEmpty(method)) || !lovFieldDetail.isWorkflow()){
