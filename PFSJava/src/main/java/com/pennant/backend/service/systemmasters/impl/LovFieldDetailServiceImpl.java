 ...
 * 
 */
public class LovFieldDetailServiceImpl extends GenericService<LovFieldDetail> implements LovFieldDetailService {
	private static final Logger logger = LogManager.getLogger(LovFieldDetailServiceImpl.class);
	 ...
	 * RMTLovFieldDetail/RMTLovFieldDetail_Temp by using LovFieldDetailDAO's save method b) Update the Record in the
	 ...
	 * following actions a) DELETE Delete the record from the main table by using getLovFieldDetailDAO().delete with
	 * parameters lovFieldDetail,"" b) NEW Add new record in to main table by using getLovFieldDetailDAO().save with
	 * parameters lovFieldDetail,"" c) EDIT Update record in the main table by using getLovFieldDetailDAO().update with
	 * parameters lovFieldDetail,"" 3) Delete the record from the workFlow table by using getLovFieldDetailDAO().delete
	 * with parameters lovFieldDetail,"_Temp" 4) Audit the record in to AuditHeader and AdtRMTLovFieldDetail by using
	...
	public AuditHeader doApprove(AuditHeader auditHeader) {
		...
		BeanUtils.copyProperties((LovFieldDetail) auditHeader.getAuditDetail().getModelData(), lovFieldDetail);
		...
		if (!PennantConstants.RECORD_TYPE_NEW.equals(lovFieldDetail.getRecordType())) {
			auditHeader.getAuditDetail().setBefImage(lovFieldDetailDAO
					.getLovFieldDetailById(lovFieldDetail.getFieldCode(), lovFieldDetail.getFieldCodeValue(), ""));
		}
	 ...
	 * from getLovFieldDetailDAO().getErrorDetail with Error ID and language as parameters. if any error/Warnings then
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage) {
		...
		if (lovFieldDetail.isSystemDefault()) {
			int count = getLovFieldDetailDAO().getSystemDefaultCount(lovFieldDetail.getFieldCode(),
					lovFieldDetail.getFieldCodeValue());
			if (count > 0) {
				auditDetail.setErrorDetail(new ErrorDetail(PennantConstants.KEY_FIELD, "41010", new String[] {
						PennantJavaUtil.getLabel("label_FieldCodeId") + ":" + lovFieldDetail.getFieldCodeValue() },
						null));
			}
		}
