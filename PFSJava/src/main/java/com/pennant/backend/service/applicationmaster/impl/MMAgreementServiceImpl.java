...
public class MMAgreementServiceImpl extends GenericService<MMAgreement> implements MMAgreementService {
	 ...
	 * Do Add or Update the Record a) Add new Record for the new record in the DB table MMAgreement/MMAgreement_Temp by
	 * using MMAgreementDAO's save method b) Update the Record in the table. based on the module workFlow Configuration.
	 * by using MMAgreementDAO's update method 3) Audit the record in to AuditHeader and AdtMMAgreement by using
	 ...
	 * MMAgreement by using MMAgreementDAO's delete method with type as Blank 3) Audit the record in to AuditHeader and
	 * AdtMMAgreement by using auditHeaderDAO.addAudit(auditHeader)
	 * 
	 * @param AuditHeader
	 *            (auditHeader)
	 * @return auditHeader
	...
	/**
	 * getMMAgreementById fetch the details by using MMAgreementDAO's getMMAgreementById method.
	 * 
	 * @param id
	 *            (int)
	 * @param type
	 ...
	 * workFlow table by using getMMAgreementDAO().delete with parameters aMMAgreement,"_Temp" 3) Audit the record in to
	 * AuditHeader and AdtMMAgreement by using auditHeaderDAO.addAudit(auditHeader) for Work flow
	 * 
	 * @param AuditHeader
	 *            (auditHeader)
	 * @return auditHeader
	 ...
	 * from getMMAgreementDAO().getErrorDetail with Error ID and language as parameters. if any error/Warnings then
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage, String method) {
		...
		if (aMMAgreement.isWorkflow()) {
			tempMMAgreement = getmMAgreementDAO().getMMAgreementById(aMMAgreement.getMMAId(), "_Temp");
		}
			...
			if (!aMMAgreement.isWorkflow()) { // With out Work flow for update and delete
				...
				if (befMMAgreement == null) { // if records not exists in the main table
					...
					if (oldMMAgreement != null
							&& !oldMMAgreement.getLastMntOn().equals(befMMAgreement.getLastMntOn())) {
				...
				if (tempMMAgreement != null && oldMMAgreement != null
						&& !oldMMAgreement.getLastMntOn().equals(tempMMAgreement.getLastMntOn())) {
					auditDetail.setErrorDetail(new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, null));
				}
