...
public class CustomerRatingValidation {
	...
	private AuditDetail validate(AuditDetail auditDetail, String method, String usrLanguage) {
		...
		if (customerRating.isWorkflow()) {
			tempCustomerRating = getCustomerRatingDAO().getCustomerRatingByID(customerRating.getId(),
					customerRating.getCustRatingType(), "_Temp");
		}

		CustomerRating befCustomerRating = getCustomerRatingDAO().getCustomerRatingByID(customerRating.getId(),
		...
		errParm[0] = PennantJavaUtil.getLabel("Rating") + " , " + PennantJavaUtil.getLabel("label_CustCIF") + ":"
				+ valueParm[0] + " and ";
		...
		if (customerRating.isNewRecord()) { // for New record or new record into work flow
			...
			if (!customerRating.isWorkflow()) { // With out Work flow for update and delete
				...
				if (tempCustomerRating != null && oldCustomerRating != null
