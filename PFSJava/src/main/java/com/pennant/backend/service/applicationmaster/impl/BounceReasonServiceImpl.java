 ...
 * FILE HEADER *
 ********************************************************************************************
 * * FileName : BounceReasonServiceImpl.java * * Author : PENNANT TECHONOLOGIES * * Creation Date : 22-04-2017 * *
...
public class BounceReasonServiceImpl extends GenericService<BounceReason> implements BounceReasonService {
	 ...
	 * Do Add or Update the Record a) Add new Record for the new record in the DB table BounceReasons/BounceReasons_Temp
	 * by using BounceReasonsDAO's save method b) Update the Record in the table. based on the module workFlow
	 * Configuration. by using BounceReasonsDAO's update method 3) Audit the record in to AuditHeader and
	 * AdtBounceReasons by using auditHeaderDAO.addAudit(auditHeader)
	 * 
	 ...
	 * BounceReasons by using BounceReasonsDAO's delete method with type as Blank 3) Audit the record in to AuditHeader
	 * and AdtBounceReasons by using auditHeaderDAO.addAudit(auditHeader)
	 * 
	 * @param AuditHeader (auditHeader)
	 * @return auditHeader
	 */
	...
	/**
	 * getBounceReasons fetch the details by using BounceReasonsDAO's getBounceReasonsById method.
	 * 
	...
	/**
	 * getApprovedBounceReasonsById fetch the details by using BounceReasonsDAO's getBounceReasonsById method . with
	 * parameter id and type as blank. it fetches the approved records from the BounceReasons.
	 * 
	 * @param bounceID bounceID of the BounceReason. (String)
	 * @return BounceReasons
	 */
	 ...
	 * following actions a) DELETE Delete the record from the main table by using getBounceReasonDAO().delete with
	 * parameters bounceReason,"" b) NEW Add new record in to main table by using getBounceReasonDAO().save with
	 * parameters bounceReason,"" c) EDIT Update record in the main table by using getBounceReasonDAO().update with
	 * parameters bounceReason,"" 3) Delete the record from the workFlow table by using getBounceReasonDAO().delete with
	 * parameters bounceReason,"_Temp" 4) Audit the record in to AuditHeader and AdtBounceReasons by using
	 * auditHeaderDAO.addAudit(auditHeader) for Work flow 5) Audit the record in to AuditHeader and AdtBounceReasons by
	...
	public AuditHeader doApprove(AuditHeader auditHeader) {
		...
		if (bounceReason.getRecordType().equals(PennantConstants.RECORD_TYPE_DEL)) {
			...
			if (bounceReason.getRecordType().equals(PennantConstants.RECORD_TYPE_NEW)) {
	 ...
	 * workFlow table by using getBounceReasonDAO().delete with parameters bounceReason,"_Temp" 3) Audit the record in
	 * to AuditHeader and AdtBounceReasons by using auditHeaderDAO.addAudit(auditHeader) for Work flow
	 * 
	 * @param AuditHeader (auditHeader)
	 * @return auditHeader
	 */
	 ...
	 * from getBounceReasonDAO().getErrorDetail with Error ID and language as parameters. if any error/Warnings then
