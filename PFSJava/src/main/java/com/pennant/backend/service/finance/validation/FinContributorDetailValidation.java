...
public class FinContributorDetailValidation {
	...
	public List<AuditDetail> ContributorListValidation(List<AuditDetail> auditDetails, String method,
			String usrLanguage) {
		...
		if (contributorDetail.isNewRecord()) { // for New record or new record into work flow
			...
			if (!contributorDetail.isWorkflow()) { // With out Work flow for update and delete
				...
				if (befFinContributorDetail == null) { // if records not exists in the main table
					...
					if (oldFinContributorDetail != null
							&& !oldFinContributorDetail.getLastMntOn().equals(befFinContributorDetail.getLastMntOn())) {
				...
				if (tempFinContributorDetail != null && oldFinContributorDetail != null
						&& !oldFinContributorDetail.getLastMntOn().equals(tempFinContributorDetail.getLastMntOn())) {
					auditDetail.setErrorDetail(new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, null));
				}
			}
		}
