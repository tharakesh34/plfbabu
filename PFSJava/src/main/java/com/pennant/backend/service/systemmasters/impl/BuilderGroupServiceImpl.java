...
public class BuilderGroupServiceImpl extends GenericService<BuilderGroup> implements BuilderGroupService {
	private static final Logger logger = LogManager.getLogger(BuilderGroupServiceImpl.class);
	 ...
	 * Do Add or Update the Record a) Add new Record for the new record in the DB table BuilderGroup/BuilderGroup_Temp
	 * by using BuilderGroupDAO's save method b) Update the Record in the table. based on the module workFlow
	 ...
	 * following actions a) DELETE Delete the record from the main table by using getBuilderGroupDAO().delete with
	 * parameters builderGroup,"" b) NEW Add new record in to main table by using getBuilderGroupDAO().save with
	 * parameters builderGroup,"" c) EDIT Update record in the main table by using getBuilderGroupDAO().update with
	 * parameters builderGroup,"" 3) Delete the record from the workFlow table by using getBuilderGroupDAO().delete with
	 * parameters builderGroup,"_Temp" 4) Audit the record in to AuditHeader and AdtBuilderGroup by using
	...
	public AuditHeader doApprove(AuditHeader auditHeader) {
		...
		if (builderGroup.getRecordType().equals(PennantConstants.RECORD_TYPE_DEL)) {
			...
			if (builderGroup.getRecordType().equals(PennantConstants.RECORD_TYPE_NEW)) {
	 ...
	 * from getBuilderGroupDAO().getErrorDetail with Error ID and language as parameters. if any error/Warnings then
