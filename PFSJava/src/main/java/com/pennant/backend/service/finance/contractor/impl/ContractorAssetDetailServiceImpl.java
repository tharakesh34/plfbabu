 ...
 * 
 */
public class ContractorAssetDetailServiceImpl extends GenericService<ContractorAssetDetail>
		implements ContractorAssetDetailService {
	private static final Logger logger = LogManager.getLogger(ContractorAssetDetailServiceImpl.class);
	...
	/**
	 * @param contractorAssetDetailDAO
	 *            the contractorAssetDetailDAO to set
	 */
	 ...
	 * FinContractorAstDtls/FinContractorAstDtls_Temp by using ContractorAssetDetailDAO's save method b) Update the
	 * Record in the table. based on the module workFlow Configuration. by using ContractorAssetDetailDAO's update
	 * method 3) Audit the record in to AuditHeader and AdtFinContractorAstDtls by using
	...
	private AuditHeader saveOrUpdate(AuditHeader auditHeader, boolean online) {
		...
		String tableType = "";
		ContractorAssetDetail contractorAssetDetail = (ContractorAssetDetail) auditHeader.getAuditDetail()
				.getModelData();
	 ...
	 * FinContractorAstDtls by using ContractorAssetDetailDAO's delete method with type as Blank 3) Audit the record in
	 * to AuditHeader and AdtFinContractorAstDtls by using auditHeaderDAO.addAudit(auditHeader)
	 * 
	 * @param AuditHeader
	 *            (auditHeader)
	 * @return auditHeader
	...
	/**
	 * getContractorAssetDetailById fetch the details by using ContractorAssetDetailDAO's getContractorAssetDetailById
	 * method.
	 * 
	 * @param id
	 *            (String)
	...
	/**
	 * getApprovedContractorAssetDetailById fetch the details by using ContractorAssetDetailDAO's
	 * getContractorAssetDetailById method . with parameter id and type as blank. it fetches the approved records from
	 * the FinContractorAstDtls.
	 * 
	 * @param id
	 *            (String)
	 * @return ContractorAssetDetail
	 ...
	 * following actions a) DELETE Delete the record from the main table by using getContractorAssetDetailDAO().delete
	 * with parameters contractorAssetDetail,"" b) NEW Add new record in to main table by using
	 * getContractorAssetDetailDAO().save with parameters contractorAssetDetail,"" c) EDIT Update record in the main
	 * table by using getContractorAssetDetailDAO().update with parameters contractorAssetDetail,"" 3) Delete the record
	 * from the workFlow table by using getContractorAssetDetailDAO().delete with parameters
	 * contractorAssetDetail,"_Temp" 4) Audit the record in to AuditHeader and AdtFinContractorAstDtls by using
	 * auditHeaderDAO.addAudit(auditHeader) for Work flow 5) Audit the record in to AuditHeader and
	 * AdtFinContractorAstDtls by using auditHeaderDAO.addAudit(auditHeader) based on the transaction Type.
	 * 
	 * @param AuditHeader
	 *            (auditHeader)
	 * @return auditHeader
	...
	public AuditHeader doApprove(AuditHeader auditHeader) {
		...
		BeanUtils.copyProperties((ContractorAssetDetail) auditHeader.getAuditDetail().getModelData(),
	 ...
	 * workFlow table by using getContractorAssetDetailDAO().delete with parameters contractorAssetDetail,"_Temp" 3)
	 * Audit the record in to AuditHeader and AdtFinContractorAstDtls by using auditHeaderDAO.addAudit(auditHeader) for
	 * Work flow
	 * 
	 * @param AuditHeader
	 *            (auditHeader)
	 ...
	 * mismatch conditions Fetch the error details from getContractorAssetDetailDAO().getErrorDetail with Error ID and
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage, String method, boolean onlineRequest) {
		...
		if (contractorAssetDetail.isNew()) { // for New record or new record into work flow
			...
			if (!contractorAssetDetail.isWorkflow()) { // With out Work flow for update and delete
				...
				if (oldContractorAssetDetail != null
	...
	public void setContractorAssetDetails(FinanceDetail financeDetail, String tableType) {
		...
		financeDetail.setContractorAssetDetails(
				getContractorAssetDetailDAO().getContractorDetailDetailByFinRef(finReference, tableType));
		logger.debug("Leaving");
	}

	@Override
	public List<AuditDetail> saveOrUpdate(String finReference, List<ContractorAssetDetail> assetDetails,
		...
		for (ContractorAssetDetail assetDetail : assetDetails) {
			...
			auditDetails.add(new AuditDetail(auditTranType, auditDetails.size() + 1, fields[0], fields[1],
					assetDetail.getBefImage(), assetDetail));
		}
	...
	@Override
	public List<AuditDetail> doApprove(List<ContractorAssetDetail> assetDetails, String tableType,
	...
	@Override
	public List<AuditDetail> getAuditDetail(List<ContractorAssetDetail> contractorAssetDetails, FinanceMain financeMain,
		...
		for (int i = 0; i < contractorAssetDetails.size(); i++) {
			...
			if (!auditTranType.equals(PennantConstants.TRAN_WF)) {
				...
				} else if (contractorAssetDetail.getRecordType().equalsIgnoreCase(PennantConstants.RECORD_TYPE_DEL)
						|| contractorAssetDetail.getRecordType().equalsIgnoreCase(PennantConstants.RECORD_TYPE_CAN)) {
			...
			if (StringUtils.isNotEmpty(contractorAssetDetail.getRecordType())) {
				auditDetails.add(new AuditDetail(auditTranType, i + 1, fields[0], fields[1],
						contractorAssetDetail.getBefImage(), contractorAssetDetail));
			}
		}
