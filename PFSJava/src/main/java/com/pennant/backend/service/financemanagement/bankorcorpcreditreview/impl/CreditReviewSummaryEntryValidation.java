...
public class CreditReviewSummaryEntryValidation {
	...
	public List<AuditDetail> creditReviewSummaryListValidation(List<AuditDetail> auditDetails, String method,
			String usrLanguage) {
	...
	private AuditDetail validate(AuditDetail auditDetail, String method, String usrLanguage) {
		...
		if (creditReviewSummary.isWorkflow()) {
			tempCreditReviewSummary = getCreditReviewSummaryDAO().getCreditReviewSummaryById(
					creditReviewSummary.getSummaryId(), creditReviewSummary.getDetailId(), "_Temp");
		}
		FinCreditReviewSummary befcreditReviewSummary = getCreditReviewSummaryDAO()
				.getCreditReviewSummaryById(creditReviewSummary.getSummaryId(), creditReviewSummary.getDetailId(), "");
		...
		if (creditReviewSummary.isNewRecord()) { // for New record or new record into work flow
			...
			if (!creditReviewSummary.isWorkflow()) { // With out Work flow for update and delete
				...
				if (befcreditReviewSummary == null) { // if records not exists in the main table
					...
					if (oldCreditReviewSummary != null
							&& !oldCreditReviewSummary.getLastMntOn().equals(befcreditReviewSummary.getLastMntOn())) {
				...
				if (tempCreditReviewSummary != null && oldCreditReviewSummary != null
						&& !oldCreditReviewSummary.getLastMntOn().equals(tempCreditReviewSummary.getLastMntOn())) {
					auditDetail.setErrorDetail(new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, null));
				}
			}
		}
