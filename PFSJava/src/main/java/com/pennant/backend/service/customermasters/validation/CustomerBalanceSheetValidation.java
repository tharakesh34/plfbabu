...
public class CustomerBalanceSheetValidation {
	...
	public List<AuditDetail> balanceSheetListValidation(List<AuditDetail> auditDetails, String method,
			String usrLanguage) {
	...
	private AuditDetail validate(AuditDetail auditDetail, String method, String usrLanguage) {
		...
		CustomerBalanceSheet befCustomerBalanceSheet = getCustomerBalanceSheetDAO()
				.getCustomerBalanceSheetById(customerBalanceSheet.getId(), customerBalanceSheet.getCustId(), "");
		...
		if (customerBalanceSheet.isNewRecord()) { // for New record or new record into work flow
			...
			if (!customerBalanceSheet.isWorkflow()) { // With out Work flow for update and delete
				...
				if (befCustomerBalanceSheet == null) { // if records not exists in the main table
					...
					if (oldCustomerBalanceSheet != null
							&& !oldCustomerBalanceSheet.getLastMntOn().equals(befCustomerBalanceSheet.getLastMntOn())) {
				...
				if (tempCustomerBalanceSheet != null && oldCustomerBalanceSheet != null
						&& !oldCustomerBalanceSheet.getLastMntOn().equals(tempCustomerBalanceSheet.getLastMntOn())) {
					auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(
							new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, valueParm), usrLanguage));
				}
			}
