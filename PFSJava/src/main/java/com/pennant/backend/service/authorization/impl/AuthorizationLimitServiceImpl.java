 ...
 */
public class AuthorizationLimitServiceImpl extends GenericService<AuthorizationLimit> implements AuthorizationLimitService {
	private static final  Logger logger = Logger.getLogger(AuthorizationLimitServiceImpl.class);
	 ...
	 * Auth_Limits/Auth_Limits_Temp by using Auth_LimitsDAO's save method b)
	 * Update the Record in the table. based on the module workFlow
	 * Configuration. by using Auth_LimitsDAO's update method 3) Audit the record
	 * in to AuditHeader and AdtAuth_Limits by using
	...
	public AuditHeader saveOrUpdate(AuditHeader auditHeader,boolean hold) {
		...
		if (authorizationLimit.isNew()) {
			if(hold){
				...
				authorizationLimit.setId(Long.parseLong(getAuthorizationLimitDAO().save(authorizationLimit,tableType)));
			}
		...
		if(!hold){
			auditHeader.setAuditDetails(processingList(auditHeader.getAuditDetails(), authorizationLimit.getId(), tableType));
		}
	 ...
	 * table Auth_Limits by using Auth_LimitsDAO's delete method with type as
	 * Blank 3) Audit the record in to AuditHeader and AdtAuth_Limits by using
	 * auditHeaderDAO.addAudit(auditHeader)
	 * 
	 * @param AuditHeader
	 *            (auditHeader)
	...
	/**
	 * getAuth_Limits fetch the details by using Auth_LimitsDAO's getAuth_LimitsById
	 * method.
	 * 
	 * @param id
	...
	/**
	 * getApprovedAuth_LimitsById fetch the details by using Auth_LimitsDAO's
	 * getAuth_LimitsById method . with parameter id and type as blank. it fetches
	 * the approved records from the Auth_Limits.
	 * 
	 * @param id
	 *            id of the AuthorizationLimit.
	 *            (String)
	...
	private AuthorizationLimit getAuthorizationLimit(long id,String type) {
		...
		if(authorizationLimit!=null){
			authorizationLimit.setAuthorizationLimitDetails(getAuthorizationLimitDetailDAO().getListByAuthLimitId(id, type));
		}
	 ...
	 * using getAuthorizationLimitDAO().delete with parameters authorizationLimit,"" b) NEW Add new
	 * record in to main table by using getAuthorizationLimitDAO().save with parameters
	 * authorizationLimit,"" c) EDIT Update record in the main table by using
	 * getAuthorizationLimitDAO().update with parameters authorizationLimit,"" 3) Delete the record
	 * from the workFlow table by using getAuthorizationLimitDAO().delete with parameters
	 * authorizationLimit,"_Temp" 4) Audit the record in to AuditHeader and
	 * AdtAuth_Limits by using auditHeaderDAO.addAudit(auditHeader) for Work
	 * flow 5) Audit the record in to AuditHeader and AdtAuth_Limits by using
	...
	public AuditHeader doApprove(AuditHeader auditHeader,boolean hold) {
		...
		if (!PennantConstants.RECORD_TYPE_NEW.equals(authorizationLimit.getRecordType())) {
			auditHeader.getAuditDetail().setBefImage(authorizationLimitDAO.getAuthorizationLimit(authorizationLimit.getId(), ""));
		}
		...
		if (authorizationLimit.getRecordType().equals(PennantConstants.RECORD_TYPE_DEL)) {
			...
			if (authorizationLimit.getRecordType().equals(
			...
			if(!hold){
				auditHeader.setAuditDetails(processingList(auditHeader.getAuditDetails(), authorizationLimit.getId(), TableType.MAIN_TAB));
			}
		 ...
		 * the workFlow table by using getAuthorizationLimitDAO().delete with parameters
		 * authorizationLimit,"_Temp" 3) Audit the record in to AuditHeader and
		 * AdtAuth_Limits by using auditHeaderDAO.addAudit(auditHeader) for Work
		 * flow
		 * 
		 * @param AuditHeader
		...
		private List<AuditDetail> getAuditDetails(AuthorizationLimit limitDetail,String method) {
			...
			for (int i = 0; i < limitDetail.getAuthorizationLimitDetails().size(); i++) {
				...
				if (!auditTranType.equals(PennantConstants.TRAN_WF)) {
					...
					}else if (authorizationLimitDetail.getRecordType().equalsIgnoreCase(PennantConstants.RECORD_TYPE_NEW)) {
						auditTranType = PennantConstants.TRAN_ADD;
					} else if (authorizationLimitDetail.getRecordType().equalsIgnoreCase(PennantConstants.RECORD_TYPE_DEL)
							|| authorizationLimitDetail.getRecordType().equalsIgnoreCase(PennantConstants.RECORD_TYPE_CAN)) {
						auditTranType = PennantConstants.TRAN_DEL;
					} else {
				...
				auditDetails.add(new AuditDetail(auditTranType, i + 1, fields[0], fields[1], null, authorizationLimitDetail));
			}
			return auditDetails;
		}
		 ...
		 * from getAuthorizationLimitDAO().getErrorDetail with Error ID and language as parameters. if any error/Warnings then assign
		...
		private AuditDetail validation(AuditDetail auditDetail, String usrLanguage) {
			...
			if (authorizationLimit.isNew() && authorizationLimitDAO.isDuplicateKey(authorizationLimit.getId(),authorizationLimit.getLimitType(),authorizationLimit.getUserID(),authorizationLimit.getRoleId(),authorizationLimit.getModule(),
