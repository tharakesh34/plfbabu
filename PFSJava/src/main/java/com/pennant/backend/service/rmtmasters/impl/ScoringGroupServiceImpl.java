...
public class ScoringGroupServiceImpl extends GenericService<ScoringGroup> implements ScoringGroupService {
	 ...
	 * RMTScoringGroup/RMTScoringGroup_Temp by using ScoringGroupDAO's save method b) Update the Record in the table.
	...
	public AuditHeader saveOrUpdate(AuditHeader auditHeader) {
		...
		if (scoringGroup.getNonFinScoringMetricsList() != null
				&& scoringGroup.getNonFinScoringMetricsList().size() > 0) {
	...
	public AuditHeader delete(AuditHeader auditHeader) {
		...
		auditHeader
				.setAuditDetails(getListAuditDetails(listDeletion(scoringGroup, "", auditHeader.getAuditTranType())));
		getAuditHeaderDAO().addAudit(auditHeader);
		logger.debug("Leaving");
		return auditHeader;
	}
	...
	public ScoringGroup getScoringGroupById(long id) {
		...
		if (PennantConstants.PFF_CUSTCTG_CORP.equals(scoringGroup.getCategoryType())
			...
			if (scoringGroup.getFinScoringMetricsList() != null && scoringGroup.getFinScoringMetricsList().size() > 0) {
				...
				/*
				 * List<Rule> ruleList = getRuleDAO().getRulesByGroupIdList(id, "F", ""); ScoringMetrics metric = null;
				 * 
				 ...
				 * metric.setLovDescSQLRule(rule.getSQLRule()); List<ScoringMetrics> subMetricList = null;
				 * if(scoringGroup.getLovDescFinScoreMap().containsKey(rule.getGroupId())){ subMetricList =
				 * scoringGroup.getLovDescFinScoreMap().get(rule.getGroupId()); }else{ subMetricList = new
				 * ArrayList<ScoringMetrics>(); } subMetricList.add(metric);
	 ...
	 * following actions a) DELETE Delete the record from the main table by using getScoringGroupDAO().delete with
	 * parameters scoringGroup,"" b) NEW Add new record in to main table by using getScoringGroupDAO().save with
	 * parameters scoringGroup,"" c) EDIT Update record in the main table by using getScoringGroupDAO().update with
	 * parameters scoringGroup,"" 3) Delete the record from the workFlow table by using getScoringGroupDAO().delete with
	 * parameters scoringGroup,"_Temp" 4) Audit the record in to AuditHeader and AdtRMTScoringGroup by using
	...
	public AuditHeader doApprove(AuditHeader auditHeader) {
		...
		auditHeader.setAuditDetails(
				getListAuditDetails(listDeletion(scoringGroup, "_Temp", auditHeader.getAuditTranType())));
		...
		if (scoringGroup.isNewRecord()) { // for New record or new record into work flow
			...
			if (!scoringGroup.isWorkflow()) { // With out Work flow for update and delete
				...
				if (befScoringGroup == null) { // if records not exists in the main table
					...
					if (oldScoringGroup != null
							&& !oldScoringGroup.getLastMntOn().equals(befScoringGroup.getLastMntOn())) {
				...
				if (tempScoringGroup != null && oldScoringGroup != null
						&& !oldScoringGroup.getLastMntOn().equals(tempScoringGroup.getLastMntOn())) {
					auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(
							new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, valueParm), usrLanguage));
				}
			}
	...
	private AuditHeader getAuditDetails(AuditHeader auditHeader, String method) {
		...
		if (scoringMetricList != null && scoringMetricList.size() > 0) {
			auditDetailMap.put("ScoringMetrics",
					setScoringMetricsAuditData(scoringMetricList, scoringGroup, auditTranType, method));
	...
	private List<AuditDetail> setScoringSlabAuditData(ScoringGroup scoringGroup, String auditTranType, String method) {
		...
		for (int i = 0; i < scoringGroup.getScoringSlabList().size(); i++) {
			...
			auditDetails.add(new AuditDetail(auditTranType, i + 1, fields[0], fields[1], scoringSlab.getBefImage(),
					scoringSlab));
		}
		...
		for (int i = 0; i < scoringMetricList.size(); i++) {
			...
			if (StringUtils.isNotEmpty(scoringMetrics.getRecordType())) {
				auditDetails.add(new AuditDetail(auditTranType, i + 1, fields[0], fields[1],
						scoringMetrics.getBefImage(), scoringMetrics));
			}
	 ...
	 * @return
	 */
	private List<AuditDetail> processingScoringSlabList(List<AuditDetail> auditDetails, String type,
			long scoreGroupId) {
		logger.debug("Entering ");
	 ...
	 * @return
	 */
	private List<AuditDetail> processingScoringMetricsList(List<AuditDetail> auditDetails, String type,
			long scoreGroupId) {
		logger.debug("Entering ");
		boolean saveRecord = false;
		boolean updateRecord = false;
	...
	private List<AuditDetail> listDeletion(ScoringGroup scoringGroup, String tableType, String auditTranType) {
		...
		if (scoringGroup.getScoringSlabList() != null && scoringGroup.getScoringSlabList().size() > 0) {
			for (int i = 0; i < scoringGroup.getScoringSlabList().size(); i++) {
				ScoringSlab scoringSlab = scoringGroup.getScoringSlabList().get(i);
				scoringSlab.setScoreGroupId(scoringGroup.getScoreGroupId());
				auditList.add(new AuditDetail(auditTranType, i + 1, fields[0], fields[1], scoringSlab.getBefImage(),
						scoringSlab));
			}
		...
		if (scoringMetricList != null && scoringMetricList.size() > 0) {
			for (int i = 0; i < scoringMetricList.size(); i++) {
				ScoringMetrics scoringMetrics = scoringMetricList.get(i);
				scoringMetrics.setScoreGroupId(scoringGroup.getScoreGroupId());
				auditList.add(new AuditDetail(auditTranType, i + 1, fields[0], fields[1], scoringMetrics.getBefImage(),
						scoringMetrics));
			}
