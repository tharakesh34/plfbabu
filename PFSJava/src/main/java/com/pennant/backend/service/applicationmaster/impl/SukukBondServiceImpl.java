...
public class SukukBondServiceImpl extends GenericService<SukukBond> implements SukukBondService {
	private static final Logger logger = LogManager.getLogger(SukukBondServiceImpl.class);
	 ...
	 * Do Add or Update the Record a) Add new Record for the new record in the DB table SukukBonds/SukukBonds_Temp by
	 * using SukukBondDAO's save method b) Update the Record in the table. based on the module workFlow Configuration.
	 * by using SukukBondDAO's update method 3) Audit the record in to AuditHeader and AdtSukukBonds by using
	 ...
	 * SukukBonds by using SukukBondDAO's delete method with type as Blank 3) Audit the record in to AuditHeader and
	 * AdtSukukBonds by using auditHeaderDAO.addAudit(auditHeader)
	 * 
	 * @param AuditHeader
	 *            (auditHeader)
	 * @return auditHeader
	...
	/**
	 * getApprovedSukukBondById fetch the details by using SukukBondDAO's getSukukBondById method . with parameter id
	 * and type as blank. it fetches the approved records from the SukukBonds.
	 * 
	 * @param id
	 *            (String)
	 * @return SukukBond
	 ...
	 * workFlow table by using getSukukBondDAO().delete with parameters sukukBond,"_Temp" 3) Audit the record in to
	 * AuditHeader and AdtSukukBonds by using auditHeaderDAO.addAudit(auditHeader) for Work flow
	 * 
	 * @param AuditHeader
	 *            (auditHeader)
	 * @return auditHeader
	 ...
	 * mismatch conditions Fetch the error details from getSukukBondDAO().getErrorDetail with Error ID and language as
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage, String method, boolean onlineRequest) {
		...
		if (sukukBond.isNew()) { // for New record or new record into work flow
			...
			if (!sukukBond.isWorkflow()) { // With out Work flow for update and delete
				...
				if (tempSukukBond != null && oldSukukBond != null
						&& !oldSukukBond.getLastMntOn().equals(tempSukukBond.getLastMntOn())) {
					auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(
							new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, valueParm), usrLanguage));
				}
			}
