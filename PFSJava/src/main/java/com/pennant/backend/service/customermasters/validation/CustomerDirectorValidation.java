...
public class CustomerDirectorValidation {
	...
	private AuditDetail validate(AuditDetail auditDetail, String method, String usrLanguage) {
		...
		if (directorDetail.isWorkflow()) {
			tempDirectorDetail = getDirectorDetailDAO().getDirectorDetailById(directorDetail.getDirectorId(),
					directorDetail.getCustID(), "_Temp");
		}
		DirectorDetail befDirectorDetail = getDirectorDetailDAO().getDirectorDetailById(directorDetail.getDirectorId(),
				directorDetail.getCustID(), "");
		...
		errParm[0] = PennantJavaUtil.getLabel("DirectorDetails") + " , " + PennantJavaUtil.getLabel("label_CustCIF")
				+ ":" + valueParm[0] + " and ";
		...
		if (directorDetail.isNewRecord()) { // for New record or new record into work flow
			...
			if (!directorDetail.isWorkflow()) { // With out Work flow for update and delete
				...
				if (befDirectorDetail == null) { // if records not exists in the main table
					...
					if (oldDirectorDetail != null
							&& !oldDirectorDetail.getLastMntOn().equals(befDirectorDetail.getLastMntOn())) {
				...
				if (tempDirectorDetail != null && oldDirectorDetail != null
						&& !oldDirectorDetail.getLastMntOn().equals(tempDirectorDetail.getLastMntOn())) {
					auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(
							new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, valueParm), usrLanguage));
				}
			}
	...
	public ErrorDetail screenValidations(DirectorDetail directorDetail) {
		...
		if (StringUtils.isBlank(directorDetail.getShortName()) && StringUtils.isBlank(directorDetail.getFirstName())
			...
			return new ErrorDetail(PennantConstants.KEY_FIELD, "30535",
					new String[] { Labels.getLabel("DirectorDetails"),
							...
							Labels.getLabel("listheader_ShortName.label"), shareHolderName },
					new String[] {});
		}
