...
public class FinAgreementDetailValidation {
	...
	public List<AuditDetail> agreementDetailListValidation(List<AuditDetail> auditDetails, String method,
			String usrLanguage) {
		...
		if (agreementDetail.isNewRecord()) { // for New record or new record into work flow
			...
			if (!agreementDetail.isWorkflow()) { // With out Work flow for update and delete
				...
				if (befFinAgreementDetail == null) { // if records not exists in the main table
					...
					if (oldFinAgreementDetail != null
							&& !oldFinAgreementDetail.getLastMntOn().equals(befFinAgreementDetail.getLastMntOn())) {
				...
				if (tempFinAgreementDetail != null && oldFinAgreementDetail != null
						&& !oldFinAgreementDetail.getLastMntOn().equals(tempFinAgreementDetail.getLastMntOn())) {
					auditDetail.setErrorDetail(new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, null));
				}
			}
		}
