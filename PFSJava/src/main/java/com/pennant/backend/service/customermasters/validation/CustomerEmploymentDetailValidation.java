...
public class CustomerEmploymentDetailValidation {
	...
	public List<AuditDetail> employmentDetailListValidation(List<AuditDetail> auditDetails, String method,
			String usrLanguage) {
	...
	private AuditDetail validate(AuditDetail auditDetail, String method, String usrLanguage) {
		...
		if (customerEmploymentDetail.isWorkflow()) {
			tempCustomerEmploymentDetail = getCustomerEmploymentDetailDAO()
					.getCustomerEmploymentDetailByCustEmpId(customerEmploymentDetail.getCustEmpId(), "_Temp");
		}

		CustomerEmploymentDetail befCustomerEmploymentDetail = getCustomerEmploymentDetailDAO()
				.getCustomerEmploymentDetailByCustEmpId(customerEmploymentDetail.getCustEmpId(), "");
		...
		errParm[0] = PennantJavaUtil.getLabel("EmploymentDetails") + " , " + PennantJavaUtil.getLabel("label_CustCIF")
				+ ":" + valueParm[0] + " and ";
		errParm[1] = PennantJavaUtil.getLabel("label_CustEmpName") + "-" + valueParm[1];
		...
		if (customerEmploymentDetail.isNewRecord()) { // for New record or new record into work flow
			...
			if (!customerEmploymentDetail.isWorkflow()) { // With out Work flow for update and delete
				...
				if (befCustomerEmploymentDetail == null) { // if records not exists in the main table
					...
					if (oldCustomerEmploymentDetail != null && !oldCustomerEmploymentDetail.getLastMntOn()
							.equals(befCustomerEmploymentDetail.getLastMntOn())) {
				...
				if (tempCustomerEmploymentDetail != null && oldCustomerEmploymentDetail != null
						&& !oldCustomerEmploymentDetail.getLastMntOn()
								.equals(tempCustomerEmploymentDetail.getLastMntOn())) {
					auditDetail.setErrorDetail(new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, null));
				}
			}
		}
