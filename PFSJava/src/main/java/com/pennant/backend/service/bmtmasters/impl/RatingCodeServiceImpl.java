 ...
 * FILE HEADER *
 ********************************************************************************************
 * * FileName : RatingCodeServiceImpl.java * * Author : PENNANT TECHONOLOGIES * * Creation Date : 05-05-2011 * *
...
public class RatingCodeServiceImpl extends GenericService<RatingCode> implements RatingCodeService {
	...
	private static Logger logger = LogManager.getLogger(RatingCodeServiceImpl.class);
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage, String method) {
		...
		if (ratingCode.isNewRecord()) { // for New record or new record into work flow
			...
			if (!ratingCode.isWorkflow()) {// With out Work flow only new records
				...
				if (ratingCode.getRecordType().equals(PennantConstants.RECORD_TYPE_NEW)) { // if records type is new
					if (befRatingCode != null || tempRatingCode != null) { // if records already exists in the main
																			// table
						auditDetail.setErrorDetail(new ErrorDetail(PennantConstants.KEY_FIELD, "41001", errParm, null));
					}
