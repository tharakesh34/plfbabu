 ...
 * 
 */
public class CommodityBrokerDetailServiceImpl extends GenericService<CommodityBrokerDetail>
		implements CommodityBrokerDetailService {
	private static final Logger logger = LogManager.getLogger(CommodityBrokerDetailServiceImpl.class);
	 ...
	 * FCMTBrokerDetail/FCMTBrokerDetail_Temp by using CommodityBrokerDetailDAO's save method b) Update the Record in
	 * the table. based on the module workFlow Configuration. by using CommodityBrokerDetailDAO's update method 3) Audit
	 * the record in to AuditHeader and AdtFCMTBrokerDetail by using auditHeaderDAO.addAudit(auditHeader)
	 * 
	 * @param AuditHeader
	 *            (auditHeader)
	 * @return auditHeader
	...
	public AuditHeader saveOrUpdate(AuditHeader auditHeader) {
		...
		String tableType = "";
		CommodityBrokerDetail commodityBrokerDetail = (CommodityBrokerDetail) auditHeader.getAuditDetail()
				.getModelData();
		...
		if (commodityBrokerDetail.getBrokerCommodityList() != null
				&& !commodityBrokerDetail.getBrokerCommodityList().isEmpty()) {
	 ...
	 * FCMTBrokerDetail by using CommodityBrokerDetailDAO's delete method with type as Blank 3) Audit the record in to
	 * AuditHeader and AdtFCMTBrokerDetail by using auditHeaderDAO.addAudit(auditHeader)
	 * 
	 * @param AuditHeader
	 *            (auditHeader)
	 * @return auditHeader
	...
	/**
	 * getCommodityBrokerDetailById fetch the details by using CommodityBrokerDetailDAO's getCommodityBrokerDetailById
	 * method.
	 * 
	 * @param id
	 *            (String)
	...
	/**
	 * getApprovedCommodityBrokerDetailById fetch the details by using CommodityBrokerDetailDAO's
	 * getCommodityBrokerDetailById method . with parameter id and type as blank. it fetches the approved records from
	 * the FCMTBrokerDetail.
	 * 
	 * @param id
	 *            (String)
	 * @return CommodityBrokerDetail
	 ...
	 * following actions a) DELETE Delete the record from the main table by using getCommodityBrokerDetailDAO().delete
	 * with parameters commodityBrokerDetail,"" b) NEW Add new record in to main table by using
	 * getCommodityBrokerDetailDAO().save with parameters commodityBrokerDetail,"" c) EDIT Update record in the main
	 * table by using getCommodityBrokerDetailDAO().update with parameters commodityBrokerDetail,"" 3) Delete the record
	 * from the workFlow table by using getCommodityBrokerDetailDAO().delete with parameters
	 * commodityBrokerDetail,"_Temp" 4) Audit the record in to AuditHeader and AdtFCMTBrokerDetail by using
	 * auditHeaderDAO.addAudit(auditHeader) for Work flow 5) Audit the record in to AuditHeader and AdtFCMTBrokerDetail
	...
	public AuditHeader doApprove(AuditHeader auditHeader) {
		...
		BeanUtils.copyProperties((CommodityBrokerDetail) auditHeader.getAuditDetail().getModelData(),
	 ...
	 * workFlow table by using getCommodityBrokerDetailDAO().delete with parameters commodityBrokerDetail,"_Temp" 3)
	 * Audit the record in to AuditHeader and AdtFCMTBrokerDetail by using auditHeaderDAO.addAudit(auditHeader) for Work
	 * flow
	 * 
	 * @param AuditHeader
	 *            (auditHeader)
	 ...
	 * for any mismatch conditions Fetch the error details from getCommodityBrokerDetailDAO().getErrorDetail with Error
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage, String method) {
		...
		if (commodityBrokerDetail.isWorkflow()) {
			tempCommodityBrokerDetail = getCommodityBrokerDetailDAO()
					.getCommodityBrokerDetailById(commodityBrokerDetail.getId(), "_Temp");
		}
		CommodityBrokerDetail befCommodityBrokerDetail = getCommodityBrokerDetailDAO()
				.getCommodityBrokerDetailById(commodityBrokerDetail.getId(), "");
		...
		if (commodityBrokerDetail.isNew()) { // for New record or new record into work flow
			...
			if (!commodityBrokerDetail.isWorkflow()) { // With out Work flow for update and delete
				...
				if (befCommodityBrokerDetail == null) { // if records not exists in the main table
					...
					if (oldCommodityBrokerDetail != null && !oldCommodityBrokerDetail.getLastMntOn()
							.equals(befCommodityBrokerDetail.getLastMntOn())) {
				...
				if (oldCommodityBrokerDetail != null
						&& !oldCommodityBrokerDetail.getLastMntOn().equals(tempCommodityBrokerDetail.getLastMntOn())) {
					auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(
							new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, valueParm), usrLanguage));
				}
			}
	...
	private List<AuditDetail> prepareChildAuditData(AuditHeader auditHeader) {
		...
		String[] fields = PennantJavaUtil.getFieldDetails(new BrokerCommodityDetail(),
				new BrokerCommodityDetail().getExcludeFields());
		String tableType = "";
		int auditSeqCount = 1;
		if (commodityBrokerDetail.isWorkflow()) {
			tableType = "_Temp";
		}
		List<BrokerCommodityDetail> existingCommDetailList = getCommodityBrokerDetailDAO()
				.getBrokerCommodityDetails(commodityBrokerDetail.getBrokerCode(), tableType);
		...
		if (commodityBrokerDetail.getBrokerCommodityList() != null
			...
			for (BrokerCommodityDetail brokerCommodityDetail : commodityBrokerDetail.getBrokerCommodityList()) {
				...
				if (StringUtils.isNotEmpty(auditTranType)) {
					auditDetails.add(new AuditDetail(auditTranType, auditSeqCount++, fields[0], fields[1],
							brokerCommodityDetail.getBefImage(), brokerCommodityDetail));
				}
			}
		}
			...
			for (BrokerCommodityDetail existingCommodityDetail : existingCommDetailList) {
				...
				if (PennantConstants.RCD_DEL.equals(commodityBrokerDetail.getRecordType())) {
					...
					if (commodityBrokerDetail.getBrokerCommodityList() != null
							&& !commodityBrokerDetail.getBrokerCommodityList().isEmpty()) {
						auditTranType = PennantConstants.TRAN_DEL;
						for (BrokerCommodityDetail brokerCommodityDetail : commodityBrokerDetail
								.getBrokerCommodityList()) {
							if (existingCommodityDetail.getCommodityCode()
									.equals(brokerCommodityDetail.getCommodityCode())) {
								auditTranType = "";
								break;
							}
						}
				...
				if (StringUtils.isNotEmpty(auditTranType)) {
					auditDetails.add(new AuditDetail(auditTranType, auditSeqCount++, fields[0], fields[1],
							existingCommodityDetail.getBefImage(), existingCommodityDetail));
				}
			}
		}
