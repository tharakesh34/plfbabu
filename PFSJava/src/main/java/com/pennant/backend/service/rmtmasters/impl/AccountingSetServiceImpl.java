...
public class AccountingSetServiceImpl extends GenericService<AccountingSet> implements AccountingSetService {
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage, String method) {
		...
		if (accountingSet.isNew()) {
			...
			if (!accountingSet.isWorkflow()) { // With out Work flow for update
				...
				if (tempAccountingSet != null && oldAccountingSet != null && !oldAccountingSet.getLastMntOn().equals(tempAccountingSet.getLastMntOn())) {
					auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, valueParm), usrLanguage));
				}
			}
		}
		...
		if(accountset != null){
			...
			errParm1[0]=PennantJavaUtil.getLabel("label_EventCode")+":"+valueParm1[0]+ ", "+PennantJavaUtil.getLabel("label_AccountSetCode")+":"+ valueParm1[1];		
			...
			for (int k = 0; k < feeCodeList.length; k++) {
				if((StringUtils.isNotBlank(feeCodeList[k]) || "Result".equalsIgnoreCase(feeCodeList[k]))
					...
					if(!feeCode.contains(feeCodeList[k].trim().substring(0, feeCodeList[k].trim().indexOf('_'))+",")){
						feeCode = feeCode.concat(feeCodeList[k].trim().substring(0, feeCodeList[k].trim().indexOf('_'))+",");
					}
				}
			}
	...
	public Map<String,List<FinTypeFees>> fetchFinTypeFees(AccountingSet aAccountingSet) {
		...
		List<String> finTypeList = getFinTypeAccountingDAO().getFinTypeAccounting(aAccountingSet.getEventCode(), aAccountingSet.getAccountSetid(), FinanceConstants.MODULEID_FINTYPE);
		if(finTypeList == null || finTypeList.isEmpty()){
			logger.debug("Leaving");
			return null;
		}else{
			Map<String,List<FinTypeFees>> finTypeFeesMap = new HashMap<String,List<FinTypeFees>>();
			finTypeFees = getFinTypeFeesDAO().getFinTypeFeesList(aAccountingSet.getEventCode(),finTypeList, FinanceConstants.MODULEID_FINTYPE);
