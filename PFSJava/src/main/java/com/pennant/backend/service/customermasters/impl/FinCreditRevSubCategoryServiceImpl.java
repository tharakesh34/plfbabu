 ...
 * 
 */
public class FinCreditRevSubCategoryServiceImpl extends GenericService<FinCreditRevSubCategory>
		implements FinCreditRevSubCategoryService {
	private static final Logger logger = LogManager.getLogger(FinCreditRevSubCategoryServiceImpl.class);
	 ...
	 * FinCreditRevSubCategory/FinCreditRevSubCategory_Temp by using FinCreditRevSubCategoryDAO's save method b) Update
	 * the Record in the table. based on the module workFlow Configuration. by using FinCreditRevSubCategoryDAO's update
	...
	public AuditHeader saveOrUpdate(AuditHeader auditHeader) {
		...
		String tableType = "";
		FinCreditRevSubCategory finCreditRevSubCategory = (FinCreditRevSubCategory) auditHeader.getAuditDetail()
				.getModelData();
	...
	/**
	 * getApprovedFinCreditRevSubCategoryById fetch the details by using FinCreditRevSubCategoryDAO's
	 ...
	 * workFlow table by using getFinCreditRevSubCategoryDAO().delete with parameters finCreditRevSubCategory,"_Temp" 3)
	...
	public List<AuditDetail> finCreditRevSubCategoryListValidation(List<AuditDetail> auditDetails, String method,
			String usrLanguage) {
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage, String method) {
		...
		if (finCreditRevSubCategory.isWorkflow()) {
			tempFinCreditRevSubCategory = getFinCreditRevSubCategoryDAO()
					.getFinCreditRevSubCategoryById(finCreditRevSubCategory.getId(), "_Temp");
		}
		FinCreditRevSubCategory befFinCreditRevSubCategory = getFinCreditRevSubCategoryDAO()
				.getFinCreditRevSubCategoryById(finCreditRevSubCategory.getId(), "");
		...
		if (finCreditRevSubCategory.isNewRecord()) { // for New record or new record into work flow
			...
			if (!finCreditRevSubCategory.isWorkflow()) { // With out Work flow for update and delete
				...
				if (befFinCreditRevSubCategory == null) { // if records not exists in the main table
					...
					if (oldFinCreditRevSubCategory != null && !oldFinCreditRevSubCategory.getLastMntOn()
							.equals(befFinCreditRevSubCategory.getLastMntOn())) {
				...
				if (tempFinCreditRevSubCategory != null && oldFinCreditRevSubCategory != null
						&& !oldFinCreditRevSubCategory.getLastMntOn()
								.equals(tempFinCreditRevSubCategory.getLastMntOn())) {
					auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(
							new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, valueParm), usrLanguage));
				}
			}
