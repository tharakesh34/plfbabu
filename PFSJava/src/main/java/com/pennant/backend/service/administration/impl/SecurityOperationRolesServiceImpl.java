...
public class SecurityOperationRolesServiceImpl extends GenericService<SecurityOperationRoles>
		implements SecurityOperationRolesService {
	private static Logger logger = LogManager.getLogger(SecurityOperationRolesServiceImpl.class);
	...
	public AuditHeader businessValidation(AuditHeader auditHeader, String method, boolean online) {
		...
		SecurityOperation securityOperation = (SecurityOperation) auditHeader.getAuditDetail().getModelData();
		List<AuditDetail> auditDetails = getAuditUserRoles(securityOperation, auditHeader.getAuditTranType(), method,
				online);
	 ...
	 * from getSecurityOperationRoleDAO().getErrorDetail with Error ID and language as parameters. if any error/Warnings
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage, String method) {
		...
		SecurityOperation befSecurityOperation = getSecurityOperationDAO()
				.getSecurityOperationById(secOperation.getId(), "");
		...
		if (secOperation.isNewRecord()) { // for New record or new record into work flow
			...
			if (!secOperation.isWorkflow()) { // With out Work flow for update and delete
				...
				if (oldSecurityOperation != null
						&& !oldSecurityOperation.getLastMntOn().equals(tempSecurityUser.getLastMntOn())) {
					auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(
							new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, valueParm), usrLanguage));
				}
			}
	...
	public List<AuditDetail> getAuditUserRoles(SecurityOperation securityOperation, String auditTranType, String method,
			boolean online) {
		...
		for (int i = 0; i < securityOperation.getSecurityOperationRolesList().size(); i++) {
			...
			if (StringUtils.isNotEmpty(securityOperationRoles.getRecordType())) {
				auditDetails.add(new AuditDetail(auditTranType, i + 1, securityOperationRoles.getBefImage(),
						securityOperationRoles));
			}
		}
	...
	private List<AuditDetail> processingDetailList(List<AuditDetail> auditDetails, String type,
		...
		for (AuditDetail auditDetail : auditDetails) {
			...
			if (StringUtils.isNotEmpty(type)
					&& securityOperationRoles.getRecordType().equalsIgnoreCase(PennantConstants.RECORD_TYPE_CAN)) {
				deleteRecord = true;
	 ...
	 * following actions a) DELETE Delete the record from the main table by using getSecurityOperationDAO().delete with
	 * parameters securityUser,"" b) NEW Add new record in to main table by using getSecurityOperationDAO().save with
	 * parameters securityUser,"" c) EDIT Update record in the main table by using getSecurityOperationDAO().update with
	 * parameters securityUser,"" 3) Delete the record from the workFlow table by using getSecurityOperationDAO().delete
	 * with parameters securityUser,"_Temp" 4) Audit the record in to AuditHeader and AdtSecurityOperation by using
