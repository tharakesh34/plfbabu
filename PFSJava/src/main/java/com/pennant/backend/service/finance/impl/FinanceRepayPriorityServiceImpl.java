 ...
 * FILE HEADER *
 ********************************************************************************************
 * * FileName : FinanceRepayPriorityServiceImpl.java * * Author : PENNANT TECHONOLOGIES * * Creation Date : 16-03-2012 *
 ...
 * 
 */
public class FinanceRepayPriorityServiceImpl extends GenericService<FinanceRepayPriority>
		implements FinanceRepayPriorityService {
	private static final Logger logger = LogManager.getLogger(FinanceRepayPriorityServiceImpl.class);
	 ...
	 * FinRpyPriority/FinRpyPriority_Temp by using FinanceRepayPriorityDAO's save method b) Update the Record in the
	 * table. based on the module workFlow Configuration. by using FinanceRepayPriorityDAO's update method 3) Audit the
	 * record in to AuditHeader and AdtFinRpyPriority by using auditHeaderDAO.addAudit(auditHeader)
	 * 
	 ...
	 * FinRpyPriority by using FinanceRepayPriorityDAO's delete method with type as Blank 3) Audit the record in to
	 * AuditHeader and AdtFinRpyPriority by using auditHeaderDAO.addAudit(auditHeader)
	 * 
	 * @param AuditHeader (auditHeader)
	 * @return auditHeader
	 */
	...
	/**
	 * getFinanceRepayPriorityById fetch the details by using FinanceRepayPriorityDAO's getFinanceRepayPriorityById
	 * method.
	 * 
	 * @param id   (String)
	...
	/**
	 * getApprovedFinanceRepayPriorityById fetch the details by using FinanceRepayPriorityDAO's
	 * getFinanceRepayPriorityById method . with parameter id and type as blank. it fetches the approved records from
	 * the FinRpyPriority.
	 * 
	 * @param id (String)
	 * @return FinanceRepayPriority
	 ...
	 * workFlow table by using getFinanceRepayPriorityDAO().delete with parameters financeRepayPriority,"_Temp" 3) Audit
	 * the record in to AuditHeader and AdtFinRpyPriority by using auditHeaderDAO.addAudit(auditHeader) for Work flow
	 * 
	 * @param AuditHeader (auditHeader)
	 * @return auditHeader
	 */
	 ...
	 * for any mismatch conditions Fetch the error details from getFinanceRepayPriorityDAO().getErrorDetail with Error
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage, String method) {
		...
		if (financeRepayPriority.isWorkflow()) {
			tempFinanceRepayPriority = getFinanceRepayPriorityDAO()
					.getFinanceRepayPriorityById(financeRepayPriority.getId(), "_Temp");
		}
		FinanceRepayPriority befFinanceRepayPriority = getFinanceRepayPriorityDAO()
				.getFinanceRepayPriorityById(financeRepayPriority.getId(), "");
		...
		if (financeRepayPriority.isNewRecord()) { // for New record or new record into work flow
			...
			if (!financeRepayPriority.isWorkflow()) {// With out Work flow only new records
				...
				if (financeRepayPriority.getRecordType().equals(PennantConstants.RECORD_TYPE_NEW)) { // if records type
																										// is new
					if (befFinanceRepayPriority != null || tempFinanceRepayPriority != null) { // if records already
																								// exists in the main
																								// table
						auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(
								new ErrorDetail(PennantConstants.KEY_FIELD, "41001", errParm, valueParm), usrLanguage));
			...
			if (!financeRepayPriority.isWorkflow()) { // With out Work flow for update and delete
				...
				if (befFinanceRepayPriority == null) { // if records not exists in the main table
					...
					if (oldFinanceRepayPriority != null
							&& !oldFinanceRepayPriority.getLastMntOn().equals(befFinanceRepayPriority.getLastMntOn())) {
				...
				if (tempFinanceRepayPriority != null && oldFinanceRepayPriority != null
						&& !oldFinanceRepayPriority.getLastMntOn().equals(tempFinanceRepayPriority.getLastMntOn())) {
					auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(
							new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, valueParm), usrLanguage));
				}
			}
		...
		if (!PennantConstants.RECORD_TYPE_DEL.equals(financeRepayPriority.getRecordType())
			...
			if (priorityTypeList != null && !priorityTypeList.isEmpty()) {
				...
				errParm1[0] = PennantJavaUtil.getLabel("label_FinPriority") + ":"
						+ String.valueOf(financeRepayPriority.getFinPriority());
				auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(
						new ErrorDetail(PennantConstants.KEY_FIELD, "65005", errParm1, valueParm), usrLanguage));
			}
		}
