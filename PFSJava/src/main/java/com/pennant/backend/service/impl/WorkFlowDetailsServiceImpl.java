...
public class WorkFlowDetailsServiceImpl extends GenericService<WorkFlowDetails> implements WorkFlowDetailsService {
	...
	private static Logger logger = LogManager.getLogger(WorkFlowDetailsServiceImpl.class);
	 ...
	 * by using WorkFlowDAO's save method b) Update the Record in the table. based on the module workFlow Configuration.
	 * by using WorkFlowDAO's update method 3) Audit the record in to AuditHeader and AdtWorkFlowDetails by using
	...
	public List<ErrorDetail> doValidations(WorkFlowDetails workFlowDetails, String flag, boolean isActive) {
		...
		if ((isActive == true) && (workflowId != 0L) && (getWorkFlowDetailsCountByID(workflowId) == 0L)) {// record not
																											// found in
																											// get or
																											// update
			String[] valueParm = new String[2];
		...
		if (!"get".equals(flag)) {
			...
			if ("create".equals(flag) && getWorkFlowDetailsDAO().isworkFlowTypeExist(
					org.apache.commons.lang3.StringUtils.trimToEmpty(workFlowDetails.getWorkFlowType()))) {
				String[] valueParm = new String[2];
				valueParm[0] = "Process Key Is";
				ErrorDetail errorDetail = ErrorUtil.getErrorDetail(new ErrorDetail("41001", valueParm), "EN");
				errDetails.add(errorDetail);
			}
