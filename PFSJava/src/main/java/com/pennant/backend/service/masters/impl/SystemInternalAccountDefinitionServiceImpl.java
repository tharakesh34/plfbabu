...
public class SystemInternalAccountDefinitionServiceImpl extends GenericService<SystemInternalAccountDefinition> implements SystemInternalAccountDefinitionService {
	...
	public SystemInternalAccountDefinitionServiceImpl() {
		super();
	}
	...
	/**
	 * doApprove method do the following steps.<br/>
	 * 1) Do the Business validation by using businessValidation(auditHeader) method if there is any error or warning
	 * message then return the auditHeader.<br/>
	 * 2) based on the Record type do following actions<br/>
	 * a) DELETE Delete the record from the main table by using getSystemInternalAccountDefinitionDAO().delete with
	 * parameters systemInternalAccountDefinition,""<br/>
	 * b) NEW Add new record in to main table by using getSystemInternalAccountDefinitionDAO().save with parameters
	 * systemInternalAccountDefinition,""<br/>
	 * c) EDIT Update record in the main table by using getSystemInternalAccountDefinitionDAO().update with parameters
	 * systemInternalAccountDefinition,""<br/>
	 * 3) Delete the record from the workFlow table by using getSystemInternalAccountDefinitionDAO().delete with
	 * parameters systemInternalAccountDefinition,"_Temp"<br/>
	 * 4) Audit the record in to AuditHeader and AdtSystemInternalAccountDef by using
	 * auditHeaderDAO.addAudit(auditHeader) for Work flow<br/>
	 * 5) Audit the record in to AuditHeader and AdtSystemInternalAccountDef by using
	 * auditHeaderDAO.addAudit(auditHeader) based on the transaction Type.<br/>
	 * 
	 * @param AuditHeader
	 *            (auditHeader)
	 * @return auditHeader
	...
	public AuditHeader doApprove(AuditHeader auditHeader) {
		...
		BeanUtils.copyProperties((SystemInternalAccountDefinition) auditHeader.getAuditDetail().getModelData(),
		...
		private AuditDetail validation(AuditDetail auditDetail,String usrLanguage,String method){
			...
			if (systemInternalAccountDefinition.isNew()){ // for New record or new record into work flow
				...
				if (!systemInternalAccountDefinition.isWorkflow()){	// With out Work flow for update and delete
					...
					if (tempSystemInternalAccountDefinition!=null && oldSystemInternalAccountDefinition!=null && !oldSystemInternalAccountDefinition.getLastMntOn().equals(tempSystemInternalAccountDefinition.getLastMntOn())){ 
						auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(new ErrorDetails(PennantConstants.KEY_FIELD, "41005", errParm,valueParm), usrLanguage));
					}
				}
			}
			...
			if("doApprove".equals(StringUtils.trimToEmpty(method)) || !systemInternalAccountDefinition.isWorkflow()){
