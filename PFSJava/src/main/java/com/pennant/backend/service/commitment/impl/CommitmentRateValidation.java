...
public class CommitmentRateValidation {
	 ...
	 * @return
	 */
	public List<AuditDetail> commitmentRateListValidation(List<AuditDetail> auditDetails, String method,
			String usrLanguage) {
		logger.debug("Entering");
	...
	private AuditDetail validate(AuditDetail auditDetail, String method, String usrLanguage) {
		...
		if (commitmentRate.isWorkflow()) {
			tempCommitmentRate = getCommitmentRateDAO().getCommitmentRateById(commitmentRate.getCmtReference(),
					commitmentRate.getCmtRvwFrq(), "_Temp");
		}

		CommitmentRate befCommitmentRate = getCommitmentRateDAO()
				.getCommitmentRateById(commitmentRate.getCmtReference(), commitmentRate.getCmtRvwFrq(), "");
		...
		if (commitmentRate.isNewRecord()) { // for New record or new record into work flow
			...
			if (!commitmentRate.isWorkflow()) {// With out Work flow only new records
				...
				if (commitmentRate.getRecordType().equals(PennantConstants.RECORD_TYPE_NEW)) { // if records type is new
					if (befCommitmentRate != null || tempCommitmentRate != null) { // if records already exists in the
																					// main table
						auditDetail.setErrorDetail(new ErrorDetail(PennantConstants.KEY_FIELD, "41014", errParm, null));
					}
			...
			if (!commitmentRate.isWorkflow()) { // With out Work flow for update and delete
				...
				if (befCommitmentRate == null) { // if records not exists in the main table
					...
					if (oldCommitmentRate != null
							&& !oldCommitmentRate.getLastMntOn().equals(befCommitmentRate.getLastMntOn())) {
				...
				if (tempCommitmentRate != null && oldCommitmentRate != null
						&& !oldCommitmentRate.getLastMntOn().equals(tempCommitmentRate.getLastMntOn())) {
					auditDetail.setErrorDetail(new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, null));
				}
