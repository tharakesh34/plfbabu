...
public class SecurityUserOperationsServiceImpl extends GenericService<SecurityUserOperations> implements
		SecurityUserOperationsService {
	private final static Logger logger = Logger.getLogger(SecurityUserOperationsServiceImpl.class);
	...
	/**
	 * This method do the following 1)Gets the AuditDetails list by calling
	 * businessValidation() method 2)a)it checks for each AuditDetail if
	 * "AuditTranType" is "A" it saves the record by calling
	 * SecurityUsersOperationsDAO's save() method b)if "AuditTranType" is "D" it
	 * Deletes the record by calling SecurityUsersOperationsDAO's delete()
	 * method
	 * 
	 */
	@Override
	public AuditHeader saveOrUpdate(AuditHeader auditHeader) {
		...
		String tableType = "";
		SecurityUser securityUser = (SecurityUser) auditHeader.getAuditDetail()
				.getModelData();
		...
		if (securityUser.isWorkflow()) {
			if (securityUser.isNewRecord()) {
				...
				auditHeader.getAuditDetail().setModelData(securityUser);
				auditHeader.setAuditReference(String.valueOf(securityUser
						.getUsrID()));
			} else {
	...
	/**
	 * This method do the following 1)get the AuditDetails list by calling
	 * getAuditDetailsList(). 2)It validate each AuditDetail in the AuditDetails
	 * list by calling validate method
	 * 
	 * @param auditHeader
	 * @return auditHeader
	 */
	public AuditHeader businessValidation(AuditHeader auditHeader,
			String method, boolean online) {
		logger.debug("Entering");

		AuditDetail auditDetail = validation(auditHeader.getAuditDetail(),
				auditHeader.getUsrLanguage(), method, online);
		auditHeader.setErrorList(auditDetail.getErrorDetails());

		SecurityUser securityUser = (SecurityUser) auditHeader.getAuditDetail()
				.getModelData();
		List<AuditDetail> auditDetails = getAuditUserOperations(securityUser,
				auditHeader.getAuditTranType(), method,
	 ...
	 * getSecurityUserRoleDAO().getErrorDetail with Error ID and language as
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage,
		...
		if (secUser.isWorkflow()) {
			tempSecurityUser = getSecurityUserDAO().getSecurityUserById(
					secUser.getId(), "_RTemp");
		}

		SecurityUser befSecurityUser = getSecurityUserDAO()
				.getSecurityUserById(secUser.getId(), "");
		...
		if (secUser.isNewRecord()) { // for New record or new record into work
			...
			if (!secUser.isWorkflow()) { // With out Work flow only new records
				...
				if (secUser.getRecordType().equals(
			...
			if (!secUser.isWorkflow()) { // With out Work flow for update and
				...
				if (befSecurityUser == null) { // if records not exists in the
					...
					if (oldSecurityUser != null
							&& !oldSecurityUser.getLastMntOn().equals(
									befSecurityUser.getLastMntOn())) {
	...
	public List<AuditDetail> getAuditUserOperations(SecurityUser securityUser,
			String auditTranType, String method, String language, boolean online) {

		List<AuditDetail> auditDetails = new ArrayList<AuditDetail>();
		boolean delete = false;
		/*
		 * if
		 * ((PennantConstants.RECORD_TYPE_DEL.equals(securityUser.getRecordType
		 * ()) && method.equalsIgnoreCase("doApprove")) ||
		 * method.equals("delete")) { delete=true; }
		 */

		for (int i = 0; i < securityUser.getSecurityUserOperationsList().size(); i++) {
			SecurityUserOperations securityUserOperations = securityUser
					.getSecurityUserOperationsList().get(i);
			...
			if (delete) {
				securityUserOperations
						.setRecordType(PennantConstants.RECORD_TYPE_MDEL);
			} else {
				if (securityUserOperations.getRecordType().equalsIgnoreCase(
						PennantConstants.RCD_ADD)) {
					securityUserOperations
							.setRecordType(PennantConstants.RECORD_TYPE_NEW);
					isNewRecord = true;
				} else if (securityUserOperations.getRecordType()
			...
			if ("saveOrUpdate".equals(method)
					&& (isNewRecord && securityUserOperations.isWorkflow())) {
			...
			securityUserOperations.setRecordStatus(securityUser
					.getRecordStatus());
			securityUserOperations
					.setUserDetails(securityUser.getUserDetails());
			...
			if (StringUtils.isNotEmpty(securityUserOperations.getRecordType())) {
				auditDetails.add(new AuditDetail(auditTranType, i + 1,
						securityUserOperations.getBefImage(),
						securityUserOperations));
			}
		}
	 ...
	 * @return
	 */
	private List<AuditDetail> processingDetailList(
		...
		long seqNumber = getNextidviewDAO()
				.getSeqNumber("SeqSecUserOperations");
		int count = 0;
		...
		for (AuditDetail auditDetail : auditDetails) {
			...
			SecurityUserOperations suo = (SecurityUserOperations) auditDetail
					.getModelData();
			...
			} else if (suo.getRecordType().equals(
					PennantConstants.RECORD_TYPE_NEW)) {
				if (approveRec) {
					saveRecord = true;
				} else {
		...
		if (count != 0) {
			getNextidviewDAO().setSeqNumber("SeqSecUserOperations",
					seqNumber + count);
		}
	...
	/**
	 * This method fetches List< SecurityRoleGroups > with "RoleId" condition by
	 * calling <code>SecurityRoleGroupsDAO</code>'s
	 * <code>getSecRoleGroupsByRoleID()</code>
	 * 
	 * @return List<SecurityRoleGroups>
	 */
	public List<SecurityRoleGroups> getApprovedRoleGroupsByRoleId(long roleId) {
		return getSecurityRoleGroupsDAO().getRoleGroupsByRoleID(roleId,
				"_AView");
	}

	/**
	 * This method fetches List< SecurityOperationRoles > with "GrpId" condition by
	 * calling SecurityOperationRolesDAO's getSecurityOperationRolesByGrpId()
	 * 
	 * @param securityGroup
	 *            (SecurityGroup)
	 * @return List<SecurityOperationRoles>
	 */
	public List<SecurityOperationRoles> getOperationRolesByOprId(
	...
	/**
	 * This method fetches <code>List< SecurityRole > </code> with "userId"
	 * condition by calling <code>SecurityUsersOperationsDAO</code>'s
	 * <code>getOperationsByUserId()</code>
	 * 
	 * @param userId
	 *            (long)
	 * @param isAssigned
	...
	public List<SecurityOperation> getOperationsByUserId(long userId,
			boolean isAssigned) {
		return getSecurityUserOperationsDAO().getOperationsByUserId(userId,
				isAssigned);
	}

	/**
	 * This method fetches SecurityUserOperations record with "userId" and
	 * "RoleId" condition by calling SecurityUsersOperationsDAO's
	 * getUserOperationsByUsrAndRoleIds)
	 */
	public SecurityUserOperations getUserOperationsByUsrAndRoleIds(long userId,
			long roleId) {
		return getSecurityUserOperationsDAO().getUserOperationsByUsrAndRoleIds(
				userId, roleId);
	 ...
	 * using getSecurityUserDAO().delete with parameters securityUser,"" b) NEW
	 * Add new record in to main table by using getSecurityUserDAO().save with
	 * parameters securityUser,"" c) EDIT Update record in the main table by
	 * using getSecurityUserDAO().update with parameters securityUser,"" 3)
	 * Delete the record from the workFlow table by using
	 * getSecurityUserDAO().delete with parameters securityUser,"_Temp" 4) Audit
	 * the record in to AuditHeader and AdtSecurityUsers by using
	 * auditHeaderDAO.addAudit(auditHeader) for Work flow 5) Audit the record in
	 * to AuditHeader and AdtSecurityUsers by using
	...
	public AuditHeader doApprove(AuditHeader auditHeader) {
		...
		BeanUtils.copyProperties((SecurityUser) auditHeader.getAuditDetail()
				.getModelData(), securityUser);
		tranType = PennantConstants.TRAN_UPD;

		// Retrieving List of Audit Details For Security user Operations details
		// modules
		if (auditHeader.getAuditDetails() != null
				&& !auditHeader.getAuditDetails().isEmpty()) {
			auditDetails = processingAuditDetailList(
					auditHeader.getAuditDetails(), "", securityUser);
		}

		getSecurityUserOperationsDAO().deleteById(securityUser.getUsrID(),
				"_Temp");
	...
	private List<AuditDetail> processingAuditDetailList(
			List<AuditDetail> auditDetails, String type, SecurityUser secUser) {
		logger.debug("Entering ");
		...
		if (!userOperationsAuditDetails.isEmpty()) {
			userOperationsAuditDetails = processingDetailList(
					userOperationsAuditDetails, type, secUser);
		}

		return userOperationsAuditDetails;
	}

	private AuditHeader resetAuditDetails(AuditHeader auditHeader,
			SecurityUser securityUser, String tranType) {
		...
		if (auditHeader.getAuditDetails() != null
			...
			for (AuditDetail detail : auditHeader.getAuditDetails()) {
				SecurityUserOperations userOperations = (SecurityUserOperations) detail
						.getModelData();
	 ...
	 * the workFlow table by using getSecurityUserDAO().delete with parameters
	 * securityUser,"_Temp" 3) Audit the record in to AuditHeader and
