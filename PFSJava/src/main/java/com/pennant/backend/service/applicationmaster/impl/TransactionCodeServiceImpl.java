...
public class TransactionCodeServiceImpl extends GenericService<TransactionCode> implements TransactionCodeService {
	 ...
	 * BMTTransactionCode/BMTTransactionCode_Temp by using TransactionCodeDAO's save method b) Update the Record in the
	...
	public AuditHeader saveOrUpdate(AuditHeader auditHeader) {
		...
		TransactionCode transactionCode = (TransactionCode) auditHeader.getAuditDetail().getModelData();
	 ...
	 * following actions a) DELETE Delete the record from the main table by using getTransactionCodeDAO().delete with
	 * parameters transactionCode,"" b) NEW Add new record in to main table by using getTransactionCodeDAO().save with
	 * parameters transactionCode,"" c) EDIT Update record in the main table by using getTransactionCodeDAO().update
	 * with parameters transactionCode,"" 3) Delete the record from the workFlow table by using
	 * getTransactionCodeDAO().delete with parameters transactionCode,"_Temp" 4) Audit the record in to AuditHeader and
	 * AdtBMTTransactionCode by using auditHeaderDAO.addAudit(auditHeader) for Work flow 5) Audit the record in to
	...
	public AuditHeader doApprove(AuditHeader auditHeader) {
		...
		BeanUtils.copyProperties((TransactionCode) auditHeader.getAuditDetail().getModelData(), transactionCode);
		...
		if (!PennantConstants.RECORD_TYPE_NEW.equals(transactionCode.getRecordType())) {
			auditHeader.getAuditDetail()
					.setBefImage(transactionCodeDAO.getTransactionCodeById(transactionCode.getTranCode(), ""));
		}
	 ...
	 * from getTransactionCodeDAO().getErrorDetail with Error ID and language as parameters. if any error/Warnings then
