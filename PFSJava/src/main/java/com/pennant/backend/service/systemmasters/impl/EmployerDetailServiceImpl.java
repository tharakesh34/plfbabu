...
public class EmployerDetailServiceImpl extends GenericService<EmployerDetail> implements EmployerDetailService {
	 ...
	 * EmployerDetail/EmployerDetail_Temp by using EmployerDetailDAO's save method b) Update the Record in the table.
	...
	/*
	 * private AuditDetail validation(AuditDetail auditDetail, String usrLanguage) { logger.debug("Entering");
	 * 
	 ...
	 * 
	 * logger.debug("Leaving"); return auditDetail; }
	 */
		...
		if (employerDetail.isNewRecord()) { // for New record or new record into work flow
			...
			if (!employerDetail.isWorkflow()) { // With out Work flow for update and delete
				...
				if (befEmployerDetail == null) { // if records not exists in the main table
					...
					if (oldEmployerDetail != null
							&& !oldEmployerDetail.getLastMntOn().equals(befEmployerDetail.getLastMntOn())) {
				...
				if (tempEmployerDetail != null && oldEmployerDetail != null
						&& !oldEmployerDetail.getLastMntOn().equals(tempEmployerDetail.getLastMntOn())) {
					auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(
							new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, valueParm), usrLanguage));
				}
			}
