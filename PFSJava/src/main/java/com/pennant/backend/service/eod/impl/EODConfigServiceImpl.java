...
public class EODConfigServiceImpl extends GenericService<EODConfig> implements EODConfigService {
	 ...
	 * Do Add or Update the Record a) Add new Record for the new record in the DB table EodConfig/EodConfig_Temp by
	 * using EodConfigDAO's save method b) Update the Record in the table. based on the module workFlow Configuration.
	 ...
	 * following actions a) DELETE Delete the record from the main table by using getEODConfigDAO().delete with
	 * parameters eODConfig,"" b) NEW Add new record in to main table by using getEODConfigDAO().save with parameters
	 * eODConfig,"" c) EDIT Update record in the main table by using getEODConfigDAO().update with parameters
	 * eODConfig,"" 3) Delete the record from the workFlow table by using getEODConfigDAO().delete with parameters
	 * eODConfig,"_Temp" 4) Audit the record in to AuditHeader and AdtEodConfig by using
	...
	public AuditHeader doApprove(AuditHeader auditHeader) {
		...
		if (eODConfig.getRecordType().equals(PennantConstants.RECORD_TYPE_DEL)) {
			...
			if (eODConfig.getRecordType().equals(PennantConstants.RECORD_TYPE_NEW)) {
