 ...
 * FILE HEADER *
 ********************************************************************************************
 * * FileName : SubSectorServiceImpl.java * * Author : PENNANT TECHONOLOGIES * * Creation Date : 03-05-2011 * * Modified
...
public class SubSectorServiceImpl extends GenericService<SubSector> implements SubSectorService {
	...
	private static Logger logger = LogManager.getLogger(SubSectorServiceImpl.class);
	 ...
	 * Do Add or Update the Record a) Add new Record for the new record in the DB table BMTSubSectors/BMTSubSectors_Temp
	 * by using SubSectorDAO's save method b) Update the Record in the table. based on the module workFlow
	 * Configuration. by using SubSectorDAO's update method 3) Audit the record in to AuditHeader and AdtBMTSubSectors
	...
	public AuditHeader saveOrUpdate(AuditHeader auditHeader) {
		...
		if (subSector.isNewRecord()) {
			...
			auditHeader.setAuditReference(
					subSector.getSubSectorCode() + PennantConstants.KEY_SEPERATOR + subSector.getSectorCode());
		} else {
	 ...
	 * BMTSubSectors by using SubSectorDAO's delete method with type as Blank 3) Audit the record in to AuditHeader and
	 * AdtBMTSubSectors by using auditHeaderDAO.addAudit(auditHeader)
	 * 
	 * @param AuditHeader (auditHeader)
	 * @return auditHeader
	 */
	...
	/**
	 * getSubSectorById fetch the details by using SubSectorDAO's getSubSectorById method.
	 * 
	 * @param id   (String)
	...
	/**
	 * getApprovedSubSectorById fetch the details by using SubSectorDAO's getSubSectorById method . with parameter id
	 * and type as blank. it fetches the approved records from the BMTSubSectors.
	 * 
	 * @param id (String)
	 * @return SubSector
	 */
	 ...
	 * following actions a) DELETE Delete the record from the main table by using getSubSectorDAO().delete with
	 * parameters subSector,"" b) NEW Add new record in to main table by using getSubSectorDAO().save with parameters
	 * subSector,"" c) EDIT Update record in the main table by using getSubSectorDAO().update with parameters
	 * subSector,"" 3) Delete the record from the workFlow table by using getSubSectorDAO().delete with parameters
	 * subSector,"_Temp" 4) Audit the record in to AuditHeader and AdtBMTSubSectors by using
	 * auditHeaderDAO.addAudit(auditHeader) for Work flow 5) Audit the record in to AuditHeader and AdtBMTSubSectors by
	...
	public AuditHeader doApprove(AuditHeader auditHeader) {
		...
		if (!PennantConstants.RECORD_TYPE_NEW.equals(subSector.getRecordType())) {
			auditHeader.getAuditDetail().setBefImage(
					subSectorDAO.getSubSectorById(subSector.getSectorCode(), subSector.getSubSectorCode(), ""));
		}
		...
		if (subSector.getRecordType().equals(PennantConstants.RECORD_TYPE_DEL)) {
			...
			if (subSector.getRecordType().equals(PennantConstants.RECORD_TYPE_NEW)) {
	 ...
	 * workFlow table by using getSubSectorDAO().delete with parameters subSector,"_Temp" 3) Audit the record in to
	 * AuditHeader and AdtBMTSubSectors by using auditHeaderDAO.addAudit(auditHeader) for Work flow
	 * 
	 * @param AuditHeader (auditHeader)
	 * @return auditHeader
	 */
