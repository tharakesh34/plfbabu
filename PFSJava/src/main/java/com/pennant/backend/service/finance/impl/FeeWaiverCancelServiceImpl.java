...
public class FeeWaiverCancelServiceImpl implements FeeWaiverCancelService {
	...
	public void processFeeWaiverCancellation(long waiverId) {
		...
		List<FinODDetails> odDtls = new ArrayList<>();
	...
	public void processConditionalWaiver(Date promisedDate) {
		...
		List<FeeWaiverHeader> fwhList = feeWaiverHeaderDAO.fetchPromisedFeeWaivers(appDate);

		Date prvPromisedDate = DateUtil.addDays(promisedDate, -1);
		for (FeeWaiverHeader fwh : fwhList) {
			BigDecimal receiptAmount = finReceiptHeaderDAO.getReceiptAmount(prvPromisedDate, promisedDate);
