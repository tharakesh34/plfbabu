 ...
 * FILE HEADER *
 ********************************************************************************************
 * * FileName : CustomerRatingServiceImpl.java * * Author : PENNANT TECHONOLOGIES * * Creation Date : 05-05-2011 * *
...
public class CustomerRatingServiceImpl extends GenericService<CustomerRating> implements CustomerRatingService {
	 ...
	 * CustomerRatings/CustomerRatings_Temp by using CustomerRatingDAO's save method b) Update the Record in the table.
	 * based on the module workFlow Configuration. by using CustomerRatingDAO's update method 3) Audit the record in to
	 * AuditHeader and AdtCustomerRatings by using auditHeaderDAO.addAudit(auditHeader)
	 * 
	 ...
	 * CustomerRatings by using CustomerRatingDAO's delete method with type as Blank 3) Audit the record in to
	 * AuditHeader and AdtCustomerRatings by using auditHeaderDAO.addAudit(auditHeader)
	 * 
	 * @param AuditHeader (auditHeader)
	 * @return auditHeader
	...
	/**
	 * getCustomerRatingById fetch the details by using CustomerRatingDAO's getCustomerRatingById method.
	 * 
	 * @param id   (String)
	...
	/**
	 * getApprovedCustomerRatingById fetch the details by using CustomerRatingDAO's getCustomerRatingById method . with
	 * parameter id and type as blank. it fetches the approved records from the CustomerRatings.
	 * 
	 * @param id (String)
	 * @return CustomerRating
	 */
	 ...
	 * following actions a) DELETE Delete the record from the main table by using getCustomerRatingDAO().delete with
	 * parameters customerRating,"" b) NEW Add new record in to main table by using getCustomerRatingDAO().save with
	 * parameters customerRating,"" c) EDIT Update record in the main table by using getCustomerRatingDAO().update with
	 * parameters customerRating,"" 3) Delete the record from the workFlow table by using getCustomerRatingDAO().delete
	 * with parameters customerRating,"_Temp" 4) Audit the record in to AuditHeader and AdtCustomerRatings by using
	 * auditHeaderDAO.addAudit(auditHeader) for Work flow 5) Audit the record in to AuditHeader and AdtCustomerRatings
	 ...
	 * workFlow table by using getCustomerRatingDAO().delete with parameters customerRating,"_Temp" 3) Audit the record
	 * in to AuditHeader and AdtCustomerRatings by using auditHeaderDAO.addAudit(auditHeader) for Work flow
	 * 
	 * @param AuditHeader (auditHeader)
	 * @return auditHeader
	 */
