...
public class DetailStatisticsServiceImpl implements DetailStatisticsService {
	...
	private static final Logger logger = LogManager.getLogger(DetailStatisticsServiceImpl.class);
	...
	public void setDetailStatisticsHeaderDAO(DetailStatisticsHeaderDAO detailStatisticsHeaderDAO) {
		this.detailStatisticsHeaderDAO = detailStatisticsHeaderDAO;
	}

	/**
	 * This method fetches records from AuditHeader table and process the each record by calling processStatistics()
	 * method
	 * 
	 * @throws Exception
	 */
	...
	public synchronized void saveOrUpdate() throws Exception {
		...
		/*
		 * Fetching all records from AuditHeader Table where AuditId and AuditDate greater than DetailStaticAudit table
		 */
		...
		if (list != null && list.size() > 0) {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				try {
					...
					if (activeStatistics == null || activeStatistics.isEmpty()) {
						...
						if (StringUtils.isEmpty(auditStatistics.getNextRoleCode())) {
							...
							saveDetailStatistics(auditStatistics.getModuleName(), auditStatistics.getCurrentRoleCode(),
									auditStatistics.getAuditReference(),
							...
							DetailStatisticsHeader detailStatisticsHeader = new DetailStatisticsHeader(
									auditStatistics.getModuleName(), auditStatistics.getCurrentRoleCode(), 1);
					...
					logger.error(
							"Exception: Module - " + statistics.getModuleName() + "AuditID :" + statistics.getAuditId(),
							e);
					throw e;
				}
			}
	 ...
	 * @param recordStatus
	 */
	private void saveDetailStatistics(String moduleName, String roleCode, String auditReference, long timeInMS,
			Timestamp lastMntOn, boolean recordStatus) {
		getDetailStatisticsDAO()
				.save(new DetailStatistics(moduleName, roleCode, auditReference, timeInMS, lastMntOn, recordStatus));
