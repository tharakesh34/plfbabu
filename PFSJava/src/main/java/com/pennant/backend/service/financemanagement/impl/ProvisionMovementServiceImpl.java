...
public class ProvisionMovementServiceImpl extends GenericService<ProvisionMovement>
	 ...
	 * FinProvMovements/FinProvMovements_Temp by using ProvisionMovementDAO's save method b) Update the Record in the
	 * table. based on the module workFlow Configuration. by using ProvisionMovementDAO's update method 3) Audit the
	 * record in to AuditHeader and AdtFinProvMovements by using auditHeaderDAO.addAudit(auditHeader)
	 * 
	 * @param AuditHeader
	 *            (auditHeader)
	 * @return auditHeader
	 ...
	 * FinProvMovements by using ProvisionMovementDAO's delete method with type as Blank 3) Audit the record in to
	 * AuditHeader and AdtFinProvMovements by using auditHeaderDAO.addAudit(auditHeader)
	 * 
	 * @param AuditHeader
	 *            (auditHeader)
	 * @return auditHeader
	...
	/**
	 * getApprovedProvisionMovementById fetch the details by using ProvisionMovementDAO's getProvisionMovementById
	 * method . with parameter id and type as blank. it fetches the approved records from the FinProvMovements.
	 * 
	 * @param id
	 *            (String)
	 * @return ProvisionMovement
	 ...
	 * workFlow table by using getProvisionMovementDAO().delete with parameters provisionMovement,"_Temp" 3) Audit the
	 * record in to AuditHeader and AdtFinProvMovements by using auditHeaderDAO.addAudit(auditHeader) for Work flow
	 * 
	 * @param AuditHeader
	 *            (auditHeader)
	 * @return auditHeader
	 ...
	 * for any mismatch conditions Fetch the error details from getProvisionMovementDAO().getErrorDetail with Error ID
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage, String method) {
		...
		if (provisionMovement.isWorkflow()) {
			tempProvisionMovement = getProvisionMovementDAO().getProvisionMovementById(provisionMovement.getId(),
					provisionMovement.getProvMovementDate(), "_Temp");
		}
		ProvisionMovement befProvisionMovement = getProvisionMovementDAO()
				.getProvisionMovementById(provisionMovement.getId(), provisionMovement.getProvMovementDate(), "");
