 ...
 * 
 */
public class SalesOfficerServiceImpl extends GenericService<SalesOfficer> implements SalesOfficerService {

	private static final Logger logger = LogManager.getLogger(SalesOfficerServiceImpl.class);
	 ...
	 * Do Add or Update the Record a) Add new Record for the new record in the DB table SalesOfficers/SalesOfficers_Temp
	 * by using SalesOfficerDAO's save method b) Update the Record in the table. based on the module workFlow
	...
	public AuditHeader saveOrUpdate(AuditHeader auditHeader) {
		...
		String tableType = "";
		SalesOfficer salesOfficer = (SalesOfficer) auditHeader.getAuditDetail().getModelData();
	 ...
	 * following actions a) DELETE Delete the record from the main table by using getSalesOfficerDAO().delete with
	 * parameters salesOfficer,"" b) NEW Add new record in to main table by using getSalesOfficerDAO().save with
	 * parameters salesOfficer,"" c) EDIT Update record in the main table by using getSalesOfficerDAO().update with
	 * parameters salesOfficer,"" 3) Delete the record from the workFlow table by using getSalesOfficerDAO().delete with
	 * parameters salesOfficer,"_Temp" 4) Audit the record in to AuditHeader and AdtSalesOfficers by using
	...
	public AuditHeader doApprove(AuditHeader auditHeader) {
		...
		BeanUtils.copyProperties((SalesOfficer) auditHeader.getAuditDetail().getModelData(), salesOfficer);

		if (salesOfficer.getRecordType().equals(PennantConstants.RECORD_TYPE_DEL)) {
			...
			if (salesOfficer.getRecordType().equals(PennantConstants.RECORD_TYPE_NEW)) {
	 ...
	 * from getSalesOfficerDAO().getErrorDetail with Error ID and language as parameters. if any error/Warnings then
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage, String method) {
		...
		if (salesOfficer.isWorkflow()) {
			tempSalesOfficer = getSalesOfficerDAO().getSalesOfficerById(salesOfficer.getId(), "_Temp");
		}

		SalesOfficer befSalesOfficer = getSalesOfficerDAO().getSalesOfficerById(salesOfficer.getId(), "");
			...
			if (!salesOfficer.isWorkflow()) { // With out Work flow for update
				...
				if (befSalesOfficer == null) { // if records not exists in the
					...
					if (oldSalesOfficer != null
							&& !oldSalesOfficer.getLastMntOn().equals(befSalesOfficer.getLastMntOn())) {
				...
				if (tempSalesOfficer != null && oldSalesOfficer != null
						&& !oldSalesOfficer.getLastMntOn().equals(tempSalesOfficer.getLastMntOn())) {
					auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(
							new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, valueParm), usrLanguage));
				}
			}
		...
		if ("doApprove".equals(StringUtils.trimToEmpty(method)) || !salesOfficer.isWorkflow()) {
