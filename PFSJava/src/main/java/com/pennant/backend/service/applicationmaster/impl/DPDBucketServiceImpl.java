...
public class DPDBucketServiceImpl extends GenericService<DPDBucket> implements DPDBucketService {
	private static final Logger logger = LogManager.getLogger(DPDBucketServiceImpl.class);
	 ...
	 * Do Add or Update the Record a) Add new Record for the new record in the DB table DPDBUCKETS/DPDBUCKETS_Temp by
	 * using DPDBUCKETSDAO's save method b) Update the Record in the table. based on the module workFlow Configuration.
	 ...
	 * following actions a) DELETE Delete the record from the main table by using getDPDBucketDAO().delete with
	 * parameters dPDBucket,"" b) NEW Add new record in to main table by using getDPDBucketDAO().save with parameters
	 * dPDBucket,"" c) EDIT Update record in the main table by using getDPDBucketDAO().update with parameters
	 * dPDBucket,"" 3) Delete the record from the workFlow table by using getDPDBucketDAO().delete with parameters
	 * dPDBucket,"_Temp" 4) Audit the record in to AuditHeader and AdtDPDBUCKETS by using
	...
	public AuditHeader doApprove(AuditHeader auditHeader) {
		...
		if (dPDBucket.getRecordType().equals(PennantConstants.RECORD_TYPE_DEL)) {
			...
			if (dPDBucket.getRecordType().equals(PennantConstants.RECORD_TYPE_NEW)) {
