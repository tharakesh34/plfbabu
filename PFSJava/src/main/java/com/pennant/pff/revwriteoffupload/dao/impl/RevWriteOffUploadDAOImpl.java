...
public class RevWriteOffUploadDAOImpl extends SequenceDao<RevWriteOffUploadDetail> implements RevWriteOffUploadDAO {
	...
	public List<RevWriteOffUploadDetail> getDetails(long headerID) {
		String sql = "Select HeaderId, Id, FinID, FinReference, Remarks, Progress, ErrorCode, ErrorDesc, Status From REVWRITEOFF_UPLOAD Where HeaderId = ?";
	...
	public List<String> isDuplicateExists(String reference, long headerID) {
		...
		sql.append(" Select HeaderId From REVWRITEOFF_UPLOAD");
	...
	public void update(List<RevWriteOffUploadDetail> detailsList) {
		String sql = "Update REVWRITEOFF_UPLOAD set FinID = ?, Progress = ?, Status = ?, ErrorCode = ?, ErrorDesc = ? Where ID = ?";
	...
	public void update(List<Long> headerIds, String errorCode, String errorDesc, int progress) {
		String sql = "Update REVWRITEOFF_UPLOAD set Progress = ?, Status = ?, ErrorCode = ?, ErrorDesc = ? Where HeaderID = ?";
	...
	public void update(RevWriteOffUploadDetail detail) {
		String sql = "Update REVWRITEOFF_UPLOAD set Progress = ?, Status = ?, ErrorCode = ?, ErrorDesc = ? Where ID = ?";
	...
	public String getSqlQuery() {
		...
		sql.append(" From REVWRITEOFF_UPLOAD hu");
	...
	public String getStatus(String finReference) {
		String sql = "Select Status From REVWRITEOFF_UPLOAD Where FinReference = ? and Status is null";
	...
	public boolean isInProgress(String finReference, long headerID) {
		...
		sql.append(" From REVWRITEOFF_UPLOAD wu");
	...
	public long getReceiptIdByRef(String finReference) {
		String sql = "Select Max(ReceiptId) From WRITEOFF_UPLOAD_LOG Where FinReference = ? and Event = ?";

		logger.debug(Literal.SQL + sql);

		try {
			return this.jdbcOperations.queryForObject(sql, Long.class, finReference, FinanceConstants.WRITEOFF);
		} catch (EmptyResultDataAccessException dae) {
			logger.warn(Message.NO_RECORD_FOUND);
