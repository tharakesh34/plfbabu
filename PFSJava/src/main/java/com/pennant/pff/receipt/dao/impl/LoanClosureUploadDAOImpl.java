...
public class LoanClosureUploadDAOImpl extends SequenceDao<LoanClosureUpload> implements LoanClosureUploadDAO {
	...
	public List<LoanClosureUpload> loadRecordData(long headerID) {
		...
		sql.append(" Id, HeaderId, FinId, FinReference, Remarks");
	...
	public long save(LoanClosureUpload lcu) {
		...
		sql.append(" (HeaderId, FinID, FinReference, Remarks, ReasonCode, Closuretype, AllocationType)");
		sql.append(" Values(?, ?, ?, ?, ?, ?, ?)");
	...
	public void update(List<LoanClosureUpload> details) {
		String sql = "Update Loan_Closure_Upload set FinId = ?, FinReference = ?, Progress = ?, Status = ?, ErrorCode = ?, ErrorDesc = ? Where ID = ?";
