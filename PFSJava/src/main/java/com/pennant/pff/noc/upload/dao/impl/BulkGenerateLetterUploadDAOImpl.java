...
public class BulkGenerateLetterUploadDAOImpl extends SequenceDao<BulkGenerateLetterUpload>
		implements BulkGenerateLetterUploadDAO {

	@Override
	public List<BulkGenerateLetterUpload> getDetails(long headerID) {
		StringBuilder sql = new StringBuilder("Select ID, HeaderId");
		sql.append(", FinReference, LetterType, ModeOfTransfer, WaiverCharges");
		sql.append(", Progress, Status, ErrorCode, ErrorDesc");
		sql.append(" From Generate_Letter_Upload");
		sql.append(" Where HeaderId = ?");

		logger.debug(Literal.SQL.concat(sql.toString()));

		return this.jdbcOperations.query(sql.toString(), (rs, rowNum) -> {
			BulkGenerateLetterUpload noc = new BulkGenerateLetterUpload();

			noc.setId(rs.getLong("ID"));
			noc.setHeaderId(rs.getLong("HeaderId"));
			noc.setFinReference(rs.getString("FinReference"));
	...
	@Override
	public void update(List<BulkGenerateLetterUpload> detailsList) {
		String sql = "Update Generate_Letter_Upload set  FinID = ?, Progress = ?, Status = ?, ErrorCode = ?, ErrorDesc = ? Where ID = ?";
		...
		this.jdbcOperations.batchUpdate(sql, new BatchPreparedStatementSetter() {
			...
			public void setValues(PreparedStatement ps, int i) throws SQLException {
				int index = 0;
				BulkGenerateLetterUpload detail = detailsList.get(i);
	...
	public void update(List<Long> headerIds, String errorCode, String errorDesc, int progress) {
		String sql = "Update Generate_Letter_Upload set Progress = ?, Status = ?, ErrorCode = ?, ErrorDesc = ? Where HeaderId = ?";
	...
	public String getSqlQuery() {
		...
		sql.append(" From Generate_Letter_Upload noc");
	...
	public int getFinTypeLtrMap(String finReference) {
		String sql = "Select count(*) From  SaiKrishnaTable  Where FinReference= ?";
	...
	@Override
	public BulkGenerateLetterUpload getByReference(String reference) {
		StringBuilder sql = new StringBuilder("Select noc.ID, noc.HeaderId");
		sql.append(", noc.FinReference, noc.LetterType, noc.ModeOfTransfer, noc.WaiverCharges, noc.Progress");
		sql.append(", uh.ApprovedOn From Generate_Letter_Upload noc");
		sql.append(" Inner Join File_Upload_Header uh on uh.ID = noc.HeaderId");
