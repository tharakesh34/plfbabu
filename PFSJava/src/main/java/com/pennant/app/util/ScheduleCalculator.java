...
public class ScheduleCalculator {
	...
	private FinScheduleData procPlanDeferPft(FinScheduleData finScheduleData) {
		...
		if (finScheduleData.getFinanceMain().getGrcProfitDaysBasis() == null) {
			finScheduleData.getFinanceMain().setGrcProfitDaysBasis(
					finScheduleData.getFinanceMain().getProfitDaysBasis());
		}
	...
	private ScheduleCalculator(String method, FinScheduleData finScheduleData, String baseRate, String splRate,
		...
		if (StringUtils.equals(method, PROC_CHANGERATE)) {
			...
			finScheduleData = advPftRateCalculation(finScheduleData, finScheduleData.getFinanceMain()
					.getEventFromDate(), finScheduleData.getFinanceMain().getEventToDate());
			...
			/*
			 * Cloner cloner = new Cloner(); FinScheduleData orgFinScheduleData = cloner.deepClone(finScheduleData);
			 * FinanceMain orgFinMain = orgFinScheduleData.getFinanceMain(); orgFinScheduleData =
			 * calEffectiveRate(orgFinScheduleData, CalculationConstants.SCH_SPECIFIER_TOTAL,
			 * orgFinMain.getTotalProfit().add(orgFinMain.getTotalCpz()), orgFinMain.getFinStartDate(),
			 * orgFinMain.getMaturityDate(), true);
			 * 
	...
	private ScheduleCalculator(FinScheduleData finScheduleData, String method) {
		...
		if (StringUtils.equals(method, PROC_CALEFFECTIVERATE)) {
			setFinScheduleData(calEffectiveRate(finScheduleData, CalculationConstants.SCH_SPECIFIER_TOTAL,
					finScheduleData.getFinanceMain().getTotalGrossPft(), finScheduleData.getFinanceMain()
							.getFinStartDate(), finScheduleData.getFinanceMain().getMaturityDate(), true));
		}
		...
		if (StringUtils.equals(method, PROC_ADVPFTRATESCHEDULE)) {
			setFinScheduleData(advPftRateCalculation(finScheduleData, finScheduleData.getFinanceMain()
					.getEventFromDate(), finScheduleData.getFinanceMain().getEventToDate()));
		}
	...
	private FinScheduleData procGetCalSchd(FinScheduleData finScheduleData) {
		...
		// If any changes in the schedule dates happened these values doesnot have any meaning.
	...
	private FinScheduleData validateEMIHoliday(FinScheduleData finScheduleData) {
		...
		if (!finMain.isPlanEMIHAlw()) {
			finScheduleData.setErrorDetail(new ErrorDetails("SCH37", "Loan Does not allow Planned EMI's",
					new String[] { " " }));
			return finScheduleData;
		}

		if (StringUtils.equals(FinanceConstants.PRODUCT_ODFACILITY, finMain.getProductCategory())) {
			finScheduleData.setErrorDetail(new ErrorDetails("SCH37", "Product Category not allow Planned EMI's",
					new String[] { " " }));
			return finScheduleData;
		}

		if (finMain.isStepFinance()) {
			finScheduleData.setErrorDetail(new ErrorDetails("SCH37", "Step Loans does not allow Planned EMI's",
					new String[] { " " }));
			return finScheduleData;
		}
	...
	private FinScheduleData procDeleteTerm(FinScheduleData finScheduleData) {
		...
		if (evtFromDate.before(curBussniessDate)) {
			...
			finScheduleData.setErrorDetail(new ErrorDetails("SCH36",
					...
					new String[] { " " }));
			logger.warn("SCH36 - REQUETSED DELETED TERMS DATE IS BEFORE CURRENT BUSINESS DATE. DELETION OF TERMS NOT POSSIBLE");
			return finScheduleData;
		...
		for (int i = sdSize - 1; i > 0; i--) {
			...
			// If any of Profit and Principal Payments already completed for last possible date before requested deleted date then deletion of
			// terms not possible
				...
				if (curSchd.getSchdPftPaid().compareTo(BigDecimal.ZERO) > 0
					...
					finScheduleData
							.setErrorDetail(new ErrorDetails(
									...
									new String[] { " " }));
					logger.warn("SCH34 - NO UNPAID SCHEDULE DATES FOUNDS BEFORE REQUESTED DELETE TERMS DATE. DELETION OF TERMS NOT POSSIBLE");
					return finScheduleData;
				}
			}

			// Found Unpaid date after current business date and before requested deleted date
		...
		for (int i = finMain.getIndexStart(); i < finScheduleData.getFinanceScheduleDetails().size(); i++) {
			if (finScheduleData.getFinanceScheduleDetails().get(i).getSchDate().compareTo(evtFromDate) > 0) {
		...
		for (int i = 0; i < finScheduleData.getRepayInstructions().size(); i++) {
			if (finScheduleData.getRepayInstructions().get(i).getRepayDate().compareTo(finMain.getCalMaturity()) >= 0) {
				finScheduleData.getRepayInstructions().remove(i);
			}
		}
	...
	/*
	 * =================================================================================================================
	 * Method : procChangeRate Description : CHANGE RATE Process : Should change the rate for the requested dates and
	 * recalculate the repay amount based on the requested recalculation type i) CURPRD: Change the rate for the given
	 * dates and repay recalculation will happen for the period covered in the dates only. All remaining repayments
	 * should be unchanged.
	 * 
	...
	private FinScheduleData procChangeRate(FinScheduleData finScheduleData, String baseRate, String splRate,
		...
		for (int i = 0; i < sdSize; i++) {
			...
			// Setting Rates between Fromdate and Todate
			if ((schdDate.compareTo(evtFromDate) >= 0 && schdDate.compareTo(evtToDate) < 0) || (i == (sdSize - 1))) {
	...
	private FinScheduleData procChangeRepay(FinScheduleData finScheduleData, BigDecimal repayAmount, String schdMethod) {
		logger.debug("Entering");
	...
	private FinScheduleData afterChangeRepay(FinScheduleData finScheduleData) {
		...
		//TODO: PV Will be addresses while working for Flat to converting and Islamic 
		/*
	...
	private FinScheduleData procAddDisbursement(FinScheduleData orgFinScheduleData, BigDecimal newDisbAmount,
		...
		//Create Cloner for calculation purpose. If calculation is successful send back calculated DATA else send original data with errors
		...
		if (finMain.getFinStartDate().compareTo(graceEndDate) == 0) {
			utilizeGrcEndDisb = false;
		}

		//If Disbursement Date is after Grace End Date no adjustment is applicable
		if (evtFromDate.compareTo(graceEndDate) >= 0) {
			utilizeGrcEndDisb = false;
		}
		...
		if (utilizeGrcEndDisb) {
			for (int i = 0; i < sdSize; i++) {
				...
				if (schdDate.compareTo(graceEndDate) < 0) {
					continue;
				}
		...
		for (int i = 0; i < sdSize; i++) {
			...
			} else if (curSchd.getSchDate().compareTo(evtFromDate) == 0) {
		...
		// If recalculation type is TILL DATE and event to date is >= maturity date then force it to TILLMDT
		if (StringUtils.equals(recaltype, CalculationConstants.RPYCHG_TILLDATE)
				&& recalToDate.compareTo(finMain.getMaturityDate()) >= 0) {
			finMain.setRecalType(CalculationConstants.RPYCHG_TILLMDT);
		}
	...
	private FinScheduleData procChangeProfit(FinScheduleData finScheduleData, BigDecimal desiredPftAmount) {
		...
		finScheduleData = calEffectiveRate(finScheduleData, CalculationConstants.SCH_SPECIFIER_SELECT,
	...
	private FinScheduleData adjTerms(FinScheduleData finScheduleData, boolean isSetRepay) {
		...
		// If The calculated schedule last repayment is under limit then no need to adjust terms
	...
	private FinScheduleData addOneTerm(FinScheduleData finScheduleData, Date lastDateLimit, boolean isSetRepay) {
		...
		Date nextSchdDate = DateUtility.getDate(DateUtility.formatUtilDate(
				FrequencyUtil.getNextDate(finMain.getRepayFrq(), 1, curSchdDate, HolidayHandlerTypes.MOVE_NONE, false)
		...
		// Find whether record with next schedule date already exist in schedule details
	...
	private FinScheduleData resetNewSchdDetail(FinScheduleData finScheduleData, Date nextSchdDate, int scheduleFlag) {
		...
		if (nextSchdDate.compareTo(finMain.getGrcPeriodEndDate()) <= 0) {
			...
			for (int i = sdSize - 1; i > 0; i--) {
				...
				if (nextSchdDate.equals(curSchd.getSchDate())) {
					...
					if (nextSchdDate.after(curSchd.getSchDate())
							&& nextSchdDate.compareTo(finScheduleData.getFinanceMain().getCalMaturity()) <= 0) {
	...
	//TODO: The below method to be reviewd and modified if required. New Method resetCurSchdFlags added
	/*
	 * ________________________________________________________________________________________________________________
	...
	private FinScheduleData setRpyInstructDetails(FinScheduleData finScheduleData, Date fromDate, Date toDate,
		...
		if (toDate.compareTo(finMain.getMaturityDate()) >= 0 || !isAddNewInstruction) {
			finScheduleData.setRepayInstructions(sortRepayInstructions(finScheduleData.getRepayInstructions()));
			return finScheduleData;
		}
	...
	private FinScheduleData getRpyInstructDetails(FinScheduleData finScheduleData) {
		...
		if (toDate.compareTo(finMain.getMaturityDate()) <= 0) {
		...
		for (int i = indexStart; i < sdSize; i++) {
			...
			if (curSchdDate.compareTo(fromDate) >= 0
					&& (curSchdDate.before(toDate) || (finMain.getNextRolloverDate() != null && curSchd.getSchDate()
							.compareTo(finMain.getNextRolloverDate()) == 0))) {
	...
	private FinScheduleData setFinanceTotals(FinScheduleData finScheduleData) {
		...
		for (int i = 0; i < sdSize; i++) {
			...
			} else if (schdDate.compareTo(finMain.getGrcPeriodEndDate()) == 0) {
...
*/		}
	...
	private FinScheduleData prepareFirstGraceRcd(FinScheduleData finScheduleData) {
		...
		curSchd.setClosingBalance(curSchd.getDisbAmount().add(curSchd.getFeeChargeAmt())
	...
	private FinScheduleData prepareRemainingGraceRcd(FinScheduleData finScheduleData, int iCur) {
		...
		curSchd.setDayFactor(CalculationUtil.getInterestDays(prvSchd.getSchDate(), curSchd.getSchDate(),
		...
		if (!curSchd.isPftOnSchDate()) {
			...
			if (StringUtils.equals(curSchd.getBpiOrHoliday(), FinanceConstants.FLAG_HOLIDAY)
				...
				calPftPriRpy(finScheduleData, iCur, (iCur - 1));
		 ...
		 */
		if ((curSchd.getSchDate().compareTo(finMain.getGrcPeriodEndDate()) == 0 && finMain.isAllowGrcCpz())
				|| (curSchd.getSchDate().compareTo(finMain.getGrcPeriodEndDate()) == 0) && finMain.isCpzAtGraceEnd()) {
			curSchd.setCpzOnSchDate(true);
		}
		...
		if (curSchd.getSchDate().compareTo(finMain.getEventFromDate()) > 0
				&& curSchd.getSchDate().compareTo(finMain.getEventToDate()) <= 0) {
	...
	private FinScheduleData repaySchdCal(FinScheduleData finScheduleData, Boolean isCalFlat) {
		...
		for (int i = schdIndex + 1; i < schdDetails.size(); i++) {
			...
			if (finMain.getNextRolloverDate() != null && curSchDate.compareTo(finMain.getNextRolloverDate()) == 0) {
				isRolloverDate = true;
			}
			...
			if (!isRepayComplete) {
				...
				if (repayRateBasis.equals(CalculationConstants.RATE_BASIS_D)
						&& (curSchd.getClosingBalance().equals(BigDecimal.ZERO))) {
					isRepayComplete = true;
				}

				if (curSchDate.compareTo(finMain.getEventFromDate()) > 0
						&& curSchDate.compareTo(finMain.getEventToDate()) <= 0) {
					finMain.setPftForSelectedPeriod(finMain.getPftForSelectedPeriod().add(curSchd.getProfitCalc()));
				}

				// For RollOver Finance, adjust Remaining Total Balance Amount to Rollover Amount on Next Rollover Date
				if (finMain.getNextRolloverDate() != null && curSchDate.compareTo(finMain.getNextRolloverDate()) == 0) {
	 ...
	 * ************************************************ ****************************************************************
	 */
	private FinanceScheduleDetail calPftPriRpy(FinScheduleData finScheduleData, int iCur, int iPrv) {
		logger.debug("Entering");
	...
	/*
	 * ========================================================================== ======================================
	 * Method : procSubSchedule Description : SUB SCHEDULE Process : Add Term will add another term to the schedule
	 * details. ====================================================
	 * =============================================================
	 */
	...
	private FinScheduleData addSubScheduleTerm(FinScheduleData finScheduleData, Date lastDateLimit, boolean isSetRepay,
		...
		if (newSchdDate.after(lastDateLimit)) {
			// Through Error
			finScheduleData.setErrorDetail(new ErrorDetails("SCH30", "ADD/ADJ TERMS REACHED MAXIMUM FINANCE YEARS",
					new String[] { " " }));
			return finScheduleData;
		}
		...
		finScheduleData = setOtherSchdDates(finScheduleData, frqNewSchd, curSchdDate, CalculationConstants.SCHDFLAG_PFT);
	...
	private BigDecimal approxPMT(FinanceMain finMain, BigDecimal intRate, int terms, BigDecimal presentValue,
		...
		} else if (calFrq.equals(FrequencyCodeTypes.FRQ_BIWEEKLY) || calFrq.equals(FrequencyCodeTypes.FRQ_FORTNIGHTLY)) {
	...
	public FinScheduleData addBPISchd(FinScheduleData finScheduleData) {
		...
		Date bpiDate = DateUtility.getDate(DateUtility.formatUtilDate(
				FrequencyUtil.getNextDate(frqBPI, 1, finMain.getFinStartDate(), HolidayHandlerTypes.MOVE_NONE, false)
						.getNextFrequencyDate(), PennantConstants.dateFormat));

		if (bpiDate.compareTo(firstSchdDate) >= 0) {
			...
			if (bpiDate.compareTo(finMain.getGrcPeriodEndDate()) > 0) {
	...
	public FinScheduleData procMDTRecord(FinScheduleData finScheduleData, int i, boolean isRepayComplete) {
		...
		if (repayRateBasis.equals(CalculationConstants.RATE_BASIS_D)) {
			...
			BigDecimal endBal = getClosingBalance(curSchd, prvSchd, repayRateBasis).subtract(
					curSchd.getRefundOrWaiver());
	...
	private FinScheduleData calPercentageSteps(FinScheduleData finScheduleData) {
		...
		for (int i = 0; i < stepSize; i++) {
			...
			if (!isRateStepOnly) {
				closeBalPercentage = closeBalPercentage.subtract(stepDetail.getEmiSplitPerc().divide(
						new BigDecimal(100)));
			...
			for (int j = (finMain.getIndexStart()); j < sdSize; j++) {
				...
				if (finMain.getScheduleMethod().equals(CalculationConstants.SCHMTHD_EQUAL)) {
					stepAmount = approxPMT(finMain, curSchd.getCalculatedRate(), iTerms, stepOpenBal,
						...
						|| finMain.getScheduleMethod().equals(CalculationConstants.SCHMTHD_PRI)) {
					stepAmount = stepOpenBal.subtract(finMain.getCompareExpectedResult()).divide(
							BigDecimal.valueOf(iTerms), 0, RoundingMode.HALF_DOWN);
				}
	...
	private FinScheduleData insuranceCalculation(FinScheduleData finScheduleData) {
		...
		for (FinInsurances finInsurance : insuranceList) {
			...
			// If Payment Method is other than Schedule Frequency, exclude calculation
			...
			for (int i = 0; i < finInsurance.getFinSchFrqInsurances().size(); i++) {
				...
				if (StringUtils.equals(finInsurance.getCalType(), InsuranceConstants.CALTYPE_PERCENTAGE)) {
					...
					insAmount = actualFinAmt.multiply(finInsurance.getCalPerc()).divide(
							BigDecimal.valueOf(frqFactor * 100), 0, RoundingMode.HALF_DOWN);
					...
					insAmount = actualFinAmt.multiply(finInsurance.getInsuranceRate()).divide(
							BigDecimal.valueOf(frqFactor * 100), 0, RoundingMode.HALF_DOWN);
	 ...
	 * @return
	 */
	private FinScheduleData advPftRateCalculation(FinScheduleData finScheduleData, Date recalFromDate, Date recalToDate) {

		FinanceMain finMain = finScheduleData.getFinanceMain();
		if (finScheduleData.getFinanceType() != null
				&& !StringUtils.equals(FinanceConstants.PRODUCT_STRUCTMUR, finScheduleData.getFinanceType()
						.getFinCategory())) {
			return finScheduleData;
		}
		...
		for (int i = 0; i < sdSize; i++) {
			...
			// Setting Rates between From date and To date
			if ((DateUtility.compare(curSchd.getSchDate(), recalFromDate) >= 0 && DateUtility.compare(
					curSchd.getSchDate(), recalToDate) <= 0)
					|| (i == (sdSize - 1))) {
	...
	private FinScheduleData buildOverdraftSchd(FinScheduleData orgFinScheduleData) {
		...
		if (finMain.getEventFromDate().compareTo(finMain.getFinStartDate()) == 0) {
			...
			for (OverdraftScheduleDetail curODSchd : finScheduleData.getOverdraftScheduleDetails()) {
				if (DateUtility.compare(curODSchd.getDroplineDate(), finMain.getEventFromDate()) > 0) {
					if (StringUtils.isNotEmpty(finMain.getDroplineFrq())) {
						startCalFrom = FrequencyUtil.getNextDate(finMain.getDroplineFrq(), 1,
								prvODSchd.getDroplineDate(), "A", false).getNextFrequencyDate();
		...
		for (int i = 0; i < odSchdDateList.size(); i++) {
			...
			OverdraftScheduleDetail curODSchd = new OverdraftScheduleDetail();
			curODSchd.setDroplineDate(DateUtility.getDBDate(DateUtility.formatDate(odSchdDateList.get(i),
	...
	public FinScheduleData fetchRatesHistory(FinScheduleData finScheduleData) {
		...
		//If no special rates are used then return received data with base rates history
