...
public class ScheduleCalculator {
	...
	private FinScheduleData resetNewSchdDetail(FinScheduleData finScheduleData, Date nextSchdDate, int scheduleFlag) {
		...
		if (finMain.isTDSApplicable() && PennantConstants.TDS_AUTO.equalsIgnoreCase(finMain.getTdsType())) {
			// 124631 TDS applicable flag Added
			sd.setTDSApplicable(lastSchd.isTDSApplicable());
		}
	...
	private FinScheduleData setFinanceTotals(FinScheduleData finScheduleData) {
		...
		for (int i = 0; i < sdSize; i++) {
			...
			if (finMain.isTDSApplicable() && PennantConstants.TDS_AUTO.equalsIgnoreCase(finMain.getTdsType())
					&& tdsPerc.compareTo(BigDecimal.ZERO) != 0) {
					...
					if (StringUtils.equals(curSchd.getBpiOrHoliday(), FinanceConstants.FLAG_HOLIDAY)
						...
						if (finMain.isBpiResetReq()) {
							if (StringUtils.equals(curSchd.getBpiOrHoliday(), FinanceConstants.FLAG_BPI)
								...
								curSchd.setProfitSchd(calProfitToSchd(curSchd, prvSchd, cpzPOSIntact, finMain));
							}
						} else {
							curSchd.setProfitSchd(calProfitToSchd(curSchd, prvSchd, cpzPOSIntact, finMain));
						}
	...
	private FinanceScheduleDetail calPftPriRpy(FinScheduleData finScheduleData, int iCur, int iPrv, Date evtFromDate,
		...
		if (CalculationConstants.SCHMTHD_NOPAY.equals(curSchd.getSchdMethod())) {
			...
			if (!protectPftSchd) {
				BigDecimal pftToSchd = calProfitToSchd(curSchd, prvSchd, cpzPOSIntact, finMain);
			...
			if (!protectPftSchd) {
				schdInterest = calProfitToSchd(curSchd, prvSchd, cpzPOSIntact, finMain);
	...
	public FinScheduleData procMDTRecord(FinScheduleData finScheduleData, int i, boolean isRepayComplete,
		...
		curSchd.setProfitSchd(calProfitToSchd(curSchd, prvSchd, cpzPOSIntact, finScheduleData.getFinanceMain()));
	...
	private FinScheduleData resetRecalData(FinScheduleData finScheduleData, Date eventFromDate, BigDecimal amount,
		...
		Date graceEndDate = finScheduleData.getFinanceMain().getGrcPeriodEndDate();
		...
		for (int i = 0; i < schdList.size(); i++) {
			...
			if (DateUtility.compare(curSchd.getSchDate(), eventFromDate) == 0) {
				finScheduleData.getFinanceMain().setIndexMisc(i);
				openSchd = curSchd;
		...
		if (openSchd == null) {
			...
			openSchd = finScheduleData.getFinanceScheduleDetails().get(prvIndex + 1);
		...
		if (finScheduleData.getFinanceMain().isTDSApplicable()
				&& PennantConstants.TDS_AUTO.equalsIgnoreCase(finScheduleData.getFinanceMain().getTdsType())) {
			openSchd.setTDSApplicable(finScheduleData.getFinanceScheduleDetails().get(prvIndex + 2).isTDSApplicable());
		}

		Date maturityDate = finScheduleData.getFinanceMain().getMaturityDate();

		if (maturityDate.compareTo(SysParamUtil.getAppDate()) < 0) {
			openSchd.setTDSApplicable(finScheduleData.getFinanceScheduleDetails().get(prvIndex + 1).isTDSApplicable());
		} else {
			openSchd.setTDSApplicable(finScheduleData.getFinanceScheduleDetails().get(prvIndex + 2).isTDSApplicable());
		}

		finScheduleData.getFinanceMain().setRecalSchdMethod(recalSchdMethod);
		finScheduleData.getFinanceMain().setRecalFromDate(recalFromDate);
