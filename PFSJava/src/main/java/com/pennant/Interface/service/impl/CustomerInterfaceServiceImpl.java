...
public class CustomerInterfaceServiceImpl implements CustomerInterfaceService {
	...
	private List<AvailAccount> getAccountList(int startIndex, int count, String custRspData, AvailCustomerDetail detail,
			String limitCcy, int limitCcyEdit) {
		...
		for (int i = 0; i < count; i++) {
			...
			if ("-".equals(accData.substring(19, 20))) {
				account.setAcBalance(PennantApplicationUtil
						.amountFormate(new BigDecimal(accData.substring(20, 35)).negate(), ccyFormatter));
			} else {
				account.setAcBalance(
						PennantApplicationUtil.amountFormate(new BigDecimal(accData.substring(20, 35)), ccyFormatter));
			}
		...
		for (int i = 0; i < count; i++) {
			...
			if (!("0".equals(colData.substring(78, 85)) || ("").equals(colData.substring(78, 85)))) {
				if ("9999999".equals(colData.substring(78, 85))) {
					...
					collateral.setCollateralExpiry(DateUtility.formatToLongDate(
							DateUtility.convertDateFromAS400(new BigDecimal(colData.substring(78, 85)))));
				}
			}
			if (!("0".equals(colData.substring(85, 92))) || ("").equals(colData.substring(85, 92))) {
				if ("9999999".equals(colData.substring(85, 92))) {
					...
					collateral.setLastReview(DateUtility.formatToLongDate(
							DateUtility.convertDateFromAS400(new BigDecimal(colData.substring(85, 92)))));
				}
			}
			collateral.setCollateralValue(
					PennantApplicationUtil.amountFormate(new BigDecimal(colData.substring(92, 107)), ccyFormatter));
			collateral.setBankValuation(
					PennantApplicationUtil.amountFormate(new BigDecimal(colData.substring(107, 122)), ccyFormatter));
			collateral
					.setMargin(PennantApplicationUtil
							.formatRate(PennantApplicationUtil
									.formateAmount(new BigDecimal(colData.substring(122, 127)), 3).doubleValue(), 2)
							+ "%");
	 ...
	 */
	@Override
	public CustomerDetails processCustInformation(InterfaceCustomerDetail interfaceCustomerDetail)
		...
		if (interfaceCustomerDetail != null) {
			...
			if (interfaceCustomerDetail.getCustomer() != null) {
				...
				customer.setLovDescCustNationalityName(
						getCodeDescription("NationalityCode", customer.getCustNationality()));
			...
			if (interfaceCustomerDetail.getCustEmployeeDetail() != null) {
				...
				//setting descriptions
				custEmployeeDetail.setLovDescEmpStatus(
						getCodeDescription("CustEmployeeDetail", custEmployeeDetail.getEmpStatus()));
				custEmployeeDetail
						.setLovDescEmpDesg(getCodeDescription("Designation", custEmployeeDetail.getEmpDesg()));
			...
			if (interfaceCustomerDetail.getCustomerPhoneNumList() != null) {
				...
				for (InterfaceCustomerPhoneNumber interfacePhoneNum : interfaceCustomerDetail
	...
	/**
	 * This method will validate whether the customer fields exists in their respective master tables or not
	 * 
		...
		for (CustomerDetails cDetails : customerDetails) {
			...
			if (customer != null) {
				...
				if (("").equals(customer.getCustDftBranch())) {
					...
					masterMissedDetail.setDescription("Customer : " + customer.getCustCIF() + " , '"
							+ customer.getCustDftBranch() + "' Value Does Not Exist In Master RMTBranches Table ");
				...
				if (("").equals(customer.getCustTypeCode())) {
					...
					masterMissedDetail.setDescription("Customer : " + customer.getCustCIF() + " , '"
							+ customer.getCustTypeCode() + "' Value Does Not Exist In Master RMTCustTypes Table ");
					masterValueMissedDetails.add(masterMissedDetail);
					customer.setCustTypeCode("");
				}
				if (customer.getCustGroupID() != 0
						&& !valueExistInMaster(customer.getCustGroupID(), custGrpCodeMasterList)) {
					masterMissedDetail.setFieldName("CustGroupID");
					masterMissedDetail.setDescription("Customer : " + customer.getCustCIF() + " , '"
							+ customer.getCustGroupID() + "' Value Does Not Exist In Master CustomerGroups Table ");
					masterValueMissedDetails.add(masterMissedDetail);
				...
				if (("").equals(customer.getCustCOB())) {
					...
					masterMissedDetail.setDescription("Customer : " + customer.getCustCIF() + " , '"
							+ customer.getCustCOB() + "' Value Does Not Exist In Master BMTCountries Table ");
					masterValueMissedDetails.add(masterMissedDetail);
					customer.setCustCOB("");
				}
				if (("").equals(customer.getCustParentCountry())) {
					...
					masterMissedDetail.setDescription("Customer : " + customer.getCustCIF() + " , '"
							+ customer.getCustParentCountry() + "' Value Does Not Exist In Master BMTCountries Table ");
					masterValueMissedDetails.add(masterMissedDetail);
				...
				if (("").equals(customer.getCustRiskCountry())) {
					...
					masterMissedDetail.setDescription("Customer : " + customer.getCustCIF() + " , '"
							+ customer.getCustRiskCountry() + "' Value Does Not Exist In Master BMTCountries Table ");
					masterValueMissedDetails.add(masterMissedDetail);
				...
				if (("").equals(customer.getCustResdCountry())) {
					...
					masterMissedDetail.setDescription("Customer : " + customer.getCustCIF() + " , '"
							+ customer.getCustResdCountry() + "' Value Does Not Exist In Master BMTCountries Table ");
					masterValueMissedDetails.add(masterMissedDetail);
				...
				if (("").equals(customer.getCustNationality())) {
					...
					masterMissedDetail.setDescription("Customer : " + customer.getCustCIF() + " , '"
							+ customer.getCustNationality() + "' Value Does Not Exist In Master BMTCountries Table ");
					masterValueMissedDetails.add(masterMissedDetail);
				...
				if (("").equals(customer.getCustSalutationCode())) {
					...
					masterMissedDetail.setDescription(
							"Customer : " + customer.getCustCIF() + " , '" + customer.getCustSalutationCode()
									+ "' Value Does Not Exist In Master BMTSalutations Table ");
					masterValueMissedDetails.add(masterMissedDetail);
				...
				if (("").equals(customer.getCustRO1())) {
					...
					masterMissedDetail.setDescription("Customer : " + customer.getCustCIF() + " , '"
							+ customer.getCustRO1() + "' Value Does Not Exist In Master RelationshipOfficers Table ");
					masterValueMissedDetails.add(masterMissedDetail);
					customer.setCustRO1(0);
				}
				if (("").equals(customer.getCustSector()) || ("").equals(customer.getCustSubSector())) {
					...
					masterMissedDetail.setDescription("Customer : " + customer.getCustCIF() + " , CustSector:'"
							+ customer.getCustSector() + "' and CustSubSector:'" + customer.getCustSubSector()
							+ "' Values Does Not Exist In Master BMTSubSectors Table ");
					masterValueMissedDetails.add(masterMissedDetail);
				...
				if (("").equals(customer.getCustMaritalSts())) {
					...
					masterMissedDetail.setDescription(
							"Customer : " + customer.getCustCIF() + " , '" + customer.getCustMaritalSts()
									+ "' Value Does Not Exist In Master BMTMaritalStatusCodes Table ");
					masterValueMissedDetails.add(masterMissedDetail);
				...
				if (("").equals(customer.getCustEmpSts())) {
					...
					masterMissedDetail.setDescription("Customer : " + customer.getCustCIF() + " , '"
							+ customer.getCustEmpSts() + "' Value Does Not Exist In Master BMTEmpStsCodes Table ");
					masterValueMissedDetails.add(masterMissedDetail);
				...
				if (("").equals(customer.getCustBaseCcy())) {
					...
					masterMissedDetail.setDescription("Customer : " + customer.getCustCIF() + " , '"
							+ customer.getCustBaseCcy() + "' Value Does Not Exist In Master RMTCurrencies Table ");
					masterValueMissedDetails.add(masterMissedDetail);
			...
			if (cDetails.getAddressList() != null && !cDetails.getAddressList().isEmpty()) {
				...
				for (CustomerAddres customerAddres : cDetails.getAddressList()) {
					if (!valueExistInMaster(customerAddres.getCustID(), exisitingCustomerList)) {
						...
						masterMissedDetail.setDescription(
								"Customer : " + customer.getCustCIF() + " Value Does Not Exist In Customers Table ");
						masterValueMissedDetails.add(masterMissedDetail);
						...
						masterMissedDetail.setDescription(
								"Customer : " + customer.getCustCIF() + " , '" + customerAddres.getCustAddrType()
			...
			if (cDetails.getCustomerEMailList() != null && !cDetails.getCustomerEMailList().isEmpty()) {
				...
				for (CustomerEMail customerEMail : cDetails.getCustomerEMailList()) {
					if (!valueExistInMaster(customerEMail.getCustID(), exisitingCustomerList)) {
						...
						masterMissedDetail.setDescription(
								"Customer : " + customer.getCustCIF() + " , '" + customerEMail.getCustEMailTypeCode()
	...
	private boolean valueExistInMaster(Customer customer, List<SubSector> list) {
		for (SubSector subSector : list) {
			if (StringUtils.trimToEmpty(customer.getCustSector()).equalsIgnoreCase(subSector.getSectorCode())
					&& StringUtils.trimToEmpty(customer.getCustSubSector())
							.equalsIgnoreCase(subSector.getSubSectorCode())) {
				return true;
			}
		}
		return false;
