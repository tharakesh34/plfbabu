...
public class SchemeDealerGroupServiceImpl extends GenericService<SchemeDealerGroup>
	...
	public AuditHeader saveOrUpdate(AuditHeader auditHeader) {
		...
		if (schemeDealerGroup.isNew()) {
			schemeDealerGroup.setPromotionId(Long.parseLong(schemeDealerGroupDAO.save(schemeDealerGroup, tableType)));
			auditHeader.getAuditDetail().setModelData(schemeDealerGroup);
			auditHeader.setAuditReference(String.valueOf(schemeDealerGroup.getPromotionId()));
		} else {
	...
	public AuditHeader doApprove(AuditHeader auditHeader) {
		...
		if (!PennantConstants.RECORD_TYPE_NEW.equals(schemeDealerGroup.getRecordType())) {
			auditHeader.getAuditDetail()
					.setBefImage(schemeDealerGroupDAO.getSchemeDealerGroup(schemeDealerGroup.getPromotionId(), ""));
		}
