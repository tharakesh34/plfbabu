...
public class AutoWriteOffDAOImpl extends SequenceDao<AutoWriteOffLoan> implements AutoWriteOffDAO {
	...
	public void deleteQueue() {
		jdbcOperations.update("Truncate table AutoWriteOff_Calc_Queue");
	}

	@Override
	public long prepareQueueForEOM() {

		StringBuilder sql = new StringBuilder("Insert Into AutoWriteOff_Calc_Queue(ID, FinID)");
	...
	public long getQueueCount() {
		String sql = "Select count(ID) From AutoWriteOff_Calc_Queue where Progress = ?";
	...
	public int updateThreadID(long from, long to, int threadId) {
		String sql = "Update AutoWriteOff_Calc_Queue Set ThreadId = ? Where Id > ? and Id <= ?  and ThreadId = ?";
	...
	public void updateProgress(long finID, int progress) {
		...
		if (progress == EodConstants.PROGRESS_IN_PROCESS) {
			sql = "Update AutoWriteOff_Calc_Queue Set Progress = ?, StartTime = ? Where FinID = ? and Progress = ?";
			...
			sql = "Update AutoWriteOff_Calc_Queue Set EndTime = ?, Progress = ? where FinID = ?";
			...
			sql = "Update AutoWriteOff_Calc_Queue Set EndTime = ?, ThreadId = ?, Progress = ? Where FinID = ?";
	...
	public void insertlog(AutoWriteOffLoan awl) {
		StringBuilder sql = new StringBuilder("Insert Into Auto_WriteOff_Loans_Log");
		sql.append(" (FinID, FinRef, Code , ErrorMsg , executionDate , Status)");
		sql.append(" Values ( ?, ? ,? ,? ,? ,?)");
		...
		this.jdbcOperations.update(sql.toString(), ps -> {
			...
			ps.setString(index++, awl.getFinRef());
