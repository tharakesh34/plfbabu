...
public class AssetClassificationDAOImpl extends SequenceDao<AssetClassification> implements AssetClassificationDAO {
	...
	public long prepareQueue() {
		String sql = "Insert Into Asset_Classification_Queue(ID, FinReference) Select row_number() over(order by FinReference) ID, FinReference From (Select distinct FinReference From Npa_Provision_Stage) T";
	...
	public List<NpaProvisionStage> getPastDueInfoFromStage(Set<String> finReferences) {
		StringBuilder sql = new StringBuilder("Select");
		sql.append(" FinReference, PastDueDays, PastDueDate From Npa_Provision_Stage");
