...
public class AccountNumberUtil implements Serializable {
	...
	private int internalSysAccLength = Integer.parseInt(SystemParameterDetails.getSystemParameterValue("SYSINT_ACCOUNT_LEN").toString());
	 ...
	 * @param ccy
	 */
	public String getBasicAccountNumber(TransactionEntry transactionEntry,Object object,String ccy){
		logger.debug("Entering");
		if(object instanceof SubHeadRule){
			//Get Account Basic Number By Executing Rule
			String ruleAcNum = getAccountByRule(transactionEntry, object, ccy);

			//Fetch Internal Account Using Account Type
			String internalSIANumber = getInternalAccount(transactionEntry.getAccountType());

			return mergeAccountNumber(internalSIANumber,ruleAcNum);
		}
	 ...
	 * @return
	 */
	private String getAccountByRule(TransactionEntry transactionEntry,Object object,String ccy){
		logger.debug("Entering");
		
		String amountRule = getRuleDAO().getAmountRule(transactionEntry.getAccountSubHeadRule(), "SUBHEAD", "");
		
		String ruleAccount = (String) getRuleExecutionUtil().executeRule(amountRule, object, SystemParameterDetails.getGlobaVariableList(),ccy);
