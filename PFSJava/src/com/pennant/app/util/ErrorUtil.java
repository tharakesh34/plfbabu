...
public class ErrorUtil implements Serializable {
	...
	private ArrayList<ErrorDetails> errorDetails=null;
	...
	public static ArrayList<ErrorDetails> getErrorDetails(ArrayList<ErrorDetails> errorDetails,String errorLanguage){
		return new ErrorUtil(errorDetails, errorLanguage).getErrorDetails();
	} 
	
	private ErrorUtil(ArrayList<ErrorDetails> errorDetails,String errorLanguage){
		if(errorDetails!=null && errorDetails.size()!=0){
			String errorCodeList = getErrorCodeList(errorDetails);
			HashMap<String, ErrorDetails> hashMap = getErrorsByErrorCodes(errorLanguage,errorCodeList);
			this.errorDetails= new ArrayList<ErrorDetails>();
			
			for (int i = 0; i < errorDetails.size(); i++) {
				this.errorDetails.add(copyErrorDetails(errorDetails.get(i), hashMap.get(errorDetails.get(i).getErrorCode())));
			}
		}
	}
	
	private String getErrorCodeList(ArrayList<ErrorDetails> errorDetails){
