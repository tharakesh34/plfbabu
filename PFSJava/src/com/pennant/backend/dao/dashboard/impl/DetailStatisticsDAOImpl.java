...
public class DetailStatisticsDAOImpl extends BasisCodeDAO<DetailStatistics> implements DetailStatisticsDAO{
	...
	public List<DetailStatistics> getAuditDetails() {
		...
		StringBuilder   selectSql = new StringBuilder("SELECT T1.AuditId, T1.AuditDate, AuditModule AS ModuleName, AuditReference ");
		selectSql.append("FROM AuditHeader AS T1 INNER JOIN ");
		selectSql.append("DetailStaticAudit AS T2 ON T1.AuditId > T2.AuditId AND T1.AuditDate > T2.AuditDate  WHERE AuditTranType='W'");
	...
	public DetailStatistics getAuditDetail(DetailStatistics detailStatistics){
		logger.debug("Entering ");
		StringBuilder   selectSql = new StringBuilder("SELECT AuditId, AuditDate, LastMntOn, RoleCode as CurrentRoleCode, NextRoleCode ");
		selectSql.append("FROM adt");
		selectSql.append(PennantJavaUtil.getTabelMap(detailStatistics.getModuleName()));
