...
public class DashboardConfigurationDAOImpl extends BasisCodeDAO<DashboardConfiguration>
implements DashboardConfigurationDAO {

	private static Logger logger = Logger
	.getLogger(DashboardConfigurationDAOImpl.class);
	...
	public DashboardConfiguration getDashboardDetailByID(final String id, String type) {
		...
		selectSql.append("  Query,remarks,DrillDownChart,multiSeries,SeriesType,SeriesValues,FieldQuery,DataXML,ColorRangeXML,AdtDataSource,");
	...
	public void initialize(DashboardConfiguration dashboardConfiguration) {
		super.initialize(dashboardConfiguration);
	}
	 ...
	 */
	@Override
	public void refresh(DashboardConfiguration dashboardConfiguration) {
	...
	public void delete(DashboardConfiguration dashboardConfiguration, String type) {
		...
		try {
			recordCount = this.namedParameterJdbcTemplate.update(deleteSql.toString(),
					beanParameters);
			if (recordCount <= 0) {
				ErrorDetails errorDetails = getError("41004",dashboardConfiguration.getId()
						,dashboardConfiguration.getUserDetails().getUsrLanguage());
			...
			logger.error(e);
			ErrorDetails errorDetails = getError("41006",dashboardConfiguration.getId()
					,dashboardConfiguration.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {
			};
		}
	...
	public String save(DashboardConfiguration dashboardConfiguration, String type) {
		...
		insertSql.append("  Dimension,Caption, SubCaption, Query, Remarks, DrillDownChart, MultiSeries, SeriesType, SeriesValues, FieldQuery, DataXML, ColorRangeXML, AdtDataSource,");
	...
	public void update(DashboardConfiguration dashboardConfiguration, String type) {
		...
		updateSql.append(" SeriesType=:SeriesType, SeriesValues=:SeriesValues, FieldQuery=:FieldQuery, DataXML=:DataXML, ColorRangeXML=:ColorRangeXML, AdtDataSource=:AdtDataSource,  ");
		...
		if (recordCount <= 0) {
			...
			ErrorDetails errorDetails = getError(
					"41003",dashboardConfiguration.getId(), dashboardConfiguration.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {
			};
		}
		logger.debug("Leaving");
	...
	/**
	 * Get the new chart Details 
	 */
	public ChartDetail getNewChartDetails() {
	...
	public void SavePositions(DashboardPosition dashboardPosition) {
		...
		insertSql.append(" (UsrId, DashboardRef, DashboardCol, DashboardRow, ");
		...
		insertSql.append(" Values(:UsrId, :DashboardRef, :DashboardCol, :DashboardRow,");
		insertSql.append(" :Version , :LastMntBy, :LastMntOn, :RecordStatus,");
		insertSql.append(" :RoleCode, :NextRoleCode, :TaskId, :NextTaskId, :RecordType, :WorkflowId)");
