...
public class CustomerChequeInfoDAOImpl extends BasisCodeDAO<CustomerChequeInfo> implements CustomerChequeInfoDAO {
	...
	public void delete(CustomerChequeInfo customerChequeInfo,String type) {
		...
		try{
			...
			if (recordCount <= 0) {
				ErrorDetails errorDetails= getError("41004", customerChequeInfo.getCustID(),
						customerChequeInfo.getChequeSeq(), customerChequeInfo.getUserDetails().getUsrLanguage());
				throw new DataAccessException(errorDetails.getError()) {};
			}
		}catch(DataAccessException e){
			logger.error(e);
			ErrorDetails errorDetails= getError("41006", customerChequeInfo.getCustID(), 
					customerChequeInfo.getChequeSeq(), customerChequeInfo.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {};
		}
		logger.debug("Leaving");
	...
	public long save(CustomerChequeInfo customerChequeInfo,String type) {
		...
		insertSql.append(" (CustID, ChequeSeq, MonthYear, TotChequePayment, Salary, ReturnChequeAmt, ReturnChequeCount, " );
		insertSql.append(" Version , LastMntBy, LastMntOn, RecordStatus, RoleCode, NextRoleCode," );
		insertSql.append(" TaskId, NextTaskId, RecordType, WorkflowId)");
		insertSql.append(" Values(:CustID, :ChequeSeq, :MonthYear, :TotChequePayment, :Salary, :ReturnChequeAmt, :ReturnChequeCount,");
		insertSql.append(" :Version , :LastMntBy, :LastMntOn, :RecordStatus, :RoleCode, :NextRoleCode," );
		insertSql.append(" :TaskId, :NextTaskId, :RecordType, :WorkflowId)");
	...
	public void update(CustomerChequeInfo customerChequeInfo,String type) {
		...
		updateSql.append(" MonthYear = :MonthYear, TotChequePayment = :TotChequePayment, Salary = :Salary,");
		updateSql.append(" ReturnChequeAmt = :ReturnChequeAmt, ReturnChequeCount = :ReturnChequeCount, ");
		...
		if (recordCount <= 0) {
			...
			ErrorDetails errorDetails= getError("41003", customerChequeInfo.getCustID(), 
					customerChequeInfo.getChequeSeq(), customerChequeInfo.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {};
		}
		logger.debug("Leaving");
	...
	private ErrorDetails  getError(String errorId, long customerId,int chequeSeq, String userLanguage){
		String[][] parms= new String[2][2]; 
		
		parms[1][0] = String.valueOf(customerId);
		parms[1][1] = String.valueOf(chequeSeq);

		parms[0][0] = PennantJavaUtil.getLabel("label_CustID")+ ":" + parms[1][0];
		parms[0][1] = PennantJavaUtil.getLabel("label_ChequeSeq")+ ":" + parms[1][1];
