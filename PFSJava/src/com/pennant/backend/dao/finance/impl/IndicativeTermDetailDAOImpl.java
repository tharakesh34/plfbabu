...
public class IndicativeTermDetailDAOImpl extends BasisCodeDAO<IndicativeTermDetail> implements IndicativeTermDetailDAO {
    ...
    public IndicativeTermDetail getIndicateTermByRef(String finReference, String type, boolean isWIF) {
		...
		if (StringUtils.trimToEmpty(type).contains("View")) {
			...
			selectSql.append(" ,lovDescTotalFacilityCCYName, lovDescUnderWritingCCYName ,lovDescPropFinalTakeCCYName " );
			selectSql.append(" ,lovDescTotalFacilityFormatter, lovDescUnderWritingFormatter ,lovDescPropFinalTakeFormatter " );
		}
		if (isWIF) {
    ...
    public void update(IndicativeTermDetail detail, String type, boolean isWIF) {
		...
		if (recordCount <= 0) {
			...
			ErrorDetails errorDetails = getError("41004", detail.getId(), detail.getUserDetails().getUsrLanguage());
    ...
    public void delete(IndicativeTermDetail detail, String type, boolean isWIF) {
		...
		try {
			...
			if (recordCount <= 0) {
				ErrorDetails errorDetails = getError("41003", detail.getId(), detail.getUserDetails().getUsrLanguage());
				throw new DataAccessException(errorDetails.getError()) { };
			}
		} catch (DataAccessException e) {
			logger.error(e);
			ErrorDetails errorDetails = getError("41006", detail.getId(), detail.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) { };
		}
		logger.debug("Leaving");
    }
