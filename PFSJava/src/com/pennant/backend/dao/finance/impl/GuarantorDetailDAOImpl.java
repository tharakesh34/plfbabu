...
public class GuarantorDetailDAOImpl extends BasisNextidDaoImpl<GuarantorDetail> implements
	...
	public void initialize(GuarantorDetail guarantorDetail) {
		super.initialize(guarantorDetail);
	}
	 ...
	 */
	@Override
	public void refresh(GuarantorDetail guarantorDetail) {
	...
	public void delete(GuarantorDetail guarantorDetail, String type) {
		...
		try {
			recordCount = this.namedParameterJdbcTemplate.update(deleteSql.toString(),
			        beanParameters);
			if (recordCount <= 0) {
				ErrorDetails errorDetails = getError("41003", String.valueOf(guarantorDetail
				        .getGuarantorId()), guarantorDetail.getUserDetails().getUsrLanguage());
				throw new DataAccessException(errorDetails.getError()) {
				};
			}
		} catch (DataAccessException e) {
			logger.error(e);
			ErrorDetails errorDetails = getError("41006", String.valueOf(guarantorDetail
			        .getGuarantorId()), guarantorDetail.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {
			};
		}
	...
	public void update(GuarantorDetail guarantorDetail, String type) {
		...
		if (recordCount <= 0) {
			...
			ErrorDetails errorDetails = getError("41004", String.valueOf(guarantorDetail
			        .getGuarantorId()), guarantorDetail.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {
			};
		}
	...
	private ErrorDetails getError(String errorId, String guarantorId, String userLanguage) {
		String[][] parms = new String[2][1];
		parms[1][0] = guarantorId;
		parms[0][0] = PennantJavaUtil.getLabel("label_GuarantorId") + ":" + parms[1][0];
	...
	public List<GuarantorDetail> getGuarantorDetailByFinRef(String finReference, String type) {
		...
		selectSql.append(" EmailId, GuarantorProofName, GuarantorIDTypeName, "); // GuarantorProof, 
		...
		selectSql.append(" RecordType, WorkflowId,GuarantorIDTypeName");
    ...
    public List<FinanceExposure> getPrimaryExposureList(GuarantorDetail guarantorDetail) {
		...
		query.append(" T1.FinAmount as FinanceAmt, T1.TotalPriBal as CurrentExpoSure,"); 
		query.append(" T1.TotalPriBal as CurrentExpoSure, T1.FinCcy, T1.CustCIF,T2.ccyEditField as ccyEditField,"); 
		query.append(" isNull((SELECT SUM(FinCurODAmt) from FinODDetails where FinReference=T1.FinReference),0) as OverdueAmt,"); 
		query.append(" isNull((SELECT MAX(FinCurODDays) from FinODDetails where FinReference=T1.FinReference),0) as PastdueDays"); 
		query.append(" FROM FinPftDetails AS T1 INNER JOIN RMTCurrencies T2 ON T2.CcyCode = T1.FinCcy "); 
    ...
    public List<FinanceExposure> getSecondaryExposureList(GuarantorDetail guarantorDetail) {
		...
		query.append(" isNull((SELECT MAX(FinCurODDays) from FinODDetails where FinReference=T1.FinReference),0) as PastdueDays "); 
		query.append(" FROM FinPftDetails AS T1 INNER JOIN RMTCurrencies T2 ON T2.CcyCode = T1.FinCcy "); 
		query.append(" INNER JOIN FinJointAccountDetails_View as T3 on T1.FinReference=T3.FinReference "); 
    ...
    public List<FinanceExposure> getGuarantorExposureList(GuarantorDetail guarantorDetail) {
		...
		query.append(" isNull((SELECT MAX(FinCurODDays) from FinODDetails where FinReference=T1.FinReference),0) as PastdueDays "); 
		query.append(" FROM FinPftDetails AS T1 INNER JOIN RMTCurrencies T2 ON T2.CcyCode = T1.FinCcy "); 
		query.append(" INNER JOIN FinGuarantorsDetails_View as T3 on T1.FinReference=T3.FinReference "); 
