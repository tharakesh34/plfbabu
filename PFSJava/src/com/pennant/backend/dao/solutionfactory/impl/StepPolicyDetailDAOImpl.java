...
public class StepPolicyDetailDAOImpl extends BasisCodeDAO<StepPolicyDetail> implements StepPolicyDetailDAO {
	...
	public void initialize(StepPolicyDetail stepPolicyDetail) {
		super.initialize(stepPolicyDetail);
	}
	 ...
	 */
	@Override
	public void refresh(StepPolicyDetail stepPolicyDetail) {
	...
	public void update(StepPolicyDetail stepPolicyDetail, String type) {
		...
		if (recordCount <= 0) {
			...
			ErrorDetails errorDetails= getError("41004", stepPolicyDetail.getPolicyCode(),
					String.valueOf(stepPolicyDetail.getStepNumber()),  stepPolicyDetail.getUserDetails().getUsrLanguage());
	...
	public void delete(StepPolicyDetail stepPolicyDetail,String type) {
		...
		try{
			...
			if (recordCount <= 0) {
				ErrorDetails errorDetails= getError("41003",stepPolicyDetail.getPolicyCode(),String.valueOf(stepPolicyDetail.getStepNumber()),stepPolicyDetail.getUserDetails().getUsrLanguage());
				throw new DataAccessException(errorDetails.getError()) {};
			}
		}catch(DataAccessException e){
			logger.error(e);
			ErrorDetails errorDetails= getError("41006",stepPolicyDetail.getPolicyCode(),String.valueOf(stepPolicyDetail.getStepNumber()),stepPolicyDetail.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {};
		}
		logger.debug("Leaving");
	}
