...
public class IdentityDetailsDAOImpl extends BasisCodeDAO<IdentityDetails> implements IdentityDetailsDAO {
	...
	public IdentityDetails getIdentityDetails() {
		...
		WorkFlowDetails workFlowDetails = WorkFlowUtil.getWorkFlowDetails("IdentityDetails");
		IdentityDetails identityDetails = new IdentityDetails();
		if (workFlowDetails != null) {
			identityDetails.setWorkflowId(workFlowDetails.getWorkFlowId());
		}
	...
	/**
	 * This method get the module from method getIdentityDetails() and set the
	 * new record flag as true and return IdentityDetails()
	 * 
	 * @return IdentityDetails
	 */
	@Override
	...
	public void initialize(IdentityDetails identityDetails) {
		super.initialize(identityDetails);
	}
	 ...
	 */
	@Override
	public void refresh(IdentityDetails identityDetails) {
	...
	public void delete(IdentityDetails identityDetails, String type) {
		...
		try {
			...
			if (recordCount <= 0) {
				ErrorDetails errorDetails= getError("41004", identityDetails.getIdentityType(), 
					identityDetails.getUserDetails().getUsrLanguage());
			...
			logger.error(e);
			ErrorDetails errorDetails= getError("41006", identityDetails.getIdentityType(), 
					identityDetails.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {
			};
		}
	...
	public void update(IdentityDetails identityDetails, String type) {
		...
		if (recordCount <= 0) {
			...
			ErrorDetails errorDetails= getError("41003", identityDetails.getIdentityType(), 
					identityDetails.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {
			};
		}
	 ...
	 * @return ErrorDetails
	 */
	private ErrorDetails  getError(String errorId, String identityType,String userLanguage){
		String[][] parms= new String[2][2]; 
		parms[1][0] = identityType;
		parms[0][0] = PennantJavaUtil.getLabel("label_IdentityType")+ ":" + parms[1][0];
