...
public class PRelationCodeDAOImpl extends BasisCodeDAO<PRelationCode> implements PRelationCodeDAO {
	...
	public PRelationCode getPRelationCode() {
		...
		WorkFlowDetails workFlowDetails = WorkFlowUtil.getWorkFlowDetails("PRelationCode");
		PRelationCode pRelationCode = new PRelationCode();
		if (workFlowDetails != null) {
			pRelationCode.setWorkflowId(workFlowDetails.getWorkFlowId());
		}
	...
	/**
	 * This method get the module from method getPRelationCode() and set the new
	 * record flag as true and return PRelationCode()
	 * 
	 * @return PRelationCode
	 */
	@Override
	...
	public void initialize(PRelationCode pRelationCode) {
		super.initialize(pRelationCode);
	}
	 ...
	 */
	@Override
	public void refresh(PRelationCode pRelationCode) {
	...
	public void delete(PRelationCode pRelationCode, String type) {
		...
		try {
			...
			if (recordCount <= 0) {
				ErrorDetails errorDetails = getError("41004",pRelationCode.getPRelationCode(), 
					pRelationCode.getUserDetails().getUsrLanguage());
			...
			logger.error(e);
			ErrorDetails errorDetails = getError("41006",pRelationCode.getPRelationCode(), 
					pRelationCode.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {
			};
		}
	...
	public void update(PRelationCode pRelationCode, String type) {
		...
		if (recordCount <= 0) {
			...
			ErrorDetails errorDetails = getError("41003",pRelationCode.getPRelationCode(), 
					pRelationCode.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {
			};
		}
	 ...
	 * @return ErrorDetails
	 */
	private ErrorDetails  getError(String errorId, String pRelationCode, String userLanguage){
		String[][] parms= new String[2][1]; 
		parms[1][0] = String.valueOf(pRelationCode);
		parms[0][0] = PennantJavaUtil.getLabel("label_PRelationCode")+ ":" + parms[1][0];
