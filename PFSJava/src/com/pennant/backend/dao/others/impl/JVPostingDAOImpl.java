...
public class JVPostingDAOImpl extends BasisNextidDaoImpl<JVPosting> implements JVPostingDAO {
	...
	public JVPosting getJVPostingById(final long id, String type) {
		...
		if (StringUtils.trimToEmpty(type).contains("View")) {
			selectSql.append(",CurrencyEditField,CurrencyDesc, CcyNumber");
		}
	...
	public JVPosting getJVPostingByFileName(String batchName) {
		...
		selectSql.append(" Where Batch ='"+batchName+"' AND PostingDate='"+DateUtility.getSystemDate()+"'");
		...
		try {
			...
			if (recordCount <= 0) {
				ErrorDetails errorDetails = getError("41003",
				        jVPosting.getId() + "", jVPosting.getUserDetails().getUsrLanguage());
	...
	public long save(JVPosting jVPosting, String type) {
		...
		insertSql
		        .append(" (BatchReference,PostingDate, Batch, Filename,Branch, DebitCount, CreditsCount, TotDebitsByBatchCcy, TotCreditsByBatchCcy, BatchPurpose, Currency, ExchangeRateType, ValidationStatus , BatchPostingStatus");
		insertSql
		        .append(", Version , LastMntBy, LastMntOn, RecordStatus, RoleCode, NextRoleCode, TaskId, NextTaskId, RecordType, WorkflowId)");
		insertSql
		        .append(" Values(:BatchReference,:PostingDate, :Batch,  :Filename,:Branch, :DebitCount, :CreditsCount, :TotDebitsByBatchCcy, :TotCreditsByBatchCcy, :BatchPurpose, :Currency, :ExchangeRateType, :ValidationStatus, :BatchPostingStatus");
		insertSql
		        .append(", :Version , :LastMntBy, :LastMntOn, :RecordStatus, :RoleCode, :NextRoleCode, :TaskId, :NextTaskId, :RecordType, :WorkflowId)");
