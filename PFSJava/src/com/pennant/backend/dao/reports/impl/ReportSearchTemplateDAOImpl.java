...
public class ReportSearchTemplateDAOImpl  implements ReportSearchTemplateDAO{
	 ...
	 */
	@Override
	public List<ReportSearchTemplate> getReportSearchTemplateByReportId(
			long reportID,long usrID) {
		logger.debug("Entering ");
		...
		try{
			...
			RowMapper< ReportSearchTemplate> typeRowMapper = ParameterizedBeanPropertyRowMapper.newInstance(ReportSearchTemplate.class);
			logger.debug("Leaving ");
	...
	public int  getRecordCountByTemplateName(long reportId,long usrId,String templateName){
		...
		try{
			status=this.namedParameterJdbcTemplate.queryForInt(selectSql.toString(), beanParameters);
	...
	public void delete(long reportId, long usrId, String templateName) {
		...
		try{
			...
			logger.debug("Error Method"+e.toString());
			throw e; 
		}
		logger.debug("Leaving");
	}
