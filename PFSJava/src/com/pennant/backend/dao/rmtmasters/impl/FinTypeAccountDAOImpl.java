...
public class FinTypeAccountDAOImpl extends BasisCodeDAO<FinTypeAccount> implements FinTypeAccountDAO {
	...
	public List<FinTypeAccount> getFinTypeAccountListByID(final String id, String type) {
		...
		selectSql.append(" AlwCustomerAccount, AccountReceivable, CustAccountTypes,");
	...
	public void initialize(FinTypeAccount finTypeAccount) {
		super.initialize(finTypeAccount);
	}
	 ...
	 */
	@Override
	public void refresh(FinTypeAccount finTypeAccount) {
	...
	public String save(FinTypeAccount finTypeAccount, String type) {
		...
		insertSql.append(" AlwCustomerAccount, AccountReceivable, CustAccountTypes," );
	...
	public void update(FinTypeAccount finTypeAccount, String type) {
		...
		updateSql.append(" Set FinType = :FinType, FinCcy = :FinCcy, Event = :Event,");
		updateSql.append(" AlwManualEntry = :AlwManualEntry,AlwCustomerAccount = :AlwCustomerAccount,");
		...
		if (recordCount <= 0) {
			...
			ErrorDetails errorDetails= getError("41004", finTypeAccount.getFinType(),
					finTypeAccount.getFinCcy(),finTypeAccount.getEvent(),  finTypeAccount.getUserDetails().getUsrLanguage());
	...
	public void delete(FinTypeAccount finTypeAccount,String type) {
		...
		try{
			...
			if (recordCount <= 0) {
				ErrorDetails errorDetails= getError("41003",finTypeAccount.getFinType(),finTypeAccount.getFinCcy(),finTypeAccount.getEvent() ,finTypeAccount.getUserDetails().getUsrLanguage());
				throw new DataAccessException(errorDetails.getError()) {};
			}
		}catch(DataAccessException e){
			logger.error(e);
			ErrorDetails errorDetails= getError("41006",finTypeAccount.getFinType(),finTypeAccount.getFinCcy(),finTypeAccount.getEvent(),finTypeAccount.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {};
		}
		logger.debug("Leaving");
	}
	...
	private ErrorDetails  getError(String errorId, String finType,String finCcy,
		...
		parms[0][0] = PennantJavaUtil.getLabel("label_FinType")+ ":" + parms[1][0]
		                +" "+ PennantJavaUtil.getLabel("label_FinCcy")+ ":" + parms[1][1];
