...
public class SecurityRoleDAOImpl extends BasisNextidDaoImpl<SecurityRole> implements SecurityRoleDAO {
	...
	/**
	 * This method get the module from method getSecurityRole() and 
	 * set the new record flag as true and return SecurityRole()   
	 * @return SecurityRole
	 */
	@Override
	 ...
	 */
	@Override
	public void initialize(SecurityRole secRoles) {
		super.initialize(secRoles);
	}
	/**
	 ...
	 */
	@Override
	public void refresh(SecurityRole secRoles) {
	...
	public void delete(SecurityRole secRoles,String type) {
		...
		try {
			if (recordCount <= 0) {
				ErrorDetails errorDetail= getError("41003", secRoles.getRoleCd(),
						secRoles.getUserDetails().getUsrLanguage());
			...
			logger.error(e);
			ErrorDetails errorDetail= getError("41006", secRoles.getRoleCd(), 
					secRoles.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetail.getError()){}; 
		}
		logger.debug("Leaving");
	...
	public void update(SecurityRole secRoles,String type) {
		...
		updateSql.append(" Set RoleID = :RoleID, RoleApp = :RoleApp, RoleCd = :RoleCd, " );
		...
		if (recordCount <= 0) {
			...
			ErrorDetails errorDetail= getError("41004", secRoles.getRoleCd(),
					secRoles.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetail.getError()) {};
		}
		logger.debug("Leaving");
	...
	private ErrorDetails  getError(String errorId, String roleCode, String userLanguage){
