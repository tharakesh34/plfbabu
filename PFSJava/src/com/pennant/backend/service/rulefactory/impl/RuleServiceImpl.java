...
public class RuleServiceImpl extends GenericService<Rule> implements RuleService {
	...
	@Override
	public Rule getRule() {
		return getRuleDAO().getRule();
	}

	@Override
	public Rule getNewRule() {
		return getRuleDAO().getNewRule();
	}
	 ...
	 */
	@Override
	public Rule refresh(Rule rule) {
		logger.debug("Entering");
		getRuleDAO().refresh(rule);
		getRuleDAO().initialize(rule);
		logger.debug("Leaving");
		return rule;
	}
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage,
		...
		errParm[0] = PennantJavaUtil.getLabel("label_Rule") + ":"+ valueParm[0]+ " "+PennantJavaUtil.getLabel("label_RuleModule") + ":"+ valueParm[1];
		errParm[1] = PennantJavaUtil.getLabel("label_RuleEvent") + ":"+ valueParm[2];
		...
		if (StringUtils.trimToEmpty(method).equals("doApprove") || !rule.isWorkflow()) {
