...
public class CourseTypeServiceImpl extends GenericService<CourseType> implements CourseTypeService {
	...
	public CourseType getCourseType() {
		return getCourseTypeDAO().getCourseType();
	}
	public CourseType getNewCourseType() {
		return getCourseTypeDAO().getNewCourseType();
	}
	 ...
	 */
	@Override
	public CourseType refresh(CourseType courseType) {
		logger.debug("Entering");
		getCourseTypeDAO().refresh(courseType);
		getCourseTypeDAO().initialize(courseType);
		logger.debug("Leaving");
		return courseType;
	}
	...
	private AuditDetail validation(AuditDetail auditDetail,String usrLanguage,String method){
		...
		CourseType old_CourseType= courseType.getBefImage();
		...
		if (courseType.isNew()){ // for New record or new record into work flow
			...
			if (!courseType.isWorkflow()){	// With out Work flow for update and delete
				...
				if (befCourseType ==null){ // if records not exists in the main table
					...
					if (old_CourseType!=null && !old_CourseType.getLastMntOn().equals(befCourseType.getLastMntOn())){
				...
				if (old_CourseType!=null && !old_CourseType.getLastMntOn().equals(tempCourseType.getLastMntOn())){ 
					auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(new ErrorDetails(PennantConstants.KEY_FIELD, "41005", errParm,valueParm), usrLanguage));
				}
			}
		}
		...
		if(StringUtils.trimToEmpty(method).equals("doApprove") || !courseType.isWorkflow()){
