...
public class AuthorizationServiceImpl extends GenericService<Authorization> implements AuthorizationService {
	...
	public Authorization getAuthorization() {
		return getAuthorizationDAO().getAuthorization();
	}
	/**
	 * @return the authorization for New Record
	 */
	@Override
	public Authorization getNewAuthorization() {
		return getAuthorizationDAO().getNewAuthorization();
	}
	...
	private AuditHeader saveOrUpdate(AuditHeader auditHeader,boolean online) {
		...
		if (authorization.isNew()) {
			...
			auditHeader.setAuditReference(String.valueOf(authorization.getAuthorizedId()));
		}else{
	 ...
	 */
	@Override
	public Authorization refresh(Authorization authorization) {
		logger.debug("Entering");
		getAuthorizationDAO().refresh(authorization);
		getAuthorizationDAO().initialize(authorization);
		logger.debug("Leaving");
		return authorization;
	}
	 ...
	 */
		...
		private AuditDetail validation(AuditDetail auditDetail,String usrLanguage,String method,boolean onlineRequest){
			...
			valueParm[1]=String.valueOf(authorization.getAuthType());
			errParm[0]=PennantJavaUtil.getLabel("label_AuthorizedId")+":"+valueParm[0];
			errParm[1]=PennantJavaUtil.getLabel("label_AuthUserId")+":"+valueParm[1];
			...
			if(StringUtils.trimToEmpty(method).equals("doApprove") || !authorization.isWorkflow()){
