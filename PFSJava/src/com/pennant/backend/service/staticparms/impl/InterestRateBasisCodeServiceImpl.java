...
public class InterestRateBasisCodeServiceImpl extends GenericService<InterestRateBasisCode> implements InterestRateBasisCodeService {
	...
	public InterestRateBasisCode getInterestRateBasisCode() {
		return getInterestRateBasisCodeDAO().getInterestRateBasisCode();
	}

	public InterestRateBasisCode getNewInterestRateBasisCode() {
		return getInterestRateBasisCodeDAO().getNewInterestRateBasisCode();
	}
	 ...
	 */
	@Override
	public InterestRateBasisCode refresh(InterestRateBasisCode interestRateBasisCode) {
		logger.debug("Entering");
		getInterestRateBasisCodeDAO().refresh(interestRateBasisCode);
		getInterestRateBasisCodeDAO().initialize(interestRateBasisCode);
		logger.debug("Leaving");
		return interestRateBasisCode;
	}
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage,
		...
		InterestRateBasisCode old_InterestRateBasisCode = interestRateBasisCode
		.getBefImage();
		...
		if (interestRateBasisCode.isNew()) { // for New record or new record
			...
			if (!interestRateBasisCode.isWorkflow()) { // With out Work flow for update and delete
				...
				if (befInterestRateBasisCode == null) { // if records not exists in the main table
					...
					if (old_InterestRateBasisCode != null
							&& !old_InterestRateBasisCode.getLastMntOn()
				...
				if (tempInterestRateBasisCode != null
						&& old_InterestRateBasisCode != null
						&& !old_InterestRateBasisCode.getLastMntOn().equals(tempInterestRateBasisCode.getLastMntOn())) {
					auditDetail
					.setErrorDetail(new ErrorDetails(PennantConstants.KEY_FIELD, "41005",errParm, null));
				}
			}
