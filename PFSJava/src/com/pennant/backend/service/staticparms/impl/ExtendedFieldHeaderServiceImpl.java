...
public class ExtendedFieldHeaderServiceImpl extends GenericService<ExtendedFieldHeader> implements ExtendedFieldHeaderService {
	...
	public ExtendedFieldHeader getExtendedFieldHeader() {
		return getExtendedFieldHeaderDAO().getExtendedFieldHeader();
	}

	@Override
	public ExtendedFieldHeader getNewExtendedFieldHeader() {
		return getExtendedFieldHeaderDAO().getNewExtendedFieldHeader();
	}
	 ...
	 */
	@Override
	public ExtendedFieldHeader refresh(ExtendedFieldHeader extendedFieldHeader) {
		logger.debug("Entering");
		getExtendedFieldHeaderDAO().refresh(extendedFieldHeader);
		getExtendedFieldHeaderDAO().initialize(extendedFieldHeader);
		logger.debug("Leaving");
		return extendedFieldHeader;
	}
	 ...
	 * 		a)  DELETE	Delete the record from the main table by using getExtendedFieldHeaderDAO().delete with parameters extendedFieldHeader,""
	 * 		b)  NEW		Add new record in to main table by using getExtendedFieldHeaderDAO().save with parameters extendedFieldHeader,""
	 * 		c)  EDIT	Update record in the main table by using getExtendedFieldHeaderDAO().update with parameters extendedFieldHeader,""
	 * 3)	Delete the record from the workFlow table by using getExtendedFieldHeaderDAO().delete with parameters extendedFieldHeader,"_Temp"
	 * 4)	Audit the record in to AuditHeader and AdtExtendedFieldHeader by using auditHeaderDAO.addAudit(auditHeader) for Work flow
	 * 5)  	Audit the record in to AuditHeader and AdtExtendedFieldHeader by using auditHeaderDAO.addAudit(auditHeader) based on the transaction Type.
	 * @param AuditHeader (auditHeader)    
	 * @return auditHeader
	 */
	...
	private AuditDetail validation(AuditDetail auditDetail,String usrLanguage,String method){
		...
		valueParm[0]=String.valueOf(Labels.getLabel(extendedFieldHeader.getModuleName()));
		valueParm[1]=String.valueOf(extendedFieldHeader.getSubModuleName());
		...
		if(StringUtils.trimToEmpty(method).equals("doApprove") || !extendedFieldHeader.isWorkflow()){
