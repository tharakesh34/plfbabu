...
public class LovFieldCodeServiceImpl extends GenericService<LovFieldCode> implements LovFieldCodeService {
	...
	public LovFieldCode getLovFieldCode() {
		return getLovFieldCodeDAO().getLovFieldCode();
	}
	public LovFieldCode getNewLovFieldCode() {
		return getLovFieldCodeDAO().getNewLovFieldCode();
	}
	 ...
	 */
	@Override
	public LovFieldCode refresh(LovFieldCode lovFieldCode) {
		logger.debug("Entering");
		getLovFieldCodeDAO().refresh(lovFieldCode);
		getLovFieldCodeDAO().initialize(lovFieldCode);
		logger.debug("Leaving");
		return lovFieldCode;
	}
	 ...
	 * 		a)  DELETE	Delete the record from the main table by using getLovFieldCodeDAO().delete with parameters lovFieldCode,""
	 * 		b)  NEW		Add new record in to main table by using getLovFieldCodeDAO().save with parameters lovFieldCode,""
	 * 		c)  EDIT	Update record in the main table by using getLovFieldCodeDAO().update with parameters lovFieldCode,""
	 * 3)	Delete the record from the workFlow table by using getLovFieldCodeDAO().delete with parameters lovFieldCode,"_Temp"
	 * 4)	Audit the record in to AuditHeader and AdtBMTLovFieldCode by using auditHeaderDAO.addAudit(auditHeader) for Work flow
	 * 5)  	Audit the record in to AuditHeader and AdtBMTLovFieldCode by using auditHeaderDAO.addAudit(auditHeader) based on the transaction Type.
	 * @param AuditHeader (auditHeader)    
	 * @return auditHeader
	 */
	...
	private AuditDetail validation(AuditDetail auditDetail,String usrLanguage,String method){
		...
		if(StringUtils.trimToEmpty(method).equals("doApprove") || !lovFieldCode.isWorkflow()){
