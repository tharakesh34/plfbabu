...
public class TakafulProviderServiceImpl extends GenericService<TakafulProvider> implements TakafulProviderService {
	...
	/**
	 * @return the takafulProvider
	 */
	@Override
	public TakafulProvider getTakafulProvider() {
		return getTakafulProviderDAO().getTakafulProvider();
	}
	/**
	 * @return the takafulProvider for New Record
	 */
	@Override
	public TakafulProvider getNewTakafulProvider() {
		return getTakafulProviderDAO().getNewTakafulProvider();
	}
	...
	private AuditHeader saveOrUpdate(AuditHeader auditHeader,boolean online) {
		...
		if (takafulProvider.isNew()) {
			...
			auditHeader.setAuditReference(String.valueOf(takafulProvider.getTakafulCode()));
		}else{
	 ...
	 */
	@Override
	public TakafulProvider refresh(TakafulProvider takafulProvider) {
		logger.debug("Entering");
		getTakafulProviderDAO().refresh(takafulProvider);
		getTakafulProviderDAO().initialize(takafulProvider);
		logger.debug("Leaving");
		return takafulProvider;
	}
	 ...
	 * 		a)  DELETE	Delete the record from the main table by using getTakafulProviderDAO().delete with parameters takafulProvider,""
	 * 		b)  NEW		Add new record in to main table by using getTakafulProviderDAO().save with parameters takafulProvider,""
	 * 		c)  EDIT	Update record in the main table by using getTakafulProviderDAO().update with parameters takafulProvider,""
	 * 3)	Delete the record from the workFlow table by using getTakafulProviderDAO().delete with parameters takafulProvider,"_Temp"
	 * 4)	Audit the record in to AuditHeader and AdtTakafulProvider by using auditHeaderDAO.addAudit(auditHeader) for Work flow
	 * 5)  	Audit the record in to AuditHeader and AdtTakafulProvider by using auditHeaderDAO.addAudit(auditHeader) based on the transaction Type.
	 * @param AuditHeader (auditHeader)    
	 * @return auditHeader
	 */
		...
		private AuditDetail validation(AuditDetail auditDetail,String usrLanguage,String method,boolean onlineRequest){
			...
			if (takafulProvider.isNew()){ // for New record or new record into work flow
				...
				if (!takafulProvider.isWorkflow()){	// With out Work flow for update and delete
					...
					if (oldTakafulProvider!=null && !oldTakafulProvider.getLastMntOn().equals(tempTakafulProvider.getLastMntOn())){ 
						auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(new ErrorDetails(PennantConstants.KEY_FIELD, "41005", errParm,valueParm), usrLanguage));
					}
				}
			}
			...
			if(StringUtils.trimToEmpty(method).equals("doApprove") || !takafulProvider.isWorkflow()){
