...
public class QueryServiceImpl extends GenericService<Query> implements QueryService {
	...
	public Query getQuery() {
		return getQueryDAO().getQuery();
	}
	/**
	 * @return the query for New Record
	 */
	@Override
	public Query getNewQuery() {
		return getQueryDAO().getNewQuery();
	}
	...
	@Override
	public Query getQueryById(Query query) {
		return getQueryDAO().getQueryById(query,"_View");
	}
	/**
	...
	public Query getApprovedQueryById(Query query) {
		return getQueryDAO().getQueryById(query,"_AView");
	}
	 ...
	 */
	@Override
	public Query refresh(Query query) {
		logger.debug("Entering");
		getQueryDAO().refresh(query);
		getQueryDAO().initialize(query);
		logger.debug("Leaving");
		return query;
	}
	 ...
	 * 		a)  DELETE	Delete the record from the main table by using getQueryDAO().delete with parameters query,""
	 * 		b)  NEW		Add new record in to main table by using getQueryDAO().save with parameters query,""
	 * 		c)  EDIT	Update record in the main table by using getQueryDAO().update with parameters query,""
	 * 3)	Delete the record from the workFlow table by using getQueryDAO().delete with parameters query,"_Temp"
	 * 4)	Audit the record in to AuditHeader and AdtQueries by using auditHeaderDAO.addAudit(auditHeader) for Work flow
	 * 5)  	Audit the record in to AuditHeader and AdtQueries by using auditHeaderDAO.addAudit(auditHeader) based on the transaction Type.
	 * @param AuditHeader (auditHeader)    
	 * @return auditHeader
	 */
	 ...
	 */
		...
		private AuditDetail validation(AuditDetail auditDetail,String usrLanguage,String method,boolean onlineRequest){
			...
			if (query.isWorkflow()){
				tempQuery = getQueryDAO().getQueryById(query, "_Temp");
			}
			...
			if(StringUtils.trimToEmpty(method).equals("doApprove") || !query.isWorkflow()){
