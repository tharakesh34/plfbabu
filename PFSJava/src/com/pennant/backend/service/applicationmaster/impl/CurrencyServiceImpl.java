...
public class CurrencyServiceImpl extends GenericService<Currency> implements CurrencyService {
	...
	/**
	 * @return the currency for New Record
	 */
	@Override
	public Currency getNewCurrency() {
		return getCurrencyDAO().getNewCurrency();
	}
	...
	public AuditHeader saveOrUpdate(AuditHeader auditHeader) {
		...
		if (currency.isNew()) {
			...
			auditHeader.setAuditReference(String.valueOf(currency.getCcyCode()));
		}else{
	...
	public Currency refresh(Currency currency) {
		logger.debug("Entering ");
		getCurrencyDAO().refresh(currency);
		getCurrencyDAO().initialize(currency);
		logger.debug("Leaving ");
		return currency;
	}
	 ...
	 * using getCurrencyDAO().delete with parameters currency,"" b) NEW Add new
	 * record in to main table by using getCurrencyDAO().save with parameters
	 * currency,"" c) EDIT Update record in the main table by using
	 * getCurrencyDAO().update with parameters currency,"" 3) Delete the record
	 * from the workFlow table by using getCurrencyDAO().delete with parameters
	 * currency,"_Temp" 4) Audit the record in to AuditHeader and
	 * AdtRMTCurrencies by using auditHeaderDAO.addAudit(auditHeader) for Work
	 * flow 5) Audit the record in to AuditHeader and AdtRMTCurrencies by using
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage,
		...
		if (StringUtils.trimToEmpty(method).equals("doApprove") || !currency.isWorkflow()) {
