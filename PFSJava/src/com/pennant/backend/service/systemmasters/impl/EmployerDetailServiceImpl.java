...
public class EmployerDetailServiceImpl extends GenericService<EmployerDetail> implements EmployerDetailService {
	...
	/**
	 * @return the employerDetail
	 */
	@Override
	public EmployerDetail getEmployerDetail() {
		return getEmployerDetailDAO().getEmployerDetail();
	}
	/**
	 * @return the employerDetail for New Record
	 */
	@Override
	public EmployerDetail getNewEmployerDetail() {
		return getEmployerDetailDAO().getNewEmployerDetail();
	}
	 ...
	 */
	@Override
	public EmployerDetail refresh(EmployerDetail employerDetail) {
		logger.debug("Entering");
		getEmployerDetailDAO().refresh(employerDetail);
		getEmployerDetailDAO().initialize(employerDetail);
		logger.debug("Leaving");
		return employerDetail;
	}
	 ...
	 * 		a)  DELETE	Delete the record from the main table by using getEmployerDetailDAO().delete with parameters employerDetail,""
	 * 		b)  NEW		Add new record in to main table by using getEmployerDetailDAO().save with parameters employerDetail,""
	 * 		c)  EDIT	Update record in the main table by using getEmployerDetailDAO().update with parameters employerDetail,""
	 * 3)	Delete the record from the workFlow table by using getEmployerDetailDAO().delete with parameters employerDetail,"_Temp"
	 * 4)	Audit the record in to AuditHeader and AdtEmployerDetail by using auditHeaderDAO.addAudit(auditHeader) for Work flow
	 * 5)  	Audit the record in to AuditHeader and AdtEmployerDetail by using auditHeaderDAO.addAudit(auditHeader) based on the transaction Type.
	 * @param AuditHeader (auditHeader)    
	 * @return auditHeader
	 */
	 ...
	 */
		...
		private AuditDetail validation(AuditDetail auditDetail,String usrLanguage,String method,boolean onlineRequest){
			...
			if(StringUtils.trimToEmpty(method).equals("doApprove") || !employerDetail.isWorkflow()){
