...
public class SegmentServiceImpl extends GenericService<Segment> implements SegmentService {
	...
	public Segment getSegment() {
		return getSegmentDAO().getSegment();
	}

	public Segment getNewSegment() {
		return getSegmentDAO().getNewSegment();
	}
	...
	public AuditHeader saveOrUpdate(AuditHeader auditHeader) {
		...
		if (segment.isNew()) {
			...
			auditHeader.setAuditReference(String.valueOf(segment.getSegmentCode()));
		} else {
	 ...
	 */
	@Override
	public Segment refresh(Segment segment) {
		logger.debug("Entering");
		getSegmentDAO().refresh(segment);
		getSegmentDAO().initialize(segment);
		logger.debug("Leaving");
		return segment;
	}
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage,
		...
		Segment old_Segment = segment.getBefImage();
		...
		if (segment.isNew()) { // for New record or new record into work flow
			...
			if (!segment.isWorkflow()) { // With out Work flow for update and delete
				...
				if (befSegment == null) { // if records not exists in the main table
					...
					if (old_Segment != null
							&& !old_Segment.getLastMntOn().equals(befSegment.getLastMntOn())) {
				...
				if (tempSegment != null
						&& old_Segment != null
						&& !old_Segment.getLastMntOn().equals(tempSegment.getLastMntOn())) {
					auditDetail
					.setErrorDetail(new ErrorDetails(PennantConstants.KEY_FIELD, "41005",errParm, null));
				}
			}
		...
		if (StringUtils.trimToEmpty(method).equals("doApprove")|| !segment.isWorkflow()) {
