...
public class QuestionServiceImpl extends GenericService<Question> implements QuestionService {
	 ...
	 */
	@Override
	public Question refresh(Question question) {
		logger.debug("Entering");
		getQuestionDAO().refresh(question);
		getQuestionDAO().initialize(question);
		logger.debug("Leaving");
		return question;
	}
	 ...
	 * 		a)  DELETE	Delete the record from the main table by using getQuestionDAO().delete with parameters question,""
	 * 		b)  NEW		Add new record in to main table by using getQuestionDAO().save with parameters question,""
	 * 		c)  EDIT	Update record in the main table by using getQuestionDAO().update with parameters question,""
	 * 3)	Delete the record from the workFlow table by using getQuestionDAO().delete with parameters question,"_Temp"
	 * 4)	Audit the record in to AuditHeader and AdtBMTQuestion by using auditHeaderDAO.addAudit(auditHeader) for Work flow
	 * 5)  	Audit the record in to AuditHeader and AdtBMTQuestion by using auditHeaderDAO.addAudit(auditHeader) based on the transaction Type.
	 * @param AuditHeader (auditHeader)    
	 * @return auditHeader
	 */
	 ...
	 */
		...
		private AuditDetail validation(AuditDetail auditDetail,String usrLanguage,String method){
			...
			if(StringUtils.trimToEmpty(method).equals("doApprove") || !question.isWorkflow()){
