...
import com.pennant.app.util.ApplicationUtil;
...
public class ReportConfigurationServiceImpl extends GenericService<ReportConfiguration> implements ReportConfigurationService{
	 ...
	 */
	@Override
	public ReportConfiguration refresh(ReportConfiguration reportConfiguration) {
		logger.debug("Entering ");
		getReportConfigurationDAO().refresh(reportConfiguration);
		getReportConfigurationDAO().initialize(reportConfiguration);
		logger.debug("Leaving ");
		return reportConfiguration;
	}
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage,
		...
		if (reportConfiguration.isNew()) { // for New record or new record into work flow
			...
			if (!reportConfiguration.isWorkflow()) {// With out Work flow only new records
				...
				if (reportConfiguration.getRecordType().equals(
						PennantConstants.RECORD_TYPE_NEW)) { // if records type is new
					if (befReportConfiguration != null || tempReportConfiguration != null) { // if records already exists in the main table
						auditDetail.setErrorDetail(new ErrorDetails(PennantConstants.KEY_FIELD,"41014",errParm,null));
					}
				}
		...
		if (StringUtils.trimToEmpty(method).equals("doApprove") || !reportConfiguration.isWorkflow()) {
	...
	private List<AuditDetail> setReportFilterFieldsAuditData(ReportConfiguration reportConfiguration,String auditTranType,String method) {
		...
		if ((PennantConstants.RECORD_TYPE_DEL.equals(
				reportConfiguration.getRecordType()) && method.equalsIgnoreCase("doApprove")) || method.equals("delete")) {
			delete=true;
		}
		...
		for (int i = 0; i < reportConfiguration.getListReportFieldsDetails().size(); i++) {
			...
			if(method.equals("saveOrUpdate") && (isRcdType && reportFilterFields.isWorkflow())){
			...
			if(!reportFilterFields.getRecordType().equals("")){
	...
	public Map<Object ,List<ReportSearchTemplate>> getTemplatesByReportID(long reportId,long usrID){
		...
		Map<Object ,List<ReportSearchTemplate>> resultMap = ApplicationUtil
	...
	public void deleteSearchTemplate(long reportId,long usrId,String templateName) {
		getReportSearchTemplateDAO().delete(reportId, usrId, templateName);
	}
