...
public class ReportListServiceImpl extends GenericService<ReportList> implements ReportListService {
	 ...
	 */
	@Override
	public ReportList refresh(ReportList reportList) {
		logger.debug("Entering");
		getReportListDAO().refresh(reportList);
		getReportListDAO().initialize(reportList);
		logger.debug("Leaving");
		return reportList;
	}
	 ...
	 * 		a)  DELETE	Delete the record from the main table by using getReportListDAO().delete with parameters reportList,""
	 * 		b)  NEW		Add new record in to main table by using getReportListDAO().save with parameters reportList,""
	 * 		c)  EDIT	Update record in the main table by using getReportListDAO().update with parameters reportList,""
	 * 3)	Delete the record from the workFlow table by using getReportListDAO().delete with parameters reportList,"_Temp"
	 * 4)	Audit the record in to AuditHeader and AdtReportList by using auditHeaderDAO.addAudit(auditHeader) for Work flow
	 * 5)  	Audit the record in to AuditHeader and AdtReportList by using auditHeaderDAO.addAudit(auditHeader) based on the transaction Type.
	 * @param AuditHeader (auditHeader)    
	 * @return auditHeader
	 */
	...
	private AuditDetail validation(AuditDetail auditDetail,String usrLanguage,String method){
		...
		if(StringUtils.trimToEmpty(method).equals("doApprove") || !reportList.isWorkflow()){
