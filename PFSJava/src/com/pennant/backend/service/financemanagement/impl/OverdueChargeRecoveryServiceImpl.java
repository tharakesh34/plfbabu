...
public class OverdueChargeRecoveryServiceImpl extends GenericService<OverdueChargeRecovery> implements OverdueChargeRecoveryService {
	...
	public OverdueChargeRecovery getOverdueChargeRecoveryById(String id, Date finSchDate, String finOdFor) {
		...
		ocr = getFinanceScheduleDetailDAO().getODCRecoveryDetails(ocr);
		return ocr;
	}
	/**
    ...
    public List<String> getOverDueFinanceList() {
	    return getOverdueChargeRecoveryDAO().getOverDueFinanceList();
    }
	 ...
	 */
	@Override
	public OverdueChargeRecovery refresh(OverdueChargeRecovery overdueChargeRecovery) {
		logger.debug("Entering");
		getOverdueChargeRecoveryDAO().refresh(overdueChargeRecovery);
		getOverdueChargeRecoveryDAO().initialize(overdueChargeRecovery);
		logger.debug("Leaving");
		return overdueChargeRecovery;
	}
	 ...
	 * 		a)  DELETE	Delete the record from the main table by using getOverdueChargeRecoveryDAO().delete with parameters overdueChargeRecovery,""
	 * 		b)  NEW		Add new record in to main table by using getOverdueChargeRecoveryDAO().save with parameters overdueChargeRecovery,""
	 * 		c)  EDIT	Update record in the main table by using getOverdueChargeRecoveryDAO().update with parameters overdueChargeRecovery,""
	 * 3)	Delete the record from the workFlow table by using getOverdueChargeRecoveryDAO().delete with parameters overdueChargeRecovery,"_Temp"
	 * 4)	Audit the record in to AuditHeader and AdtFinODCRecovery by using auditHeaderDAO.addAudit(auditHeader) for Work flow
	 * 5)  	Audit the record in to AuditHeader and AdtFinODCRecovery by using auditHeaderDAO.addAudit(auditHeader) based on the transaction Type.
	 * @param AuditHeader (auditHeader)    
	 * @return auditHeader
	 */
	...
	private AuditDetail validation(AuditDetail auditDetail,String usrLanguage,String method){
		...
		if(StringUtils.trimToEmpty(method).equals("doApprove") || !overdueChargeRecovery.isWorkflow()){
