...
public class FacilityReferenceDetailServiceImpl extends GenericService<FacilityReferenceDetail> implements FacilityReferenceDetailService {
	 ...
	 */
	@Override
	public FacilityReferenceDetail refresh(FacilityReferenceDetail facilityReferenceDetail) {
		logger.debug("Entering");
		getFacilityReferenceDetailDAO().refresh(facilityReferenceDetail);
		getFacilityReferenceDetailDAO().initialize(facilityReferenceDetail);
		logger.debug("Leaving");
		return facilityReferenceDetail;
	}
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage, String method) {
		...
		if (facilityReferenceDetail.isNew()) { // for New record or new record
			...
			if (!facilityReferenceDetail.isWorkflow()) {// With out Work flow
				...
				if (facilityReferenceDetail.getRecordType().equals(PennantConstants.RECORD_TYPE_NEW)) { // if
					// records
					// type
					// is
					// new
					if (befFacilityReferenceDetail != null || tempFacilityReferenceDetail != null) { // if
						// records
						// already
						// exists
						// in
		...
		if (StringUtils.trimToEmpty(method).equals("doApprove") || !facilityReferenceDetail.isWorkflow()) {
