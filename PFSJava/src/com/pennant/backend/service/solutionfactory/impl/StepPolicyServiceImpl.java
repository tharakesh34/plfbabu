...
public class StepPolicyServiceImpl extends GenericService<StepPolicyHeader> implements StepPolicyService {
	...
	public StepPolicyHeader getStepPolicyHeader() {
		return getStepPolicyHeaderDAO().getStepPolicyHeader();
	}
	 ...
	 */
	@Override
	public StepPolicyHeader refresh(StepPolicyHeader stepPolicyHeader) {
		logger.debug("Entering");
		getStepPolicyHeaderDAO().refresh(stepPolicyHeader);
		getStepPolicyHeaderDAO().initialize(stepPolicyHeader);
		logger.debug("Leaving");
		return stepPolicyHeader;
	}
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage, String method) {
		...
		if (StringUtils.trimToEmpty(method).equals("doApprove")	|| !stepPolicyHeader.isWorkflow()) {
	...
	//=================================== List maintain
		...
		public List<AuditDetail> deleteChilds(StepPolicyHeader stepPolicyHeader, String tableType,String auditTranType) {
			...
			if (stepPolicyHeader.getStepPolicyDetails() != null && stepPolicyHeader.getStepPolicyDetails().size() > 0) {
				...
				for (int i = 0; i < stepPolicyHeader.getStepPolicyDetails().size(); i++) {
					...
					if (!stepPolicyDetail.getRecordType().equals("") || tableType.equals("")) {
		...
		private ArrayList<ErrorDetails> validateChilds(AuditHeader auditHeader,String usrLanguage,String method){
			...
			if (stepPolicyHeader.getAuditDetailMap().get("StepPolicyDetail")!=null) {
				...
				for (AuditDetail auditDetail : auditDetails) {
					ArrayList<ErrorDetails> details=validationStepPolicyDetail(auditDetail, usrLanguage, method).getErrorDetails();
					if (details!=null) {
		...
		private List<AuditDetail> setStepPolicyDetailAuditData(StepPolicyHeader stepPolicyHeader, String auditTranType, String method) {
			...
			for (int i = 0; i < stepPolicyHeader.getStepPolicyDetails().size(); i++) {
				...
				if (!StringUtils.trimToEmpty(stepPolicyDetail.getRecordType()).equals("")) {
	...
	private AuditDetail validationStepPolicyDetail(AuditDetail auditDetail,String usrLanguage,String method){
		...
		if(StringUtils.trimToEmpty(method).equals("doApprove") || !stepPolicyDetail.isWorkflow()){
