...
public class DedupFieldsServiceImpl implements DedupFieldsService {
	 ...
	 */
	@Override
	public DedupFields refresh(DedupFields dedupFields) {
		logger.debug("Entering");
		getDedupFieldsDAO().refresh(dedupFields);
		getDedupFieldsDAO().initialize(dedupFields);
		logger.debug("Leaving");
		return dedupFields;
	}
			...
			if (!dedupFields.isWorkflow()) {// With out Work flow only new records
				if (befDedupFields != null) { // Record Already Exists in the
					...
					auditDetail.setErrorDetail(getDedupFieldsDAO()
							.getErrorDetail("41001", usrLanguage, parm));
				}
			} else { // with work flow
				...
				if (befDedupFields == null) { // if records not exists in the main table
					...
					if (old_DedupFields != null
						...
						if (StringUtils.trimToEmpty(
							...
							auditDetail.setErrorDetail(getDedupFieldsDAO()
								.getErrorDetail("41003",usrLanguage, parm));
						} else {
							auditDetail.setErrorDetail(getDedupFieldsDAO()
							.getErrorDetail("41004",usrLanguage, parm));
						}
					}
				}
			} else {
	...
	public ErrorDetails getErrorDetails(String errorCode,String language,String[] parm) {
		return getDedupFieldsDAO().getErrorDetail(errorCode, language,parm);
	}
