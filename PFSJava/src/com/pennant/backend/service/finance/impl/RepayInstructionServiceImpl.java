...
public class RepayInstructionServiceImpl extends GenericService<RepayInstruction> implements RepayInstructionService {
	private final static Logger logger = Logger.getLogger(RepayInstructionServiceImpl.class);
	...
	/**
	 * @return the repayInstruction
	 */
	@Override
	public RepayInstruction getRepayInstruction(boolean isWIF) {
		return getRepayInstructionDAO().getRepayInstruction(isWIF);
	}
	/**
	 * @return the repayInstruction for New Record
	 */
	@Override
	public RepayInstruction getNewRepayInstruction(boolean isWIF) {
		return getRepayInstructionDAO().getNewRepayInstruction(isWIF);
	}
	 ...
	 * @param RepayInstruction (repayInstruction)
 	 * @return repayInstruction
	 */
	@Override
	public RepayInstruction refresh(RepayInstruction repayInstruction) {
		logger.debug("Entering");
		getRepayInstructionDAO().refresh(repayInstruction);
		getRepayInstructionDAO().initialize(repayInstruction);
		logger.debug("Leaving");
		return repayInstruction;
	}
	 ...
	 * 		a)  DELETE	Delete the record from the main table by using getRepayInstructionDAO().delete with parameters repayInstruction,""
	 * 		b)  NEW		Add new record in to main table by using getRepayInstructionDAO().save with parameters repayInstruction,""
	 * 		c)  EDIT	Update record in the main table by using getRepayInstructionDAO().update with parameters repayInstruction,""
	 * 3)	Delete the record from the workFlow table by using getRepayInstructionDAO().delete with parameters repayInstruction,"_Temp"
	 * 4)	Audit the record in to AuditHeader and AdtFinRepayInstruction by using auditHeaderDAO.addAudit(auditHeader) for Work flow
	 * 5)  	Audit the record in to AuditHeader and AdtFinRepayInstruction by using auditHeaderDAO.addAudit(auditHeader) based on the transaction Type.
	 * @param AuditHeader (auditHeader)    
	 * @return auditHeader
	 */
	 ...
	 */
		...
		private AuditDetail validation(AuditDetail auditDetail,String usrLanguage,String method,boolean isWIF){
			...
			if(StringUtils.trimToEmpty(method).equals("doApprove") || !repayInstruction.isWorkflow()){
