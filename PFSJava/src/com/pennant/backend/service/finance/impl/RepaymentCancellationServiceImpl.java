...
public class RepaymentCancellationServiceImpl extends GenericService<FinanceMain> implements RepaymentCancellationService {
	...
	public AuditHeader saveOrUpdate(AuditHeader aAuditHeader) throws AccountNotFoundException, IllegalAccessException, InvocationTargetException {
		...
		financeMain.setRcdMaintainSts(PennantConstants.CANCELREPAY);
		if(tableType.equals("")){
		...
		if(!financeMain.isWorkflow()){
			...
			if(!StringUtils.trimToEmpty(errorCode).equals("")){
				throw new AccountNotFoundException("XXXX : "+errorCode);
			}
		}else{
	 ...
	 * @throws InvocationTargetException
	 */
	private String processRepayCancellation(FinanceMain financeMain) throws AccountNotFoundException, IllegalAccessException, InvocationTargetException {
		logger.debug("Entering");
		...
		if(repayList != null && !repayList.isEmpty()){
			...
			FinRepayHeader repayHeader = getFinanceRepaymentsDAO().getFinRepayHeader(finReference, repayment.getLinkedTranId(),  "");
			String finEventCode = "";
			if(repayHeader == null){
				finEventCode = PennantConstants.SCH_REPAY;
			}else{
			...
			if(!financeMain.isFinIsActive()){
				...
				/*if(repayHeader == null){
					return "Fiannce Cannot be Processed for Reversal of Payment. Closing Status Updated on CRB file.";
				}else if(curAppDate.compareTo(repayHeader.getValueDate()) != 0){
					return "Fiannce Cannot be Processed for Reversal of Payment. Closing Status Updated on CRB file.";
				}*/
			}
			...
			if(detail == null){
				...
				detail.setEventAction(PennantConstants.SCH_REPAY);
			...
			if(!isMigratedRepayment){
				List<Object> returnList = getPostingsPreparationUtil().processFinCanclPostings(finReference, String.valueOf(repayment.getLinkedTranId()));
			...
			//TODO
			/*BigDecimal totalPenaltyPaid = getPostingsDAO().getPostAmtByTranIdandEvent(finReference, "LATEPAY" , repayment.getLinkedTranId());
			if(totalPenaltyPaid.compareTo(BigDecimal.ZERO) > 0){
			...
			if(totalPriAmount.compareTo(BigDecimal.ZERO) > 0){
				...
				getFinanceMainDAO().updateRepaymentAmount(finReference, financeMain.getFinAmount().add(
						...
						financeMain.getFinRepaymentAmount(), curFinStatus, PennantConstants.FINSTSRSN_MANUAL,true, false);
				...
				if(!StringUtils.trimToEmpty(financeMain.getFinCommitmentRef()).equals("")){
					...
					if(commitment != null && commitment.isRevolving()){
						...
						BigDecimal cmtUtlAmt = CalculationUtil.getConvertedAmount(financeMain.getFinCcy(), commitment.getCmtCcy(),
								totalPriAmount);
						getCommitmentDAO().updateCommitmentAmounts(commitment.getCmtReference(), cmtUtlAmt, commitment.getCmtExpDate());
						CommitmentMovement cmtMovement = prepareCommitMovement(commitment, financeMain, cmtUtlAmt, linkedTranId);
						if(cmtMovement != null){
			...
			//============================================
			List<FinanceScheduleDetail> finSchedeuleDetails = getFinanceScheduleDetailDAO().getFinSchdDetailsForBatch(finReference);
			FinanceProfitDetail profitDetail = getFinanceProfitDetailDAO().getFinPftDetailForBatch(finReference);

			AEAmountCodes amountCodes = AEAmounts.procAEAmounts(financeMain, finSchedeuleDetails, profitDetail,curAppDate);
			profitDetail = getFinanceProfitDetailFiller().prepareFinPftDetails(amountCodes, profitDetail, curAppDate);
	...
	private FinanceScheduleDetail updateScheduleDetailsData(FinanceScheduleDetail schedule, FinanceRepayments repayment) {
		...
		// Finance Repayments for Deferment & Schedule 
		if (repayment.getFinRpyFor().equals(PennantConstants.DEFERED)) {

			schedule.setDefSchdPftPaid(schedule.getDefSchdPftPaid().subtract(repayment.getFinSchdPftPaid()));
			schedule.setDefSchdPriPaid(schedule.getDefSchdPriPaid().subtract(repayment.getFinSchdPriPaid()));

			// Finance Defered Schedule Profit Balance Check
			...
			if ((schedule.getDefProfitSchd().subtract(schedule.getDefSchdPftPaid())).compareTo(BigDecimal.ZERO) == 0) {
				...
				// Finance Defered Schedule Principal Balance Check
		...
		} else if (repayment.getFinRpyFor().equals(PennantConstants.SCHEDULE)) {
	...
	private FinScheduleData getFinSchDataByFinRef(String finReference, long logKey, String type) {
		...
		finSchData.setDefermentHeaders(getDefermentHeaderDAO().getDefermentHeaders(finReference,type, false, logKey));
		finSchData.setDefermentDetails(getDefermentDetailDAO().getDefermentDetails(finReference,type, false, logKey));
		logger.debug("Leaving");
		return finSchData;
	}
	
	/**
	 * Method to delete schedule, disbursement, deferementheader, defermentdetail,repayinstruction lists.
	 * 
	 * @param finDetail
	 * @param tableType
	 * @param isWIF
	...
	private void listDeletion(String finReference, String tableType, boolean isWIF, long logKey) {
		...
		getDefermentHeaderDAO().deleteByFinReference(finReference, tableType, isWIF, logKey);
		getDefermentDetailDAO().deleteByFinReference(finReference, tableType, isWIF, logKey);
	...
	private void listSave(FinScheduleData finDetail, String tableType, long logKey) {
		...
		for (int i = 0; i < finDetail.getFinanceScheduleDetails().size(); i++) {
			finDetail.getFinanceScheduleDetails().get(i)
			        .setLastMntBy(finDetail.getFinanceMain().getLastMntBy());
		...
		getFinanceScheduleDetailDAO().saveList(finDetail.getFinanceScheduleDetails(), tableType,
		        false);
		...
		for (int i = 0; i < finDetail.getDefermentHeaders().size(); i++) {
			finDetail.getDefermentHeaders().get(i).setFinReference(finDetail.getFinanceMain().getFinReference());
		...
		for (int i = 0; i < finDetail.getDefermentDetails().size(); i++) {
			finDetail.getDefermentDetails().get(i).setFinReference(finDetail.getFinanceMain().getFinReference());
