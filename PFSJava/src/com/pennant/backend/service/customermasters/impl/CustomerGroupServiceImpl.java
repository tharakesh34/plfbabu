...
public class CustomerGroupServiceImpl extends GenericService<CustomerGroup> implements CustomerGroupService {
	...
	public CustomerGroup getCustomerGroup() {
		return getCustomerGroupDAO().getCustomerGroup();
	}
	@Override
	public CustomerGroup getNewCustomerGroup() {
		return getCustomerGroupDAO().getNewCustomerGroup();
	}
	 ...
	 */
	@Override
	public CustomerGroup refresh(CustomerGroup customerGroup) {
		logger.debug("Entering");
		getCustomerGroupDAO().refresh(customerGroup);
		getCustomerGroupDAO().initialize(customerGroup);
		logger.debug("Leaving");
		return customerGroup;
	}
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage, String method) {
		...
		if (customerGroup.isWorkflow()) {
			tempCustomerGroup = getCustomerGroupDAO().getCustomerGroupByID(
					customerGroup.getId(), "_Temp");
		}
		...
		valueParm[0] = String.valueOf(customerGroup.getCustGrpID());
		...
		if (StringUtils.trimToEmpty(method).equals("doApprove")|| !customerGroup.isWorkflow()) {
