...
public class FinCreditRevSubCategoryServiceImpl extends GenericService<FinCreditRevSubCategory> implements FinCreditRevSubCategoryService {
	 ...
	 */
	@Override
	public FinCreditRevSubCategory refresh(FinCreditRevSubCategory finCreditRevSubCategory) {
		logger.debug("Entering");
		getFinCreditRevSubCategoryDAO().refresh(finCreditRevSubCategory);
		getFinCreditRevSubCategoryDAO().initialize(finCreditRevSubCategory);
		logger.debug("Leaving");
		return finCreditRevSubCategory;
	}
	 ...
	 * 		a)  DELETE	Delete the record from the main table by using getFinCreditRevSubCategoryDAO().delete with parameters finCreditRevSubCategory,""
	 * 		b)  NEW		Add new record in to main table by using getFinCreditRevSubCategoryDAO().save with parameters finCreditRevSubCategory,""
	 * 		c)  EDIT	Update record in the main table by using getFinCreditRevSubCategoryDAO().update with parameters finCreditRevSubCategory,""
	 * 3)	Delete the record from the workFlow table by using getFinCreditRevSubCategoryDAO().delete with parameters finCreditRevSubCategory,"_Temp"
	 * 4)	Audit the record in to AuditHeader and AdtFinCreditRevSubCategory by using auditHeaderDAO.addAudit(auditHeader) for Work flow
	 * 5)  	Audit the record in to AuditHeader and AdtFinCreditRevSubCategory by using auditHeaderDAO.addAudit(auditHeader) based on the transaction Type.
	 * @param AuditHeader (auditHeader)    
	 * @return auditHeader
	 */
	...
	public AuditHeader doApprove(AuditHeader auditHeader) {
		...
		BeanUtils.copyProperties((FinCreditRevSubCategory) auditHeader.getAuditDetail().getModelData(), finCreditRevSubCategory);
	 ...
	 */
		...
		private AuditDetail validation(AuditDetail auditDetail,String usrLanguage,String method){
			...
			if(StringUtils.trimToEmpty(method).equals("doApprove") || !finCreditRevSubCategory.isWorkflow()){
