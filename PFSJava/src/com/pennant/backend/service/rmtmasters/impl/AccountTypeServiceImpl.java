...
public class AccountTypeServiceImpl extends GenericService<AccountType> implements AccountTypeService {
	...
	public AccountType getAccountType() {
		return getAccountTypeDAO().getAccountType();
	}
	@Override
	public AccountType getNewAccountType() {
		return getAccountTypeDAO().getNewAccountType();
	}
	...
	public AuditHeader saveOrUpdate(AuditHeader auditHeader) {
		...
		if (accountType.isNew()) {
			...
			auditHeader.setAuditReference(String.valueOf(accountType.getAcType()));
		}else{
	 ...
	 */
	@Override
	public AccountType refresh(AccountType accountType) {
		logger.debug("Entering");
		getAccountTypeDAO().refresh(accountType);
		getAccountTypeDAO().initialize(accountType);
		logger.debug("Leaving");
		return accountType;
	}
	...
	private AuditDetail validation(AuditDetail auditDetail,String usrLanguage,String method) {
		...
		if (accountType.isNew()) { // for New record or new record into workFlow
			...
			if (!accountType.isWorkflow()) {// With out Work flow only new records
				...
				if (accountType.getRecordType().equals(
						PennantConstants.RECORD_TYPE_NEW)) { // if records type is new
					if (befAccountType != null || tempAccountType != null) { // if records already exists in the main table
						auditDetail.setErrorDetail(new ErrorDetails(PennantConstants.KEY_FIELD,"41001",errParm,null));
					}
				}
		...
		if (StringUtils.trimToEmpty(method).equals("doApprove") || !accountType.isWorkflow()) {
