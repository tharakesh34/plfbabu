...
public class DSRCalculationReportData implements Serializable{
	...
	public DSRCalculationReportData getDSRCalculationReportData(FinanceDetail detail){
		...
		reportData.setMaturityDate(PennantApplicationUtil.formateDate(financeMain.getMaturityDate(), PennantConstants.dateFormate));
		reportData.setFinAmount(PennantApplicationUtil.amountFormate(financeMain.getFinAmount(),financeMain.getLovDescFinFormatter()));
		reportData.setDownPay(PennantApplicationUtil.amountFormate(financeMain.getDownPayBank().add(financeMain.getDownPaySupl()), financeMain.getLovDescFinFormatter()));
		reportData.setDownPayBank(PennantApplicationUtil.amountFormate(financeMain.getDownPayBank(), financeMain.getLovDescFinFormatter()));
		reportData.setDownPaySupl(PennantApplicationUtil.amountFormate(financeMain.getDownPaySupl(), financeMain.getLovDescFinFormatter()));
		reportData.setFirstRepay(PennantApplicationUtil.amountFormate(financeMain.getFirstRepay(), financeMain.getLovDescFinFormatter()));
		reportData.setLastRepay(PennantApplicationUtil.amountFormate(financeMain.getLastRepay(), financeMain.getLovDescFinFormatter()));
		reportData.setEffectiveRate(PennantApplicationUtil.formatRate(financeMain.getEffectiveRateOfReturn().doubleValue(), 2)+" %");
		...
		reportData.setTotalFinAmount(PennantApplicationUtil.amountFormate(financeMain.getFinAmount().add(
				financeMain.getFeeChargeAmt() == null ? BigDecimal.ZERO : financeMain.getFeeChargeAmt()).
				subtract(financeMain.getDownPayment()),financeMain.getLovDescFinFormatter()));
		reportData.setTotalGrcPft(PennantApplicationUtil.amountFormate(financeMain.getTotalGracePft(),financeMain.getLovDescFinFormatter()));
		reportData.setTotalRepayPft(PennantApplicationUtil.amountFormate(financeMain.getTotalProfit().subtract(financeMain.getTotalGracePft()),financeMain.getLovDescFinFormatter()));
		reportData.setTotalProfit(PennantApplicationUtil.amountFormate(financeMain.getTotalProfit(),financeMain.getLovDescFinFormatter()));
		reportData.setTotRepayAmount(PennantApplicationUtil.amountFormate(financeMain.getTotalProfit().add(financeMain.getFinAmount().add(
				financeMain.getFeeChargeAmt() == null ?  BigDecimal.ZERO : financeMain.getFeeChargeAmt()).
				subtract(financeMain.getDownPayment())),financeMain.getLovDescFinFormatter()));
		...
		if(financeMain.getRepayRateBasis().equals(CalculationConstants.RATE_BASIS_R)){
			...
			BigDecimal totalFinAmt = financeMain.getFinAmount().add(financeMain.getFeeChargeAmt() == null ? BigDecimal.ZERO : 
				financeMain.getFeeChargeAmt()).subtract(financeMain.getDownPayment());
			BigDecimal totalPft = financeMain.getTotalProfit(); 
			BigDecimal totalDays = new BigDecimal(DateUtility.getDaysBetween(financeMain.getFinStartDate(), financeMain.getMaturityDate()));
			
			BigDecimal flatRate = ((totalPft.multiply(new BigDecimal(360))).divide(totalFinAmt.multiply(totalDays), 9, RoundingMode.HALF_DOWN)).multiply(new BigDecimal(100));
			reportData.setRepayPftRate(PennantApplicationUtil.formatRate(flatRate.doubleValue(), 2)+" %");
		}else{
			reportData.setRepayPftRate(PennantApplicationUtil.formatRate(financeMain.getRepayProfitRate().doubleValue(), 2)+" %");
		}
		
		reportData.setRepayFrq(FrequencyUtil.getFrequencyDetail(financeMain.getRepayFrq()).getFrequencyDescription());
		reportData.setNextrepayDate(PennantApplicationUtil.formateDate(financeMain.getNextRepayDate(), PennantConstants.dateFormate));
		...
		if(feeRules != null && !feeRules.isEmpty()){
			for (FeeRule feeRule : feeRules) {
	            ...
	            fee.setFeeAmount(PennantApplicationUtil.amountFormate(feeRule.getFeeAmount().subtract(
	            		feeRule.getWaiverAmount()).subtract(feeRule.getPaidAmount()), financeMain.getLovDescFinFormatter()));
		...
		if(incomes != null && !incomes.isEmpty()){
			for (CustomerIncome income : incomes) {
				...
				Income inc = new Income(financeMain.getLovDescFinFormatter());
				if(incMap.containsKey("I_"+income.getCategory()+"_"+income.getCustIncomeType())){
					...
					if(!income.isJointCust()){
						inc.setIncAmount(PennantApplicationUtil.amountFormate(income.getCustIncome(), financeMain.getLovDescFinFormatter()));
					}else{
						inc.setJointIncAmount(PennantApplicationUtil.amountFormate(income.getCustIncome(), financeMain.getLovDescFinFormatter()));
					}
		...
		reportData.setNetIncome(PennantApplicationUtil.amountFormate(totalInc, financeMain.getLovDescFinFormatter()));
		...
		if(detail.getElgRuleList() != null && !detail.getElgRuleList().isEmpty()){
			for (FinanceEligibilityDetail eligibility : detail.getElgRuleList()) {
	            ...
	            if("D".equals(eligibility.getRuleResultType())){
	            	
					if(eligibility.getRuleResult().equals("E")){
						 elg.setEligibilityLimit(Labels.getLabel("common.InSuffData"));
					}else if("DSRCAL".equals(eligibility.getLovDescElgRuleCode())){
						elg.setEligibilityLimit(eligibility.getRuleResult()+"%");
					}else{
						...
						elg.setOverrideEL(PennantApplicationUtil.amountFormate(eligibility.getOverrideResult(),
								financeMain.getLovDescFinFormatter()));
					}
				}else{
		...
		if(detail.getFinScoreHeaderList() != null && !detail.getFinScoreHeaderList().isEmpty()){
			for (FinanceScoreHeader header : detail.getFinScoreHeaderList()) {
				...
				if(detail.getScoreDetailListMap() != null && detail.getScoreDetailListMap().containsKey(header.getHeaderId())){
	...
	public ArrayList<Income> sortIncomeDetails(
			ArrayList<Income> incomeDetails) {
