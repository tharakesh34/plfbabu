...
public class FinMainReportData implements Serializable{
	...
	public FinMainReportData getFinMainReportData(FinScheduleData finSchData, FinanceSummary finSummary){
		...
		if (financeMain.isFinIsActive()) {
			...
			if (StringUtils.trimToEmpty(financeMain.getClosingStatus()).equals("W")) {
				reportData.setFinStatus("Written-Off");
			} else if (StringUtils.trimToEmpty(financeMain.getClosingStatus()).equals("P")) {
				reportData.setFinStatus("Pay-Off");
			} else if (StringUtils.trimToEmpty(financeMain.getClosingStatus()).equals("M")) {
				reportData.setFinStatus("Matured");
			} else {
				reportData.setFinStatus("In-Active");
			}
		}
		...
		if(finSchData.isFinPftSuspended()){
			reportData.setFinSuspense("True("+
					PennantApplicationUtil.formateDate(finSchData.getFinSuspDate(), PennantConstants.dateFormate)+")");
		} else {
		...
		reportData.setNextRepayRvwDate(PennantApplicationUtil.formateDate(financeMain.getNextRepayRvwDate(), PennantConstants.dateFormate));
		reportData.setRepayCpzFrq(FrequencyUtil.getFrequencyDetail(financeMain.getRepayCpzFrq()).getFrequencyDescription());
		reportData.setNextRepayCpzDate(PennantApplicationUtil.formateDate(financeMain.getNextRepayCpzDate(), PennantConstants.dateFormate));
		...
		reportData.setUnPaidInstlementPft(PennantApplicationUtil.amountFormate(finSummary.getTotalUnPaid(), ccyFormatter));
