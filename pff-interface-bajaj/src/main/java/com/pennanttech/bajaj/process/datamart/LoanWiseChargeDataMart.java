...
public class LoanWiseChargeDataMart extends DatabaseDataEngine implements Runnable {
	private static final Logger logger = Logger.getLogger(LoanWiseChargeDataMart.class);

	public LoanWiseChargeDataMart(DataSource dataSource, long userId, Date valueDate, Date appDate, boolean logBatch) {
		...
		jdbcTemplate.query(sql.toString(), parmMap, new ResultSetExtractor<Integer>() {
			...
			public Integer extractData(ResultSet rs) throws SQLException, DataAccessException {
				...
				while (rs.next()) {
					...
					try {
						map = mapData(rs);
						saveOrUpdate(map, "DM_LOANWISE_CHARGE_DETAILS", destinationJdbcTemplate, filterFields);
					} catch (Exception e) {
