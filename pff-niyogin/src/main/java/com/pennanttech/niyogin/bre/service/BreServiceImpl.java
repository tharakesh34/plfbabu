...
public class BreServiceImpl extends NiyoginService implements BreService {
	...
	private Bureau prepareBureau(FinanceDetail financeDetail) {
		...
		if (extendedMap != null) {
			...
			bureau.setNoPrvsLoansAsOfAppDate(
					getBooleanValue(ExtFieldMapConstants.NO_PREVS_LOANS_AS_OF_APPLICATION_DATE) ? "1" : "0");
			bureau.setIsApplicant90PlusDpdinL6M(
					getBooleanValue(ExtFieldMapConstants.IS_APPLICANT_90_PLUS_DPD_IN_L6M) ? "1" : "0");
			bureau.setIsApplicantSubStandardinL6M(
					getBooleanValue(ExtFieldMapConstants.IS_APPLICANT_SUBSTANDARD_IN_L6M) ? "1" : "0");
			bureau.setIsApplicantReportedAsLossinL6M(
					getBooleanValue(ExtFieldMapConstants.IS_APPLICANT_REPORTED_AS_LOSS_IN_L6M) ? "1" : "0");
			bureau.setIsApplicantDoubtfulinL6M(
					getBooleanValue(ExtFieldMapConstants.IS_APPLICANT_DOUBTFUL_IN_L6M) ? "1" : "0");
			bureau.setIsApplicantMentionedAsSMA(
					getBooleanValue(ExtFieldMapConstants.IS_APPLICANT_MENTIONED_AS_SMA) ? "1" : "0");
			//TODO:
			String lstUpdDate = NiyoginUtility.formatDate(getDateValue(ExtFieldMapConstants.LAST_UPDATE_DT_IN_BUREAU),
					"yyyy-MM-dd");
	...
	private Financials prepareFinancials(FinanceDetail financeDetail) {
		...
		financials.setPartnersOrDirectorsRemuneration(
				getBigDecimalValue(ExtFieldMapConstants.PARTNERS_DIRECTORS_REMUN_YR1));
		...
		//TODO:
		String oldDisDate = NiyoginUtility.formatDate(getDateValue(ExtFieldMapConstants.OLDEST_LOANDISBURSED_DT),
				"yyyy-MM-dd");
	...
	private CodeMogs prepareCodeMogs(CustomerDetails customerDetails) {
		...
		List<CustomerAddres> addressList = customerDetails.getAddressList();
		if (addressList != null && !addressList.isEmpty()) {
			CustomerAddres curAddress = NiyoginUtility.getCustomerAddress(addressList,
					InterfaceConstants.ADDR_TYPE_CURRES);
			codeMogs.setCurrentResidencePincode(StringUtils.trimToNull(curAddress.getCustAddrZIP()));
			CustomerAddres perAddress = NiyoginUtility.getCustomerAddress(addressList,
					InterfaceConstants.ADDR_TYPE_PERNMENT);
