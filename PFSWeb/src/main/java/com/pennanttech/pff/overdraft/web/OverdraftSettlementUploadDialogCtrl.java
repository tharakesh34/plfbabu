...
public class OverdraftSettlementUploadDialogCtrl extends GFCBaseCtrl<Configuration> {
	...
	private long userId;
	private DataEngineStatus OD_SETTLEMENT_REQUEST_UPLOAD = new DataEngineStatus(
			OverdraftConstants.OD_SETTLEMENT_REQUEST_UPLOAD);

	protected DataEngineConfig dataEngineConfig;
	private OverdraftSettlementResponseUpload overdraftSettlementResponseUpload;

	public OverdraftSettlementUploadDialogCtrl() {
		super();
	}
	...
	public void onClick$btnImport(Event event) throws InterruptedException {
		...
		try {
			try {
				Thread thread = new Thread(new ProcessData(this.userId, OD_SETTLEMENT_REQUEST_UPLOAD));
				thread.start();
			} catch (Exception e) {
	...
	public class ProcessData implements Runnable {
		...
		public void run() {
			try {
				overdraftSettlementResponseUpload.oDSettlementFileUploadProcessResponseFile(this.userId, status, file,
						media, false);
			} catch (Exception e) {
	...
	public void getConfigData() throws Exception {
		...
		for (Configuration config : configList) {
			...
			if ("OD_SETTLEMENT_REQUEST_UPLOAD".equals(configName)) {
				OD_SETTLEMENT_REQUEST_UPLOAD = dataEngineConfig.getLatestExecution("OD_SETTLEMENT_REQUEST_UPLOAD");
				valueLabel = new ValueLabel(configName, "OD Settlement process upload");
				doFillPanel(config, OD_SETTLEMENT_REQUEST_UPLOAD);
		...
		for (Row row : rows) {
			...
			for (Hbox hbox : hboxs) {
				...
				for (ProcessExecution pe : list) {
					...
					if ("OD_SETTLEMENT_REQUEST_UPLOAD".equals(pe.getProcess().getName())) {
