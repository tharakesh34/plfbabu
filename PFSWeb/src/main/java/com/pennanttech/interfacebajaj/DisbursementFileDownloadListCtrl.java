 ...
 * 
 */
public class DisbursementFileDownloadListCtrl extends GFCBaseListCtrl<FileDownlaod> implements Serializable {

	private static final long serialVersionUID = 1L;
	private final static Logger logger = Logger.getLogger(DisbursementFileDownloadListCtrl.class);

	protected Window window_DisbursementFileDownloadList;
	protected Borderlayout borderLayout_DisbursementFileDownloadList;
	...
	private class FileDownloadListModelItemRenderer implements ListitemRenderer<FileDownlaod>, Serializable {
		...
		public void render(Listitem item, FileDownlaod fileDownlaod, int count) throws Exception {
			...
			lc = new Listcell(String.valueOf(fileDownlaod.getId()));
			lc.setParent(item);

			lc = new Listcell(fileDownlaod.getName());
			lc.setParent(item);

			lc = new Listcell((fileDownlaod.getFinReference()));
			lc.setParent(item);

			lc = new Listcell((fileDownlaod.getPartnerBankCode()));
			lc.setParent(item);
