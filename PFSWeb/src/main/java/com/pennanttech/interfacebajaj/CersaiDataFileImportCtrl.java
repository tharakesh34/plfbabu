...
public class CersaiDataFileImportCtrl extends GFCBaseCtrl<Configuration> {
	...
	private static final Logger logger = Logger.getLogger(CersaiDataFileImportCtrl.class);
	...
	public void onCreate$window_CersaiDataFileImportCtrl(Event event) throws Exception {
		...
		for (Configuration config : configList) {
			...
			if ((StringUtils.equals(configName, PennantConstants.CERSAI_IMPORT))) {
				this.config = config;
				menuList.add(valueLabel);
				DataEngineStatus status = dataEngineConfig.getLatestExecution(config.getName());

				if (status != null) {
					BeanUtils.copyProperties(CersaiProcess.CERSAI_IMPORT, status);
				}
				doFillPanel();
				break;
			}
	...
	public void onClick$btnImport(Event event) throws InterruptedException {
		...
		if (CersaiProcess.CERSAI_IMPORT != null
				&& ExecutionStatus.I.name().equals(CersaiProcess.CERSAI_IMPORT.getStatus())) {
		...
		for (Row row : rows) {
			...
			for (Hbox hbox : hboxs) {
				...
				for (ProcessExecution pe : list) {
					pe.setProcess(CersaiProcess.CERSAI_IMPORT);
	...
	private void doFillPanel() {
		...
		pannelExecution.setProcess(CersaiProcess.CERSAI_IMPORT);
		...
		public void run() {
			try {
				if (cersaiProcess != null) {
					...
					cersaiProcess.processResponseFile(userId, file, media);
				}
			} catch (Exception e) {
