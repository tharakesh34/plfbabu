...
@Component(value = "pdVerificationDialogCtrl")
@Scope(ConfigurableBeanFactory.SCOPE_PROTOTYPE)
public class PDVerificationDialogCtrl extends GFCBaseCtrl<Verification> {
	private static final long serialVersionUID = 8661799804403963415L;
	private static final Logger logger = LogManager.getLogger(PDVerificationDialogCtrl.class);

	protected Window window_PDVerificationDialog;
	 ...
	 */
	public PDVerificationDialogCtrl() {
		super();
	}
	...
	public void onCreate$window_PDVerificationDialog(Event event) throws Exception {
		...
		this.verification = financeDetail.getPdVerification();
		if (this.verification == null) {
			this.verification = new Verification();
			this.financeDetail.setPdVerification(this.verification);
		}
		...
		if (arguments.containsKey("financeMainBaseCtrl")) {
			((FinanceMainBaseCtrl) arguments.get("financeMainBaseCtrl")).setPdVerificationDialogCtrl(this);
		} else {
	...
	public void onCheck$pd(Event event) {
		PersonalDiscussion personalDiscussion = null;
		if (pdInquiry.getChildren().size() >= 2) {
	...
	/**
	 * Method to fill Pd Initiation/Approval tab.
	 * 
	 * @param customer
	 */
	public void renderPDVerificationList() {
		logger.debug(Literal.ENTERING);

		if (this.listBoxPDVerification.getItems() != null) {
		...
		for (Verification vrf : this.verification.getVerifications()) {
			...
			requestType.addForward("onChange", self, "onChnagePdv", item);
			...
			this.listBoxPDVerification.appendChild(item);
	...
	private void fillDecision(Verification vrf, Combobox decision) {
		...
		if (vrf.getRequestType() == RequestType.NOT_REQUIRED.getKey()
				|| vrf.getStatus() == PDStatus.POSITIVE.getKey()) {
			...
			if (current != null) {
				for (CustomerAddres newAddress : addresses) {
					...
					if (StringUtils.equals(newAddress.getCustAddrType(), previous.getReferenceFor())
						...
						if (personalDiscussionService.isAddressChange(oldAddres, newAddress)) {
							if (verificationService.isVerificationInRecording(previous, VerificationType.PD)) {
	...
	private List<Verification> getOldVerifications(String custCif) {
		...
		verifications = verificationService.getVerifications(keyReference, VerificationType.PD.getKey());
		if (custCif == null) {
			List<PersonalDiscussion> pdList = personalDiscussionService.getList(keyReference);
			for (Verification vrf : verifications) {
				for (PersonalDiscussion item : pdList) {
	...
	private void setOldVerificationFields(Verification current, Verification previous) {
		...
		current.setVerificationType(VerificationType.PD.getKey());
	}
	...
	protected void doClearMessage() {
		for (Listitem listitem : listBoxPDVerification.getItems()) {
	...
	public boolean doSave(FinanceDetail financeDetail, Tab tab, boolean recSave, Radiogroup userAction)
		...
		financeDetail.setPdVerification(this.verification);

		logger.debug("Leaving");
		if (tab.getId().equals("TAB".concat(AssetConstants.UNIQUE_ID_PDAPPROVAL))) {
			return validateReinitiation(financeDetail.getPdVerification().getVerifications());
		}
		return true;
	}
