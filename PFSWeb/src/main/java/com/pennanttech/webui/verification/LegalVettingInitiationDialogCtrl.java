 ...
 * FILE HEADER *
 ********************************************************************************************
 * * FileName : LegalVettingInitiationDialogCtrl.java * * Author : PENNANT TECHONOLOGIES * * Creation Date : 26-04-2018
...
/**
 * This is the controller class for the /WEB-INF/pages/Finance/FinanceMain/Verification/LegalVettingInitiationDialog.zul
 * file.
 */
@Component(value = "legalVettingInitiationDialogCtrl")
@Scope(ConfigurableBeanFactory.SCOPE_PROTOTYPE)
public class LegalVettingInitiationDialogCtrl extends GFCBaseCtrl<Verification> {
	private static final long serialVersionUID = -3093280086658721485L;
	private static final Logger logger = LogManager.getLogger(LegalVettingInitiationDialogCtrl.class);

	protected Window window_LegalVettingInitiationDialog;
	...
	private List<LVDocument> vettingDocuments;

	private FinanceMainBaseCtrl financeMainDialogCtrl = null;
	private FinanceDetail financeDetail;

	private Set<String> vettingRequiredDocs = new HashSet<>();
	 ...
	 */
	public LegalVettingInitiationDialogCtrl() {
		super();
	}

	@Override
	protected void doSetProperties() {
		super.pageRightName = "LegalVettingInitiationDialog";
	}
	...
	public void onCreate$window_LegalVettingInitiationDialog(Event event) throws Exception {
		logger.debug(Literal.ENTERING);

		// Set the page level components.
		setPageComponents(window_LegalVettingInitiationDialog);
		...
		try {
			...
			if (arguments.containsKey("vettingDocuments")) {
				this.vettingDocuments = (List<LVDocument>) arguments.get("vettingDocuments");
			}
			...
			if (isWorkFlowEnabled()) {
				...
				getUserWorkspace().allocateRoleAuthorities(getRole(), "LegalVettingInitiationDialog");
			}

			if (arguments.containsKey("legalVettingVerificationListCtrl")) {
				setLegalVettingInitiationCtrl(
						(LegalVettingInitiationCtrl) arguments.get("legalVettingVerificationListCtrl"));
			} else {
				setLegalVettingInitiationCtrl(null);
			}
			...
			this.window_LegalVettingInitiationDialog.onClose();
		}
	...
	private void fillListBox() {
		if (legalVettingInitiationCtrl != null) {
			if (verification.isWaiveTab()) {
				legalVettingInitiationCtrl.renderVettingInitiationList();
				legalVettingInitiationCtrl.renderVettingWaiverList();
			} else if (initiation) {
				legalVettingInitiationCtrl.renderVettingInitiationList();
			} else {
				legalVettingInitiationCtrl.renderVettingWaiverList();
	...
	public void onChangeCollateral(ForwardEvent event) throws Exception {
		...
		if (dataObject != null) {
			...
			if (dataObject instanceof String) {
				...
				fillDocuments(this.listBoxCollateralDocuments, this.vettingDocuments, DocumentType.COLLATRL,
	...
	public void onClick$btnHelp(Event event) throws InterruptedException {
		MessageUtil.showHelpWindow(event, window_LegalVettingInitiationDialog);
	}
	...
	public void doWriteBeanToComponents(Verification aVerification) {
		...
		if (initiation) {
			fillDocuments(this.listBoxLoanDocuments, this.vettingDocuments, DocumentType.LOAN, null);
			fillDocuments(this.listBoxCustomerDocuments, this.vettingDocuments, DocumentType.CUSTOMER, null);
		}
	...
	public void fillDocuments(Listbox listbox, List<LVDocument> documents, DocumentType docType, String collateralRef) {
		...
		if (initiation) {
			...
			if (!verification.getVettingDocuments().isEmpty()) {
				checkedDocuments = getCheckedDocuments(this.verification.getVettingDocuments());
			}
		}
				...
				for (LVDocument oldDoc : oldDocuments) {
					if (newDoc.getDocumentType() == DocumentType.COLLATRL.getKey()
						...
						if (!newDoc.getDocumentRefId().equals(oldDoc.getDocumentRefId())
								&& verificationService.isVerificationInRecording(vrf, VerificationType.VETTING)
		...
		for (LVDocument document : documents) {
			...
			if ((initiation && vettingRequiredDocs.contains(document.getDocumentSubId())
					&& this.verification.isNewRecord()) || checkedDocuments.contains(reference)) {
	...
	private void setVettingDocuments() {
		verification.getVettingDocuments().clear();
		for (Listitem listitem : listBoxCollateralDocuments.getItems()) {
			...
			if (docIdBox.isChecked()) {
				this.verification.getVettingDocuments().add(docIdBox.getValue());
			}
		}
	...
	public void doShowDialog(Verification verification) throws Exception {
		...
		if (isNewRecord()) {
			...
			this.agency.focus();
			boolean isLVExists = legalVettingService.isVettingExists(verification.getId());
			if (isWorkFlowEnabled()) {
		...
		try {
			...
			getLegalVettingInitiationCtrl().setLegalVettingInitiationDialogCtrl(this);

			doCheckEnquiry();
			this.window_LegalVettingInitiationDialog.setHeight("80%");
			this.window_LegalVettingInitiationDialog.setWidth("75%");
			this.groupboxWf.setVisible(false);
			this.window_LegalVettingInitiationDialog.doModal();
	...
	private void doSetValidation() {
		...
		if (!this.collateral.isReadonly()) {
			this.collateral.setConstraint(new PTStringValidator(
					Labels.getLabel("label_LegalVettingInitiationDialog_Collateral.value"), null, true, true));
		}

		if (initiation) {
			if (!this.agency.isReadonly()) {
				this.agency.setConstraint(new PTStringValidator(
						Labels.getLabel("label_LegalVettingInitiationDialog_Agency.value"), null, true, true));
			}
		} else {
			if (!this.reason.isReadonly()) {
				this.reason.setConstraint(new PTStringValidator(
						Labels.getLabel("label_LegalVettingInitiationDialog_Reason.value"), null, true, true));
			}
		}
	 ...
	 * @return
	 */
	private boolean validateVettingDocuments() {
		for (Listitem listitem : listBoxCollateralDocuments.getItems()) {
			Checkbox docIdBox = (Checkbox) listitem.getFirstChild().getFirstChild();
			LVDocument document = docIdBox.getValue();
			if (!docIdBox.isChecked() && vettingRequiredDocs.contains(document.getDocumentSubId()) && initiation) {
			...
			if (StringUtils.isBlank(method)) {
				if (auditHeader.getAuditTranType().equals(PennantConstants.TRAN_DEL)) {
					...
					if (verification.isApproveTab()) {
						verificationService.savereInitLegalVetting(financeDetail, verification);
					} else {
						verificationService.saveLegalVetting(verification);
					}
				}
				...
				if (StringUtils.trimToEmpty(method).equalsIgnoreCase(PennantConstants.method_doApprove)) {
					...
					ErrorControl.showErrorControl(this.window_LegalVettingInitiationDialog, auditHeader);
					return processCompleted;
				}
			}
			auditHeader = ErrorControl.showErrorDetails(this.window_LegalVettingInitiationDialog, auditHeader);
	...
	private void setDocumentDetails() {
		this.vettingRequiredDocs.clear();
		...
		for (com.pennant.backend.model.systemmasters.DocumentType documentType : list) {
			if (documentType.isLvReq()) {
				this.vettingRequiredDocs.add(documentType.getDocTypeCode());
			}
