...
/**
 * This is the controller class for the /WEB-INF/pages/Finance/FinanceMain/ SelectRolloverFinanceDialog.zul file.
 */
public class SelectRolloverFinanceDialogCtrl extends GFCBaseCtrl<RolledoverFinanceDetail> {
	private static final long serialVersionUID = 8556168885363682933L;
	private static final Logger logger = LogManager.getLogger(SelectRolloverFinanceDialogCtrl.class);
	...
	/**
	 * Method to get List of Rollover Dates from Finance Main which are used for Rollover & Selected Limit Ref with
	 * FinaceType Selection
	 * 
	 * @return
	 */
	...
	public void onSelect$rolloverDate(Event event) {
		...
		if (this.rolloverDate.getSelectedIndex() > 0) {
			...
			rolledoverFinances = getFinanceDetailService().getRolloverFinanceList(this.limitRef.getValue(),
	...
	public void onClick$btnProceed(Event event) throws Exception {
		...
		if (StringUtils.isNotBlank(this.finType.getValue())) {
			...
			if (getFinanceWorkFlow() == null) {
				FinanceWorkFlow financeWorkFlow = getFinanceWorkFlowService().getApprovedFinanceWorkFlowById(
						this.finType.getValue().trim(), FinanceConstants.FINSER_EVENT_ROLLOVER,
		...
		try {
			...
			if (financeDetail.getFinScheduleData().getFinanceMain() != null) {
				...
				financeDetail.getFinScheduleData().getFinanceMain()
						.setFinStatus(getFinanceDetailService().getCustStatusByMinDueDays());
			}
		} catch (Exception e) {
			logger.debug(e);
		}
		...
		financeDetail.getFinScheduleData().getFinanceMain()
				.setLovDescCustCIF(customerDetails.getCustomer().getCustCIF());
		financeDetail.getFinScheduleData().getFinanceMain()
				.setLovDescCustShrtName(customerDetails.getCustomer().getCustShrtName());
		financeDetail.getFinScheduleData().getFinanceMain()
				.setFinBranch(customerDetails.getCustomer().getCustDftBranch());
		financeDetail.getFinScheduleData().getFinanceMain()
				.setLovDescFinBranchName(customerDetails.getCustomer().getLovDescCustDftBranchName());
		financeDetail.getFinScheduleData().getFinanceMain().setCustID(customerDetails.getCustomer().getCustID());
		...
		if (!isFinAssetTypeEmpty) {
			if (this.finPurposerow.isVisible()
					&& !getComboboxValue(this.finPurpose).equals(PennantConstants.List_Select)) {
				financeDetail.getFinScheduleData().getFinanceMain()
						.setLovDescFinPurposeName(this.finPurpose.getSelectedItem().getLabel());
				financeDetail.getFinScheduleData().getFinanceMain()
						.setFinPurpose(this.finPurpose.getSelectedItem().getValue().toString());
			} else if (!this.finPurposerow.isVisible()) {
				financeDetail.getFinScheduleData().getFinanceMain()
						.setLovDescFinPurposeName(this.finPurpose.getItemAtIndex(1).getLabel());
				financeDetail.getFinScheduleData().getFinanceMain()
						.setFinPurpose(this.finPurpose.getItemAtIndex(1).getValue().toString());
			}
		}
		...
		header.setLatePayAmount(CalculationUtil.calInterest((Date) this.rolloverDate.getSelectedItem().getValue(),
				DateUtility.getAppDate(), totalRolloverAmount,
				financeDetail.getFinScheduleData().getFinanceMain().getProfitDaysBasis(),
				latestRollover.getProfitRate()));
