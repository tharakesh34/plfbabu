...
public class BulkDefermentChangeCtrl extends GFCBaseListCtrl<BulkDefermentChange> {
	...
	private static final Logger logger = LogManager.getLogger(BulkDefermentChangeCtrl.class);
	...
	public void throwValidation() {
		...
		try {
			if (this.cbReCalType.getSelectedItem() != null && this.cbReCalType.getSelectedItem().getValue().toString()
	...
	public void onClick$btnProceed(Event event) throws InterruptedException, WrongValueException, InterfaceException,
		...
		boolean success = getFinanceDetailService().bulkDefermentChanges(getDefermentChangeFinances(),
				this.cbReCalType.getSelectedItem().getValue().toString(), this.exDefDate.isChecked(), addTermAfter,
				this.calFromDate.getValue(), this.calToDate.getValue());
	...
	private void doSetValidation() {
		...
		if (!this.fromDate.isDisabled()) {
			this.fromDate
					.setConstraint(new PTDateValidator(Labels.getLabel("label_BulkDefferment_FromDate.value"), true));
		}
		...
		if (this.recalFromDateRow.isVisible()) {
			...
			if (!this.calFromDate.isDisabled()) {
				this.calFromDate.setConstraint(
						new PTDateValidator(Labels.getLabel("label_BulkDefferment_CalFromDate.value"), true));
			}

			if (!this.calToDate.isDisabled()) {
				this.calToDate.setConstraint(
						new PTDateValidator(Labels.getLabel("label_BulkDefferment_CalToDate.value"), true));
			}
	...
	public void setDefermentChangeFinances(List<BulkDefermentChange> defermentChangeFinances) {
