 ...
 * FILE HEADER *
 ********************************************************************************************
 * * FileName : OverdueChargeRecoverySearchCtrl.java * * Author : PENNANT TECHONOLOGIES * * Creation Date : 11-05-2012 *
...
public class OverdueChargeRecoverySearchCtrl extends GFCBaseCtrl<OverdueChargeRecovery> {
	...
	private static final Logger logger = LogManager.getLogger(OverdueChargeRecoverySearchCtrl.class);
	...
	public void onCreate$window_OverdueChargeRecoverySearch(Event event) {
	 ...
	 */
	private void showOverdueChargeRecoverySeekDialog() {
	...
	public void doSearch() {
		logger.debug("Entering");
		final JdbcSearchObject<OverdueChargeRecovery> so = new JdbcSearchObject<OverdueChargeRecovery>(
		...
		if (StringUtils.isNotEmpty(this.finODCCustCtg.getValue())) {
			...
			if (item_FinODCCustCtg != null) {
				final int searchOpId = ((SearchOperators) item_FinODCCustCtg.getAttribute("data"))
						.getSearchOperatorId();

				if (searchOpId == Filter.OP_LIKE) {
					so.addFilter(new Filter("finODCCustCtg", "%" + this.finODCCustCtg.getValue().toUpperCase() + "%",
							searchOpId));
				} else if (searchOpId == -1) {
					// do nothing
				} else {
		...
		if (this.finODCLastPaidDate.getValue() != null) {
			...
			if (item_FinODCLastPaidDate != null) {
				final int searchOpId = ((SearchOperators) item_FinODCLastPaidDate.getAttribute("data"))
						.getSearchOperatorId();
		...
		if (StringUtils.isNotEmpty(this.finODCRecoverySts.getValue())) {
			...
			if (item_FinODCRecoverySts != null) {
				final int searchOpId = ((SearchOperators) item_FinODCRecoverySts.getAttribute("data"))
						.getSearchOperatorId();

				if (searchOpId == Filter.OP_LIKE) {
					so.addFilter(new Filter("finODCRecoverySts",
							"%" + this.finODCRecoverySts.getValue().toUpperCase() + "%", searchOpId));
				} else if (searchOpId == -1) {
					// do nothing
				} else {
		...
		// DAO method.
		final SearchResult<OverdueChargeRecovery> searchResult = this.overdueChargeRecoveryCtrl.getPagedListService()
		...
		this.label_OverdueChargeRecoverySearchResult
				.setValue(Labels.getLabel("label_OverdueChargeRecoverySearchResult.value") + " "
	...
	public void setOverdueChargeRecoveryCtrl(OverdueChargeRecoveryListCtrl overdueChargeRecoveryCtrl) {
