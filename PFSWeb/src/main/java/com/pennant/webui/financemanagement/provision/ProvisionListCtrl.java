...
public class ProvisionListCtrl extends GFCBaseListCtrl<Provision> {
	...
	private void doCheckRights() {
		...
		if ("Provision".equals(this.module)) {
			this.button_ProvisionList_NewProvision
					.setVisible(getUserWorkspace().isAllowed("button_ProvisionList_NewProvision"));
			this.button_ProvisionList_ProvisionSearchDialog
					.setVisible(getUserWorkspace().isAllowed("button_ProvisionList_ProvisionFindDialog"));
		} else {
			this.button_ProvisionList_NewProvision.setVisible(false);
		}

		this.button_ProvisionList_PrintList.setVisible(getUserWorkspace().isAllowed("button_ProvisionList_PrintList"));
		...
		if (item != null) {
			...
			if (provision == null) {
				...
				if (isWorkFlowEnabled()) {
					String whereCond = " AND FinReference='" + provision.getFinReference() + "' AND version="
							+ provision.getVersion() + " ";

					boolean userAcces = validateUserAccess(provision.getWorkflowId(),
							getUserWorkspace().getLoggedInUser().getUserId(), "Provision", whereCond,
							provision.getTaskId(), provision.getNextTaskId());
					if (userAcces) {
	...
	private void showDetailView(Provision aProvision) throws Exception {
		 ...
		 * listbox Listmodel. This is fine for synchronizing the data in the ProvisionListbox from the dialog when we do
		 * a delete, edit or insert a Provision.
		 */
		...
		try {
			if ("Provision".equals(this.module)) {
				...
				Executions.createComponents(
						"/WEB-INF/pages/FinanceManagement/ProvisionMovement/ProvisionMovementEnquiryDialog.zul", null,
						map);
			}
		} catch (Exception e) {
			MessageUtil.showError(e);
	...
	public void onClick$button_ProvisionList_PrintList(Event event) throws InterruptedException {
		logger.debug("Entering" + event.toString());
		PTListReportUtils reportUtils = new PTListReportUtils("Provision", getSearchObj(),
				this.pagingProvisionList.getTotalSize() + 1);
		logger.debug("Leaving" + event.toString());
	}
	...
	public void doSearch() {
		...
		if (this.provisionedAmt.getValue() != null) {
			searchObj = getSearchFilter(searchObj, this.sortOperator_provisionedAmt.getSelectedItem(),
