...
public class DocumentDialogCtrl extends GFCBaseCtrl<DocumentDetails> {
	...
	private static final Logger logger = LogManager.getLogger(DocumentDialogCtrl.class);
	...
	private ManagerChequeDialogCtrl managerChequeDialogCtrl;
	...
	public void doWriteBeanToComponents(DocumentDetails aDocumentDetails) {
		...
		if (aDocumentDetails.getDocImage() != null) {
			...
			if (aDocumentDetails.getDoctype().equals(PennantConstants.DOC_TYPE_PDF)) {
				...
				amedia = new AMedia("document.octet-stream", "octet-stream", "application/octet-stream", data);
			} else if (getDocumentDetails().getDoctype().equals(PennantConstants.DOC_TYPE_RAR)) {
				amedia = new AMedia("document.x-rar-compressed", "x-rar-compressed", "application/x-rar-compressed",
						data);
			}
