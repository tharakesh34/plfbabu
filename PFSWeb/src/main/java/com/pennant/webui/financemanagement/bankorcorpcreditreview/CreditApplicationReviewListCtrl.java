...
public class CreditApplicationReviewListCtrl extends GFCBaseListCtrl<FinCreditReviewDetails> {
	 ...
	 * 
	 * logger.debug("Entering "); // ++ create the searchObject and initialize sorting ++//
	 * 
	 * this.searchObjCreditReviewDetails = new JdbcSearchObject<FinCreditReviewDetails>(FinCreditReviewDetails.class,
	 * getListRows());
	 * 
	 * if (isMaintinence) { this.searchObjCreditReviewDetails.addTabelName("FinCreditReviewDetails_AMView");
	 * this.searchObjCreditReviewDetails.addFilterNotIn("RecordStatus", "'Approved'"); } else {
	 ...
	 * 
	 * this.searchObjCreditReviewDetails.addFilterIn("AuditYear", new ArrayList<String>() {{
	 * add(String.valueOf(dateAppCurrentYear)); add(String.valueOf(dateAppPrevYear));
	 * add(String.valueOf(dateAppPrevYear-1)); }} ,true);
	 * 
	 * 
	 * String auditYears = "'" + dateAppCurrentYear + "','" + dateAppPrevYear + "','" + (dateAppPrevYear - 1) + "'";
	 * String whereCondition = "AuditYear IN(" + auditYears + ")" + " OR " + " (AuditYear < " + (dateAppPrevYear - 1) +
	 * " AND RecordStatus <> 'Approved')"; this.searchObjCreditReviewDetails.addWhereClause(whereCondition); }
	 * 
	 * if (!isMaintinence) { this.searchObjCreditReviewDetails.addFilterEqual("Division", creditDivision); }
	 * 
	 * // Work flow if (isWorkFlowEnabled()) {
	 * 
	 * if (isFirstTask() && this.moduleType == null && !isMaintinence) {
	 * button_CreditApplicationReviewList_NewCreditApplicationReview.setVisible(true); } else {
	 * button_CreditApplicationReviewList_NewCreditApplicationReview.setVisible(false); }
	 * 
	 * if (this.moduleType == null) { this.searchObjCreditReviewDetails.addFilterIn("nextRoleCode",
	 * getUserWorkspace().getUserRoles(), isFirstTask()); approvedList = false; } else { if
	 * (this.fromApproved.isSelected()) { approvedList = true; } else {
	 * this.searchObjCreditReviewDetails.addTabelName("FinCreditReviewDetails_View"); approvedList = false; } } } else {
	 * approvedList = true; } if (approvedList) {
	 * this.searchObjCreditReviewDetails.addTabelName("FinCreditReviewDetails_AView"); }
	 * 
	 * // Cust CIF if (StringUtils.isNotBlank(this.custCIF.getValue())) { searchObjCreditReviewDetails =
	 * getSearchFilter(searchObjCreditReviewDetails, this.sortOperator_custCIF.getSelectedItem(),
	 * this.custCIF.getValue(), "lovDescCustCIF"); } // Cust Name if (StringUtils.isNotBlank(this.custName.getValue()))
	 * { searchObjCreditReviewDetails = getSearchFilter(searchObjCreditReviewDetails,
	 * this.sortOperator_CustName.getSelectedItem(), this.custName.getValue(), "lovDescCustShrtName"); } // cust Credit
	 * Review Code if (!"#".equals(this.custCreditReviewCode.getSelectedItem().getValue())) {
	 * searchObjCreditReviewDetails = getSearchFilter(searchObjCreditReviewDetails,
	 * this.sortOperator_custCreditReviewCode.getSelectedItem(), this.custCreditReviewCode
	 * .getSelectedItem().getValue(), "CreditRevCode"); }
	 * 
	 * // Cust Audit Year if (!"#".equals(this.custAuditYear.getSelectedItem().getValue())) {
	 * searchObjCreditReviewDetails = getSearchFilter(searchObjCreditReviewDetails,
	 * this.sortOperator_custAuditYear.getSelectedItem(),
	 * Integer.parseInt(this.custAuditYear.getSelectedItem().getValue().toString()), "AuditYear"); }
	 * 
	 * // Cust Bank Name if (StringUtils.isNotBlank(this.custBankName.getValue())) { searchObjCreditReviewDetails =
	 * getSearchFilter(searchObjCreditReviewDetails, this.sortOperator_custBankName.getSelectedItem(),
	 * this.custBankName.getValue(), "BankName"); }
	 * 
	 * if (logger.isDebugEnabled()) { final List<Filter> lf = this.searchObjCreditReviewDetails.getFilters(); for (final
	 * Filter filter : lf) { logger.debug(filter.getProperty() + " / " + filter.getValue().toString());
	 * 
	 * if (Filter.OP_ILIKE == filter.getOperator()) { logger.debug(filter.getOperator()); } } }
	 * 
	 * // Set the ListModel for the articles. getPagedListWrapper().init(this.searchObjCreditReviewDetails,
	 * this.listBoxCreditApplicationReview, this.pagingCreditApplicationReviewList); logger.debug("Leaving ");
	 * 
	 * }
	 */
