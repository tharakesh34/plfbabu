 ...
 */
public class CrossLoanKnockOffDialogCtrl extends GFCBaseCtrl<CrossLoanKnockOffHeader> {
	 ...
	 */
	protected Window window_CrossLoanKnockOffDialog;
	...
	protected Listheader listheader_ReceiptSchedule_ScheduleEndBal;
	...
	private List<XcessPayables> xcessPayables = new ArrayList<XcessPayables>();
	...
	protected CrossLoanKnockOffHeader crossLoanHeader;
	 ...
	 * @throws Exception
	 */
	public void onCreate$window_CrossLoanKnockOffDialog(Event event) throws Exception {
		logger.debug("Entering");

		// Set the page level components.
		setPageComponents(window_CrossLoanKnockOffDialog);
		...
		try {
			...
			if (arguments.containsKey("crossLoanHeader")) {
				crossLoanHeader = (CrossLoanKnockOffHeader) arguments.get("crossLoanHeader");
				if (crossLoanHeader != null) {
					...
					xcessPayables = receiptData.getReceiptHeader().getXcessPayables();
				}
			}
			...
			this.window_CrossLoanKnockOffDialog.onClose();
		}
	...
	public void onClick$btnSearchFinreference(Event event) throws SuspendNotAllowedException, InterruptedException {
		...
		map.put("window_ReceiptDialog", this.window_CrossLoanKnockOffDialog);
		Executions.createComponents("/WEB-INF/pages/Enquiry/FinanceInquiry/FinanceEnquiryHeaderDialog.zul",
				this.window_CrossLoanKnockOffDialog, map);
	...
	private void setClosedLoanDetails(long finId) {
		FinanceMain finMain = getReceiptService().getClosingStatus(finId, TableType.MAIN_TAB, false);
	...
	public void doEdit() {
		...
		// this.receivedDate);
		readOnlyComponent(isReadOnly("ReceiptDialog_DrawerName"), this.drawerName);
	...
	private boolean setSummaryData(boolean isChgReceipt)
		...
		if (orgReceiptData != null) {
			...
			FinScheduleData befFinSchedData = receiptData.getFinanceDetail().getFinScheduleData().copyEntity();
	...
	public void doFillScheduleList(FinScheduleData aFinScheduleData) {
		...
		if (StringUtils.equals(FinanceConstants.PRODUCT_ODFACILITY, financeMain.getProductCategory())
			...
			if (financeMain.isAlwFlexi()) {
				...
				listheader_ReceiptSchedule_ScheduleEndBal
						.setLabel(Labels.getLabel("listheader_ScheduleDetailDialog_SchdUtilization"));
		...
		for (int i = 0; i < aFinScheduleData.getFinanceScheduleDetails().size(); i++) {
			...
			map.put("window", this.window_CrossLoanKnockOffDialog);
	...
	private void doProcessReceipt() throws Exception {
		...
		Cloner cloner = new Cloner();
		CrossLoanKnockOffHeader crossLoanHKnockOff = cloner.deepClone(crossLoanHeader);
	...
	private void createAllocateItem(ReceiptAllocationDetail allocate, boolean isManAdv, String desc, int idx) {
		...
		if (allocate.getAllocationTo() < 0) {
			for (FinFeeDetail fee : receiptData.getFinanceDetail().getFinScheduleData().getFinFeeDetailList()) {
				if (allocate.getAllocationTo() == -(fee.getFeeTypeID())
					...
					button.addForward("onClick", window_CrossLoanKnockOffDialog, "onFeeDetailsClick", button.getId());
		...
		allocationPaid.addForward("onFulfill", this.window_CrossLoanKnockOffDialog, "onAllocatePaidChange", idx);
		...
		allocationWaived.addForward("onFulfill", this.window_CrossLoanKnockOffDialog, "onAllocateWaivedChange", idx);
		allocationWaived.setReadonly(true);
	...
	private void doWriteComponentsToBean() {
		...
		header.setReference(this.fromFinReference.getValue());
	...
	private String getServiceTasks(String taskId, CrossLoanKnockOffHeader rch, String finishedTasks) {
		logger.debug("Entering");
	...
	private void setNextTaskDetails(String taskId, CrossLoanKnockOffHeader clkHeader) {
		logger.debug("Entering");
	 ...
	 * @throws Exception
	 */
	public boolean doProcess(CrossLoanKnockOffHeader crossLoanKnocKOff, String tranType) throws Exception {
		logger.debug("Entering");
		...
		if (isWorkFlowEnabled()) {
			...
			while (!"".equals(serviceTasks)) {
				...
				if (StringUtils.trimToEmpty(method).contains(PennantConstants.method_doCheckCollaterals)) {
					...
					CrossLoanKnockOffHeader clkHeader = (CrossLoanKnockOffHeader) auditHeader.getAuditDetail()
							.getModelData();
			...
			CrossLoanKnockOffHeader clkHeader = (CrossLoanKnockOffHeader) auditHeader.getAuditDetail().getModelData();
	...
	private boolean doSaveProcess(AuditHeader auditHeader, String method) throws Exception {
		...
		CrossLoanKnockOffHeader crossLoanHeader = (CrossLoanKnockOffHeader) auditHeader.getAuditDetail().getModelData();
		...
		try {
			...
			while (retValue == PennantConstants.porcessOVERIDE) {
				...
				if (StringUtils.isBlank(method)) {
					...
					auditHeader = getCrossLoanKnockOffService().saveOrUpdate(auditHeader);

				} else {
					if (StringUtils.trimToEmpty(method).equalsIgnoreCase(PennantConstants.method_doApprove)) {

						if (crossLoanHeader.isNew()) {
							((FinReceiptData) auditHeader.getAuditDetail().getModelData()).getFinanceDetail()
									.setDirectFinalApprove(true);
						}

						auditHeader = getCrossLoanKnockOffService().doApprove(auditHeader);
						...
						auditHeader = getCrossLoanKnockOffService().doReject(auditHeader);
						...
						retValue = ErrorControl.showErrorControl(this.window_CrossLoanKnockOffDialog, auditHeader);
						return processCompleted;
					}
				}

				auditHeader = ErrorControl.showErrorDetails(this.window_CrossLoanKnockOffDialog, auditHeader);
	 ...
	 */
	private AuditHeader getAuditHeader(CrossLoanKnockOffHeader crossLoanHeader, String tranType) {
	...
	public void addBoldTextCell(Listitem item, String value, boolean hasChild, int buttonId) {
		...
		if (hasChild) {
			...
			button.addForward("onClick", window_CrossLoanKnockOffDialog, "onDetailsClick", button.getId());
			lc.appendChild(button);
		}
		lc.setParent(item);
	}
