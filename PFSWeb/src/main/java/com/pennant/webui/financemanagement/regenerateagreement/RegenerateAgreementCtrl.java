...
public class RegenerateAgreementCtrl extends GFCBaseCtrl<FinReceiptHeader> {
	...
	private void doSetFieldProperties() {
		...
		List<String> regenerateAggDocTypes = documentDetailsDAO.getRegenerateAggDocTypes();
		ArrayList<ValueLabel> productList = new ArrayList<ValueLabel>();
		for (int i = 0; i < regenerateAggDocTypes.size(); i++) {
			ValueLabel product = new ValueLabel(String.valueOf(regenerateAggDocTypes.get(i)),
					regenerateAggDocTypes.get(i));
	...
	public void onClick$btnProceed(Event event) throws Exception {
		...
		if (aggDef != null && !aggDef.getDocType().isEmpty()) {
			long docId = documentDetailsDAO.getDocIdByDocTypeAndFinRef(this.loanReference.getValue(),
					aggDef.getDocType(), "_view");
