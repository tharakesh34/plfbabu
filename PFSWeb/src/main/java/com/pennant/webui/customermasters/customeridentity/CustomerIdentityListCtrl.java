 ...
 * FILE HEADER *
 ********************************************************************************************
 * * FileName : CustomerIdentityListCtrl.java * * Author : PENNANT TECHONOLOGIES * * Creation Date : 26-05-2011 * *
...
public class CustomerIdentityListCtrl extends GFCBaseListCtrl<CustomerIdentity> {
	...
	private static final Logger logger = LogManager.getLogger(CustomerIdentityListCtrl.class);
	...
	public void onCustomerIdentityItemDoubleClicked(Event event) {
		...
		if (item != null) {
			...
			String whereCond = " where IdCustID= ?";

			if (doCheckAuthority(customerIdentity, whereCond, new Object[] { customerIdentity.getIdCustID() })) {
	 ...
	 * 
	 * @param customerIdentity The entity that need to be passed to the dialog.
	 */
