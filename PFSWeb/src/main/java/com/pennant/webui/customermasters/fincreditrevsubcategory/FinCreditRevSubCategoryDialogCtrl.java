...
public class FinCreditRevSubCategoryDialogCtrl extends GFCBaseCtrl<FinCreditRevSubCategory> {
	...
	private static final Logger logger = LogManager.getLogger(FinCreditRevSubCategoryDialogCtrl.class);
	...
	public void onCreate$window_FinCreditRevSubCategoryDialog(Event event) {
		...
		try {
			...
			doLoadWorkFlow(this.finCreditRevSubCategory.isWorkflow(), this.finCreditRevSubCategory.getWorkflowId(),
					this.finCreditRevSubCategory.getNextTaskId());
			...
			if (arguments.containsKey("finCreditRevSubCategoryListCtrl")) {
				setFinCreditRevSubCategoryListCtrl(
						(FinCreditRevSubCategoryListCtrl) arguments.get("finCreditRevSubCategoryListCtrl"));
			} else {
	...
	public void doWriteComponentsToBean(FinCreditRevSubCategory aFinCreditRevSubCategory) {
		...
		/*
		 * try { String categoryId =null;
		 * if(StringUtils.trimToEmpty(this.categoryId.getSelectedItem().getValue().toString())!=null){ categoryId =
		 * this.categoryId.getSelectedItem().getValue().toString(); } if(categoryId==null ||
		 * categoryId.equals(PennantConstants.List_Select)){ throw new WrongValueException(
		 * this.categoryId,Labels.getLabel("STATIC_INVALID",new String[] {
		 * Labels.getLabel("label_FinCreditRevSubCategoryDialog_CategoryId.value") })); }
		 * 
		 * if(categoryId==PennantConstants.List_Select){ categoryId =null; }
		 * aFinCreditRevSubCategory.setCategoryId(Long.valueOf(categoryId)); }catch (WrongValueException we ) {
		 * wve.add(we); }
		 */
		try {
		...
		/*
		 * try { aFinCreditRevSubCategory.setSubCategoryItemType(this.subCategoryItemType.getSelectedItem().getValue().
		 * toString()); }catch (WrongValueException we ) { wve.add(we); }
		 */
		try {
		...
		/*
		 * try { aFinCreditRevSubCategory.setMainSubCategoryCode(this.mainSubCategoryCode.getSelectedItem().getValue().
		 * toString()); }catch (WrongValueException we ) { wve.add(we); }
		 */
		try {
	...
	public void doShowDialog(FinCreditRevSubCategory aFinCreditRevSubCategory) throws InterruptedException {
		...
		try {
			 ...
			 * getCreditApplicationReviewDialogCtrl().window_CreditApplicationReviewDialog.getParent().appendChild(this.
			 * window_FinCreditRevSubCategoryDialog);
			 */

			this.getBorderLayoutHeight();
	...
	public void doFillListOperators(Listbox listbox) {
		...
		if (listRuleOperators != null) {
			for (int i = 0; i < listRuleOperators.size(); i++) {
				...
				cell.setParent(item);
				listbox.setHeight(
						Integer.parseInt(getBorderLayoutHeight().substring(0, getBorderLayoutHeight().indexOf("px")))
								- 200 + "px");
	...
	public void getTabsForBreakDown() {
		...
		this.gb_calculationDetails
				.setHeight(Integer.parseInt(getBorderLayoutHeight().substring(0, getBorderLayoutHeight().indexOf("px")))
						- 50 + "px");
		this.tabpanelsBoxIndexCenter
				.setHeight(Integer.parseInt(getBorderLayoutHeight().substring(0, getBorderLayoutHeight().indexOf("px")))
						- 100 + "px");
		this.tabsIndexCenter
				.setHeight(Integer.parseInt(getBorderLayoutHeight().substring(0, getBorderLayoutHeight().indexOf("px")))
						- 80 + "px");
		this.tabpanelBreakDown
				.setHeight(Integer.parseInt(getBorderLayoutHeight().substring(0, getBorderLayoutHeight().indexOf("px")))
						- 100 + "px");
	...
	public void getListboxRender(FinCreditRevCategory fcrc, Listbox listbox) {
		...
		List<FinCreditRevSubCategory> listOfFinCreditRevSubCategory = this.creditApplicationReviewService
				.getFinCreditRevSubCategoryByCategoryId(fcrc.getCategoryId());
		...
		for (int i = 0; i < listOfFinCreditRevSubCategory.size(); i++) {
			...
			cell.setParent(item);
			listbox.setHeight(
					Integer.parseInt(getBorderLayoutHeight().substring(0, getBorderLayoutHeight().indexOf("px"))) - 300
							+ "px");
			item.setParent(listbox);
		}
		logger.debug("Leaving");
	...
	/*
	 * public void doFillListBoxListCodes(){ logger.debug("Entering"); Listitem item; Listcell cell; for (int i = 0; i <
	 * objectFieldList.size(); i++){ item = new Listitem(); cell = new
	 * Listcell(objectFieldList.get(i).getSubCategoryCode()); cell.setStyle("text-align:left;"); cell.setParent(item);
	 * cell = new Listcell(objectFieldList.get(i).getSubCategoryDesc()); cell.setStyle("text-align:left;");
	 * cell.setParent(item); item.setParent(listboxFieldCodes); } logger.debug("Leaving"); }
	 * 
	...
	private void doSetValidation() {
		...
		if (!this.subCategoryCode.isReadonly()) {
			this.subCategoryCode.setConstraint(new PTStringValidator(
					Labels.getLabel("label_FinCreditRevSubCategoryDialog_SubCategoryCode.value"), null, true));
		}

		if (!this.subCategoryDesc.isReadonly()) {
			this.subCategoryDesc.setConstraint(new PTStringValidator(
					Labels.getLabel("label_FinCreditRevSubCategoryDialog_SubCategoryDesc.value"), null, true));
		}
		/*
		 * if (!this.subCategoryItemType.isReadonly()){ this.subCategoryItemType.setConstraint(new
		 * PTStringValidator(Labels.getLabel("label_FinCreditRevSubCategoryDialog_SubCategoryItemType.value"),null,true)
		 * ); }
		 */
		/*
		 * if (!this.itemsToCal.isReadonly()){ this.itemsToCal.setConstraint(new
		 * PTStringValidator(Labels.getLabel("label_FinCreditRevSubCategoryDialog_ItemsToCal.value"),null,true)); }
		 */
		if (!this.itemRule.isReadonly()) {
			this.itemRule.setConstraint(new PTStringValidator(
					Labels.getLabel("label_FinCreditRevSubCategoryDialog_ItemRule.value"), null, true));
		}
		/*
		 * if (!this.mainSubCategoryCode.isReadonly()){ this.mainSubCategoryCode.setConstraint(new
		 * PTStringValidator(Labels.getLabel("label_FinCreditRevSubCategoryDialog_mainSubCategoryCode.value"),null,true)
		 * ); }
		 */
		if (!this.calcSeque.isReadonly()) {
			this.calcSeque.setConstraint(new PTNumberValidator(
					Labels.getLabel("label_FinCreditRevSubCategoryDialog_CalcSeque.value"), true));
		}
		logger.debug("Leaving");
	}
	...
	public void doSave() throws InterruptedException {
		...
		if (this.formula.getValue() != null) {
			...
			if (!this.finCreditRevSubCategoryList.isEmpty()) {
				...
				for (FinCreditRevSubCategory finCreditRevSubCategory : finCreditRevSubCategoryList) {
					if (sFormula.contains(finCreditRevSubCategory.getSubCategoryCode())
						...
						sFormula = sFormula.replace(finCreditRevSubCategory.getSubCategoryCode(),
								"YN." + finCreditRevSubCategory.getSubCategoryCode());
					}
				}
			}

			if (this.subCategoryFolmulaType.getSelectedItem().getValue()
					.equals(Labels.getLabel("label_BreakDown.value"))) {
			...
			for (int i = 0; i < this.getCreditApplicationReviewDialogCtrl().listOfFinCreditRevSubCategory.size(); i++) {
				FinCreditRevSubCategory aFinCreditRevSubCategory = this
						.getCreditApplicationReviewDialogCtrl().listOfFinCreditRevSubCategory.get(i);

				if (aFinCreditRevSubCategory.getSubCategoryCode()
						.equals(this.finCreditRevSubCategory.getSubCategoryCode())) {
					if (this.finCreditRevSubCategory.equals(aFinCreditRevSubCategory)) {
						...
						if (isModified) {
							...
							this.getCreditApplicationReviewDialogCtrl().modifiedFinCreditRevSubCategoryList
									.add(aFinCreditRevSubCategory);
						}
					}
				}
			}
	...
	private boolean doSaveProcess(AuditHeader auditHeader, String method) {
		...
		FinCreditRevSubCategory aFinCreditRevSubCategory = (FinCreditRevSubCategory) auditHeader.getAuditDetail()
				.getModelData();
			...
			while (retValue == PennantConstants.porcessOVERIDE) {
				...
				if (StringUtils.isBlank(method)) {
					...
					if (StringUtils.trimToEmpty(method).equalsIgnoreCase(PennantConstants.method_doApprove)) {
						...
						retValue = ErrorControl.showErrorControl(this.window_FinCreditRevSubCategoryDialog,
								auditHeader);
						return processCompleted;
					}
				}
	...
	private void setListCategoryId() {
		/*
		 * for (int i = 0; i < listCategoryId.size(); i++) { Comboitem comboitem = new Comboitem(); comboitem = new
		 * Comboitem(); comboitem.setLabel(listCategoryId.get(i).getLabel());
		 * comboitem.setValue(listCategoryId.get(i).getValue()); this.categoryId.appendChild(comboitem); }
	...
	private AuditHeader getAuditHeader(FinCreditRevSubCategory aFinCreditRevSubCategory, String tranType) {
		AuditDetail auditDetail = new AuditDetail(tranType, 1, aFinCreditRevSubCategory.getBefImage(),
				aFinCreditRevSubCategory);
		return new AuditHeader(aFinCreditRevSubCategory.getSubCategoryCode(), null, null, null, auditDetail,
				aFinCreditRevSubCategory.getUserDetails(), getOverideMap());
	}
	...
	private void createSimulationWindow(List<ValueLabel> variablesList) throws InterruptedException {
		...
		try {
			Executions.createComponents("/WEB-INF/pages/ApplicationMaster/JavaScriptBuilder/RuleResultView.zul", null,
					map);
		} catch (Exception e) {
			MessageUtil.showError(e);
		}
