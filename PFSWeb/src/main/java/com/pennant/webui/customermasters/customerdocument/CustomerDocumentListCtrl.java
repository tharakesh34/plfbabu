 ...
 * FILE HEADER *
 ********************************************************************************************
 * * FileName : CustomerDocumentListCtrl.java * * Author : PENNANT TECHONOLOGIES * * Creation Date : 26-05-2011 * *
...
public class CustomerDocumentListCtrl extends GFCBaseListCtrl<CustomerDocument> {
	...
	private static final Logger logger = LogManager.getLogger(CustomerDocumentListCtrl.class);
	 ...
	 * @param event An event sent to the event handler of the component.
	 */
	public void onClick$button_CustomerDocumentList_CustomerDocumentSearchDialog(Event event) {
		search();
	}
	...
	public void onCustomerDocumentItemDoubleClicked(Event event) {
		...
		if (doCheckAuthority(customerDocument, whereCond, new Object[] { customerDocument.getCustID() })) {
	 ...
	 * 
	 * @param customerDocument The entity that need to be passed to the dialog.
	 */
