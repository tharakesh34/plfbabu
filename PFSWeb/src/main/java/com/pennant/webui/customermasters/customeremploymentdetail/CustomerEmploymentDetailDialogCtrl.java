...
public class CustomerEmploymentDetailDialogCtrl extends GFCBaseCtrl<CustomerEmploymentDetail> {
	...
	public void onCreate$window_CustomerEmploymentDetailDialog(Event event) throws Exception {
		...
		doLoadWorkFlow(this.customerEmploymentDetail.isWorkflow(), this.customerEmploymentDetail.getWorkflowId(),
				this.customerEmploymentDetail.getNextTaskId());
		...
		if (arguments.containsKey("customerEmploymentDetailListCtrl")) {
			setCustomerEmploymentDetailListCtrl(
					(CustomerEmploymentDetailListCtrl) arguments.get("customerEmploymentDetailListCtrl"));
		} else {
	...
	public void doWriteBeanToComponents(CustomerEmploymentDetail aCustomerEmploymentDetail) {
		...
		this.custCIF.setValue(aCustomerEmploymentDetail.getLovDescCustCIF() == null ? ""
				: aCustomerEmploymentDetail.getLovDescCustCIF().trim());
		this.custShrtName.setValue(aCustomerEmploymentDetail.getLovDescCustShrtName() == null ? ""
				: aCustomerEmploymentDetail.getLovDescCustShrtName().trim());
	...
	private void doSetValidation() {
		...
		if (!this.custID.isReadonly()) {
			this.custCIF.setConstraint(new PTStringValidator(
					Labels.getLabel("label_CustomerEmploymentDetailDialog_CustID.value"), null, true));
		}
		if (this.custEmpName.isButtonVisible()) {
			this.custEmpName.setConstraint(new PTStringValidator(
					Labels.getLabel("label_CustomerEmploymentDetailDialog_CustEmpName.value"), null, true, true));
		}
		/*
		 * if (this.custEmpName.isButtonVisible()){ this.custEmpName.setConstraint(new
		 * PTStringValidator(Labels.getLabel("label_CustomerEmploymentDetailDialog_CustEmpName.value"),
		 * PennantRegularExpressions.REGEX_ALPHANUM_SPACE, true)); }
		 */
		if (!this.companyName.isReadonly()) {
			this.companyName.setConstraint(
					new PTStringValidator(Labels.getLabel("label_CustomerEmploymentDetailDialog_CompanyName.value"),
			...
			if (isNewCustomer()) {
				if (isNewRecord()) {
					...
					if (workflow && !isFinanceProcess
							&& StringUtils.isBlank(aCustomerEmploymentDetail.getRecordType())) {
						aCustomerEmploymentDetail.setNewRecord(true);
					}
	...
	private AuditHeader newCusomerProcess(CustomerEmploymentDetail aCustomerRating, String tranType) {
		...
		if (getCustomerDialogCtrl().getCustomerEmploymentDetailList() != null
				&& getCustomerDialogCtrl().getCustomerEmploymentDetailList().size() > 0) {
					...
					if (PennantConstants.TRAN_DEL.equals(tranType)) {
						if (aCustomerRating.getRecordType().equals(PennantConstants.RECORD_TYPE_UPD)) {
							...
							for (int j = 0; j < getCustomerDialogCtrl().getCustomerDetails().getRatingsList()
									.size(); j++) {
								CustomerEmploymentDetail rating = getCustomerDialogCtrl().getCustomerDetails()
										.getEmploymentDetailsList().get(j);
								if (rating.getCustID() == aCustomerRating.getCustID()
										&& rating.getCustEmpName() == aCustomerRating.getCustEmpName()) {
	...
	private boolean doSaveProcess(AuditHeader auditHeader, String method) {
		...
		CustomerEmploymentDetail aCustomerEmploymentDetail = (CustomerEmploymentDetail) auditHeader.getAuditDetail()
				.getModelData();
			...
			while (retValue == PennantConstants.porcessOVERIDE) {
				if (StringUtils.isBlank(method)) {
					...
					if (StringUtils.trimToEmpty(method).equalsIgnoreCase(PennantConstants.method_doApprove)) {
						...
						if (aCustomerEmploymentDetail.getRecordType().equals(PennantConstants.RECORD_TYPE_NEW)) {
							deleteNotes = true;
						}
					} else {
						auditHeader.setErrorDetails(new ErrorDetail(PennantConstants.ERR_9999,
								Labels.getLabel("InvalidWorkFlowMethod"), null));
						retValue = ErrorControl.showErrorControl(this.window_CustomerEmploymentDetailDialog,
								auditHeader);
						logger.debug("Leaving");
						return processCompleted;
					}
	...
	private AuditHeader getAuditHeader(CustomerEmploymentDetail aCustomerEmploymentDetail, String tranType) {
		...
		return new AuditHeader(String.valueOf(aCustomerEmploymentDetail.getId()),
				...
				aCustomerEmploymentDetail.getUserDetails(), getOverideMap());
	}
