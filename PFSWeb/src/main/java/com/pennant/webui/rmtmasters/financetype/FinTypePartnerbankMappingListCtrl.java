...
public class FinTypePartnerbankMappingListCtrl extends GFCBaseListCtrl<FinTypePartnerBank> {
	...
	protected void doAddFilters() {
		...
		if (ImplementationConstants.PARTNERBANK_MAPPING_BRANCH_OR_CLUSTER.equals("B")) {
			if (StringUtils.isNotEmpty(this.branchOrCluster.getId())
					&& !this.branchOrCluster.getId().equals("branchOrCluster")) {
			...
			if (StringUtils.isNotEmpty(this.branchOrCluster.getId())
					&& !this.branchOrCluster.getId().equals("branchOrCluster")) {
				this.searchObject.addFilterEqual("CLUSTERID", Long.valueOf(this.branchOrCluster.getId()));
			} else {
	...
	public void doSetFieldProperties() {
		...
		if (ImplementationConstants.PARTNERBANK_MAPPING_BRANCH_OR_CLUSTER.equals("B")) {
			...
			filter[0] = new Filter("CLUSTERTYPE", ImplementationConstants.PARTNERBANK_MAPPING_CLUSTER, Filter.OP_EQUAL);
	...
	public void onFinTypePartnerBankMappingItemDoubleClicked(ForwardEvent event) throws InterruptedException {
		...
		afintypePartnerBank = finTypePartnerBankService.getFinTypePartnerBank(afintypePartnerBank.getFinType(),
