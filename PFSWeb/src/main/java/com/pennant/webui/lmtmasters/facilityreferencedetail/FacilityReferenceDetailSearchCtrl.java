...
public class FacilityReferenceDetailSearchCtrl extends GFCBaseCtrl<FacilityReferenceDetail> {
	...
	private static final Logger logger = LogManager.getLogger(FacilityReferenceDetailSearchCtrl.class);
	...
	public void onCreate$window_FacilityReferenceDetailSearch(Event event) throws Exception {
		...
		if (arguments.containsKey("facilityReferenceDetailCtrl")) {
			this.facilityReferenceDetailCtrl = (FacilityReferenceDetailListCtrl) arguments
					.get("facilityReferenceDetailCtrl");
		} else {
		...
		this.sortOperator_finRefDetailId
	...
	public void doSearch() {
		...
		final JdbcSearchObject<FacilityReferenceDetail> so = new JdbcSearchObject<FacilityReferenceDetail>(
				FacilityReferenceDetail.class);
		...
		if (StringUtils.isNotEmpty(this.finRefDetailId.getValue())) {
			...
			if (itemFinRefDetailId != null) {
				final int searchOpId = ((SearchOperators) itemFinRefDetailId.getAttribute("data"))
						.getSearchOperatorId();

				if (searchOpId == Filter.OP_LIKE) {
					so.addFilter(new Filter("finRefDetailId", "%" + this.finRefDetailId.getValue().toUpperCase() + "%",
							searchOpId));
		...
		if (StringUtils.isNotEmpty(this.finRefType.getValue())) {
			...
			if (itemFinRefType != null) {
				...
				if (searchOpId == Filter.OP_LIKE) {
					so.addFilter(
							new Filter("finRefType", "%" + this.finRefType.getValue().toUpperCase() + "%", searchOpId));
				} else if (searchOpId == -1) {
					// do nothing
				} else {
		...
		if (StringUtils.isNotEmpty(this.finRefId.getValue())) {
			...
			if (itemFinRefId != null) {
				...
				if (searchOpId == Filter.OP_LIKE) {
					so.addFilter(
							new Filter("finRefId", "%" + this.finRefId.getValue().toUpperCase() + "%", searchOpId));
				} else if (searchOpId == -1) {
					// do nothing
				} else {
		...
		if (StringUtils.isNotEmpty(this.showInStage.getValue())) {
			...
			if (itemShowInStage != null) {
				...
				if (searchOpId == Filter.OP_LIKE) {
					so.addFilter(new Filter("showInStage", "%" + this.showInStage.getValue().toUpperCase() + "%",
							searchOpId));
				} else if (searchOpId == -1) {
					// do nothing
				} else {
		...
		if (StringUtils.isNotEmpty(this.mandInputInStage.getValue())) {
			...
			if (itemMandInputInStage != null) {
				final int searchOpId = ((SearchOperators) itemMandInputInStage.getAttribute("data"))
						.getSearchOperatorId();

				if (searchOpId == Filter.OP_LIKE) {
					so.addFilter(new Filter("mandInputInStage",
							"%" + this.mandInputInStage.getValue().toUpperCase() + "%", searchOpId));
				} else if (searchOpId == -1) {
					// do nothing
				} else {
		...
		if (StringUtils.isNotEmpty(this.allowInputInStage.getValue())) {
			...
			if (itemAllowInputInStage != null) {
				final int searchOpId = ((SearchOperators) itemAllowInputInStage.getAttribute("data"))
						.getSearchOperatorId();

				if (searchOpId == Filter.OP_LIKE) {
					so.addFilter(new Filter("allowInputInStage",
							"%" + this.allowInputInStage.getValue().toUpperCase() + "%", searchOpId));
				} else if (searchOpId == -1) {
					// do nothing
				} else {
		...
		this.label_FacilityReferenceDetailSearchResult
				.setValue(Labels.getLabel("label_FacilityReferenceDetailSearchResult.value") + " "
