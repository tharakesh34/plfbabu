...
public class CERSAIFileDownloadListctrl extends GFCBaseListCtrl<FileDownlaod> implements Serializable {
	...
	public void onClick$btnexecute(Event event) throws Exception {
		...
		try {
			if (CersaiConstants.CERSAI_ADD.equals(downloadType)) {
				cersaiDownloadProcess.generateAddReport();
			} else if (CersaiConstants.CERSAI_MODIFY.equals(downloadType)) {
				cersaiDownloadProcess.generateModifyReport();
			} else if (CersaiConstants.CERSAI_SATISFY.equals(downloadType)) {
