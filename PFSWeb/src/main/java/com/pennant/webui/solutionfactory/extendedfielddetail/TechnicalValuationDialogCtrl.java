...
public class TechnicalValuationDialogCtrl extends GFCBaseCtrl<ExtendedFieldDetail> {
	...
	private static final Logger logger = LogManager.getLogger(TechnicalValuationDialogCtrl.class);
	...
	public void onCreate$window_TechValuationFieldDialog(Event event) throws Exception {
		...
		setPageComponents(window_TechValuationFieldDialog);
		try {
			...
			window_TechValuationFieldDialog.onClose();
		}
	...
	public void onClick$btnHelp(Event event) throws InterruptedException {
		logger.debug("Entering" + event.toString());
		MessageUtil.showHelpWindow(event, window_TechValuationFieldDialog);
		logger.debug("Leaving" + event.toString());
	}
	...
	public void doWriteComponentsToBean(ExtendedFieldHeader aExtendedFieldHeader, Tab tab) {
		...
		aExtendedFieldHeader.setTechnicalValuationDetailList(getTechValuationFieldDetailsList());
		doRemoveValidation();
	...
	public void doShowDialog(ExtendedFieldHeader aExtendedFieldHeader) throws Exception {
		...
		try {
			...
			if (parentTabPanel != null) {
				...
				this.groupboxWf.setVisible(false);
				this.window_TechValuationFieldDialog.setHeight(borderLayoutHeight - 75 + "px");
				parentTabPanel.appendChild(this.window_TechValuationFieldDialog);
			} else {
				setDialog(DialogType.EMBEDDED);
			}
		} catch (UiException e) {
			logger.error("Exception: ", e);
			this.window_TechValuationFieldDialog.onClose();
		} catch (Exception e) {
			throw e;
		}
		logger.debug("Leaving");
	...
	private void showMessage(Exception e) {
		...
		try {
			...
			ErrorControl.showErrorControl(this.window_TechValuationFieldDialog, auditHeader);
		} catch (Exception exp) {
	...
	public void onClick$btnNew_FieldDet(Event event) throws Exception {
		...
		try {
			Executions.createComponents(
					...
					window_TechValuationFieldDialog, map);
		} catch (Exception e) {
			MessageUtil.showError(e);
		}
		logger.debug("Leaving" + event.toString());
	 ...
	 */
	public void doFillFieldsList(List<ExtendedFieldDetail> techValuationFieldDetailsList) {
		logger.debug("Entering");
		if (techValuationFieldDetailsList == null) {
			this.techValuationFieldDetailsList = new ArrayList<ExtendedFieldDetail>();
		} else {
		...
		for (int i = 0; i < getTechValuationFieldDetailsList().size(); i++) {

			if (maxSeqNo < techValuationFieldDetailsList.get(i).getFieldSeqOrder()) {
				maxSeqNo = techValuationFieldDetailsList.get(i).getFieldSeqOrder();
			}

			if (!StringUtils.equals(getTechValuationFieldDetailsList().get(i).getRecordType(),
					PennantConstants.RECORD_TYPE_DEL)
					&& !StringUtils.equals(getTechValuationFieldDetailsList().get(i).getRecordType(),
	...
	public class ExtendedFieldListItemRenderer implements ListitemRenderer<ExtendedFieldDetail>, Serializable {
		...
		public void render(Listitem item, ExtendedFieldDetail detail, int count) throws Exception {
			...
			ComponentsCtrl.applyForward(item, "onDoubleClick=onTechnicalValuationItemDoubleClicked");
		}
	}

	public List<ExtendedFieldDetail> doSave_TechnicalValuationFields(Tab tab) {
		logger.debug("Entering");
		...
		logger.debug("Leaving");
		return extendedFieldHeader.getTechnicalValuationDetailList();
	}
	...
	public List<ExtendedFieldDetail> getTechValuationFieldDetailsList() {
		return techValuationFieldDetailsList;
	}
