 ...
 * FILE HEADER *
 ********************************************************************************************
 * * FileName : SecurityGroupListCtrl.java * * Author : PENNANT TECHONOLOGIES * * Creation Date : 27-05-2011 * *
...
public class SecurityGroupListCtrl extends GFCBaseListCtrl<SecurityGroup> {
	...
	private static final Logger logger = LogManager.getLogger(SecurityGroupListCtrl.class);
	...
	public void search() {
		this.searchObject.addWhereClause(
				" GrpID not in (Select sgr.GrpID From SecRoleGroups srg Inner Join SecGroupRights sgr on sgr.GrpID = srg.GrpID Inner Join SecRights sr on sr.RightID = sgr.RightID and sr.RightType = 0)");
		super.search();
	}
