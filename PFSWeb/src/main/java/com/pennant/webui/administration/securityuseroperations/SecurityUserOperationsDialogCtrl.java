...
public class SecurityUserOperationsDialogCtrl extends GFCBaseCtrl<SecurityOperation> {
	...
	private static final Logger logger = LogManager.getLogger(SecurityUserOperationsDialogCtrl.class);
	...
	public void onCreate$win_SecurityUserOperationsDialog(Event event) {
		...
		try {
			...
			ModuleMapping moduleMapping = PennantJavaUtil.getModuleMap("SecurityUserOperations");
	...
	public void onClick$btnHelp(Event event) throws InterruptedException {
		logger.debug("Entering" + event.toString());
		MessageUtil.showHelpWindow(event, this.win_SecurityUserOperationsDialog);
		logger.debug("Leaving" + event.toString());
	}
	...
	public void doShowDialog() {
		...
		for (SecurityUserOperations detail : getSecurityUser().getSecurityUserOperationsList()) {
		...
		List<SecurityOperation> securityOperations = getSecurityUserOperationsService().getApprovedOperations();
	...
	public void doReadOnly(boolean readOnly) {
		...
		if (isWorkFlowEnabled()) {
			accessType = getUserWorkspace().getAccessType("SecurityUserOperationsDialog_SelectRole");
		}
		...
		if ("Submitted".equals(getSecurityUser().getRecordStatus())
				|| ("Saved".equals(getSecurityUser().getRecordStatus())
						&& "MSTGRP1_APPROVER".equals(getSecurityUser().getNextRoleCode()))) {
	...
	public void doSetPanelProperties() {
		...
		this.panel_SecurityOperationRoles.setTitle(Labels.getLabel("panel_OperationRoles.title"));
		logger.debug("Leaving ");
	}
	...
	private void doCheckRights() {
		...
		this.btnCancel.setVisible(getUserWorkspace().isAllowed("button_SecurityUserOperationsDialog_btnCancel"));

		if (getUserWorkspace().isAllowed("SecurityUserOperationsDialog_SelectRole")) {
			this.btnSelectRoles.setVisible(true);
			this.btnUnSelectAllRoles.setVisible(true);
			this.btnUnSelectRoles.setVisible(true);
		} else {
	...
	public void doWriteBeanToComponents(SecurityUser aSecurityUser) {
		...
		refreshListBox(getAssigneListWrapper(), new ArrayList<SecurityUserOperations>(assignedHashMap.values()),
	 ...
	 * in new selectedMap creates new SecurityUserOperations Object, sets data and add it to SecurityUser
	 * LovDescAssignedOperations b)if roleId in oldselectedMap and not in new selectedMap gets the
	 * SecurityUserOperations from back end , sets RecordStatus "DELETE" add it to SecurityUser
	 * LovDescAssignedOperations
	 */
	public void doWriteComponentsToBean(SecurityUser aSecurityUser) {
		...
		if (addHashMap != null && !addHashMap.isEmpty()) {
			...
			Iterator<SecurityUserOperations> addList = addHashMap.values().iterator();
		...
		if (delHashMap != null && !delHashMap.isEmpty()) {
			...
			Iterator<SecurityUserOperations> delList = delHashMap.values().iterator();
		...
		if (cancilHashMap != null && !cancilHashMap.isEmpty()) {
			...
			Iterator<SecurityUserOperations> cancilList = cancilHashMap.values().iterator();

			while (cancilList.hasNext()) {
				SecurityUserOperations securityUserOperations = cancilList.next();
	...
	public void onClick$btn_SearchOperations(Event event) {
		...
		try {
			Executions.createComponents("/WEB-INF/pages/Administration/SecuritySearchDialog.zul",
					this.win_SecurityUserOperationsDialog, map);
		} catch (Exception e) {
			MessageUtil.showError(e);
		}
	...
	private void setAssignedRoles() {
		...
		if (assignedHashMap.size() > 0) {
			getAssigneListWrapper().initList(new ArrayList<SecurityUserOperations>(assignedHashMap.values()),
					this.listbox_AssignedRoles, new Paging(assignedHashMap.size(), assignedHashMap.size()));
			this.listbox_AssignedRoles.setItemRenderer(new SecurityUserOperationListModelItemRenderer());
		} else {
			getAssigneListWrapper().initList(new ArrayList<SecurityUserOperations>(assignedHashMap.values()),
		...
		if (unAssignedHashMap.size() > 0) {
			getUnAssigneListWrapper().initList(new ArrayList<SecurityUserOperations>(unAssignedHashMap.values()),
					this.listbox_UnAssignedRoles, new Paging(unAssignedHashMap.size(), unAssignedHashMap.size()));
			this.listbox_UnAssignedRoles.setItemRenderer(new SecurityUserOperationListModelItemRenderer());
		} else {
			getUnAssigneListWrapper().initList(new ArrayList<SecurityUserOperations>(unAssignedHashMap.values()),
	...
	private void setUnAssignedRoles() {
		...
		if (unAssignedHashMap.size() > 0) {
			getUnAssigneListWrapper().initList(new ArrayList<SecurityUserOperations>(unAssignedHashMap.values()),
					this.listbox_UnAssignedRoles, new Paging(unAssignedHashMap.size(), unAssignedHashMap.size()));
			this.listbox_UnAssignedRoles.setItemRenderer(new SecurityOperationListModelItemRenderer());
		} else {
			getUnAssigneListWrapper().initList(new ArrayList<SecurityUserOperations>(unAssignedHashMap.values()),
					this.listbox_UnAssignedRoles, new Paging());
			this.listbox_UnAssignedRoles.setItemRenderer(new SecurityOperationListModelItemRenderer());
	...
	public void doShowRoleGroups(ForwardEvent event) {
		...
		if (item != null) {
			...
			this.panel_SecurityRoleGroups.setTitle(Labels.getLabel("listbox_SecurityUserRoleGroups.value") + " - "
					+ aSecurityOperationRoles.getLovDescOprCode());
			this.listbox_SecurityRoleGroups.setVisible(true);
			this.panel_SecurityRoleGroups.setTitle(Labels.getLabel("listbox_SecurityUserOperationRole.value") + "  :  "
					+ aSecurityOperationRoles.getLovDescRoleDesc());

			List<SecurityRoleGroups> roleGroupsList = getSecurityUserOperationsService()
			...
			getRoleGroupsListWrapper().initList(new ArrayList<SecurityRoleGroups>(roleGroupsList),
					this.listbox_SecurityRoleGroups, new Paging());
	...
	public void doShowOperationRoles(ForwardEvent event) {
		...
		if (item != null) {
			SecurityUserOperations aSecurityUserOperations = (SecurityUserOperations) item.getAttribute("data");
			this.panel_SecurityOperationRoles.setTitle(Labels.getLabel("listbox_SecurityUserOperationRole.value")
			...
			getOperationRolesListWrapper().initList(new ArrayList<SecurityOperationRoles>(oprRolesList),
					this.listbox_SecurityOperationRoles, new Paging());
			this.listbox_SecurityOperationRoles.setItemRenderer(new OperationRolesListModelItemRenderer());
		}
		logger.debug("Leaving ");
	}
	 ...
	 */
	public class SecurityOperationListModelItemRenderer
			implements ListitemRenderer<SecurityUserOperations>, Serializable {
		...
		public void render(Listitem item, SecurityUserOperations userOperations, int count) {
			...
			item.addForward("onDoubleClick", win_SecurityUserOperationsDialog, "onSecurityOprRoleItemDoubleClicked",
					item);
		}
	}
	 ...
	 * @param SecurityUserOperations
	 */
	public class SecurityUserOperationListModelItemRenderer
			implements ListitemRenderer<SecurityUserOperations>, Serializable {
	 ...
	 * @param SecurityUserOperations
	 */
	public class SecurityOprRoleListModelItemRenderer implements ListitemRenderer<SecurityRoleGroups>, Serializable {
	 ...
	 * @param SecurityUserOperations
	 */
	private void refreshListBox(PagedListWrapper<SecurityUserOperations> listWrapper,
			List<SecurityUserOperations> userOperations, Listbox listBox) {
		logger.debug("Entering");
		Paging paging = new Paging();
	...
	private AuditHeader getAuditHeader(SecurityUser aSecurityUser, String tranType) {
		...
		AuditHeader auditHeader = new AuditHeader(String.valueOf(aSecurityUser.getId()), null, null, null, auditDetail,
	...
	/*
	 * private Notes getNotes(){ Notes notes = new Notes(); notes.setModuleName("EmirateDetail");
	 * notes.setReference(getEmirateDetail().getEmirateId()); notes.setVersion(getEmirateDetail().getVersion()); return
	 * notes; }
	 */
	...
	private void fillListBoxWithData(List<SecurityUserOperations> userOperations, Listbox listbox) {
		...
		if (userOperations != null && !userOperations.isEmpty()) {
			for (SecurityUserOperations securityUserOperations : userOperations) {
				...
				item.addForward("onDoubleClick", win_SecurityUserOperationsDialog, "onSecurityRoleItemDoubleClicked",
						item);
	...
	public void setOperationRolesListWrapper(PagedListWrapper<SecurityOperationRoles> operationRolesListWrapper) {
	...
	public void setSecurityUserOperations(SecurityUserOperations securityUserOperations) {
