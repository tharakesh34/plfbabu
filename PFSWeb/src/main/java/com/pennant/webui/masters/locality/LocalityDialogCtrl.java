 ...
 ********************************************************************************************
 *																							*
 * FileName    		:  LocalityDialogCtrl.java                                                   * 	  
 *                                                                    						*
...
public class LocalityDialogCtrl extends GFCBaseCtrl<Locality> {
	...
	protected String getReference() {
		StringBuffer referenceBuffer = new StringBuffer(String.valueOf(this.locality.getId()));
	 ...
	 * @throws Exception
	 */
	public void onCreate$window_LocalityDialog(Event event) throws Exception {
	...
	/**
	 * Deletes a Locality object from database.<br>
	 * 
	...
	private void doDelete() throws InterruptedException {
		...
		if (MessageUtil.confirm(msg) == MessageUtil.YES) {
			if (StringUtils.trimToEmpty(aLocality.getRecordType()).equals("")) {
				aLocality.setVersion(aLocality.getVersion() + 1);
				aLocality.setRecordType(PennantConstants.RECORD_TYPE_DEL);

				if (isWorkFlowEnabled()) {
					aLocality.setRecordStatus(userAction.getSelectedItem().getValue().toString());
					aLocality.setNewRecord(true);
					tranType = PennantConstants.TRAN_WF;
					getWorkFlowDetails(userAction.getSelectedItem().getLabel(), aLocality.getNextTaskId(), aLocality);
				} else {
