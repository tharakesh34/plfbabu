...
public class SelectAccountDetailsDialogCtrl extends GFCBaseCtrl<Accounts> {
	...
	private static final Logger logger = LogManager.getLogger(SelectAccountDetailsDialogCtrl.class);
	...
	public void onClick$btnSearchBranch(Event event) {
		...
		if (dataObject instanceof String) {
			...
			if (details != null) {
				...
				this.lovDescBranchCodeName.setValue(details.getBranchCode() + "-" + details.getBranchDesc());
			}
		}
		logger.debug("Leaving" + event.toString());
	}
	...
	private void doSetLOVValidation() {
		...
		this.lovDescAccType.setConstraint(
				new PTStringValidator(Labels.getLabel("label_SelectAccountDetailsDialog_AccType.value"), null, true));

		this.lovDescCurrency.setConstraint(
				new PTStringValidator(Labels.getLabel("label_SelectAccountDetailsDialog_Currency.value"), null, true));

		this.lovDescBranchCodeName.setConstraint(
				new PTStringValidator(Labels.getLabel("label_SelectAccountDetailsDialog_Branch.value"), null, true));
		if (!isInterAcc) {
			this.lovDescCustCIF.setConstraint(new PTStringValidator(
					Labels.getLabel("label_SelectAccountDetailsDialog_CustCIF.value"), null, true));
		}
		logger.debug("Leaving ");
	}
