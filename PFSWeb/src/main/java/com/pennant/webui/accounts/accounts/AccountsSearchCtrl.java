 ...
 * FILE HEADER *
 ********************************************************************************************
 * * FileName : AccountsSearchCtrl.java * * Author : PENNANT TECHONOLOGIES * * Creation Date : 02-01-2012 * * Modified
...
public class AccountsSearchCtrl extends GFCBaseCtrl<Accounts> {
	...
	public void onCreate$window_AcountsSearch(Event event) {
		...
		if (arguments.containsKey("searchObject")) {
			final JdbcSearchObject<Accounts> searchObj = (JdbcSearchObject<Accounts>) arguments.get("searchObject");
	 ...
	 */
	private void showAcountsSeekDialog() {
		logger.debug("Entering ");
		try {
	...
	public void doSearch() {
		...
		if (StringUtils.isNotEmpty(this.accountId.getValue())) {
			...
			if (itemAccountId != null) {
				...
				if (searchOpId == Filter.OP_LIKE) {
					so.addFilter(
							new Filter("accountId", "%" + this.accountId.getValue().toUpperCase() + "%", searchOpId));
		...
		if (StringUtils.isNotEmpty(this.acBranch.getValue())) {
			...
			if (itemAcBranch != null) {
				...
				if (searchOpId == Filter.OP_LIKE) {
					so.addFilter(
							new Filter("acBranch", "%" + this.acBranch.getValue().toUpperCase() + "%", searchOpId));
				} else if (searchOpId == -1) {
					// do nothing
				} else {
		...
		if (this.acCustCIF.getValue() != null) {
			...
			if (itemAcCustId != null) {
				...
				if (searchOpId == -1) {
					...
					if (this.acCustCIF.getValue() != null) {
						so.addFilter(new Filter("lovDescCustCIF", "%" + this.acCustCIF.getValue().toUpperCase() + "%",
								searchOpId));
					} else {
		...
		if (StringUtils.isNotEmpty(this.acFullName.getValue())) {
			...
			if (itemAcFullName != null) {
				...
				if (searchOpId == Filter.OP_LIKE) {
					so.addFilter(
							new Filter("acFullName", "%" + this.acFullName.getValue().toUpperCase() + "%", searchOpId));
				} else if (searchOpId == -1) {
					// do nothing
				} else {
		...
		if (StringUtils.isNotEmpty(this.acShortName.getValue())) {
			...
			if (itemAcShortName != null) {
				...
				if (searchOpId == Filter.OP_LIKE) {
					so.addFilter(new Filter("acShortName", "%" + this.acShortName.getValue().toUpperCase() + "%",
							searchOpId));
				} else if (searchOpId == -1) {
					// do nothing
				} else {
		...
		if ((this.acPurpose.getSelectedItem().getValue() != null) && (!StringUtils
				.equals(this.acPurpose.getSelectedItem().getLabel(), Labels.getLabel("common.Select")))) {
			// get the search operator
			...
			if (itemAcPurpose != null) {
				...
				if (searchOpId == -1) {
					...
					if (searchOpId == Filter.OP_LIKE) {
						so.addFilter(new Filter("acPurpose", "%" + this.acPurpose.getSelectedItem().getValue() + "%",
								searchOpId));
					} else if (searchOpId == -1) {
						// do nothing
					} else {
		...
		if (this.acLastCustTrnDate.getValue() != null) {
			...
			if (itemAcLastCustTrnDate != null) {
				final int searchOpId = ((SearchOperators) itemAcLastCustTrnDate.getAttribute("data"))
						.getSearchOperatorId();
		...
		if (this.acLastSysTrnDate.getValue() != null) {
			...
			if (itemAcLastSysTrnDate != null) {
				final int searchOpId = ((SearchOperators) itemAcLastSysTrnDate.getAttribute("data"))
						.getSearchOperatorId();
		...
		if (StringUtils.isNotEmpty(this.hostAcNumber.getValue())) {
			...
			if (itemHostAcNumber != null) {
				...
				if (searchOpId == Filter.OP_LIKE) {
					so.addFilter(new Filter("hostAcNumber", "%" + this.hostAcNumber.getValue().toUpperCase() + "%",
							searchOpId));
				} else if (searchOpId == -1) {
					// do nothing
				} else {
		...
		this.acountsCtrl.setSearchObj(so);
		this.label_AcountsSearchResult
				.setValue(Labels.getLabel("label_AcountsSearchResult.value") + " " + paging.getTotalSize());
		logger.debug("Leaving ");
	}
