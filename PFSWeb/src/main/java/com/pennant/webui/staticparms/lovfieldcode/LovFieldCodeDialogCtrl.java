...
public class LovFieldCodeDialogCtrl extends GFCBaseCtrl<LovFieldCode> {
	...
	private static final Logger logger = LogManager.getLogger(LovFieldCodeDialogCtrl.class);
	...
	public void onCreate$window_LovFieldCodeDialog(Event event) {
		...
		try {
			...
			doLoadWorkFlow(this.lovFieldCode.isWorkflow(), this.lovFieldCode.getWorkflowId(),
					this.lovFieldCode.getNextTaskId());

			if (isWorkFlowEnabled()) {
				this.userAction = setListRecordStatus(this.userAction);
				getUserWorkspace().allocateRoleAuthorities(getRole(), "LovFieldCodeDialog");
			}
			...
			if (arguments.containsKey("lovFieldCodeListCtrl")) {
				setLovFieldCodeListCtrl((LovFieldCodeListCtrl) arguments.get("lovFieldCodeListCtrl"));
			} else {
	...
	public void doWriteBeanToComponents(LovFieldCode aLovFieldCode) {
		...
		if (aLovFieldCode.isNewRecord()
				|| StringUtils.equals(aLovFieldCode.getRecordType(), PennantConstants.RECORD_TYPE_NEW)) {
	...
	private void doSetValidation() {
		...
		if (!this.fieldCode.isReadonly()) {
			this.fieldCode
					.setConstraint(new PTStringValidator(Labels.getLabel("label_LovFieldCodeDialog_FieldCode.value"),
							PennantRegularExpressions.REGEX_UPP_BOX_ALPHANUM, true));
		}
		if (!this.fieldCodeDesc.isReadonly()) {
			this.fieldCodeDesc.setConstraint(
					new PTStringValidator(Labels.getLabel("label_LovFieldCodeDialog_FieldCodeDesc.value"),
							PennantRegularExpressions.REGEX_DESCRIPTION, true));
		}
		if (!this.fieldCodeType.isDisabled()) {
			this.fieldCodeType.setConstraint(new StaticListValidator(listFieldCodeType,
					Labels.getLabel("label_LovFieldCodeDialog_FieldCodeType.value")));
		}
		logger.debug("Leaving");
	}
	...
	public void doSave() throws InterruptedException {
		...
		isNew = aLovFieldCode.isNewRecord();
		String tranType = "";
	 ...
	 * 
	 */
	protected boolean doProcess(LovFieldCode aLovFieldCode, String tranType) {
		logger.debug("Entering");
	...
	private AuditHeader getAuditHeader(LovFieldCode aLovFieldCode, String tranType) {
		...
		AuditDetail auditDetail = new AuditDetail(tranType, 1, aLovFieldCode.getBefImage(), aLovFieldCode);
		return new AuditHeader(aLovFieldCode.getFieldCode(), null, null, null, auditDetail,
				aLovFieldCode.getUserDetails(), getOverideMap());
	}
