...
/**
 * This is the controller class for the /WEB-INF/pages/Application Masters/SukukBroker/sukukBrokerDialog.zul file.
 */
public class SukukBrokerDialogCtrl extends GFCBaseCtrl<SukukBroker> {
	private static final long serialVersionUID = 1L;
	private static final Logger logger = LogManager.getLogger(SukukBrokerDialogCtrl.class);
	 ...
	 * selected SukukBroker object in a Map.
	 * 
	 * @param event
	 * @throws Exception
	 */
	public void onCreate$window_SukukBrokerDialog(Event event) throws Exception {
		...
		try {
			...
			doLoadWorkFlow(this.sukukBroker.isWorkflow(), this.sukukBroker.getWorkflowId(),
					this.sukukBroker.getNextTaskId());
	...
	private void doCheckRights() {
		...
		if (!enqModule) {
			...
			this.button_SukukBrokerBondsList_NewSukukBrokerBonds
					.setVisible(getUserWorkspace().isAllowed("button_SukukBrokerDialog_btnNew"));
	...
	public void doWriteComponentsToBean(SukukBroker aSukukBroker) {
		...
		try {
			if (this.listBoxSukukBrokerBonds.getItemCount() <= 0) {
				throw new WrongValueException(this.button_SukukBrokerBondsList_NewSukukBrokerBonds, Labels.getLabel(
						"FIELD_NO_EMPTY", new String[] { Labels.getLabel("window_SukukBrokerDialog.title") }));
			}
	...
	private void doSetValidation() {
		...
		if (!this.brokerCode.isReadonly()) {
			this.brokerCode.setConstraint(new PTStringValidator(Labels.getLabel("label_BrokerDialog_BrokerCode.value"),
					PennantRegularExpressions.REGEX_UPPERCASENAME, true));
		}
		//Description
		if (!this.brokerDesc.isReadonly()) {
			this.brokerDesc.setConstraint(new PTStringValidator(Labels.getLabel("label_BrokerDialog_BrokerDesc.value"),
		...
		if (MessageUtil.confirm(msg) == MessageUtil.YES) {
			if (StringUtils.isBlank(aSukukBroker.getRecordType())) {
				...
				if (isWorkFlowEnabled()) {
					...
					getWorkFlowDetails(userAction.getSelectedItem().getLabel(), aSukukBroker.getNextTaskId(),
							aSukukBroker);
				} else {
		...
		try {
			...
			while (retValue == PennantConstants.porcessOVERIDE) {
				...
				if (retValue == PennantConstants.porcessCONTINUE) {
					...
					if (deleteNotes) {
						deleteNotes(getNotes("SukukBroker", aSukukBroker.getBrokerCode(), aSukukBroker.getVersion()),
								true);
					}
				}
	...
	private AuditHeader getAuditHeader(SukukBroker aSukukBroker, String tranType) {
		...
		return new AuditHeader(aSukukBroker.getBrokerCode(), null, null, null, auditDetail,
				aSukukBroker.getUserDetails(), getOverideMap());
	}
	...
	public void onClick$button_SukukBrokerBondsList_NewSukukBrokerBonds(Event event) throws Exception {
		...
		try {
			Executions.createComponents("/WEB-INF/pages/ApplicationMaster/SukukBrokerBonds/SukukBrokerBondsDialog.zul",
					null, map);
		} catch (Exception e) {
			MessageUtil.showError(e);
		}
	...
	public void doFilllistbox(List<SukukBrokerBonds> transactionEntryList) {
		...
		if (transactionEntryList != null) {
			...
			getSukukBrokerBondsPagedListWrapper().initList(transactionEntryList, this.listBoxSukukBrokerBonds,
					this.pagingSukukBrokerBondsList);
	...
	public void setSukukBrokerBondsPagedListWrapper() {
		if (this.sukukBrokerBondsPagedListWrapper == null) {
			this.sukukBrokerBondsPagedListWrapper = (PagedListWrapper<SukukBrokerBonds>) SpringUtil
