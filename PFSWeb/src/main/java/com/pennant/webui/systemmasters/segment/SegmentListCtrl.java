 ...
 * FILE HEADER *
 ********************************************************************************************
 * * FileName : SegmentListCtrl.java * * Author : PENNANT TECHONOLOGIES * * Creation Date : 03-05-2011 * * Modified Date
...
public class SegmentListCtrl extends GFCBaseListCtrl<Segment> {
	...
	private static final Logger logger = LogManager.getLogger(SegmentListCtrl.class);
	...
	public void onSegmentItemDoubleClicked(Event event) {
		...
		String whereCond = " where SegmentCode=?";

		if (doCheckAuthority(segment, whereCond, new Object[] { segment.getSegmentCode() })) {
	 ...
	 * 
	 * @param segment The entity that need to be passed to the dialog.
	 */
