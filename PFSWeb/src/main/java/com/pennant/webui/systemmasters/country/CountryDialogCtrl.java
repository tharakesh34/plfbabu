...
public class CountryDialogCtrl extends GFCBaseCtrl<Country> {
	...
	protected void doSetProperties() {
		super.pageRightName = "CountryDialog";
	}
	...
	public void onCreate$window_CountryDialog(Event event) throws Exception {
		...
		setPageComponents(window_CountryDialog);
		...
		try {
			...
			if (arguments.containsKey("country")) {
				this.country = (Country) arguments.get("country");
	...
	public void onClick$btnHelp(Event event) throws InterruptedException {
		logger.debug("Entering" + event.toString());
		MessageUtil.showHelpWindow(event, window_CountryDialog);
		logger.debug("Leaving" + event.toString());
	}
	...
	private void doCancel() {
		logger.debug("Entering");
		doWriteBeanToComponents(this.country.getBefImage());
		doReadOnly();
	...
	public void doWriteComponentsToBean(Country aCountry) {
		...
		try {
			aCountry.setSystemDefault(this.systemDefault.isChecked());
		} catch (WrongValueException we) {
			wve.add(we);
		}
	...
	private void doEdit() {
		...
		this.systemDefault.setDisabled(isReadOnly("CountryDialog_systemDefault"));
		if (isWorkFlowEnabled()) {
		...
		if (isWorkFlowEnabled()) {
			...
			if ("Save".equals(userAction.getSelectedItem().getLabel())) {
				...
				if ("".equals(nextTaskId)) {
					nextTaskId = getNextTaskIds(taskId, aCountry);
				}

				if (isNotesMandatory(taskId, aCountry)) {
					if (!notesEntered) {
			...
			String operationRefs = getServiceOperations(taskId, aCountry);
		...
		try {
			while (retValue == PennantConstants.porcessOVERIDE) {
				...
				if (retValue == PennantConstants.porcessCONTINUE) {
					...
					if (deleteNotes) {
						deleteNotes(getNotes(this.country), true);
					}
				}
