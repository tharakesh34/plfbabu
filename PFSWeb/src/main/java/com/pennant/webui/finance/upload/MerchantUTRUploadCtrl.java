...
public class MerchantUTRUploadCtrl extends GFCBaseCtrl<Configuration> {
	...
	private com.pennanttech.pff.external.merchant.MerchantUTRProcess merchantUTRProcess;
	private com.pennanttech.pff.external.merchant.MerchantUTRProcess defaultMerchantUTRProcess;
	private DataEngineStatus MERCHANT_UTR_UPLOAD = new DataEngineStatus("MERCHANT_UTR_UPLOAD");

	/**
	 * default constructor.<br>
	 */
	public MerchantUTRUploadCtrl() {
		super();
	}

	@Override
	protected void doSetProperties() {
		super.pageRightName = "MerchantUTRUpload";
	}
	 ...
	 * @throws Exception
	 */
	public void onCreate$window_MerchantUTRUploadCtrl(Event event) throws Exception {
		...
		for (Configuration config : configList) {
			...
			if ("MERCHANT_UTR_UPLOAD".equals(configName)) {
				if ("MERCHANT_UTR_UPLOAD".equals(configName)) {
					MERCHANT_UTR_UPLOAD = dataEngineConfig.getLatestExecution("MERCHANT_UTR_UPLOAD");
					valueLabel = new ValueLabel(configName, "Merchant UTR Upload");
					doFillPanel(config, MERCHANT_UTR_UPLOAD);
	...
	public void onClick$btnImport(Event event) throws InterruptedException {
		...
		try {
			Thread thread;
			thread = new Thread(new ProcessData(userId, MERCHANT_UTR_UPLOAD));
			thread.start();
		} catch (Exception e) {
	...
	public class ProcessData implements Runnable {
		...
		public void run() {
			try {
				getMerchantUTRProcess().processMerchantUTRUpload(userId, file, media, status);
			} catch (Exception e) {
	...
	@Qualifier(value = "merchantUTRProcess")
	public void setMerchantUtrProcess(com.pennanttech.pff.external.merchant.MerchantUTRProcess merchantUTRProcess) {
		this.merchantUTRProcess = merchantUTRProcess;
	}

	@Autowired
	public void setDefaultmerchantUTRProcess(
			com.pennanttech.pff.external.merchant.MerchantUTRProcess defaultmerchantUTRProcess) {
		this.defaultMerchantUTRProcess = defaultmerchantUTRProcess;
	}

	private com.pennanttech.pff.external.merchant.MerchantUTRProcess getMerchantUTRProcess() {
		return merchantUTRProcess == null ? defaultMerchantUTRProcess : merchantUTRProcess;
	}
