...
public class ReceiptUploadHeaderListCtrl extends GFCBaseListCtrl<ReceiptUploadHeader> {
	...
	private Map<Long, String>						receiptIdMap		= new HashMap<Long, String>();
	...
	public void onCreate$window_ReceiptUploadList(Event event) {
		...
		setPageComponents(window_ReceiptUploadList, borderLayout_ReceiptUploadList, listBoxReceiptUpload,pagingReceiptUploadList);
	...
	private void doSetValidations() {
		...
		try {
			if (!this.entityCode.isReadonly())
				this.entityCode.setConstraint(new PTStringValidator(Labels.getLabel("label_ReceiptUploadList_EntityCode.value"), null, true,true));
	...
	public void onClick$button_ReceiptUploadList_NewReceiptUpload(Event event) {
		...
		try {
			Executions.createComponents("/WEB-INF/pages/Finance/ReceiptUpload/SelectReceiptUploadHeaderDialog.zul", null, arg);
		} catch (Exception e) {
	...
	public void onReceiptUploadItemDoubleClicked(Event event) {
		...
		ReceiptUploadHeader receiptUploadHeader = receiptUploadHeaderService.getUploadHeaderById(id);
	...
	public void onClick$btndownload(Event event) throws IOException{
		...
		List<ReceiptUploadHeader> listReceiptUploadHeader = doCheckAuthority(receiptidList);
		...
		if (receiptidList.size() > 1) {
			...
			for (long id : receiptidList) {
				ByteArrayOutputStream outputStream = null;
				outputStream = doDownloadFiles(String.valueOf(id));
				Filedownload.save(new AMedia(String.valueOf(id), "xls", "application/vnd.ms-excel", outputStream.toByteArray()));
				this.receiptUploadHeaderService.updateUploadProgress(id, PennantConstants.RECEIPT_DOWNLOADED);
			}
		}

		doRefresh();
		Clients.showNotification(Labels.getLabel("label_DataExtractionList_DownloadedSuccess.value"), "info", null, null, -1);
	...
	private List<ReceiptUploadHeader> doCheckAuthority(List<Long> receiptidList) {
		...
		for (long id : receiptidList) {
			...
			if (doCheckAuthority(receiptUploadHeader, whereCond)) {
				...
				doLoadWorkFlow(isWorkFlowEnabled(), receiptUploadHeader.getWorkflowId(),receiptUploadHeader.getNextTaskId());
			} else {
				MessageUtil.showMessage(Labels.getLabel("info.not_authorized"));
	...
	private boolean checkFileDownloaded(List<Long> listUploadId) {
		...
		for (long id : listUploadId) {
			...
			boolean isDownloaded = this.receiptUploadHeaderService.isFileDownloaded(id, PennantConstants.RECEIPT_DOWNLOADED);
	...
	private void doApprove(ReceiptUploadHeader aReceiptUploadHeader) {
		...
		Thread thread = new Thread(new ReceiptHeaderThread(aReceiptUploadHeader));
		thread.start();
		
		aReceiptUploadHeader.setUploadProgress(PennantConstants.RECEIPT_APPROVED);
		
		//call dosaveProgress
		
		doProcess(aReceiptUploadHeader, PennantConstants.TRAN_ADD, PennantConstants.RCD_STATUS_APPROVED);

	}

	private void ApiRequestMethod(ReceiptUploadHeader aReceiptUploadHeader) {
		logger.debug(Literal.ENTERING);
		...
		this.receiptUploadHeaderService.uploadHeaderStatusCnt(aReceiptUploadHeader.getUploadHeaderId(), sucessCount,
				failedCount);
	...
	private List<ReceiptUploadDetail> callApi(ReceiptUploadHeader aReceiptUploadHeader) {
		...
		//sorting
		Collections.sort(aReceiptUploadHeader.getReceiptUploadList(), Comparator.comparingLong(ReceiptUploadDetail ::getUploadDetailId));
		...
		for (ReceiptUploadDetail receiptUploadDetail : aReceiptUploadHeader.getReceiptUploadList()) {
			...
			JSONObject reqJson = new JSONObject(receiptUploadDetail.getJsonObject());
	...
	private ByteArrayOutputStream doDownloadFiles(String id) {
		...
		String whereCond = " where t.uploadHeaderId in (" + "'" + id + "'" + ") and  UploadStatus in (" + "'" + PennantConstants.UPLOAD_STATUS_SUCCESS + "'" + ")";
	...
	private WebClient getClient(String serviceEndPoint, String messageId, String authorization) {
		...
		try {
			...
			client.header(BatchUploadProcessorConstatnt.ENTITYID, "PFF".concat("RU"));
	...
	public class ReceiptHeaderThread extends Thread {

		ReceiptUploadHeader receiptUploadHeader = null;

		public ReceiptHeaderThread(ReceiptUploadHeader aReceiptUploadHeader) {
			this.receiptUploadHeader = aReceiptUploadHeader;
		}

		@Override
		public void run() {
			try {
				ApiRequestMethod(receiptUploadHeader);
			} catch (Exception e) {
