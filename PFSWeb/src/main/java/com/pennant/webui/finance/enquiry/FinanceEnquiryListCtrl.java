...
public class FinanceEnquiryListCtrl extends GFCBaseListCtrl<FinanceEnquiry> {
	...
	public void onCreate$window_FinanceEnquiry(Event event) {
		...
		this.sortOperator_StartDate
				.setModel(new ListModelList<SearchOperators>(new SearchOperators().getMultiDateOperators()));
		this.sortOperator_StartDate.setItemRenderer(new SearchOperatorListModelItemRenderer());

		this.sortOperator_MaturityDate
				.setModel(new ListModelList<SearchOperators>(new SearchOperators().getMultiDateOperators()));
		this.sortOperator_MaturityDate.setItemRenderer(new SearchOperatorListModelItemRenderer());

		this.sortOperator_FinProduct
				.setModel(new ListModelList<SearchOperators>(new SearchOperators().getMultiStringOperators()));
	...
	public void onClick$btnSearchFinRef(Event event) {
		...
		if (this.oldVar_sortOperator_FinCcy == Filter.OP_IN || this.oldVar_sortOperator_FinCcy == Filter.OP_NOT_IN) {
			...
			if (this.fromApproved.isChecked()) {
				...
				selectedValues = (String) MultiSelectionSearchListBox.show(this.window_FinanceEnquiry, "FinanceMain",
	...
	public void onSelect$sortOperator_FinProduct(Event event) {
		this.oldVar_sortOperator_FinProduct = doChangeStringOperator(sortOperator_FinProduct,
				oldVar_sortOperator_FinProduct, this.finProduct);
	}
	...
	public void onSelect$sortOperator_FinRef(Event event) {
		this.oldVar_sortOperator_FinRef = doChangeStringOperator(sortOperator_FinRef, oldVar_sortOperator_FinRef,
				this.finRef);
	}
	...
	public void onSelect$sortOperator_MaturityDate(Event event) {
		this.oldVar_sortOperator_MaturityDate = doChangeDateOperator(sortOperator_MaturityDate,
				oldVar_sortOperator_MaturityDate, this.maturityDate_one, this.maturityDate_two);
		this.maturityDate_two.setText("");
	...
	public void doSearch() {
		...
		if ("FINMANDENQ".equalsIgnoreCase(enquiryType.getValue())) {
			List<ValueLabel> mandates = InstrumentTypes.list();
		...
		if (this.startDate_one.getValue() != null || this.startDate_two.getValue() != null) {
			...
			if (itemStartDate != null) {
				final int searchOpId = ((SearchOperators) itemStartDate.getAttribute("data")).getSearchOperatorId();
				...
				if (searchOpId == -1) {
					...
					if (this.startDate_one.getValue() != null) {
						this.searchObj.addFilter(
								new Filter("FinStartDate", this.startDate_one.getValue(), Filter.OP_GREATER_OR_EQUAL));
					}
					if (this.startDate_two.getValue() != null) {
						this.searchObj.addFilter(
								new Filter("FinStartDate", this.startDate_two.getValue(), Filter.OP_LESS_OR_EQUAL));
					}
				} else {
					this.searchObj.addFilter(new Filter("FinStartDate", this.startDate_one.getValue(), searchOpId));
				}
			}
		}
		...
		if (StringUtils.isNotBlank(this.finRef.getValue())) {
			...
			if (itemCustID != null) {
				...
				if (searchOpId == -1) {
					...
					this.searchObj.addFilter(
							new Filter("FinReference", this.finRef.getValue().trim().split(","), Filter.OP_IN));
				} else if (searchOpId == Filter.OP_NOT_IN) {
					this.searchObj.addFilter(
							new Filter("FinReference", this.finRef.getValue().trim().split(","), Filter.OP_NOT_IN));
				} else {
		...
		if (StringUtils.isNotBlank(this.finProduct.getValue())) {
			...
			if (itemCustID != null) {
				...
				if (searchOpId == -1) {
					...
					this.searchObj.addFilter(new Filter("LovDescProductCodeName",
							this.finProduct.getValue().trim().split(","), Filter.OP_IN));
				} else if (searchOpId == Filter.OP_NOT_IN) {
					this.searchObj.addFilter(new Filter("LovDescProductCodeName",
							this.finProduct.getValue().trim().split(","), Filter.OP_NOT_IN));
				} else {
					this.searchObj.addFilter(
							new Filter("LovDescProductCodeName", this.finProduct.getValue().trim(), searchOpId));
				}
			}
		}
		if (StringUtils.isNotBlank(this.finCcy.getValue())) {
			...
			if (itemCustID != null) {
				...
				if (searchOpId == -1) {
					...
					this.searchObj.addFilter(
							new Filter("FinCcy", "%" + this.finCcy.getValue().trim().toUpperCase() + "%", searchOpId));
				} else if (searchOpId == Filter.OP_IN) {
					this.searchObj
	...
	private String setFinReferences() {
		...
		sql.append(" (CurrentOwner = ").append(getUserWorkspace().getLoggedInUser().getUserId()).append(')');
		sql.append("))");
