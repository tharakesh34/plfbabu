 ...
 */
public class InvestmentDealDialogCtrl extends DealFinanceBaseCtrl<FinanceMain> {
	...
	protected void doSetProperties() {
		super.pageRightName = "InvestmentDealDialog";
	}
	...
	public void onCreate$window_InvestmentDealDialogCtrl(Event event) throws Exception {
		...
		if (arguments.containsKey("treasuaryFinHeaderDialogCtrl")) {
			setTreasuaryFinHeaderDialogCtrl((TreasuaryFinHeaderDialogCtrl) arguments.get("treasuaryFinHeaderDialogCtrl"));
			...
			getUserWorkspace().allocateRoleAuthorities(getRole(), "InvestmentDealDialog");
		}
	...
	private void doCheckRights() {
		...
		getUserWorkspace().allocateAuthorities("InvestmentDealDialog", getRole());
	...
	public void doWriteBeanToComponents(InvestmentFinHeader aFinHeader, FinanceDetail aFinanceDetail) {
		...
		this.profitDaysBasis.setValue(PennantAppUtil.getlabelDesc(aFinHeader.getProfitDaysBasis(), PennantStaticListUtil.getProfitDaysBasis()));
		...
		if (StringUtils.isNotBlank(aFinanceMain.getFinType())) {
		...
		if(StringUtils.trimToNull(aFinanceMain.getFinReference()) == null) {
			aFinanceMain.setFinReference(String.valueOf(ReferenceGenerator.generateNewFinRef(false, aFinanceMain)));
		}
	...
	public void doWriteComponentsToBean(FinanceDetail aFinanceDetail) {
		...
		try {	
			aFinanceMain.setFinAmount(PennantAppUtil.unFormateAmount(this.finAmount.getValidateValue(), ccyFormat));
		} catch (WrongValueException we) {
			wve.add(we);
		}
	...
	private void doSetValidation() {
		...
		if (!this.repayProfitRate.isReadonly()){
			this.repayProfitRate.setConstraint(new PTDecimalValidator(Labels.getLabel("label_InvestmentDealDialog_RatePerc.value"), 9, false));
		}
	...
	private void doSetFieldProperties() {
		...
		ccyFormat = CurrencyUtil.getFormat(getInvestmentFinHeader().getFinCcy());

		this.finBranch.setTextBoxWidth(171);
		...
		filtersFinType[0] = new Filter("FinDivision", FinanceConstants.FIN_DIVISION_TREASURY, Filter.OP_LIKE);
		this.finType.setFilters(filtersFinType);
         
		this.finAmount.setTextBoxWidth(165);		
	...
	private void setRepaymentAmt() {
		...
		BigDecimal finAmount = PennantAppUtil.unFormateAmount(this.finAmount.getActualValue(), ccyFormat);
	...
	private void setHeaderFields(BigDecimal finAmount, BigDecimal totalRepayAmt) {
		...
		if (principalAmt.compareTo(totalPrincipalAmt) > 0) {
			...
			Clients.clearWrongValue(this.finAmount);
		}
		...
		if (dataObject instanceof String){
			...
			if (financeType != null) {
				...
				if (getFinanceDetail().getFinScheduleData().getFinanceMain() != null) {
					getFinanceDetail().getFinScheduleData().getFinanceMain().setFinStsReason(FinanceConstants.FINSTSRSN_SYSTEM);
