 ...
 * /WEB-INF/pages/Finance/wiffinanceMain/MurabahaWIFFinanceMainDialog.zul file.
 */
public class RetailWIFFinanceMainDialogCtrl extends GFCBaseCtrl<FinanceMain> {
	...
	private void doSetFieldProperties() {
		...
		/*this.custCcy.setMaxlength(LengthConstants.LEN_CURRENCY);
		this.custCcy.setTextBoxWidth(121);
		this.custCcy.setMandatoryStyle(true);
		this.custCcy.setModuleName("Currency");
		this.custCcy.setValueColumn("CcyCode");
		this.custCcy.setDescColumn("CcyDesc");
	...
	public void doWriteBeanToComponents(FinanceDetail aFinanceDetail, boolean onLoadProcess) throws ParseException, InterruptedException, PFFInterfaceException, IllegalAccessException, InvocationTargetException {
		...
		if(StringUtils.isNotEmpty(financeType.getProduct())){
			...
			this.label_FinanceMainDialog_FinType.setValue(Labels.getLabel("label_WIFFinanceMainDialog_PromotionCode.value"));
		}
		...
		if (aFinanceDetail.getFinScheduleData().getFinanceType().isFInIsAlwGrace()) {
			...
			if (StringUtils.isNotEmpty(aFinanceMain.getGraceBaseRate()) && 
				...
				this.grcEffectiveRate.setValue(PennantApplicationUtil.formatRate(aFinanceMain.getGrcPftRate().doubleValue(), 2));
				this.row_FinGrcRates.setVisible(false);
		...
		this.finRepaymentAmount.setValue(PennantAppUtil.formateAmount(
				aFinanceMain.getReqRepayAmount(), finFormatter));
		...
		if (StringUtils.isNotEmpty(aFinanceMain.getRepayBaseRate()) && 
			...
			RateDetail rateDetail = RateUtil.rates(
					this.repayRate.getBaseValue(), this.finCcy.getValue(), this.repayRate.getSpecialValue(), this.repayRate.getMarginValue(),
			...
			this.repayEffectiveRate.setValue(PennantApplicationUtil.formatRate(aFinanceMain.getRepayProfitRate().doubleValue(), 2));
			this.row_FinRepRates.setVisible(false);
		...
		aFinanceMain.getSupplementRent(),finFormatter));
		this.increasedCost.setValue(PennantAppUtil.formateAmount(
		aFinanceMain.getIncreasedCost(), finFormatter));
	...
	private void fillCustomerData(WIFCustomer customer){
		...
		fillComboBox(this.custSalutationCode, customer.getCustSalutationCode(), PennantAppUtil.getSalutationCodes(customer.getCustGenderCode()), "");

		this.custCtgCode.setDescription(StringUtils.isBlank(customer.getLovDescCustCtgCodeName()) ? "" : customer.getLovDescCustCtgCodeName());
		this.custTypeCode.setDescription(StringUtils.isBlank(customer.getLovDescCustTypeCodeName()) ? "" : customer.getLovDescCustTypeCodeName());
		this.custSector.setDescription(StringUtils.isBlank(customer.getLovDescCustSectorName()) ? "" : customer.getLovDescCustSectorName());
		this.custSubSector.setDescription(StringUtils.isBlank(customer.getLovDescCustSubSectorName()) ? "" : customer.getLovDescCustSubSectorName());
		this.custEmpSts.setDescription(StringUtils.isBlank(customer.getLovDescCustEmpStsName()) ? "" : customer.getLovDescCustEmpStsName());
		this.custNationality.setDescription(StringUtils.isBlank(customer.getLovDescCustNationalityName()) ? "" : customer.getLovDescCustNationalityName());
		this.custMaritalSts.setDescription(StringUtils.isBlank(customer.getLovDescCustMaritalStsName()) ? "" : customer.getLovDescCustMaritalStsName());
		this.custBaseCcy.setDescription(CurrencyUtil.getCcyDesc(customer.getCustBaseCcy()));
		this.custEmpDept.setDescription(StringUtils.isBlank(customer.getLovDescEmpDept()) ? "" : customer.getLovDescEmpDept());
		this.custEmpDesg.setDescription(StringUtils.isBlank(customer.getLovDescEmpDesg()) ? "" : customer.getLovDescEmpDesg());
	...
	private void appendFinScoringDetailTab(boolean onLoadProcess){
		...
		if (scoringGroupList != null && !scoringGroupList.isEmpty()) {
			...
			getFinanceDetail().getFinScheduleData().getFinanceMain().setLovDescCustCtgCode(PennantConstants.PFF_CUSTCTG_INDIV);
	...
	public void doWriteComponentsToBean(FinanceDetail aFinanceDetail) throws InterruptedException, IllegalAccessException, InvocationTargetException {
		...
		try {
			if (StringUtils.isEmpty(this.finReference.getValue()) && getFinanceDetail().getFinScheduleData().getFinanceType().isFinIsGenRef()) {
			...
			if (this.depreciationFrq.isValidComboValue()) {
				...
				if(StringUtils.isNotEmpty(this.depreciationFrq.getValue()) &&
					...
					aFinanceMain.setNextDepDate(FrequencyUtil.getNextDate(this.depreciationFrq.getValue(), 1,
							...
							aFinanceDetail.getFinScheduleData().getFinanceType().getFddLockPeriod()).getNextFrequencyDate());
				}
			}
		} catch (WrongValueException we) {
			wve.add(we);
		...
		if(buildEvent) {
			...
			if (getFeeDetailDialogCtrl() != null) {
				...
				aFinanceDetail.setFinScheduleData(getFeeDetailDialogCtrl().doWriteComponentsToBean(aFinanceDetail.getFinScheduleData(), false , true));
			}
		...
		if(getFeeDetailDialogCtrl() != null){
			...
			aFinanceDetail.setFinScheduleData(getFeeDetailDialogCtrl().doWriteComponentsToBean(aFinanceDetail.getFinScheduleData(), true , false));
		}
	...
	private WIFCustomer prepareCustomerDetails(WIFCustomer aCustomer, ArrayList<WrongValueException> wve){
		...
		try {
			if(StringUtils.isEmpty(this.custBaseCcy.getValue())) {
				wve.add(new WrongValueException(this.custBaseCcy, Labels.getLabel("FIELD_NO_INVALID", new String[] { Labels.getLabel("label_FinanceMainDialog_CustBaseCcy.value") })));
			} else {
		...
		try {
			if(this.custGenderCode.isVisible() && !this.custGenderCode.isDisabled()&&this.row_custGenderCode.isVisible()){
		...
		try {
			if(this.custSalutationCode.isVisible()  && !this.custSalutationCode.isDisabled()&&this.row_custGenderCode.isVisible()){
		...
		try {
			...
			if(this.elgRequired.isChecked()){
				aCustomer.setTotalIncome(PennantAppUtil.unFormateAmount(this.custIncome.getValidateValue(), ccyformater));
			}
		} catch (WrongValueException we) {
			wve.add(we);
		}
		try {
			aCustomer.setTotalExpense(PennantAppUtil.unFormateAmount(this.custTotExpense.getActualValue(),ccyformater));
			if(this.elgRequired.isChecked()){
				aCustomer.setTotalExpense(PennantAppUtil.unFormateAmount(this.custTotExpense.getValidateValue(),ccyformater));
			}
		} catch (WrongValueException we) {
			wve.add(we);
		}
	...
	private void doSetValidation() {
		...
		if(recSave){
			...
			this.custShrtName.setConstraint(new PTStringValidator(Labels.getLabel("label_FinanceMainDialog_CustShrName.value"),null,true));

			if(this.elgRequired.isChecked()){
				this.custDOB.setConstraint(new PTDateValidator(Labels.getLabel("label_FinanceMainDialog_CustDOB.value"),true,startDate,appStartDate,false));
			}
		}
		if((getFinanceDetail().getFinScheduleData().getFeeRules() != null && 
				getFinanceDetail().getFinScheduleData().getFeeRules().size() > 0)  || 
			...
			this.custEmpSts.setConstraint(new PTStringValidator(Labels.getLabel("label_FinanceMainDialog_CustEmpSts.value"),
					null, true, true));
	...
	private void doSetLOVValidation() {
		...
		if(recSave && this.elgRequired.isChecked()){
			...
			this.custEmpDesg.setConstraint(new PTStringValidator(Labels.getLabel("label_FinanceMainDialog_CustEmpDesg.value"),null,true,true));

			this.custEmpDept.setConstraint(new PTStringValidator(Labels.getLabel("label_FinanceMainDialog_CustEmpDept.value"),null,true,true));
			
			this.custSalutationCode.setConstraint(new PTStringValidator(Labels.getLabel("label_FinanceMainDialog_CustSalutationCode.value"), null, true));
			
			this.custIncome.setConstraint(new PTDecimalValidator(Labels.getLabel("label_FinanceMainDialog_TotIncome.value"),finformatter, true));
			
			this.custTotExpense.setConstraint(new PTDecimalValidator(Labels.getLabel("label_FinanceMainDialog_TotExpense.value"), finformatter, true));
			
			this.custEmpName.setConstraint(new PTStringValidator(Labels.getLabel("label_FinanceMainDialog_CustEmpName.value"),null,true,true));
		}
	...
	public void onFulfill$custSubSector(Event event) {
		...
		if (dataObject instanceof String) {
			...
			if (details != null) {
				this.custEmpName.setValue("", "");
				/*Filter[] filters=new Filter[1];
				filters[0]=new Filter("lovDescSectorCode", this.custSubSector.getValue(), Filter.OP_EQUAL);
				this.custEmpName.setFilters(null);
...
/*		if(!StringUtils.trimToEmpty(this.depreciationFrq.getValue()).equals("")){
	...
	public FinanceDetail dofillEligibilityData(Boolean isUserAction) throws InterruptedException, IllegalAccessException, InvocationTargetException{
		logger.debug("Entering");
		...
		if(aCustomer != null){
			...
			getFinanceDetail().setCustomerEligibilityCheck(getFinanceDetailService().getWIFCustEligibilityDetail(
					aCustomer,financeMain.getFinCcy()));
	...
	private void doStoreDefaultValues() {
		...
		if (StringUtils.isEmpty(this.finCcy.getValue())) {
			this.finCcy.setValue(getFinanceDetail().getFinScheduleData().getFinanceType().getFinCcy(),
					 CurrencyUtil.getCurrencyObject(getFinanceDetail().getFinScheduleData().getFinanceType().getFinCcy()).getCcyDesc());
		}
		...
		if (getFinanceDetail().getFinScheduleData().getFinanceMain().isAllowGrcPeriod()) {
			...
			if (this.graceTerms_Two.intValue() > 0) {
				List<Calendar> scheduleDateList = FrequencyUtil.getNextDate(this.gracePftFrq.getValue(),
						this.graceTerms_Two.intValue(), this.finStartDate.getValue(), HolidayHandlerTypes.MOVE_NONE, false, 0).getScheduleList();
		...
		while (retValue == PennantConstants.porcessOVERIDE) {
			...
			if (!this.finReference.isReadonly() && this.finReference.getValue() != null) {
				if (getFinanceDetailService().isFinReferenceExits(this.finReference.getValue(), "_View", false)) {
					...
					errorList.add(new ErrorDetails(Labels.getLabel("label_FinReference"),"30506",new String[] {
			...
			if (getFinanceDetail().getFinScheduleData().getFinanceMain().isAllowGrcPeriod()) {
				...
				if (!this.repayFrq.validateFrquency(this.nextGrcPftDate_two.getValue(), this.gracePeriodEndDate.getValue())) {
...
/*						errorList.add(new ErrorDetails("nextGrcCpzDate_two","30526", new String[] {
	...
	public FinanceMain getFinanceMain(){
		...
		financeMain.setDownPayment(PennantAppUtil.unFormateAmount(this.downPayBank.getActualValue().add(this.downPaySupl.getActualValue()),format));
		return financeMain;
	}
...
public void onChange$custGenderCode(Event event){
	...
	if (StringUtils.trimToEmpty(this.custGenderCode.getValue())
