...
public class SukukFinanceMainDialogCtrl extends FinanceMainBaseCtrl {
	...
	private static final Logger logger = LogManager.getLogger(SukukFinanceMainDialogCtrl.class);
	 ...
	 */
	/*
	 * private FinancePremiumDetail doPreparePremiumDetail(FinanceDetail detail, String repayPftFrq,
	 * ArrayList<WrongValueException> wve){ logger.debug("Entering");
	 * 
	 * int formatter = CurrencyUtil.getFormat(getFinanceDetail().getFinScheduleData().getFinanceMain().getFinCcy());
	 * 
	 * FinancePremiumDetail premiumDetail = detail.getPremiumDetail(); //Finance Premium or Discount Details
	 * if(premiumDetail == null){ premiumDetail = new FinancePremiumDetail(); } try {
	 * premiumDetail.setIssueNumber(this.issueNumber.getValue()); } catch (WrongValueException we) { wve.add(we); } try
	 * { premiumDetail.setNoOfUnits(this.noOfUnits.intValue()); } catch (WrongValueException we) { wve.add(we); } try {
	 * premiumDetail.setFaceValue(PennantApplicationUtil.unFormateAmount(this.faceValue.getValue(),formatter)); } catch
	 * (WrongValueException we) { wve.add(we); } try { premiumDetail.setPremiumType(getComboboxValue(this.premiumType));
	 ...
	 * catch (WrongValueException we) { wve.add(we); } try { premiumDetail.setYieldValue(this.yieldValue.getValue()); }
	 * catch (WrongValueException we) { wve.add(we); } try {
	 * 
	 * if(!this.lastCouponDate.isDisabled()){
	 * 
	 * if(this.lastCouponDate.getValue() != null){ if(!FrequencyUtil.isFrqDate(repayPftFrq,
	 * this.lastCouponDate.getValue())){ throw new WrongValueException(this.lastCouponDate,
	 * Labels.getLabel("FRQ_DATE_MISMATCH", new String[] {
	 * Labels.getLabel("label_SukukFinanceMainDialog_LastCouponDate.value"),
	 * Labels.getLabel("label_FinanceMainDialog_RepayPftFrq.value") })); }
	 * 
	 * if(this.finStartDate.getValue() != null){
	 * if(this.finStartDate.getValue().compareTo(this.lastCouponDate.getValue()) > 0){ throw new
	 * WrongValueException(this.lastCouponDate, Labels.getLabel("DATE_NOT_BEFORE", new String[] {
	 * Labels.getLabel("label_SukukFinanceMainDialog_LastCouponDate.value"),
	 * Labels.getLabel("label_FinanceMainDialog_FinStartDate.value") })); }
	 * 
	 * if(this.lastCouponDate.getValue().compareTo(this.purchaseDate.getValue()) > 0){ throw new
	 * WrongValueException(this.lastCouponDate, Labels.getLabel("DATE_NOT_AFTER", new String[] {
	 * Labels.getLabel("label_SukukFinanceMainDialog_LastCouponDate.value"),
	 * Labels.getLabel("label_SukukFinanceMainDialog_PurchaseDate.value") })); } }
	 * 
	 * if(this.maturityDate_two.getValue() != null){
	 * if(this.lastCouponDate.getValue().compareTo(this.maturityDate_two.getValue()) >= 0){ throw new
	 * WrongValueException(this.lastCouponDate, Labels.getLabel("DATE_ALLOWED_BEFORE", new String[] {
	 * Labels.getLabel("label_SukukFinanceMainDialog_LastCouponDate.value"),
	 * Labels.getLabel("label_FinanceMainDialog_MaturityDate.value") })); } } } }
	 * premiumDetail.setLastCouponDate(this.lastCouponDate.getValue()); } catch (WrongValueException we) { wve.add(we);
	 * } try {
	 * premiumDetail.setAccruedProfit(PennantApplicationUtil.unFormateAmount(this.accruedProfit.getValue(),formatter));
	 * } catch (WrongValueException we) { wve.add(we); } try { if(this.maturityDate_two.getValue() != null &&
	 * !this.maturityDate.isDisabled()){ if(this.purchaseDate.getValue().compareTo(this.maturityDate_two.getValue()) >=
	 * 0){ throw new WrongValueException(this.maturityDate_two, Labels.getLabel("DATE_ALLOWED_AFTER", new String[] {
	 * Labels.getLabel("label_FinanceMainDialog_MaturityDate.value"),
	 * Labels.getLabel("label_SukukFinanceMainDialog_PurchaseDate.value") })); } }
	 * premiumDetail.setPurchaseDate(this.purchaseDate.getValue()); } catch (WrongValueException we) { wve.add(we); }
	 * try { premiumDetail.setFairValuePerUnit(PennantApplicationUtil.unFormateAmount(this.fairValuePerUnit.getValue(),
	 * formatter)); } catch (WrongValueException we) { wve.add(we); } try {
	 ...
	 * 
	 * logger.debug("Leaving"); return premiumDetail; }
	 */
	...
	protected void doSetValidation() {
		...
		if (!this.issueNumber.isReadonly()) {
			this.issueNumber.setConstraint(new PTStringValidator(
					Labels.getLabel("label_SukukFinanceMainDialog_IssueNumber.value"), null, true));
		}
		if (!this.noOfUnits.isReadonly()) {
			this.noOfUnits.setConstraint(
					new PTNumberValidator(Labels.getLabel("label_SukukFinanceMainDialog_IssueNumber.value"), true));
		}
		if (!this.faceValue.isDisabled()) {
			this.faceValue.setConstraint(new PTDecimalValidator(
					Labels.getLabel("label_SukukFinanceMainDialog_FaceValue.value"), ccyformatter, true));
		}
		if (!this.premiumType.isDisabled()) {
			this.premiumType.setConstraint(new StaticListValidator(PennantStaticListUtil.getPremiumTypeList(),
					Labels.getLabel("label_SukukFinanceMainDialog_PremiumType.value")));
		}
		if (!this.premiumValue.isDisabled()) {
			this.premiumValue.setConstraint(new PTDecimalValidator(
					Labels.getLabel("label_SukukFinanceMainDialog_PremiumValue.value"), ccyformatter, true));
		}
		if (!this.pricePerUnit.isDisabled()) {
			this.pricePerUnit.setConstraint(new PTDecimalValidator(
					Labels.getLabel("label_SukukFinanceMainDialog_PurchasePricePerUnit.value"), ccyformatter, true));
		}
		if (!this.fairValuePerUnit.isDisabled()) {
			this.fairValuePerUnit.setConstraint(new PTDecimalValidator(
					Labels.getLabel("label_SukukFinanceMainDialog_FairValuePerUnit.value"), ccyformatter, false));
		}
		if (!this.lastCouponDate.isDisabled()) {
			this.lastCouponDate.setConstraint(
					new PTDateValidator(Labels.getLabel("label_SukukFinanceMainDialog_LastCouponDate.value"), true));
		}
		/*
		 * if (!this.yieldValue.isDisabled()) { this.yieldValue.setConstraint(new
		 * PTDecimalValidator(Labels.getLabel("label_SukukFinanceMainDialog_Yield.value"), ccyformatter,false)); } if
		 * (!this.accruedProfit.isDisabled()) { this.accruedProfit.setConstraint(new
		 * PTDecimalValidator(Labels.getLabel("label_SukukFinanceMainDialog_AccruedProfit.value"), ccyformatter,false));
		 * } if (!this.fairValuePerUnit.isDisabled()) { this.fairValuePerUnit.setConstraint(new
		 * PTDecimalValidator(Labels.getLabel("label_SukukFinanceMainDialog_FairValuePerUnit.value"),
		 * ccyformatter,false)); } if (!this.fairValueAmount.isDisabled()) { this.fairValueAmount.setConstraint(new
		 * PTDecimalValidator(Labels.getLabel("label_SukukFinanceMainDialog_fairValueAmount.value"),
		 * ccyformatter,false)); }
		 */
