 ...
 * FILE HEADER *
 ********************************************************************************************
 * * FileName : SelectCDSchemeDialogCtrl.java * * Author : PENNANT TECHONOLOGIES * * Creation Date : 16-11-2011 * *
...
/**
 * This is the controller class for the /WEB-INF/pages/Finance/FinanceMain/SelectCDSchemeDialog.zul file.
 */
public class SelectCDSchemeDialogCtrl extends GFCBaseCtrl<FinanceDetail> {
	 ...
	 */
	protected Window window_SelectCDSchemeDialog;
	...
	protected Label label_SelectCDSchemeDialog_EIDNumber;
	protected Label label_SelectCDSchemeDialog_MobileNo;
	 ...
	 */
	public SelectCDSchemeDialogCtrl() {
		super();
	}
	...
	public void onCreate$window_SelectCDSchemeDialog(Event event) {
		logger.debug("Entering");

		// Set the page level components.
		setPageComponents(window_SelectCDSchemeDialog);
	...
	private void showDetailView(FinanceDetail financeDetail) {
		...
		if (isWorkFlowEnabled()) {
			...
			if (financeDetail.getCustomerDedupList() != null && !financeDetail.getCustomerDedupList().isEmpty()) {
				...
				map.put("SelectCDSchemeDialogCtrl", this);
			...
			try {
				if (financeDetail.getCustomerDedupList() == null || financeDetail.getCustomerDedupList().isEmpty()) {
					...
					this.window_SelectCDSchemeDialog.onClose();
				}
			} catch (Exception e) {
	...
	/**
	 * Opens the SelectCDSchemeDialog window modal.
	 */
	private void showSelectCDSchemeDialog() {
		logger.debug("Entering");
		try {
			// open the dialog in modal mode
			this.window_SelectCDSchemeDialog.doModal();
		} catch (Exception e) {
			MessageUtil.showError(e);
		}
		logger.debug("Leaving");
	...
	private void doFieldValidation() {
		...
		try {
			if (StringUtils.isBlank(this.finType.getValue()) && StringUtils.isNotBlank(this.promotionCode.getValue())) {
				throw new WrongValueException(this.finType, Labels.getLabel("CHECK_NO_EMPTY",
						new String[] { Labels.getLabel("label_SelectCDSchemeDialog_FinType.value") }));
			}
			...
			if (this.existingCust.isChecked()) {
				if (StringUtils.isEmpty(this.custCIF.getValue())) {
					throw new WrongValueException(this.custCIF, Labels.getLabel("FIELD_NO_EMPTY",
							new String[] { Labels.getLabel("label_SelectCDSchemeDialog_CustCIF.value") }));
				}
			}
		} catch (WrongValueException e) {
			wve.add(e);
		...
		if (this.newCust.isChecked()) {
			...
			try {
				if (getComboboxValue(this.custCtgType).equals(PennantConstants.List_Select)) {
					throw new WrongValueException(this.custCtgType, Labels.getLabel("FIELD_NO_EMPTY",
							new String[] { Labels.getLabel("label_SelectCDSchemeDialog_CustCtg.value") }));
				}
			} catch (WrongValueException e) {
				wve.add(e);
			}
			...
			this.mobileNo.setConstraint(
					new PTMobileNumberValidator(Labels.getLabel("label_SelectCDSchemeDialog_MobileNo.value"), true));
	...
	private boolean checkDedup() {
		if (newCust.isChecked()) {
			 ...
			 * 
			 * if (cif != null) { String msg = Labels.getLabel("label_SelectCDSchemeDialog_ProspectExist", new String[]
			 * { isRetailCustomer ? Labels.getLabel(primaryIdLabel) : Labels.getLabel(primaryIdLabel), cif + ". \n" });
			 * 
	...
	private void showDetailViewforDedUp(CustomerDetails customerDetails) {
		...
		// call the ZUL-file with the parameters packed in a map
		try {
			map.put("parentWindow", window_SelectCDSchemeDialog);
	...
	private void doSetPrimaryIdAttributes() {
		...
		label_SelectCDSchemeDialog_EIDNumber.setValue(Labels.getLabel(primaryIdLabel));
