...
public class CovenantEnquiryDialogCtrl extends GFCBaseCtrl<FinAgreementDetail> {
	...
	public void onCreate$window_CovenantEnquiryDialog(ForwardEvent event) throws Exception {
	...
	public void doFillCovenantList(List<FinCovenantType> finCovenantTypeDetails) {
		...
		if (finCovenantTypeDetails != null && !finCovenantTypeDetails.isEmpty()) {
			for (FinCovenantType detail : finCovenantTypeDetails) {
				...
				lc = new Listcell(
						DateUtility.format(detail.getReceivableDate(), DateFormat.LONG_DATE.getPattern()));
				lc.setParent(item);
				lc = new Listcell(
						DateUtility.format(detail.getDocReceivedDate(), DateFormat.LONG_DATE.getPattern()));
				lc.setParent(item);
