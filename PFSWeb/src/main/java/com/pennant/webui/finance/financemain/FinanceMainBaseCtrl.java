...
public class FinanceMainBaseCtrl extends GFCBaseCtrl<FinanceMain> {
	...
	protected void doSetFieldProperties() {
		...
		this.referralId.setProperties("RelationshipOfficer", "ROfficerCode", "ROfficerDesc", false, LengthConstants.LEN_MASTER_CODE);
		this.dmaCode.setProperties("RelationshipOfficer", "ROfficerCode", "ROfficerDesc", false, LengthConstants.LEN_MASTER_CODE);
		this.salesDepartment.setProperties("GeneralDepartment", "GenDepartment", "GenDeptDesc", false, LengthConstants.LEN_MASTER_CODE);
		...
		this.accountsOfficer.setProperties("GeneralDepartment", "GenDepartment", "GenDeptDesc", true, 8);
		...
		this.grcEffectiveRate.setMaxlength(LengthConstants.LEN_RATE);
		this.grcEffectiveRate.setFormat(PennantConstants.rateFormate9);
		this.grcEffectiveRate.setRoundingMode(BigDecimal.ROUND_DOWN);
		this.grcEffectiveRate.setScale(LengthConstants.LEN_RATE_SCALE);
		...
		this.repayEffectiveRate.setMaxlength(LengthConstants.LEN_RATE);
		...
		this.repayEffectiveRate.setScale(LengthConstants.LEN_RATE_SCALE);
	...
	public void appendScheduleDetailTab(Boolean onLoadProcess, Boolean isFeeRender) {
		...
		if (StringUtils.isNotBlank(moduleDefiner)
				&& isOverdraft	&& (finSchdData.getFinanceScheduleDetails() == null || finSchdData.getFinanceScheduleDetails()
						.isEmpty())) {
			getFinanceDetail()
		...
		if (!onLoadProcess) {
			...
			map.put("feeDetailDialogCtrl", feeDetailDialogCtrl);
	 ...
	 * Method for Rendering Schedule Details Data in finance
	 */
	protected void appendFeeDetailsTab(boolean onLoadProcess) {
		logger.debug("Entering");
		if (onLoadProcess) {
			createTab(AssetConstants.UNIQUE_ID_FEES, false);
		} else {
			clearTabpanelChildren(AssetConstants.UNIQUE_ID_FEES);
		}
		if ((getFinanceDetail().getFinScheduleData().getFeeRules() != null && getFinanceDetail().getFinScheduleData()
			...
			map.put("menuItemRightName", menuItemRightName);
			Executions.createComponents("/WEB-INF/pages/Finance/FinanceMain/FeeDetailDialog.zul",
					getTabpanel(AssetConstants.UNIQUE_ID_FEES), map);
			Tab tab = getTab(AssetConstants.UNIQUE_ID_FEES);
			if (tab != null) {
				tab.setVisible(true);
			}
		}

		//if Insurances are added to the financetype or finance then fees tab to be appeneded.
		if ((getFinanceDetail().getFinScheduleData().getFinanceType().getFinTypeInsurances() != null && !getFinanceDetail()
				.getFinScheduleData().getFinanceType().getFinTypeInsurances().isEmpty())
				|| getFinanceDetail().getFinScheduleData().getFinInsuranceList() != null
				&& !getFinanceDetail().getFinScheduleData().getFinInsuranceList().isEmpty()) {
	...
	protected void appendVasRecordingTab(boolean onLoad) {
		...
		if (onLoad) {
			...
			map.put("finType", getFinanceDetail().getFinScheduleData().getFinanceMain().getFinType());			Executions.createComponents("/WEB-INF/pages/Finance/FinanceMain/FinVasRecordingDialog.zul",
	...
	public void onSelectTab(ForwardEvent event) throws IllegalAccessException, InvocationTargetException,
		...
		case AssetConstants.UNIQUE_ID_STEPDETAILS:
			...
			break;
		case AssetConstants.UNIQUE_ID_FEES:
			feeDetailDialogCtrl.doSetLabels(getFinBasicDetails());
			break;
		...
		case AssetConstants.UNIQUE_ID_COLLATERAL:
			if (ImplementationConstants.COLLATERAL_INTERNAL) {
				...
				if (!getFinanceDetail().getFinScheduleData().getFinanceMain().isLovDescIsSchdGenerated()) {
					utilizedAmt = PennantAppUtil.unFormateAmount(this.finAmount.getActualValue().subtract(
							this.downPayBank.getActualValue()).subtract(this.downPaySupl.getActualValue()), formatter)
							.add(getFinanceDetail().getFinScheduleData().getFinanceMain().getFeeChargeAmt()).add(
									getFinanceDetail().getFinScheduleData().getFinanceMain().getInsuranceAmt());
	...
	public void doWriteBeanToComponents(FinanceDetail aFinanceDetail, boolean onLoadProcess) throws ParseException,
		...
		if (StringUtils.equals(ImplementationConstants.CLIENT_NAME,
				ImplementationConstants.CLIENT_AHB)) {
		...
		if (aFinanceDetail.getRolledoverFinanceHeader() != null) {
			...
			this.latePayAmount.setValue(PennantAppUtil.formateAmount(rolledoverFinanceHeader.getLatePayAmount(),
					format));
	...
	public boolean isSchdlRegenerate() {
		...
		/*if (feeDetailDialogCtrl != null && feeDetailDialogCtrl.isDataChanged(false)) {
			return true;
		}
		...
		if (StringUtils.isEmpty(moduleDefiner) && finFeeDetailListCtrl != null && finFeeDetailListCtrl.getFinInsuranceList() != oldVar_finInsuranceList) {
			return true;
		}
	...
	protected void doSetValidation() {
		...
		if (financeDate != null) {
			if (this.finStartDate.isVisible() && this.finContractDate.isVisible() && !this.finContractDate.isReadonly()) {
				this.finContractDate.setConstraint(new PTDateValidator(Labels
						.getLabel("label_FinanceMainDialog_FinContractDate.value"), false,appStartDate, financeDate, true));
			}
		} else {
			if (this.finContractDate.isVisible() && !this.finContractDate.isReadonly()) {
				this.finContractDate.setConstraint(new PTDateValidator(Labels
						.getLabel("label_FinanceMainDialog_FinContractDate.value"), false, appStartDate,
						appEndDate, true));
			}
		}
		...
		/*if (this.row_supplementRent.isVisible() && !this.supplementRent.isDisabled()) {
			this.supplementRent.setConstraint(new PTDecimalValidator(Labels.getLabel("label_FinanceMainDialog_SupplementRent.value"), 3,
					false));
		}
		if (this.row_supplementRent.isVisible()
				&& !this.increasedCost.isDisabled()) {
			this.increasedCost.setConstraint(new PTDecimalValidator(Labels.getLabel("label_FinanceMainDialog_IncreasedCost.value"), 3,
					false));
		}*/
	...
	protected void doSetLOVValidation() {
		...
		if (feeDetailDialogCtrl != null && !recSave && !buildEvent && feeDetailDialogCtrl.feeActId.isMandatory()) {
			feeDetailDialogCtrl.feeActId.setConstraint(new PTStringValidator(Labels
					.getLabel("label_FeeDetailDialog_FeeAccount.value"), null, true));
		}
		logger.debug("Leaving ");
	}
	...
	public void doSave() throws Exception {
		...
		if (!isOverdraft) {
			if (feeDetailDialogCtrl != null && aFinScheduleData.getFeeRules() != null
					&& aFinScheduleData.getFeeRules().size() > 0) {

				// check if fee & charges rules executed or not
				if (!feeDetailDialogCtrl.isFeeChargesExecuted()) {
		...
		if (collateralHeaderDialogCtrl != null) {
			...
			if (!recSave && finType.isFinCollateralReq()) {
				...
				if (!aFinanceMain.isLovDescIsSchdGenerated()) {
					utilizedAmt = aFinanceMain.getFinAmount().subtract(aFinanceMain.getDownPayment()).add(
		...
		if (StringUtils.isBlank(this.custCIF.getValue())) {
			...
			if (accountingDetailDialogCtrl != null
					&& (!isOverdraft && !finType.isDroplineOD())) {
			...
			if (doProcess(aFinanceDetail, tranType)) {
				...
				if (!"Save".equalsIgnoreCase(this.userAction.getSelectedItem().getLabel())) {
					...
					if (!referenceIdList.isEmpty()) {
						...
						try {
							HashMap<String, Object> fieldsAndValues = getPreparedMailData(aFinanceDetail.getFinScheduleData().getFinanceMain());
							getMailUtil().sendMail(notificationIdlist, fieldsAndValues, aFinanceDetail.getDocumentDetailsList(), mailIDMap, null);
						} catch (Exception e) {
								...
								if (StringUtils.isNotEmpty(reference)) {
									if (!PennantConstants.RCD_STATUS_CANCELLED.equalsIgnoreCase(aFinanceMain
										...
										if (StringUtils.isBlank(aFinanceMain.getNextTaskId())) {
											getEventManager().publish(
													Labels.getLabel("REC_FINALIZED_MESSAGE") + " with Reference" + ":"
															+ reference, notify, to);
										} else {
											getEventManager().publish(
													Labels.getLabel("REC_PENDING_MESSAGE") + " with Reference" + ":"
				...
				if (this.gracePeriodEndDate.getValue() != null || this.gracePeriodEndDate_two.getValue() != null) {
					if (this.finStartDate.getValue().compareTo(this.nextGrcPftDate_two.getValue()) == 0) {
						this.graceTerms_Two.setValue(FrequencyUtil.getTerms(this.gracePftFrq.getValue(),
								this.nextGrcPftDate_two.getValue(), this.gracePeriodEndDate.getValue() == null ? 
										this.gracePeriodEndDate_two.getValue() : this.gracePeriodEndDate.getValue(), false, false)
								.getTerms());
					} else if (this.finStartDate.getValue().compareTo(this.nextGrcPftDate_two.getValue()) < 0) {
						this.graceTerms_Two.setValue(FrequencyUtil.getTerms(this.gracePftFrq.getValue(),
								this.nextGrcPftDate_two.getValue(), this.gracePeriodEndDate.getValue() == null ? 
										this.gracePeriodEndDate_two.getValue() : this.gracePeriodEndDate.getValue(), true, false)
								.getTerms());
					}
		...
		if (StringUtils.equals(rateType, PennantConstants.RATE_BASE)) {
			...
			if (StringUtils.isNotBlank(this.graceRate.getBaseValue())) {
				calculateRate(this.graceRate.getBaseValue(), this.finCcy.getValue(), this.graceRate.getSpecialComp(),
						this.graceRate.getBaseComp(), this.graceRate.getMarginValue(), this.grcEffectiveRate,
	...
	private boolean doValidation(AuditHeader auditHeader) throws InterruptedException {
		...
		while (retValue == PennantConstants.porcessOVERIDE) {
			...
			if (this.row_RpyAdvBaseRate.isVisible() && !this.rpyAdvRate.isBaseReadonly()
					&& this.repayEffectiveRate.getValue().compareTo(this.rpyAdvRate.getEffRateValue()) < 0) {
			...
			// BPI Validations
			if(this.alwBpiTreatment.isChecked() && !StringUtils.equals(FinanceConstants.BPI_NO, getComboboxValue(this.dftBpiTreatment))){
				String frqBPI = "";
				Date frqDate = null;
				...
				Date bpiDate = DateUtility.getDate(DateUtility.formatUtilDate(
						FrequencyUtil.getNextDate(frqBPI, 1, this.finStartDate.getValue(), HolidayHandlerTypes.MOVE_NONE, false)
	...
	public FinanceMain getFinanceMain() {
		...
		financeMain.setFinCurrAssetValue(PennantAppUtil.unFormateAmount(this.finCurrentAssetValue.getActualValue(), formatter));
	...
	protected ArrayList<WrongValueException> doWriteComponentsToBean(FinScheduleData aFinanceSchData)
		...
		if (feeDetailDialogCtrl != null) {
			feeDetailDialogCtrl.doClearErrorMessages();
		}
		...
		try {
			...
			if (this.row_FinAssetValue.isVisible()) {
				...
				if (!isBuildEvent() && ((StringUtils.isEmpty(moduleDefiner) || StringUtils.equals(FinanceConstants.FINSER_EVENT_ADDDISB, moduleDefiner)))){
		...
		if (isOverDraft) {
			...
			try {
				...
				if (this.odYearlyTerms.intValue() > 20) {
					String years = 20 + "years";
					throw new WrongValueException(this.odYearlyTerms, Labels.getLabel("NUMBER_MAXVALUE", new String[] {
							Labels.getLabel("label_FinanceMainDialog_ODTenor.value"), years }));
			...
			try {
				if (this.odMnthlyTerms.intValue() > 11) {
					String months = 11 + "months";
					throw new WrongValueException(this.odMnthlyTerms, Labels.getLabel("NUMBER_MAXVALUE", new String[] {
							Labels.getLabel("label_FinanceMainDialog_ODTenor.value"), months }));
			...
			try {
				if (this.firstDroplineDate.getValue() != null) {
					...
					if (DateUtility.compare(this.firstDroplineDate.getValue(), this.odMaturityDate.getValue()) >= 0) {
						throw new WrongValueException(this.firstDroplineDate, Labels.getLabel(
								"DATE_ALLOWED_BEFORE",
								new String[] { Labels.getLabel("label_FinanceMainDialog_DroplineDate.value"),
	...
	/*
	 * validates finAmount or FinAssetvalue based on the field visibility by  Finmax and finmin Amount from
	...
	private void executeAccounting(boolean onLoadProcess) throws Exception {
		...
		if (StringUtils.isNotBlank(this.custCIF.getValue())) {
			...
			dataSet.setModuledefiner(StringUtils.isEmpty(moduleDefiner) ? FinanceConstants.FINSER_EVENT_ORG
					: moduleDefiner);
			...
			if (getProductCode().equalsIgnoreCase(FinanceConstants.PRODUCT_ISTISNA)) {
				if (disbursementDetailDialogCtrl != null
						&& disbursementDetailDialogCtrl.getDisbursementDetails() != null
						&& disbursementDetailDialogCtrl.getDisbursementDetails().size() > 0) {

					List<FinanceDisbursement> disbList = sortDisbDetails(disbursementDetailDialogCtrl
					...
					for (FinanceDisbursement disbursement : disbList) {
						...
						if (StringUtils.isEmpty(eventCode)) {
							...
							dataSet.setGrcPftTillNow(calculateTillGrcProfit(getFinanceDetail().getFinScheduleData(),
						...
						if (!isRIAExist) {
							returnSetEntries = getEngineExecution().getAccEngineExecResults(dataSet, getAmountCodes(),
									"N", map, false, getFinanceDetail().getFinScheduleData().getFinanceType());
						} else {
		...
		if (accountingDetailDialogCtrl != null) {
			...
			if ("".equals(moduleDefiner) && StringUtils.isNotBlank(this.commitmentRef.getValue())) {
				...
				getFinanceDetail().setCmtDataSetList(
						getEngineExecution().getCommitmentExecResults(aeCommitment, commitment,
								AccountEventConstants.ACCEVENT_CMTDISB, "N", null));
				accountingDetailDialogCtrl.doFillCmtAccounting(getFinanceDetail().getCmtDataSetList(), CurrencyUtil.getFormat(commitment.getCmtCcy()));
			} else if (StringUtils.equals(FinanceConstants.PRODUCT_ODFACILITY,finMain.getProductCategory())
					&& getFinanceDetail().getFinScheduleData().getFinanceType().isDroplineOD()) {
				List<OverdraftScheduleDetail> odSchdDetail = getFinanceDetail().getFinScheduleData()
						.getOverdraftScheduleDetails();
				if (odSchdDetail != null && odSchdDetail.size() > 0) {

					OverdraftScheduleDetail od = odSchdDetail.get(0);
					Commitment commitment = new Commitment();

					commitment.setCmtReference(this.finReference.getValue());
					commitment.setCmtBranch(this.finBranch.getValue());
					commitment.setCmtCcy(this.finCcy.getValue());
					commitment.setCmtAccount(this.disbAcctId.getValidatedValue());
					commitment.setCustID(this.custID.getValue());
					commitment.setChargesAccount(this.disbAcctId.getValidatedValue());
					commitment.setCmtCharges(BigDecimal.ZERO);
			...
			//Ijarah Grace End Accounting
			if (getProductCode().equalsIgnoreCase(FinanceConstants.PRODUCT_IJARAH)
					|| getProductCode().equalsIgnoreCase(FinanceConstants.PRODUCT_FWIJARAH)) {
				Date curBDay = DateUtility.getAppDate();
				if (finMain != null
						&& StringUtils.trimToEmpty(finMain.getRecordType()).equals(PennantConstants.RECORD_TYPE_NEW)) {
					if (finMain.getGrcPeriodEndDate() != null && finMain.getGrcPeriodEndDate().compareTo(curBDay) <= 0) {
						...
						dataSet.setFeeAmount(finMain.getFeeChargeAmt() == null ? BigDecimal.ZERO : finMain
								.getFeeChargeAmt());
						dataSet.setInsAmount(finMain.getInsuranceAmt() == null ? BigDecimal.ZERO : finMain
								.getInsuranceAmt());
						if (!isRIAExist) {
							returnSetEntries = getEngineExecution().getAccEngineExecResults(dataSet, getAmountCodes(),
									"N", null, false, getFinanceDetail().getFinScheduleData().getFinanceType());
						} else {
							List<AEAmountCodesRIA> riaDetailList = getEngineExecutionRIA().prepareRIADetails(
									contributorDetailsDialogCtrl == null ? null
											: contributorDetailsDialogCtrl.getContributorsList(),
									dataSet.getFinReference());
							returnSetEntries = getEngineExecutionRIA().getAccEngineExecResults(dataSet,
									getAmountCodes(), "N", riaDetailList, null);
						}
						accountingDetailDialogCtrl.doFillGraceEndAccounting(returnSetEntries);
					}
				}
			}
	...
	/**
	 * Method for Return List of Fee Rule Details into Map
	 * 
	 * @return
	 */
	public Map<String, FeeRule> getFeeRuleDetailMap() {
		if (feeDetailDialogCtrl == null) {
			return null;
		}
		return feeDetailDialogCtrl.getFeeRuleDetailsMap();
	}

	/**
	 * Method for Sorting Disbursement Details List
	 * 
	 * @param financeDisbursement
	 * @return
	 */
	private List<FinanceDisbursement> sortDisbDetails(List<FinanceDisbursement> financeDisbursement) {
		...
		if (financeDisbursement != null && financeDisbursement.size() > 0) {
			Collections.sort(financeDisbursement, new Comparator<FinanceDisbursement>() {
				...
				public int compare(FinanceDisbursement detail1, FinanceDisbursement detail2) {
					if ((DateUtility.compare(detail1.getDisbDate(), detail2.getDisbDate()) > 1)
							&& detail1.getDisbType().compareTo(detail2.getDisbType()) > 0) {
						return 1;
					}
					return 0;
				}
	...
	private FinScheduleData doWriteSchData(FinScheduleData aFinanceSchData, boolean isIstisnaProd)
		...
		if (buildEvent) {
			...
			if (feeDetailDialogCtrl != null) {
				try {
					aFinanceSchData = feeDetailDialogCtrl.doExecuteFeeCharges(true, isFeeReExecute, aFinanceSchData,
				...
				ArrayList<WrongValueException> valueException = feeDetailDialogCtrl.doValidate(aFinanceSchData
						.getFinanceMain().getNumberOfTerms());
				if (valueException != null && !valueException.isEmpty()) {
					Tab tab = getTab(AssetConstants.UNIQUE_ID_FEES);
					if (tab != null) {
						feeDetailDialogCtrl.doRemoveConstraints();
						feeDetailDialogCtrl.doSetLabels(getFinBasicDetails());
						showFeeErrorDetails(valueException, tab);
					}
				}

				feeDetailDialogCtrl.doStoreInitValues();
				//Fee Details Data set to Bean Object
				aFinanceSchData = feeDetailDialogCtrl.doWriteComponentsToBean(aFinanceSchData, false, true);
				//Fill the Insurances listbox's data for the  amounts calculated
				feeDetailDialogCtrl.doFillFinInsurances(aFinanceSchData.getFinInsuranceList());
			}
			if (finFeeDetailListCtrl != null) {
			...
			if (!isIstisnaProd
				...
				if (aFinanceMain.getFinAssetValue().compareTo(aFinanceMain.getFinAmount()) > 0 
						&& (StringUtils.equals(FinanceConstants.PRODUCT_IJARAH, aFinanceSchData.getFinanceType().getFinCategory()) || 
						StringUtils.equals(FinanceConstants.PRODUCT_FWIJARAH, aFinanceSchData.getFinanceType().getFinCategory()))) {
			...
			if (feeDetailDialogCtrl != null) {
				...
				try {
					if (recSave) {
						feeDetailDialogCtrl.feeActId.validateValue();
						aFinanceSchData.getFinanceMain().setFeeAccountId(
								PennantApplicationUtil.unFormatAccountNumber(feeDetailDialogCtrl.feeActId.getValue()));
					} else {
						aFinanceSchData.getFinanceMain().setFeeAccountId(
								PennantApplicationUtil.unFormatAccountNumber(feeDetailDialogCtrl.feeActId
										.getValidatedValue()));
					}
				} catch (WrongValueException e) {
					valueException.add(e);
				}
				if (valueException != null && !valueException.isEmpty()) {
					Tab tab = getTab(AssetConstants.UNIQUE_ID_FEES);
					if (tab != null) {
				...
				aFinanceSchData = feeDetailDialogCtrl.doWriteComponentsToBean(aFinanceSchData, true, false);
			}
			if (finFeeDetailListCtrl != null) {
				finFeeDetailListCtrl.doExecuteFeeCharges(true, aFinanceSchData);
				//Fill the Insurances listbox's data for the  amounts calculated
	...
	private void doStoreDefaultValues() {
		...
		if (CalculationConstants.RATE_BASIS_R.equals(getComboboxValue(this.repayRateBasis))) {
			...
			if (StringUtils.isNotEmpty(this.repayRate.getBaseValue())) {
				...
				this.repayEffectiveRate.setValue(this.repayProfitRate.getValue() == null ? BigDecimal.ZERO
						: this.repayProfitRate.getValue());
			}
		}
				...
				if (isNewRecord) {
					...
					if (this.allowGrace.isChecked()) {
						if (this.grcBaseRateRow.isVisible()) {
							...
							this.grcEffectiveRate.setValue(actRate);
						}
					}
				}
			...
			if (rule != null) {
				...
				if (isnewRecord) {
					...
					if (StringUtils.isNotEmpty(repayRate.getBaseValue())) {
						...
						this.repayEffectiveRate.setValue(actRate);
					}
				}
	...
	public void closeDialog() {
		...
		if (getFeeDetailDialogCtrl() != null) {
			getFeeDetailDialogCtrl().closeDialog();
		}
	...
	public AEAmountCodes doGetFeeCustomerData(AEAmountCodes amountCodes) {
		if (customerDialogCtrl != null) {
			amountCodes = customerDialogCtrl.doGetFeeCustomerData(amountCodes);
		}
		return amountCodes;
	}
		...
		if (detail.getCustomerDetails().getCustEmployeeDetail() != null) {
			detail.getCustomerEligibilityCheck().setCustMonthlyIncome(
					PennantAppUtil.formateAmount(
							detail.getCustomerDetails().getCustEmployeeDetail().getMonthlyIncome(), CurrencyUtil.getFormat(detail.getCustomerDetails().getCustomer().getCustBaseCcy())));
	...
	private void onChangeODChargeType(boolean changeAction) {
		...
		if (getComboboxValue(this.oDChargeType).equals(PennantConstants.List_Select)) {
			...
			this.oDChargeAmtOrPerc.setFormat(PennantApplicationUtil.getAmountFormate(CurrencyUtil.getFormat(
		...
		if (validate() != null) {
			...
			if (manualSchedule.isChecked()) {
				...
				if(StringUtils.equals(FinanceConstants.FINSER_EVENT_OVERDRAFTSCHD,moduleDefiner)){
					...
					/*getFinanceDetail().setFinScheduleData(
							ScheduleCalculator.buildOverdraftSchd(getFinanceDetail().getFinScheduleData(),BigDecimal.ZERO,BigDecimal.ZERO));*/
					getFinanceDetail().setFinScheduleData(
				...
				//Setting Finance Step Policy Details to Finance Schedule Data Object
				if (getFeeDetailDialogCtrl() != null) {
					this.oldVar_finInsuranceList = getFeeDetailDialogCtrl().getFinInsuranceList();
				}
				//Setting Finance Step Policy Details to Finance Schedule Data Object
				if (getFinFeeDetailListCtrl() != null) {
				...
				if (getFinanceDetail().getFinScheduleData().getErrorDetails() != null
					...
					MessageUtil.showErrorMessage(getFinanceDetail().getFinScheduleData().getErrorDetails().get(0).getError());
	...
	private FinScheduleData rebuildODSchd(FinScheduleData finScheduleData){
		...
		for(OverdraftScheduleDetail odScheduleDetail:finScheduleData.getOverdraftScheduleDetails()){
			...
			if(DateUtility.compare(odScheduleDetail.getDroplineDate(), financeMain.getFinStartDate())==0 && limitIncreaseAmt.compareTo(BigDecimal.ZERO)>0){
				odSchd.add(odScheduleDetail);
			}else if(DateUtility.compare(odScheduleDetail.getDroplineDate(),odFinDate)>=0){
				if(isDropline && DateUtility.compare(this.firstDroplineDate.getValue(), financeMain.getFinStartDate())==0){
				...
				if(DateUtility.compare(odScheduleDetail.getDroplineDate(), financeMain.getFinStartDate())!=0){
					...
					odScheduleDetail.setODLimit(this.finAssetValue.getActualValue().subtract(odScheduleDetail.getLimitDrop()));
					odSchd.add(odScheduleDetail);
				}
			}
		}
		...
		if (finScheduleData.getOverdraftScheduleDetails() != null && finScheduleData.getOverdraftScheduleDetails().size() > 0) {
		...
		startRepayCalDate = FrequencyUtil.getNextDate(this.repayFrq.getValue(), 1, tempStartdate, "A", false, finScheduleData.getFinanceType().getFddLockPeriod())
		...
		for (int i = 0; i < finScheduleData.getFinanceScheduleDetails().size(); i++) {
			...
			if (tempStartdate.compareTo(financeMain.getGrcPeriodEndDate()) > 0) {
				if (i != 0 && curSchd.getSchDate().compareTo(tempStartdate) > 0) {
					curSchd.setCalculatedRate(finScheduleData.getFinanceScheduleDetails().get(i - 1).getCalculatedRate());
				}
			} else {
				if (curSchd.getSchDate().compareTo(financeMain.getGrcPeriodEndDate()) < 0) {
					if (i != 0 && curSchd.getSchDate().compareTo(tempStartdate) > 0) {
	...
	private ArrayList<Object> getFinBasicDetails() {
		...
		String custShrtName = getFinanceDetail().getCustomerDetails().getCustomer().getCustShrtName();
		if (customerDialogCtrl != null) {
			try {
