...
public class FinanceMainBaseCtrl extends GFCBaseCtrl<FinanceMain> {
	...
	private boolean doProcess(FinanceDetail aFinanceDetail, String tranType) throws Exception, InterfaceException {
		...
		if ("Submit".equalsIgnoreCase(this.userAction.getSelectedItem().getLabel())){
			getAutoGeneratedAgreement(aFinanceDetail);
		}
	...
	//story #491  
	//Auto Generation of Loan Agreements while submitting
	private void getAutoGeneratedAgreement(FinanceDetail aFinanceDetail) throws Exception {
		logger.debug("Entering");
		List<DocumentDetails>  newDocList = new ArrayList<DocumentDetails>();
		...
		newDocList.addAll(aFinanceDetail.getDocumentDetailsList());
		aFinanceDetail.setDocumentDetailsList(newDocList);
	...
	private DocumentDetails autoGenerateAgreement(FinanceReferenceDetail data ,FinanceDetail detail,AgreementDefinition agreementDefinition) throws Exception 
	{
		...
		try {
			if (detail != null && detail.getFinScheduleData() != null
				...
				// tasks #504 Auto Download of Agreements
				...
				if (exstDetails != null) {
					...
					if (PennantConstants.DOC_TYPE_PDF.equals(agreementDefinition.getDocType()))
				...
				details.setDocReceivedDate(data.getLastMntOn());
				details.setVersion(1);
				details.setFinEvent(data.getFinEvent());
