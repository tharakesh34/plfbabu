...
public class FinanceMainBaseCtrl extends GFCBaseCtrl<FinanceMain> {
	...
	/**
	 * Method for Checking Conditions for Displaying Tab on particular Stage or
	 * not
	 * 
	 * @param tabID
	 * @return
	...
	public void doWriteBeanToComponents(FinanceDetail aFinanceDetail, boolean onLoadProcess) throws ParseException,
		...
		/*
		 * if
		 * (CollectionUtils.isNotEmpty(getFinanceDetail().getFinScheduleData().
		 * getLowerTaxDeductionDetails())) { for (LowerTaxDeduction list :
		 * getFinanceDetail().getFinScheduleData().getLowerTaxDeductionDetails()
		 * ) { if (list.getSeqno() == 1) {
		 * this.tDSPercentage.setValue(getFinanceDetail().getFinScheduleData().
		 ...
		 * this.tDSLimitAmt
		 * .setValue(PennantApplicationUtil.formateAmount(aFinanceMain.
		 * getTdsLimitAmt(), format)); } } }
		 */
		...
		if (aFinanceDetail.getRolledoverFinanceHeader() != null) {
			...
			this.custPaymentAmount
					.setValue(PennantApplicationUtil.formateAmount(rolledoverFinanceHeader.getCustPayment(), format));
			...
			this.latePayAmount
					.setValue(PennantApplicationUtil.formateAmount(rolledoverFinanceHeader.getLatePayAmount(), format));
			this.latePayWaiverAmount.setValue(
					PennantApplicationUtil.formateAmount(rolledoverFinanceHeader.getLatePayWaiverAmount(), format));
			...
			for (int i = 0; i < rolledoverFinanceHeader.getRolledoverFinanceDetails().size(); i++) {
				...
				lc = new Listcell(PennantApplicationUtil
						.amountFormate(detail.getFinAmount().add(detail.getTotalProfit()), format));
				listitem.appendChild(lc);
	...
	/**
	 * Method for Displaying Supplementary Rent & increased Cost for Ijarah
	 * product
	 * 
	 * @param financeMain
	 * @param isGrace
	...
	public boolean isSchdlRegenerate() {
		 ...
		 * feeDetailDialogCtrl.isDataChanged(false)) { return true; }
		 * 
		 * if (feeDetailDialogCtrl != null &&
		 * feeDetailDialogCtrl.getFinInsuranceList() != oldVar_finInsuranceList)
		 * { return true; }
		 */
	...
	protected void doSetValidation() {
			...
			if (!this.gracePeriodEndDate.isReadonly() && this.manualSchedule.isChecked()) {
				...
				if (!this.gracePeriodEndDate.isReadonly()
					...
					for (int i = 1; i < scheduelist.size(); i++) {
						 ...
						 * financeMain.isAllowGrcCpz() &&
						 * curSchd.getSchDate().compareTo(financeMain.
						 * getNextGrcCpzDate()) <= 0) { validFrom =
						 * curSchd.getSchDate(); continue; } if
						 * (financeMain.getNextGrcPftRvwDate() != null &&
						 * financeMain.isAllowGrcPftRvw() &&
						 * curSchd.getSchDate().compareTo(financeMain.
						 * getNextGrcPftRvwDate()) <= 0) { validFrom =
						 * curSchd.getSchDate(); continue; } if
						 * (financeMain.getNextGrcPftDate() != null &&
						 * curSchd.getSchDate().compareTo(financeMain.
						 * getNextGrcPftDate()) <= 0) { validFrom =
		 ...
		 * !this.supplementRent.isDisabled()) {
		 * this.supplementRent.setConstraint(new
		 * PTDecimalValidator(Labels.getLabel(
		 * "label_FinanceMainDialog_SupplementRent.value"), 3, false)); } if
		 * (this.row_supplementRent.isVisible() &&
	...
	public void doSave() throws Exception {
		...
		if (financialSummaryDialogCtrl != null) {
			aFinanceDetail.setDealRecommendationMeritsDetailsList(
		...
		if (ImplementationConstants.ALLOW_DEVIATIONS) {
			if (StringUtils.isEmpty(moduleDefiner) && deviationDetailDialogCtrl != null) {
				...
				if (this.userAction.getSelectedItem() != null
					 ...
					 * "";
					 * 
					 * for (String deviation : pendingDecisions) { errorMessage
					 * = errorMessage.concat("\n - " + deviation); }
					 * 
					 * if ("Cancel".equalsIgnoreCase(this.userAction.
					 * getSelectedItem().getLabel()) ||
					 * this.userAction.getSelectedItem().getLabel().contains(
					 * "Reject") ||
					 * this.userAction.getSelectedItem().getLabel().contains(
					 * "Resubmit") ||
					 * this.userAction.getSelectedItem().getLabel().contains(
					 ...
					 * { return; } } else { errorMessage =
					 * "Please mark your decision for the below deviations."
					 * .concat(errorMessage);
					 * 
					...
					// Not Required to invoke the for the reject and resubmit
					// stage.
					// Please mark your decision validation not required for the
					// each stage.
					if (!("Cancel".equalsIgnoreCase(this.userAction.getSelectedItem().getLabel())
							|| this.userAction.getSelectedItem().getLabel().contains("Reject")
							|| this.userAction.getSelectedItem().getLabel().contains("Resubmit")
	...
	/**
	 * This method is for updating Review frequency with latest data based on
	 * base rate code against frequency date if exists.
	 * 
	 * @param baseRateCode
	 */
				...
				if (StringUtils.equals(listcell.getLabel(), FinanceConstants.REPAYMTH_PDC)) {
					...
					for (FinanceScheduleDetail finSchdDetail : schdList) {
						if (DateUtility.compare(emiDate, finSchdDetail.getSchDate()) == 0) {
							...
							if (finSchdDetail.getTDSAmount() != null
									&& finSchdDetail.getTDSAmount().compareTo(BigDecimal.ZERO) > 0) {
								repayAmount = repayAmount.subtract(finSchdDetail.getTDSAmount());
							}
	...
	/**
	 * This method is for updating frequency with latest data based on finance
	 * start date
	 */
	...
	/*
	 * on check manual Schedule ,if Grace is Checked the visibility of the Grace
	 * fields
	 */
	...
	/**
	 * Method for Setting Variables on Schedule Tab based on selected Planned
	 * EMI Holiday Method
	 * 
	 * @param isAction
	 */
	...
	/**
	 * Payment Account field Validation compare to Secondary Account To Avaid
	 * dublication compare to Secondary Account
	 * 
	 * @param event
	 */
	...
	/**
	 * Method for getting selected Step Type Using In Step Detail Dialog
	 * Controller for EMI Validation
	 * 
	 * @return
	 */
			...
			if (StringUtils.equals(financeType.getFinDivision(), FinanceConstants.FIN_DIVISION_RETAIL)) {
				...
				if (maxInstAlwDays != -1) {
					...
					if (diffDays > maxInstAlwDays) {
						...
						/*
						 * Date nextRepayDate =
						 * DateUtility.addDays(this.finStartDate.getValue(),
						 * maxInstAlwDays); if(this.allowGrace.isChecked()) {
						 * nextRepayDate =
						 * DateUtility.addDays(this.gracePeriodEndDate_two.
						 * getValue(), maxInstAlwDays); } errorList.add(new
						 * ErrorDetails("nextRepayDate_two", "30547", new
						 * String[] {
						 * Labels.getLabel("label_WIFinNextRepaymentDate"),
						 * String.valueOf(maxInstAlwDays),
						 * DateUtility.formatToLongDate(nextRepayDate)}, new
						 * String[] {}));
						 */
					}
				}
	...
	/**
	 * Method for Returning Finance Amount and Currency Data for Contributor
	 * Validation
	 * 
	 * @return
	 */
	...
	protected ArrayList<WrongValueException> doWriteComponentsToBean(FinScheduleData aFinanceSchData)
		...
		/*
		 * try { if (StringUtils.isNotEmpty(financeType.getProduct())) { //
		 * calculate max allowed future Promotion Date Date prmotionEndDate =
		 * DateUtility.addDays(financeType.getEndDate(),
		 * SysParamUtil.getValueAsInt("ALWPROMO_MAX_FUTUREDAYS")); if
		 * (this.finStartDate.getValue() != null) { if
		 * (this.finStartDate.getValue().compareTo(financeType.getStartDate()) <
		 * 0 || this.finStartDate.getValue().compareTo(prmotionEndDate) > 0) {
		 * throw new WrongValueException(this.finStartDate, Labels.getLabel(
		 * "DATE_ALLOWED_RANGE", new String[] {
		 * Labels.getLabel("label_FinStartDate"),
		 * DateUtility.formatToShortDate(financeType.getStartDate()),
		 * DateUtility.formatToShortDate(prmotionEndDate) })); } } } } catch
		...
		try {
			...
			aFinanceMain.setSecurityDeposit(
					PennantApplicationUtil.unFormateAmount(this.securityDeposit.getActualValue(), formatter));
		...
		try {
			aFinanceMain.setSupplementRent(
					PennantApplicationUtil.unFormateAmount(this.supplementRent.getActualValue(), formatter));
		} catch (WrongValueException we) {
			wve.add(we);
		}
		try {
			aFinanceMain.setIncreasedCost(
					PennantApplicationUtil.unFormateAmount(this.increasedCost.getActualValue(), formatter));
		} catch (WrongValueException we) {
			wve.add(we);
		}
		...
		try {
			if (!this.nextRollOverDate.isReadonly() && StringUtils.isNotEmpty(this.rolloverFrq.getValue())) {
				...
				if (StringUtils.isNotEmpty(this.rolloverFrq.getValue())
						&& FrequencyUtil.validateFrequency(this.rolloverFrq.getValue()) == null) {
					aFinanceMain.setNextRolloverDate(DateUtility.getDate(
							DateUtility.format(this.nextRollOverDate_two.getValue(), PennantConstants.dateFormat)));
				}
				// Validation Against the Repay Frequency and the next Frequency
				// Date
		...
		if (this.gb_RolloverFinance.isVisible()) {
			...
			try {
				rolledoverFinanceHeader.setCustPayment(
						PennantApplicationUtil.unFormateAmount(this.custPaymentAmount.getActualValue(), formatter));
			} catch (WrongValueException we) {
				wve.add(we);
			}
			try {
			...
			try {
				rolledoverFinanceHeader.setLatePayAmount(
						PennantApplicationUtil.unFormateAmount(this.latePayAmount.getActualValue(), formatter));
			} catch (WrongValueException we) {
				wve.add(we);
			}
			try {
				...
				rolledoverFinanceHeader.setLatePayWaiverAmount(
						PennantApplicationUtil.unFormateAmount(this.latePayWaiverAmount.getActualValue(), formatter));
			} catch (WrongValueException we) {
				wve.add(we);
			}
		...
		if (wve.isEmpty()) {
			...
			// 133697:Schedule Error during grace period change
			if (!moduleDefiner.equals(FinanceConstants.FINSER_EVENT_CHGGRCEND)) {
	...
	/*
	 * validates finAmount or FinAssetvalue based on the field visibility by
	 * Finmax and finmin Amount from loan type configuration.
	 */
	...
	private void executeAccounting(boolean onLoadProcess) throws Exception {
		...
		if (StringUtils.isBlank(moduleDefiner)) {
			 ...
			 * getFinanceDetailService().prepareGstMappingDetails(
		...
		if (eventCode.equals(AccountEventConstants.ACCEVENT_ADDDBS)
			...
			if (!SysParamUtil.isAllowed(SMTParameterConstants.HOLD_DISB_INST_POST)) {
				accountingSetEntries.addAll(disbursementPostings
						.getDisbPosting(getFinanceDetail().getAdvancePaymentsList(), finScheduleData.getFinanceMain()));
			}
		...
		if (StringUtils.isEmpty(moduleDefiner)) {
			...
			/*
			 * accountingSetEntries.addAll(advancePaymentService.
			 * processBackDatedAdvansePayments(getFinanceDetail(), profitDetail,
			 * appDate, "", false));
			 */
		}
	...
	private void prepareFeeRulesMap(AEAmountCodes amountCodes, Map<String, Object> dataMap) {
		...
		/*
		 * Setting the balance up-front fee amount to excess amount for
		 * accounting purpose
		 */
	...
	private void doChangeTerms() {
		...
		if (this.maturityDate.getValue() != null && !isOverdraft) {
			...
			if (singleTermFinance) {
				 ...
				 * if(nextPftDate == null){ nextPftDate =
				 * FrequencyUtil.getNextDate(this.repayPftFrq.getValue(), 1,
				 * this.gracePeriodEndDate_two.getValue(),
				 * HolidayHandlerTypes.MOVE_NONE, false,
				 * true).getNextFrequencyDate(); }
				 * 
				 * this.numberOfTerms_two.setValue(FrequencyUtil.getTerms(this.
				 * repayPftFrq.getValue(), nextPftDate,
		...
		if (this.numberOfTerms_two.intValue() != 0 && !singleTermFinance && !isRollover
			 ...
			 * if(nextPftDate == null){ nextPftDate =
			 * FrequencyUtil.getNextDate(this.repayPftFrq.getValue(), 1,
			 * this.gracePeriodEndDate_two.getValue(),HolidayHandlerTypes.
			 * MOVE_NONE, false, true).getNextFrequencyDate(); }
			 * 
			 * scheduleDateList =
			 * FrequencyUtil.getNextDate(this.repayPftFrq.getValue(),
			 * this.numberOfTerms_two.intValue(), nextPftDate,
			 * HolidayHandlerTypes.MOVE_NONE, true, false).getScheduleList();
			 * }else{
			 */
						...
						.setValue(
								FrequencyUtil
										.getNextDate(this.repayRvwFrq.getValue(), 1,
												DateUtility.addMonths(this.gracePeriodEndDate_two.getValue(),
														fixedTenor - 1),
												HolidayHandlerTypes.MOVE_NONE, false,
												this.allowGrace.isChecked() ? 0 : financeType.getFddLockPeriod())
										.getNextFrequencyDate());
	...
	protected void doEdit() {
		 ...
		 * readOnlyComponent(isReadOnly("FinanceMainDialog_finAssetValue"),
		 * this.finAssetValue); this.row_FinAssetValue.setVisible(true); } else
		...
		/**
		 * Enabling the sampling required flag for the role who is having the
		 * sampling required
		 */
		...
		/**
		 * Enabling the Legal required flag for the role who is having the Legal
		 * required
		 */
	...
	protected void doEditGrace(FinScheduleData finScheduleData) {
		...
		/*
		 * readOnlyComponent(isReadOnly("FinanceMainDialog_graceBaseRate"),
		 * this.graceRate.getBaseComp());
		 * readOnlyComponent(isReadOnly("FinanceMainDialog_graceSpecialRate"),
		 * this.graceRate.getSpecialComp());
		 * readOnlyComponent(isReadOnly("FinanceMainDialog_grcMargin"),
		 * this.graceRate.getMarginComp());
		 */
	...
	/**
	 * Executes the down payment rule from finance type and set the minimum down
	 * payment percentage required for finance
	 * 
	 */
	...
	/*
	 * setting the Grace rate or grace margin based on the rule execution on the
	 * pricing policy desc in financetype.
	 */
	...
	/*
	 * setting the repay Actual rate or repay margin based on the rule execution
	 * on the pricing policy desc in financetype.
	 */
			...
			if (this.finAmount.getActualValue() != null
				...
				this.downPayBank
						.setValue(PennantApplicationUtil.formateAmount(
								PennantApplicationUtil.getPercentageValue(PennantApplicationUtil.unFormateAmount(
										this.finAmount.getActualValue(), formatter), financeMain.getMinDownPayPerc()),
								formatter));
			} else {
				this.downPayBank.setValue(BigDecimal.ZERO);
			}
	...
	/*
	 * Validation in overdraft Maintenance if the curfinasset value is less than
	 * the finasset
	 */
	...
	/**
	 * Validation check for Commitment For Available Amount and Expiry Date
	 * Check
	 * 
	 * @param aFinanceDetail
	 * @return
	...
	/**
	 * Method for Setting Mandatory Check to Repay Account ID based on Repay
	 * Method
	 * 
	 * @param event
	 */
	...
	public FinanceDetail prepareCustElgDetail(Boolean isLoadProcess) {
		...
		if (toYear > 0) {
			...
			dataMap = creditReviewSummaryData.setDataMap(custId, custIds, toYear, noOfYears, customer.getCustCtgCode(),
					true, true, extValuesMap, creditApplicationReviewService
							.getCreditRevCategoryByCreditRevCode(customerDetails.getCustomer().getCustCtgCode()));
		}
	...
	/**
	 * This method is to fetch EID Number and calling it from
	 * DocumentTypeSelectDialogCtrl and DocumentDetailDialogCtrl controllers
	 * when document type is 01.
	 * 
	 */
	...
	/*
	 * public void onCheck$oDIncGrcDays(Event event){
	 * logger.debug(Literal.ENTERING + event.toString());
	 * if(this.oDIncGrcDays.isChecked()){
	 * readOnlyComponent(isReadOnly("FinanceMainDialog_oDGraceDays"),
	 * this.oDGraceDays); }else{ readOnlyComponent(true, this.oDGraceDays); }
	 * logger.debug(Literal.LEAVING + event.toString()); }
	 */
		...
		if (validate() != null) {
			...
			if (manualSchedule.isChecked()) {
				if (manualScheduleDetailDialogCtrl != null) {
					...
					&& (StringUtils.isNotEmpty(financeMain.getDroplineFrq())
	...
	/**
	 * Method to Rebuild the Schedule and recalculate the Overdraft Schedule
	 * Details
	 * 
	 * @throws InterruptedException
	 */
	...
	/*
	 * Set the Maturitydate on change of the tenor, droplinedate and
	 * droplinefrequency
	 */
	...
	/**
	 * Method for calculation of maturity Date based on Dropline frequency &
	 * Tenor mentioned.
	 */
