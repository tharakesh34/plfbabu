...
public class FinanceMainBaseCtrl extends GFCBaseCtrl<FinanceMain> {
	...
	public void doWriteBeanToComponents(FinanceDetail aFinanceDetail, boolean onLoadProcess) throws ParseException,
		...
		/*if (CollectionUtils.isNotEmpty(getFinanceDetail().getFinScheduleData().getLowerTaxDeductionDetails())) {
			for (LowerTaxDeduction list : getFinanceDetail().getFinScheduleData().getLowerTaxDeductionDetails()) {
				if (list.getSeqno() == 1) {
					this.tDSPercentage.setValue(getFinanceDetail().getFinScheduleData().getLowerTaxDeductionDetails()
							.get(0).getTdsPercentage());
	...
	protected ArrayList<WrongValueException> doWriteComponentsToBean(FinScheduleData aFinanceSchData)
		...
		if (this.allowGrace.isChecked()) {
			...
			try {
				aFinanceMain
						.setGrcMaxAmount(PennantAppUtil.unFormateAmount(this.grcMaxAmount.getActualValue(), formatter));
			} catch (WrongValueException we) {
				wve.add(we);
			}
	...
	private void executeAccounting(boolean onLoadProcess) throws Exception {
		...
		if (StringUtils.isBlank(moduleDefiner)) {
			...
			Map<String, Object> gstExecutionMap = getFinanceDetailService().prepareGstMappingDetails(getFinanceDetail(),
					branch);
			...
			List<FinServiceInstruction> serviceInsts = getFinanceDetail().getFinScheduleData()
