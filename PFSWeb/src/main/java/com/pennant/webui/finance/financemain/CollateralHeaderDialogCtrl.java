...
public class CollateralHeaderDialogCtrl extends GFCBaseCtrl<CollateralAssignment> {
	...
	private void doCheckRights() {
		...
		this.btnNew_CollateralAssignment
				.setVisible(getUserWorkspace().isAllowed("button_CollateralAssignmentDialog_btnNew"));
	...
	public void onClick$btnNew_CollateralAssignment(Event event) {
		...
		Executions.createComponents("/WEB-INF/pages/Collateral/CollateralAssignment/CollateralAssignmentDialog.zul",
				window_CollateralAssignmentDialog, map);
	...
	public void onClick$btnNew_AssetType(Event event) {
		...
		Executions.createComponents("/WEB-INF/pages/AssetType/AssetTypeAssignmentDialog.zul",
				window_CollateralAssignmentDialog, map);
	...
	public void doFillAssetDetails(List<ExtendedFieldRender> extendedFieldRenderList) throws AppException {
		...
		if (extendedFieldRenderList != null && !extendedFieldRenderList.isEmpty()) {
			...
			for (int i = 0; i < extendedFieldRenderList.size(); i++) {
				...
				// Asset Value
				lc = new Listcell();
				lc.setLabel(PennantApplicationUtil.amountFormate(unitPrice.multiply(new BigDecimal(noOfunits)),
						getFormat()));
				lc.setStyle("text-align:right;");
				item.appendChild(lc);
		...
		if (fromAssignment) {
			...
			if (rcuVerificationDialogCtrl != null) {
				rcuVerificationDialogCtrl.addCollateralDocuments(collateralAssignments,
						getFinanceDetail().getCollaterals());
			}
	...
	public void updateShortfall(BigDecimal utilizedAmount) {
		...
		if (utilizedAmount != null && getCollateralAssignments() != null && !getCollateralAssignments().isEmpty()) {
			for (CollateralAssignment colAssignment : getCollateralAssignments()) {
				...
				BigDecimal curAssignValue = (colAssignment.getBankValuation().multiply(colAssignment.getAssignPerc()))
	...
	public void onCollateralAssignItemDoubleClicked(Event event) {
		...
		if (item != null) {
			...
			if (StringUtils.equalsIgnoreCase(PennantConstants.RECORD_TYPE_CAN,
					StringUtils.trimToEmpty(assignment.getRecordType()))
