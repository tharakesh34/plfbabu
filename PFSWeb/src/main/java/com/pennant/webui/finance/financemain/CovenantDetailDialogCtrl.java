 ...
 * file.
 */
public class CovenantDetailDialogCtrl extends GFCBaseCtrl<FinCovenantType> {
	private static final long serialVersionUID = 1L;
	private static final Logger logger = Logger.getLogger(CovenantDetailDialogCtrl.class);
	...
	private CovenantDetailListCtrl covenantDetailListCtrl;
	...
	protected boolean covenantDetail= false;

	/**
	 * default constructor.<br>
	 */
	public CovenantDetailDialogCtrl() {
		super();
	}
	...
	public void onCreate$window_FinCovenantTypeDialog(Event event) throws Exception {
		...
		// READ OVERHANDED params !
			...
			if (arguments.containsKey("covenantDetailListCtrl")) {
				setCovenantDetailListCtrl((CovenantDetailListCtrl) arguments.get("covenantDetailListCtrl"));
			}
			if (arguments.containsKey("covenantDetail")) {
				covenantDetail = (Boolean) arguments.get("covenantDetail");
				setNewCustomer(true);
	...
	public void doShowDialog(FinCovenantType aFinCovenantType) throws InterruptedException {
		...
		if (aFinCovenantType.isNew()) {
			...
			if (isNewFinance()) {
				...
				if(!getFinCovenantType().getRecordStatus().equals(PennantConstants.RCD_STATUS_APPROVED) && !newRecord){
	...
	private AuditHeader newFinCovenantTypeProcess(FinCovenantType afinCovenantTypes, String tranType) {
		...
		if (getCovenantDetailListCtrl().getFinCovenantTypeDetailList() != null
				&& getCovenantDetailListCtrl().getFinCovenantTypeDetailList().size() > 0) {
			for (int i = 0; i < getCovenantDetailListCtrl().getFinCovenantTypeDetailList().size(); i++) {
				FinCovenantType loanDetail = getCovenantDetailListCtrl().getFinCovenantTypeDetailList().get(i);
				...
				if (StringUtils.equals(afinCovenantTypes.getCovenantType(), loanDetail.getCovenantType())) { // Both
					...
					if (tranType == PennantConstants.TRAN_DEL) {
						if (afinCovenantTypes.getRecordType().equals(PennantConstants.RECORD_TYPE_UPD)) {
							...
							recordAdded = true;
							for (int j = 0; j < getCovenantDetailListCtrl().getFinancedetail().getCovenantTypeList()
									.size(); j++) {
								FinCovenantType detail = getCovenantDetailListCtrl().getFinancedetail()
	...
	public CovenantDetailListCtrl getCovenantDetailListCtrl() {
		return covenantDetailListCtrl;
	}

	public void setCovenantDetailListCtrl(CovenantDetailListCtrl covenantDetailListCtrl) {
		this.covenantDetailListCtrl = covenantDetailListCtrl;
	}
