...
public class AdvPftRateChangeDialogCtrl extends GFCBaseCtrl<FinScheduleData> {
	...
	private static final Logger logger = LogManager.getLogger(AdvPftRateChangeDialogCtrl.class);
		...
		try {
			if (isValidComboValue(this.cbRateChangeFromDate,
				...
				getFinScheduleData().getFinanceMain()
						.setEventFromDate((Date) this.cbRateChangeFromDate.getSelectedItem().getValue());
			}
		} catch (WrongValueException we) {
			wve.add(we);
		}
				...
				if (((Date) this.cbRateChangeToDate.getSelectedItem().getValue())
					...
					throw new WrongValueException(this.cbRateChangeToDate,
							Labels.getLabel("DATE_ALLOWED_AFTER",
									new String[] { Labels.getLabel("label_AdvPftRateChangeDialog_ToDate.value"),
											Labels.getLabel("label_AdvPftRateChangeDialog_FromDate.value") }));
				} else {
					getFinScheduleData().getFinanceMain()
							.setEventToDate((Date) this.cbRateChangeToDate.getSelectedItem().getValue());
				}
			}
		} catch (WrongValueException we) {
			wve.add(we);
		...
		for (FinanceScheduleDetail curSchd : schDetailList) {
			if (curSchd.getSchDate().compareTo(financeMain.getEventFromDate()) >= 0
				...
				if (StringUtils.isEmpty(this.baseRate.getValue())) {
					curSchd.setAdvPftRate(
							this.rateChange.getValue() == null ? BigDecimal.ZERO : this.rateChange.getValue());
					...
					curSchd.setAdvMargin(
							this.marginRate.getValue() == null ? BigDecimal.ZERO : this.marginRate.getValue());
				}
			}
		}
	...
	private void doSetValidation() {
		...
		if (this.rateChange.isVisible()) {
			this.rateChange.setConstraint(new PTDecimalValidator(
					Labels.getLabel("label_AdvPftRateChangeDialog_Rate.value"), 9, false, false, 0, 9999));
		}
		if (this.marginRateRow.isVisible()) {
			this.marginRate.setConstraint(
					new PTDecimalValidator(Labels.getLabel("label_AdvPftRateChangeDialog_MarginRate.value"), 9, false));
		}
		if (this.baseRateRow.isVisible()) {
			this.baseRate.setConstraint(
					new PTStringValidator(Labels.getLabel("label_AdvPftRateChangeDialog_BaseRate.value"), null, false));
		}
		logger.debug("Leaving");
	}
