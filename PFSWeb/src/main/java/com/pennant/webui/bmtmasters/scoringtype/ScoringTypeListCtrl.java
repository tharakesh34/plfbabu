...
/**
 * This is the controller class for the /WEB-INF/pages/RulesFactory/ScoringType/ScoringTypeList.zul file.
 */
public class ScoringTypeListCtrl extends GFCBaseListCtrl<ScoringType> {
	...
	private static final Logger logger = LogManager.getLogger(ScoringTypeListCtrl.class);
	...
	private void doCheckRights() {
		...
		this.button_ScoringTypeList_NewScoringType
				.setVisible(getUserWorkspace().isAllowed("button_ScoringTypeList_NewScoringType"));
		this.button_ScoringTypeList_ScoringTypeSearchDialog
				.setVisible(getUserWorkspace().isAllowed("button_ScoringTypeList_ScoringTypeFindDialog"));
		this.button_ScoringTypeList_PrintList
				.setVisible(getUserWorkspace().isAllowed("button_ScoringTypeList_PrintList"));
		logger.debug("Leaving");
	}
	...
	public void onScoringTypeItemDoubleClicked(Event event) {
		...
		if (item != null) {
			...
			if (scoringType == null) {
				...
				if (isWorkFlowEnabled()) {
					String whereCond = " AND ScoType='" + scoringType.getScoType() + "' AND version="
							+ scoringType.getVersion() + " ";

					boolean userAcces = validateUserAccess(scoringType.getWorkflowId(),
							getUserWorkspace().getLoggedInUser().getUserId(), "ScoringType", whereCond,
							scoringType.getTaskId(), scoringType.getNextTaskId());
					if (userAcces) {
	...
	private void showDetailView(ScoringType aScoringType) {
		 ...
		 * listBox ListModel. This is fine for synchronizing the data in the ScoringTypeListbox from the dialog when we
		 * do a delete, edit or insert a ScoringType.
		 */
		map.put("scoringTypeListCtrl", this);

		// call the ZUL-file with the parameters packed in a map
		try {
			Executions.createComponents("/WEB-INF/pages/RulesFactory/ScoringType/ScoringTypeDialog.zul", null, map);
		} catch (Exception e) {
	...
	public void onClick$button_ScoringTypeList_ScoringTypeSearchDialog(Event event) {
		...
		/*
		 * we can call our ScoringTypeDialog ZUL-file with parameters. So we can call them with a object of the selected
		 * ScoringType. For handed over these parameter only a Map is accepted. So we put the ScoringType object in a
		 * HashMap.
		 */
		Map<String, Object> map = getDefaultArguments();
		...
		// call the ZUL-file with the parameters packed in a map
		try {
			Executions.createComponents("/WEB-INF/pages/RulesFactory/ScoringType/ScoringTypeSearchDialog.zul", null,
					map);
		} catch (Exception e) {
			MessageUtil.showError(e);
		}
