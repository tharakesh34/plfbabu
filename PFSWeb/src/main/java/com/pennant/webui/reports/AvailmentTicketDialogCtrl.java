...
/**
 * This is the controller class for the /WEB-INF/pages/Reports/AvailmentTicketDialog.zul file.
 */
public class AvailmentTicketDialogCtrl extends GFCBaseCtrl<Customer> {
	...
	private void printTicket() throws InterfaceException, InterruptedException {
		...
		availCustomer.setAmount(PennantApplicationUtil.amountFormate(
				PennantApplicationUtil.unFormateAmount(this.custAmount.getActualValue(), ccyformatt), ccyformatt));
		...
		availCustomer
				.setTolerance((this.tolerance.longValue() == 0 ? "000" : this.tolerance.longValue()) + " % "
						+ PennantApplicationUtil.amountFormate(
								PennantApplicationUtil.unFormateAmount(this.toleranceVal.getValue(), ccyformatt),
								ccyformatt));
		...
		try {
			...
			detail = getCustomerInterfaceService().fetchAvailCustDetails(detail, newExposure,
					this.custDftCcy.getValue());
			...
			if (detail.getAvailLimit() != null) {
				...
				BigDecimal curExposure = CalculationUtil.getConvertedAmount(this.custDftCcy.getValue(),
						detail.getAvailLimit().getLimitCcy(), newExposure);

				availCustomer.setLimitCcy(detail.getAvailLimit().getLimitCcy());
				availCustomer.setAmountInLimitCcy(
						PennantApplicationUtil.amountFormate(curExposure, detail.getAvailLimit().getLimitCcyEdit()));

				curExposure = curExposure.multiply(new BigDecimal(this.tolerance.longValue()))
						.divide(new BigDecimal(100), 0, RoundingMode.HALF_DOWN);
				availCustomer.setTolerance((this.tolerance.longValue() == 0 ? "000"
						: StringUtils.leftPad(String.valueOf(this.tolerance.longValue()), 3, '0')) + " % "
						+ PennantApplicationUtil.amountFormate(curExposure, detail.getAvailLimit().getLimitCcyEdit()));
			}
		...
		if (detail.getAvailLimit() != null) {
			...
			pastDue = getCustomerDetailsService().getCustPastDueDetailByCustId(pastDue,
					detail.getAvailLimit().getLimitCcy());
			AvailLimit availLimit = detail.getAvailLimit();

			if (pastDue != null) {
				BigDecimal amt = PennantApplicationUtil.unFormateAmount(pastDue.getPastDueAmount(),
						detail.getAvailLimit().getLimitCcyEdit());
				availLimit.setPastdueAmount(
						PennantApplicationUtil.amountFormate(amt, detail.getAvailLimit().getLimitCcyEdit()));
				...
				availLimit.setPastdueAmount(PennantApplicationUtil.amountFormate(BigDecimal.ZERO,
						detail.getAvailLimit().getLimitCcyEdit()));
	...
	private List<AvailCommitment> getCommitmentDetails(AvailLimit availLimit) {
		...
		if (this.gb_cmtDetails.isVisible() && this.listBoxCmtDetails.getItemCount() > 0) {
			...
			for (Listitem listitem : items) {
				...
				if (listitem.getFellowIfAny("cb_" + orgData.getCmtReference()) != null) {
					...
					if (isNewDeal) {
						actualenteredAmt = CalculationUtil.getConvertedAmountASString(this.custDftCcy.getValue(),
								orgData.getCmtCcy(),
								PennantApplicationUtil.unFormateAmount(this.custAmount.getActualValue(), ccyformatt));
					}
				...
				orgData.setCmtAmount(PennantApplicationUtil.amountFormate(new BigDecimal(orgData.getCmtAmount()),
						orgData.getCcyEditField()));
				orgData.setCmtUtilizedAmount(PennantApplicationUtil
						.amountFormate(new BigDecimal(orgData.getCmtUtilizedAmount()), orgData.getCcyEditField()));

				if (isNewDeal) {

					BigDecimal actualenteredAmt = CalculationUtil.getConvertedAmount(this.custDftCcy.getValue(),
							orgData.getCmtCcy(),
							PennantApplicationUtil.unFormateAmount(this.custAmount.getActualValue(), ccyformatt));
				...
				if (StringUtils.isBlank(orgData.getCmtExpDate())) {
					...
					Date date = DateUtility.parse(orgData.getCmtExpDate(), PennantConstants.DBDateTimeFormat);
					if (date.compareTo(DateUtility.getAppDate()) < 0) {
						orgData.setCmtExpFlag("T");
					}

					orgData.setCmtExpDate(DateUtility.formatToLongDate(
							DateUtility.parse(orgData.getCmtExpDate(), PennantConstants.DBDateTimeFormat)));
				}
				...
				List<AvailFinance> availFinances = getFinanceDetailService()
						.getFinanceDetailByCmtRef(orgData.getCmtReference(), 0);
				for (AvailFinance availFinance : availFinances) {
					...
					availFinance
							.setFinReference(availFinance.getFinType().concat(" " + availFinance.getFinReference()));
					...
					finCmt.setFinAmount(PennantApplicationUtil
							.amountFormate(new BigDecimal(availFinance.getFinAmount()), formatter));
					finCmt.setDrawnPrinciple(PennantApplicationUtil
							.amountFormate(new BigDecimal(availFinance.getDrawnPrinciple()), formatter));

					BigDecimal finamtBHD = (new BigDecimal(availFinance.getDrawnPrinciple())
							.multiply(CurrencyUtil.getExChangeRate(availFinance.getFinCcy())))
									.multiply(new BigDecimal(Math.pow(10, 3 - formatter)));
					finCmt.setFinAmtBHD(PennantApplicationUtil.amountFormate(finamtBHD, 3));

					String outStandBHD = CalculationUtil.getConvertedAmountASString(availFinance.getFinCcy(), ccy,
							new BigDecimal(availFinance.getOutStandingBal()));
					finCmt.setOutStandingBal(outStandBHD);

					/*
					 * BigDecimal finamtBHD = (new
					 * BigDecimal(availFinance.getDrawnPrinciple()).multiply(availFinance.getCcySpotRate())).multiply(
					 * new BigDecimal(Math.pow(10,3 - availFinance.getCcyEditField())));
					 * finCmt.setFinAmtBHD(PennantApplicationUtil.amountFormate(finamtBHD,3));
					 * 
					 * BigDecimal outStandBHD = (new
					 * BigDecimal(availFinance.getOutStandingBal()).multiply(availFinance.getCcySpotRate())).multiply(
					 * new BigDecimal(Math.pow(10,3 - availFinance.getCcyEditField())));
					 * finCmt.setOutStandingBal(PennantApplicationUtil.amountFormate(outStandBHD,3));
					 */
					finCmt.setLastRepay(availFinance.getLastRepay() == null ? ""
							: DateUtility.formatToLongDate(DateUtility.parse(availFinance.getLastRepay(),
									PennantConstants.DBDateTimeFormat)));
					finCmt.setMaturityDate(DateUtility.formatToLongDate(DateUtility
							.parse(availFinance.getMaturityDate(), PennantConstants.DBDateTimeFormat)));
				...
				if (!cmtAdded) {
					...
					finCmt.setOutStandingBal(PennantApplicationUtil.amountFormate(
							PennantApplicationUtil.unFormateAmount(totOSAmt, ccyFormatter), ccyFormatter));
					commitments.add(finCmt);
				}
			}
		}
		...
		List<AvailFinance> availFinances = getFinanceDetailService().getFinanceDetailByCmtRef("",
				this.custID.longValue());
		if (availFinances != null && !availFinances.isEmpty()) {
			...
			for (AvailFinance availFinance : availFinances) {
				...
				finCmt.setFinCcy(availFinance.getFinCcy());
				finCmt.setFinAmount(
						PennantApplicationUtil.amountFormate(new BigDecimal(availFinance.getFinAmount()), formatter));
				finCmt.setDrawnPrinciple(PennantApplicationUtil
						.amountFormate(new BigDecimal(availFinance.getDrawnPrinciple()), formatter));
				...
				BigDecimal outStandBHD = (new BigDecimal(availFinance.getOutStandingBal())
						.multiply(CurrencyUtil.getExChangeRate(availFinance.getFinCcy())))
								.multiply(new BigDecimal(Math.pow(10, 3 - formatter)));
				finCmt.setOutStandingBal(PennantApplicationUtil.amountFormate(outStandBHD, 3));
				finCmt.setLastRepay(availFinance.getLastRepay() == null ? ""
						: DateUtility.formatToLongDate(DateUtility.parse(availFinance.getLastRepay(),
								PennantConstants.DBDateTimeFormat)));
				finCmt.setMaturityDate(DateUtility.formatToLongDate(
						DateUtility.parse(availFinance.getMaturityDate(), PennantConstants.DBDateTimeFormat)));
				finCmt.setProfitRate(availFinance.getProfitRate());
				finCmt.setRepayFrq(availFinance.getRepayFrq());
				finCmt.setStatus(availFinance.getStatus());
				finCmt.setFinDivision(availFinance.getFinDivision());
	...
	private void setCustomerData() {
		...
		this.custID.setValue(customer.getCustID());
		this.custShrtName.setValue(customer.getCustShrtName());
		this.custDftBranch.setValue(StringUtils.isBlank(customer.getLovDescCustDftBranchName()) ? ""
				: customer.getCustDftBranch() + "-" + customer.getLovDescCustDftBranchName());
		this.custTypeCode.setValue(StringUtils.isBlank(customer.getLovDescCustTypeCodeName()) ? ""
				: customer.getCustTypeCode() + "-" + customer.getLovDescCustTypeCodeName());
		this.custGroupID.setValue(StringUtils.isBlank(customer.getLovDescCustGroupCode()) ? ""
				: customer.getLovDescCustGroupCode() + "-" + customer.getLovDesccustGroupIDName());
		this.custSubSector.setValue(StringUtils.isBlank(customer.getLovDescCustSectorName()) ? ""
				: customer.getCustSector() + "-" + customer.getLovDescCustSectorName());
		this.custNationality.setValue("");
		this.custRiskCountry.setValue(StringUtils.isBlank(customer.getLovDescCustRiskCountryName()) ? ""
				: customer.getCustRiskCountry() + "-" + customer.getLovDescCustRiskCountryName());
		this.custSts.setValue(StringUtils.isBlank(customer.getLovDescCustStsName()) ? ""
				: customer.getCustSts() + "-" + customer.getLovDescCustStsName());
		this.custDftCcy.setValue(StringUtils.trimToEmpty(customer.getCustBaseCcy()),
				CurrencyUtil.getCcyDesc(customer.getCustBaseCcy()));
		...
		List<CustomerRating> ratingList = getCustomerDetailsService().getCustomerRatingByCustId(customer.getCustID(),
				"_AView");
		...
		if (ratingList != null && !ratingList.isEmpty()) {
			for (CustomerRating rating : ratingList) {
				if ("AIB".equals(rating.getCustRatingType())) {
					this.crCommitteeDec
							.setValue(rating.getCustRatingCode() + "-" + rating.getLovDesccustRatingCodeDesc());
				} else {

					if (!"ABG".equals(rating.getCustRatingType())) {
						extrating = extrating
								+ (rating.getCustRatingType() + ":" + rating.getLovDesccustRatingCodeDesc()) + " ; ";
					}
				}
			}
