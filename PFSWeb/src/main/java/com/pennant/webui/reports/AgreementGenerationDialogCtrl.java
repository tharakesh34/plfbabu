...
public class AgreementGenerationDialogCtrl extends GFCBaseCtrl<Object> {
	...
	private static final Logger logger = LogManager.getLogger(AgreementGenerationDialogCtrl.class);
	...
	private void doSetValidation() {
		...
		if (!this.custCIF.isReadonly()) {
			this.custCIF.setConstraint(new PTStringValidator(
					Labels.getLabel("label_AgreementGenerationDialog_CustCIF.value"), null, true, true));
		}
		if (!this.purchRegOffice.isReadonly()) {
			this.purchRegOffice.setConstraint(new PTStringValidator(
					Labels.getLabel("label_AgreementGenerationDialog_PurchRegOffice.value"), null, true, true));
		}
		if (!this.purchaddress.isReadonly()) {
			this.purchaddress.setConstraint(
					new PTStringValidator(Labels.getLabel("label_AgreementGenerationDialog_Purchaddress.value"),
							PennantRegularExpressions.REGEX_ADDRESS, true));
		}
		if (!this.contractDate.isReadonly()) {
			this.contractDate.setConstraint(new PTDateValidator(
					Labels.getLabel("label_AgreementGenerationDialog_Date.value"), true, appDate, appEndDate, false));
		}
		if (!this.attention.isReadonly()) {
			this.attention.setConstraint(
					new PTStringValidator(Labels.getLabel("label_AgreementGenerationDialog_Attention.value"),
							PennantRegularExpressions.REGEX_ADDRESS, true));
		}
		if (!this.faxCountryCode.isReadonly()) {
			this.faxCountryCode.setConstraint(new PTPhoneNumberValidator(
					Labels.getLabel("label_AgreementGenerationDialog_FaxCountryCode.value"), false, 1));
		}
		if (!this.faxAreaCode.isReadonly()) {
			this.faxAreaCode.setConstraint(new PTPhoneNumberValidator(
					Labels.getLabel("label_AgreementGenerationDialog_FaxAreaCode.value"), false, 2));
		}
		if (!this.fax.isReadonly()) {
			this.fax.setConstraint(
					new PTPhoneNumberValidator(Labels.getLabel("label_AgreementGenerationDialog_fax.value"), false, 3));
		}
	}
	...
	public void setAgreementGenerationDialogCtrl(AgreementGenerationDialogCtrl agreementGenerationDialogCtrl) {
