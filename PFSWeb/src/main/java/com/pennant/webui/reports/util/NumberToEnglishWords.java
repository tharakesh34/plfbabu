...
package com.pennant.webui.reports.util;
...
public class NumberToEnglishWords extends JRDefaultScriptlet implements Serializable {
	...
	public static String getAmountInText(BigDecimal amount, String ccyCode) throws Exception, JRScriptletException {
		logger.debug("Entering ");
		boolean minus = false;
		String strAmount = "";
		String minorCcyDesc = "";
		...
		if (StringUtils.isNotEmpty(ccyCode.trim())) {
			...
			if (currency != null) {
				...
				if (currency.getCcyMinorCcyDesc() != null && (decPos > 0)) {
					minorCcyDesc = currency.getCcyMinorCcyDesc().toLowerCase();
					String minorCcyDescCapitalized = minorCcyDesc.substring(0, 1).toUpperCase();
					minorCcyDesc = minorCcyDescCapitalized + minorCcyDesc.substring(1);
				}
		...
		if (decPos > 0) {// if after decimal point is greater than zero than only add minorCcyDesc
			...
			if (StringUtils.isNotBlank(currnecySymbol)) {
				strAmount = StringUtils.trimToEmpty(currnecySymbol + " " + getNumberToWords(new BigInteger(majorCcy))
						+ " and " + getNumberToWords(new BigInteger(minorCcy)) + " " + minorCcyDesc + " " + "Only");
			} else {
				strAmount = StringUtils.trimToEmpty(getNumberToWords(new BigInteger(majorCcy)) + " " + ccyCode + " and "
						+ getNumberToWords(new BigInteger(minorCcy)) + " " + minorCcyDesc + " " + "Only");
			}
		...
		logger.debug("Leaving ");
		strAmount = capitailizeWord(strAmount);
		return strAmount;
	}
	...
	/**
	 * This method reduces multiple database hits 1 )This method takes input as List of objects and sort it with
	 * "sortField" and compare the values against compareMethod(like getFloorCode()) .Then splits the main list into
	 * sublists against compareMethod value 2) Put Sublists into map Where key is compareMethod's value and value is
	 * sublist created against that value E.g : Main list is List of PremiseUnits.This method splits this list into
	 * sublists by floorCode . Keeps floor code as key and value is sublist i.e get against FloorCode from main list .
	 * 
	 * @param <T>
	 ...
	 * @return
	 */
	public static <T> Map<Object, List<T>> getSubListMapGroupingByField(List<T> childRecordsList, String sortField,
