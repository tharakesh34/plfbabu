...
/**
 * This is the controller class for the /WEB-INF/pages/Enquiry/FinanceInquiry/ActivityLog.zul
 */
public class ActivityLogCtrl extends GFCBaseCtrl<Activity> implements Comparator<Notes> {
	private static final long serialVersionUID = 8285763521940736965L;
	private static final Logger logger = LogManager.getLogger(ActivityLogCtrl.class);

	/*
	 * All the components that are defined here and have a corresponding component with the same 'id' in the ZUL-file
	 * are getting autoWired by our 'extends GFCBaseCtrl' GenericForwardComposer.
	 */
	private Window window_ActivityLog; // autoWired
	...
	public void onCreate$window_ActivityLog(ForwardEvent event) {
		...
		try {
			...
			Collections.sort(notesList, new ActivityLogCtrl());
	...
	public void onNotesItemClicked(ForwardEvent event) {
		...
		try {
			...
			window.setParent(window_ActivityLog);
	...
	private void fillActivityLog(List<Activity> activities) {
		...
		for (Activity activity : activities) {
			if (addHeader) {
				...
				if (moduleCode.equals("FinanceMain")) {
					...
					cell = new Listcell(activity.getRecordType());
				}
	...
	public void onClick$btnClose(Event event) {
		logger.debug(Literal.ENTERING + event.toString());
		this.window_ActivityLog.onClose();
		logger.debug(Literal.LEAVING + event.toString());
	}
	...
	private void doSetFieldProperties() {
		...
		this.window_ActivityLog.setWidth(getDesktopWidth() + "px");
		this.window_ActivityLog.setHeight(getDesktopHeight() + "px");
		...
		this.window_ActivityLog.doModal();
