...
/**
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
 * This is the controller class for the /WEB-INF/pages/ApplicationMaster/ChequePurpose/chequePurposeDialog.zul file.
 * <br>
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
 */
public class ChequePurposeDialogCtrl extends GFCBaseCtrl<ChequePurpose> {
	private static final long serialVersionUID = 1L;
	private static final Logger logger = LogManager.getLogger(ChequePurposeDialogCtrl.class);
	 ...
	 * selected ChequePurpose object in a Map.
	 * 
	 * @param event
	 * @throws Exception
	 */
	public void onCreate$window_ChequePurposeDialog(Event event) throws Exception {
		...
		try {
			...
			doLoadWorkFlow(this.chequePurpose.isWorkflow(), this.chequePurpose.getWorkflowId(),
					this.chequePurpose.getNextTaskId());
	...
	public void onClick$btnNotes(Event event) throws Exception {
		...
		try {
			...
			ScreenCTL.displayNotes(
					getNotes("ChequePurpose", getChequePurpose().getCode(), getChequePurpose().getVersion()), this);
	...
	private void displayComponents(int mode) {
		...
		doReadOnly(ScreenCTL.initButtons(mode, this.btnCtrl, this.btnNotes, isWorkFlowEnabled(), isFirstTask(),
				this.userAction, this.code, this.description));

		if (getChequePurpose().isNewRecord()) {
			setComponentAccessType("ChequePurposeDialog_Code", false, this.code, this.space_Code, this.label_Code,
	...
	public void doReadOnly(boolean readOnly) {
		...
		setComponentAccessType("ChequePurposeDialog_Code", true, this.code, this.space_Code, this.label_Code,
				this.hlayout_Code, null);
		setComponentAccessType("ChequePurposeDialog_Description", tempReadOnly, this.description,
		...
		setComponentAccessType("ChequePurposeDialog_Active", tempReadOnly, this.active, this.space_Active,
				this.label_Active, this.hlayout_Active, null);
		setRowInvisible(this.row1, this.hlayout_Active, null);

		if (!getChequePurpose().isNewRecord()
				&& PennantConstants.FIN_MGRCHQ__CHQPURPOSECODE.equals(getChequePurpose().getCode())) {
			setComponentAccessType("ChequePurposeDialog_Active", true, this.active, this.space_Active,
					this.label_Active, this.hlayout_Active, null);
		}
	...
	private void doSetValidation() {
		...
		if (!this.code.isReadonly()) {
			this.code.setConstraint(new PTStringValidator(Labels.getLabel("label_ChequePurposeDialog_Code.value"),
					PennantRegularExpressions.REGEX_UPPERCASENAME, true));
		}
		//Description
		if (!this.description.isReadonly()) {
			this.description
					.setConstraint(new PTStringValidator(Labels.getLabel("label_ChequePurposeDialog_Description.value"),
		...
		if (MessageUtil.confirm(msg) == MessageUtil.YES) {
			if (StringUtils.isBlank(aChequePurpose.getRecordType())) {
				...
				if (isWorkFlowEnabled()) {
					...
					getWorkFlowDetails(userAction.getSelectedItem().getLabel(), aChequePurpose.getNextTaskId(),
							aChequePurpose);
				} else {
		...
		try {
			...
			while (retValue == PennantConstants.porcessOVERIDE) {
				...
				if (retValue == PennantConstants.porcessCONTINUE) {
					...
					if (deleteNotes) {
						deleteNotes(getNotes("ChequePurpose", aChequePurpose.getCode(), aChequePurpose.getVersion()),
								true);
					}
				}
	...
	private AuditHeader getAuditHeader(ChequePurpose aChequePurpose, String tranType) {
		...
		return new AuditHeader(aChequePurpose.getCode(), null, null, null, auditDetail, aChequePurpose.getUserDetails(),
				getOverideMap());
	}
