 ...
 * FILE HEADER *
 ********************************************************************************************
 * * FileName : DPDBucketDialogCtrl.java * * Author : PENNANT TECHONOLOGIES * * Creation Date : 21-04-2017 * * Modified
...
public class DPDBucketDialogCtrl extends GFCBaseCtrl<DPDBucket> {
	...
	private static final Logger logger = LogManager.getLogger(DPDBucketDialogCtrl.class);
	 ...
	 */
	public void onCreate$window_DPDBucketDialog(Event event) {
	...
	private void doCheckRights() {
		...
		this.btnNew.setVisible(getUserWorkspace().isAllowed("button_DPDBucketDialog_btnNew"));
		this.btnEdit.setVisible(getUserWorkspace().isAllowed("button_DPDBucketDialog_btnEdit"));
		this.btnDelete.setVisible(getUserWorkspace().isAllowed("button_DPDBucketDialog_btnDelete"));
		this.btnSave.setVisible(getUserWorkspace().isAllowed("button_DPDBucketDialog_btnSave"));
	...
	private void doCancel() {
		...
		doWriteBeanToComponents(this.dPDBucket.getBefImage());
		doReadOnly();
	 ...
	 * 
	 * @param dPDBucket The entity that need to be render.
	 */
	public void doShowDialog(DPDBucket dPDBucket) {
		...
		if (dPDBucket.isNewRecord()) {
			...
			if (isWorkFlowEnabled()) {
				if (StringUtils.isNotBlank(dPDBucket.getRecordType())) {
		...
		doWriteBeanToComponents(dPDBucket);
	...
	private void doSetValidation() {
		...
		if (!this.bucketCode.isReadonly()) {
			this.bucketCode
					.setConstraint(new PTStringValidator(Labels.getLabel("label_DPDBucketDialog_BucketCode.value"),
							PennantRegularExpressions.REGEX_UPP_BOX_ALPHANUM, true));
		}
		if (!this.bucketDesc.isReadonly()) {
			this.bucketDesc
					.setConstraint(new PTStringValidator(Labels.getLabel("label_DPDBucketDialog_BucketDesc.value"),
	...
	private void doDelete() {
		...
		doDelete(Labels.getLabel("label_DPDBucketDialog_BucketCode.value") + " : " + aDPDBucket.getBucketCode(),
				aDPDBucket);
