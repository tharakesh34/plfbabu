...
/**
 * This is the controller class for the /WEB-INF/pages/ApplicationMaster/CheckList/checkListDialog.zul file.
 */
public class CheckListDialogCtrl extends GFCBaseCtrl<CheckList> {
	...
	private static final Logger logger = LogManager.getLogger(CheckListDialogCtrl.class);
	...
	/*
	 * protected Textbox lovDescCheckRule; protected Button btnSearchCheckRule;
	...
	private void doCheckRights() {
		...
		this.btnNew_CheckListDetail
				.setVisible(getUserWorkspace().isAllowed("button_CheckListDialog_btnNew_CheckListDetail"));
	...
	public void doWriteBeanToComponents(CheckList aCheckList) {
		...
		this.checkRule.setDescription(
				StringUtils.isBlank(aCheckList.getCheckRule()) ? "" : aCheckList.getLovDescCheckRuleName());
		logger.debug("Leaving");
	}
	...
	public void doWriteComponentsToBean(CheckList aCheckList) {
		...
		try {
			...
			if (this.listbox_ChkListDetails.getItemCount() == 0) {
				throw new WrongValueException(this.btnNew_CheckListDetail, Labels.getLabel("FIELD_IS_MAND",
						new String[] { Labels.getLabel("label_FinanceCheckListList.title") }));
			} else if (this.listbox_ChkListDetails.getItemCount() < this.checkMaxCount.intValue()) {
				throw new WrongValueException(this.btnNew_CheckListDetail,
						Labels.getLabel("label_CheckListDialog_Checklist_Madatory"));
			}
		...
		try {
			if ((!this.checkMaxCount.isReadonly()) && this.checkMaxCount.intValue() < this.checkMinCount.intValue()) {
				throw new WrongValueException(this.checkMaxCount,
	...
	private void doSetValidation() {
		...
		if (!this.checkListDesc.isReadonly()) {
			this.checkListDesc
					.setConstraint(new PTStringValidator(Labels.getLabel("label_CheckListDialog_CheckListDesc.value"),
							PennantRegularExpressions.REGEX_DESCRIPTION, true));
		}
		if (!this.checkMinCount.isReadonly()) {
			this.checkMinCount
					.setConstraint(new PTNumberValidator(Labels.getLabel("label_CheckListDialog_CheckMinCount.value"),
							false, false, 0, this.listbox_ChkListDetails.getItemCount()));
		}
		/*
		 * if (!this.checkMaxCount.isReadonly()){ this.checkMaxCount.setConstraint(new
		 * PTNumberValidator(Labels.getLabel("label_CheckListDialog_CheckMaxCount.value"),true, false,1,2)); }
		 */
		logger.debug("Leaving");
	}
	...
	private void doSetLOVValidation() {
		...
		if (!this.checkRule.isReadonly()) {
			this.checkRule.setConstraint(
					new PTStringValidator(Labels.getLabel("label_CheckListDialog_CheckRule.value"), null, false, true));
		}
		logger.debug("Leaving");
	}
	...
	private AuditHeader getAuditHeader(CheckList aCheckList, String tranType) {
		...
		return new AuditHeader(String.valueOf(aCheckList.getCheckListId()), null, null, null, auditDetail,
				aCheckList.getUserDetails(), getOverideMap());
	}
	...
	public void onClick$btnNew_CheckListDetail(Event event) throws Exception {
		...
		try {
			Executions.createComponents("/WEB-INF/pages/ApplicationMaster/CheckList/CheckListDetailDialog.zul", null,
					map);
	...
	public void doFillCheckListDetailsList(List<CheckListDetail> checkListDetailList) {
		...
		getChkListDetailPagedListWrapper().initList(checkListDetailList, this.listbox_ChkListDetails,
				pagingChkListDetailsList);
	...
	public void setChkListDetailPagedListWrapper() {
		if (this.chkListDetailPagedListWrapper == null) {
			this.chkListDetailPagedListWrapper = (PagedListWrapper<CheckListDetail>) SpringUtil
