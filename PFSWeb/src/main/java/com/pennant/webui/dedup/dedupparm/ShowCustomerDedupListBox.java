...
public class ShowCustomerDedupListBox extends Window implements Serializable {
	...
	private static final Logger logger = Logger
			.getLogger(ShowCustomerDedupListBox.class);
	 ...
	 */
	public static Object show(Component parent, List<?> custDedupList,
			String dedupFields, CustomerDedup custDedup, String curUser) {
		return new ShowCustomerDedupListBox(parent, custDedupList, dedupFields,
				custDedup, curUser);
	}
	 ...
	 * @param parent
	 */
	private ShowCustomerDedupListBox(Component parent, List<?> custDedupList,String dedupFields, CustomerDedup custDedup, String curUser) {
		super();
		...
		public void render(Listitem item, Object data, int count) throws Exception {
			...
			for (int j = 0; j < fieldString.length; j++) {
				...
				if (data.getClass().getMethod(fieldMethod).getReturnType().equals(String.class)) {
					...
					currentFieldValue = (String) getCustomerDedup().getClass().getMethod(fieldMethod).invoke(getCustomerDedup());
					...
					for (int k = 0; k < ruleFields.length; k++) {
						if(!StringUtils.equals(ruleFields[k], InterfaceConstants.DEDUP_CORE)) {
							if (StringUtils.equals(fieldMethod, "get" + ruleFields[k])) {
								if(StringUtils.equals(ruleFields[k], MOB_NUM) || StringUtils.equals(ruleFields[k], CRCPR)) {
					...
					Date curdateFieldValue = (Date) getCustomerDedup().getClass().getMethod(fieldMethod).invoke(getCustomerDedup());
