...
public class PTStringValidator implements Constraint {
	private static final Logger logger = LogManager.getLogger(PTStringValidator.class);
	...
	private String getErrorMessage(Object value) {
		...
		if (StringUtils.isBlank(compValue)) {
			...
			if (regExp != null) {
				String expression = PennantRegularExpressions.getRegexMapper(regExp);
			...
			if (minLenValid && maxLenValid) {
				...
				if (compValue.length() < minLength || compValue.length() > maxLength) {
					return Labels.getLabel("FIELD_ALLOWED_RANGE",
							new String[] { fieldParm, String.valueOf(minLength), String.valueOf(maxLength) });
				}
			...
			if (minLenValid) {
				...
				if (compValue.length() < minLength) {
					return Labels.getLabel("FIELD_ALLOWED_MINLENGTH",
							new String[] { fieldParm, String.valueOf(minLength) });
				}
				return null;
			}
