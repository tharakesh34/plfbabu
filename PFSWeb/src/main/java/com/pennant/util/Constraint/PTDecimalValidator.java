...
public class PTDecimalValidator implements Constraint {
	...
	public PTDecimalValidator(String fieldParm, int decPos, boolean mandatory, boolean negative, double minValue,
			double maxValue) {
		setFieldParm(fieldParm);
		setMandatory(mandatory);
		setNegative(negative);
	...
	private String getErrorMessage(Object value) {
		...
		if (regExp != null) {
			...
			try {
				pattern = Pattern.compile(PennantRegularExpressions.getRegexMapper(regExp));
			} catch (Exception e) {
				logger.error("Exception: ", e);
				return Labels.getLabel("REGEX_INVALID", new String[] { regExp });
			}
