...
public class WorkFlowDialogCtrl extends GFCBaseCtrl implements Serializable{
	...
	public void onCreate$window_workFlowDialog(Event event) throws Exception {
		...
		if (args.containsKey("workFlowDetails")) {
			this.workFlowDetails = (WorkFlowDetails) args.get("workFlowDetails");
	 ...
	 * @throws Exception 
	 */
	public void doWriteBeanToComponents(WorkFlowDetails aWorkFlowDetails) throws Exception {
		logger.debug("Entering ");
		...
		if(aWorkFlowDetails.getWorkFlowXml()  != null){
			this.workFlowXML.setValue(aWorkFlowDetails.getWorkFlowXml().toString());
		}else{
			this.workFlowXML.setValue("");
		}
	...
	public void doShowDialog(WorkFlowDetails aWorkFlowDetails) throws Exception {
		...
		if (aWorkFlowDetails == null) {
			aWorkFlowDetails = getWorkFlowDetailsService().getNewWorkFlowDetails();
	...
	public void doSave() throws Exception {
		...
		try {
			...
			aWorkFlowDetails.setLastMntBy(getUserWorkspace().getLoginUserDetails().getLoginUsrID());
			aWorkFlowDetails.setLastMntOn(new Timestamp(System.currentTimeMillis()));
			aWorkFlowDetails.setUserDetails(getUserWorkspace().getLoginUserDetails());
			...
			if(doSaveProcess(auditHeader)){
				final JdbcSearchObject<WorkFlowDetails> so = getWorkFlowListCtrl().getSearchObj();
			...
			ErrorControl.showErrorControl(this.window_workFlowDialog, auditHeader);
		}
		logger.debug("Leaving ");
	}
	 ...
	 * @throws Exception
	 */
	public void onClose$window_invoiceHeaderDialog(Event event) throws Exception {
		logger.debug("Entering" + event.toString());
		doClose();
		logger.debug("Leaving" + event.toString());
	}
