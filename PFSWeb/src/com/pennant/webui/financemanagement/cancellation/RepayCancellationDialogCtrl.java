 ...
 * /WEB-INF/pages/RulesFactory/RepayCancellation/RepayCancellationDialog.zul file. <br>
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
 */
public class RepayCancellationDialogCtrl extends GFCBaseListCtrl<FinanceMain> implements Serializable {
	...
	public void onCreate$window_RepayCancellationDialog(Event event) throws Exception {
		...
		if (isWorkFlowEnabled()) {
			...
			if(recStatus.equals(PennantConstants.RCD_STATUS_REJECTED)){
				...
				getUserWorkspace().alocateMenuRoleAuthorities(getRole(), "RepayCancelDialog", menuItemRightName);	
			}
		}else{
	...
	private void doCheckRights() {
		...
		getUserWorkspace().alocateAuthorities("RepayCancelDialog",getRole(), menuItemRightName);
	 ...
	 * @throws Exception
	 */
	public void onClose$window_RepayCancellationDialog(Event event) throws Exception {
		logger.debug("Entering" + event.toString());
		doClose();
		logger.debug("Leaving" + event.toString());
	}
	...
	private void doClose() throws InterruptedException {
		logger.debug("Entering");
		closeDialog(this.window_RepayCancellationDialog, "RepayCancelDialog");
		logger.debug("Leaving");
	}
	...
	public void doWriteBeanToComponents(FinanceDetail financeDetail) {
		...
		if(repayList != null && repayList.size() > 0){
			...
			this.postDate.setValue(DateUtility.formatDate(repayList.get(0).getFinPostDate(), PennantConstants.dateFormate));
			this.rpyAmount.setValue(PennantAppUtil.amountFormate(repayList.get(0).getFinRpyAmount(), financeMain.getLovDescFinFormatter()));
	...
	public void doShowDialog(FinanceDetail afinanceDetail) throws InterruptedException {
		...
		if (afinanceDetail == null) {
			...
			afinanceDetail = new FinanceDetail();
			afinanceDetail.setFinScheduleData(new FinScheduleData());
		...
		try {
			...
			setDialog(this.window_RepayCancellationDialog);
	...
	public void doFilllistbox(List<FinanceRepayments> repayList) {
		...
		if (repayList != null && repayList.size() > 0) {
			...
			for (int i=0; i < repayList.size();i++) {
				...
				lc = new Listcell(DateUtility.formatDate(repay.getFinSchdDate(), PennantConstants.dateFormate));
				lc.setParent(item);
	...
	public void doSave() throws Exception {
		...
		try {
			...
			logger.error(e);
			showErrorMessage(this.window_RepayCancellationDialog, e);
		}
		logger.debug("Leaving");
	}
	...
	private void doFillPostingdetails(long linkedTranId) {
		...
		if(postingList != null && !postingList.isEmpty()){
			...
			for (ReturnDataSet returnDataSet : postingList) {
				...
				lc.setParent(item);
				lc = new Listcell(PennantAppUtil.amountFormate(returnDataSet.getPostAmount(), financeMain.getLovDescFinFormatter()));
