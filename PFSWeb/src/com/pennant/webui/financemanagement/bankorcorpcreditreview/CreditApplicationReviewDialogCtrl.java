...
public class CreditApplicationReviewDialogCtrl extends GFCBaseListCtrl<FinCreditReviewSummary> implements Serializable {
	...
	public void onCreate$window_CreditApplicationReviewDialog(Event event) throws Exception {
		...
		if(!StringUtils.trimToEmpty(this.creditReviewDetails.getLovDescCustCIF()).equals("")){
	...
	public void doWriteBeanToComponents(FinCreditReviewDetails aCreditReviewDetails) throws Exception {
		...
		if(aCreditReviewDetails.isNewRecord() && StringUtils.trimToEmpty(aCreditReviewDetails.getCurrency()).equals("")){
			Currency currency = PennantAppUtil.getCurrencyBycode(PennantConstants.CURRENCY_BHD);
		...
		if(aCreditReviewDetails.getConversionRate() == null){
			...
			BigDecimal converstnRate = PennantAppUtil.formateAmount(CalculationUtil.getConvertedAmount(PennantConstants.CURRENCY_USD, this.currencyType.getValue(),
	...
	public void doWriteComponentsToBean(FinCreditReviewDetails aCreditReviewDetails) {
		...
		try {
			if(this.conSolOrUnConsol.getSelectedItem().getValue().equals(PennantConstants.CREDITREVIEW_CONSOLIDATED)){
		...
		try {
			aCreditReviewDetails.setMarketPrice(this.marketPrice.getValue());
		} catch (WrongValueException we) {
			//wve.add(we);
		}
		try {
		...
		try {
			if(this.auditPeriod.getSelectedItem().getValue().toString().equals("#")){
				throw new WrongValueException();
			} else {
				aCreditReviewDetails.setAuditPeriod(Integer.valueOf(this.auditPeriod.getSelectedItem().getValue().toString()));
			}
		} catch (WrongValueException we) {
			wve.add(we);
		}
		try {
			if(this.auditType.getSelectedItem() != null && !this.auditType.getSelectedItem().getValue().toString().equals("#")){
		...
		try {
			if(this.qualifiedUnQualified.getSelectedItem().getValue().toString().equals(PennantConstants.CREDITREVIEW_QUALIFIED)){
		...
		try {
			if (this.currencyType.getValue().equals("")) {
	...
	public void doShowDialog(FinCreditReviewDetails aCreditReviewDetails) throws Exception {
		...
		if(customer != null){
			if(customer.getCustCtgCode().equals(PennantConstants.PFF_CUSTCTG_BANK)){
			listOfFinCreditRevSubCategory = this.creditApplicationReviewService.getFinCreditRevSubCategoryByMainCategory(PennantConstants.INTERFACE_CUSTCTG_BANK);  
			} else {
			listOfFinCreditRevSubCategory = this.creditApplicationReviewService.getFinCreditRevSubCategoryByMainCategory(PennantConstants.INTERFACE_CUSTCTG_CORP);  	
			}
		}
	...
	private void doStoreInitValues() {
		...
		this.oldVar_consolOrUnConsol = this.conSolOrUnConsol.getSelectedItem().getValue();
		...
		this.oldVar_lovDescFinCcyName = this.currencyType.getDescription();
		this.oldVar_finCcy = this.currencyType.getValue();
	...
	private void doResetInitValues() {
		...
		if(this.oldVar_consolOrUnConsol.equals(PennantConstants.CREDITREVIEW_CONSOLIDATED)){
			this.conSolOrUnConsol.setSelectedIndex(0);
		} else {
			this.conSolOrUnConsol.setSelectedIndex(1);
		}
		...
		this.currencyType.setDescription(this.oldVar_lovDescFinCcyName);
		this.currencyType.setValue(this.oldVar_finCcy);
	...
	private boolean isDataChanged() {
		...
		if (!this.oldVar_consolOrUnConsol.equals(this.conSolOrUnConsol.getSelectedItem().getValue().toString())) {
			return true;
		}
		...
		if (this.oldVar_finCcy != this.currencyType.getValue()) {
			return true;
		}
		return false;
	}
	...
	private void doSetValidation() {
		...
		if (!this.conversionRate.isReadonly()) {
			...
			this.conversionRate.setConstraint(new RateValidator(18, 0, 
					Labels.getLabel("label_CreditApplicationReviewDialog_ConversionRate.value"), true));
		}
		...
		/*if (!this.marketPrice.isReadonly()) {
			this.marketPrice.setConstraint(new AmountValidator(18,0,
					Labels.getLabel("label_CreditApplicationReviewDialog_MarketPrice.value"), false));
		}*/
		/*if (!this.noOfShares.isReadonly()) {
			this.noOfShares.setConstraint(new AmountValidator(18,0,Labels.getLabel("label_CreditApplicationReviewDialog_NoOfShares.value"), false));
		}*/
		if (!this.auditPeriod.isReadonly()) {
			this.auditPeriod.setConstraint(new IntValidator(2,Labels.getLabel("label_CreditApplicationReviewDialog_auditPeriod.value"), false));
		}
		logger.debug("Leaving");
	}
	...
	public void doSave() throws Exception {
		...
		if((this.totLibAsstDiff.getValue() == null ? BigDecimal.ZERO : this.totLibAsstDiff.getValue()).compareTo(BigDecimal.ZERO) != 0
			...
			PTMessageUtils.showErrorMessage("Total Assets and Total Liabilities & Net Worth not Matched..");
	...
	public void setListDetails(FinCreditReviewDetails finCreditReviewDetails) throws InterruptedException {
		...
		for(int k=0;k<listOfTabPanels.size();k++){
			...
			/*Listitem liTOTASST = (Listitem) listBox.getFellowIfAny("liTOTASST");
			Listitem liTOTLIABANETWORTH = (Listitem) listBox.getFellowIfAny("liTOTLIABANETWORTH");
			FinCreditReviewSummary liTOTASSTSummary=null;
			FinCreditReviewSummary liTOTLIABANETWORTHSummary=null;

			if(liTOTASST != null){
				liTOTASSTSummary = (FinCreditReviewSummary) liTOTASST.getAttribute("finSummData");
			}
			
			if(liTOTASST != null){
				liTOTLIABANETWORTHSummary = (FinCreditReviewSummary) liTOTLIABANETWORTH.getAttribute("finSummData");
			}
			 
			
			if(liTOTASSTSummary != null && liTOTLIABANETWORTHSummary != null){
				if(!liTOTASSTSummary.getItemValue().subtract(liTOTLIABANETWORTHSummary.getItemValue()).equals(BigDecimal.ZERO)){
					PTMessageUtils.showErrorMessage("Total Assets and Total Liabilities & Net Worth not Matched..");
					return false;
				}
			}*/
			...
			for(int i =0;i<listItems.size();i++){
				...
				if (isWorkFlowEnabled()) {
					if (StringUtils.trimToEmpty(creditReviewSummary.getRecordType()).equals("")) {
				...
				if(listItem.getAttribute("NewSubCategory")!=null){
					...
					finCreditRevSubCategory.setLastMntBy(getUserWorkspace().getLoginUserDetails().getLoginUsrID());
					finCreditRevSubCategory.setLastMntOn(new Timestamp(System.currentTimeMillis()));
				}  

				creditReviewSummary.setLastMntBy(getUserWorkspace().getLoginUserDetails().getLoginUsrID());
	...
	public Listbox setListToTab(Tabpanel tabPanel,FinCreditRevCategory fcrc){
		...
		if(getCreditReviewDetails().getAuditType() != null){
			  audQualRePort =Labels.getLabel("UnAudUnQual_Months",new String[]{getCreditReviewDetails().getAuditType(), 
				  getCreditReviewDetails().isQualified() ? PennantConstants.CREDITREVIEW_QUALIFIED : PennantConstants.CREDITREVIEW_UNQUALIFIED, 
				String.valueOf(getCreditReviewDetails().getAuditPeriod())});
			    audQual = getCreditReviewDetails().getAuditType()+"/"+ (getCreditReviewDetails().isQualified() ? PennantConstants.CREDITREVIEW_QUALIFIED : PennantConstants.CREDITREVIEW_UNQUALIFIED);
			    auxHeaderCurYerLabel = getCreditReviewDetails().getAuditYear()+" - " + String.valueOf(getCreditReviewDetails().getAuditPeriod())+PennantConstants.MONTH;
			...
			String qualUnQual= this.qualifiedUnQualified.getSelectedItem().getValue().toString();
			 audQualRePort = Labels.getLabel("UnAudUnQual_Months", new String[] { this.auditType.getSelectedItem() == null ?
			    		   PennantConstants.CREDITREVIEW_AUDITED :
			 ...
			 auxHeaderCurYerLabel = this.auditedYear.getValue()+" - " +String.valueOf(getCreditReviewDetails().getAuditPeriod())+PennantConstants.MONTH;
			 creditReviewSubCtgDetailsHeaders.setCurYearAuditValueHeader(audQualRePort);
		}
		...
		Listheader listheader_curUSConvrtn = new Listheader(PennantConstants.CURRENCY_USD);
		...
		creditReviewSubCtgDetailsHeaders.setCurYearBreakDownHeader(Labels.getLabel("listheader_breakDowns.value", "")+"-"
                                         +this.creditReviewDetails.getAuditPeriod()+PennantConstants.MONTH);
		...
		if(finCreditReviewDetails != null && !StringUtils.trimToEmpty(finCreditReviewDetails.getAuditType()).equals("")){
			qualOrUnqual = finCreditReviewDetails.isQualified() ? PennantConstants.CREDITREVIEW_QUALIFIED : PennantConstants.CREDITREVIEW_UNQUALIFIED ;
		...
		Listheader listheader_prevcurUSConvrtn = new Listheader(PennantConstants.CURRENCY_USD);
		...
		creditReviewSubCtgDetailsHeaders.setCurYearPerHeader(String.valueOf(currentAuditYear)+"-"+String.valueOf(currentAuditYear-1)
	...
	public void doSetCustomer(Object nCustomer,JdbcSearchObject<Customer> newSearchObject) throws Exception{
		...
		if(aCustomer.getCustCtgCode().equalsIgnoreCase(PennantConstants.PFF_CUSTCTG_BANK)){
			category = PennantConstants.INTERFACE_CUSTCTG_BANK;
		} else {
			category = PennantConstants.INTERFACE_CUSTCTG_CORP;
		}
	...
	public void render(FinCreditRevCategory fcrc,Listbox listbox) throws Exception {
		...
		if(fcrc.getRemarks().equals(PennantConstants.CREDITREVIEW_REMARKS)){
			isRatio = true;
		}
	...
	Listitem newItem = new Listitem();
        ...
        for(int i=0; i < listOfFinCreditRevSubCategory.size(); i++){
        	...
        	if(aFinCreditRevSubCategory.getCategoryId() == finCreditRevSubCategory.getCategoryId() && 
        			aFinCreditRevSubCategory.getSubCategoryItemType().equals(PennantConstants.CREDITREVIEW_CALCULATED_FIELD))
        	{
        ...
        cb_MainCategoryCode.setVisible(!finCreditRevSubCategory.getRemarks().equals(PennantConstants.CREDITREVIEW_REMARKS));
        cb_MainCategoryCode.setParent(lc);
        lc.setParent(newItem);
        
       /* Label labelFormula = new Label("-");
        labelFormula.setVisible(false);
        labelFormula.setParent(lc);
        
        Button btnForFormula = new Button("Formula");
        btnForFormula.setId("btnForFormula_NewRecord");
		ComponentsCtrl.applyForward(btnForFormula, "onClick=onClick$btnForFormula");
        btnForFormula.setVisible(finCreditRevSubCategory.getRemarks().equals("R"));
        btnForFormula.setParent(lc);
        lc.setParent(newItem);*/
        
        // Check Box
        /*lc = new Listcell();
        Checkbox  cbIsCalculated = new Checkbox("Is Calculated");
        cbIsCalculated.setId("rdCalculated");
		ComponentsCtrl.applyForward(cbIsCalculated, "onCheck=onCheck$Calculated");
        cbIsCalculated.setParent(lc);
        lc.setParent(newItem);*/
		...
		for (Listitem item : listbox.getItems()) {
			if(!(item instanceof Listgroup)) {
				...
				if(label_BreakDown != null){
					breakDown = (BigDecimal) (curYearValuesMap.get(PennantConstants.CREDITREVIEW_REMARKS+finCreditRevSubCategory.getSubCategoryCode())==null ? BigDecimal.ZERO:
						curYearValuesMap.get(PennantConstants.CREDITREVIEW_REMARKS+finCreditRevSubCategory.getSubCategoryCode()));
		...
		logger.debug("Entering");
		 ...
		 for(FinCreditRevSubCategory finCreditRevSubCategory : listOfFinCreditRevSubCategory) {
        	 if(aFinCreditRevSubCategory.getMainSubCategoryCode().equals(finCreditRevSubCategory.getSubCategoryCode())) {
	        	 ...
	        	 curYearValuesMap.remove(PennantConstants.CREDITREVIEW_REMARKS+aFinCreditRevSubCategory.getSubCategoryCode()); 
	...
	public void onClick$btnForSave(ForwardEvent event){
		...
		FinCreditRevSubCategory aFinCreditRevSubCategory = null;
			...
			aFinCreditRevSubCategory.setCategoryId(Long.valueOf(listitem.getParent().getId().substring(listitem.getParent().getId().length()-1)));
			...
			if(categoryItemType != null){
				aFinCreditRevSubCategory.setSubCategoryItemType(categoryItemType.isChecked() ? 
						 PennantConstants.CREDITREVIEW_CALCULATED_FIELD : PennantConstants.CREDITREVIEW_ENTRY_FIELD);
			} else {
				aFinCreditRevSubCategory.setSubCategoryItemType(PennantConstants.CREDITREVIEW_ENTRY_FIELD);
			}
		...
		if(aFinCreditRevSubCategory != null && aFinCreditRevSubCategory.getRemarks().equals(PennantConstants.CREDITREVIEW_REMARKS)){
			...
			if(cb_MathOperation.getSelectedItem().getValue().toString().equals("#")){
	...
	public void fillRenderer(FinCreditRevSubCategory finCreditRevSubCategory, Listbox listbox, Listitem oldListItem, boolean isNewRecord){
		...
		if(finCreditRevSubCategory.getRemarks().equals(PennantConstants.CREDITREVIEW_REMARKS)){
			isRatio = true;
			creditReviewSubCtgDetails.setRemarks(PennantConstants.CREDITREVIEW_REMARKS);
		}
		...
		if(isRatio && !mainCategory.equals(finCreditRevSubCategory.getMainSubCategoryCode())){
			...
			if(!listbox.hasFellow(mainCategory)){
				...
				lg.setParent(listbox);
				creditReviewSubCtgDetails.setGroupCode(PennantConstants.CREDITREVIEW_GRP_CODE_TRUE);
			}
		}
		...
		//if(!getUserWorkspace().isAllowed("CreditApplicationReviewDialog_AuditAmt")){
		...
		BigDecimal curYrbreakDown = (BigDecimal) (curYearValuesMap.get(PennantConstants.CREDITREVIEW_REMARKS+finCreditRevSubCategory.getSubCategoryCode())==null?BigDecimal.ZERO:
			curYearValuesMap.get(PennantConstants.CREDITREVIEW_REMARKS+finCreditRevSubCategory.getSubCategoryCode()));
		...
		lc.setStyle("text-align:right;");
		lc.setParent(item);
		if(!isRatio && StringUtils.trimToEmpty(finCreditRevSubCategory.getSubCategoryItemType()).equals(PennantConstants.CREDITREVIEW_CALCULATED_FIELD)){
		...
		if(StringUtils.trimToEmpty(finCreditRevSubCategory.getSubCategoryItemType()).equals(PennantConstants.CREDITREVIEW_CALCULATED_FIELD)){
			creditReviewSubCtgDetails.setCalC("C");
			db_Amount.setReadonly(true);
       ...
       /*************************** Previous Year US$ Conversion  ************************/
		...
		BigDecimal prevYrbreakDown = (BigDecimal) (prvYearValuesMap.get(PennantConstants.CREDITREVIEW_REMARKS+finCreditRevSubCategory.getSubCategoryCode())==null?BigDecimal.ZERO:
			prvYearValuesMap.get(PennantConstants.CREDITREVIEW_REMARKS+finCreditRevSubCategory.getSubCategoryCode()));
		...
		/*if(finCreditRevSubCategory.getSubCategoryItemType().equals(PennantConstants.CREDITREVIEW_CALCULATED_FIELD)){
			  ComponentsCtrl.applyForward(item, "onDoubleClick=onSubCategoryItemDoubleClicked");
		}*/
	...
	public void onChange$subCategoryCode(ForwardEvent event){
		logger.debug("Entering" + event.toString());
         /*Textbox tdsubCategoryCode = (Textbox)event.getOrigin().getTarget();
        	 getSubcategoryCodeValidation(listOfFinCreditRevSubCategory, tdsubCategoryCode);*/
		logger.debug("Leaving" + event.toString());
	}
	...
	public void onClick$btnForFormula(ForwardEvent event) throws InterruptedException{
		...
		if(StringUtils.trim(tb_NewSubCategoryCode.getValue()).equals("")){
			 throw new WrongValueException(tb_NewSubCategoryCode, Labels.getLabel("FIELD_NO_EMPTY",
					 new String[] {Labels.getLabel("label_FinCreditRevSubCategoryDialog_SubCategoryCode.value"),
 					Labels.getLabel("label_FinCreditRevSubCategoryDialog_SubCategoryCode.value") }));
		 } else if(StringUtils.trim(tb_NewSubCategoryDesc.getValue()).equals("")){
	...
	public void onAddListitem(ForwardEvent event) throws Exception{
		...
		// listboxOld.removeItemFromSelection(listitemOld);
		 logger.debug("Leaving" + event.toString());
	...
	public void setData(Map<String,BigDecimal> dataMap) throws Exception{
        ...
        //total calculation	
			for(int i=0;i<listOfFinCreditRevSubCategory.size();i++){
				...
				if(finCreditRevSubCategory.getSubCategoryItemType().equals(PennantConstants.CREDITREVIEW_CALCULATED_FIELD) &&
						!finCreditRevSubCategory.getItemsToCal().equals("")){		
					BigDecimal value = BigDecimal.ZERO;
					try{
						if((engine.eval(replaceYear(finCreditRevSubCategory.getItemsToCal(), year)).toString().equals("NaN")) ||
			...
			for(int i=0;i<listOfFinCreditRevSubCategory.size();i++){
				...
				if(!finCreditRevSubCategory.getItemRule().equals("")){		
					BigDecimal value = BigDecimal.ZERO;
					try{
						if((engine.eval(replaceYear(finCreditRevSubCategory.getItemRule(), year)).toString().equals("NaN")) ||
					...
					if(finCreditRevSubCategory.getCategoryId() == 4 || finCreditRevSubCategory.getCategoryId() == 7){
						...
						dataMap.put(PennantConstants.CREDITREVIEW_REMARKS+finCreditRevSubCategory.getSubCategoryCode(),value==null?BigDecimal.ZERO:value);
						curYearValuesMap.put(PennantConstants.CREDITREVIEW_REMARKS+finCreditRevSubCategory.getSubCategoryCode(),value==null?BigDecimal.ZERO:value);
					}
				}
			}
		}
	...
	public void onChange$auditedValue(ForwardEvent event) throws Exception {
		...
		engine.put("Y"+this.auditedYear.getValue()+PennantConstants.CREDITREVIEW_REMARKS+aFinCreditRevSubCategory.getSubCategoryCode(),
				  db_Amount.getValue() == null? BigDecimal.ZERO: db_Amount.getValue());
	...
	public void onClick$btnPrint(Event event) throws InterruptedException{
		...
		try {
			if(this.custCIF.getValue() != null){
				creditReviewMainCtgDetails.setCustomerId(String.valueOf(this.custCIF.getValue())+"-"+String.valueOf(this.custShrtName.getValue()));
			} else {
		...
		try {
			if(this.bankName.isVisible()) {
				...
				if(this.bankName.getValue() != null && !this.bankName.getValue().equals("")){
		...
		try {
			if(this.auditors.getValue() != null && !this.auditors.getValue().equals("")){
		...
		try {
			if(this.auditedDate.getValue() != null){
				creditReviewMainCtgDetails.setAuditedDate(PennantAppUtil.formateDate(this.auditedDate.getValue(), PennantConstants.dateFormat));
			} else {
		...
		try {
			if(this.marketPrice.getValue() != null){
				creditReviewMainCtgDetails.setMarketPrice(String.valueOf(this.marketPrice.getValue()));
			} else {
				throw new WrongValueException( this.marketPrice, Labels.getLabel( "FIELD_NO_EMPTY",
						new String[] {Labels.getLabel("label_CreditApplicationReviewDialog_MarketPrice.value"),
						Labels.getLabel("label_CreditApplicationReviewDialog_MarketPrice.value") }));
			}
		} catch (WrongValueException we) {
			wve.add(we);
		}
		...
		creditReviewMainCtgDetails.setConsolOrUnConsol(this.conSolOrUnConsol.getSelectedItem().getValue().
				toString().equals(PennantConstants.CREDITREVIEW_CONSOLIDATED) ? "True" : "False");
		...
		if (wve.size() > 0) {
			...
			for (CreditReviewSubCtgDetails ctgDetails : creditReviewSubCtgDetailsList) {
				if (!ctgDetails.getSubCategoryDesc().equals("")) {
					continue;
				}
				ctgDetails.setCurYearAuditValueHeader(audQual);
				ctgDetails.setCurrencyConvertion(PennantConstants.CURRENCY_USD);
			}
			...
			ReportGenerationUtil.generateReport("CreditApplication_Review", creditReviewMainCtgDetails, list, true, 1,
					usrName, this.window_CreditApplicationReviewDialog);	
		}
	...
	public void onCheck$qualifiedUnQualified(Event event){
		logger.debug("Entering" + event.toString());
		setLables();
		if(this.qualifiedUnQualified.getSelectedItem().getValue().equals(PennantConstants.CREDITREVIEW_UNQUALIFIED)){
			//this.auditPeriod.setDisabled(false);
		} else {
	...
	private List<Filter> getFilterList() {
		...
		//filterList.add(new Filter("CustCtgCode", new String("RETAIL")));
		return filterList;
	}
	...
	private void setLables() {
		String qualUnQual=this.qualifiedUnQualified.getSelectedItem().getValue().toString();
		String audQual = Labels.getLabel("UnAudUnQual_Months", new String[] { this.auditType.getSelectedItem().getValue().toString().equals("#") ? 
				         PennantConstants.CREDITREVIEW_AUDITED : this.auditType.getSelectedItem().getValue().toString(), qualUnQual});
	...
	public void onClick$gb_basicDetails(Event event){
