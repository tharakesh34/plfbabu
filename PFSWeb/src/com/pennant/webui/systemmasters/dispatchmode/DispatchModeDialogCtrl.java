 ...
 * /WEB-INF/pages/SystemMaster/DispatchMode/dispatchModeDialog.zul file. <br>
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
 */

public class DispatchModeDialogCtrl extends GFCBaseCtrl implements Serializable {
	...
	private transient String 	oldVar_dispatchModeCode;
	private transient String 	oldVar_dispatchModeDesc;
	private transient boolean 	oldVar_dispatchModeIsActive;
	...
	private transient final String btnCtroller_ClassPrefix = "button_DispatchModeDialog_";
	...
	public void onCreate$window_DispatchModeDialog(Event event)throws Exception {
		...
		if (args.containsKey("dispatchMode")) {
			this.dispatchMode = (DispatchMode) args.get("dispatchMode");
		...
		doLoadWorkFlow(this.dispatchMode.isWorkflow(), this.dispatchMode.getWorkflowId(), this.dispatchMode.getNextTaskId());

		if (isWorkFlowEnabled()) {
			this.userAction = setListRecordStatus(this.userAction);
			getUserWorkspace().alocateRoleAuthorities(getRole(), "DispatchModeDialog");
		}
		...
		if (args.containsKey("dispatchModeListCtrl")) {
			setDispatchModeListCtrl((DispatchModeListCtrl) args.get("dispatchModeListCtrl"));
		} else {
	...
	private void doCheckRights() {
		...
		getUserWorkspace().alocateAuthorities("DispatchModeDialog");
	 ...
	 * @throws Exception
	 */
	public void onClose$window_DispatchModeDialog(Event event) throws Exception {
		logger.debug("Entering" + event.toString());
		doClose();
		logger.debug("Leaving" + event.toString());
	}
	...
	public void onClick$btnHelp(Event event) throws InterruptedException {
		logger.debug("Entering" + event.toString());
		PTMessageUtils.showHelpWindow(event, window_DispatchModeDialog);
		logger.debug("Leaving" + event.toString());
	}
	...
	private void doClose() throws InterruptedException {
		...
		if (close) {
			closeDialog(this.window_DispatchModeDialog, "DispatchMode");
		}
		logger.debug("Leaving");
	}
	 ...
	 * @throws InterruptedException
	 */
	public void doShowDialog(DispatchMode aDispatchMode) throws InterruptedException {
		logger.debug("Entering");

		// if aDispatchMode == null then we opened the Dialog without
		// arguments for a given entity, so we get a new Object().
		if (aDispatchMode == null) {
			/** !!! DO NOT BREAK THE TIERS !!! */
			// We don't create a new DomainObject() in the frontEnd.
			// We GET it from the backEnd.
			aDispatchMode = getDispatchModeService().getNewDispatchMode();

			setDispatchMode(aDispatchMode);
		} else {
			setDispatchMode(aDispatchMode);
		}
		...
		if (aDispatchMode.isNew()) {
			...
			if (isWorkFlowEnabled()) {
				if (!StringUtils.trimToEmpty(aDispatchMode.getRecordType()).equals("")) {
		...
		try {
			...
			// during user action.
			doStoreInitValues();
			setDialog(this.window_DispatchModeDialog);
	...
	private void doStoreInitValues() {
		logger.debug("Entering");
		this.oldVar_dispatchModeCode = this.dispatchModeCode.getValue();
		this.oldVar_dispatchModeDesc = this.dispatchModeDesc.getValue();
		this.oldVar_dispatchModeIsActive = this.dispatchModeIsActive.isChecked();
	...
	private void doResetInitValues() {
		logger.debug("Entering");
		this.dispatchModeCode.setValue(this.oldVar_dispatchModeCode);
		this.dispatchModeDesc.setValue(this.oldVar_dispatchModeDesc);
		this.dispatchModeIsActive.setChecked(this.oldVar_dispatchModeIsActive);
	...
	private boolean isDataChanged() {
		...
		if (this.oldVar_dispatchModeCode != this.dispatchModeCode.getValue()) {
			return true;
		}
		if (this.oldVar_dispatchModeDesc != this.dispatchModeDesc.getValue()) {
			return true;
		}
		if (this.oldVar_dispatchModeIsActive != this.dispatchModeIsActive.isChecked()) {
			return true;
		}
		return false;
	}
	...
	/**
	 * Create a new DispatchMode object. <br>
	 */
	private void doNew() {
		logger.debug("Entering");
	...
	private boolean doProcess(DispatchMode aDispatchMode, String tranType) {
		...
		aDispatchMode.setLastMntBy(getUserWorkspace().getLoginUserDetails().getLoginUsrID());
		aDispatchMode.setLastMntOn(new Timestamp(System.currentTimeMillis()));
		aDispatchMode.setUserDetails(getUserWorkspace().getLoginUserDetails());
		...
		if (isWorkFlowEnabled()) {
			...
			if ("Save".equals(userAction.getSelectedItem().getLabel())) {
				...
				if ("".equals(nextTaskId)) {
					nextTaskId = getWorkFlow().getNextTaskIds(taskId, aDispatchMode);
				}

				if (PennantConstants.WF_Audit_Notes.equals(getWorkFlow().getAuditingReq(taskId, aDispatchMode))) {
					try {
						if (!isNotes_Entered()) {
			...
			String operationRefs = getWorkFlow().getOperationRefs(taskId, aDispatchMode);
	...
	private void refreshList() {
		logger.debug("Entering");
		final JdbcSearchObject<DispatchMode> soDispatchMode = getDispatchModeListCtrl().getSearchObj();
		getDispatchModeListCtrl().pagingDispatchModeList.setActivePage(0);
		getDispatchModeListCtrl().getPagedListWrapper().setSearchObject(soDispatchMode);
		if (getDispatchModeListCtrl().listBoxDispatchMode != null) {
			getDispatchModeListCtrl().listBoxDispatchMode.getListModel();
		}
		logger.debug("Leaving");
	}
	...
	private Notes getNotes() {
		...
		notes.setModuleName("DispatchMode");
		notes.setReference(getDispatchMode().getDispatchModeCode());
		notes.setVersion(getDispatchMode().getVersion());
		logger.debug("Leaving");
		return notes;
	}
