...
public class AccountEngineEventSearchCtrl extends GFCBaseCtrl implements Serializable {

	private static final long serialVersionUID = -5457162932741884160L;
	private final static Logger logger = Logger.getLogger(AccountEngineEventSearchCtrl.class);
	 ...
	 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	 */
	protected Window  window_AccountEngineEventSearch; 	// autoWired
	protected Textbox aEEventCode; 						// autoWired
	protected Listbox sortOperator_aEEventCode; 		// autoWired
	protected Textbox aEEventCodeDesc; 					// autoWired
	protected Listbox sortOperator_aEEventCodeDesc; 	// autoWired
	protected Textbox recordStatus; 					// autoWired
	protected Listbox recordType;						// autoWired
	protected Listbox sortOperator_recordStatus; 		// autoWired
	protected Listbox sortOperator_recordType; 			// autoWired

	protected Label label_AccountEngineEventSearch_RecordStatus; 	// autoWired
	protected Label label_AccountEngineEventSearch_RecordType; 		// autoWired
	protected Label label_AccountEngineEventSearchResult; 			// autoWired
	...
	public void onCreate$window_AccountEngineEventSearch(Event event) throws Exception {
		...
		if (args.containsKey("accountEngineEventCtrl")) {
			this.accountEngineEventCtrl = (AccountEngineEventListCtrl)args.get("accountEngineEventCtrl");
		} else {
		...
		this.sortOperator_aEEventCode.setModel(new ListModelList(new SearchOperators().getStringOperators()));
		this.sortOperator_aEEventCode.setItemRenderer(new SearchOperatorListModelItemRenderer());

		this.sortOperator_aEEventCodeDesc.setModel(new ListModelList(new SearchOperators().getStringOperators()));
		...
		if (args.containsKey("searchObject")) {
			...
			final JdbcSearchObject<AccountEngineEvent> searchObj = (JdbcSearchObject<AccountEngineEvent>) args.get("searchObject");
			...
			for (final Filter filter : ft) {
				...
				if (filter.getProperty().equals("aEEventCode")) {
				...
				} else if (filter.getProperty().equals("aEEventCodeDesc")) {
	...
	private void doClose() {
		...
		this.window_AccountEngineEventSearch.onClose();
