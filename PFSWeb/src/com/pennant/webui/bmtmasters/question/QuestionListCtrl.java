 ...
 * 
 */
public class QuestionListCtrl extends GFCBaseListCtrl<Question> implements Serializable {

	private static final long serialVersionUID = 1L;
	private final static Logger logger = Logger.getLogger(QuestionListCtrl.class);
	...
	public void onCreate$window_QuestionList(Event event) throws Exception {
		...
		ModuleMapping moduleMapping = PennantJavaUtil.getModuleMap("Question");
		boolean wfAvailable=true;
		
		if (moduleMapping.getWorkflowType()!=null){
			workFlowDetails = WorkFlowUtil.getWorkFlowDetails("Question");
		...
		if (!isWorkFlowEnabled() && wfAvailable){
			this.button_QuestionList_NewQuestion.setVisible(false);
			this.button_QuestionList_QuestionSearchDialog.setVisible(false);
			this.button_QuestionList_PrintList.setVisible(false);
	...
	private void doCheckRights() {
		logger.debug("Entering");
		getUserWorkspace().alocateAuthorities("QuestionList");
		...
		if (item != null) {
			...
			if(question==null){
				...
				if(isWorkFlowEnabled()){
					...
					boolean userAcces =  validateUserAccess(workFlowDetails.getId(),getUserWorkspace().getLoginUserDetails().getLoginUsrID(), "Question", whereCond, question.getTaskId(), question.getNextTaskId());
					if (userAcces){
	...
	private void showDetailView(Question aQuestion) throws Exception {
		...
		if(aQuestion.getWorkflowId()==0 && isWorkFlowEnabled()){
			aQuestion.setWorkflowId(workFlowDetails.getWorkFlowId());
		}
	...
	public void onClick$btnHelp(Event event) throws InterruptedException {
		logger.debug(event.toString());
		PTMessageUtils.showHelpWindow(event, window_QuestionList);
		logger.debug("Leaving");
	}
