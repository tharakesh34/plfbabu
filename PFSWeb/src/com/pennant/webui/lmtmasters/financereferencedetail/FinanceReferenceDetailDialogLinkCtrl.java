...
public class FinanceReferenceDetailDialogLinkCtrl extends GFCBaseCtrl implements Serializable {
	...
	private void doEdit() {
		...
		if (getFinanceReferenceDetail().getFinRefType() == PennantConstants.ScoringGroup ||
				...
				getFinanceReferenceDetail().getFinRefType() == PennantConstants.PoliceDedupe) {
	...
	public void onClick$btnSearchPoliceDedupe(Event event) {
		logger.debug("Entering");
		Filter[] filters = new Filter[1];
		filters[0] = new Filter("QueryModule", PennantConstants.DedupPolice, Filter.OP_EQUAL);
	...
	private void doDesignByType(FinanceReferenceDetail finRefDetail) {
		...
		case PennantConstants.PoliceDedupe:
