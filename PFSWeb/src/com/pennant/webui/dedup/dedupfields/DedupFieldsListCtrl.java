 ...
 * 
 */
public class DedupFieldsListCtrl extends GFCBaseListCtrl<DedupFields> implements Serializable {

	private static final long serialVersionUID = 1L;
	private final static Logger logger = Logger.getLogger(DedupFieldsListCtrl.class);
	...
	public void onCreate$window_DedupFieldsList(Event event) throws Exception {
		...
		ModuleMapping moduleMapping = PennantJavaUtil.getModuleMap("DedupFields");
		boolean wfAvailable=true;
		
		if (moduleMapping.getWorkflowType()!=null){
			workFlowDetails = WorkFlowUtil.getWorkFlowDetails("DedupFields");
		...
		if (!isWorkFlowEnabled() && wfAvailable){
			this.button_DedupFieldsList_NewDedupFields.setVisible(false);
			this.button_DedupFieldsList_DedupFieldsSearchDialog.setVisible(false);
			this.button_DedupFieldsList_PrintList.setVisible(false);
	...
	private void doCheckRights() {
		logger.debug("Enterring");
		getUserWorkspace().alocateAuthorities("DedupFieldsList");
	...
	public void onDedupFieldsItemDoubleClicked(Event event) throws Exception {
		...
		if (item != null) {
			...
			if(dedupFields==null){
				...
				ErrorDetails errorDetails = getDedupFieldsService().getErrorDetails("41005", getUserWorkspace().getUserLanguage(), parm);
				PTMessageUtils.showErrorMessage(errorDetails.getErrorMessage());
			}else{
				if(isWorkFlowEnabled()){
					String whereCond =  " AND FieldName='"+ dedupFields.getFieldName()+"' AND version=" + dedupFields.getVersion()+" ";

					boolean userAcces =  validateUserAccess(workFlowDetails.getId(),getUserWorkspace().getLoginUserDetails().getLoginUsrID(), "DedupFields", whereCond, dedupFields.getTaskId(), dedupFields.getNextTaskId());
					if (userAcces){
	...
	private void showDetailView(DedupFields aDedupFields) throws Exception {
		...
		if(aDedupFields.getWorkflowId()==0 && isWorkFlowEnabled()){
			aDedupFields.setWorkflowId(workFlowDetails.getWorkFlowId());
		}
	...
	public void onClick$btnHelp(Event event) throws InterruptedException {
		logger.debug(event.toString());
		PTMessageUtils.showHelpWindow(event, window_DedupFieldsList);
		logger.debug("Leaving");
	}
	...
	public void onClick$button_DedupFieldsList_PrintList(Event event) throws InterruptedException {
		logger.debug("Enterring");
		logger.debug(event.toString());
		PTReportUtils.getReport("DedupFields", getSearchObj());
		logger.debug("Leaving");
	}
