...
/**
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
 * This is the controller class for the /WEB-INF/pages/CustomerMasters/CustomerRating/CustomerRatingList.zul
 * file.<br>
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
 * 
 */
public class CustomerRatingListCtrl extends GFCBaseListCtrl<CustomerRating> implements Serializable {
	 ...
	 * ZUL-file is called with a parameter for a selected CustomerRating object in
	 * a Map.
	 * 
	 * @param event
	 * @throws Exception
	...
	public void findSearchObject(){
		...
		final SearchResult<CustomerRating> searchResult = getPagedListService()
		.getSRBySearchObject(this.searchObj);
		listBoxCustomerRating.setModel(new GroupsModelArray(
				searchResult.getResult().toArray(),new CustomerRatingComparator()));
	 ...
	 * see: com.pennant.webui.customermasters.customerrating.model.CustomerRatingListModelItemRenderer.java <br>
	 * 
	 * @param event
	 * @throws Exception
	 */
	public void onCustomerRatingItemDoubleClicked(Event event) throws Exception {
		...
		if (item != null) {
			...
			final CustomerRating customerRating = getCustomerRatingService().getCustomerRatingById(
					aCustomerRating.getId(),aCustomerRating.getCustRatingType());
			...
			if (customerRating == null) {
				...
				String whereCond =  " AND CustID='"+ customerRating.getCustID()+"' AND custRatingType='"+
									customerRating.getCustRatingType()+"' AND version=" + 
									customerRating.getVersion()+" ";

				if(isWorkFlowEnabled()){
					boolean userAcces =  validateUserAccess(workFlowDetails.getId(),
							getUserWorkspace().getLoginUserDetails().getLoginUsrID(),
							"CustomerRating", whereCond, customerRating.getTaskId(),
							customerRating.getNextTaskId());
					if (userAcces){
	 ...
	 * 
	 * @param CustomerRating (aCustomerRating)
	 * @throws Exception
	 */
	private void showDetailView(CustomerRating aCustomerRating) throws Exception {
		logger.debug("Entering");
		/*
		...
		try {
			Executions.createComponents(
					"/WEB-INF/pages/CustomerMasters/CustomerRating/CustomerRatingDialog.zul",null,map);
	...
	public void onClick$btnRefresh(Event event) throws InterruptedException {
		logger.debug("Entering" + event.toString());
		this.pagingCustomerRatingList.setActivePage(0);
		Events.postEvent("onCreate", this.window_CustomerRatingList, event);
		this.window_CustomerRatingList.invalidate();
		logger.debug("Leaving" + event.toString());
	}
	...
	public void onClick$button_CustomerRatingList_CustomerRatingSearchDialog(Event event) throws Exception {
		logger.debug("Entering" + event.toString());
		/*
		 * we can call our CustomerRatingDialog ZUL-file with parameters. So we can
		 * call them with a object of the selected CustomerRating. For handed over
		 * these parameter only a Map is accepted. So we put the CustomerRating object
		 * in a HashMap.
		 */
		final HashMap<String, Object> map = new HashMap<String, Object>();
		map.put("customerRatingCtrl", this);
		map.put("searchObject", this.searchObj);

		// call the ZUL-file with the parameters packed in a map
		try {
			Executions.createComponents("/WEB-INF/pages/CustomerMasters/CustomerRating/CustomerRatingSearchDialog.zul",null,map);
		} catch (final Exception e) {
			logger.error("onOpenWindow:: error opening window / " + e.getMessage());
			PTMessageUtils.showErrorMessage(e.toString());
		}
	 ...
	 * @throws InterruptedException
	 */
	public void onClick$button_CustomerRatingList_PrintList(Event event) throws InterruptedException {
		logger.debug("Entering" + event.toString());
		PTReportUtils.getReport("CustomerRating", getSearchObj());
		logger.debug("Leaving" + event.toString());
	}
