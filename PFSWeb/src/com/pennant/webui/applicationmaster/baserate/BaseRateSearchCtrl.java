...
public class BaseRateSearchCtrl extends GFCBaseCtrl implements Serializable {
	...
	public void onCreate$window_BaseRateSearch(Event event) throws Exception {
		...
		this.sortOperator_bREffDate.setModel(new ListModelList<SearchOperators>(
				new SearchOperators().getNumericOperators()));
		this.sortOperator_bREffDate.setItemRenderer(
				new SearchOperatorListModelItemRenderer());

		this.sortOperator_bRRate.setModel(new ListModelList<SearchOperators>(
				new SearchOperators().getNumericOperators()));
	...
	public void doSearch() {
		...
		if (this.bREffDate.getValue()!=null) {
			...
			// get the search operator
			final Listitem itemBREffDate = this.sortOperator_bREffDate.getSelectedItem();
			if (itemBREffDate != null) {
				final int searchOpId = ((SearchOperators) itemBREffDate
						.getAttribute("data")).getSearchOperatorId();
		...
		if (this.bRRate.getValue()!=null) {
			...
			// get the search operator
			final Listitem itemBRRate = this.sortOperator_bRRate.getSelectedItem();
			if (itemBRRate != null) {
				final int searchOpId = ((SearchOperators) itemBRRate
						.getAttribute("data")).getSearchOperatorId();
