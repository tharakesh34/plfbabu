 ...
 * /WEB-INF/pages/ApplicationMaster/ReportFilterFields/reportFilterFieldsDialog.zul file. <br>
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
 */
public class ReportFilterFieldsDialogCtrl extends GFCBaseCtrl implements Serializable {
	...
	private transient String  		oldVar_fieldDBName;
	private transient String  		oldVar_appUtilMethodName;
	private transient String  	    oldVar_moduleName;
	private transient String  		oldVar_lovHiddenFieldMethod; 					
	private transient String 		oldVar_lovTextFieldMethod;
	private transient boolean  		oldVar_multiSelectSearch;
	private transient int  		    oldVar_fieldLength;
	private transient int  			oldVar_fieldMaxValue;
	private transient int  			oldVar_fieldMinValue;
	private transient String  		oldVar_fieldConstraint;
	private transient boolean  	    oldVar_mandatory;
	private transient int  			oldVar_seqOrder; 					
	private transient String 		oldVar_whereCondition;
	private transient String  		oldVar_fieldErrorMessage;
	private transient String  		oldVar_staticValue;
	private transient int  			oldVar_fieldWidth;
	private transient boolean  		oldVar_filterRequired;
	private transient String  		oldVar_defaultFilter;
	...
	private transient final String btnCtroller_ClassPrefix = "button_ReportFilterFieldDialog_";
	...
	public enum FIELDTYPE {TXT, DATE,TIME,DATETIME, STATICLIST, DYNAMICLIST, LOVSEARCH, DECIMAL,INTRANGE,DECIMALRANGE
		...
		public void onCreate$window_ReportFilterFieldsDialog(Event event) throws Exception {
			...
			try {
				...
				if (args.containsKey("reportFilterFields")) {
					this.reportFilterFields = (ReportFilterFields) args.get("reportFilterFields");
				...
				if (isWorkFlowEnabled()){
					...
					getUserWorkspace().alocateRoleAuthorities(getRole(), "ReportFilterFieldDialog");
				}else{
					getUserWorkspace().alocateAuthorities("ReportFilterFieldDialog");
				}
				...
				if (args.containsKey("reportConfigurationDialogCtrl")) {
					this.reportConfigurationDialogCtrl = (ReportConfigurationDialogCtrl) args.get("reportConfigurationDialogCtrl");
				...
				e.printStackTrace();
				window_ReportFilterFieldsDialog.onClose();
			}
		 ...
		 * @throws Exception
		 */
		public void onClose$window_ReportFilterFieldsDialog(Event event) throws Exception {
			logger.debug("Entering" + event.toString());
			doClose();
			logger.debug("Leaving" + event.toString());
		}
		...
		public void onClick$btnHelp(Event event) throws InterruptedException {
			logger.debug("Entering" + event.toString());
			PTMessageUtils.showHelpWindow(event, window_ReportFilterFieldsDialog);
			logger.debug("Leaving" + event.toString());
		}
		...
		public void doWriteBeanToComponents(ReportFilterFields  aReportFilterFields ) throws Exception {
			...
			if(!aReportFilterFields.isNewRecord()){
				...
				if(this.moduleName.getSelectedItem()!=null && !this.moduleName.getSelectedItem().getValue().equals("")
			...
			this.recordType.setValue(PennantJavaUtil.getLabel(aReportFilterFields.getRecordType()));
		...
		public void doWriteComponentsToBean(ReportFilterFields  aReportFilterFields) {
			...
			try {
				if(filedType.equals("STATICVALUE")){
			...
			try {
				if(filedType.equals("STATICVALUE") || filedType.equals("MULTISELANDLIST")){
			...
			try {
				if(filedType.equals("STATICLIST") ||filedType.equals("MULTISELANDLIST") ||filedType.equals("MULTISELINLIST")){
			...
			if(filedType.equals("DYNAMICLIST")|| filedType.equals("LOVSEARCH") ){
				try {
			...
			try {
				if((filedType.equals("TXT") || filedType.equals("DECIMAL") || filedType.equals("NUMBER")) && this.fieldLength.intValue()==0){
			...
			if(filedType.equals("TXT") ){
				try {
					if(this.fieldConstraint.getValue().trim().equals("") ){
				...
				try {
					if(this.fieldErrorMessage.getValue().trim().equals("") ){
		 ...
		 * @throws InterruptedException
		 */
		public void doShowDialog(ReportFilterFields aReportFilterFields) throws InterruptedException {
			logger.debug("Entering ");
			// if aReportFilterFields == null then we opened the Dialog without
			// arguments for a given entity, so we get a new Object().
			if (aReportFilterFields == null) {
				/** !!! DO NOT BREAK THE TIERS !!! */
				// We don't create a new DomainObject() in the frontEnd.
				// We GET it from the backEnd.
				aReportFilterFields = new ReportFilterFields();
				aReportFilterFields.setNewRecord(true);
				setReportFilterFields(aReportFilterFields);
			} else {
				setReportFilterFields(aReportFilterFields);
			}
		...
		private void doStoreInitValues() {
			...
			this.oldVar_fieldDBName = this.fieldDBName.getValue();
			this.oldVar_appUtilMethodName = this.appUtilMethodName.getValue();
			this.oldVar_moduleName = this.moduleName.getValue();
			this.oldVar_lovHiddenFieldMethod = this.lovHiddenFieldMethod.getValue();
			this.oldVar_lovTextFieldMethod = this.lovTextFieldMethod.getValue();
			this.oldVar_multiSelectSearch = this.multiSelectSearch.isChecked();
			this.oldVar_fieldLength = this.fieldLength.intValue();
			this.oldVar_fieldMaxValue = this.fieldMaxValue.intValue();
			this.oldVar_fieldMinValue = this.fieldMinValue.intValue();
			this.oldVar_seqOrder = this.seqOrder.intValue();
			this.oldVar_mandatory = this.mandatory.isChecked();
			this.oldVar_fieldConstraint = this.fieldConstraint.getValue();
			this.oldVar_fieldErrorMessage = this.fieldErrorMessage.getValue();
			this.oldVar_whereCondition = this.whereCondition.getValue();
			this.oldVar_staticValue = this.staticValue.getValue();
			this.oldVar_fieldWidth = this.fieldWidth.intValue();
			this.oldVar_filterRequired = this.filterRequired.isChecked();
			this.oldVar_defaultFilter = this.defaultFilter.getValue();
		...
		private void doResetInitValues() {
			...
			this.fieldDBName.setValue(this.oldVar_fieldDBName);
			this.appUtilMethodName.setValue(this.oldVar_appUtilMethodName);
			this.moduleName.setValue(this.oldVar_moduleName);
			this.lovHiddenFieldMethod.setValue(this.oldVar_lovHiddenFieldMethod);
			this.lovTextFieldMethod.setValue(this.oldVar_lovTextFieldMethod);
			this.multiSelectSearch.setChecked(this.oldVar_multiSelectSearch);
			...
			this.seqOrder.setValue(this.oldVar_seqOrder);
			this.mandatory.setChecked(this.oldVar_mandatory);
			this.fieldConstraint.setValue(this.oldVar_fieldConstraint);
			this.fieldErrorMessage.setValue(this.oldVar_fieldErrorMessage);
			this.whereCondition.setValue(this.oldVar_whereCondition);
			this.staticValue.setValue(this.oldVar_staticValue);
			this.fieldWidth.setValue(this.oldVar_fieldWidth);
			this.filterRequired.setChecked(this.oldVar_filterRequired);
			this.defaultFilter.setValue(this.oldVar_defaultFilter);
		...
		private boolean isDataChanged() {
			...
			if (!this.oldVar_fieldName.equals(this.fieldName.getValue())){
				return true;
			}
			if (!this.oldVar_fieldType.equals(this.fieldType.getValue()) ){
				return true;
			}
			if (!this.oldVar_fieldLabel.equals(this.fieldLabel .getValue())) {
				return true;
			}
			if (!this.oldVar_fieldDBName.equals(this.fieldDBName.getValue())) {
				return true;
			}
			if (!this.oldVar_appUtilMethodName.equals(this.appUtilMethodName.getValue())) {
				return true;
			}
			if (!this.oldVar_moduleName.equals(this.moduleName.getValue())){
				return true;
			}
			if (!this.oldVar_lovHiddenFieldMethod.equals(this.lovHiddenFieldMethod.getValue()) ){
				return true;
			}
			if (!this.oldVar_lovTextFieldMethod.equals(this.lovTextFieldMethod.getValue())) {
				return true;
			}
			if (this.oldVar_multiSelectSearch != this.multiSelectSearch.isChecked()) {
				return true;
			}
			if (this.oldVar_fieldLength!=this.fieldLength.intValue()) {
				return true;
			}
			if (this.oldVar_fieldMaxValue!=this.fieldMaxValue.intValue()) {
				return true;
			}
			if (this.oldVar_fieldMinValue!=this.fieldMinValue.intValue()){
				return true;
			}
			if (this.oldVar_seqOrder!=this.seqOrder.intValue()){
				return true;
			}
			if (this.oldVar_mandatory != this.mandatory.isChecked()) {
				return true;
			}
			if (!this.oldVar_fieldConstraint.equals(this.fieldConstraint.getValue())) {
				return true;
			}
			if (!this.oldVar_fieldErrorMessage.equals(this.fieldErrorMessage.getValue())) {
				return true;
			}
			if (!this.oldVar_whereCondition.equals(this.whereCondition.getValue())) {
				return true;
			}
			if (!this.oldVar_staticValue.equals(this.staticValue.getValue())) {
				return true;
			}
			if (this.oldVar_fieldWidth!=this.fieldWidth.intValue()) {
				return true;
			}
			if (this.oldVar_filterRequired!=this.filterRequired.isChecked()) {
				return true;
			}
			if (!this.oldVar_defaultFilter.equals(this.defaultFilter.getValue())) {
				return true;
			}
			return false;
		}
		...
		private void doDelete() throws InterruptedException {
			...
			if (conf==MultiLineMessageBox.YES){
				...
				if (StringUtils.trimToEmpty(aReportFilterFields.getRecordType()).equals("")){
		...
		public void doReadOnly(boolean readOnly) {
			...
			if(readOnly|| (!readOnly && (PennantConstants.RECORD_TYPE_DEL.equals(reportFilterFields.getRecordType())))) {
				tempReadOnly=true;
			}
		...
		public void doClear(boolean isAll) {
			...
			if(isAll ||this.fieldType.getSelectedItem().getValue().equals("STATICVALUE") ){
				if(isAll){
		...
		public void setFieldType(String selectedFieldType){
			...
			if(!selectedFieldType.equals(PennantConstants.List_Select)&&
					selectedFieldType.equals("LOVSEARCH") 
					|| selectedFieldType.equals("DYNAMICLIST")){
				...
				this.appUtilMethodName.setValue(Labels.getLabel("Combo.Select"));
				if(selectedFieldType.equals("DYNAMICLIST")){
			...
			if(!selectedFieldType.equals(PennantConstants.List_Select)&&
					(selectedFieldType.equals("NUMBER")|| selectedFieldType.equals("DECIMAL"))){
			...
			if(!selectedFieldType.equals(PennantConstants.List_Select)&&
					(selectedFieldType.equals("MULTISELANDLIST")|| selectedFieldType.equals("MULTISELINLIST")) 
					|| selectedFieldType.equals("STATICLIST")){
				this.row_Two.setVisible(true);
				this.moduleName.setDisabled(true);
			}else{
				this.moduleName.setDisabled(false);
			}   
			if(selectedFieldType.equals("MULTISELANDLIST")){
			...
			if(!selectedFieldType.equals(PennantConstants.List_Select)&&
					(selectedFieldType.equals("STATICVALUE"))){
			...
			if(selectedFieldType.equals("TXT") || selectedFieldType.equals("DECIMAL") 
					|| selectedFieldType.equals("NUMBER")){
		...
		public void onChange$appUtilMethodName(Event event) throws Exception {
			logger.debug("Entering " + event.toString());
			if(!this.appUtilMethodName.getSelectedItem().getValue().equals("")
		...
		public void onChange$moduleName(Event event) throws Exception {
			logger.debug("Entering " + event.toString());
			if(!this.moduleName.getSelectedItem().getValue().equals("")
					&& !this.moduleName.getSelectedItem().getValue().equals(PennantConstants.List_Select)){
		...
		public void getAllMethods(String className,Combobox component)throws Exception{
			...
			Class value= PennantJavaUtil.getModuleMap().get(className).getModuleClass();
			//Get the methods
		...
		private Notes getNotes() {
			...
			notes.setModuleName("ReportFilterFields");
			notes.setReference(getReportFilterFields().getFieldName());
			notes.setVersion(getReportFilterFields().getVersion());
			logger.debug("Leaving");
			return notes;
		}
