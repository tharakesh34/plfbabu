...
public class CustomerEnquiryDialogCtrl extends GFCBaseListCtrl<FinanceEnquiry> implements Serializable {
	...
	public void doFillFinDetails(long custId) {
		...
		for (int i = 0; i < financeEnqList.size(); i++) {
			...
			if((i == financeEnqList.size() -1)){
				addListFooter(grpTotal, aFinanceEnq.getLovDescFinFormatter());
			}else{
	...
	private void addListItem(FinanceEnquiry aFinanceEnq){
		...
		lc.setParent(item);
		lc = new Listcell(DateUtility.formatDate(aFinanceEnq.getFinStartDate(),	PennantConstants.dateFormate));
		lc.setParent(item);
		lc = new Listcell(DateUtility.formatDate(aFinanceEnq.getGrcPeriodEndDate(),	PennantConstants.dateFormate));
		lc.setParent(item);
		lc = new Listcell(String.valueOf(aFinanceEnq.getNumberOfTerms()));
		lc.setParent(item);
		lc = new Listcell(DateUtility.formatDate(aFinanceEnq.getMaturityDate(),PennantConstants.dateFormate));
		lc.setParent(item);
		lc = new Listcell(PennantAppUtil.amountFormate(aFinanceEnq.getFinAmount(),aFinanceEnq.getLovDescFinFormatter()));
		...
		lc.setStyle("text-align:right");
		lc.setParent(item);
		lc = new Listcell(DateUtility.formatDate(aFinanceEnq.getNextDueDate(), PennantConstants.dateFormate));
		lc.setParent(item);
		lc = new Listcell(PennantAppUtil.amountFormate(aFinanceEnq.getNextDueAmount(),aFinanceEnq.getLovDescFinFormatter()));
		lc.setStyle("text-align:right");
		lc.setParent(item);
	...
	private void addListGroup(FinanceEnquiry aFinanceEnq){
		...
		cell = new Listcell(aFinanceEnq.getFinCcy() +" - "+aFinanceEnq.getLovDescFinCcyName());
		cell.setSpan(7);
		listgroup.appendChild(cell);
