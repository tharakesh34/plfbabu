 ...
 * /WEB-INF/pages/Finance/ReScheduleDialog.zul file. <br>
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
 */
public class ReScheduleDialogCtrl extends GFCBaseCtrl implements Serializable {
	 ...
	 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	 */
	protected Window window_ReScheduleDialog; 		// autowired
	...
	protected Decimalbox repayPftRate; 					// autowired
	...
	public void onCreate$window_ReScheduleDialog(Event event) throws Exception {
		...
		this.window_ReScheduleDialog.doModal();
	...
	public void doShowDialog(FinScheduleData aFinScheduleData) throws InterruptedException {
		...
		try {
			...
			// during user action.
			doStoreInitValues();
			setDialog(this.window_ReScheduleDialog);
	...
	public void doWriteBeanToComponents(FinScheduleData aFinSchData) {
		...
		fillFrqCode(this.cbRepayFrqCode, aFinanceMain.getRepayFrq(), false);
		clearField(this.cbRepayFrqMth);
		fillFrqMth(this.cbRepayFrqMth, aFinanceMain.getRepayFrq(), false);
	...
	public void doWriteComponentsToBean() throws InterruptedException {
		...
		try {
			if(this.nextRepayDate.getValue() != null){
				if(this.nextRepayDate.getValue().compareTo(tempStartDate) <= 0){
					throw new WrongValueException(this.nextRepayDate, Labels.getLabel("DATE_ALLOWED_MINDATE", new String[]{
							Labels.getLabel("label_ReScheduleDialog_NextRepayDate.value"),DateUtility.formatUtilDate(tempStartDate, PennantConstants.dateFormat)}));
				}
			}
		}catch (WrongValueException we ) {
			wve.add(we);
		...
		try {
			if(this.repayPftRate.getValue() == null){
				throw new WrongValueException(this.repayPftRate, Labels.getLabel("RATE_NO_NUMBER", new String[]{
						Labels.getLabel("label_ReScheduleDialog_RepayPftRate.value"), " 0 "}));
			}
		}catch (WrongValueException we ) {
			wve.add(we);
		}
	...
	public void onSelect$cbRepayFrqCode(Event event) {
		logger.debug("Entering" + event.toString());
		String frqCode = getComboboxValue(cbRepayFrqCode);
		onSelectFrqCode(frqCode, cbRepayFrqCode, cbRepayFrqMth, cbRepayFrqDay, repayFrq, false);
	...
	public void onSelect$cbRepayFrqMth(Event event) {
		...
		onSelectFrqMth(frqCode, frqMth, this.cbRepayFrqMth,	this.cbRepayFrqDay, this.repayFrq,false);
		this.nextGrcRepayDate.setText("");
		this.nextRepayDate.setText("");
		this.grcPeriodEndDate.setText("");
		logger.debug("Leaving" + event.toString());
