...
public class TreasuaryFinHeaderListModelItemRenderer implements ListitemRenderer<InvestmentFinHeader>, Serializable {
   ...
   @Override
	public void render(Listitem item, InvestmentFinHeader data, int count) throws Exception {
		...
		lc.setParent(item);
		lc = new Listcell(PennantAppUtil.amountFormate(treasuaryFinance.getTotPrincipalAmt(), treasuaryFinance.getLovDescFinFormatter()));
		lc.setStyle("text-align:right;");
		lc.setParent(item);
		lc = new Listcell(treasuaryFinance.getFinCcy());
		lc.setParent(item);
		lc = new Listcell(PennantApplicationUtil.formateDate(treasuaryFinance.getStartDate(), PennantConstants.dateFormate));
		lc.setParent(item);
		lc = new Listcell(PennantApplicationUtil.formateDate(treasuaryFinance.getMaturityDate(), PennantConstants.dateFormate));
		lc.setParent(item);
		lc = new Listcell(treasuaryFinance.getRecordStatus());
		lc.setParent(item);
	  	lc = new Listcell(treasuaryFinance.getRecordType());
		lc.setParent(item);
