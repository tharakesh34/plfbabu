 ...
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
 */
public class IjarahWIFFinanceMainDialogCtrl extends GFCBaseCtrl implements Serializable {

	private static final long serialVersionUID = 6004939933729664895L;
	private final static Logger logger = Logger.getLogger(IjarahWIFFinanceMainDialogCtrl.class);
	 ...
	 */
	public IjarahWIFFinanceMainDialogCtrl() {
		super();
	}
	...
	public void onClick$btnHelp(Event event) throws InterruptedException {
		logger.debug("Entering " + event.toString());
		PTMessageUtils.showHelpWindow(event, window_IjarahFinanceMainDialog);
		logger.debug("Leaving " + event.toString());
	}
	...
	public void onClick$btnClose(Event event) throws InterruptedException, IllegalAccessException, InvocationTargetException {
		...
		try {
			...
			closeDialog(this.window_IjarahFinanceMainDialog,"financeMain");
		}
		logger.debug("Leaving " + event.toString());
	}
	...
	public void doWriteBeanToComponents(FinanceDetail aFinanceDetail, boolean onLoadProcess) throws ParseException, InterruptedException, AccountNotFoundException, IllegalAccessException, InvocationTargetException {
		...
		if (aFinanceDetail.getFinScheduleData().getFinanceType().isFinDepreciationReq()) {
			...
			this.label_IjarahFinanceMainDialog_DepriFrq.setVisible(true);
			...
			this.label_IjarahFinanceMainDialog_DepriFrq.setVisible(false);
		...
		if (getFinanceDetail().getFinScheduleData().getFinanceType().isFinIsAlwFrqDifferment()) {
			...
			this.label_IjarahFinanceMainDialog_FrqDef.setVisible(false);
		}
	...
	public void doWriteComponentsToBean(FinScheduleData aFinanceSchData) throws InterruptedException, IllegalAccessException, InvocationTargetException {
		...
		try {
			...
			if(this.finCcy.getValue().equals("")) {
				wve.add(new WrongValueException(this.lovDescFinCcyName, Labels.getLabel("FIELD_NO_INVALID", new String[] { Labels.getLabel("label_IjarahFinanceMainDialog_FinCcy.value") })));
			} else {
		...
		try {
			if (getComboboxValue(this.cbScheduleMethod).equals("#")) {
				throw new WrongValueException(this.cbScheduleMethod, Labels.getLabel("STATIC_INVALID",
						new String[] { Labels.getLabel("label_IjarahFinanceMainDialog_ScheduleMethod.value") }));
			}
		...
		try {
			if (getComboboxValue(this.cbProfitDaysBasis).equals("#")) {
				throw new WrongValueException(this.cbProfitDaysBasis, Labels.getLabel("STATIC_INVALID",
						new String[] { Labels.getLabel("label_IjarahFinanceMainDialog_ProfitDaysBasis.value") }));
			}
		...
		try {
			if (!this.defferments.isReadonly() && this.defferments.intValue() != 0 && 
				...
				throw new WrongValueException(this.defferments, Labels.getLabel("FIELD_IS_LESSER",
						new String[] {Labels.getLabel("label_IjarahFinanceMainDialog_Defferments.value"),
		...
		try {
			if (!this.frqDefferments.isReadonly() && this.frqDefferments.intValue() != 0 && 
				...
				throw new WrongValueException(this.frqDefferments,Labels.getLabel("FIELD_IS_LESSER",
						new String[] { Labels.getLabel("label_IjarahFinanceMainDialog_FrqDefferments.value"),
			...
			try {
				if(getComboboxValue(this.grcRateBasis).equals("#")) {
					throw new WrongValueException(this.grcRateBasis, Labels.getLabel("STATIC_INVALID",
							new String[] { Labels.getLabel("label_IjarahFinanceMainDialog_GrcRateBasis.value") }));
				}
				...
				if (this.gracePftRate.getValue() != null && !this.gracePftRate.isDisabled()) {
					if ((this.gracePftRate.getValue().intValue() > 0) && 
						...
						throw new WrongValueException(this.gracePftRate, Labels.getLabel("EITHER_OR",
								new String[] {Labels.getLabel("label_IjarahFinanceMainDialog_GraceBaseRate.value"),
								Labels.getLabel("label_IjarahFinanceMainDialog_GracePftRate.value") }));
					}
			...
			try {
				if (this.allowGrcRepay.isChecked() && getComboboxValue(this.cbGrcSchdMthd).equals("#")) {
					throw new WrongValueException(this.cbGrcSchdMthd, Labels.getLabel("STATIC_INVALID",
							new String[] { Labels.getLabel("label_IjarahFinanceMainDialog_GrcSchdMthd.value") }));
				}
		...
		try {
			...
			if (!recSave && this.graceTerms_Two.intValue() == 0 && this.gracePeriodEndDate_two.getValue() == null) {
				throw new WrongValueException(this.graceTerms, Labels.getLabel("EITHER_OR",
						new String[] {Labels.getLabel("label_IjarahFinanceMainDialog_GracePeriodEndDate.value"),
						Labels.getLabel("label_IjarahFinanceMainDialog_GraceTerms.value") }));
				...
				if(this.finStartDate.getValue().compareTo(this.gracePeriodEndDate.getValue()) > 0){
					...
					throw new WrongValueException(this.gracePeriodEndDate, Labels.getLabel("NUMBER_MINVALUE_EQ",
							new String[] {Labels.getLabel("label_IjarahFinanceMainDialog_GracePeriodEndDate.value"), 
							Labels.getLabel("label_IjarahFinanceMainDialog_FinStartDate.value")}));
				}
			}
		} catch (WrongValueException we) {
			wve.add(we);
			...
			if (this.repayProfitRate.getValue() != null && !this.repayProfitRate.isDisabled()) {
				if ((this.repayProfitRate.getValue().intValue() > 0)
					...
					throw new WrongValueException(this.repayProfitRate, Labels.getLabel("EITHER_OR",
							new String[] {Labels.getLabel("label_IjarahFinanceMainDialog_RepayBaseRate.value"),
							Labels.getLabel("label_IjarahFinanceMainDialog_ProfitRate.value") }));
				}
		...
		try {
			...
			if (this.numberOfTerms.intValue() != 0 && this.maturityDate_two.getValue() == null) {
				if (this.numberOfTerms.intValue() < 0) {
					this.numberOfTerms.setConstraint("NO NEGATIVE:" + Labels.getLabel("CONST_NO_EMPTY_NEGATIVE_ZERO",
							new String[] { Labels.getLabel("label_IjarahFinanceMainDialog_NumberOfTerms.value") }));
				}
			...
			if (!recSave && this.numberOfTerms_two.intValue() == 0 && this.maturityDate_two.getValue() == null) {
				throw new WrongValueException(this.numberOfTerms, Labels.getLabel("EITHER_OR",
						...
						Labels.getLabel("label_IjarahFinanceMainDialog_NumberOfTerms.value") }));
			...
			if (recSave) {
				...
				if (downPayment.compareTo(this.finAmount.getValue()) > 0) {
					throw new WrongValueException(this.downPayBank, Labels.getLabel("MAND_FIELD_MIN",
							new String[] {Labels.getLabel("label_IjarahFinanceMainDialog_DownPayment.value"),
				...
				if (downPayment.compareTo(reqDwnPay) == -1) {
					throw new WrongValueException(this.downPayBank, Labels.getLabel("PERC_MIN",
							new String[] {Labels.getLabel("label_IjarahFinanceMainDialog_DownPayBS.value"),
		...
		if(buildEvent) {
			...
			if (getFeeDetailDialogCtrl() != null) {
				try {
					aFinanceSchData = getFeeDetailDialogCtrl().doExecuteFeeCharges(true, true, aFinanceSchData,true,aFinanceMain.getFinStartDate());
		...
		try {
			...
			if(getFinanceDetail().getFinScheduleData().getFinanceType().getFinMinTerm() == 1 && 
				...
				if(!getFinanceDetail().getFinScheduleData().getFinanceType().isFinRepayPftOnFrq()){
					this.label_IjarahFinanceMainDialog_FinRepayPftOnFrq.setVisible(false);
					this.rpyPftFrqRow.setVisible(false);
					this.hbox_finRepayPftOnFrq.setVisible(false);
				}else{
					this.label_IjarahFinanceMainDialog_FinRepayPftOnFrq.setVisible(true);
					this.rpyPftFrqRow.setVisible(true);
	...
	private void doSetValidation() {
		...
		if(!this.frqDefferments.isDisabled()){
			this.frqDefferments.setConstraint(new PTNumberValidator(Labels.getLabel("label_IjarahFinanceMainDialog_FrqDefferments.value"), false, false));
		}
		...
		if (this.gb_gracePeriodDetails.isVisible()) {
			...
			if (!this.graceTerms.isReadonly()) {
				this.graceTerms_Two.setConstraint("NO NEGATIVE:" + Labels.getLabel("CONST_NO_EMPTY_NEGATIVE_ZERO",
						new String[] { Labels.getLabel("label_IjarahFinanceMainDialog_GraceTerms.value") }));
			}	
			
			if (!this.grcMargin.isDisabled()) {
				this.grcMargin.setConstraint(new RateValidator(13, 9, 
						Labels.getLabel("label_IjarahFinanceMainDialog_GraceMargin.value"), true));
			}

			if(this.allowGrace.isChecked()){
				this.grcEffectiveRate.setConstraint(new RateValidator(13, 9,
						Labels.getLabel("label_IjarahFinanceMainDialog_GracePftRate.value"), true));
			}
		...
		this.repayEffectiveRate.setConstraint(new RateValidator(13, 9,
				Labels.getLabel("label_IjarahFinanceMainDialog_ProfitRate.value"), true));

		if (!this.repayMargin.isDisabled()) {
			this.repayMargin.setConstraint(new RateValidator(13, 9, 
					Labels.getLabel("label_IjarahFinanceMainDialog_RepayMargin.value"), true));
		}
		...
		if (isWorkFlowEnabled()) {
			...
			while (!"".equals(serviceTasks)) {
				...
				if (StringUtils.trimToEmpty(method).contains(PennantConstants.method_doDedup)) {
					...
					tFinanceDetail = FetchDedupDetails.getLoanDedup(getRole(),aFinanceDetail, this.window_IjarahFinanceMainDialog);
			...
			while (retValue == PennantConstants.porcessOVERIDE) {
				...
				if (StringUtils.trimToEmpty(method).equalsIgnoreCase("")) {
					...
					if (StringUtils.trimToEmpty(method).equalsIgnoreCase(PennantConstants.method_doApprove)) {
						...
						retValue = ErrorControl.showErrorControl(this.window_IjarahFinanceMainDialog, auditHeader);
						return processCompleted;
					}
				}

				auditHeader = ErrorControl.showErrorDetails(this.window_IjarahFinanceMainDialog, auditHeader);
	...
	public void onClick$btnSearchFinType(Event event) {
		...
		Object dataObject = ExtendedSearchListBox.show(this.window_IjarahFinanceMainDialog, "FinanceType");
	...
	public void onClick$btnSearchGraceBaseRate(Event event) throws InterruptedException {
		...
		this.grcEffectiveRate.setConstraint("");
		Object dataObject = ExtendedSearchListBox.show(this.window_IjarahFinanceMainDialog, "BaseRateCode");
	...
	public void onClick$btnSearchGraceSpecialRate(Event event) throws InterruptedException {
		...
		this.grcEffectiveRate.setConstraint("");
		Object dataObject = ExtendedSearchListBox.show(this.window_IjarahFinanceMainDialog, "SplRateCode");
		...
		while (retValue == PennantConstants.porcessOVERIDE) {
			...
			if (!this.finReference.isReadonly() && this.finReference.getValue() != null) {
				if (getFinanceDetailService().isFinReferenceExits(this.finReference.getValue(), "_View", false)) {
					...
					errorList.add(new ErrorDetails("finReference","E0006",new String[] {
							Labels.getLabel("label_IjarahFinanceMainDialog_FinReference.value"),this.finReference.getValue().toString() },new String[] {}));
				}
			}
			...
			if (getFinanceDetail().getFinScheduleData().getFinanceMain().isAllowGrcPeriod()) {
				...
				if (!validateFrquency(this.cbGracePftFrqCode, this.gracePftFrq, this.nextGrcPftDate_two)) {
					...
					errorList.add(new ErrorDetails("nextGrcPftDate_two", "W0004", new String[] {
							Labels.getLabel("label_IjarahFinanceMainDialog_NextGrcPftDate.value"), Labels.getLabel("label_IjarahFinanceMainDialog_GracePftFrq.value"),
				...
				if (!validateFrquency(this.cbGracePftRvwFrqCode, this.gracePftRvwFrq, this.nextGrcPftRvwDate_two)) {
					errorList.add(new ErrorDetails("nextGrcPftRvwDate_two", "W0004", new String[] {
							Labels.getLabel("label_IjarahFinanceMainDialog_NextGrcPftRvwDate.value"),
							Labels.getLabel("label_IjarahFinanceMainDialog_GracePftRvwFrq.value"), Labels.getLabel("finGracePeriodDetails") }, 
				...
				if (!validateFrquency(this.cbGraceCpzFrqCode, this.graceCpzFrq, this.nextGrcCpzDate_two)) {
					errorList.add(new ErrorDetails("nextGrcCpzDate_two","W0004", new String[] {
							Labels.getLabel("label_IjarahFinanceMainDialog_NextGrcCpzDate.value"),
							Labels.getLabel("label_IjarahFinanceMainDialog_GraceCpzFrq.value"), Labels.getLabel("finGracePeriodDetails") },
			...
			if (!validateFrquency(this.cbRepayFrqCode, this.repayFrq, this.nextRepayDate_two)) {
				errorList.add(new ErrorDetails("nextRepayDate_two", "W0004", new String[] {
						Labels.getLabel("label_IjarahFinanceMainDialog_NextRepayDate.value"),
						Labels.getLabel("label_IjarahFinanceMainDialog_RepayFrq.value"), Labels.getLabel("finRepaymentDetails") },
			...
			if (!validateFrquency(this.cbRepayPftFrqCode, this.repayPftFrq, this.nextRepayPftDate_two)) {
				errorList.add(new ErrorDetails("nextRepayPftDate_two","W0004", new String[] {
						Labels.getLabel("label_IjarahFinanceMainDialog_NextRepayPftDate.value"), 
						Labels.getLabel("label_IjarahFinanceMainDialog_RepayPftFrq.value"), Labels.getLabel("WIFinRepaymentDetails") },
			...
			if (!validateFrquency(this.cbRepayRvwFrqCode, this.repayRvwFrq, this.nextRepayRvwDate_two)) {
				errorList.add(new ErrorDetails("nextRepayRvwDate_two", "W0004", new String[] {
						Labels.getLabel("label_IjarahFinanceMainDialog_NextRepayRvwDate.value"),
						Labels.getLabel("label_IjarahFinanceMainDialog_RepayRvwFrq.value"), Labels.getLabel("finRepaymentDetails") },
			...
			if (!validateFrquency(this.cbRepayCpzFrqCode, this.repayCpzFrq, this.nextRepayCpzDate_two)) {
				errorList.add(new ErrorDetails("nextRepayCpzDate_two", "W0004", new String[] {
						Labels.getLabel("label_IjarahFinanceMainDialog_NextRepayCpzDate.value"),
						Labels.getLabel("label_IjarahFinanceMainDialog_RepayCpzFrq.value"), Labels.getLabel("finRepaymentDetails") },
			...
			if (!this.numberOfTerms.isReadonly() && this.numberOfTerms.intValue() != 0 && !singleTermFinance) {
				if (this.numberOfTerms.intValue() >= 1 && this.maturityDate.getValue() != null) {
					errorList.add(new ErrorDetails("numberOfTerms","E0011", new String[] {
							Labels.getLabel("label_IjarahFinanceMainDialog_NumberOfTerms.value"),
							Labels.getLabel("label_IjarahFinanceMainDialog_MaturityDate.value") }, new String[] {}));
				}
			}
			...
			auditHeader = ErrorControl.showErrorDetails(window_IjarahFinanceMainDialog, auditHeader);
	...
	private void showMessage(Exception e) {
		...
		try {
			...
			ErrorControl.showErrorControl(this.window_IjarahFinanceMainDialog, auditHeader);
		} catch (Exception exp) {
			logger.error(exp);
		}
	}
