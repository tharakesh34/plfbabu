...
public class FinanceRepayPriorityListCtrl extends GFCBaseListCtrl<FinanceRepayPriority> implements Serializable {
	...
	public void onClick$button_FinanceRepayPriorityList_FinanceRepayPrioritySearchDialog(Event event) throws Exception {
		...
		logger.debug(event.toString());
		/*
		 * we can call our FinanceRepayPriorityDialog zul-file with parameters. So we can
		 * call them with a object of the selected FinanceRepayPriority. For handed over
		 * these parameter only a Map is accepted. So we put the FinanceRepayPriority object
		 * in a HashMap.
		 */
		final HashMap<String, Object> map = new HashMap<String, Object>();
		map.put("financeRepayPriorityCtrl", this);
		map.put("searchObject", this.searchObj);

		// call the zul-file with the parameters packed in a map
		try {
			Executions.createComponents("/WEB-INF/pages/Finance/FinanceRepayPriority/FinanceRepayPrioritySearchDialog.zul",null,map);
		} catch (final Exception e) {
			logger.error("onOpenWindow:: error opening window / " + e.getMessage());
			PTMessageUtils.showErrorMessage(e.toString());
		}
	...
	public void onClick$button_FinanceRepayPriorityList_PrintList(Event event) throws InterruptedException {
		logger.debug("Entering");
		logger.debug(event.toString());
		PTReportUtils.getReport("FinanceRepayPriority", getSearchObj());
		logger.debug("Leaving");
	}
