 ...
 ********************************************************************************************
 *																							*
 * FileName    		:  FinanceDisbursementDialogCtrl.java                                                   * 	  
 *                                                                    						*
 ...
 * /WEB-INF/pages/Finance/FinanceDisbursement/financeDisbursementDialog.zul file. <br>
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
 */
public class FinanceDisbursementDialogCtrl extends GFCBaseCtrl implements Serializable {

	private static final long serialVersionUID = 1L;
	private final static Logger logger = Logger.getLogger(FinanceDisbursementDialogCtrl.class);
	 ...
	 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	 */
	protected Window window_FinanceDisbursementDialog; // autowired
	...
	private FinanceDisbursement prvFinanceDisbursement; // overhanded per param
	private transient FinanceDisbursementListCtrl financeDisbursementListCtrl; // overhanded per param
	 ...
	 */
	public FinanceDisbursementDialogCtrl() {
		super();
	}
	 ...
	 * @throws Exception
	 */
	public void onCreate$window_FinanceDisbursementDialog(Event event) throws Exception {
		...
		if (args.containsKey("financeDisbursement")) {
			...
			BeanUtils.copyProperties(this.financeDisbursement, befImage);
		...
		doLoadWorkFlow(this.financeDisbursement.isWorkflow(),this.financeDisbursement.getWorkflowId(),this.financeDisbursement.getNextTaskId());
		...
		// READ OVERHANDED params !
		// we get the financeDisbursementListWindow controller. So we have access
		// to it and can synchronize the shown data when we do insert, edit or
		// delete financeDisbursement here.
		if (args.containsKey("financeDisbursementListCtrl")) {
			setFinanceDisbursementListCtrl((FinanceDisbursementListCtrl) args.get("financeDisbursementListCtrl"));
		} else {
			setFinanceDisbursementListCtrl(null);
		}
	...
	private void doCheckRights() {
		...
		getUserWorkspace().alocateAuthorities("FinanceDisbursementDialog");
	...
	public void onClick$btnHelp(Event event) throws InterruptedException {
		logger.debug(event.toString());
		PTMessageUtils.showHelpWindow(event, window_FinanceDisbursementDialog);
		logger.debug("Leaving");
	}
	...
	private void doClose() throws InterruptedException {
		...
		if(close){
			closeDialog(this.window_FinanceDisbursementDialog, "FinanceDisbursement");	
		}
	...
	public void doWriteBeanToComponents(FinanceDisbursement aFinanceDisbursement) {
		logger.debug("Entering") ;
		this.finReference.setValue(aFinanceDisbursement.getFinReference());
		this.disbDate.setValue(aFinanceDisbursement.getDisbDate());
		this.disbSeq.setValue(aFinanceDisbursement.getDisbSeq());
		this.disbDesc.setValue(aFinanceDisbursement.getDisbDesc());
  		this.disbAmount.setValue(PennantAppUtil.formateAmount(aFinanceDisbursement.getDisbAmount(),0));
		this.disbActDate.setValue(aFinanceDisbursement.getDisbReqDate());
		this.disbDisbursed.setChecked(aFinanceDisbursement.isDisbDisbursed());
		this.disbIsActive.setChecked(aFinanceDisbursement.isDisbIsActive());
	...
	public void doWriteComponentsToBean(FinanceDisbursement aFinanceDisbursement) {
		...
		try {
		    aFinanceDisbursement.setFinReference(this.finReference.getValue());
		...
		try {
		    aFinanceDisbursement.setDisbSeq(this.disbSeq.getValue());
		}catch (WrongValueException we ) {
			wve.add(we);
		}
		try {
		    aFinanceDisbursement.setDisbDesc(this.disbDesc.getValue());
		}catch (WrongValueException we ) {
			wve.add(we);
		}
		try {
			if(this.disbAmount.getValue()!=null){
			 	aFinanceDisbursement.setDisbAmount(PennantAppUtil.unFormateAmount(this.disbAmount.getValue(), 0));
			}
		}catch (WrongValueException we ) {
			wve.add(we);
		}
		try {
		    aFinanceDisbursement.setDisbReqDate(this.disbActDate.getValue());
		}catch (WrongValueException we ) {
			wve.add(we);
		}
		try {
			aFinanceDisbursement.setDisbDisbursed(this.disbDisbursed.isChecked());
		}catch (WrongValueException we ) {
			wve.add(we);
		}
		try {
			aFinanceDisbursement.setDisbIsActive(this.disbIsActive.isChecked());
		}catch (WrongValueException we ) {
			wve.add(we);
		}
		try {
	...
	public void doShowDialog(FinanceDisbursement aFinanceDisbursement) throws InterruptedException {
		...
		if (aFinanceDisbursement == null) {
			...
			aFinanceDisbursement = getFinanceDisbursementService().getNewFinanceDisbursement(false);
		...
		try {
			...
			// during user action.
			doStoreInitValues();
			setDialog(this.window_FinanceDisbursementDialog);
	...
	private void doSetValidation() {
		...
		if (!this.finReference.isReadonly()){
			this.finReference.setConstraint("NO EMPTY:" + Labels.getLabel("FIELD_NO_EMPTY",new String[]{Labels.getLabel("label_FinanceDisbursementDialog_FinReference.value")}));
		}	
		if (!this.disbDate.isDisabled()){
			this.disbDate.setConstraint("NO EMPTY:" + Labels.getLabel("FIELD_NO_EMPTY",new String[]{Labels.getLabel("label_FinanceDisbursementDialog_DisbDate.value")}));
		}
		if (!this.disbSeq.isReadonly()){
			this.disbSeq.setConstraint(new IntValidator(10,Labels.getLabel("label_FinanceDisbursementDialog_DisbSeq.value")));
		}	
		if (!this.disbDesc.isReadonly()){
			this.disbDesc.setConstraint("NO EMPTY:" + Labels.getLabel("FIELD_NO_EMPTY",new String[]{Labels.getLabel("label_FinanceDisbursementDialog_DisbDesc.value")}));
		}	
		if (!this.disbAmount.isReadonly()){
			this.disbAmount.setConstraint(new AmountValidator(18,0,Labels.getLabel("label_FinanceDisbursementDialog_DisbAmount.value")));
		}	
		if (!this.disbActDate.isDisabled()){
			this.disbActDate.setConstraint("NO EMPTY:" + Labels.getLabel("FIELD_NO_EMPTY",new String[]{Labels.getLabel("label_FinanceDisbursementDialog_DisbActDate.value")}));
		}
		if (!this.disbRemarks.isReadonly()){
			this.disbRemarks.setConstraint("NO EMPTY:" + Labels.getLabel("FIELD_NO_EMPTY",new String[]{Labels.getLabel("label_FinanceDisbursementDialog_DisbRemarks.value")}));
		}	
	logger.debug("Leaving");
	}
	...
	private void doDelete() throws InterruptedException {
		...
		BeanUtils.copyProperties(getFinanceDisbursement(), aFinanceDisbursement);
	...
	private void doEdit() {
		...
		this.disbSeq.setReadonly(isReadOnly("FinanceDisbursementDialog_disbSeq"));
		this.disbDesc.setReadonly(isReadOnly("FinanceDisbursementDialog_disbDesc"));
		this.disbAmount.setReadonly(isReadOnly("FinanceDisbursementDialog_disbAmount"));
	 	this.disbActDate.setDisabled(isReadOnly("FinanceDisbursementDialog_disbActDate"));
	 	this.disbDisbursed.setDisabled(isReadOnly("FinanceDisbursementDialog_disbDisbursed"));
	 	this.disbIsActive.setDisabled(isReadOnly("FinanceDisbursementDialog_disbIsActive"));
	...
	private boolean doProcess(FinanceDisbursement aFinanceDisbursement,String tranType){
		logger.debug("Entering");
		boolean processCompleted=false;
		AuditHeader auditHeader =  null;
		String nextRoleCode="";
		
		aFinanceDisbursement.setLastMntBy(getUserWorkspace().getLoginUserDetails().getLoginUsrID());
		aFinanceDisbursement.setLastMntOn(new Timestamp(System.currentTimeMillis()));
		aFinanceDisbursement.setUserDetails(getUserWorkspace().getLoginUserDetails());
		
		if (isWorkFlowEnabled()) {
			String taskId = getWorkFlow().getTaskId(getRole());
			String nextTaskId = "";
			//Upgraded to ZK-6.5.1.1 Added casting to String 	
			aFinanceDisbursement.setRecordStatus(userAction.getSelectedItem().getValue().toString());

			if ("Save".equals(userAction.getSelectedItem().getLabel())) {
				nextTaskId = taskId + ";";
			} else {
				nextTaskId = StringUtils.trimToEmpty(aFinanceDisbursement.getNextTaskId());

				nextTaskId = nextTaskId.replaceFirst(taskId + ";", "");
				if ("".equals(nextTaskId)) {
					nextTaskId = getWorkFlow().getNextTaskIds(taskId, aFinanceDisbursement);
				}

				if (PennantConstants.WF_Audit_Notes.equals(getWorkFlow().getAuditingReq(taskId,aFinanceDisbursement))) {
					try {
						if (!isNotes_Entered()){
			...
			aFinanceDisbursement.setTaskId(taskId);
			aFinanceDisbursement.setNextTaskId(nextTaskId);
			aFinanceDisbursement.setRoleCode(getRole());
			aFinanceDisbursement.setNextRoleCode(nextRoleCode);
			
			auditHeader =  getAuditHeader(aFinanceDisbursement, tranType);
			
			String operationRefs = getWorkFlow().getOperationRefs(taskId,aFinanceDisbursement);
			...
			if ("".equals(operationRefs)) {
				...
				for (int i = 0; i < list.length; i++) {
					auditHeader =  getAuditHeader(aFinanceDisbursement, PennantConstants.TRAN_WF);
	...
	private boolean doSaveProcess(AuditHeader auditHeader,String method){
		...
		FinanceDisbursement aFinanceDisbursement = (FinanceDisbursement) auditHeader.getAuditDetail().getModelData();
			...
			while(retValue==PennantConstants.porcessOVERIDE){
				...
				if (StringUtils.trimToEmpty(method).equalsIgnoreCase("")){
					if (auditHeader.getAuditTranType().equals(PennantConstants.TRAN_DEL)){
						auditHeader = getFinanceDisbursementService().delete(auditHeader,false);
						deleteNotes=true;
					}else{
						auditHeader = getFinanceDisbursementService().saveOrUpdate(auditHeader,false);	
					}
					
				}else{
					if (StringUtils.trimToEmpty(method).equalsIgnoreCase(PennantConstants.method_doApprove)){
						auditHeader = getFinanceDisbursementService().doApprove(auditHeader,false);

						if(aFinanceDisbursement.getRecordType().equals(PennantConstants.RECORD_TYPE_DEL)){
							deleteNotes=true;
						}

					}else if (StringUtils.trimToEmpty(method).equalsIgnoreCase(PennantConstants.method_doReject)){
						auditHeader = getFinanceDisbursementService().doReject(auditHeader,false);
						if(aFinanceDisbursement.getRecordType().equals(PennantConstants.RECORD_TYPE_NEW)){
							deleteNotes=true;
						}

					}else{
						auditHeader.setErrorDetails(new ErrorDetails(PennantConstants.ERR_9999, Labels.getLabel("InvalidWorkFlowMethod"), null));
						retValue = ErrorControl.showErrorControl(this.window_FinanceDisbursementDialog, auditHeader);
						return processCompleted; 
					}
				}
				
				auditHeader =	ErrorControl.showErrorDetails(this.window_FinanceDisbursementDialog, auditHeader);
	...
	public void setFinanceDisbursement(FinanceDisbursement financeDisbursement) {
	...
	public void setFinanceDisbursementListCtrl(FinanceDisbursementListCtrl financeDisbursementListCtrl) {
		this.financeDisbursementListCtrl = financeDisbursementListCtrl;
	}

	public FinanceDisbursementListCtrl getFinanceDisbursementListCtrl() {
		return this.financeDisbursementListCtrl;
	}
	...
	private AuditHeader getAuditHeader(FinanceDisbursement aFinanceDisbursement, String tranType){
		AuditDetail auditDetail = new AuditDetail(tranType, 1, aFinanceDisbursement.getBefImage(), aFinanceDisbursement);   
		return new AuditHeader(aFinanceDisbursement.getFinReference(),null,null,null,auditDetail,aFinanceDisbursement.getUserDetails(),getOverideMap());
	}
	...
	private void showMessage(Exception e){
		...
		try {
			...
			ErrorControl.showErrorControl(this.window_FinanceDisbursementDialog, auditHeader);
		} catch (Exception exp) {
			logger.error(exp);
		}
	}
	...
	private Notes getNotes(){
		...
		notes.setReference(getFinanceDisbursement().getFinReference());
...
private void refreshList(){
		final JdbcSearchObject<FinanceDisbursement> soFinanceDisbursement = getFinanceDisbursementListCtrl().getSearchObj();
		getFinanceDisbursementListCtrl().pagingFinanceDisbursementList.setActivePage(0);
		getFinanceDisbursementListCtrl().getPagedListWrapper().setSearchObject(soFinanceDisbursement);
		if(getFinanceDisbursementListCtrl().listBoxFinanceDisbursement!=null){
			getFinanceDisbursementListCtrl().listBoxFinanceDisbursement.getListModel();
		}
	} 
	...
	public FinanceDisbursement getPrvFinanceDisbursement() {
		return prvFinanceDisbursement;
	}
}
