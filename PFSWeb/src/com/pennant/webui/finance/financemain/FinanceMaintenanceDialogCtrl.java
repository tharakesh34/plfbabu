 ...
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
 */
public class FinanceMaintenanceDialogCtrl extends FinanceBaseCtrl implements Serializable {
	...
	protected Label 		label_FinanceMaintenanceDialog_ScheduleMethod;
	...
	public void onCreate$window_FinanceMaintenanceDialog(Event event) throws Exception {
		...
		setLabel_FinanceMainDialog_CbbApproved(label_FinanceMaintenanceDialog_CbbApproved);
	 ...
	 * @throws Exception
	 */
	public void onClose$window_FinanceMaintenanceDialog(Event event) throws Exception {
		logger.debug("Entering " + event.toString());
		doClose();
		logger.debug("Leaving " + event.toString());
	}
	...
	public void onClick$btnHelp(Event event) throws InterruptedException {
		logger.debug("Entering " + event.toString());
		PTMessageUtils.showHelpWindow(event, window_FinanceMaintenanceDialog);
		logger.debug("Leaving " + event.toString());
	}
	...
	private void closeWindow(){
		...
		closeDialog(this.window_FinanceMaintenanceDialog, "FinanceMainDialog");
	}
		...
		try {
			...
			if(financeType.getFinMinTerm() == 1 &&  financeType.getFinMaxTerm() == 1){
				if(!financeType.isFinRepayPftOnFrq()){
					this.label_FinanceMaintenanceDialog_FinRepayPftOnFrq.setVisible(false);
					this.rpyPftFrqRow.setVisible(false);
					this.hbox_finRepayPftOnFrq.setVisible(false);
				}else{
					this.label_FinanceMaintenanceDialog_FinRepayPftOnFrq.setVisible(true);
			...
			setDialog(this.window_FinanceMaintenanceDialog);
	...
	/*		if(StringUtils.trimToEmpty(this.repayAcctId.getValue()).equals("")){
				this.repayAcctId.setValue(this.disbAcctId.getValue(),this.disbAcctId.getDesc());
			}*/
		} catch (WrongValueException we) {
			wve.add(we);
		}
...
/*	// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	*//**
	...
	private void doCheckFeeReExecution(){
		...
		if (this.oldVar_graceTerms != this.graceTerms_Two.intValue()) {
			isFeeReExecute = true;
		}
...
/*	*//**
	...
	private void doSetValidation() {
		...
		if (!this.finReference.isReadonly() && 
			...
			this.finReference.setConstraint(new PTStringValidator(Labels.getLabel("label_FinanceMaintenanceDialog_FinReference.value"),null,true));
		}

		if (!this.finAmount.isReadonly()) {
			this.finAmount.setConstraint(new AmountValidator(18,0,
					Labels.getLabel("label_FinanceMaintenanceDialog_FinAmount.value"), false));
		}
		if(!this.defferments.isReadonly()){
			this.defferments.setConstraint(new PTNumberValidator(Labels.getLabel("label_FinanceMaintenanceDialog_Defferments.value"), false, false));
		}

		if(!this.planDeferCount.isReadonly()){
			this.planDeferCount.setConstraint(new PTNumberValidator(Labels.getLabel("label_FinanceMaintenanceDialog_PlanDeferCount.value"), false, false));
		}
		...
		if (this.gb_gracePeriodDetails.isVisible()) {
			...
			if (!this.graceTerms.isReadonly()) {
				this.graceTerms.setConstraint(new PTStringValidator(Labels.getLabel("label_FinanceMaintenanceDialog_GraceTerms.value"),null,true));
			}
			
			if (!this.grcMargin.isReadonly()) {
				this.grcMargin.setConstraint(new RateValidator(13, 9, 
						Labels.getLabel("label_FinanceMaintenanceDialog_GraceMargin.value"), true));
			}

			if(this.allowGrace.isChecked()){
				this.grcEffectiveRate.setConstraint(new RateValidator(13, 9,
						Labels.getLabel("label_FinanceMaintenanceDialog_GracePftRate.value"), true));
			}

			if (!this.nextGrcPftDate.isReadonly() && 
					FrequencyUtil.validateFrequency(this.gracePftFrq.getValue()) == null) {

				this.nextGrcPftDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_FinanceMaintenanceDialog_NextGrcPftDate.value"),true));
			}

			if (!this.nextGrcPftRvwDate.isReadonly() && 
					FrequencyUtil.validateFrequency(this.gracePftRvwFrq.getValue()) == null) {

				this.nextGrcPftRvwDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_FinanceMaintenanceDialog_NextGrcPftRvwDate.value"),true));
			}
		}
		...
		if (!this.nextRepayDate.isReadonly() && 
			...
			this.nextRepayDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_FinanceMaintenanceDialog_NextRepayDate.value"),true));
		}

		if (!this.nextRepayPftDate.isReadonly() && 
				FrequencyUtil.validateFrequency(this.repayPftFrq.getValue()) == null) {

			this.nextRepayPftDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_FinanceMaintenanceDialog_NextRepayPftDate.value"),true));
		}

		if (!this.nextRepayRvwDate.isReadonly() && 
				FrequencyUtil.validateFrequency(this.repayRvwFrq.getValue()) == null) {

			this.nextRepayRvwDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_FinanceMaintenanceDialog_NextRepayRvwDate.value"),true));
		}

		if (!this.nextRepayCpzDate.isReadonly() && 
				FrequencyUtil.validateFrequency(this.repayCpzFrq.getValue()) == null) {

			this.nextRepayCpzDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_FinanceMaintenanceDialog_NextRepayCpzDate.value"),true));
		}

		this.repayEffectiveRate.setConstraint(new RateValidator(13, 9,
				Labels.getLabel("label_FinanceMaintenanceDialog_ProfitRate.value"), true));

		if (!this.repayMargin.isReadonly()) {
			this.repayMargin.setConstraint(new RateValidator(13, 9, 
					Labels.getLabel("label_FinanceMaintenanceDialog_RepayMargin.value"), true));
		}

		if(financeType.getFinMinTerm() == 1 &&
				financeType.getFinMaxTerm() == 1){

			this.maturityDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_FinanceMaintenanceDialog_MaturityDate.value"),true));
		}
	...
	private void doSetLOVValidation() {
		...
		this.lovDescFinTypeName.setConstraint(new PTStringValidator(Labels.getLabel("label_FinanceMaintenanceDialog_FinType.value"),null,true));

		this.finCcy.setConstraint(new PTStringValidator(Labels.getLabel("label_FinanceMaintenanceDialog_FinCcy.value"),null,true));

		if (!this.finBranch.isReadonly()) {
			this.finBranch.setConstraint(new PTStringValidator(Labels.getLabel("label_FinanceMaintenanceDialog_FinBranch.value"),null,true));
		}

		if (!this.custCIF.isReadonly()) {
			this.custCIF.setConstraint(new PTStringValidator(Labels.getLabel("label_FinanceMaintenanceDialog_CustID.value"),null,true));
		}

		if (StringUtils.trimToEmpty(this.disbAcctId.getSclass()).equals("mandatory")) {
			this.disbAcctId.setConstraint(new PTStringValidator(Labels.getLabel("label_FinanceMaintenanceDialog_DisbAcctId.value"),null,true));
		}

		if (this.repayAcctId.getSclass().equals("mandatory")) {
			this.repayAcctId.setConstraint(new PTStringValidator(Labels.getLabel("label_FinanceMaintenanceDialog_RepayAcctId.value"),null,true));
		}

		if (this.downPayAccount.getSclass().equals("mandatory")) {
			this.downPayAccount.setConstraint(new PTStringValidator(Labels.getLabel("label_FinanceMaintenanceDialog_DownPayAccount.value"),null,true));
		}

		if (!this.btnSearchCommitmentRef.isDisabled() && 
				StringUtils.trimToEmpty(space_commitmentRef.getSclass()).equals("mandatory")) {
			this.lovDescCommitmentRefName.setConstraint(new PTStringValidator(Labels.getLabel("label_FinanceMaintenanceDialog_CommitRef.value"),null,true));
		}

		if (!this.finPurpose.isReadonly()) {
			this.finPurpose.setConstraint(new PTStringValidator(Labels.getLabel("label_FinanceMaintenanceDialog_FinPurpose.value"),null,true));
		}

		//FinanceMain Details Tab ---> 2. Grace Period Details

		if(!this.graceBaseRate.isReadonly()) {
			this.graceBaseRate.setConstraint(new PTStringValidator(Labels.getLabel("label_FinanceMaintenanceDialog_GraceBaseRate.value"),null,true));
		}

		if(this.allowGrcInd.isChecked() && !this.btnSearchGrcIndBaseRate.isDisabled()){
			this.lovDescGrcIndBaseRateName.setConstraint(new PTStringValidator(Labels.getLabel("label_FinanceMaintenanceDialog_FinGrcIndBaseRate.value"),null,true));			
		}

		//FinanceMain Details Tab ---> 3. Repayment Period Details

		if(!this.repayBaseRate.isReadonly()) {
			this.repayBaseRate.setConstraint(new PTStringValidator(Labels.getLabel("label_FinanceMaintenanceDialog_RepayBaseRate.value"),null,true));
		}

		if(this.allowRpyInd.isChecked() && !this.btnSearchRpyIndBaseRate.isDisabled()){
			this.lovDescRpyIndBaseRateName.setConstraint(new PTStringValidator(Labels.getLabel("label_FinanceMaintenanceDialog_FinRpyIndBaseRate.value"),null,true));			
		}
		logger.debug("Leaving ");
	}
	...
	private void doDelete() throws InterruptedException {
		...
		if (conf == MultiLineMessageBox.YES) {
			...
			try {
				...
				showErrorMessage(this.window_FinanceMaintenanceDialog, e);
			}
		}
		logger.debug("Leaving");
	}
	...
	public void doSave() throws Exception {
		...
		String nextRoleCode = aFinanceDetail.getFinScheduleData().getFinanceMain().getNextRoleCode();
		if(nextRoleCode.equals("OPERATIONS_OFFICER") || 
				nextRoleCode.equals("MORT_OPERATIONS_OFFICER") || 
				nextRoleCode.equals("GOODS_OPERATIONS_OFFICER")){
...
/*
	*//**
	...
	public void onClick$btnSearchFinType(Event event) {
		...
		Object dataObject = ExtendedSearchListBox.show(this.window_FinanceMaintenanceDialog, "FinanceType");
	...
	*//**
	...
	public void onChange$lovDescFinCcyName(Event event) throws InterruptedException {
		...
		if (customer != null) {
			...
			finCcy.setValue("");
			throw new WrongValueException(this.lovDescFinCcyName, Labels.getLabel("FIELD_NO_INVALID", new String[] { Labels.getLabel("label_FinanceMaintenanceDialog_FinCcy.value") }));
		}
	...
	*//**
	...
	public void onClick$btnSearchFinCcy(Event event) {
		...
		Object dataObject = ExtendedSearchListBox.show(this.window_FinanceMaintenanceDialog, "Currency");
		if (dataObject instanceof String) {
	...
	public void onClick$btnSearchDisbAcctId(Event event) throws InterruptedException {
		...
		if(!StringUtils.trimToEmpty(this.lovDescCustCIF.getValue()).equals("")) {
			...
			try {
				...
				dataObject = ExtendedSearchListBox.show(this.window_FinanceMaintenanceDialog, "Accounts", iAccountList);
				if (dataObject instanceof String) {
...
*/
	...
	IllegalAccessException, InvocationTargetException {
		...
		if(this.finStartDate.getValue() != null){
			filters[2] = new Filter("CmtExpDate", this.finStartDate.getValue().toString(), Filter.OP_GREATER_OR_EQUAL);
		}else{
			throw new WrongValueException(finStartDate,Labels.getLabel("FIELD_NO_EMPTY",
					new String[] { Labels.getLabel("label_FinanceMaintenanceDialog_FinStartDate.value")}));
		}

		Object dataObject = ExtendedSearchListBox.show(this.window_FinanceMaintenanceDialog, "Commitment", filters);
	...
	private void onCheckCustAcceptance(){
		...
		this.label_FinanceMaintenanceDialog_CbbApproved.setVisible(false);
...
/*
	*//**
	...
	public void onChange$lovDescCustCIF(Event event) throws InterruptedException {
		...
		if (customer != null) {
			...
			this.commitmentRef.setValue("");
			this.lovDescCommitmentRefName.setValue("");
			throw new WrongValueException(this.lovDescCustCIF, Labels.getLabel("FIELD_NO_INVALID", new String[] { Labels.getLabel("label_FinanceMaintenanceDialog_CustID.value") }));
		}
	...
	*//**
	...
	private void setCustomerData(){
		...
		setAgreementDetailTab(this.window_FinanceMaintenanceDialog);
		...
		if(getWorkFlow().getTaskTabs(getWorkFlow().getTaskId(getRole())).equals("Accounting")){
			...
			if(getAccountingDetailDialogCtrl() != null){
				...
				if(!StringUtils.trimToEmpty(this.commitmentRef.getValue()).equals("")){
					getAccountingDetailDialogCtrl().doFillCmtAccounting(financeDetail.getCmtFinanceEntries());
				}
			}else{
				setAccountingDetailTab(this.window_FinanceMaintenanceDialog);
...
*/
	...
	*//**
	 ...
	 * @throws Exception 
	 *//*
	private boolean doSave_CheckList(FinanceDetail aFinanceDetail) throws Exception {
		logger.debug("Entering ");
	...
	*/
	...
	public void onClick$viewCustInfo(Event event){
		try {
			...
			if (finDivision.equals(PennantConstants.FIN_DIVISION_RETAIL)) {
				Executions.createComponents("/WEB-INF/pages/CustomerMasters/Customer/FinCustomerDetailsEnq.zul", window_FinanceMaintenanceDialog, map);
			}else{
				Executions.createComponents("/WEB-INF/pages/CustomerMasters/Enquiry/CustomerSummary.zul", window_FinanceMaintenanceDialog, map);
			}
		} catch (final Exception e) {
			logger.error("onOpenWindow:: error opening window / " + e.getMessage());
		}
	...
	public FinanceDetail getAgrFinanceDetails() throws Exception {
		...
		if(getScoringDetailDialogCtrl() != null){
			if (aFinanceDetail.isSufficientScore()) {
				if (!aFinanceDetail.isSufficientScore()) {
