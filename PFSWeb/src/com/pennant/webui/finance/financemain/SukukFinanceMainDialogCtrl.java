 ...
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
 */
public class SukukFinanceMainDialogCtrl extends FinanceBaseCtrl implements Serializable {
	...
	protected Label 		label_SukukFinanceMainDialog_ScheduleMethod;
	protected Label 		label_SukukFinanceMainDialog_FinRepayPftOnFrq;
	protected Label 		label_SukukFinanceMainDialog_CommitRef; 	// autoWired
	protected Label 		label_SukukFinanceMainDialog_DepriFrq; 	// autoWired
	protected Label 		label_SukukFinanceMainDialog_PlanDeferCount;	// autoWired
	protected Label 		label_SukukFinanceMainDialog_CbbApproved;
	protected Label 		label_SukukFinanceMainDialog_AlwGrace;
	protected Label 		label_SukukFinanceMainDialog_GraceMargin; 		// autoWired
	protected Label 		label_SukukFinanceMainDialog_StepPolicy; 		// autoWired
	protected Label 		label_SukukFinanceMainDialog_numberOfSteps; 		// autoWired
	
	protected Uppercasebox  issueNumber;
	protected Intbox		noOfUnits;
	protected Decimalbox  	faceValue;
	protected Combobox	  	premiumType;
	protected Decimalbox 	premiumValue;
	protected Decimalbox  	pricePerUnit;
	protected Decimalbox  	yieldValue;
	protected Datebox  		lastCouponDate;
	protected Decimalbox  	accruedProfit;
	protected Datebox  		purchaseDate;
	protected Decimalbox  	fairValuePerUnit;
	protected Decimalbox  	fairValueAmount;
	...
	public void onCreate$window_SukukFinanceMainDialog(Event event) throws Exception {
		...
		setLabel_FinanceMainDialog_CbbApproved(label_SukukFinanceMainDialog_CbbApproved);
		setLabel_FinanceMainDialog_CommitRef(label_SukukFinanceMainDialog_CommitRef);
		setLabel_FinanceMainDialog_DepriFrq(label_SukukFinanceMainDialog_DepriFrq);
		setLabel_FinanceMainDialog_FinRepayPftOnFrq(label_SukukFinanceMainDialog_FinRepayPftOnFrq);
		setLabel_FinanceMainDialog_PlanDeferCount(label_SukukFinanceMainDialog_PlanDeferCount);
		setLabel_FinanceMainDialog_AlwGrace(label_SukukFinanceMainDialog_AlwGrace);
		setLabel_FinanceMainDialog_GraceMargin(label_SukukFinanceMainDialog_GraceMargin);
		setLabel_FinanceMainDialog_StepPolicy(label_SukukFinanceMainDialog_StepPolicy);
		setLabel_FinanceMainDialog_numberOfSteps(label_SukukFinanceMainDialog_numberOfSteps);
	...
	public void onClick$btnHelp(Event event) throws InterruptedException {
		logger.debug("Entering " + event.toString());
		PTMessageUtils.showHelpWindow(event, window_SukukFinanceMainDialog);
		logger.debug("Leaving " + event.toString());
	}
	...
	private void doClose() throws Exception {
		...
		if (close) {
			closeWindow(this.window_SukukFinanceMainDialog);
		}
	...
	public void doWriteBeanToComponents(FinanceDetail aFinanceDetail, boolean onLoadProcess) throws ParseException, InterruptedException, 
		AccountNotFoundException, IllegalAccessException, InvocationTargetException {
		logger.debug("Entering");
		...
		this.space_commitmentRef.setSclass("");
		setDownPayAcMand();
		...
		int finFormatter = aFinanceDetail.getFinScheduleData().getFinanceMain().getLovDescFinFormatter();
		...
		if(premiumDetail.getPurchaseDate() == null && moduleDefiner.equals("")){
			Date curBussDate = (Date) SystemParameterDetails.getSystemParameterValue(PennantConstants.APP_DATE_CUR);
			premiumDetail.setPurchaseDate(curBussDate);
		}
	...
	public void doWriteComponentsToBean(FinanceDetail aFinanceDetail) throws InterruptedException, IllegalAccessException, InvocationTargetException {
		...
		super.doWriteComponentsToBean(aFinanceDetail.getFinScheduleData(), wve);
	...
	private FinancePremiumDetail doPreparePremiumDetail(FinanceDetail detail, String repayPftFrq, ArrayList<WrongValueException> wve){
		...
		int formatter = detail.getFinScheduleData().getFinanceMain().getLovDescFinFormatter();
				...
				if(this.lastCouponDate.getValue() != null){
					if(!FrequencyUtil.isFrqDate(repayPftFrq, this.lastCouponDate.getValue())){
						throw new WrongValueException(this.lastCouponDate,  Labels.getLabel("FRQ_DATE_MISMATCH", 
								...
								Labels.getLabel("label_SukukFinanceMainDialog_RepayPftFrq.value") }));
					}
					...
					if(this.finStartDate.getValue() != null){ 
						if(this.finStartDate.getValue().compareTo(this.lastCouponDate.getValue()) > 0){
							throw new WrongValueException(this.lastCouponDate,  Labels.getLabel("DATE_NOT_BEFORE", 
									new String[] { Labels.getLabel("label_SukukFinanceMainDialog_LastCouponDate.value"),
									Labels.getLabel("label_SukukFinanceMainDialog_FinStartDate.value") }));
						}
					...
					if(this.maturityDate_two.getValue() != null){
						if(this.lastCouponDate.getValue().compareTo(this.maturityDate_two.getValue()) >= 0){
							throw new WrongValueException(this.lastCouponDate,  Labels.getLabel("DATE_ALLOWED_BEFORE", 
									new String[] { Labels.getLabel("label_SukukFinanceMainDialog_LastCouponDate.value"),
									Labels.getLabel("label_SukukFinanceMainDialog_MaturityDate.value") }));
						}
					}
				}
			}
			...
			if(this.maturityDate_two.getValue() != null && !this.maturityDate.isDisabled()){
				if(this.purchaseDate.getValue().compareTo(this.maturityDate_two.getValue()) >= 0){
					throw new WrongValueException(this.maturityDate_two,  Labels.getLabel("DATE_ALLOWED_AFTER", 
							new String[] { Labels.getLabel("label_SukukFinanceMainDialog_MaturityDate.value"),
							Labels.getLabel("label_SukukFinanceMainDialog_PurchaseDate.value") }));
				}
			}
		...
		try {
			...
			if(financeType.getFinMinTerm() == 1 &&  financeType.getFinMaxTerm() == 1){
				if(!financeType.isFinRepayPftOnFrq()){
					this.label_SukukFinanceMainDialog_FinRepayPftOnFrq.setVisible(false);
					this.rpyPftFrqRow.setVisible(false);
					this.hbox_finRepayPftOnFrq.setVisible(false);
				}else{
					this.label_SukukFinanceMainDialog_FinRepayPftOnFrq.setVisible(true);
				...
				this.label_SukukFinanceMainDialog_ScheduleMethod.setVisible(false);
						...
						if(!isRIAExist){
							returnSetEntries = getEngineExecution().getAccEngineExecResults(dataSet, 
									getAmountCodes(), "N", getFeeDetailDialogCtrl() != null ? getFeeDetailDialogCtrl().getFeeRuleDetailsMap() : null,
											false, getFinanceDetail().getFinScheduleData().getFinanceType(),getFinanceDetail().getPremiumDetail());
						}else{
	...
	private void doStoreInitValues() {
		...
		//Finance Premium or Discount Details
		this.oldVar_issueNumber = this.issueNumber.getValue();
		this.oldVar_noOfUnits = this.noOfUnits.intValue();
		this.oldVar_faceValue = this.faceValue.getValue();
		this.oldVar_premiumType = this.premiumType.getSelectedIndex();
		this.oldVar_premiumValue = this.premiumValue.getValue();
		this.oldVar_pricePerUnit = this.pricePerUnit.getValue();
		this.oldVar_yieldValue = this.yieldValue.getValue();
		this.oldVar_lastCouponDate = this.lastCouponDate.getValue();
		this.oldVar_accruedProfit = this.accruedProfit.getValue();
		this.oldVar_purchaseDate = this.purchaseDate.getValue();
		this.oldVar_fairValuePerUnit = this.fairValuePerUnit.getValue();
		this.oldVar_fairValueAmount = this.fairValueAmount.getValue();
	...
	private void doResetInitValues() {
		...
		//Finance Premium or Discount Details
		this.issueNumber.setValue(this.oldVar_issueNumber);
		this.noOfUnits.setValue(this.oldVar_noOfUnits);
		this.faceValue.setValue(this.oldVar_faceValue);
		this.premiumType.setSelectedIndex(this.oldVar_premiumType);
		this.premiumValue.setValue(this.oldVar_premiumValue);
		this.pricePerUnit.setValue(this.oldVar_pricePerUnit);
		this.yieldValue.setValue(this.oldVar_yieldValue);
		this.lastCouponDate.setValue(this.oldVar_lastCouponDate);
		this.accruedProfit.setValue(this.oldVar_accruedProfit);
		this.purchaseDate.setValue(this.oldVar_purchaseDate);
		this.fairValuePerUnit.setValue(this.oldVar_fairValuePerUnit);
		this.fairValueAmount.setValue(this.oldVar_fairValueAmount);
	...
	public boolean isDataChanged(boolean close) {
		...
		//Finance Premium or Discount Details
		if(this.issueNumber.getValue() != this.oldVar_issueNumber){
			return true;
		}
		if(this.noOfUnits.intValue() != this.oldVar_noOfUnits){
			return true;
		}
		if(this.faceValue.getValue() != this.oldVar_faceValue){
			return true;
		}
		if(this.premiumType.getSelectedIndex() != this.oldVar_premiumType){
			return true;
		}
		if(this.premiumValue.getValue() != this.oldVar_premiumValue){
			return true;
		}
		if(this.pricePerUnit.getValue() != this.oldVar_pricePerUnit){
			return true;
		}
		if(this.yieldValue.getValue() != this.oldVar_yieldValue){
			return true;
		}
		if(this.lastCouponDate.getValue() != this.oldVar_lastCouponDate){
			return true;
		}
		if(this.accruedProfit.getValue() != this.oldVar_accruedProfit){
			return true;
		}
		if(this.purchaseDate.getValue() != this.oldVar_purchaseDate){
			return true;
		}
		if(this.fairValuePerUnit.getValue() != this.oldVar_fairValuePerUnit){
			return true;
		}
		if(this.fairValueAmount.getValue() != this.oldVar_fairValueAmount){
			return true;
		}
	...
	private void doSetValidation() {
		...
		if (!this.finReference.isReadonly() && 
			...
			this.finReference.setConstraint(new PTStringValidator(Labels.getLabel("label_SukukFinanceMainDialog_FinReference.value"),null,true));
		}

		if (!this.finAmount.isReadonly()) {
			this.finAmount.setConstraint(new AmountValidator(18,0,
					Labels.getLabel("label_SukukFinanceMainDialog_FinAmount.value"), false));
		}
		if(!this.defferments.isReadonly()){
			this.defferments.setConstraint(new PTNumberValidator(Labels.getLabel("label_SukukFinanceMainDialog_Defferments.value"), false, false));
		}

		if(!this.planDeferCount.isReadonly()){
			this.planDeferCount.setConstraint(new PTNumberValidator(Labels.getLabel("label_SukukFinanceMainDialog_PlanDeferCount.value"), false, false));
		}
		
		if(!this.stepPolicy.isReadonly() && this.stepFinance.isChecked() && !this.alwManualSteps.isChecked()){
			this.stepPolicy.setConstraint(new PTStringValidator( Labels.getLabel("label_SukukFinanceMainDialog_StepPolicy.value"), null, true ,true));
		}
        
		if(!this.noOfSteps.isReadonly() && this.stepFinance.isChecked() && this.alwManualSteps.isChecked()){
			this.noOfSteps.setConstraint(new PTNumberValidator(Labels.getLabel("label_SukukFinanceMainDialog_NumberOfSteps.value"), true, false));
		}
		...
		if (this.gb_gracePeriodDetails.isVisible()) {
			...
			if (!this.graceTerms.isReadonly()) {
				this.graceTerms.setConstraint(new PTStringValidator(Labels.getLabel("label_SukukFinanceMainDialog_GraceTerms.value"),null,true));
			}
			
			if (!this.grcMargin.isReadonly()) {
				this.grcMargin.setConstraint(new RateValidator(13, 9, 
						Labels.getLabel("label_SukukFinanceMainDialog_GraceMargin.value"), true));
			}

			if(this.allowGrace.isChecked()){
				this.grcEffectiveRate.setConstraint(new RateValidator(13, 9,
						Labels.getLabel("label_SukukFinanceMainDialog_GracePftRate.value"), true));
			}

			if (!this.nextGrcPftDate.isReadonly() && 
					FrequencyUtil.validateFrequency(this.gracePftFrq.getValue()) == null) {

				this.nextGrcPftDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_SukukFinanceMainDialog_NextGrcPftDate.value"),true));
			}

			if (!this.nextGrcPftRvwDate.isReadonly() && 
					FrequencyUtil.validateFrequency(this.gracePftRvwFrq.getValue()) == null) {

				this.nextGrcPftRvwDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_SukukFinanceMainDialog_NextGrcPftRvwDate.value"),true));
			}
		}
		...
		if (!this.nextRepayDate.isReadonly() && 
			...
			this.nextRepayDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_SukukFinanceMainDialog_NextRepayDate.value"),true));
		}

		if (!this.nextRepayPftDate.isReadonly() && 
				FrequencyUtil.validateFrequency(this.repayPftFrq.getValue()) == null) {

			this.nextRepayPftDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_SukukFinanceMainDialog_NextRepayPftDate.value"),true));
		}

		if (!this.nextRepayRvwDate.isReadonly() && 
				FrequencyUtil.validateFrequency(this.repayRvwFrq.getValue()) == null) {

			this.nextRepayRvwDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_SukukFinanceMainDialog_NextRepayRvwDate.value"),true));
		}

		if (!this.nextRepayCpzDate.isReadonly() && 
				FrequencyUtil.validateFrequency(this.repayCpzFrq.getValue()) == null) {

			this.nextRepayCpzDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_SukukFinanceMainDialog_NextRepayCpzDate.value"),true));
		}

		this.repayEffectiveRate.setConstraint(new RateValidator(13, 9,
				Labels.getLabel("label_SukukFinanceMainDialog_ProfitRate.value"), true));

		if (!this.repayMargin.isReadonly()) {
			this.repayMargin.setConstraint(new RateValidator(13, 9, 
					Labels.getLabel("label_SukukFinanceMainDialog_RepayMargin.value"), true));
		}
	...
	private void doSetLOVValidation() {
		...
		this.lovDescFinTypeName.setConstraint(new PTStringValidator(Labels.getLabel("label_SukukFinanceMainDialog_FinType.value"),null,true));

		this.finCcy.setConstraint(new PTStringValidator(Labels.getLabel("label_SukukFinanceMainDialog_FinCcy.value"),null,true,true));

		if (!this.finBranch.isReadonly()) {
			this.finBranch.setConstraint(new PTStringValidator(Labels.getLabel("label_SukukFinanceMainDialog_FinBranch.value"),null,true,true));
		}

		if (!this.custCIF.isReadonly()) {
			this.custCIF.setConstraint(new PTStringValidator(Labels.getLabel("label_SukukFinanceMainDialog_CustID.value"),null,true,true));
		}

		if (!recSave && StringUtils.trimToEmpty(this.disbAcctId.getSclass()).equals("mandatory")) {
			this.disbAcctId.setConstraint(new PTStringValidator(Labels.getLabel("label_SukukFinanceMainDialog_DisbAcctId.value"),null,true));
		}

		if (!recSave && this.repayAcctId.getSclass().equals("mandatory")) {
			this.repayAcctId.setConstraint(new PTStringValidator(Labels.getLabel("label_SukukFinanceMainDialog_RepayAcctId.value"),null,true));
		}

		if (!recSave && this.downPayAccount.getSclass().equals("mandatory")) {
			this.downPayAccount.setConstraint(new PTStringValidator(Labels.getLabel("label_SukukFinanceMainDialog_DownPayAccount.value"),null,true));
		}

		if (!this.btnSearchCommitmentRef.isDisabled() && 
				space_commitmentRef.getSclass().equals("mandatory")) {
			this.lovDescCommitmentRefName.setConstraint(new PTStringValidator(Labels.getLabel("label_SukukFinanceMainDialog_CommitRef.value"),null,true));
		}

		if (!this.finPurpose.isReadonly()) {
			this.finPurpose.setConstraint(new PTStringValidator(Labels.getLabel("label_SukukFinanceMainDialog_FinPurpose.value"),null,true,true));
		}

		//FinanceMain Details Tab ---> 2. Grace Period Details

		if(!this.graceBaseRate.isReadonly()) {
			this.graceBaseRate.setConstraint(new PTStringValidator(Labels.getLabel("label_SukukFinanceMainDialog_GraceBaseRate.value"),null,true,true));
		}

		if(this.allowGrcInd.isChecked() && !this.btnSearchGrcIndBaseRate.isDisabled()){
			this.lovDescGrcIndBaseRateName.setConstraint(new PTStringValidator(Labels.getLabel("label_SukukFinanceMainDialog_FinGrcIndBaseRate.value"),null,true));			
		}

		//FinanceMain Details Tab ---> 3. Repayment Period Details

		if(!this.repayBaseRate.isReadonly()) {
			this.repayBaseRate.setConstraint(new PTStringValidator(Labels.getLabel("label_SukukFinanceMainDialog_RepayBaseRate.value"),null,true,true));
		}

		if(this.allowRpyInd.isChecked() && !this.btnSearchRpyIndBaseRate.isDisabled()){
			this.lovDescRpyIndBaseRateName.setConstraint(new PTStringValidator(Labels.getLabel("label_SukukFinanceMainDialog_FinRpyIndBaseRate.value"),null,true));			
		}
		logger.debug("Leaving ");
	}
	...
	public void doClear() {
		...
		//Finance Premium or Discount Details
		this.issueNumber.setValue("");
		this.noOfUnits.setText("");
		this.faceValue.setValue("");
		this.premiumType.setValue("");
		this.premiumValue.setValue("");
		this.pricePerUnit.setValue("");
		this.yieldValue.setValue("");
		this.accruedProfit.setValue("");
		this.lastCouponDate.setValue(null);
		this.purchaseDate.setValue(null);
		this.fairValuePerUnit.setValue("");
		this.fairValueAmount.setValue("");
	...
	public void doSave() throws Exception {
		...
		//Checking Data Modification For Fair Value on Reevaluation
		if(!moduleDefiner.equals("") && !this.fairValuePerUnit.isDisabled()){
			if(this.oldVar_fairValuePerUnit == this.fairValuePerUnit.getValue()){
				PTMessageUtils.showErrorMessage(Labels.getLabel("label_SukukFinanceMainDialog_DataModified.value"));
				return;
			}
		}
		...
		if (isWorkFlowEnabled()) {
			...
			while (!"".equals(serviceTasks)) {
				...
				if (StringUtils.trimToEmpty(method).contains(PennantConstants.method_doDedup)) {
					...
					tFinanceDetail = FetchDedupDetails.getLoanDedup(getRole(),aFinanceDetail, this.window_SukukFinanceMainDialog);
					...
					if(!StringUtils.trimToEmpty(tFinanceMain.getLovDescCustCoreBank()).equals("")){
						...
						tFinanceDetail = FetchBlackListDetails.getBlackListCustomers(tFinanceDetail ,
								this.window_SukukFinanceMainDialog);
					}
			...
			while (retValue == PennantConstants.porcessOVERIDE) {
				...
				if (StringUtils.trimToEmpty(method).equalsIgnoreCase("")) {
					...
					if (StringUtils.trimToEmpty(method).equalsIgnoreCase(PennantConstants.method_doApprove)) {
						...
						retValue = ErrorControl.showErrorControl(this.window_SukukFinanceMainDialog, auditHeader);
						return processCompleted;
					}
				}

				auditHeader = ErrorControl.showErrorDetails(this.window_SukukFinanceMainDialog, auditHeader);
	...
	public void onClick$btnSearchFinType(Event event) {
		...
		Object dataObject = ExtendedSearchListBox.show(this.window_SukukFinanceMainDialog, "FinanceType");
	...
	public void onClick$btnSearchDisbAcctId(Event event) throws InterruptedException {
		...
		if(!StringUtils.trimToEmpty(this.lovDescCustCIF.getValue()).equals("")) {
			...
			try {
				...
				dataObject = ExtendedSearchListBox.show(this.window_SukukFinanceMainDialog, "Accounts", iAccountList);
				if (dataObject instanceof String) {
...
*/
	...
	//filters[1] = new Filter("cmtCcy", this.finCcy.getValue(), Filter.OP_EQUAL);
		...
		if(this.finStartDate.getValue() != null){
			...
			throw new WrongValueException(finStartDate,Labels.getLabel("FIELD_NO_EMPTY",
					new String[] { Labels.getLabel("label_SukukFinanceMainDialog_FinStartDate.value")}));
		}

		Object dataObject = ExtendedSearchListBox.show(this.window_SukukFinanceMainDialog, "Commitment", filters);
	...
	private void onCheckCustAcceptance(){
		...
		this.label_SukukFinanceMainDialog_CbbApproved.setVisible(false);
	...
	public void onClick$btnSearchGrcIndBaseRate(Event event) {
		...
		Object dataObject = ExtendedSearchListBox.show(this.window_SukukFinanceMainDialog, "BaseRateCode");
		if (dataObject instanceof String) {
	...
	public void onClick$btnBuildSchedule(Event event) throws Exception {
		...
		if (validate() != null) {
			...
			int formatter = getFinanceDetail().getFinScheduleData().getFinanceMain().getLovDescFinFormatter();
			this.accruedProfit.setValue(PennantApplicationUtil.formateAmount(BigDecimal.ZERO, formatter));
			Date purchaseDate = this.purchaseDate.getValue();
			Date lastcouponDate = this.lastCouponDate.getValue();
			if(lastcouponDate != null && lastcouponDate.before(purchaseDate)){
				
				BigDecimal finAmt = getFinanceDetail().getFinScheduleData().getFinanceMain().getFinAmount();
				BigDecimal feeAmt = getFinanceDetail().getFinScheduleData().getFinanceMain().getFeeChargeAmt();
				String pftDayBasis = getFinanceDetail().getFinScheduleData().getFinanceMain().getProfitDaysBasis(); 
				BigDecimal actRate = getFinanceDetail().getFinScheduleData().getFinanceMain().getRepayProfitRate();
				
				BigDecimal calint = CalculationUtil.calInterest(lastcouponDate, purchaseDate, finAmt.add(feeAmt), pftDayBasis, actRate);
				this.accruedProfit.setValue(PennantApplicationUtil.formateAmount(calint.setScale(0, RoundingMode.HALF_DOWN), formatter));
			}
	...
	private void setCustomerData(){
		...
		setAgreementDetailTab(this.window_SukukFinanceMainDialog);
		...
		if(getWorkFlow().getTaskTabs(getWorkFlow().getTaskId(getRole())).equals("Accounting")){
			...
			if(getAccountingDetailDialogCtrl() != null){
				...
				setAccountingDetailTab(this.window_SukukFinanceMainDialog);
	...
	public void onClick$viewCustInfo(Event event){
		...
		try {
			...
			if (finDivision.equals(PennantConstants.FIN_DIVISION_RETAIL)) {
				Executions.createComponents("/WEB-INF/pages/CustomerMasters/Customer/FinCustomerDetailsEnq.zul", window_SukukFinanceMainDialog, map);
			}else{
				Executions.createComponents("/WEB-INF/pages/CustomerMasters/Enquiry/CustomerSummary.zul", window_SukukFinanceMainDialog, map);
			}
		} catch (final Exception e) {
			logger.error("onOpenWindow:: error opening window / " + e.getMessage());
		}
	...
	private void fairValueCalculation(){
		...
		if(startDate != null && purchaseDate != null && moduleDefiner.equals("")){
