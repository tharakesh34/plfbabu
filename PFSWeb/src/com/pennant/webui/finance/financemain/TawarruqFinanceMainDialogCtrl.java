 ...
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
 */
public class TawarruqFinanceMainDialogCtrl extends FinanceBaseCtrl implements Serializable {

	private static final long serialVersionUID = 6004939933729664895L;
	private final static Logger logger = Logger.getLogger(TawarruqFinanceMainDialogCtrl.class);
	...
	protected Label 		label_TawarruqFinanceMainDialog_ScheduleMethod;	// autoWired
	protected Label 		label_TawarruqFinanceMainDialog_CommitRef;	// autoWired
	protected Label 		label_TawarruqFinanceMainDialog_DepriFrq;	// autoWired
	protected Label 		label_TawarruqFinanceMainDialog_PlanDeferCount;		// autoWired
	...
	protected Label 		label_TawarruqFinanceMainDialog_GraceMargin; 		// autoWired
	...
	public void onCreate$window_TawarruqFinanceMainDialog(Event event) throws Exception {
		...
		setLabel_FinanceMainDialog_CbbApproved(label_TawarruqFinanceMainDialog_CbbApproved);
		setLabel_FinanceMainDialog_CommitRef(label_TawarruqFinanceMainDialog_CommitRef);
		setLabel_FinanceMainDialog_DepriFrq(label_TawarruqFinanceMainDialog_DepriFrq);
		setLabel_FinanceMainDialog_FinRepayPftOnFrq(label_TawarruqFinanceMainDialog_FinRepayPftOnFrq);
		setLabel_FinanceMainDialog_PlanDeferCount(label_TawarruqFinanceMainDialog_PlanDeferCount);
		setLabel_FinanceMainDialog_AlwGrace(label_TawarruqFinanceMainDialog_AlwGrace);
		setLabel_FinanceMainDialog_GraceMargin(label_TawarruqFinanceMainDialog_GraceMargin);
	...
	public void doReject() throws InterruptedException{
		...
		try{
			Executions.createComponents("/WEB-INF/pages/Finance/FinanceMain/FinanceReject.zul",
					window_TawarruqFinanceMainDialog, map);
	...
	public void onClick$btnHelp(Event event) throws InterruptedException {
		logger.debug("Entering " + event.toString());
		PTMessageUtils.showHelpWindow(event, window_TawarruqFinanceMainDialog);
		logger.debug("Leaving " + event.toString());
	}
	...
	private void doClose() throws Exception {
		...
		if (close) {
			closeWindow(this.window_TawarruqFinanceMainDialog);
		}
		...
		try {
			...
			if(getFinanceDetail().getFinScheduleData().getFinanceType().getFinMinTerm() == 1 && 
				...
				if(!getFinanceDetail().getFinScheduleData().getFinanceType().isFinRepayPftOnFrq()){
					this.label_TawarruqFinanceMainDialog_FinRepayPftOnFrq.setVisible(false);
					this.rpyPftFrqRow.setVisible(false);
					this.hbox_finRepayPftOnFrq.setVisible(false);
				}else{
					this.label_TawarruqFinanceMainDialog_FinRepayPftOnFrq.setVisible(true);
				...
				this.label_TawarruqFinanceMainDialog_ScheduleMethod.setVisible(false);
			...
			setDialog(this.window_TawarruqFinanceMainDialog);
	...
	private void doSetValidation() {
		...
		if (!this.finReference.isReadonly() && 
			...
			this.finReference.setConstraint(new PTStringValidator( Labels.getLabel("label_TawarruqFinanceMainDialog_FinReference.value"),null,true));
		}

		if (!this.finAmount.isDisabled()) {
			this.finAmount.setConstraint(new AmountValidator(18,0,
					Labels.getLabel("label_TawarruqFinanceMainDialog_FinAmount.value"), false));
		}
		
		if(!this.defferments.isReadonly()){
			this.defferments.setConstraint(new PTNumberValidator(Labels.getLabel("label_TawarruqFinanceMainDialog_Defferments.value"), false, false));
		}
		
		if(!this.planDeferCount.isReadonly()){
			this.planDeferCount.setConstraint(new PTNumberValidator(Labels.getLabel("label_TawarruqFinanceMainDialog_PlanDeferCount.value"), false, false));
		}
		...
		if(!this.stepPolicy.isReadonly() && this.stepFinance.isChecked() && !this.alwManualSteps.isChecked()){
			this.stepPolicy.setConstraint(new PTStringValidator( Labels.getLabel("label_TawarruqFinanceMainDialog_StepPolicy.value"), null, true,true));
		}
		...
		if (this.gb_gracePeriodDetails.isVisible()) {
			...
			if (!this.graceTerms.isReadonly()) {
				this.graceTerms.setConstraint(new PTStringValidator(Labels.getLabel("label_TawarruqFinanceMainDialog_GraceTerms.value"),null,true));
			}
			
			if (!this.grcMargin.isDisabled()) {
				this.grcMargin.setConstraint(new RateValidator(13, 9, 
						Labels.getLabel("label_TawarruqFinanceMainDialog_GraceMargin.value"), true));
			}

			if(this.allowGrace.isChecked()){
				this.grcEffectiveRate.setConstraint(new RateValidator(13, 9,
						Labels.getLabel("label_TawarruqFinanceMainDialog_GracePftRate.value"), true));
			}

			if (!this.nextGrcPftDate.isDisabled() && 
					FrequencyUtil.validateFrequency(this.gracePftFrq.getValue()) == null) {

				this.nextGrcPftDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_TawarruqFinanceMainDialog_NextGrcPftDate.value"),true));
			}

			if (!this.nextGrcPftRvwDate.isDisabled() && 
					FrequencyUtil.validateFrequency(this.gracePftRvwFrq.getValue()) == null) {

				this.nextGrcPftRvwDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_TawarruqFinanceMainDialog_NextGrcPftRvwDate.value"),true));
			}
		}
		...
		if (!this.nextRepayDate.isDisabled() && 
			...
			this.nextRepayDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_TawarruqFinanceMainDialog_NextRepayDate.value"),true));
		}

		if (!this.nextRepayPftDate.isDisabled() && 
				FrequencyUtil.validateFrequency(this.repayPftFrq.getValue()) == null) {

			this.nextRepayPftDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_TawarruqFinanceMainDialog_NextRepayPftDate.value"),true));
		}

		if (!this.nextRepayRvwDate.isDisabled() && 
				FrequencyUtil.validateFrequency(this.repayRvwFrq.getValue()) == null) {

			this.nextRepayRvwDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_TawarruqFinanceMainDialog_NextRepayRvwDate.value"),true));
		}

		if (!this.nextRepayCpzDate.isDisabled() && 
				FrequencyUtil.validateFrequency(this.repayCpzFrq.getValue()) == null) {

			this.nextRepayCpzDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_TawarruqFinanceMainDialog_NextRepayCpzDate.value"),true));
		}

		this.repayEffectiveRate.setConstraint(new RateValidator(13, 9,
				Labels.getLabel("label_TawarruqFinanceMainDialog_ProfitRate.value"), true));

		if (!this.repayMargin.isDisabled()) {
			this.repayMargin.setConstraint(new RateValidator(13, 9, 
					Labels.getLabel("label_TawarruqFinanceMainDialog_RepayMargin.value"), true));
		}
	...
	private void doSetLOVValidation() {
		...
		this.lovDescFinTypeName.setConstraint(new PTStringValidator(Labels.getLabel("label_TawarruqFinanceMainDialog_FinType.value"),null,true));

		this.finCcy.setConstraint(new PTStringValidator(Labels.getLabel("label_TawarruqFinanceMainDialog_FinCcy.value"),null,true,true));

		if (!this.finBranch.isReadonly()) {
			this.finBranch.setConstraint(new PTStringValidator(Labels.getLabel("label_TawarruqFinanceMainDialog_FinPurpose.value"),null,true,true));
		}

		if (!this.custCIF.isReadonly()) {
			this.custCIF.setConstraint(new PTStringValidator(Labels.getLabel("label_TawarruqFinanceMainDialog_CustID.value"),null,true,true));
		}
		...
		if (!this.btnSearchCommitmentRef.isDisabled() && 
			...
			this.lovDescCommitmentRefName.setConstraint(new PTStringValidator(Labels.getLabel("label_TawarruqFinanceMainDialog_CommitRef.value"),null,true));
		}

		if (!this.finPurpose.isReadonly()) {
			this.finPurpose.setConstraint(new PTStringValidator(Labels.getLabel("label_TawarruqFinanceMainDialog_FinPurpose.value"),null,true,true));
		}

		//FinanceMain Details Tab ---> 2. Grace Period Details

		if(!this.graceBaseRate.isReadonly()) {
			this.graceBaseRate.setConstraint(new PTStringValidator(Labels.getLabel("label_TawarruqFinanceMainDialog_GraceBaseRate.value"),null,true,true));
		}

		if(this.allowGrcInd.isChecked() && !this.btnSearchGrcIndBaseRate.isDisabled()){
			this.lovDescGrcIndBaseRateName.setConstraint(new PTStringValidator(Labels.getLabel("label_TawarruqFinanceMainDialog_FinGrcIndBaseRate.value"),null,true));			
		}

		//FinanceMain Details Tab ---> 3. Repayment Period Details

		if(!this.repayBaseRate.isReadonly()) {
			this.repayBaseRate.setConstraint(new PTStringValidator(Labels.getLabel("label_TawarruqFinanceMainDialog_RepayBaseRate.value"),null,true,true));
		}

		if(this.allowRpyInd.isChecked() && !this.btnSearchRpyIndBaseRate.isDisabled()){
			this.lovDescRpyIndBaseRateName.setConstraint(new PTStringValidator(Labels.getLabel("label_TawarruqFinanceMainDialog_FinRpyIndBaseRate.value"),null,true));			
		}
		logger.debug("Leaving ");
	}
	...
	private void doDelete() throws InterruptedException {
		...
		if (conf == MultiLineMessageBox.YES) {
			...
			try {
				...
				showErrorMessage(this.window_TawarruqFinanceMainDialog, e);
			}
		}
		logger.debug("Leaving");
	}
		...
		if (isWorkFlowEnabled()) {
			...
			while (!"".equals(serviceTasks)) {
				...
				if (StringUtils.trimToEmpty(method).contains(PennantConstants.method_doDedup)) {
					...
					tFinanceDetail = FetchDedupDetails.getLoanDedup(getRole(),aFinanceDetail, this.window_TawarruqFinanceMainDialog);
					...
					if(!StringUtils.trimToEmpty(tFinanceMain.getLovDescCustCoreBank()).equals("")){
						...
						tFinanceDetail = FetchBlackListDetails.getBlackListCustomers(tFinanceDetail ,
								this.window_TawarruqFinanceMainDialog);
					}
			...
			while (retValue == PennantConstants.porcessOVERIDE) {
				...
				if (StringUtils.trimToEmpty(method).equalsIgnoreCase("")) {
					...
					if (StringUtils.trimToEmpty(method).equalsIgnoreCase(PennantConstants.method_doApprove)) {
						...
						retValue = ErrorControl.showErrorControl(this.window_TawarruqFinanceMainDialog, auditHeader);
						return processCompleted;
					}
				}

				auditHeader = ErrorControl.showErrorDetails(this.window_TawarruqFinanceMainDialog, auditHeader);
	...
	public void onClick$btnSearchFinType(Event event) {
		...
		Object dataObject = ExtendedSearchListBox.show(this.window_TawarruqFinanceMainDialog, "FinanceType");
	...
	public void onFulfill$finCcy(Event event) {
		...
		this.finCcy.setConstraint("");
		Object dataObject = ExtendedSearchListBox.show(this.window_TawarruqFinanceMainDialog, "Currency");
		if (dataObject instanceof String) {
	...
	//	filters[1] = new Filter("cmtCcy", this.finCcy.getValue(), Filter.OP_EQUAL);
		...
		Object dataObject = ExtendedSearchListBox.show(this.window_TawarruqFinanceMainDialog, "Commitment", filters);
	...
	private void onCheckCustAcceptance(){
		...
		this.label_TawarruqFinanceMainDialog_CbbApproved.setVisible(false);
	...
	public void onClick$btnSearchGrcIndBaseRate(Event event) {
		...
		Object dataObject = ExtendedSearchListBox.show(this.window_TawarruqFinanceMainDialog, "BaseRateCode");
		if (dataObject instanceof String) {
	...
	private void setCustomerData(){
		...
		setAgreementDetailTab(this.window_TawarruqFinanceMainDialog);
		...
		if(getWorkFlow().getTaskTabs(getWorkFlow().getTaskId(getRole())).equals("Accounting")){
			...
			if(getAccountingDetailDialogCtrl() != null){
				...
				setAccountingDetailTab(this.window_TawarruqFinanceMainDialog);
			}
		}
	...
	public void onClick$viewCustInfo(Event event){
		try {
			...
			if (finDivision.equals(PennantConstants.FIN_DIVISION_RETAIL)) {
				Executions.createComponents("/WEB-INF/pages/CustomerMasters/Customer/FinCustomerDetailsEnq.zul", window_TawarruqFinanceMainDialog, map);
			}else{
				Executions.createComponents("/WEB-INF/pages/CustomerMasters/Enquiry/CustomerSummary.zul", window_TawarruqFinanceMainDialog, map);
			}
		} catch (final Exception e) {
			logger.error("onOpenWindow:: error opening window / " + e.getMessage());
		}
