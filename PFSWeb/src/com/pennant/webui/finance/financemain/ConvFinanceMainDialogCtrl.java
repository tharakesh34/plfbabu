 ...
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
 */
public class ConvFinanceMainDialogCtrl extends FinanceBaseCtrl implements Serializable {
	...
	protected Label 		label_ConvFinanceMainDialog_ScheduleMethod;		// autoWired
	...
	//protected Label 		label_ConvFinanceMainDialog_CcyConversionRate; 	// autoWired
	...
	public void onCreate$window_ConvFinanceMainDialog(Event event) throws Exception {
		...
		setLabel_FinanceMainDialog_CbbApproved(label_ConvFinanceMainDialog_CbbApproved);
		...
		setLabel_FinanceMainDialog_StepPolicy(label_ConvFinanceMainDialog_StepPolicy);
		setLabel_FinanceMainDialog_numberOfSteps(label_ConvFinanceMainDialog_numberOfSteps);
		setProductCode("Conv");
	...
	public void onClick$btnHelp(Event event) throws InterruptedException {
		logger.debug("Entering " + event.toString());
		PTMessageUtils.showHelpWindow(event, window_ConvFinanceMainDialog);
		logger.debug("Leaving " + event.toString());
	}
	...
	public void doWriteBeanToComponents(FinanceDetail aFinanceDetail, boolean onLoadProcess) throws ParseException, InterruptedException, AccountNotFoundException, IllegalAccessException, InvocationTargetException {
		...
		if (aFinanceDetail.getFinScheduleData().getFinanceType().isFinIsDwPayRequired() && 
			...
			if(this.downPaySupl.isDisabled() && aFinanceMain.getDownPaySupl().compareTo(BigDecimal.ZERO) == 0){
		...
		try {
			...
			if(financeType.getFinMinTerm() == 1 &&  financeType.getFinMaxTerm() == 1){
				if(!financeType.isFinRepayPftOnFrq()){
					this.label_ConvFinanceMainDialog_FinRepayPftOnFrq.setVisible(false);
					this.rpyPftFrqRow.setVisible(false);
					this.hbox_finRepayPftOnFrq.setVisible(false);
				}else{
					this.label_ConvFinanceMainDialog_FinRepayPftOnFrq.setVisible(true);
				...
				this.label_ConvFinanceMainDialog_ScheduleMethod.setVisible(false);
	...
	private void doStoreInitValues() {
		...
		this.oldVar_ccyConversionRate = this.ccyConversionRate.getValue();
		this.oldVar_disbCcyFinAmount = this.disbCcyFinAmount.getValue();*/
	...
	private void doResetInitValues() {
		...
		this.ccyConversionRate.setValue(this.oldVar_ccyConversionRate);
		this.disbCcyFinAmount.setValue(this.oldVar_disbCcyFinAmount);*/
	...
	private void doSetValidation() {
		...
		if (!this.finAmount.isDisabled()) {
			this.finAmount.setConstraint(new AmountValidator(18,0,
					Labels.getLabel("label_ConvFinanceMainDialog_FinAmount.value"), false));
		}
		...
		if(!this.stepPolicy.isReadonly() && this.stepFinance.isChecked() && !this.alwManualSteps.isChecked()){
			this.stepPolicy.setConstraint(new PTStringValidator( Labels.getLabel("label_ConvFinanceMainDialog_StepPolicy.value"), null, true,true));
		}
        
		if(!this.noOfSteps.isReadonly() && this.stepFinance.isChecked() && this.alwManualSteps.isChecked()){
			this.noOfSteps.setConstraint(new PTNumberValidator(Labels.getLabel("label_ConvFinanceMainDialog_NumberOfSteps.value"), true, false));
		}
		...
		/*if(this.diffDisbCcy.isChecked()){
			if (!this.ccyConversionRate.isReadonly()) {
				this.ccyConversionRate.setConstraint(new AmountValidator(18,0,
						Labels.getLabel("label_ConvFinanceMainDialog_CcyConversionRate.value"), false));
			}
			if (!this.disbCcyFinAmount.isReadonly()) {
				this.disbCcyFinAmount.setConstraint(new AmountValidator(18,0,
						Labels.getLabel("label_ConvFinanceMainDialog_DisbCcyFinAmount.value"), false));
			}
		}*/
		...
		if (this.gb_gracePeriodDetails.isVisible()) {
			...
			if (!this.grcMargin.isReadonly()) {
				this.grcMargin.setConstraint(new RateValidator(13, 9, 
						Labels.getLabel("label_ConvFinanceMainDialog_GraceMargin.value"), true));
			}

			if(this.allowGrace.isChecked()){
				this.grcEffectiveRate.setConstraint(new RateValidator(13, 9,
						Labels.getLabel("label_ConvFinanceMainDialog_GracePftRate.value"), true));
			}
		...
		this.repayEffectiveRate.setConstraint(new RateValidator(13, 9,
				Labels.getLabel("label_ConvFinanceMainDialog_ProfitRate.value"), true));

		if (!this.repayMargin.isReadonly()) {
			this.repayMargin.setConstraint(new RateValidator(13, 9, 
					Labels.getLabel("label_ConvFinanceMainDialog_RepayMargin.value"), true));
		}
		...
		this.finStartDate.setConstraint(new PTDateValidator(Labels.getLabel("label_ConvFinanceMainDialog_FinStartDate.value"), true,startDate,endDate,false));
		}
		logger.debug("Leaving");
	}
	...
	private void doSetLOVValidation() {
		...
		if (!this.finBranch.isReadonly()) {
			this.finBranch.setConstraint(new PTStringValidator(Labels.getLabel("label_ConvFinanceMainDialog_FinBranch.value"),null,true,true));
		}

		if (!this.custCIF.isReadonly()) {
			this.custCIF.setConstraint(new PTStringValidator(Labels.getLabel("label_ConvFinanceMainDialog_CustID.value"),null,true,true));
		}
		...
		if (!this.finPurpose.isReadonly()) {
			this.finPurpose.setConstraint(new PTStringValidator(Labels.getLabel("label_ConvFinanceMainDialog_FinPurpose.value"),null,true,true));
		}
		...
		if(!this.graceBaseRate.isReadonly()) {
			this.graceBaseRate.setConstraint(new PTStringValidator(Labels.getLabel("label_ConvFinanceMainDialog_GraceBaseRate.value"),null,true,true));
		}
		...
		if(!this.repayBaseRate.isReadonly()) {
			this.repayBaseRate.setConstraint(new PTStringValidator(Labels.getLabel("label_ConvFinanceMainDialog_RepayBaseRate.value"),null,true,true));
		}
	...
	private void doDelete() throws InterruptedException {
		...
		if (conf == MultiLineMessageBox.YES) {
			...
			try {
				...
				showErrorMessage(this.window_ConvFinanceMainDialog, e);
			}
		}
		logger.debug("Leaving");
	}
		...
		if (isWorkFlowEnabled()) {
			...
			while (!"".equals(serviceTasks)) {
				...
				if (StringUtils.trimToEmpty(method).contains(PennantConstants.method_doDedup)) {
					...
					tFinanceDetail = FetchDedupDetails.getLoanDedup(getRole(),aFinanceDetail, this.window_ConvFinanceMainDialog);
					...
					boolean isDisError  = getDiscrepancyDetails(discrepancies,aFinanceDetail, false);
			...
			while (retValue == PennantConstants.porcessOVERIDE) {
				...
				if (StringUtils.trimToEmpty(method).equalsIgnoreCase("")) {
					...
					if (StringUtils.trimToEmpty(method).equalsIgnoreCase(PennantConstants.method_doApprove)) {
						...
						retValue = ErrorControl.showErrorControl(this.window_ConvFinanceMainDialog, auditHeader);
						return processCompleted;
					}
				}

				auditHeader = ErrorControl.showErrorDetails(this.window_ConvFinanceMainDialog, auditHeader);
	...
	public void onClick$btnSearchFinType(Event event) {
		...
		Object dataObject = ExtendedSearchListBox.show(this.window_ConvFinanceMainDialog, "FinanceType");
	...
	//	filters[1] = new Filter("cmtCcy", this.finCcy.getValue(), Filter.OP_EQUAL);
		if(this.finStartDate.getValue() != null){
			...
			throw new WrongValueException(finStartDate,Labels.getLabel("FIELD_NO_EMPTY",
					new String[] { Labels.getLabel("label_ConvFinanceMainDialog_FinStartDate.value")}));
		}

		Object dataObject = ExtendedSearchListBox.show(this.window_ConvFinanceMainDialog, "Commitment", filters);
	...
	private void onCheckCustAcceptance(){
		...
		this.label_ConvFinanceMainDialog_CbbApproved.setVisible(false);
	...
	public void onClick$btnSearchGrcIndBaseRate(Event event) {
		...
		Object dataObject = ExtendedSearchListBox.show(this.window_ConvFinanceMainDialog, "BaseRateCode");
		if (dataObject instanceof String) {
	...
	/*public void onCheck$diffDisbCcy(Event event) {
		logger.debug("Entering" + event.toString());
		onCheckDiffDisbCcy(true);		
		logger.debug("Leaving" + event.toString());
	}
	
	public void onFulfill$disbCcy(Event event) {
		logger.debug("Entering" + event.toString());
		int formatter = 0;
		this.disbCcyFinAmount.setFormat(PennantAppUtil.getAmountFormate(formatter));
		this.ccyConversionRate.setValue(BigDecimal.ONE);
		getFinanceDetail().getFinScheduleData().getFinanceMain().setLovDescDisbCcyFormatter(formatter);
		
		Object dataObject = this.disbCcy.getObject();
		if (dataObject instanceof String) {
		} else {
			Currency details = (Currency) dataObject;
			if (details != null) {
				...
				this.disbCcyFinAmount.setFormat(PennantAppUtil.getAmountFormate(formatter));
				this.ccyConversionRate.setValue(details.getCcySpotRate());
				getFinanceDetail().getFinScheduleData().getFinanceMain().setLovDescDisbCcyFormatter(formatter);
				onChangeDisbCcyAmt();
			}
		}
	...
	public void onChange$ccyConversionRate(Event event) {
		logger.debug("Entering" + event.toString());
		onChangeDisbCcyAmt();
		logger.debug("Leaving" + event.toString());
	}

	public void onFulfill$disbCcyFinAmount(Event event) {
		logger.debug("Entering" + event.toString());
		onChangeDisbCcyAmt();
		logger.debug("Leaving" + event.toString());
	}*/
	...
	private void setCustomerData(){
		...
		setAgreementDetailTab(this.window_ConvFinanceMainDialog);
		...
		if(getWorkFlow().getTaskTabs(getWorkFlow().getTaskId(getRole())).equals("Accounting")){
			...
			if(getAccountingDetailDialogCtrl() != null){
				...
				setAccountingDetailTab(this.window_ConvFinanceMainDialog);
	...
	public void onClick$viewCustInfo(Event event){
		try {
			...
			if (finDivision.equals(PennantConstants.FIN_DIVISION_RETAIL)) {
				Executions.createComponents("/WEB-INF/pages/CustomerMasters/Customer/FinCustomerDetailsEnq.zul", window_ConvFinanceMainDialog, map);
			}else{
				Executions.createComponents("/WEB-INF/pages/CustomerMasters/Enquiry/CustomerSummary.zul", window_ConvFinanceMainDialog, map);
			}
		} catch (final Exception e) {
			logger.error("onOpenWindow:: error opening window / " + e.getMessage());
		}
	...
	public byte[] getAgreement (FinanceDetail detail, AgreementDefinition agreementDef) throws InterruptedException{
		if(getAgreementDetailDialogCtrl() != null){
			return getAgreementDetailDialogCtrl().doGenerateAgreement(detail, agreementDef);
		}
		return null;
 	}
