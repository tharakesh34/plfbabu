 ...
 ********************************************************************************************
 *																							*
 * FileName    		:  IstnormFinanceMainDialogCtrl.java                                                   * 	  
 *                                                                    						*
 ...
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
 */
public class IstnormFinanceMainDialogCtrl extends FinanceBaseCtrl implements Serializable {

	private static final long serialVersionUID = 6004939933729664895L;
	private final static Logger logger = Logger.getLogger(IstnormFinanceMainDialogCtrl.class);
	 ...
	 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	 */
	protected Window 		window_IstnormFinanceMainDialog; 				// autoWired

	//Finance Main Details Tab---> 1. Key Details
	protected CurrencyBox 	downPaySupl; 							// autoWired
	protected Row			row_downPaySupl;						// autoWired

	protected Label 		label_IstnormFinanceMainDialog_ScheduleMethod;
	protected Label 		label_IstnormFinanceMainDialog_FinRepayPftOnFrq;
	protected Label 		label_IstnormFinanceMainDialog_CommitRef; 	// autoWired
	protected Label 		label_IstnormFinanceMainDialog_DepriFrq; 	// autoWired
	protected Label 		label_IstnormFinanceMainDialog_PlanDeferCount;	// autoWired
	 ...
	 */
	public IstnormFinanceMainDialogCtrl() {
		super();
	}
	 ...
	 * @throws Exception
	 */
	public void onCreate$window_IstnormFinanceMainDialog(Event event) throws Exception {
		...
		setMainWindow(window_IstnormFinanceMainDialog);
		setLabel_FinanceMainDialog_CbbApproved(label_IstnormFinanceMainDialog_CbbApproved);
		setLabel_FinanceMainDialog_CommitRef(label_IstnormFinanceMainDialog_CommitRef);
		setLabel_FinanceMainDialog_DepriFrq(label_IstnormFinanceMainDialog_DepriFrq);
		setLabel_FinanceMainDialog_FinRepayPftOnFrq(label_IstnormFinanceMainDialog_FinRepayPftOnFrq);
		setLabel_FinanceMainDialog_PlanDeferCount(label_IstnormFinanceMainDialog_PlanDeferCount);
	 ...
	 * @throws Exception
	 */
	public void onClose$window_IstnormFinanceMainDialog(Event event) throws Exception {
		logger.debug("Entering " + event.toString());
		doClose();
		logger.debug("Leaving " + event.toString());
	}
	...
	public void doReject() throws InterruptedException{
		...
		try{
			Executions.createComponents("/WEB-INF/pages/Finance/FinanceMain/FinanceReject.zul",
					window_IstnormFinanceMainDialog, map);
	...
	public void onClick$btnHelp(Event event) throws InterruptedException {
		logger.debug("Entering " + event.toString());
		PTMessageUtils.showHelpWindow(event, window_IstnormFinanceMainDialog);
		logger.debug("Leaving " + event.toString());
	}
		...
		try {
			...
			if(financeType.getFinMinTerm() == 1 &&  financeType.getFinMaxTerm() == 1){
				if(!financeType.isFinRepayPftOnFrq()){
					this.label_IstnormFinanceMainDialog_FinRepayPftOnFrq.setVisible(false);
					this.rpyPftFrqRow.setVisible(false);
					this.hbox_finRepayPftOnFrq.setVisible(false);
				}else{
					this.label_IstnormFinanceMainDialog_FinRepayPftOnFrq.setVisible(true);
				...
				this.label_IstnormFinanceMainDialog_ScheduleMethod.setVisible(false);
	...
	private void doSetValidation() {
		...
		if (!this.finReference.isReadonly() && 
			...
			this.finReference.setConstraint(new PTStringValidator(Labels.getLabel("label_IstnormFinanceMainDialog_FinReference.value"),null,true));
		}

		if (!this.finAmount.isReadonly()) {
			this.finAmount.setConstraint(new AmountValidator(18,0,
					Labels.getLabel("label_IstnormFinanceMainDialog_FinAmount.value"), false));
		}
		if(!this.defferments.isReadonly()){
			this.defferments.setConstraint(new PTNumberValidator(Labels.getLabel("label_IstnormFinanceMainDialog_Defferments.value"), false, false));
		}

		if(!this.planDeferCount.isReadonly()){
			this.planDeferCount.setConstraint(new PTNumberValidator(Labels.getLabel("label_IstnormFinanceMainDialog_PlanDeferCount.value"), false, false));
		}
		...
		if(!this.noOfSteps.isReadonly() && this.stepFinance.isChecked() && this.alwManualSteps.isChecked()){
			this.noOfSteps.setConstraint(new PTNumberValidator(Labels.getLabel("label_IstnormFinanceMainDialog_NumberOfSteps.value"), true, false));
		}
		...
		if (this.gb_gracePeriodDetails.isVisible()) {
			...
			if (!this.graceTerms.isReadonly()) {
				this.graceTerms.setConstraint(new PTStringValidator(Labels.getLabel("label_IstnormFinanceMainDialog_GraceTerms.value"),null,true));
			}
			
			if (!this.grcMargin.isReadonly()) {
				this.grcMargin.setConstraint(new RateValidator(13, 9, 
						Labels.getLabel("label_IstnormFinanceMainDialog_GraceMargin.value"), true));
			}

			if(this.allowGrace.isChecked()){
				this.grcEffectiveRate.setConstraint(new RateValidator(13, 9,
						Labels.getLabel("label_IstnormFinanceMainDialog_GracePftRate.value"), true));
			}

			if (!this.nextGrcPftDate.isReadonly() && 
					FrequencyUtil.validateFrequency(this.gracePftFrq.getValue()) == null) {

				this.nextGrcPftDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_IstnormFinanceMainDialog_NextGrcPftDate.value"),true));
			}

			if (!this.nextGrcPftRvwDate.isReadonly() && 
					FrequencyUtil.validateFrequency(this.gracePftRvwFrq.getValue()) == null) {

				this.nextGrcPftRvwDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_IstnormFinanceMainDialog_NextGrcPftRvwDate.value"),true));
			}
		}
		...
		if (!this.nextRepayDate.isReadonly() && 
			...
			this.nextRepayDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_IstnormFinanceMainDialog_NextRepayDate.value"),true));
		}

		if (!this.nextRepayPftDate.isReadonly() && 
				FrequencyUtil.validateFrequency(this.repayPftFrq.getValue()) == null) {

			this.nextRepayPftDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_IstnormFinanceMainDialog_NextRepayPftDate.value"),true));
		}

		if (!this.nextRepayRvwDate.isReadonly() && 
				FrequencyUtil.validateFrequency(this.repayRvwFrq.getValue()) == null) {

			this.nextRepayRvwDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_IstnormFinanceMainDialog_NextRepayRvwDate.value"),true));
		}

		if (!this.nextRepayCpzDate.isReadonly() && 
				FrequencyUtil.validateFrequency(this.repayCpzFrq.getValue()) == null) {

			this.nextRepayCpzDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_IstnormFinanceMainDialog_NextRepayCpzDate.value"),true));
		}

		this.repayEffectiveRate.setConstraint(new RateValidator(13, 9,
				Labels.getLabel("label_IstnormFinanceMainDialog_ProfitRate.value"), true));

		if (!this.repayMargin.isReadonly()) {
			this.repayMargin.setConstraint(new RateValidator(13, 9, 
					Labels.getLabel("label_IstnormFinanceMainDialog_RepayMargin.value"), true));
		}

		if(financeType.getFinMinTerm() == 1 &&
				financeType.getFinMaxTerm() == 1){

			this.maturityDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_IstnormFinanceMainDialog_MaturityDate.value"),true));
		}
		if(!this.finStartDate.isReadonly()){
			this.finStartDate.setConstraint(new PTDateValidator(Labels.getLabel("label_IstnormFinanceMainDialog_FinStartDate.value"), true,startDate,endDate,false));
			}
		logger.debug("Leaving");
	}
	...
	private void doSetLOVValidation() {
		...
		this.lovDescFinTypeName.setConstraint(new PTStringValidator(Labels.getLabel("label_IstnormFinanceMainDialog_FinType.value"),null,true));
		...
		if (!recSave && StringUtils.trimToEmpty(this.disbAcctId.getSclass()).equals("mandatory")) {
			this.disbAcctId.setConstraint(new PTStringValidator(Labels.getLabel("label_IstnormFinanceMainDialog_DisbAcctId.value"),null,true));
		}

		if (!recSave && this.repayAcctId.getSclass().equals("mandatory")) {
			this.repayAcctId.setConstraint(new PTStringValidator(Labels.getLabel("label_IstnormFinanceMainDialog_RepayAcctId.value"),null,true));
		}

		if (!recSave && this.downPayAccount.getSclass().equals("mandatory")) {
			this.downPayAccount.setConstraint(new PTStringValidator(Labels.getLabel("label_IstnormFinanceMainDialog_DownPayAccount.value"),null,true));
		}

		if (!this.btnSearchCommitmentRef.isDisabled() && 
				StringUtils.trimToEmpty(space_commitmentRef.getSclass()).equals("mandatory")) {
			this.lovDescCommitmentRefName.setConstraint(new PTStringValidator(Labels.getLabel("label_IstnormFinanceMainDialog_CommitRef.value"),null,true));
		}
		...
		if(this.allowGrcInd.isChecked() && !this.btnSearchGrcIndBaseRate.isDisabled()){
			this.lovDescGrcIndBaseRateName.setConstraint(new PTStringValidator(Labels.getLabel("label_IstnormFinanceMainDialog_FinGrcIndBaseRate.value"),null,true));			
		}
		...
		if(this.allowRpyInd.isChecked() && !this.btnSearchRpyIndBaseRate.isDisabled()){
			this.lovDescRpyIndBaseRateName.setConstraint(new PTStringValidator(Labels.getLabel("label_IstnormFinanceMainDialog_FinRpyIndBaseRate.value"),null,true));			
		}
		logger.debug("Leaving ");
	}
		...
		if (isWorkFlowEnabled()) {
			...
			while (!"".equals(serviceTasks)) {
				...
				if (StringUtils.trimToEmpty(method).contains(PennantConstants.method_doDedup)) {
					...
					tFinanceDetail = FetchDedupDetails.getLoanDedup(getRole(),aFinanceDetail, this.window_IstnormFinanceMainDialog);
					...
					if(!StringUtils.trimToEmpty(tFinanceMain.getLovDescCustCoreBank()).equals("")){
						...
						tFinanceDetail = FetchBlackListDetails.getBlackListCustomers(tFinanceDetail ,
								this.window_IstnormFinanceMainDialog);
					}
			...
			while (retValue == PennantConstants.porcessOVERIDE) {
				...
				if (StringUtils.trimToEmpty(method).equalsIgnoreCase("")) {
					...
					if (StringUtils.trimToEmpty(method).equalsIgnoreCase(PennantConstants.method_doApprove)) {
						...
						retValue = ErrorControl.showErrorControl(this.window_IstnormFinanceMainDialog, auditHeader);
						return processCompleted;
					}
				}

				auditHeader = ErrorControl.showErrorDetails(this.window_IstnormFinanceMainDialog, auditHeader);
	...
	public void onClick$btnSearchFinType(Event event) {
		...
		Object dataObject = ExtendedSearchListBox.show(this.window_IstnormFinanceMainDialog, "FinanceType");
	...
	public void onClick$btnSearchDisbAcctId(Event event) throws InterruptedException {
		...
		if(!StringUtils.trimToEmpty(this.lovDescCustCIF.getValue()).equals("")) {
			...
			try {
				...
				dataObject = ExtendedSearchListBox.show(this.window_IstnormFinanceMainDialog, "Accounts", iAccountList);
				if (dataObject instanceof String) {
...
*/
	...
	//	filters[1] = new Filter("cmtCcy", this.finCcy.getValue(), Filter.OP_EQUAL);
		if(this.finStartDate.getValue() != null){
			...
			throw new WrongValueException(finStartDate,Labels.getLabel("FIELD_NO_EMPTY",
					new String[] { Labels.getLabel("label_IstnormFinanceMainDialog_FinStartDate.value")}));
		}

		Object dataObject = ExtendedSearchListBox.show(this.window_IstnormFinanceMainDialog, "Commitment", filters);
	...
	private void onCheckCustAcceptance(){
		...
		this.label_IstnormFinanceMainDialog_CbbApproved.setVisible(false);
	...
	public void onClick$btnSearchGrcIndBaseRate(Event event) {
		...
		Object dataObject = ExtendedSearchListBox.show(this.window_IstnormFinanceMainDialog, "BaseRateCode");
		if (dataObject instanceof String) {
	...
	private void setCustomerData(){
		...
		setAgreementDetailTab(this.window_IstnormFinanceMainDialog);
		...
		if(getWorkFlow().getTaskTabs(getWorkFlow().getTaskId(getRole())).equals("Accounting")){
			...
			if(getAccountingDetailDialogCtrl() != null){
				...
				setAccountingDetailTab(this.window_IstnormFinanceMainDialog);
	...
	public void onClick$viewCustInfo(Event event){
		try {
			...
			if (finDivision.equals(PennantConstants.FIN_DIVISION_RETAIL)) {
				Executions.createComponents("/WEB-INF/pages/CustomerMasters/Customer/FinCustomerDetailsEnq.zul", window_IstnormFinanceMainDialog, map);
			}else{
				Executions.createComponents("/WEB-INF/pages/CustomerMasters/Enquiry/CustomerSummary.zul", window_IstnormFinanceMainDialog, map);
			}
		} catch (final Exception e) {
			logger.error("onOpenWindow:: error opening window / " + e.getMessage());
		}
