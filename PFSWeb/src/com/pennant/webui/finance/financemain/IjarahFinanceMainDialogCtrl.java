 ...
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
 */
public class IjarahFinanceMainDialogCtrl extends FinanceBaseCtrl implements Serializable {

	private static final long serialVersionUID = 6004939933729664895L;
	private final static Logger logger = Logger.getLogger(IjarahFinanceMainDialogCtrl.class);
	...
	public void onCreate$window_IjarahFinanceMainDialog(Event event) throws Exception {
		...
		setLabel_FinanceMainDialog_CbbApproved(label_IjarahFinanceMainDialog_CbbApproved);
	...
	public void doReject() throws InterruptedException{
		...
		try{
			Executions.createComponents("/WEB-INF/pages/Finance/FinanceMain/FinanceReject.zul",
					window_IjarahFinanceMainDialog, map);
	...
	public void doWriteComponentsToBean(FinScheduleData aFinanceSchData) throws InterruptedException, IllegalAccessException, InvocationTargetException { 
		...
		try {
			aFinanceMain.setSecurityDeposit(PennantAppUtil.unFormateAmount(
					this.securityDeposit.getValue(), formatter));
	...
	public void doShowDialog(FinanceDetail afinanceDetail) throws InterruptedException {
		...
		try {
			...
			if(financeType.getFinMinTerm() == 1 && financeType.getFinMaxTerm() == 1){
				...
				this.label_IjarahFinanceMainDialog_ScheduleMethod.setVisible(false);
						...
						if(!isRIAExist){
							returnSetEntries = getEngineExecution().getAccEngineExecResults(dataSet, 
									getAmountCodes(), "N", getFeeDetailDialogCtrl() != null ? getFeeDetailDialogCtrl().getFeeRuleDetailsMap() : null,
											false, getFinanceDetail().getFinScheduleData().getFinanceType());						}else{
		...
		if(getAccountingDetailDialogCtrl() != null){
			...
			if(finMain != null && StringUtils.trimToEmpty(finMain.getRecordType()).equals(PennantConstants.RECORD_TYPE_NEW)){
				if(finMain.getGrcPeriodEndDate() != null && finMain.getGrcPeriodEndDate().compareTo(curBDay) <= 0){
					...
					dataSet.setFeeAmount(finMain.getFeeChargeAmt() == null ?  BigDecimal.ZERO : finMain.getFeeChargeAmt());
					if(!isRIAExist){
						returnSetEntries = getEngineExecution().getAccEngineExecResults(dataSet, 
								getAmountCodes(), "N", null, false, getFinanceDetail().getFinScheduleData().getFinanceType());						
					}else{
						List<AEAmountCodesRIA> riaDetailList = getEngineExecutionRIA().prepareRIADetails(
								getContributorDetailsDialogCtrl() == null? null: getContributorDetailsDialogCtrl().getContributorsList(), 
										dataSet.getFinReference());
					...
					getAccountingDetailDialogCtrl().doFillGraceEndAccounting(returnSetEntries);
				}
			}
		}
		logger.debug("Leaving");
	...
	private void doSetValidation() {
		...
		if(!this.stepPolicy.isReadonly() && this.stepFinance.isChecked() && !this.alwManualSteps.isChecked()){
			this.stepPolicy.setConstraint(new PTStringValidator( Labels.getLabel("label_IjarahFinanceMainDialog_StepPolicy.value"), null, true,true));
		}
		...
		/*if (!this.downPayBank.isDisabled()) {
			this.downPayBank.setConstraint(new AmountValidator(18, 0, 
					Labels.getLabel("label_IjarahFinanceMainDialog_DownPayment.value"), false));
	...
	private void doSetLOVValidation() {
		...
		if (!this.finBranch.isReadonly()) {
			this.finBranch.setConstraint(new PTStringValidator(Labels.getLabel("label_IjarahFinanceMainDialog_FinBranch.value"),null,true,true));
		}

		if (!this.custCIF.isReadonly()) {
			this.custCIF.setConstraint(new PTStringValidator(Labels.getLabel("label_IjarahFinanceMainDialog_CustID.value"),null,true,true));
		}
		...
		if (!this.finPurpose.isReadonly()) {
			this.finPurpose.setConstraint(new PTStringValidator(Labels.getLabel("label_IjarahFinanceMainDialog_FinPurpose.value"),null,true,true));
		}

		//FinanceMain Details Tab ---> 2. Grace Period Details

		if(!this.graceBaseRate.isReadonly()) {
			this.graceBaseRate.setConstraint(new PTStringValidator(Labels.getLabel("label_IjarahFinanceMainDialog_GraceBaseRate.value"),null,true,true));
		}
		...
		if(!this.repayBaseRate.isReadonly()) {
			this.repayBaseRate.setConstraint(new PTStringValidator(Labels.getLabel("label_IjarahFinanceMainDialog_RepayBaseRate.value"),null,true,true));
		}
	...
	public void doEdit() {
		logger.debug("Entering");
		super.doEdit();
		this.securityDeposit.setVisible(!isReadOnly("FinanceMainDialog_securityDeposit"));
		...
		this.label_IjarahFinanceMainDialog_SecurityDeposit.setVisible(this.securityDeposit.isVisible());
		logger.debug("Leaving");
	}
	...
	public void onClick$btnSearchDisbAcctId(Event event) throws InterruptedException {
		...
		if(!StringUtils.trimToEmpty(this.custCIF.getValue()).equals("")) {
			...
			try {
				...
				dataObject = ExtendedSearchListBox.show(this.window_IjarahFinanceMainDialog, "Accounts", iAccountList);
				if (dataObject instanceof String) {
...
*/	/**
	...
	public void onClick$btnSearchCommitmentRef(Event event) throws InterruptedException, AccountNotFoundException, 
		...
		Object dataObject = ExtendedSearchListBox.show(this.window_IjarahFinanceMainDialog, "Commitment", filters);
	...
	private void onCheckCustAcceptance(){
		...
		this.label_IjarahFinanceMainDialog_CbbApproved.setVisible(false);
	...
	private void setCustomerData(){
		...
		setAgreementDetailTab(this.window_IjarahFinanceMainDialog);
		...
		if(getWorkFlow().getTaskTabs(getWorkFlow().getTaskId(getRole())).equals("Accounting")){
			...
			if(getAccountingDetailDialogCtrl() != null){
				...
				setAccountingDetailTab(this.window_IjarahFinanceMainDialog);
			}
		}
	...
	public void onClick$viewCustInfo(Event event){
		try {
			...
			if (finDivision.equals(PennantConstants.FIN_DIVISION_RETAIL)) {
				Executions.createComponents("/WEB-INF/pages/CustomerMasters/Customer/FinCustomerDetailsEnq.zul", window_IjarahFinanceMainDialog, map);
			}else{
				Executions.createComponents("/WEB-INF/pages/CustomerMasters/Enquiry/CustomerSummary.zul", window_IjarahFinanceMainDialog, map);
			}
		} catch (final Exception e) {
			logger.error("onOpenWindow:: error opening window / " + e.getMessage());
		}
