 ...
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
 */
public class MurabahaFinanceMainDialogCtrl extends FinanceBaseCtrl implements Serializable {
	...
	protected Label 		label_MurabahaFinanceMainDialog_ScheduleMethod;		// autoWired
	protected Label 		label_MurabahaFinanceMainDialog_FinRepayPftOnFrq;	// autoWired
	protected Label 		label_MurabahaFinanceMainDialog_CommitRef; 			// autoWired
	...
	public void onCreate$window_MurabahaFinanceMainDialog(Event event) throws Exception {
		...
		setLabel_FinanceMainDialog_CbbApproved(label_MurabahaFinanceMainDialog_CbbApproved);
		setLabel_FinanceMainDialog_CommitRef(label_MurabahaFinanceMainDialog_CommitRef);
		setLabel_FinanceMainDialog_DepriFrq(label_MurabahaFinanceMainDialog_DepriFrq);
	...
	public void onClick$btnHelp(Event event) throws InterruptedException {
		logger.debug("Entering " + event.toString());
		PTMessageUtils.showHelpWindow(event, window_MurabahaFinanceMainDialog);
		logger.debug("Leaving " + event.toString());
	}
	...
	public void doShowDialog(FinanceDetail afinanceDetail) throws InterruptedException {
		...
		try {
			...
			if(financeType.getFinMinTerm() == 1 &&  financeType.getFinMaxTerm() == 1){
				...
				this.label_MurabahaFinanceMainDialog_ScheduleMethod.setVisible(false);
		...
		/*if(this.diffDisbCcy.isChecked()){
			if (!this.ccyConversionRate.isReadonly()) {
				this.ccyConversionRate.setConstraint(new AmountValidator(18,0,
						Labels.getLabel("label_MurabahaFinanceMainDialog_CcyConversionRate.value"), false));
			}
			if (!this.disbCcyFinAmount.isReadonly()) {
				this.disbCcyFinAmount.setConstraint(new AmountValidator(18,0,
						Labels.getLabel("label_MurabahaFinanceMainDialog_DisbCcyFinAmount.value"), false));
			}
		}*/
	...
	private void doSetLOVValidation() {
		...
		if (!this.btnSearchCommitmentRef.isDisabled() && 
			...
			this.lovDescCommitmentRefName.setConstraint("NO EMPTY:" + Labels.getLabel("FIELD_NO_EMPTY",
					new String[] { Labels.getLabel("label_MurabahaFinanceMainDialog_CommitRef.value") }));
		}
		...
		/*if(this.diffDisbCcy.isChecked()){
			if (!this.disbCcy.isReadonly()) {
				this.disbCcy.setConstraint("NO EMPTY:" + Labels.getLabel("FIELD_NO_EMPTY",
						new String[] { Labels.getLabel("label_MurabahaFinanceMainDialog_DisbCcy.value") }));
			}
		}*/
		...
		if (isWorkFlowEnabled()) {
			...
			while (!"".equals(serviceTasks)) {
				...
				if (StringUtils.trimToEmpty(method).contains(PennantConstants.method_doDedup)) {
					...
					tFinanceDetail = FetchDedupDetails.getLoanDedup(getRole(),aFinanceDetail, this.window_MurabahaFinanceMainDialog);
			...
			while (retValue == PennantConstants.porcessOVERIDE) {
				...
				if (StringUtils.trimToEmpty(method).equalsIgnoreCase("")) {
					...
					if (StringUtils.trimToEmpty(method).equalsIgnoreCase(PennantConstants.method_doApprove)) {
						...
						retValue = ErrorControl.showErrorControl(this.window_MurabahaFinanceMainDialog, auditHeader);
						return processCompleted;
					}
				}

				auditHeader = ErrorControl.showErrorDetails(this.window_MurabahaFinanceMainDialog, auditHeader);
	...
	public void onClick$btnSearchFinType(Event event) {
		...
		Object dataObject = ExtendedSearchListBox.show(this.window_MurabahaFinanceMainDialog, "FinanceType");
	...
	public void onChange$lovDescFinCcyName(Event event) throws InterruptedException {
		...
		if (customer != null) {
			...
			finCcy.setValue("");
			throw new WrongValueException(this.lovDescFinCcyName, Labels.getLabel("FIELD_NO_INVALID", new String[] { Labels.getLabel("label_MurabahaFinanceMainDialog_FinCcy.value") }));
		}
	...
	public void onClick$btnSearchFinCcy(Event event) {
		...
		this.lovDescFinCcyName.setConstraint("");
		Object dataObject = ExtendedSearchListBox.show(this.window_MurabahaFinanceMainDialog, "Currency");
		if (dataObject instanceof String) {
	...
	//	filters[1] = new Filter("cmtCcy", this.finCcy.getValue(), Filter.OP_EQUAL);
		...
		Object dataObject = ExtendedSearchListBox.show(this.window_MurabahaFinanceMainDialog, "Commitment", filters);
	...
	private void onCheckCustAcceptance(){
		...
		this.label_MurabahaFinanceMainDialog_CbbApproved.setVisible(false);
	...
	public void onClick$btnSearchGraceBaseRate(Event event) throws InterruptedException {
		...
		this.grcEffectiveRate.setConstraint("");
		Object dataObject = ExtendedSearchListBox.show(this.window_MurabahaFinanceMainDialog, "BaseRateCode");
	...
	public void onClick$btnSearchGraceSpecialRate(Event event) throws InterruptedException {
		...
		this.grcEffectiveRate.setConstraint("");
		Object dataObject = ExtendedSearchListBox.show(this.window_MurabahaFinanceMainDialog, "SplRateCode");
	...
	private void setCustomerData(){
		...
		setAgreementDetailTab(this.window_MurabahaFinanceMainDialog);
		...
		if(getWorkFlow().getTaskTabs(getWorkFlow().getTaskId(getRole())).equals("Accounting")){
			...
			if(getAccountingDetailDialogCtrl() != null){
				...
				setAccountingDetailTab(this.window_MurabahaFinanceMainDialog);
	...
	public void onClick$viewCustInfo(Event event){
		try {
			...
			if (finDivision.equals(PennantConstants.FIN_DIVISION_RETAIL)) {
				Executions.createComponents("/WEB-INF/pages/CustomerMasters/Customer/FinCustomerDetailsEnq.zul", window_MurabahaFinanceMainDialog, map);
			}else{
				Executions.createComponents("/WEB-INF/pages/CustomerMasters/Enquiry/CustomerSummary.zul", window_MurabahaFinanceMainDialog, map);
			}
		} catch (final Exception e) {
			logger.error("onOpenWindow:: error opening window / " + e.getMessage());
		}
