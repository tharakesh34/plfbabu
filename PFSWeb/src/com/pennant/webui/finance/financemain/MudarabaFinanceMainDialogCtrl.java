 ...
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
 */
public class MudarabaFinanceMainDialogCtrl extends FinanceBaseCtrl implements Serializable {

	private static final long serialVersionUID = 6004939933729664895L;
	private final static Logger logger = Logger.getLogger(MudarabaFinanceMainDialogCtrl.class);
	...
	protected Label 		label_MudarabaFinanceMainDialog_PlanDeferCount;				// autoWired
	 ...
	 */
	public MudarabaFinanceMainDialogCtrl() {
		super();
	}
	...
	public void onCreate$window_MudarabaFinanceMainDialog(Event event) throws Exception {
		...
		setMainWindow(window_MudarabaFinanceMainDialog);
		setLabel_FinanceMainDialog_CbbApproved(label_MudarabaFinanceMainDialog_CbbApproved);
		setLabel_FinanceMainDialog_CommitRef(label_MudarabaFinanceMainDialog_CommitRef);
		setLabel_FinanceMainDialog_DepriFrq(label_MudarabaFinanceMainDialog_DepriFrq);
		setLabel_FinanceMainDialog_FinRepayPftOnFrq(label_MudarabaFinanceMainDialog_FinRepayPftOnFrq);
		setLabel_FinanceMainDialog_PlanDeferCount(label_MudarabaFinanceMainDialog_PlanDeferCount);
	...
	public void doShowDialog(FinanceDetail afinanceDetail) throws InterruptedException {
		...
		try {
			...
			if(financeType.getFinMinTerm() == 1 &&  financeType.getFinMaxTerm() == 1){
				...
				this.label_MudarabaFinanceMainDialog_ScheduleMethod.setVisible(false);
	...
	private void doSetValidation() {
		...
		if (!this.finReference.isReadonly() && 
			...
			this.finReference.setConstraint(new PTStringValidator(Labels.getLabel("label_MudarabaFinanceMainDialog_FinReference.value"),null,true));
		}
		...
		if(!this.stepPolicy.isReadonly() && this.stepFinance.isChecked() && !this.alwManualSteps.isChecked()){
			this.stepPolicy.setConstraint(new PTStringValidator( Labels.getLabel("label_MudarabaFinanceMainDialog_StepPolicy.value"), null,true, true));
		}
		...
		/*if (!this.downPayBank.isDisabled()) {
			this.downPayBank.setConstraint(new AmountValidator(18, 0, 
					Labels.getLabel("label_MudarabaFinanceMainDialog_DownPayment.value"), false));
		...
		if (this.gb_gracePeriodDetails.isVisible()) {
			...
			if (!this.graceTerms.isReadonly()) {
				this.graceTerms.setConstraint(new PTStringValidator(Labels.getLabel("label_MudarabaFinanceMainDialog_GraceTerms.value"),null,true));
			}
			...
			if (!this.nextGrcPftDate.isDisabled() && 
				...
				this.nextGrcPftDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_MudarabaFinanceMainDialog_NextGrcPftDate.value"),true));
			}

			if (!this.nextGrcPftRvwDate.isDisabled() && 
					FrequencyUtil.validateFrequency(this.gracePftRvwFrq.getValue()) == null) {

				this.nextGrcPftRvwDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_MudarabaFinanceMainDialog_NextGrcPftRvwDate.value"),true));
			}
		}
		...
		if (!this.nextRepayDate.isDisabled() && 
			...
			this.nextRepayDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_MudarabaFinanceMainDialog_NextRepayDate.value"),true));
		}

		if (!this.nextRepayPftDate.isDisabled() && 
				FrequencyUtil.validateFrequency(this.repayPftFrq.getValue()) == null) {

			this.nextRepayPftDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_MudarabaFinanceMainDialog_NextRepayPftDate.value"),true));
		}

		if (!this.nextRepayRvwDate.isDisabled() && 
				FrequencyUtil.validateFrequency(this.repayRvwFrq.getValue()) == null) {

			this.nextRepayRvwDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_MudarabaFinanceMainDialog_NextRepayRvwDate.value"),true));
		}

		if (!this.nextRepayCpzDate.isDisabled() && 
				FrequencyUtil.validateFrequency(this.repayCpzFrq.getValue()) == null) {

			this.nextRepayCpzDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_MudarabaFinanceMainDialog_NextRepayCpzDate.value"),true));
		}
	...
	private void doSetLOVValidation() {
		...
		this.lovDescFinTypeName.setConstraint(new PTStringValidator(Labels.getLabel("label_MudarabaFinanceMainDialog_FinType.value"),null,true));

		this.finCcy.setConstraint(new PTStringValidator(Labels.getLabel("label_MudarabaFinanceMainDialog_FinCcy.value"),null,true,true));

		if (!this.finBranch.isReadonly()) {
			this.finBranch.setConstraint(new PTStringValidator(Labels.getLabel("label_MudarabaFinanceMainDialog_FinBranch.value"),null,true,true));
		}

		if (!this.custCIF.isReadonly()) {
			this.custCIF.setConstraint(new PTStringValidator(Labels.getLabel("label_MudarabaFinanceMainDialog_CustID.value"),null,true,true));
		}
		...
		if (!this.btnSearchCommitmentRef.isDisabled() && 
			...
			this.lovDescCommitmentRefName.setConstraint(new PTStringValidator(Labels.getLabel("label_MudarabaFinanceMainDialog_CommitRef.value"),null,true));
		}

		if (!this.finPurpose.isReadonly()) {
			this.finPurpose.setConstraint(new PTStringValidator(Labels.getLabel("label_MudarabaFinanceMainDialog_FinPurpose.value"),null,true,true));
		}

		//FinanceMain Details Tab ---> 2. Grace Period Details

		if(!this.graceBaseRate.isReadonly()) {
			this.graceBaseRate.setConstraint(new PTStringValidator(Labels.getLabel("label_MudarabaFinanceMainDialog_GraceBaseRate.value"),null,true,true));
		}

		if(this.allowGrcInd.isChecked() && !this.btnSearchGrcIndBaseRate.isDisabled()){
			this.lovDescGrcIndBaseRateName.setConstraint(new PTStringValidator(Labels.getLabel("label_MudarabaFinanceMainDialog_FinGrcIndBaseRate.value"),null,true));			
		}

		//FinanceMain Details Tab ---> 3. Repayment Period Details

		if(!this.repayBaseRate.isReadonly()) {
			this.repayBaseRate.setConstraint(new PTStringValidator(Labels.getLabel("label_MudarabaFinanceMainDialog_RepayBaseRate.value"),null,true,true));
		}

		if(this.allowRpyInd.isChecked() && !this.btnSearchRpyIndBaseRate.isDisabled()){
			this.lovDescRpyIndBaseRateName.setConstraint(new PTStringValidator(Labels.getLabel("label_MudarabaFinanceMainDialog_FinRpyIndBaseRate.value"),null,true));			
		}
		logger.debug("Leaving ");
	}
	...
	private void doDelete() throws InterruptedException {
		...
		if (conf == MultiLineMessageBox.YES) {
			...
			try {
				...
				showErrorMessage(this.window_MudarabaFinanceMainDialog, e);
			}
		}
		logger.debug("Leaving");
	}
				...
				if (StringUtils.trimToEmpty(method).contains(PennantConstants.method_doDedup)) {
					...
					if(!StringUtils.trimToEmpty(tFinanceMain.getLovDescCustCoreBank()).equals("")){
						...
						tFinanceDetail = FetchBlackListDetails.getBlackListCustomers(tFinanceDetail ,
								this.window_MudarabaFinanceMainDialog);
					}
	...
	public void onClick$btnSearchDisbAcctId(Event event) throws InterruptedException {
		...
		if(!StringUtils.trimToEmpty(this.custCIF.getValue()).equals("")) {
			...
			try {
				...
				dataObject = ExtendedSearchListBox.show(this.window_MudarabaFinanceMainDialog, "Accounts", iAccountList);
				if (dataObject instanceof String) {
			...
			throw new WrongValueException(this.custCIF,Labels.getLabel("FIELD_NO_EMPTY",
					new String[] { Labels.getLabel("label_MudarabaFinanceMainDialog_CustID.value") }));
		}
	...
	//	filters[1] = new Filter("cmtCcy", this.finCcy.getValue(), Filter.OP_EQUAL);
		...
		Object dataObject = ExtendedSearchListBox.show(this.window_MudarabaFinanceMainDialog, "Commitment", filters);
	...
	private void onCheckCustAcceptance(){
		...
		this.label_MudarabaFinanceMainDialog_CbbApproved.setVisible(false);
	...
	private void setCustomerData(){
		...
		setFinanceDetail(getFinanceDetailService().fetchFinCustDetails(getFinanceDetail(), custCtgType,getFinanceDetail().getFinScheduleData().getFinanceMain().getFinType(), getRole()));

		getFinanceDetail().getFinScheduleData().getFinanceMain().setLovDescCustFName(StringUtils.trimToEmpty(customer.getCustFName()));
		getFinanceDetail().getFinScheduleData().getFinanceMain().setLovDescCustLName(StringUtils.trimToEmpty(customer.getCustLName()));
		getFinanceDetail().getFinScheduleData().getFinanceMain().setLovDescCustCIF(StringUtils.trimToEmpty(customer.getCustCIF()));
		...
		setAgreementDetailTab(this.window_MudarabaFinanceMainDialog);
		...
		if(getWorkFlow().getTaskTabs(getWorkFlow().getTaskId(getRole())).equals("Accounting")){
			...
			if(getAccountingDetailDialogCtrl() != null){
				...
				setAccountingDetailTab(this.window_MudarabaFinanceMainDialog);
			}
		}
	...
	public void onClick$viewCustInfo(Event event){
		try {
			...
			if (finDivision.equals(PennantConstants.FIN_DIVISION_RETAIL)) {
				Executions.createComponents("/WEB-INF/pages/CustomerMasters/Customer/FinCustomerDetailsEnq.zul", window_MudarabaFinanceMainDialog, map);
			}else{
				Executions.createComponents("/WEB-INF/pages/CustomerMasters/Enquiry/CustomerSummary.zul", window_MudarabaFinanceMainDialog, map);
			}
		} catch (final Exception e) {
			logger.error("onOpenWindow:: error opening window / " + e.getMessage());
		}
