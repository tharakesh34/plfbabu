...
public class InvestMentFinanceMainListModelItemRenderer implements ListitemRenderer<FinanceMain>, Serializable {
	...
	public void render(Listitem item, FinanceMain financeMain, int count) throws Exception {
		...
		lc.setParent(item);
		lc = new Listcell(PennantAppUtil.amountFormate(financeMain.getFinAmount().subtract(financeMain.getDownPayment()).subtract(financeMain.getFeeChargeAmt() == null ?BigDecimal.ZERO : financeMain.getFeeChargeAmt()), financeMain.getLovDescFinFormatter()));
		lc.setStyle("text-align:right;");
		lc.setParent(item);
