 ...
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
 */
public class MusharakFinanceMainDialogCtrl extends FinanceBaseCtrl implements Serializable {

	private static final long serialVersionUID = 6004939933729664895L;
	private final static Logger logger = Logger.getLogger(MusharakFinanceMainDialogCtrl.class);
	 ...
	 */
	public MusharakFinanceMainDialogCtrl() {
		super();
	}
	...
	public void onCreate$window_MusharakFinanceMainDialog(Event event) throws Exception {
		...
		setMainWindow(window_MusharakFinanceMainDialog);
		setLabel_FinanceMainDialog_CbbApproved(label_MusharakFinanceMainDialog_CbbApproved);
	...
	public void doShowDialog(FinanceDetail afinanceDetail) throws InterruptedException {
		...
		try {
			...
			if(financeType.getFinMinTerm() == 1 && 
				...
				this.label_MusharakFinanceMainDialog_ScheduleMethod.setVisible(false);
			...
			if(!afinanceDetail.getFinScheduleData().getFinanceMain().isAllowGrcPeriod()){
				...
				this.label_MusharakFinanceMainDialog_AlwGrace.setVisible(false);
			}
			if (moduleDefiner.equals("")){
	...
	public List<Object> prepareContributor(){
		...
		list.add(this.downPayBank.getValue() == null?BigDecimal.ZERO:this.downPayBank.getValue());
		logger.debug("Leaving");
		return list;
	...
	private void doSetValidation() {
		...
		if (!this.finReference.isReadonly() && 
			...
			this.finReference.setConstraint(new PTStringValidator(Labels.getLabel("label_MusharakFinanceMainDialog_FinReference.value"),null,true));
		}
		...
		/*if (!this.downPayBank.isDisabled()) {
			this.downPayBank.setConstraint(new AmountValidator(18, 0, 
		...
		if(!this.noOfSteps.isReadonly() && this.stepFinance.isChecked() && this.alwManualSteps.isChecked()){
			this.noOfSteps.setConstraint(new PTNumberValidator(Labels.getLabel("label_MusharakFinanceMainDialog_NumberOfSteps.value"), true, false));
		}
		...
		if (this.gb_gracePeriodDetails.isVisible()) {
			...
			if (!this.graceTerms.isReadonly()) {
				this.graceTerms.setConstraint(new PTStringValidator(Labels.getLabel("label_MusharakFinanceMainDialog_GraceTerms.value"),null,true));
			}
			...
			if (!this.nextGrcPftDate.isDisabled() && 
				...
				this.nextGrcPftDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_MusharakFinanceMainDialog_NextGrcPftDate.value"),true));
			}

			/*if (!this.nextGrcPftRvwDate.isDisabled() && 
					FrequencyUtil.validateFrequency(this.gracePftRvwFrq.getValue()) == null) {

				this.nextGrcPftRvwDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_MusharakFinanceMainDialog_NextGrcPftRvwDate.value"),true));
			}*/
		}
		...
		if (!this.nextRepayDate.isDisabled() && 
			...
			this.nextRepayDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_MusharakFinanceMainDialog_NextRepayDate.value"),true));
		}

		if (!this.nextRepayPftDate.isDisabled() && 
				FrequencyUtil.validateFrequency(this.repayPftFrq.getValue()) == null) {

			this.nextRepayPftDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_MusharakFinanceMainDialog_NextRepayPftDate.value"),true));
		}

		if (!this.nextRepayRvwDate.isDisabled() && 
				FrequencyUtil.validateFrequency(this.repayRvwFrq.getValue()) == null) {

			this.nextRepayRvwDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_MusharakFinanceMainDialog_NextRepayRvwDate.value"),true));
		}

		if (!this.nextRepayCpzDate.isDisabled() && 
				FrequencyUtil.validateFrequency(this.repayCpzFrq.getValue()) == null) {

			this.nextRepayCpzDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_MusharakFinanceMainDialog_NextRepayCpzDate.value"),true));
		}
		...
		if(financeType.getFinMinTerm() == 1 &&
			...
			this.maturityDate_two.setConstraint(new PTDateValidator(Labels.getLabel("label_MusharakFinanceMainDialog_MaturityDate.value"),true));
		}
		if(!this.finStartDate.isReadonly()){
			this.finStartDate.setConstraint(new PTDateValidator(Labels.getLabel("label_MusharakFinanceMainDialog_FinStartDate.value"), true,startDate,endDate,false));
			}
		logger.debug("Leaving");
	}
	...
	private void doSetLOVValidation() {
		...
		this.lovDescFinTypeName.setConstraint(new PTStringValidator(Labels.getLabel("label_MusharakFinanceMainDialog_FinType.value"),null,true));
...
/*		if (row_downpayAcc.isVisible() && !this.btnSearchDownPayAcc.isDisabled()) {
			this.downPayAccount.setConstraint(new PTStringValidator(Labels.getLabel("label_MusharakFinanceMainDialog_DownPayAccount.value"),null,true));
		}*/

		if (!this.btnSearchCommitmentRef.isDisabled() && 
				StringUtils.trimToEmpty(space_commitmentRef.getSclass()).equals("mandatory")) {
			this.lovDescCommitmentRefName.setConstraint(new PTStringValidator(Labels.getLabel("label_MusharakFinanceMainDialog_CommitRef.value"),null,true));
		}
		...
		if(this.allowGrcInd.isChecked() && !this.btnSearchGrcIndBaseRate.isDisabled()){
			this.lovDescGrcIndBaseRateName.setConstraint(new PTStringValidator(Labels.getLabel("label_MusharakFinanceMainDialog_FinGrcIndBaseRate.value"),null,true));			
		}
		...
		if(this.allowRpyInd.isChecked() && !this.btnSearchRpyIndBaseRate.isDisabled()){
			this.lovDescRpyIndBaseRateName.setConstraint(new PTStringValidator(Labels.getLabel("label_MusharakFinanceMainDialog_FinRpyIndBaseRate.value"),null,true));			
		}
		logger.debug("Leaving ");
	}
	...
	private void doDelete() throws InterruptedException {
		...
		if (conf == MultiLineMessageBox.YES) {
			...
			try {
				...
				showErrorMessage(this.window_MusharakFinanceMainDialog, e);
			}
		}
		logger.debug("Leaving");
	}
				...
				if (StringUtils.trimToEmpty(method).contains(PennantConstants.method_doDedup)) {
					...
					if(!StringUtils.trimToEmpty(tFinanceMain.getLovDescCustCoreBank()).equals("")){
						...
						tFinanceDetail = FetchBlackListDetails.getBlackListCustomers(tFinanceDetail ,
								this.window_MusharakFinanceMainDialog);
					}
	...
	public void onClick$btnSearchDisbAcctId(Event event) throws InterruptedException {
		...
		if(!StringUtils.trimToEmpty(this.lovDescCustCIF.getValue()).equals("")) {
			...
			try {
				...
				dataObject = ExtendedSearchListBox.show(this.window_MusharakFinanceMainDialog, "Accounts", iAccountList);
				if (dataObject instanceof String) {
...
*/
	...
	public void onClick$btnSearchCommitmentRef(Event event) throws InterruptedException, AccountNotFoundException, 
		...
		Object dataObject = ExtendedSearchListBox.show(this.window_MusharakFinanceMainDialog, "Commitment", filters);
	...
	private void onCheckCustAcceptance(){
		...
		this.label_MusharakFinanceMainDialog_CbbApproved.setVisible(false);
	...
	private void setCustomerData(){
		...
		financeDetail.setCustomerEligibilityCheck(getFinanceDetailService().getCustEligibilityDetail(customer,
				productCode, financeMain.getFinCcy(), curFinRepayAmt, months, financeMain.getFinAmount(), 
				financeMain.getCustDSR(),custEmpDesg));
		...
		setAgreementDetailTab(this.window_MusharakFinanceMainDialog);
		...
		if(getWorkFlow().getTaskTabs(getWorkFlow().getTaskId(getRole())).equals("Accounting")){
			...
			if(getAccountingDetailDialogCtrl() != null){
				...
				setAccountingDetailTab(this.window_MusharakFinanceMainDialog);
	...
	public void onChange$grcRateBasis(Event event) throws InterruptedException {
		...
		if(!this.grcRateBasis.getSelectedItem().getValue().toString().equals("#")) {
			if("F".equals(this.grcRateBasis.getSelectedItem().getValue().toString()) || 
				...
				this.grcEffectiveRate.setText("0.00");
				this.gracePftRate.setText("0.00");
			}else if("M".equals(this.grcRateBasis.getSelectedItem().getValue().toString())) {
				this.gracePftRate.setDisabled(isReadOnly("FinanceMainDialog_gracePftRate"));
	...
	public void onChange$repayRateBasis(Event event) throws InterruptedException {
		...
		if(!this.repayRateBasis.getSelectedItem().getValue().toString().equals("#")) {
			if("F".equals(this.repayRateBasis.getSelectedItem().getValue().toString()) ||
				...
				this.repayEffectiveRate.setText("0.00");
				this.repayProfitRate.setText("0.00");
			}else if("M".equals(this.repayRateBasis.getSelectedItem().getValue().toString())) {
				this.repayProfitRate.setDisabled(isReadOnly("FinanceMainDialog_profitRate"));
	...
	public void onClick$viewCustInfo(Event event){
		try {
			...
			if (finDivision.equals(PennantConstants.FIN_DIVISION_RETAIL)) {
				Executions.createComponents("/WEB-INF/pages/CustomerMasters/Customer/FinCustomerDetailsEnq.zul", window_MusharakFinanceMainDialog, map);
			}else{
				Executions.createComponents("/WEB-INF/pages/CustomerMasters/Enquiry/CustomerSummary.zul", window_MusharakFinanceMainDialog, map);
			}
		} catch (final Exception e) {
			logger.error("onOpenWindow:: error opening window / " + e.getMessage());
		}
