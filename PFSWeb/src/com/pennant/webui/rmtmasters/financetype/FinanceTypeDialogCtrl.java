...
public class FinanceTypeDialogCtrl extends GFCBaseCtrl implements Serializable {
	...
	protected Checkbox 				takafulMandatory;					// autoWired
	...
	public void doWriteComponentsToBean(FinanceType aFinanceType) {
		...
		try {
			aFinanceType.setTakafulMandatory(this.takafulMandatory.isChecked());
		} catch (WrongValueException we) {
			wve.add(we);
		}
		
		try {
			aFinanceType.setTakafulReq(this.takafulRequired.isChecked());
		} catch (WrongValueException we) {
			wve.add(we);
		}
		
		try {
			aFinanceType.setRemFeeSchdMethod(this.remFeeSchdMethod.getSelectedItem().getValue().toString());
		} catch (WrongValueException we) {
			wve.add(we);
		}
		...
		try {
			if(this.finIsIntCpz.isChecked() && this.finAECapitalize.getValue().equals("0")){
				throw new WrongValueException(this.finAECapitalize, Labels.getLabel("FIELD_NO_EMPTY",
						new String[] { Labels.getLabel("label_FinanceTypeSearch_FinCapitalize.value") }));
			}
	...
	private boolean isDataChanged() {
		...
		if(this.oldVar_takafulMandatory != this.takafulMandatory.isChecked()){
			return true;
		}
...
/*			if (!this.finMaxAmount.isReadonly() && this.finMaxAmount.getValue().compareTo(BigDecimal.ZERO) != 0 ) {
	...
	private void doDisableTakafulSchdMethod(){
		logger.debug("Entering");
 ...
 */
  ...
  public void setSteppingFieldsVisibility(boolean isVisible){
	...
	/*
	 * onCheck Event For takaful mandatory
	 */
	public void onCheck$takafulMandatory(Event event) {
		logger.debug("Entering : " + event.toString());
		if (this.takafulMandatory.isChecked()) {
			this.takafulRequired.setChecked(true);
			this.takafulRequired.setDisabled(true);
