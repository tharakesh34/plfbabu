 ...
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
 */
public class SelectNewFacilityDialogCtrl extends GFCBaseCtrl implements Serializable {
	 ...
	 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	 */
	protected Window window_SelectNewFacilityDialog; // autoWired
	...
	public void onCreate$window_SelectNewFacilityDialog(Event event) throws Exception {
		...
		if (args.containsKey("filter")) {
			termsdetails= (String[]) args.get("filter");
		}
		if (args.containsKey("facilityDetail")) {
			setFacilityDetail((FacilityDetail) args.get("facilityDetail"));
		} else {
	...
	public void onChange$approvalFor(Event event) {
		...
		if (this.approvalFor.getSelectedItem() != null && !StringUtils.trimToEmpty(this.approvalFor.getSelectedItem().getValue().toString()).equals(PennantConstants.List_Select)) {
			...
			if (StringUtils.trimToEmpty(this.approvalFor.getSelectedItem().getValue().toString()).equals(PennantConstants.FACILITY_NEW)) {
			...
			} else if (StringUtils.trimToEmpty(this.approvalFor.getSelectedItem().getValue().toString()).equals(PennantConstants.FACILITY_REVIEW)||
					StringUtils.trimToEmpty(this.approvalFor.getSelectedItem().getValue().toString()).equals(PennantConstants.FACILITY_AMENDMENT)) {
	...
	public void onClick$btnProceed(Event event) throws Exception {
		...
		if (!StringUtils.trimToEmpty(this.approvalFor.getSelectedItem().getValue().toString()).equals(PennantConstants.FACILITY_NEW)) {
			if (this.financeReference.getValidatedValue() == null || StringUtils.trimToEmpty(this.financeReference.getValidatedValue()).equals("")) {
	...
	private void dowriteData() {
		...
		if (getFacilityDetail().getFacilityFor().equals(PennantConstants.FACILITY_NEW)) {
			getFacilityDetail().setFacilityRef(PennantReferenceIDUtil.genNewFacilityRef(getFacilityDetail().getCAFReference()));
			...
			if (termDetail !=null) {
				...
				getFacilityDetail().setCCYformat(financeMain.getLovDescFinFormatter());
				...
				getFacilityDetail().setTotalFacilityCcyName(termDetail.getLovDescTotalFacilityCCYName());
				getFacilityDetail().setTotalFacilityFormatter(termDetail.getLovDescTotalFacilityFormatter());
				...
				getFacilityDetail().setUnderWritingCcyName(termDetail.getLovDescUnderWritingCCYName());
				getFacilityDetail().setUnderWritingFormatter(termDetail.getLovDescUnderWritingFormatter());
				...
				getFacilityDetail().setPropFinalTakeCcyName(termDetail.getLovDescPropFinalTakeCCYName());
				getFacilityDetail().setPropFinalTakeFormatter(termDetail.getLovDescPropFinalTakeFormatter());

			}
		} else if (getFacilityDetail().getFacilityFor().equals(PennantConstants.FACILITY_REVIEW) || 
				getFacilityDetail().getFacilityFor().equals(PennantConstants.FACILITY_AMENDMENT)) {
			...
			if (object!=null) {
				if( object instanceof Commitment){
					...
					if (facilitydetails != null) {
						...
						getFacilityDetail().setFacilityCCYName(commitment.getCcyDesc());
						getFacilityDetail().setCCYformat(commitment.getCcyEditField());
