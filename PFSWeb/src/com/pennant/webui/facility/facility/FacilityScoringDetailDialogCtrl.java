 ...
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
 */
public class FacilityScoringDetailDialogCtrl extends GFCBaseListCtrl<FinanceScoreDetail> implements Serializable {
	private static final long serialVersionUID = 6004939933729664895L;
	private final static Logger logger = Logger.getLogger(FacilityScoringDetailDialogCtrl.class);
	/*
	 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	public void onCreate$window_ScoringDetailDialog(Event event) throws Exception {
		...
		if (args.containsKey("custCtgType")) {
			custCtgType = (String) args.get("custCtgType");
		}
		...
		if ((financeReferenceDetail != null && !financeReferenceDetail.isEmpty()) || (scoringHeaderList != null && !scoringHeaderList.isEmpty())) {
			...
			for (FacilityReferenceDetail finrefdet : financeReferenceDetail) {
				...
				if (getFacility().getScoringMetrics().containsKey(finrefdet.getFinRefId())) {
					...
					if (isExecute) {
						getScoringDetailService().executeScoringMetrics(scoringMetricsList, getFacility().getCustomerScoringCheck());
	...
	public void setCustomerScoringData() {
		...
		if (getFacility().getCustomerEligibilityCheck() != null) {
			BeanUtils.copyProperties(getFacility().getCustomerEligibilityCheck(), customerScoringCheck);
			getFacility().setCustomerScoringCheck(customerScoringCheck);
		}
		logger.debug("Leaving");
	}
