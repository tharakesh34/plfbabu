 ...
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
 */
public class FacilityAgreementDetailDialogCtrl extends GFCBaseListCtrl<FinAgreementDetail> implements Serializable {
	...
	public void onCreate$window_AgreementDetailDialog(Event event) throws Exception {
		...
		if (args.containsKey("control")) {
			setCtrlObject(args.get("control"));
		}
		...
		if (aggrementList != null && !aggrementList.isEmpty()) {
			for (FacilityReferenceDetail financeReferenceDetail : aggrementList) {
				if (isAllowedToShow(financeReferenceDetail,userRole)) {
					if (AGGREEMENT_CODE_BANKCAF.equals(financeReferenceDetail.getLovDescCodelov()) && !getFacility().getCustCtgCode().equals(PennantConstants.PFF_CUSTCTG_BANK)) {
						continue;
					}
	...
	public void onGenerateReportClicked(ForwardEvent event) throws Exception {
		...
		try {
			if (getFacility() != null) {
				...
				engine.loadTemplate();
				String reportName = (getFacility().getCAFReference().replace("/", "")) + "_" + data.getLovDescAggReportName();
	...
	private FacilityAgreementDetail getAggrementData(Facility detail, String aggModuleDetails) {
		...
		try {
			if (custid != 0) {
				...
				String custCtg = getCustCtg(getFacility().getCustCtgCode());
				...
				if (aggModuleDetails.contains(PennantConstants.AGG_SCOREDE)) {
					if (custCtg.equals("C") || custCtg.equals("B")) {
		...
		try {
			...
			try {
				...
				if (collateralsFromEquation != null && !collateralsFromEquation.isEmpty()) {
					for (CustomerCollateral collateral : collateralsFromEquation) {
						...
						coll.setMarketValue(PennantApplicationUtil.amountFormate(new BigDecimal(collateral.getCollValue().toString()), PennantConstants.CURRENCY_BHD_FORMATTER));
						coll.setBankValue(PennantApplicationUtil.amountFormate(new BigDecimal(collateral.getCollBankVal().toString()), PennantConstants.CURRENCY_BHD_FORMATTER));
			...
			if (detail.getCollaterals() != null && !detail.getCollaterals().isEmpty()) {
				for (Collateral collateral : detail.getCollaterals()) {
					...
					BigDecimal marketValue = CalculationUtil.getConvertedAmount(collateral.getCurrency(), PennantConstants.CURRENCY_BHD, collateral.getValue());
					BigDecimal bankValue = CalculationUtil.getConvertedAmount(collateral.getCurrency(), PennantConstants.CURRENCY_BHD, collateral.getBankvaluation());
					coll.setMarketValue(PennantApplicationUtil.amountFormate(marketValue, PennantConstants.CURRENCY_BHD_FORMATTER));
					coll.setBankValue(PennantApplicationUtil.amountFormate(bankValue, PennantConstants.CURRENCY_BHD_FORMATTER));
			...
			agreement.setTotValue(PennantApplicationUtil.amountFormate(totValue, PennantConstants.CURRENCY_BHD_FORMATTER));
	...
	private FacilityAgreementDetail setFacilityDetails(FacilityAgreementDetail agreement, Facility detail) {
		try {
			...
			for (FacilityDetail facilityDetail : detail.getFacilityDetails()) {
				...
				amountBD = amountBD.add(CalculationUtil.getConvertedAmount(facilityDetail.getFacilityCCY(), PennantConstants.CURRENCY_BHD, facilityDetail.getNewLimit()));
				amountUSD = amountUSD.add(CalculationUtil.getConvertedAmount(facilityDetail.getFacilityCCY(), PennantConstants.CURRENCY_USD, facilityDetail.getNewLimit()));
				totExposure = totExposure.add(CalculationUtil.getConvertedAmount(facilityDetail.getFacilityCCY(), PennantConstants.CURRENCY_USD, facilityDetail.getExposure()));
				totExsisting = totExsisting.add(CalculationUtil.getConvertedAmount(facilityDetail.getFacilityCCY(), PennantConstants.CURRENCY_USD, facilityDetail.getExistingLimit()));
			...
			if (finalMaturity != null) {
				agreement.setFinalMaturityDate(formatdDate(finalMaturity));
			}
	...
	public ProposedFacility setFacilityDetailsData(ProposedFacility proposedFacility,FacilityDetail facilityDetail,Facility  detail){
		...
		//== NONC
		proposedFacility.setDate(formatdDate(detail.getStartDate()));
		...
		if (detail.getCustDOB() != null) {
			proposedFacility.setRelationshipSince(formatdDate(detail.getCustDOB()));
		} else {
		...
		proposedFacility.setNextReviewDate(formatdDate(detail.getNextReviewDate()));
		...
		if (!StringUtils.trimToEmpty(facilityDetail.getLCPeriod()).equals("")) {
			content.append("\n");
			content.append("L/C Period:");
			content.append(facilityDetail.getLCPeriod());
		}
		if (!StringUtils.trimToEmpty(facilityDetail.getUsancePeriod()).equals("")) {
			content.append("\n");
		...
		if (facilityDetail.getTotalFacility()!=null && facilityDetail.getTotalFacility().compareTo(BigDecimal.ZERO)!=0) {
			proposedFacility.setTotalFacilityAmount(PennantAppUtil.amountFormate(facilityDetail.getTotalFacility(),facilityDetail.getTotalFacilityFormatter()));;
			proposedFacility.setTotalFacilityAmountUSD (CalculationUtil.getConvertedAmountASString(facilityDetail.getTotalFacilityCcy(), PennantConstants.CURRENCY_USD,facilityDetail.getTotalFacility()));;
		}
		
		if (facilityDetail.getUnderWriting()!=null && facilityDetail.getUnderWriting().compareTo(BigDecimal.ZERO)!=0) {
			proposedFacility.setUnderWritingAmount(PennantAppUtil.amountFormate(facilityDetail.getUnderWriting(),facilityDetail.getUnderWritingFormatter()));;
			proposedFacility.setUnderWritingAmountUSD (CalculationUtil.getConvertedAmountASString(facilityDetail.getUnderWritingCcy(), PennantConstants.CURRENCY_USD,facilityDetail.getUnderWriting()));;
		}
		
		if (facilityDetail.getPropFinalTake()!=null && facilityDetail.getPropFinalTake().compareTo(BigDecimal.ZERO)!=0) {
			proposedFacility.setPropFinalTakeAmount(PennantAppUtil.amountFormate(facilityDetail.getPropFinalTake(),facilityDetail.getPropFinalTakeFormatter()));;
			proposedFacility.setPropFinalTakeAmountUSD(CalculationUtil.getConvertedAmountASString(facilityDetail.getPropFinalTakeCcy(), PennantConstants.CURRENCY_USD,facilityDetail.getPropFinalTake()));;
		}
		
		proposedFacility.setExposure(CalculationUtil.getConvertedAmountASString(facilityDetail.getFacilityCCY(), PennantConstants.CURRENCY_USD, facilityDetail.getExposure()));
		proposedFacility.setLimitExisting(CalculationUtil.getConvertedAmountASString(facilityDetail.getFacilityCCY(), PennantConstants.CURRENCY_USD, facilityDetail.getExistingLimit()));
		proposedFacility.setLimitNew(CalculationUtil.getConvertedAmountASString(facilityDetail.getFacilityCCY(), PennantConstants.CURRENCY_USD, facilityDetail.getNewLimit()));
		
		int years = facilityDetail.getTenorYear();
		int months = facilityDetail.getTenorMonth();
		
		if (detail.getFacilityType().equals(PennantConstants.FACILITY_COMMERCIAL)) {
			proposedFacility.setTenor(String.valueOf(new BigDecimal(years + "." + months)));
		} else if (detail.getFacilityType().equals(PennantConstants.FACILITY_CORPORATE)) {
			if (years > 0 && months > 0) {
	...
	private void setFacilityFor(FacilityAgreementDetail agreement, FacilityDetail facilityDetail) {
		...
		if (facilityfor.equals(PennantConstants.FACILITY_NEW)) {
			agreement.setAvailmentNew("X");
		} else if (facilityfor.equals(PennantConstants.FACILITY_REVIEW)) {
			agreement.setAnnualReview("X");
		} else if (facilityfor.equals(PennantConstants.FACILITY_AMENDMENT)) {
	...
	private String getBookingUnit(Facility detail) {
		if (detail.getFacilityType().equals(PennantConstants.FACILITY_COMMERCIAL)) {
			return PennantConstants.FACILITY_BOOKING_COMM_UNIT;
		} else if (detail.getFacilityType().equals(PennantConstants.FACILITY_CORPORATE)) {
			return PennantConstants.FACILITY_BOOKING_CORP_UNIT;
		}
		return "";
	}
	...
	private FacilityAgreementDetail setBasicDetails(Facility detail, FacilityAgreementDetail agreement) {
		try {
			...
			agreement.setEstablishedDate(formatdDate(detail.getEstablishedDate()));
			...
			agreement.setDeadline(formatdDate(detail.getDeadLine()));
			agreement.setNextReviewDate(formatdDate(detail.getNextReviewDate()));
			...
			if (StringUtils.trimToEmpty(detail.getCustRelation()).equals(PennantConstants.CUSTRELATION_CONNECTED)) {
				agreement.setConnectedCustomer("Yes");
			}else{
				agreement.setConnectedCustomer("No");
			}
			if (StringUtils.trimToEmpty(detail.getCustRelation()).equals(PennantConstants.CUSTRELATION_RELATED)) {
				agreement.setRelatedCustomer("Yes");
			} else{
				agreement.setRelatedCustomer("No");
			}
			if (detail.getCustDOB() != null) {
				agreement.setRelationshipSince(formatdDate(detail.getCustDOB()));
			} else {
			...
			agreement.setInterim(formatdDate(detail.getInterim()));
	...
	private FacilityAgreementDetail setScoringDetails(FacilityAgreementDetail agreementDetail, Facility detail) {
		...
		try {
			...
			if (!creditWorth.equals("") && creditWorth.contains("-")) {
			...
			if (detailedMap.size() > 0) {
				...
				if (listCreditReviewSummaries1 != null && !listCreditReviewSummaries1.isEmpty()) {
					...
					for (int i = 0; i < listCreditReviewSummaries1.size(); i++) {
						...
						if (category.equals(PennantConstants.INTERFACE_CUSTCTG_CORP)) {
							if (!getCorpDisplayfiled().contains(finCreditReviewSummary.getSubCategoryCode())) {
								continue;
							}
						} else if (category.equals(PennantConstants.INTERFACE_CUSTCTG_BANK)) {
	...
	private String getCustCtg(String custCtgCode) {
		
		String custCtg = StringUtils.trimToEmpty(custCtgCode);
