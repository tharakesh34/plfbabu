 ...
 * /WEB-INF/pages/Collateral/FacilityDetail/FacilityDetailList.zul file.<br>
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
 * 
 */
public class FacilityDetailListCtrl extends GFCBaseListCtrl<FacilityDetail> implements Serializable {
	...
	public void onCreate$window_FacilityDetailList(ForwardEvent event) throws Exception {
		...
		try {
			...
			if (args.containsKey("control")) {
				this.setCtrlObject((Object) args.get("control"));
			}
			doCheckRights();
			try {
	...
	private void fillFacilityDetails(List<FacilityDetail> facilityDetails) {
		...
		for (FacilityDetail facilityDetail : facilityDetails) {
			...
			lc.setParent(item);
			lc = new Listcell(CalculationUtil.getConvertedAmountASString(facilityDetail.getFacilityCCY(), PennantConstants.CURRENCY_USD, facilityDetail.getExposure()));
			lc.setParent(item);
			lc = new Listcell(CalculationUtil.getConvertedAmountASString(facilityDetail.getFacilityCCY(), PennantConstants.CURRENCY_USD, facilityDetail.getExistingLimit()));
			lc.setParent(item);
			lc = new Listcell(CalculationUtil.getConvertedAmountASString(facilityDetail.getFacilityCCY(), PennantConstants.CURRENCY_USD, facilityDetail.getNewLimit()));
			lc.setParent(item);
	...
	private void prepareRenderer(List<FinanceSummary> existFinances) {
		...
		for (FinanceSummary financeSummary : existFinances) {
			...
			if (!StringUtils.trimToEmpty(financeSummary.getFinCommitmentRef()).equals("")) {
			...
			if (!StringUtils.trimToEmpty(financeSummary.getFinCommitmentRef()).equals("")) {
				totCmtAmount = totCmtAmount.add(CalculationUtil.getConvertedAmount(financeSummary.getFinCcy(),PennantConstants.CURRENCY_USD,financeSummary.getCmtAmount()));
				totUnUsed = totUnUsed.add(CalculationUtil.getConvertedAmount(financeSummary.getFinCcy(),PennantConstants.CURRENCY_USD,financeSummary.getCmtAvailable()));
				...
				unCmtTotOutstanding = unCmtTotOutstanding.add(CalculationUtil.getConvertedAmount(financeSummary.getFinCcy(), PennantConstants.CURRENCY_USD, financeSummary.getTotalOutStanding()));
		...
		cell.setParent(item);
		cell = new Listcell(formatdAmount(totCmtAmount, PennantConstants.CURRENCY_USD_FORMATTER));
		cell.setStyle("text-align:right;");
		cell.setParent(item);
		cell = new Listcell(formatdAmount(totUnUsed, PennantConstants.CURRENCY_USD_FORMATTER));
		cell.setStyle("text-align:right;");
		cell.setParent(item);
		cell = new Listcell(formatdAmount(cmtTotOutstanding.add(unCmtTotOutstanding), PennantConstants.CURRENCY_USD_FORMATTER));
		cell.setStyle("text-align:right;");
		cell.setParent(item);
		cell = new Listcell();
		cell.setParent(item);
	...
	private void doFillCommitmentGroups(Listitem item,Listcell cell,Listgroup group,HashMap<String, List<FinanceSummary>> commitmentMap ){
		for (String key : commitmentMap.keySet()) {
			...
			if (list != null && !list.isEmpty()) {
				...
				cell.setParent(group);
				cell = new Listcell(CalculationUtil.getConvertedAmountASString(finsum.getFinCcy(), PennantConstants.CURRENCY_USD, finsum.getCmtAmount()));
				cell.setStyle("text-align:right;");
				cell.setParent(group);
				cell = new Listcell(CalculationUtil.getConvertedAmountASString(finsum.getFinCcy(), PennantConstants.CURRENCY_USD, finsum.getCmtAvailable()));
				cell.setStyle("text-align:right;");
				cell.setParent(group);
				cell = new Listcell();
				cell.setStyle("text-align:right;");
				...
				for (FinanceSummary financeSummary : list) {
					...
					cell.setParent(item);
					cell = new Listcell(CalculationUtil.getConvertedAmountASString(financeSummary.getFinCcy(), PennantConstants.CURRENCY_USD, financeSummary.getTotalOriginal()));
					cell.setStyle("text-align:right;");
					cell.setParent(item);
					cell = new Listcell(CalculationUtil.getConvertedAmountASString(financeSummary.getFinCcy(), PennantConstants.CURRENCY_USD, financeSummary.getTotalPaid()));
					cell.setStyle("text-align:right;");
					cell.setParent(item);
					cell = new Listcell(CalculationUtil.getConvertedAmountASString(financeSummary.getFinCcy(), PennantConstants.CURRENCY_USD, financeSummary.getTotalOutStanding()));
					cell.setStyle("text-align:right;");
					cell.setParent(item);
					cell = new Listcell();
					cell.setParent(item);
					...
					cmtTotOutStanding = cmtTotOutStanding.add(CalculationUtil.getConvertedAmount(financeSummary.getFinCcy(),PennantConstants.CURRENCY_USD,financeSummary.getTotalOutStanding()));
				...
				lccmtTotOutStanding.setLabel(formatdAmount(cmtTotOutStanding, PennantConstants.CURRENCY_USD_FORMATTER));
			
				BigDecimal outStandPerc = BigDecimal.ZERO;
				if (cmtTotOutStanding != BigDecimal.ZERO && finsum.getCmtAmount() != BigDecimal.ZERO) {
	...
	private void doFillUnCommitedGroup(Listitem item,Listcell cell,Listgroup group,List<FinanceSummary> nonCommitmentList,BigDecimal unCmtTotOutstanding){
		...
		if (!nonCommitmentList.isEmpty()) {
			...
			cell.setParent(group);
			cell = new Listcell(formatdAmount(BigDecimal.ZERO, PennantConstants.CURRENCY_USD_FORMATTER));
			cell.setStyle("text-align:right;");
			cell.setParent(group);
			cell = new Listcell(formatdAmount(BigDecimal.ZERO, PennantConstants.CURRENCY_USD_FORMATTER));
			cell.setStyle("text-align:right;");
			cell.setParent(group);
			cell = new Listcell(formatdAmount(unCmtTotOutstanding, PennantConstants.CURRENCY_USD_FORMATTER));
			cell.setStyle("text-align:right;");
			cell.setParent(group);
			cell = new Listcell();
			cell.setParent(group);
	...
	private String[] getFacilityNewFilters() {
		...
		if (facilities != null && !facilities.isEmpty()) {
			for (FacilityDetail facilityDetail : facilities) {
				if (!StringUtils.trimToEmpty(facilityDetail.getTermSheetRef()).equals("")) {
		...
		if (getFacilityDetailList() != null && !getFacilityDetailList().isEmpty()) {
			for (FacilityDetail facilityDetail : getFacilityDetailList()) {
				if (!StringUtils.trimToEmpty(facilityDetail.getTermSheetRef()).equals("") && !strings.contains(facilityDetail.getTermSheetRef())) {
					strings.add(facilityDetail.getTermSheetRef());
				}
			}
		}
	...
	private String getBookingUnit(){
		...
		}else if (getFacility().getFacilityType().equals(PennantConstants.FACILITY_CORPORATE)) {
