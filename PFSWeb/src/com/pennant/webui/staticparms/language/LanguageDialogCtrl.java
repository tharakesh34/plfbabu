 ...
 * /WEB-INF/pages/StaticParms/Language/languageDialog.zul file. <br>
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
 */
public class LanguageDialogCtrl extends GFCBaseCtrl implements Serializable {
	...
	private transient String 	oldVar_lngCode;
	private transient String 	oldVar_lngDesc;
	private transient int 		oldVar_lngNumber;
	...
	private transient final String btnCtroller_ClassPrefix = "button_LanguageDialog_";
	...
	public void onCreate$window_LanguageDialog(Event event) throws Exception {
		...
		if (isWorkFlowEnabled()) {
			...
			getUserWorkspace().alocateRoleAuthorities(getRole(), "LanguageDialog");
		}
		...
		if (args.containsKey("languageListCtrl")) {
			setLanguageListCtrl((LanguageListCtrl) args.get("languageListCtrl"));
		} else {
	...
	private void doCheckRights() {
		...
		getUserWorkspace().alocateAuthorities("LanguageDialog");
	 ...
	 * @throws Exception
	 */
	public void onClose$window_LanguageDialog(Event event) throws Exception {
		logger.debug("Entering" + event.toString());
		doClose();
		logger.debug("Leaving" + event.toString());
	}
	...
	public void onClick$btnHelp(Event event) throws InterruptedException {
		logger.debug("Entering" + event.toString());
		PTMessageUtils.showHelpWindow(event, window_LanguageDialog);
		logger.debug("Leaving" + event.toString());
	}
	...
	private void doClose() throws InterruptedException {
		...
		if (close) {
			closeDialog(this.window_LanguageDialog, "Language");
		}
	 ...
	 * @throws InterruptedException
	 */
	public void doShowDialog(Language aLanguage) throws InterruptedException {
		logger.debug("Entering");

		// if aLanguage == null then we opened the Dialog without
		// arguments for a given entity, so we get a new Object().
		if (aLanguage == null) {
			/** !!! DO NOT BREAK THE TIERS !!! */
			// We don't create a new DomainObject() in the front end.
			// We GET it from the back end.
			aLanguage = getLanguageService().getNewLanguage();
		...
		try {
			...
			// during user action.
			doStoreInitValues();
			setDialog(this.window_LanguageDialog);
	...
	private void doStoreInitValues() {
		logger.debug("Entering");
		this.oldVar_lngCode = this.lngCode.getValue();
		this.oldVar_lngDesc = this.lngDesc.getValue();
		this.oldVar_lngNumber = this.lngNumber.intValue();
	...
	private void doResetInitValues() {
		...
		this.lngCode.setValue(this.oldVar_lngCode);
		this.lngDesc.setValue(this.oldVar_lngDesc);
		this.lngNumber.setValue(this.oldVar_lngNumber);
	...
	private boolean isDataChanged() {
		...
		doClearMessage();
		if (this.oldVar_lngCode != this.lngCode.getValue()) {
			return true;
		}
		if (this.oldVar_lngDesc != this.lngDesc.getValue()) {
			return true;
		}
		if (this.oldVar_lngNumber != this.lngNumber.intValue()) {
			return true;
		}
		return false;
	}
	...
	private void doSetValidation() {
		...
		if (!this.lngCode.isReadonly()){
			this.lngCode.setConstraint(new SimpleConstraint(PennantConstants.ALPHANUM_CAPS_REGEX, Labels.getLabel(
					"FIELD_ALNUM_CAPS",new String[]{Labels.getLabel(
					"label_LanguageDialog_LngCode.value")})));
		}

		if (!this.lngDesc.isReadonly()){
			this.lngDesc.setConstraint(new SimpleConstraint(PennantConstants.DESC_REGEX, Labels.getLabel(
					"MAND_FIELD_DESC",new String[]{Labels.getLabel(
					"label_LanguageDialog_LngDesc.value")})));
		}

		if (!this.lngNumber.isReadonly()) {
			this.lngNumber.setConstraint(new IntValidator(10, Labels.getLabel(
			"label_LanguageDialog_LngNumber.value")));
		}
		logger.debug("Leaving");
	}
	...
	private void doDelete() throws InterruptedException {
		...
		if (conf == MultiLineMessageBox.YES) {
			...
			if (StringUtils.trimToEmpty(aLanguage.getRecordType()).equals("")) {
	...
	/**
	 * Create a new Language object. <br>
	 */
	private void doNew() {
		logger.debug("Entering");
		...
		doStoreInitValues();
		/** !!! DO NOT BREAK THE TIERS !!! */
		// we don't create a new Language() in the front end.
		// we get it from the back end.
	...
	private boolean doProcess(Language aLanguage, String tranType) {
		...
		aLanguage.setLastMntBy(getUserWorkspace().getLoginUserDetails().getLoginUsrID());
		aLanguage.setLastMntOn(new Timestamp(System.currentTimeMillis()));
		aLanguage.setUserDetails(getUserWorkspace().getLoginUserDetails());
		...
		if (isWorkFlowEnabled()) {
			...
			if ("Save".equals(userAction.getSelectedItem().getLabel())) {
				...
				if ("".equals(nextTaskId)) {
					nextTaskId = getWorkFlow().getNextTaskIds(taskId, aLanguage);
				}

				if (PennantConstants.WF_Audit_Notes.equals(getWorkFlow().getAuditingReq(taskId, aLanguage))) {
					try {
						if (!isNotes_Entered()) {
			...
			String operationRefs = getWorkFlow().getOperationRefs(taskId, aLanguage);
	...
	private void refreshList() {
		logger.debug("Entering");
		final JdbcSearchObject<Language> soLanguage = getLanguageListCtrl().getSearchObj();
		getLanguageListCtrl().pagingLanguageList.setActivePage(0);
		getLanguageListCtrl().getPagedListWrapper().setSearchObject(soLanguage);
		if (getLanguageListCtrl().listBoxLanguage != null) {
			getLanguageListCtrl().listBoxLanguage.getListModel();
		}
		logger.debug("Leaving");
	}
	...
	private Notes getNotes() {
		...
		notes.setModuleName("Language");
		notes.setReference(getLanguage().getLngCode());
		notes.setVersion(getLanguage().getVersion());
		logger.debug("Leaving");
		return notes;
	}
