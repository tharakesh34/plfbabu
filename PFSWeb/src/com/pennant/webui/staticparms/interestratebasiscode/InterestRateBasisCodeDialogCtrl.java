 ...
 ********************************************************************************************
 *																							*
 * FileName    		:  InterestRateBasisCodeDialogCtrl.java                                                   * 	  
 *                                                                    						*
 ...
 * /WEB-INF/pages/StaticParms/InterestRateBasisCode
 * /interestRateBasisCodeDialog.zul file. <br>
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
 */
public class InterestRateBasisCodeDialogCtrl extends GFCBaseCtrl implements Serializable {

	private static final long serialVersionUID = 6369726125407866076L;
	private final static Logger logger = Logger.getLogger(InterestRateBasisCodeDialogCtrl.class);
	...
	private transient String 	oldVar_intRateBasisCode;
	private transient String 	oldVar_intRateBasisDesc;
	private transient boolean 	oldVar_intRateBasisIsActive;
	...
	private transient final String btnCtroller_ClassPrefix = "button_InterestRateBasisCodeDialog_";
	...
	public void onCreate$window_InterestRateBasisCodeDialog(Event event)throws Exception {
		...
		if (args.containsKey("interestRateBasisCode")) {
			this.interestRateBasisCode = (InterestRateBasisCode) args.get("interestRateBasisCode");
		...
		if (isWorkFlowEnabled()) {
			...
			getUserWorkspace().alocateRoleAuthorities(getRole(),"InterestRateBasisCodeDialog");
		}

		// READ OVERHANDED parameters !
		// we get the interestRateBasisCodeListWindow controller. So we have access
		// to it and can synchronize the shown data when we do insert, edit or
		// delete interestRateBasisCode here.
		if (args.containsKey("interestRateBasisCodeListCtrl")) {
			setInterestRateBasisCodeListCtrl((InterestRateBasisCodeListCtrl) args.get("interestRateBasisCodeListCtrl"));
		} else {
	...
	private void doCheckRights() {
		...
		getUserWorkspace().alocateAuthorities("InterestRateBasisCodeDialog");
	 ...
	 * @throws Exception
	 */
	public void onClose$window_InterestRateBasisCodeDialog(Event event)throws Exception {
		logger.debug("Entering" + event.toString());
		doClose();
		logger.debug("Leaving" + event.toString());
	}
	...
	public void onClick$btnHelp(Event event) throws InterruptedException {
		logger.debug("Entering" + event.toString());
		PTMessageUtils
		.showHelpWindow(event, window_InterestRateBasisCodeDialog);
		logger.debug("Leaving" + event.toString());
	}
	...
	private void doClose() throws InterruptedException {
		...
		if (close) {
			closeDialog(this.window_InterestRateBasisCodeDialog,"InterestRateBasisCode");
		}
	...
	public void doWriteBeanToComponents(InterestRateBasisCode aInterestRateBasisCode) {
		...
		if(aInterestRateBasisCode.isNew() || aInterestRateBasisCode.getRecordType().equals(PennantConstants.RECORD_TYPE_NEW)){
	 ...
	 * @throws InterruptedException
	 */
	public void doShowDialog(InterestRateBasisCode aInterestRateBasisCode)throws InterruptedException {
		logger.debug("Entering");

		// if aInterestRateBasisCode == null then we opened the Dialog without
		// arguments for a given entity, so we get a new Object().
		if (aInterestRateBasisCode == null) {
			/** !!! DO NOT BREAK THE TIERS !!! */
			// We don't create a new DomainObject() in the front end.
			// We GET it from the back end.
			aInterestRateBasisCode = getInterestRateBasisCodeService().getNewInterestRateBasisCode();
			setInterestRateBasisCode(aInterestRateBasisCode);
		} else {
			setInterestRateBasisCode(aInterestRateBasisCode);
		}

		// set Read only mode accordingly if the object is new or not.
		if (aInterestRateBasisCode.isNew()) {
		...
		try {
			...
			// during user action.
			doStoreInitValues();
			setDialog(this.window_InterestRateBasisCodeDialog);
	...
	private void doStoreInitValues() {
		logger.debug("Entering");
		this.oldVar_intRateBasisCode = this.intRateBasisCode.getValue();
		this.oldVar_intRateBasisDesc = this.intRateBasisDesc.getValue();
		this.oldVar_intRateBasisIsActive = this.intRateBasisIsActive.isChecked();
	...
	private void doResetInitValues() {
		logger.debug("Entering");
		this.intRateBasisCode.setValue(this.oldVar_intRateBasisCode);
		this.intRateBasisDesc.setValue(this.oldVar_intRateBasisDesc);
		this.intRateBasisIsActive.setChecked(this.oldVar_intRateBasisIsActive);
	...
	private boolean isDataChanged() {
		...
		if(this.oldVar_intRateBasisCode != this.intRateBasisCode.getValue()){
			return true;
		}
		if (this.oldVar_intRateBasisDesc != this.intRateBasisDesc.getValue()) {
			return true;
		}
		if (this.oldVar_intRateBasisIsActive != this.intRateBasisIsActive.isChecked()) {
			return true;
		}
		return false;
	}
	...
	/**
	 * Deletes a InterestRateBasisCode object from database.<br>
	 * 
	 * @throws InterruptedException
	 */
	private void doDelete() throws InterruptedException {
		...
		if (conf == MultiLineMessageBox.YES) {
			...
			if (StringUtils.trimToEmpty(aInterestRateBasisCode.getRecordType()).equals("")) {
				aInterestRateBasisCode.setVersion(aInterestRateBasisCode.getVersion() + 1);
				aInterestRateBasisCode.setRecordType(PennantConstants.RECORD_TYPE_DEL);
	...
	/**
	 * Create a new InterestRateBasisCode object. <br>
	 */
	private void doNew() {
		logger.debug("Entering");
		// remember the old variables
		doStoreInitValues();
		/** !!! DO NOT BREAK THE TIERS !!! */
		// we don't create a new InterestRateBasisCode() in the front end.
		// we get it from the back end.
	...
	public void doSave() throws InterruptedException {
		...
		isNew = aInterestRateBasisCode.isNew();
		String tranType = "";
	 ...
	 * @return
	 */
	private boolean doProcess(InterestRateBasisCode aInterestRateBasisCode,String tranType) {
		logger.debug("Entering");
		...
		aInterestRateBasisCode.setLastMntBy(getUserWorkspace().getLoginUserDetails().getLoginUsrID());
		aInterestRateBasisCode.setLastMntOn(new Timestamp(System.currentTimeMillis()));
		aInterestRateBasisCode.setUserDetails(getUserWorkspace().getLoginUserDetails());
		...
		if (isWorkFlowEnabled()) {
			...
			if ("Save".equals(userAction.getSelectedItem().getLabel())) {
				...
				if ("".equals(nextTaskId)) {
					nextTaskId = getWorkFlow().getNextTaskIds(taskId,aInterestRateBasisCode);
				}

				if (PennantConstants.WF_Audit_Notes.equals(getWorkFlow().getAuditingReq(taskId, aInterestRateBasisCode))) {
					try {
						if (!isNotes_Entered()) {
			...
			String operationRefs = getWorkFlow().getOperationRefs(taskId,aInterestRateBasisCode);
	...
	private boolean doSaveProcess(AuditHeader auditHeader, String method) {
		...
		InterestRateBasisCode aInterestRateBasisCode = (InterestRateBasisCode) auditHeader
	...
	private AuditHeader getAuditHeader(InterestRateBasisCode aInterestRateBasisCode, String tranType) {
		...
		AuditDetail auditDetail = new AuditDetail(tranType, 1,
				aInterestRateBasisCode.getBefImage(), aInterestRateBasisCode);
		return new AuditHeader(String.valueOf(aInterestRateBasisCode.getId()),
	...
	private void showMessage(Exception e) {
		...
		try {
			...
			ErrorControl.showErrorControl(this.window_InterestRateBasisCodeDialog, auditHeader);
		} catch (Exception exp) {
			logger.error(exp);
		}
		logger.debug("Leaving");
	...
	private void refreshList() {
		logger.debug("Entering");
		final JdbcSearchObject<InterestRateBasisCode> soInterestRateBasisCode = 
			getInterestRateBasisCodeListCtrl().getSearchObj();
		getInterestRateBasisCodeListCtrl().pagingInterestRateBasisCodeList.setActivePage(0);
		getInterestRateBasisCodeListCtrl().getPagedListWrapper().setSearchObject(soInterestRateBasisCode);
		if (getInterestRateBasisCodeListCtrl().listBoxInterestRateBasisCode != null) {
			getInterestRateBasisCodeListCtrl().listBoxInterestRateBasisCode.getListModel();
		}
		logger.debug("Leaving");
	}
	...
	private Notes getNotes() {
		...
		notes.setModuleName("InterestRateBasisCode");
		notes.setReference(getInterestRateBasisCode().getIntRateBasisCode());
		notes.setVersion(getInterestRateBasisCode().getVersion());
		logger.debug("Leaving");
		return notes;
	}
	...
	public void setInterestRateBasisCode(
			InterestRateBasisCode interestRateBasisCode) {
