 ...
 * /WEB-INF/pages/Accounts/Accounts/AccountsDialog.zul file. <br>
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
 */
public class AccountsDialogCtrl extends GFCBaseCtrl implements Serializable {
	...
	private transient String  		    oldVar_accountId;
	private transient String  		    oldVar_acCcy;
	private transient String  		    oldVar_acType;
	private transient String  		    oldVar_acBranch;
	private transient long  		    oldVar_acCustId;
	private transient String  		    oldVar_acFullName;
	private transient String  		    oldVar_acShortName;
	...
	private transient BigDecimal  		oldVar_acPrvDayBal;
	private transient BigDecimal  		oldVar_acTodayDr;
	private transient BigDecimal  		oldVar_acTodayCr;
	private transient BigDecimal  		oldVar_acTodayNet;
	private transient BigDecimal  		oldVar_acAccrualBal;
	private transient BigDecimal  		oldVar_acTodayBal;
	private transient Timestamp  		oldVar_acOpenDate;
	private transient Timestamp  		oldVar_acLastCustTrnDate;
	private transient Timestamp  		oldVar_acLastSysTrnDate;
	private transient boolean  		    oldVar_acInactive;
	private transient boolean  		    oldVar_acBlocked;
	private transient boolean  		    oldVar_acClosed;
	private transient String  		    oldVar_hostAcNumber;
	...
	private transient final String btnCtroller_ClassPrefix = "button_AccountsDialog_";
	...
	public void onCreate$window_AccountsDialog(Event event) throws Exception {
		...
		if (isWorkFlowEnabled()){
			...
			getUserWorkspace().alocateRoleAuthorities(getRole(), "AccountsDialog");
		}
	...
	private void doSetFieldProperties() {
		...
		if(!this.accounts.isInternalAc()){
			this.acSeqNumber.setMaxlength(PennantConstants.accountLength-
					(PennantConstants.branchLength+PennantConstants.headCodeLength));
			...
			this.acSeqNumber.setMaxlength(PennantConstants.accountLength-
					(PennantConstants.branchLength+PennantConstants.headCodeLength+PennantConstants.cCyLength));
		...
		if(this.accounts.getRecordType()!=null){
			if (!this.accounts.getRecordType().equals(PennantConstants.RECORD_TYPE_UPD)) {
				if (this.accounts.isNewRecord() || !this.accounts.getRecordType().equals("")) {
		...
		this.acLastCustTrnDate.setFormat(PennantConstants.dateFormat);
		this.acLastSysTrnDate.setFormat(PennantConstants.dateFormat);
	...
	private void doCheckRights() {
		...
		getUserWorkspace().alocateAuthorities("AccountsDialog");
	 ...
	 * @throws Exception
	 */
	public void onClose$window_AccountsDialog(Event event) throws Exception {
		logger.debug("Entering " + event.toString());
		doClose();
		logger.debug("Leaving " + event.toString());
	}
	...
	public void onClick$btnHelp(Event event) throws InterruptedException {
		logger.debug("Entering " + event.toString());
		PTMessageUtils.showHelpWindow(event, window_AccountsDialog);
		logger.debug("Leaving " + event.toString());
	}
	...
	private void doClose() throws InterruptedException {
		...
		if(close){
			closeDialog(this.window_AccountsDialog, "Accounts");	
		}
			...
			if(!this.accounts.isInternalAc() && this.accounts.isNew()){
				if(StringUtils.trimToEmpty(this.acSeqNumber.getValue()).equals("")){
					...
					aAccounts.setAccountId(this.branchCode.getValue()+this.acHead.getValue()
							+StringUtils.leftPad(this.acSeqNumber.getValue().trim()
									,PennantConstants.accountLength 
									-(PennantConstants.branchLength+PennantConstants.headCodeLength),'0'));	
				}
				...
				aAccounts.setAccountId(this.branchCode.getValue()+this.acHead.getValue()
						+StringUtils.leftPad(this.acSeqNumber.getValue().trim()
								,PennantConstants.accountLength 
								- (PennantConstants.branchLength+PennantConstants.headCodeLength
										+PennantConstants.cCyLength),'0')+this.ccyNumber.getValue());
	 ...
	 * @throws InterruptedException
	 */
	public void doShowDialog(Accounts aAccounts) throws InterruptedException {
		logger.debug("Entering") ;

		// if aAccounts == null then we opened the Dialog without
		// arguments for a given entity, so we get a new Object().
		if (aAccounts == null) {
			/** !!! DO NOT BREAK THE TIERS !!! */
			// We don't create a new DomainObject() in the front end.
			// We GET it from the back end.
			aAccounts = getAccountsService().getNewAccounts();

			setAcounts(aAccounts);
		} else {
			setAcounts(aAccounts);
		}
		...
		try {
			...
			// during user action.
			doStoreInitValues();
			setDialog(this.window_AccountsDialog);
	...
	private void doStoreInitValues() {
		logger.debug("Entering");
		this.oldVar_accountId = this.accountId.getValue();
		this.oldVar_acCcy = this.acCcy.getValue();
		this.oldVar_acType = this.acType.getValue();
		this.oldVar_acBranch = this.acBranch.getValue();
		this.oldVar_acCustId = this.acCustId.longValue();
		this.oldVar_acFullName = this.acFullName.getValue();
		this.oldVar_acShortName = this.acShortName.getValue();
		...
		this.oldVar_acPrvDayBal = this.acPrvDayBal.getValue();
		this.oldVar_acTodayDr = this.acTodayDr.getValue();
		this.oldVar_acTodayCr = this.acTodayCr.getValue();
		this.oldVar_acTodayNet = this.acTodayNet.getValue();
		this.oldVar_acAccrualBal = this.acAccrualBal.getValue();
		this.oldVar_acTodayBal = this.acTodayBal.getValue();
		this.oldVar_acOpenDate = PennantAppUtil.getTimestamp(this.acOpenDate.getValue());	
		this.oldVar_acLastCustTrnDate = PennantAppUtil.getTimestamp(this.acLastCustTrnDate.getValue());	
		this.oldVar_acLastSysTrnDate = PennantAppUtil.getTimestamp(this.acLastSysTrnDate.getValue());	
		this.oldVar_acInactive = this.acActive.isChecked();
		this.oldVar_acBlocked = this.acBlocked.isChecked();
		this.oldVar_acClosed = this.acClosed.isChecked();
		this.oldVar_hostAcNumber = this.hostAcNumber.getValue();
	...
	private void doResetInitValues() {
		logger.debug("Entering");
		this.accountId.setValue(this.oldVar_accountId);
		this.acCcy.setValue(this.oldVar_acCcy);
		this.acType.setValue(this.oldVar_acType);
		this.acBranch.setValue(this.oldVar_acBranch);
		this.acCustId.setValue(this.oldVar_acCustId);
		this.acFullName.setValue(this.oldVar_acFullName);
		this.acShortName.setValue(this.oldVar_acShortName);
		...
		this.acPrvDayBal.setValue(this.oldVar_acPrvDayBal);
		this.acTodayDr.setValue(this.oldVar_acTodayDr);
		this.acTodayCr.setValue(this.oldVar_acTodayCr);
		this.acTodayNet.setValue(this.oldVar_acTodayNet);
		this.acAccrualBal.setValue(this.oldVar_acAccrualBal);
		this.acTodayBal.setValue(this.oldVar_acTodayBal);
		this.acOpenDate.setValue(this.oldVar_acOpenDate);
		this.acLastCustTrnDate.setValue(this.oldVar_acLastCustTrnDate);
		this.acLastSysTrnDate.setValue(this.oldVar_acLastSysTrnDate);
		this.acActive.setChecked(this.oldVar_acInactive);
		this.acBlocked.setChecked(this.oldVar_acBlocked);
		this.acClosed.setChecked(this.oldVar_acClosed);
		this.hostAcNumber.setValue(this.oldVar_hostAcNumber);
	...
	private void doSetValidation() {
		...
		if (!this.acPrvDayBal.isReadonly()){
			this.acPrvDayBal.setConstraint(
					new AmountValidator(18,0,Labels.getLabel("label_AcountsDialog_AcPrvDayBal.value")));
		}	
		if (!this.acTodayDr.isReadonly()){
			this.acTodayDr.setConstraint(
					new AmountValidator(18,0,Labels.getLabel("label_AcountsDialog_AcTodayDr.value")));
		}	
		if (!this.acTodayCr.isReadonly()){
			this.acTodayCr.setConstraint(
					new AmountValidator(18,0,Labels.getLabel("label_AcountsDialog_AcTodayCr.value")));
		}	
		if (!this.acTodayNet.isReadonly()){
			this.acTodayNet.setConstraint(
					new AmountValidator(18,0,Labels.getLabel("label_AcountsDialog_AcTodayNet.value")));
		}	
		if (!this.acAccrualBal.isReadonly()){
			this.acAccrualBal.setConstraint(
					new AmountValidator(18,0,Labels.getLabel("label_AcountsDialog_AcAccrualBal.value")));
		}	
	...
	private void showMessage(Exception e){
		...
		try {
			...
			ErrorControl.showErrorControl(this.window_AccountsDialog, auditHeader);
		} catch (Exception exp) {
			logger.error(exp);
		}
		logger.debug("Leaving ");
	...
	private void doDelete() throws InterruptedException {
		...
		if (conf==MultiLineMessageBox.YES){
			...
			if (StringUtils.trimToEmpty(aAccounts.getRecordType()).equals("")){
	...
	/**
	 * Create a new Accounts object. <br>
	 */
	private void doNew() {
		logger.debug("Entering");
	...
	private boolean doProcess(Accounts aAccounts,String tranType){
		...
		aAccounts.setLastMntBy(getUserWorkspace().getLoginUserDetails().getLoginUsrID());
		aAccounts.setLastMntOn(new Timestamp(System.currentTimeMillis()));
		aAccounts.setUserDetails(getUserWorkspace().getLoginUserDetails());

		if (isWorkFlowEnabled()) {
			String taskId = getWorkFlow().getTaskId(getRole());
			String nextTaskId = "";
			//Upgraded to ZK-6.5.1.1 Added to String
			...
			if ("Save".equals(userAction.getSelectedItem().getLabel())) {
				...
				if ("".equals(nextTaskId)) {
					nextTaskId = getWorkFlow().getNextTaskIds(taskId, aAccounts);
				}

				if (PennantConstants.WF_Audit_Notes.equals(getWorkFlow().getAuditingReq(taskId,aAccounts))) {
					try {
						if (!isNotes_Entered()){
			...
			String operationRefs = getWorkFlow().getOperationRefs(taskId,aAccounts);
	...
	private Notes getNotes(){
		...
		notes.setModuleName("Accounts");
		notes.setReference(getAcounts().getAccountId());
		notes.setVersion(getAcounts().getVersion());
		return notes;
	}
