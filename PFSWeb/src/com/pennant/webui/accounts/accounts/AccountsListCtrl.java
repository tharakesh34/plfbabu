...
/**
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
 * This is the controller class for the /WEB-INF/pages/Accounts/Accounts/AcountsList.zul
 * file.<br>
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
 * 
 */
public class AccountsListCtrl extends GFCBaseListCtrl<Accounts> implements Serializable {
	...
	private final static Logger logger = Logger.getLogger(AccountsListCtrl.class);
	...
	public void onCreate$window_AccountsList(Event event) throws Exception {
		...
		ModuleMapping moduleMapping = PennantJavaUtil.getModuleMap("Accounts");
		boolean wfAvailable=true;

		if (moduleMapping.getWorkflowType()!=null){
			workFlowDetails = WorkFlowUtil.getWorkFlowDetails("Accounts");
		...
		if(StringUtils.trimToEmpty(recordType.getValue()).equals("NEW")){
		...
		if (!isWorkFlowEnabled() && wfAvailable){
			this.button_AccountsList_NewAccounts.setVisible(false);
			this.button_AccountsList_AccountsSearchDialog.setVisible(false);
			this.button_AccountsList_PrintList.setVisible(false);
	...
	private void doCheckRights() {
		...
		getUserWorkspace().alocateAuthorities("AccountsList");
			...
			if(acounts==null){
				...
				if(isWorkFlowEnabled()){
					...
					boolean userAcces =  validateUserAccess(workFlowDetails.getId()
							,getUserWorkspace().getLoginUserDetails().getLoginUsrID(), "Accounts"
							, whereCond, acounts.getTaskId(), acounts.getNextTaskId());
					if (userAcces){
						showDetailView(acounts);
					}else{
	 ...
	 * 
	 * @param Accounts (aAccounts)
	 * @throws Exception
	 */
	private void showDetailView(Accounts aAccounts) throws Exception {
		logger.debug("Entering");
		/*
		...
		if(aAccounts.getWorkflowId()==0 && isWorkFlowEnabled()){
			aAccounts.setWorkflowId(workFlowDetails.getWorkFlowId());
		}

		final HashMap<String, Object> map = new HashMap<String, Object>();
		map.put("acounts", aAccounts);
		/*
		 * we can additionally handed over the listBox or the controller self,
		 * so we have in the dialog access to the list box List model. This is
		 * fine for synchronizing the data in the AcountsListbox from the
		 * dialog when we do a delete, edit or insert a Accounts.
		 */
		map.put("acountsListCtrl", this);

		// call the ZUL-file with the parameters packed in a map
		try {
			if(aAccounts.isNew()){
				Executions.createComponents("/WEB-INF/pages/Account/Accounts/SelectAccountDetailsDialog.zul",null,map);
			}else{
	...
	public void onClick$btnHelp(Event event) throws InterruptedException {
		logger.debug("Entering " + event.toString());
		PTMessageUtils.showHelpWindow(event, window_AccountsList);
		logger.debug("Leaving " + event.toString());
	}
