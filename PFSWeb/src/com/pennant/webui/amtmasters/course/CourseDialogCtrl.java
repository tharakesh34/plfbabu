 ...
 * /WEB-INF/pages/AMTMaster/Course/courseDialog.zul file. <br>
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
 */
public class CourseDialogCtrl extends GFCBaseCtrl implements Serializable {
	...
	private Course         prvCourse;                  // overhanded per param
	private transient CourseListCtrl courseListCtrl;   // overhanded per param

	// old value vars for edit mode. that we can check if something
	// on the values are edited since the last init.
	private transient String  		oldVar_courseName;
	private transient String  		oldVar_courseDesc;
	private transient String        oldVar_recordStatus;
	private transient boolean       validationOn;
	private boolean                 notes_Entered=false;

	// Button controller for the CRUD buttons
	private transient final String     btnCtroller_ClassPrefix = "button_CourseDialog_";
	...
	public void onCreate$window_CourseDialog(Event event) throws Exception {
		...
		if (args.containsKey("course")) {
			this.course = (Course) args.get("course");
		...
		doLoadWorkFlow(this.course.isWorkflow(),this.course.getWorkflowId(),this.course.getNextTaskId());

		if (isWorkFlowEnabled()){
			this.userAction	= setListRecordStatus(this.userAction);
			getUserWorkspace().alocateRoleAuthorities(getRole(), "CourseDialog");
		}
		...
		// delete course here.
		if (args.containsKey("courseListCtrl")) {
			setCourseListCtrl((CourseListCtrl) args.get("courseListCtrl"));
		} else {
	...
	private void doCheckRights() {
		...
		getUserWorkspace().alocateAuthorities("CourseDialog");
	 ...
	 * @throws Exception
	 */
	public void onClose$window_CourseDialog(Event event) throws Exception {
		logger.debug(event.toString());
		doClose();
		logger.debug("Leaving");
	}
	...
	public void onClick$btnHelp(Event event) throws InterruptedException {
		logger.debug(event.toString());
		PTMessageUtils.showHelpWindow(event, window_CourseDialog);
		logger.debug("Leaving");
	}
	...
	private void doClose() throws InterruptedException {
		...
		if(close){
			closeDialog(this.window_CourseDialog, "Course");	
		}
	 ...
	 * @throws InterruptedException
	 */
	public void doShowDialog(Course aCourse) throws InterruptedException {
		logger.debug("Entering") ;

		// if aCourse == null then we opened the Dialog without
		// args for a given entity, so we get a new Obj().
		if (aCourse == null) {
			/** !!! DO NOT BREAK THE TIERS !!! */
			// We don't create a new DomainObject() in the frontend.
			// We GET it from the backend.
			aCourse = getCourseService().getNewCourse();
		...
		try {
			...
			// during user action.
			doStoreInitValues();
			setDialog(this.window_CourseDialog);
		} catch (final Exception e) {
			logger.error(e);
			PTMessageUtils.showErrorMessage(e.toString());
		}
	...
	private void doStoreInitValues() {
		logger.debug("Entering");
		this.oldVar_courseName = this.courseName.getValue();
		this.oldVar_courseDesc = this.courseDesc.getValue();
	...
	private void doResetInitValues() {
		logger.debug("Entering");
		this.courseName.setValue(this.oldVar_courseName);
		this.courseDesc.setValue(this.oldVar_courseDesc);
	...
	private void doDelete() throws InterruptedException {
		...
		if (conf==MultiLineMessageBox.YES){
			...
			if (StringUtils.trimToEmpty(aCourse.getRecordType()).equals("")){
	...
	private boolean doProcess(Course aCourse,String tranType){
		...
		aCourse.setLastMntBy(getUserWorkspace().getLoginUserDetails().getLoginUsrID());
		aCourse.setLastMntOn(new Timestamp(System.currentTimeMillis()));
		aCourse.setUserDetails(getUserWorkspace().getLoginUserDetails());
		...
		if (isWorkFlowEnabled()) {
			...
			if ("Save".equals(userAction.getSelectedItem().getLabel())) {
				...
				if ("".equals(nextTaskId)) {
					nextTaskId = getWorkFlow().getNextTaskIds(taskId, aCourse);
				}

				if (PennantConstants.WF_Audit_Notes.equals(getWorkFlow().getAuditingReq(taskId,aCourse))) {
					try {
						if (!isNotes_Entered()){
			...
			String operationRefs = getWorkFlow().getOperationRefs(taskId,aCourse);
	...
	private void refreshList(){
		final JdbcSearchObject<Course> soCourse = getCourseListCtrl().getSearchObj();
		getCourseListCtrl().pagingCourseList.setActivePage(0);
		getCourseListCtrl().getPagedListWrapper().setSearchObject(soCourse);
		if(getCourseListCtrl().listBoxCourse!=null){
			getCourseListCtrl().listBoxCourse.getListModel();
		}
	} 
	...
	private Notes getNotes(){
		...
		notes.setModuleName("Course");
		notes.setReference(getCourse().getCourseName());
		notes.setVersion(getCourse().getVersion());
		return notes;
	}
