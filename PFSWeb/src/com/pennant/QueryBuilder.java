...
public class QueryBuilder extends Groupbox {
	...
	private List<ValueLabel> 	 operandTypesList		=	PennantStaticListUtil.getOperandTypes(""); //retrieve all selection types
	...
	private Treeitem createNewCondition(String condition) {
		...
		String excludeFields = "," + PennantConstants.SUBQUERY + ","+PennantConstants.DBVALUE+","+PennantConstants.CALCVALUE+",";
		...
		addCondition.addForward("onClick", this,
				"onAddCondition", treeItem);
		...
		addSubCondition.addForward("onClick", this,
				"onAddSubCondition", treeItem);
		...
		if(buildCondition==true){
			...
			if(leftOperand instanceof Combobox){
				if(((Combobox) leftOperand).getSelectedIndex()!=0 && leftOperandType.getSelectedItem().getValue().equals(PennantConstants.FIELDLIST)){
					QBFieldDetail fielddetails=(QBFieldDetail) ((Combobox) leftOperand).getSelectedItem().getAttribute("FieldDetails");
	...
	public String getExcludeFieldsByOperandType(Combobox operandType, String type){
		...
		if(operandType.getSelectedIndex() == 0 ){
			...
			if(type.equals("operator")){
				...
				excludeFields = "," + PennantConstants.CALCVALUE + "," +PennantConstants.DBVALUE+","+ PennantConstants.SUBQUERY + ","+PennantConstants.STATICTEXT+",";
			}
			...
			if(type.equals("operandType")){
				excludeFields =  "," +PennantConstants.DBVALUE+"," + PennantConstants.SUBQUERY + ","+PennantConstants.CALCVALUE+",";
			}
		}else if(operandType.getSelectedItem().getValue().equals(PennantConstants.CALCVALUE)){
			if(type.equals("operator")){
				excludeFields = ", LIKE , NOT LIKE ,";
			}else if(type.equals("operandType")){
				excludeFields = "," + PennantConstants.STATICTEXT + ","+PennantConstants.DBVALUE+","+ PennantConstants.SUBQUERY + ",";
			}
		...
		if(operand instanceof Combobox) {
			if(operand.getId().endsWith("_leftOperand")){
				...
				if(operandtype.getSelectedItem().getValue().equals(PennantConstants.FIELDLIST)&& 
					...
					QBFieldDetail fielddetails=(QBFieldDetail) ((Combobox) operand).getSelectedItem().getAttribute("FieldDetails");
					String uUID = treeCell.getId();
					if(treeCell.getFellowIfAny(uUID+"_operator") != null ) { 
						Combobox comboBox = (Combobox) treeCell.getFellowIfAny(uUID+"_operator");
	...
	public String getGlobalExcludeFieldsByOperands(Combobox operand, String type,GlobalVariable globalVariable){
		...
		if((fieldType.equals(PennantConstants.DECIMAL))||(fieldType.equals(PennantConstants.BIGINT))||
			...
			if(type.equals("operator")){
				...
				excludeFields = "," + PennantConstants.CALCVALUE+ ","+PennantConstants.DBVALUE+"," + PennantConstants.SUBQUERY + ",";
			}
		}
	...
	public void onChangingOperator(Combobox operator){
		...
		if(treeCell.getFellowIfAny(uUID+"_operator") != null ){
			...
			if((selectedOperator.equals(Labels.getLabel("ISNULL_LABEL")))||(selectedOperator.equals(Labels.getLabel("ISNOTNULL_LABEL")))){
				rightOperandType.setVisible(false);
				if(rightOperand instanceof Textbox){
			...
			if((selectedOperator.equals(Labels.getLabel("EXISTS_LABEL")))||(selectedOperator.equals(Labels.getLabel("NOTEXISTS_LABEL")))||
					(selectedOperator.equals(Labels.getLabel("IN_LABEL")))||(selectedOperator.equals(Labels.getLabel("NOTIN_LABEL")))){
				String	excludeFields = getExcludeFieldsByRightOperandType((Combobox) operator, leftOperand,leftOperandType);
	...
	public String getExcludeFieldsByRightOperandType(Combobox operator,Component leftOperand,Combobox leftOperandType){
		...
		try{
			if(operator.getSelectedIndex() == 0 ){
				...
				excludeFields = "," + PennantConstants.CALCVALUE + "," +PennantConstants.DBVALUE+","+PennantConstants.STATICTEXT + ","
				...
				if(leftOperand instanceof Combobox){
					if(leftOperandType.getSelectedItem().getValue().equals(PennantConstants.FIELDLIST)){
						...
						if(fielddetails!=null){
							...
							if(!StringUtils.trimToEmpty(fielddetails.getModuleCode()).equals("")){
								...
								excludeFields = "," + PennantConstants.CALCVALUE + ","
										+PennantConstants.GLOBALVAR+","+PennantConstants.FIELDLIST+","+PennantConstants.DBVALUE+",";
							}
							if((fieldType.equals(PennantConstants.DECIMAL))||(fieldType.equals(PennantConstants.BIGINT))||
									(fieldType.equals(PennantConstants.NUMERIC))||(fieldType.equalsIgnoreCase(PennantConstants.DATETIME))||
									(fieldType.equalsIgnoreCase(PennantConstants.SMALLDATETIME))){
								excludeFields = "," + PennantConstants.CALCVALUE + ","
										+PennantConstants.GLOBALVAR+","+PennantConstants.FIELDLIST+","+PennantConstants.DBVALUE+","+PennantConstants.STATICTEXT + ",";
							}
						}
					}else if(leftOperandType.getSelectedItem().getValue().equals(PennantConstants.GLOBALVAR)){
						excludeFields =  "," + PennantConstants.CALCVALUE + ","
								+PennantConstants.DBVALUE+","+PennantConstants.GLOBALVAR+","+PennantConstants.FIELDLIST+",";
						GlobalVariable globalVariable=(GlobalVariable) ((Combobox) leftOperand).getSelectedItem().getAttribute("GlobalVariableDetails");
					...
					excludeFields =  "," + PennantConstants.CALCVALUE + ","
							+PennantConstants.DBVALUE+","+PennantConstants.GLOBALVAR+","+PennantConstants.FIELDLIST+","+ PennantConstants.STATICTEXT + ",";
				}
				...
				if(leftOperand instanceof Combobox){
							...
							if(fielddetails!=null){
								if(!StringUtils.trimToEmpty(fielddetails.getModuleCode()).equals("")){
									...
									if((((Combobox) leftOperand).getSelectedItem().getTooltiptext().toLowerCase().contains(PennantConstants.NCHAR))||
											...
											(((Combobox) leftOperand).getSelectedItem().getTooltiptext().toLowerCase().contains(PennantConstants.CHAR))){
										excludeFields =  ","+PennantConstants.DBVALUE+","+ PennantConstants.SUBQUERY + ","+PennantConstants.CALCVALUE + ",";
										...
										excludeFields = ","+PennantConstants.STATICTEXT + ","+PennantConstants.CALCVALUE + "," +PennantConstants.DBVALUE+"," +PennantConstants.SUBQUERY+",";
										...
										excludeFields = ","+PennantConstants.DBVALUE+","+ PennantConstants.SUBQUERY  + ","+PennantConstants.STATICTEXT + ",";
									}
								}
							}
						}
					...
					excludeFields = "," + PennantConstants.SUBQUERY + "," +PennantConstants.DBVALUE+","+","+PennantConstants.CALCVALUE + ","+PennantConstants.STATICTEXT + ",";
				}
				...
				if(leftOperand instanceof Combobox){
					if(((Combobox) leftOperand).getSelectedIndex()!=0){
						if((((Combobox) leftOperand).getSelectedItem().getTooltiptext().toLowerCase().contains(PennantConstants.DATETIME))||
								(((Combobox) leftOperand).getSelectedItem().getTooltiptext().toLowerCase().contains(PennantConstants.SMALLDATETIME))){
							excludeFields = ","+PennantConstants.DBVALUE+","+ PennantConstants.SUBQUERY  + ","+PennantConstants.STATICTEXT + ","+PennantConstants.CALCVALUE + ",";
						}else {
							excludeFields= "," + PennantConstants.SUBQUERY + "," +PennantConstants.DBVALUE+","+PennantConstants.STATICTEXT + ",";
						}
					} 
				}else {
					excludeFields= "," + PennantConstants.SUBQUERY + "," +PennantConstants.DBVALUE+","+PennantConstants.STATICTEXT + ",";
			...
			if(operand.getId().endsWith("_leftOperand")){
				if(objectFieldList!=null && objectFieldList .size()>0){
					for (int i = 0; i < objectFieldList.size(); i++) {
						...
						comboitem = new Comboitem();
						comboitem.setLabel(fieldDetails.getQbFldName() + "("
								+ fieldDetails.getQbFldLen() + ")");
				...
				if(treeCell.getFellowIfAny(treeCell.getId()+"_leftOperand") instanceof Combobox){
					...
					if(objectFieldList!=null && objectFieldList .size()>0){
						for (int i = 0; i < objectFieldList.size(); i++) {
							...
							comboitem = new Comboitem();
							if(leftOperand.getSelectedIndex()>0){
								if(leftOperand.getSelectedItem().getTooltiptext().contains(fieldDetails.getQbFldType().toUpperCase())){
									comboitem.setLabel(fieldDetails.getQbFldName() + "("
											+ fieldDetails.getQbFldLen() + ")");
									comboitem.setValue(fieldDetails.getQbFldName());
									comboitem.setTooltiptext("Data Type : "+fieldDetails.getQbFldType().toUpperCase());
	...
	public void onSearchButtonClick(ForwardEvent event) throws SecurityException, IllegalArgumentException, NoSuchMethodException, IllegalAccessException, InvocationTargetException{
		...
		if((operator.getSelectedItem().getLabel().equals(Labels.getLabel("IN_LABEL")))||
			...
			if(lovText.getValue().trim()!=""){

				selectedValues.put(lovText.getValue().trim().replace("," , ""), lovText.getValue().trim().replace("," ,""));
			}
		...
		if(component instanceof Combobox ){
			...
			if(((Textbox) component).getValue().contains(",")){
				if(!((Textbox) component).isReadonly()){
					String staticvalue=((Textbox)component).getValue().toString();
					String[] values=staticvalue.split(",");
					String newValue=StringUtils.join(values,"'");
					return "("+"'"+newValue.replace("'","','")+"'"+")";
				}else{
					String staticvalue=((Textbox)component).getValue().trim().toString();
					String[] values=staticvalue.split(",");
					String newValue=StringUtils.join(values,"'");
	...
	public boolean validateQuery() throws InterruptedException{
		...
		try{
			...
			PTMessageUtils.showErrorMessage(e.getLocalizedMessage());
			return false;
		}
	}
	...
	private void createResultWindow() {
		...
		for (String string : str) {	
			if (string.contains(":A") && !string.contains(":AC") ) {							  //TODO FIXME
				listHeader = new Listheader(string.substring(0,string.indexOf(":")) ); //TODO FIXME
		...
		PagedListWrapper<List> pagedListWrapper = new PagedListWrapper();
		JdbcSearchObject searchObject = new JdbcSearchObject();
		searchObject.addTabelName(queryModule.getTableName());
		...
		pagedListWrapper.init(searchObject,listBox,paging);
			...
			for (int i = 0; i < str.length; i++) {
				...
				if (index > 0 ) {
					...
					if (type.equalsIgnoreCase("A")) {
						...
						if(StringUtils.trimToEmpty(data).equals("") || data.equals("0")){
							listCell = new Listcell("-- NO VALUE --");
							listCell.setParent(item);
						}else{
							listCell = new Listcell(DateUtility.formatUtilDate(DateUtility.getUtilDate(data, PennantConstants.dateFormate), PennantConstants.dateFormate));
							listCell.setParent(item);
						}
						...
						if(StringUtils.trimToEmpty(data).equals("") || data.equals("0")){
							...
							listCell = new Listcell(DateUtility.formatUtilDate(DateUtility.getformatCDate(data), PennantConstants.dateFormate));
							listCell.setParent(item);
						}
					} else {
						listCell = new Listcell(data);
