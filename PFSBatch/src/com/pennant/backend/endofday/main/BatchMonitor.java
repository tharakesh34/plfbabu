	...
	private static long getJobExecutionId() {
		...
		if (jobExecutionId == 0) {
			try {
				...
				resultSet = statement.executeQuery("SELECT ISNULL(MAX(JOB_EXECUTION_ID), 0) JOBID FROM BATCH_JOB_EXECUTION");
				if (resultSet.next()) {
	...
	private static long getAvgTime() {
		...
		if(avgTime == 0) {
			try {
				...
				query.append(" SELECT  TOP 30 Avg(DATEDIFF (millisecond, START_TIME, END_TIME)) as avg FROM");
