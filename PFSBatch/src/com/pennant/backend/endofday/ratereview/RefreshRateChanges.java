 ...
 ********************************************************************************************
 *
 * FileName    		:  RefreshRateChanges.java													*                           
 *                                                                    
...
public class RefreshRateChanges  implements Tasklet {

	private Logger logger = Logger.getLogger(RefreshRateChanges.class);
	...
	public RepeatStatus execute(StepContribution arg0, ChunkContext context) throws Exception {
		...
		logger.debug("START: Refresh Rate Changes for Value Date: " + DateUtility.addDays(dateValueDate,-1));
		...
		logger.debug("COMPLETE: Refresh Rate Changes for Value Date: "+ DateUtility.addDays(dateValueDate,-1));
		return RepeatStatus.FINISHED;

	}
	
	/**
	 * Method for get count of rate change finances data
	 * @return selQuery 
	 */
	private String getCountQuery() {
		...
		selQuery.append(" AND br.LastMdfDate = ? " );
		...
		selQuery.append(" AND sr.LastMdfDate = ? " ); 
		selQuery.append(" UNION " );
		selQuery.append(" SELECT count(fm.FinReference) " );
		selQuery.append(" FROM FinanceMain AS fm, RMTBaseRates br " );
		selQuery.append(" WHERE fm.FinIsActive ='1' AND (fm.GrcIndBaseRate = br.brtype OR fm.IndBaseRate = br.brtype) " );
		selQuery.append(" AND br.LastMdfDate = ? " ); 
