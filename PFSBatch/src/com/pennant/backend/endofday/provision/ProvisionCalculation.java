...
public class ProvisionCalculation implements Tasklet {
	...
	public RepeatStatus execute(StepContribution arg0, ChunkContext context) throws Exception {
		...
		alwAutoProv = SystemParameterDetails.getSystemParameterValue("ALW_PROV_EOD").toString();
		if("Y".equals(alwAutoProv)){
			...
			try {
				...
				while (resultSet.next()) {
					...
					getProvisionCalculationUtil().processProvCalculations(provision, dateValueDate, false, false, financeType.isAllowRIAInvestment());
				...
				while (resultSet.next()) {
					...
					//Call Method for Provision Calculation
					getProvisionCalculationUtil().processProvCalculations(provision, dateValueDate, true, false ,financeType.isAllowRIAInvestment());
	...
	private String prepareOverDueSelectQuery() {
		...
		StringBuilder selQuery = new StringBuilder(" SELECT T1.FinReference , T1.FinBranch, T1.FinType, T1.CustID,  T1.FinODSchdDate " );
		selQuery.append(" FROM FinODDetails AS T1 ");
		selQuery.append(" WHERE T1.FinCurODAmt > 0  AND " );
		selQuery.append(" T1.FinCurODDays = (SELECT Max(FinCurODDays) FROM FinODDetails T3 WHERE T3.FinReference = T1.FinReference) " );
