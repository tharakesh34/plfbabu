...
public class DepreciationPostings implements Tasklet {
	...
	public RepeatStatus execute(StepContribution arg0, ChunkContext context) throws Exception {
		...
		try {
			...
			sqlStatement.setString(1, FinanceConstants.PRODUCT_IJARAH);
			sqlStatement.setString(2, FinanceConstants.PRODUCT_FWIJARAH);
			...
			sqlStatement.setString(2, FinanceConstants.PRODUCT_IJARAH);
			sqlStatement.setString(3, FinanceConstants.PRODUCT_FWIJARAH);
			sqlStatement.setDate(4, DateUtility.getDBDate(DateUtility.getYearStartDate(dateValueDate).toString()));
			...
			while (resultSet.next()) {
				...
				dataSet.setFinEvent(AccountEventConstants.ACCEVENT_DPRCIATE);
	...
	private String getCountQuery() {
		...
		selQuery.append(" WHERE COALESCE(T1.DepreciationFrq, ' ') <> ' ' AND (T3.FinCategory = ? or T3.FinCategory = ? )");
		selQuery.append(" AND (T1.FinIsActive = 1 OR  (T1.FinIsActive = 0 AND T4.LatestRpyDate >= ? AND T4.LatestRpyDate <= ?)) " );
		selQuery.append(" AND COALESCE(T1.ClosingStatus, ' ') <> 'C'");
	...
	private String prepareSelectQuery() {
		...
		selQuery.append(" AND (T2.RepayonSchdate= 1 OR (T2.PftOnSchDate = 1 AND T2.RepayAmount > 0)) and T2.DefSchdDate <= ?)  AS ElapsedTerms, " );
