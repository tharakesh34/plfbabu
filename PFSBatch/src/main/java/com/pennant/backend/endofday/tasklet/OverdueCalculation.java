...
public class OverdueCalculation implements Tasklet {

	private Logger				logger		= Logger.getLogger(OverdueCalculation.class);

	private DataSource			dataSource;
	private LatePaymentService	latePaymentService;
	String						logRefernce	= "";

	public OverdueCalculation() {
	...
	public RepeatStatus execute(StepContribution arg0, ChunkContext context) throws Exception {
		...
		try {
			...
			while (resultSet.next()) {
				...
				FinanceMain finMain = getLatePaymentService().getFinanceMain(finRepayQueue.getFinReference());

				// Overdue Details preparation
				getLatePaymentService().processLatePayInEOD(finMain, finRepayQueue, DateUtility.getValueDate());
	...
	private String prepareSelectQuery() {
		...
		StringBuilder selectSql = new StringBuilder();
		selectSql.append(" SELECT RQ.FinReference, RQ.FinType, RQ.RpyDate, RQ.Branch, RQ.PenaltyPayNow, RQ.SchdRate , RQ.LatePayPftPayNow, ");
	 ...
	 * @throws SQLException
	 */
	private FinRepayQueue doWriteDataToBean(FinRepayQueue finRepayQueue, ResultSet resultSet) throws SQLException {
		...
		try {
			...
			finRepayQueue.setPenaltyPayNow(resultSet.getBigDecimal("PenaltyPayNow"));
			finRepayQueue.setProfitDaysBasis(resultSet.getString("ProfitDaysBasis"));
			finRepayQueue.setSchdRate(resultSet.getBigDecimal("SchdRate"));
			finRepayQueue.setLatePayPftPayNow(resultSet.getBigDecimal("LatePayPftPayNow"));
