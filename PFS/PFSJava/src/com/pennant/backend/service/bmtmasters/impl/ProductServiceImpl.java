...
public class ProductServiceImpl extends GenericService<Product> implements ProductService {
	...
	public Product getNewProduct() {
		return getProductDAO().getNewProduct();
	}
	...
	public Product refresh(Product product) {
		logger.debug("Entering");
		getProductDAO().refresh(product);
		getProductDAO().initialize(product);
		logger.debug("Leaving");
		return product;
	}
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage,String method) {
		...
		if (StringUtils.trimToEmpty(method).equals("doApprove")|| !product.isWorkflow()) {
	...
	private List<AuditDetail> setPAssetAuditData(Product product,String auditTranType,String method) {
		...
		for (int i = 0; i < product.getProductAssetList().size(); i++) {
			...
			if(!productAsset.getRecordType().equals("")){
