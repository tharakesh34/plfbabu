...
public class BaseRateCodeServiceImpl extends GenericService<BaseRateCode> implements BaseRateCodeService {
	...
	public BaseRateCode getBaseRateCode() {
		return getBaseRateCodeDAO().getBaseRateCode();
	}
	@Override
	public BaseRateCode getNewBaseRateCode() {
		return getBaseRateCodeDAO().getNewBaseRateCode();
	}
	...
	public AuditHeader saveOrUpdate(AuditHeader auditHeader) {
		...
		if (baseRateCode.isNew()) {
			...
			auditHeader.setAuditReference(String.valueOf(baseRateCode.getBRType()));
		}else{
	...
	public AuditHeader delete(AuditHeader auditHeader) {
		...
		BaseRateCode baseRateCode = (BaseRateCode) auditHeader.getAuditDetail().getModelData();
		getBaseRateCodeDAO().delete(baseRateCode,"");
	 ...
	 */
	@Override
	public BaseRateCode refresh(BaseRateCode baseRateCode) {
		logger.debug("Entering");
		getBaseRateCodeDAO().refresh(baseRateCode);
		getBaseRateCodeDAO().initialize(baseRateCode);
		logger.debug("Leaving");
		return baseRateCode;
	}
	...
	public AuditHeader doApprove(AuditHeader auditHeader) {
		...
		if (baseRateCode.getRecordType().equals(PennantConstants.RECORD_TYPE_DEL)) {
			...
			{	
				...
				getBaseRateCodeDAO().save(baseRateCode,"");
			} else {
				tranType=PennantConstants.TRAN_UPD;
				baseRateCode.setRecordType("");
				getBaseRateCodeDAO().update(baseRateCode,"");
			}
		}

		getBaseRateCodeDAO().delete(baseRateCode,"_TEMP");
	...
	private AuditDetail validation(AuditDetail auditDetail,String usrLanguage,
		...
		BaseRateCode tempBaseRateCode = null;
		if (baseRateCode.isWorkflow()) {
			tempBaseRateCode = getBaseRateCodeDAO().getBaseRateCodeById(
					baseRateCode.getId(), "_Temp");
		}
		...
		valueParm[0] = baseRateCode.getBRType();
		errParm[0] = PennantJavaUtil.getLabel("label_BRType") + ":"+ valueParm[0];

		if (baseRateCode.isNew()) { // for New record or new record into workFlow

			if (!baseRateCode.isWorkflow()) {// With out Work flow only new records
				if (befBaseRateCode != null) { // Record Already Exists in the table then error
					auditDetail.setErrorDetail(new ErrorDetails(PennantConstants.KEY_FIELD,"41001",errParm,null));
				}
			} else { // with work flow
				if (baseRateCode.getRecordType().equals(
						PennantConstants.RECORD_TYPE_NEW)) { // if records type
					// is new
					if (befBaseRateCode != null || tempBaseRateCode != null) { // if records already exists
						// in the main table
						auditDetail.setErrorDetail(new ErrorDetails(PennantConstants.KEY_FIELD,"41001",errParm,null));
					}
				} else { // if records not exists in the Main flow table
					if (befBaseRateCode == null || tempBaseRateCode != null) {
			...
			// out work flow)
			if (!baseRateCode.isWorkflow()) { // With out Work flow for update and delete
				if (befBaseRateCode == null) { // if records not exists in the main table
				...
				if (tempBaseRateCode == null) { // if records not exists in the
					// Work flow table
					auditDetail.setErrorDetail(new ErrorDetails(PennantConstants.KEY_FIELD,"41005",errParm,null));
				}
		...
		if (StringUtils.trimToEmpty(method).equals("doApprove") || !baseRateCode.isWorkflow()) {
			auditDetail.setBefImage(befBaseRateCode);
		}
		logger.debug("Leaving");
		return auditDetail;
	}
