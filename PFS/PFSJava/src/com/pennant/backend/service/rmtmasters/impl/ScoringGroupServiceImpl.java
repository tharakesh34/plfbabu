 ...
 * 
 */
public class ScoringGroupServiceImpl extends 
GenericService<ScoringGroup> implements ScoringGroupService {
	private final static Logger logger = Logger.getLogger(ScoringGroupServiceImpl.class);
	...
	/**
	 * @return the scoringGroup
	 */
	@Override
	public ScoringGroup getScoringGroup() {
		return getScoringGroupDAO().getScoringGroup();
	}
	/**
	 * @return the scoringGroup for New Record
	 */
	@Override
	public ScoringGroup getNewScoringGroup() {
		return getScoringGroupDAO().getNewScoringGroup();
	}
	 ...
	 */
	@Override
	public ScoringGroup refresh(ScoringGroup scoringGroup) {
		logger.debug("Entering");
		getScoringGroupDAO().refresh(scoringGroup);
		getScoringGroupDAO().initialize(scoringGroup);
		logger.debug("Leaving");
		return scoringGroup;
	}
	...
	private AuditDetail validation(AuditDetail auditDetail,String usrLanguage,String method){
		...
		if(StringUtils.trimToEmpty(method).equals("doApprove") || !scoringGroup.isWorkflow()){
	...
	private List<AuditDetail> setScoringSlabAuditData(ScoringGroup scoringGroup,String auditTranType,String method) {
		...
		for (int i = 0; i < scoringGroup.getScoringSlabList().size(); i++) {
			...
			if(!scoringSlab.getRecordType().equals("")){
			...
			for (int i = 0; i < list.size(); i++) {
				...
				try {
					...
					if (!(transType.equals(""))) {
						...
						Object befImg = object.getClass().getMethod("getBefImage" +
								"", object.getClass().getClasses()).invoke(object, object.getClass().getClasses());
