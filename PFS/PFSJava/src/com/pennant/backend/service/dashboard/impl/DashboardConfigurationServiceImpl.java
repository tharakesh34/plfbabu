...
implements DashboardConfigurationService {
	 ...
	 */
	@Override
	public DashboardConfiguration refresh(DashboardConfiguration dashboardConfiguration) {
		logger.debug("Entering");
		getDashboardConfigurationDAO().refresh(dashboardConfiguration);
		getDashboardConfigurationDAO().initialize(dashboardConfiguration);
		logger.debug("Leaving");
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage,
		...
		if (StringUtils.trimToEmpty(method).equals("doApprove") || !dashboardConfiguration.isWorkflow()) {
	...
	public void savePositions(List<DashboardPosition> dashboardPositionsList, long userId) {
		...
		for (int i = 0; i < dashboardPositionsList.size(); i++) {
			...
			this.dashboardConfigurationDAO
			.SavePositions(dashboardPositionsList.get(i));
		}
	}
	 ...
	 */
	@Override
	public List<ChartSetElement> getLabelAndValues(
			DashboardConfiguration aDashboardConfiguration,String condition)throws DataAccessException {
		...
		if(aDashboardConfiguration.isDrillDownChart()){
			return getDrillDownLabelAndValues( aDashboardConfiguration,condition);
		}
		aDashboardConfiguration.setQuery(DynamicWhereConditionUtil.getModifiedQuery(aDashboardConfiguration));
	 ...
	 */
	public List<ChartSetElement> getDrillDownLabelAndValues(
			DashboardConfiguration aDashboardConfiguration,String condition)throws DataAccessException {
		/*Check this query contains any dynamic where condition parameters 
		 if contains replace where condition by calling DynamicWhereConditionUtil.getModifiedQuery()*/
		...
		for(int i=0;i<querysList.length;i++){
			...
			dashBoardConfiguration.setQuery(DynamicWhereConditionUtil.getModifiedQuery(dashBoardConfiguration));
		...
		try{
			...
			logger.error("Eroor Occured while mapping child record to parent record.Follow Drill down chart insrtuctions");
		}
