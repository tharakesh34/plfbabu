...
public class FinanceRepayPriorityServiceImpl extends GenericService<FinanceRepayPriority> implements FinanceRepayPriorityService {
	private final static Logger logger = Logger.getLogger(FinanceRepayPriorityServiceImpl.class);
	...
	/**
	 * @return the financeRepayPriority
	 */
	@Override
	public FinanceRepayPriority getFinanceRepayPriority() {
		return getFinanceRepayPriorityDAO().getFinanceRepayPriority();
	}
	/**
	 * @return the financeRepayPriority for New Record
	 */
	@Override
	public FinanceRepayPriority getNewFinanceRepayPriority() {
		return getFinanceRepayPriorityDAO().getNewFinanceRepayPriority();
	}
	 ...
	 * @param FinanceRepayPriority (financeRepayPriority)
 	 * @return financeRepayPriority
	 */
	@Override
	public FinanceRepayPriority refresh(FinanceRepayPriority financeRepayPriority) {
		logger.debug("Entering");
		getFinanceRepayPriorityDAO().refresh(financeRepayPriority);
		getFinanceRepayPriorityDAO().initialize(financeRepayPriority);
		logger.debug("Leaving");
		return financeRepayPriority;
	}
	 ...
	 * 		a)  DELETE	Delete the record from the main table by using getFinanceRepayPriorityDAO().delete with parameters financeRepayPriority,""
	 * 		b)  NEW		Add new record in to main table by using getFinanceRepayPriorityDAO().save with parameters financeRepayPriority,""
	 * 		c)  EDIT	Update record in the main table by using getFinanceRepayPriorityDAO().update with parameters financeRepayPriority,""
	 * 3)	Delete the record from the workFlow table by using getFinanceRepayPriorityDAO().delete with parameters financeRepayPriority,"_Temp"
	 * 4)	Audit the record in to AuditHeader and AdtFinRpyPriority by using auditHeaderDAO.addAudit(auditHeader) for Work flow
	 * 5)  	Audit the record in to AuditHeader and AdtFinRpyPriority by using auditHeaderDAO.addAudit(auditHeader) based on the transaction Type.
	 * @param AuditHeader (auditHeader)    
	 * @return auditHeader
	 */
	...
	public AuditHeader doApprove(AuditHeader auditHeader) {
		...
		BeanUtils.copyProperties((FinanceRepayPriority) auditHeader.getAuditDetail().getModelData(), financeRepayPriority);
	 ...
	 */
		...
		private AuditDetail validation(AuditDetail auditDetail,String usrLanguage,String method){
			...
			if (priority!=null && priority.size()>0) {	
				...
				auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(new ErrorDetails(PennantConstants.KEY_FIELD, "W0005", errParm1,valueParm), usrLanguage));
            }
			auditDetail.setErrorDetails(ErrorUtil.getErrorDetails(auditDetail.getErrorDetails(), usrLanguage));
			
			if(StringUtils.trimToEmpty(method).equals("doApprove") || !financeRepayPriority.isWorkflow()){
