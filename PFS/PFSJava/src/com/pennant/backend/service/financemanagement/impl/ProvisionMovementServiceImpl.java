...
public class ProvisionMovementServiceImpl extends GenericService<ProvisionMovement> implements ProvisionMovementService {
	...
	public ProvisionMovement getProvisionMovement() {
		return getProvisionMovementDAO().getProvisionMovement();
	}
	
	@Override
	public ProvisionMovement getNewProvisionMovement() {
		return getProvisionMovementDAO().getNewProvisionMovement();
	}
	 ...
	 */
	@Override
	public ProvisionMovement refresh(ProvisionMovement provisionMovement) {
		logger.debug("Entering");
		getProvisionMovementDAO().refresh(provisionMovement);
		getProvisionMovementDAO().initialize(provisionMovement);
		logger.debug("Leaving");
		return provisionMovement;
	}
	 ...
	 * 		a)  DELETE	Delete the record from the main table by using getProvisionMovementDAO().delete with parameters provisionMovement,""
	 * 		b)  NEW		Add new record in to main table by using getProvisionMovementDAO().save with parameters provisionMovement,""
	 * 		c)  EDIT	Update record in the main table by using getProvisionMovementDAO().update with parameters provisionMovement,""
	 * 3)	Delete the record from the workFlow table by using getProvisionMovementDAO().delete with parameters provisionMovement,"_Temp"
	 * 4)	Audit the record in to AuditHeader and AdtFinProvMovements by using auditHeaderDAO.addAudit(auditHeader) for Work flow
	 * 5)  	Audit the record in to AuditHeader and AdtFinProvMovements by using auditHeaderDAO.addAudit(auditHeader) based on the transaction Type.
	 * @param AuditHeader (auditHeader)    
	 * @return auditHeader
	 */
	 ...
	 */
		...
		private AuditDetail validation(AuditDetail auditDetail,String usrLanguage,String method){
			...
			if(StringUtils.trimToEmpty(method).equals("doApprove") || !provisionMovement.isWorkflow()){
