...
public class SplRateCodeDAOImpl extends BasisCodeDAO<SplRateCode> implements SplRateCodeDAO {
	...
	/**
	 * This method set the Work Flow id based on the module name and return the new SplRateCode 
	 * @return SplRateCode
	 */
	@Override
	public SplRateCode getSplRateCode() {
		logger.debug("Entering");
		WorkFlowDetails workFlowDetails=WorkFlowUtil.getWorkFlowDetails("SplRateCode");
		SplRateCode splRateCode= new SplRateCode();
		if (workFlowDetails!=null){
			splRateCode.setWorkflowId(workFlowDetails.getWorkFlowId());
		}
	...
	/**
	 * This method get the module from method getSplRateCode() and set the new
	 * record flag as true and return SplRateCode()
	 * 
	 * @return SplRateCode
	 */
	@Override
	 ...
	 * @param SplRateCode (splRateCode)
	 * @return SplRateCode
	 */
	@Override
	public void initialize(SplRateCode splRateCode) {
		super.initialize(splRateCode);
	}
	 ...
	 */
	@Override
	public void refresh(SplRateCode splRateCode) {
	...
	public void delete(SplRateCode splRateCode,String type) {
		...
		try{
			...
			if (recordCount <= 0) {
				ErrorDetails errorDetails= getError("41003",splRateCode.getSRType(),
						splRateCode.getUserDetails().getUsrLanguage());
				throw new DataAccessException(errorDetails.getError()) {};
			}
		}catch(DataAccessException e){
			logger.error(e);
			ErrorDetails errorDetails= getError("41006",splRateCode.getSRType(),
					splRateCode.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {};
		}
		logger.debug("Leaving");
	...
	public void update(SplRateCode splRateCode,String type) {
		...
		updateSql.append(" Set SRType = :SRType, SRTypeDesc = :SRTypeDesc, SRIsActive = :SRIsActive,");
		...
		if (recordCount <= 0) {
			...
			ErrorDetails errorDetails= getError("41004",splRateCode.getSRType(),
					splRateCode.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {};
		}
		logger.debug("Leaving");
	...
	private ErrorDetails  getError(String errorId,String sRType, String userLanguage){
