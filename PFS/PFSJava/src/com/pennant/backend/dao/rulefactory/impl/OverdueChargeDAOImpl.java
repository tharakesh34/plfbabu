...
public class OverdueChargeDAOImpl extends BasisCodeDAO<OverdueCharge> implements OverdueChargeDAO {
	...
	public void initialize(OverdueCharge overdueCharge) {
		super.initialize(overdueCharge);
	}
	 ...
	 */
	@Override
	public void refresh(OverdueCharge overdueCharge) {
	}
	...
	public void delete(OverdueCharge overdueCharge,String type) {
		...
		try{
			...
			if (recordCount <= 0) {
				ErrorDetails errorDetails= getError("41003",overdueCharge.getId() ,
						overdueCharge.getUserDetails().getUsrLanguage());
				throw new DataAccessException(errorDetails.getError()) {};
			}
		}catch(DataAccessException e){
			logger.error(e);
			ErrorDetails errorDetails= getError("41006",overdueCharge.getId() ,
					overdueCharge.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {};
		}
		logger.debug("Leaving");
	...
	public void update(OverdueCharge overdueCharge,String type) {
		...
		if (recordCount <= 0) {
			...
			ErrorDetails errorDetails= getError("41004",overdueCharge.getId() ,overdueCharge.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {};
		}
		logger.debug("Leaving");
	}
