...
public class LovFieldCodeDAOImpl extends BasisCodeDAO<LovFieldCode> implements LovFieldCodeDAO {
	...
	/**
	 * This method set the Work Flow id based on the module name and return the new LovFieldCode 
	 * @return LovFieldCode
	 */
	@Override
	public LovFieldCode getLovFieldCode() {
		logger.debug("Entering");
		WorkFlowDetails workFlowDetails=WorkFlowUtil.getWorkFlowDetails("LovFieldCode");
		LovFieldCode lovFieldCode= new LovFieldCode();
		if (workFlowDetails!=null){
			lovFieldCode.setWorkflowId(workFlowDetails.getWorkFlowId());
		}
		logger.debug("Leaving  getLovFieldCode()");
		return lovFieldCode;
	}

	/**
	 * This method get the module from method getLovFieldCode() and set the new record flag as true and return LovFieldCode()   
	 * @return LovFieldCode
	 */
	@Override
	public LovFieldCode getNewLovFieldCode() {
		logger.debug("Entering  getNewLovFieldCode()");
		LovFieldCode lovFieldCode = getLovFieldCode();
		lovFieldCode.setNewRecord(true);
		logger.debug("Leaving getNewLovFieldCode()");
		return lovFieldCode;
	}
	 ...
	 * @param LovFieldCode (lovFieldCode)
	 * @return LovFieldCode
	 */
	@Override
	public void initialize(LovFieldCode lovFieldCode) {
		super.initialize(lovFieldCode);
	}
	 ...
	 */
	@Override
	public void refresh(LovFieldCode lovFieldCode) {
	...
	public void delete(LovFieldCode lovFieldCode,String type) {
		...
		try{
			...
			if (recordCount <= 0) {
				ErrorDetails errorDetails= getError("41004", lovFieldCode.getFieldCode(), lovFieldCode.getUserDetails().getUsrLanguage());
				throw new DataAccessException(errorDetails.getError()) {};
			}
		}catch(DataAccessException e){
			logger.debug("Error in delete Method");
			logger.error(e);
			ErrorDetails errorDetails= getError("41006", lovFieldCode.getFieldCode(), lovFieldCode.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {};
		}
		logger.debug("Leaving");
	}
	...
	public void update(LovFieldCode lovFieldCode,String type) {
		...
		if (recordCount <= 0) {
			...
			ErrorDetails errorDetails= getError("41003", lovFieldCode.getFieldCode(), lovFieldCode.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {};
		}
		logger.debug("Leaving");
	}
	 ...
	 * @return ErrorDetails
	 */
	private ErrorDetails  getError(String errorId, String fieldCode, String userLanguage){
		String[][] parms= new String[2][2]; 
		parms[1][0] = fieldCode;
		parms[0][0] = PennantJavaUtil.getLabel("label_FieldCode")+ ":" + parms[1][0];
