...
public class ScoringTypeDAOImpl extends BasisCodeDAO<ScoringType> implements ScoringTypeDAO {
	 ...
	 */
	@Override
	public void refresh(ScoringType scoringType) {
	...
	public void delete(ScoringType scoringType,String type) {
		...
		try{
			...
			if (recordCount <= 0) {
				ErrorDetails errorDetails= getError("41003",scoringType.getId() ,scoringType.getUserDetails().getUsrLanguage());
				throw new DataAccessException(errorDetails.getError()) {};
			}
		}catch(DataAccessException e){
			logger.error(e);
			ErrorDetails errorDetails= getError("41006",scoringType.getId() ,scoringType.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {};
		}
		logger.debug("Leaving");
	}
	...
	public void update(ScoringType scoringType,String type) {
		...
		if (recordCount <= 0) {
			...
			ErrorDetails errorDetails= getError("41004",scoringType.getId() ,scoringType.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {};
		}
		logger.debug("Leaving");
	}
