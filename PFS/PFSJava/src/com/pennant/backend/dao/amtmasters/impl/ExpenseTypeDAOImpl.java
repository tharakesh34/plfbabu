...
public class ExpenseTypeDAOImpl extends BasisNextidDaoImpl<ExpenseType> implements ExpenseTypeDAO {
	...
	/**
	 * This method set the Work Flow id based on the module name and return the new ExpenseType 
	 * @return ExpenseType
	 */
	@Override
	public ExpenseType getExpenseType() {
		logger.debug("Entering");
		WorkFlowDetails workFlowDetails=WorkFlowUtil.getWorkFlowDetails("ExpenseType");
		ExpenseType expenseType= new ExpenseType();
		if (workFlowDetails!=null){
			expenseType.setWorkflowId(workFlowDetails.getWorkFlowId());
		}
	...
	/**
	 * This method get the module from method getExpenseType() and  
	 * set the new record flag as true and return ExpenseType()   
	 * @return ExpenseType
	 */
	@Override
	...
	public ExpenseType getExpenseTypeById(final long id, String type) {
		...
		RowMapper<ExpenseType> typeRowMapper = ParameterizedBeanPropertyRowMapper.newInstance(
				ExpenseType.class);

		try{
			expenseType = this.namedParameterJdbcTemplate.queryForObject(selectSql.toString(),
	 ...
	 */
	@Override
	public void refresh(ExpenseType expenseType) {
	...
	/**
	 * This method Deletes the Record from the AMTExpenseType or AMTExpenseType_Temp.
	 * if Record not deleted then throws DataAccessException with  error  41003.
	 * delete Expense Type Details by key ExpenceTypeId
	 * 
	...
	public void delete(ExpenseType expenseType,String type) {
		...
		try{
			...
			if (recordCount <= 0) {
				ErrorDetails errorDetails= getError("41003", expenseType.getExpenceTypeId(), 
						expenseType.getUserDetails().getUsrLanguage());
				throw new DataAccessException(errorDetails.getError()) {};
			}
		}catch(DataAccessException e){
			logger.error(e);
			ErrorDetails errorDetails= getError("41006", expenseType.getExpenceTypeId(),
					expenseType.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {};
		}
		logger.debug("Leaving");
	...
	/**
	 * This method insert new Records into AMTExpenseType or AMTExpenseType_Temp.
	 * it fetches the available Sequence form SeqAMTExpenseType by using 
	...
	/**
	 * This method updates the Record AMTExpenseType or AMTExpenseType_Temp.
	...
	public void update(ExpenseType expenseType,String type) {
		...
		if (recordCount <= 0) {
			...
			ErrorDetails errorDetails= getError("41004",expenseType.getExpenceTypeId(),
					expenseType.getUserDetails().getUsrLanguage());
			throw new DataAccessException(errorDetails.getError()) {};
		}
		logger.debug("Leaving");
	 ...
	 * @return ErrorDetails
	 */
	private ErrorDetails  getError(String errorId, long expenceTypeId, String userLanguage){
		String[][] parms= new String[2][1]; 
		parms[1][0] =String.valueOf(expenceTypeId);
		parms[0][0] = PennantJavaUtil.getLabel("label_ExpenceTypeId")+":" +parms[1][0];
