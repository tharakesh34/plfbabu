...
public class CustomerPRelationDAOImpl extends BasisNextidDaoImpl<CustomerPRelation> implements CustomerPRelationDAO {
	...
	/**
	 * This method set the Work Flow id based on the module name and return the new CustomerPRelation 
	 * @return CustomerPRelation
	 */
	@Override
	public CustomerPRelation getCustomerPRelation() {
		logger.debug("Entering");
		WorkFlowDetails workFlowDetails=WorkFlowUtil.getWorkFlowDetails("CustomerPRelation");
		CustomerPRelation customerPRelation= new CustomerPRelation();
		if (workFlowDetails!=null){
			customerPRelation.setWorkflowId(workFlowDetails.getWorkFlowId());
		}
	...
	/**
	 * This method get the module from method getCustomerPRelation() and set the
	 * new record flag as true and return CustomerPRelation()
	 * 
	 * @return CustomerPRelation
	 */
	@Override
	...
	public CustomerPRelation getCustomerPRelationByID(final long pRCustID,int pRCustPRSNo, String type) {
		...
		selectSql.append(" FROM  CustomersPRelations"+StringUtils.trimToEmpty(type));
	 ...
	 * @param CustomerPRelation (customerPRelation)
	 * @return CustomerPRelation
	 */
	@Override
	public void initialize(CustomerPRelation customerPRelation) {
		super.initialize(customerPRelation);
	}
	 ...
	 */
	@Override
	public void refresh(CustomerPRelation customerPRelation) {
	...
	public long save(CustomerPRelation customerPRelation,String type) {
		...
		if(!type.equals("") && customerPRelation.getPRCustPRSNo()==0){
	...
	public int getMaxSeqNo(long pRCustID) {
		...
		try {
			...
			logger.debug("selectSql: " + selectSql.toString());
			count = this.namedParameterJdbcTemplate.getJdbcOperations().queryForInt(selectSql.toString());
		} catch (Exception e) {
			logger.error(e);
			e.printStackTrace();
		}
