...
public class AccountNumberGeneration implements Serializable {
	...
	private AccountNumberGeneration (String acType,String ccyCode,
		...
		//this.ccyCode =StringUtils.trimToEmpty(ccyCode);
	...
	private void generateCustomerAccount(){
		...
		if(accBranch.length()<=PennantConstants.branchLength){
			branch  = StringUtils.leftPad(this.accBranch, this.accBranch.length(), '0');
		}else{
			branch  = this.accBranch.substring(this.accBranch.length()-PennantConstants.branchLength);	
		}

		if(this.accountHeade.length()<=PennantConstants.headCodeLength){
		...
		while(status){
			...
			.leftPad(String.valueOf(accountSeq),PennantConstants.accountLength -(branch.length()+headCode.length()),'0');
