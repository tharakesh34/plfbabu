...
public class BaseRateCodeDialogCtrl extends GFCBaseCtrl implements Serializable {
	...
	private void doSetValidation() {
		...
		if (!this.bRType.isReadonly()){
			this.bRType.setConstraint(new SimpleConstraint(PennantConstants.ALPHANUM_CAPS_REGEX,
					Labels.getLabel("FIELD_ALNUM_CAPS",new String[]{Labels.getLabel(
					"label_BaseRateCodeDialog_BRType.value")})));
		}	
		if (!this.bRTypeDesc.isReadonly()){
			this.bRTypeDesc.setConstraint(new SimpleConstraint(PennantConstants.DESC_REGEX,
					Labels.getLabel("MAND_FIELD_DESC",new String[]{Labels.getLabel(
					"label_BaseRateCodeDialog_BRTypeDesc.value")})));
		}
		logger.debug("Leaving");
	}
