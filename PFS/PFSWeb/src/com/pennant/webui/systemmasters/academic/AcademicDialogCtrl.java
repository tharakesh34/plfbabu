 ...
 ********************************************************************************************
 *																							*
 * FileName    		:  AcademicDialogCtrl.java                                              * 	  
 *                                                                    						*
 ...
 * /WEB-INF/pages/SystemMaster/Academic/AcademicDialog.zul file. <br>
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
 */

public class AcademicDialogCtrl extends GFCBaseCtrl implements Serializable {

	private static final long serialVersionUID = -6945930303723518608L;
	private final static Logger logger = Logger.getLogger(AcademicDialogCtrl.class);
	...
	private Academic academic; // overHanded per parameter
	private transient AcademicListCtrl academicListCtrl; // overHanded per parameter

	// old value variables for edit mode. that we can check if something
	// on the values are edited since the last initialization.
	private transient String oldVar_AcademicLevel;
	private transient String oldVar_AcademicDecipline;
	private transient String oldVar_AcademicDesc;
	...
	private transient final String btnCtroller_ClassPrefix = "button_AcademicDialog_";
	...
	public void onCreate$window_AcademicDialog(Event event) throws Exception {
		...
		if (args.containsKey("academic")) {
			this.academic = (Academic) args.get("academic");
			Academic befImage = new Academic();
			BeanUtils.copyProperties(this.academic, befImage);
			this.academic.setBefImage(befImage);

			setAcademic(this.academic);
		} else {
			setAcademic(null);
		}
		...
		if (isWorkFlowEnabled()) {
			...
			getUserWorkspace().alocateRoleAuthorities(getRole(), "AcademicDialog");
		}
		...
		if (args.containsKey("academicListCtrl")) {
			setAcademicListCtrl((AcademicListCtrl) args.get("academicListCtrl"));
		} else {
			setAcademicListCtrl(null);
		}

		// set Field Properties
		doSetFieldProperties();
		doShowDialog(getAcademic());
	 ...
	 * @throws Exception
	 */
	public void onClose$window_AcademicDialog(Event event) throws Exception {
		logger.debug("Entering" + event.toString());
		doClose();
		logger.debug("Leaving" + event.toString());
	}
	...
	public void onClick$btnHelp(Event event) throws InterruptedException {
		logger.debug("Entering" + event.toString());
		PTMessageUtils.showHelpWindow(event, window_AcademicDialog);
		logger.debug("Leaving" + event.toString());
	}
	...
	private void doClose() throws InterruptedException {
		...
		if (close) {
			closeDialog(this.window_AcademicDialog, "Academic");
		}
	 ...
	 *            Academic
	 */
	public void doWriteBeanToComponents(Academic aAcademic) {
		logger.debug("Entering");
		this.academicLevel.setValue(aAcademic.getAcademicLevel());
		this.academicDecipline.setValue(aAcademic.getAcademicDecipline());
		this.academicDesc.setValue(aAcademic.getAcademicDesc());
		this.recordStatus.setValue(aAcademic.getRecordStatus());
		logger.debug("Leaving");
	}
	 ...
	 * @param aAcademic
	 */
	public void doWriteComponentsToBean(Academic aAcademic) {
		logger.debug("Entering");
		doSetLOVValidation();
		...
		try {
			aAcademic.setAcademicLevel(this.academicLevel.getValue());
		} catch (WrongValueException we) {
			wve.add(we);
		}
		try {
			aAcademic.setAcademicDecipline(this.academicDecipline.getValue());
		} catch (WrongValueException we) {
			wve.add(we);
		}
		try {
			aAcademic.setAcademicDesc(this.academicDesc.getValue());
		} catch (WrongValueException we) {
			wve.add(we);
		}
		...
		aAcademic.setRecordStatus(this.recordStatus.getValue());
	 ...
	 * @throws InterruptedException
	 */
	public void doShowDialog(Academic aAcademic) throws InterruptedException {
		logger.debug("Entering");
		...
		if (aAcademic == null) {
			...
			aAcademic = getAcademicService().getNewAcademic();
		...
		if (aAcademic.isNew()) {
			...
			if (isWorkFlowEnabled()) {
				...
				if (!StringUtils.trimToEmpty(aAcademic.getRecordType()).equals("")) {
		...
		try {
			...
			doWriteBeanToComponents(aAcademic);

			// stores the initial data for comparing if they are changed
			// during user action.
			doStoreInitValues();
			setDialog(this.window_AcademicDialog);
	...
	private void doStoreInitValues() {
		logger.debug("Entering");
		this.oldVar_AcademicLevel = this.academicLevel.getValue();
		this.oldVar_AcademicDecipline = this.academicDecipline.getValue();
		this.oldVar_AcademicDesc = this.academicDesc.getValue();
	...
	private void doResetInitValues() {
		logger.debug("Entering");
		this.academicLevel.setValue(this.oldVar_AcademicLevel);
		this.academicDecipline.setValue(this.oldVar_AcademicDecipline);
		this.academicDesc.setValue(this.oldVar_AcademicDesc);
	...
	private boolean isDataChanged() {
		...
		if (this.oldVar_AcademicLevel != this.academicLevel.getValue()) {
			return true;
		}
		if (this.oldVar_AcademicDecipline != this.academicDecipline.getValue()) {
			return true;
		}
		if (this.oldVar_AcademicDesc != this.academicDesc.getValue()) {
			return true;
		}
		return false;
	}
	...
	private void doDelete() throws InterruptedException {
		...
		BeanUtils.copyProperties(getAcademic(), aAcademic);
		String tranType = PennantConstants.TRAN_WF;

		// Show a confirm box
		final String msg = Labels.getLabel(
		"message.Question.Are_you_sure_to_delete_this_record") + "\n\n --> " + aAcademic.getAcademicLevel();
		...
		if (conf == MultiLineMessageBox.YES) {
			...
			if (StringUtils.trimToEmpty(aAcademic.getRecordType()).equals("")) {
				...
				aAcademic.setRecordType(PennantConstants.RECORD_TYPE_DEL);
	...
	/**
	 * Create a new Academic object. <br>
	 */
	private void doNew() {
		logger.debug("Entering");
		// remember the old variables
	...
	private boolean doProcess(Academic aAcademic, String tranType) {
		...
		aAcademic.setLastMntBy(getUserWorkspace().getLoginUserDetails().getLoginUsrID());
		aAcademic.setLastMntOn(new Timestamp(System.currentTimeMillis()));
		aAcademic.setUserDetails(getUserWorkspace().getLoginUserDetails());
		...
		if (isWorkFlowEnabled()) {
			...
			if ("Save".equals(userAction.getSelectedItem().getLabel())) {
				...
				if ("".equals(nextTaskId)) {
					nextTaskId = getWorkFlow().getNextTaskIds(taskId, aAcademic);
				}

				if (PennantConstants.WF_Audit_Notes.equals(getWorkFlow().getAuditingReq(taskId, aAcademic))) {
					try {
						if (!isNotes_Entered()) {
			...
			String operationRefs = getWorkFlow().getOperationRefs(taskId, aAcademic);
	...
	private boolean doSaveProcess(AuditHeader auditHeader, String method) {
		...
		Academic aAcademic = (Academic) auditHeader.getAuditDetail().getModelData();
			...
			while (retValue == PennantConstants.porcessOVERIDE) {
				if (StringUtils.trimToEmpty(method).equalsIgnoreCase("")) {
					if (auditHeader.getAuditTranType().equals(PennantConstants.TRAN_DEL)) {
						auditHeader = getAcademicService().delete(auditHeader);
						deleteNotes = true;
					} else {
						auditHeader = getAcademicService().saveOrUpdate(auditHeader);
					}

				} else {
					if (StringUtils.trimToEmpty(method).equalsIgnoreCase(PennantConstants.method_doApprove)) {
						auditHeader = getAcademicService().doApprove(auditHeader);
						...
						auditHeader = getAcademicService().doReject(auditHeader);
						...
						retValue = ErrorControl.showErrorControl(this.window_AcademicDialog, auditHeader);
						return processCompleted;
					}
				}

				auditHeader = ErrorControl.showErrorDetails(this.window_AcademicDialog, auditHeader);
	...
	private AuditHeader getAuditHeader(Academic aAcademic, String tranType) {
		...
		return new AuditHeader(getReference(), null, null,
				null, auditDetail, aAcademic.getUserDetails(), getOverideMap());
	}
	...
	private void showMessage(Exception e) {
		...
		try {
			...
			ErrorControl.showErrorControl(this.window_AcademicDialog, auditHeader);
		} catch (Exception exp) {
			logger.error(exp);
		}
		logger.debug("Leaving");
	...
	private void refreshList() {
		logger.debug("Entering");
		final JdbcSearchObject<Academic> soAcademic = getAcademicListCtrl().getSearchObj();
		getAcademicListCtrl().pagingAcademicList.setActivePage(0);
		getAcademicListCtrl().getPagedListWrapper().setSearchObject(soAcademic);
		if (getAcademicListCtrl().listBoxAcademic != null) {
			getAcademicListCtrl().listBoxAcademic.getListModel();
		}
		logger.debug("Leaving");
	}
	...
	private Notes getNotes() {
		...
		notes.setModuleName("Academic");
		notes.setReference(getReference());
		notes.setVersion(getAcademic().getVersion());
		logger.debug("Leaving");
		return notes;
	}
	...
	private String getReference(){
		return String.valueOf(getAcademic().getAcademicID());
	}
	...
	public void setAcademicListCtrl(AcademicListCtrl academicListCtrl) {
		this.academicListCtrl = academicListCtrl;
	}
	public AcademicListCtrl getAcademicListCtrl() {
		return this.academicListCtrl;
	}
