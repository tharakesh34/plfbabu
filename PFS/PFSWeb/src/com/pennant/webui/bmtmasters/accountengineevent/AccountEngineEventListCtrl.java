 ...
 * /WEB-INF/pages/BMTMasters/AccountEngineEvent/AccountEngineEventList.zul file.<br>
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
 * 
 */
public class AccountEngineEventListCtrl extends GFCBaseListCtrl<AccountEngineEvent> implements Serializable {
	...
	public void onCreate$window_AccountEngineEventList(Event event)	throws Exception {
		...
		ModuleMapping moduleMapping = PennantJavaUtil.getModuleMap("AccountEngineEvent");
		boolean wfAvailable = true;

		if (moduleMapping.getWorkflowType() != null) {
			workFlowDetails = WorkFlowUtil.getWorkFlowDetails("AccountEngineEvent");
		...
		if (!isWorkFlowEnabled() && wfAvailable) {
			this.button_AccountEngineEventList_NewAccountEngineEvent.setVisible(false);
			this.button_AccountEngineEventList_AccountEngineEventSearchDialog.setVisible(false);
			this.button_AccountEngineEventList_PrintList.setVisible(false);
	...
	private void doCheckRights() {
		...
		getUserWorkspace().alocateAuthorities("AccountEngineEventList");
	...
	private void showDetailView(AccountEngineEvent aAccountEngineEvent)	throws Exception {
		...
		if (aAccountEngineEvent.getWorkflowId() == 0 && isWorkFlowEnabled()) {
			aAccountEngineEvent.setWorkflowId(workFlowDetails.getWorkFlowId());
		}
	...
	public void onClick$btnHelp(Event event) throws InterruptedException {
		logger.debug("Entering" + event.toString());
		PTMessageUtils.showHelpWindow(event, window_AccountEngineEventList);
		logger.debug("Leaving" + event.toString());
	}
