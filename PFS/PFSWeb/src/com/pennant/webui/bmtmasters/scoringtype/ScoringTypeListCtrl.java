 ...
 * /WEB-INF/pages/RulesFactory/ScoringType/ScoringTypeList.zul file.<br>
 * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++<br>
 * 
 */
public class ScoringTypeListCtrl extends GFCBaseListCtrl<ScoringType> implements Serializable {
	...
	public void onCreate$window_ScoringTypeList(Event event) throws Exception {
		...
		ModuleMapping moduleMapping = PennantJavaUtil.getModuleMap("ScoringType");
		boolean wfAvailable=true;

		if (moduleMapping.getWorkflowType()!=null){
			workFlowDetails = WorkFlowUtil.getWorkFlowDetails("ScoringType");
		...
		if (!isWorkFlowEnabled() && wfAvailable){
			this.button_ScoringTypeList_NewScoringType.setVisible(false);
			this.button_ScoringTypeList_ScoringTypeSearchDialog.setVisible(false);
			this.button_ScoringTypeList_PrintList.setVisible(false);
	...
	private void doCheckRights() {
		...
		getUserWorkspace().alocateAuthorities("ScoringTypeList");
			...
			if(scoringType==null){
				...
				if(isWorkFlowEnabled()){
					...
					boolean userAcces =  validateUserAccess(workFlowDetails.getId(),
							getUserWorkspace().getLoginUserDetails().getLoginUsrID(), "ScoringType", 
	...
	private void showDetailView(ScoringType aScoringType) throws Exception {
		...
		if(aScoringType.getWorkflowId()==0 && isWorkFlowEnabled()){
			aScoringType.setWorkflowId(workFlowDetails.getWorkFlowId());
		}
	...
	public void onClick$btnHelp(Event event) throws InterruptedException {
		logger.debug("Entering" + event.toString());
		PTMessageUtils.showHelpWindow(event, window_ScoringTypeList);
		logger.debug("Leaving" + event.toString());
	}
	...
	public void onClick$button_ScoringTypeList_PrintList(Event event) throws InterruptedException {
		logger.debug("Entering" + event.toString());
		PTReportUtils.getReport("ScoringType", getSearchObj());
		logger.debug("Leaving" + event.toString());
	}
