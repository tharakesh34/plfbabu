...
public class JVPostingListCtrl extends GFCBaseListCtrl<JVPosting> implements Serializable {
	...
	// +++++++++++++++++++++++ DropDown ListBox ++++++++++++++++++++++ //
		...
		this.sortOperator_BatchReference.setModel(new ListModelList<SearchOperators>(new SearchOperators().getStringOperators()));
		this.sortOperator_BatchReference.setItemRenderer(new SearchOperatorListModelItemRenderer());
	
		this.sortOperator_Batch.setModel(new ListModelList<SearchOperators>(new SearchOperators().getStringOperators()));
		this.sortOperator_Batch.setItemRenderer(new SearchOperatorListModelItemRenderer());
	
		this.sortOperator_TotDebitsByBatchCcy.setModel(new ListModelList<SearchOperators>(new SearchOperators().getStringOperators()));
		this.sortOperator_TotDebitsByBatchCcy.setItemRenderer(new SearchOperatorListModelItemRenderer());
	
		this.sortOperator_TotCreditsByBatchCcy.setModel(new ListModelList<SearchOperators>(new SearchOperators().getStringOperators()));
		this.sortOperator_TotCreditsByBatchCcy.setItemRenderer(new SearchOperatorListModelItemRenderer());
		...
		this.listBoxJVPosting.setHeight(getListBoxHeight(searchGrid.getRows().getChildren().size()-1)); 
		
		// set the paging parameters
		this.pagingJVPostingList.setPageSize(getListRows());
		this.pagingJVPostingList.setDetailed(true);

		this.listheader_BatchReference.setSortAscending(new FieldComparator("batchReference", true));
		this.listheader_BatchReference.setSortDescending(new FieldComparator("batchReference", false));
		this.listheader_Batch.setSortAscending(new FieldComparator("batch", true));
		this.listheader_Batch.setSortDescending(new FieldComparator("batch", false));
		this.listheader_DebitCount.setSortAscending(new FieldComparator("debitCount", true));
		this.listheader_DebitCount.setSortDescending(new FieldComparator("debitCount", false));
		this.listheader_CreditsCount.setSortAscending(new FieldComparator("creditsCount", true));
		this.listheader_CreditsCount.setSortDescending(new FieldComparator("creditsCount", false));
		this.listheader_TotDebitsByBatchCcy.setSortAscending(new FieldComparator("totDebitsByBatchCcy", true));
		this.listheader_TotDebitsByBatchCcy.setSortDescending(new FieldComparator("totDebitsByBatchCcy", false));
		this.listheader_TotCreditsByBatchCcy.setSortAscending(new FieldComparator("totCreditsByBatchCcy", true));
		this.listheader_TotCreditsByBatchCcy.setSortDescending(new FieldComparator("totCreditsByBatchCcy", false));
		// set the itemRenderer
		this.listBoxJVPosting.setItemRenderer(new JVPostingListModelItemRenderer());
		...
		if (item != null) {
			...
			if(jVPosting==null){
				...
				if(isWorkFlowEnabled() && moduleType==null){
					...
					WorkflowLoad flowLoad= new WorkflowLoad(jVPosting.getWorkflowId(), jVPosting.getNextTaskId(), getUserWorkspace().getUserRoleSet());
					
					boolean userAcces =  validateUserAccess("JVPosting", new String[] {"BatchReference"}, flowLoad.getRole(), getUserWorkspace().getLoginUserDetails().getLoginUsrID(), jVPosting);
					if (userAcces){
	 ...
	 */
	public void onClick$button_JVPostingList_NewJVPosting(Event event) throws Exception {
		logger.debug(event.toString());
	...
	public void onClick$button_JVPostingList_JVPostingSearch(Event event) throws Exception {
	...
	*/		
	...
	public void onClick$btnHelp(Event event) throws InterruptedException {
		logger.debug(event.toString());
		PTMessageUtils.showHelpWindow(event, window_JVPostingList);
		logger.debug("Leaving");
	}
	 ...
	 * @throws InterruptedException
	 */
	public void onClick$button_JVPostingList_PrintList(Event event) throws InterruptedException {
		logger.debug("Entering");
		logger.debug(event.toString());
	...
	private void doCheckRights() {
		...
		if(moduleType==null){
			this.button_JVPostingList_NewJVPosting.setVisible(getUserWorkspace().isAllowed("button_JVPostingList_NewJVPosting"));
		}else{
			this.button_JVPostingList_NewJVPosting.setVisible(false);
		}	
		this.button_JVPostingList_PrintList.setVisible(getUserWorkspace().isAllowed("button_JVPostingList_PrintList"));
	logger.debug("Leaving");
	}
	...
	private void showDetailView(JVPosting aJVPosting) throws Exception {
		...
		try {
			Executions.createComponents("/WEB-INF/pages/others/JVPosting/JVPostingDialog.zul",null,map);
	...
	public void doSearch() {
		...
		this.searchObj = new JdbcSearchObject<JVPosting>(JVPosting.class,getListRows());
		...
		if (isWorkFlowEnabled()) {
			...
			if (isFirstTask() && this.moduleType==null) {
				button_JVPostingList_NewJVPosting.setVisible(true);
			} else {
				button_JVPostingList_NewJVPosting.setVisible(false);
			}
	...
	// Batch Reference
		if (!StringUtils.trimToEmpty(this.batchReference.getValue()).equals("")) {
			searchObj = getSearchFilter(searchObj, this.sortOperator_BatchReference.getSelectedItem(), this.batchReference.getValue(), "BatchReference");
		}
	// Batch
		if (!StringUtils.trimToEmpty(this.batch.getValue()).equals("")) {
			searchObj = getSearchFilter(searchObj, this.sortOperator_Batch.getSelectedItem(), this.batch.getValue(), "Batch");
		}
	// Tot Debits By Batch Ccy
		if (!StringUtils.trimToEmpty(this.totDebitsByBatchCcy.getValue()).equals("")) {
			searchObj = getSearchFilter(searchObj, this.sortOperator_TotDebitsByBatchCcy.getSelectedItem(), this.totDebitsByBatchCcy.getValue(), "TotDebitsByBatchCcy");
		}
	// Tot Credits By Batch Ccy
		if (!StringUtils.trimToEmpty(this.totCreditsByBatchCcy.getValue()).equals("")) {
			searchObj = getSearchFilter(searchObj, this.sortOperator_TotCreditsByBatchCcy.getSelectedItem(), this.totCreditsByBatchCcy.getValue(), "TotCreditsByBatchCcy");
		}
		...
		getPagedListWrapper().init(this.searchObj,this.listBoxJVPosting,this.pagingJVPostingList);
