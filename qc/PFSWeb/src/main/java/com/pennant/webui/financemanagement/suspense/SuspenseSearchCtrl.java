	...
	public void doSearch() {
		...
		if (this.custID.getValue() != null) {
			...
			if (item_CustID != null) {
				...
				if (searchOpId == Filter.OP_LIKE) {
					so.addFilter(new Filter("lovDescCustCIFName", "%" + this.custID.getValue().toUpperCase() + "%",
							searchOpId));
				} else if (searchOpId == -1) {
					// do nothing
				} else {
		...
		if (this.finCurSuspAmt.getValue() != null) {
			...
			if (item_finCurSuspAmt != null) {
				final int searchOpId = ((SearchOperators) item_finCurSuspAmt.getAttribute("data"))
						.getSearchOperatorId();
		...
		this.label_SuspenseSearchResult.setValue(
				Labels.getLabel("label_SuspenseSearchResult.value") + " " + String.valueOf(paging.getTotalSize()));
