...
public class FinCreditRevSubCategorySearchCtrl extends GFCBaseCtrl<FinCreditRevSubCategory> {
	...
	public void onCreate$window_FinCreditRevSubCategorySearch(Event event) throws Exception {
		...
		if (arguments.containsKey("finCreditRevSubCategoryCtrl")) {
			this.finCreditRevSubCategoryCtrl = (FinCreditRevSubCategoryListCtrl) arguments
					.get("finCreditRevSubCategoryCtrl");
		} else {
	...
	public void doSearch() {
		logger.debug("Entering");
		final JdbcSearchObject<FinCreditRevSubCategory> so = new JdbcSearchObject<FinCreditRevSubCategory>(
				FinCreditRevSubCategory.class);
		...
		if (StringUtils.isNotEmpty(this.subCategoryCode.getValue())) {
			...
			if (itemSubCategoryCode != null) {
				final int searchOpId = ((SearchOperators) itemSubCategoryCode.getAttribute("data"))
						.getSearchOperatorId();

				if (searchOpId == Filter.OP_LIKE) {
					so.addFilter(new Filter("subCategoryCode",
							"%" + this.subCategoryCode.getValue().toUpperCase() + "%", searchOpId));
		...
		if (this.subCategorySeque.getValue() != null) {
			...
			if (itemSubCategorySeque != null) {
				final int searchOpId = ((SearchOperators) itemSubCategorySeque.getAttribute("data"))
						.getSearchOperatorId();
		...
		if (StringUtils.isNotEmpty(this.categoryId.getValue())) {
			...
			if (itemCategoryId != null) {
				...
				if (searchOpId == Filter.OP_LIKE) {
					so.addFilter(
							new Filter("categoryId", "%" + this.categoryId.getValue().toUpperCase() + "%", searchOpId));
				} else if (searchOpId == -1) {
					// do nothing
				} else {
		...
		if (StringUtils.isNotEmpty(this.subCategoryDesc.getValue())) {
			...
			if (itemSubCategoryDesc != null) {
				final int searchOpId = ((SearchOperators) itemSubCategoryDesc.getAttribute("data"))
						.getSearchOperatorId();

				if (searchOpId == Filter.OP_LIKE) {
					so.addFilter(new Filter("subCategoryDesc",
							"%" + this.subCategoryDesc.getValue().toUpperCase() + "%", searchOpId));
				} else if (searchOpId == -1) {
					// do nothing
				} else {
		...
		if (StringUtils.isNotEmpty(this.subCategoryItemType.getValue())) {
			...
			if (itemSubCategoryItemType != null) {
				final int searchOpId = ((SearchOperators) itemSubCategoryItemType.getAttribute("data"))
						.getSearchOperatorId();

				if (searchOpId == Filter.OP_LIKE) {
					so.addFilter(new Filter("subCategoryItemType",
							"%" + this.subCategoryItemType.getValue().toUpperCase() + "%", searchOpId));
				} else if (searchOpId == -1) {
					// do nothing
				} else {
		...
		if (StringUtils.isNotEmpty(this.itemsToCal.getValue())) {
			...
			if (itemItemsToCal != null) {
				...
				if (searchOpId == Filter.OP_LIKE) {
					so.addFilter(
							new Filter("itemsToCal", "%" + this.itemsToCal.getValue().toUpperCase() + "%", searchOpId));
				} else if (searchOpId == -1) {
					// do nothing
				} else {
		...
		if (StringUtils.isNotEmpty(this.itemRule.getValue())) {
			...
			if (itemItemRule != null) {
				...
				if (searchOpId == Filter.OP_LIKE) {
					so.addFilter(
							new Filter("itemRule", "%" + this.itemRule.getValue().toUpperCase() + "%", searchOpId));
				} else if (searchOpId == -1) {
					// do nothing
				} else {
		...
		if (StringUtils.isNotEmpty(this.mainSubCategoryCode.getValue())) {
			...
			if (itemMainSubCategoryCode != null) {
				final int searchOpId = ((SearchOperators) itemMainSubCategoryCode.getAttribute("data"))
						.getSearchOperatorId();

				if (searchOpId == Filter.OP_LIKE) {
					so.addFilter(new Filter("mainSubCategoryCode",
							"%" + this.mainSubCategoryCode.getValue().toUpperCase() + "%", searchOpId));
				} else if (searchOpId == -1) {
					// do nothing
				} else {
		...
		this.label_FinCreditRevSubCategorySearchResult
				.setValue(Labels.getLabel("label_FinCreditRevSubCategorySearchResult.value") + " "
