...
/**
 * This is the controller class for the /WEB-INF/pages/LMTMasters/AgreementFields/AgreementFieldsDetailDialog.zul file.
 */
public class AgreementFieldsDetailDialogCtrl extends GFCBaseCtrl<AgreementFieldDetails> {
	 ...
	 * selected AgreementFieldDetails object in a Map.
	 * 
	 * @param event
	 * @throws Exception
	 */
	public void onCreate$window_AgreementFieldsDetailDialog(Event event) throws Exception {
		...
		if (arguments.containsKey("financeMainDialogCtrl")) {
			...
			try {
				financeMainDialogCtrl.getClass().getMethod("setAgreementFieldsDetailDialogCtrl", this.getClass())
	...
	private void doSetValidation() {
		...
		if (!this.custNationalityArabic.isReadonly()) {
			this.custNationalityArabic.setConstraint(new PTStringValidator(
					Labels.getLabel("label_AgreementDialog_CustNationalityArabic.value"), null, true));
		}
		if (!this.propertyUse.isReadonly()) {
			this.propertyUse.setConstraint(
					new PTStringValidator(Labels.getLabel("label_AgreementDialog_PropertyUse.value"), null, true));
		}
		if (!this.propertyDescArabic.isReadonly()) {
			this.propertyDescArabic.setConstraint(new PTStringValidator(
					Labels.getLabel("label_AgreementDialog_PropertyDescArabic.value"), null, true));
		}
		if (!this.propertyOwner.isReadonly()) {
			this.propertyOwner.setConstraint(
					new PTStringValidator(Labels.getLabel("label_AgreementDialog_PropertyOwner.value"), null, true));
		}
		if (!this.plotUnitNumArabic.isReadonly()) {
			this.plotUnitNumArabic.setConstraint(new PTStringValidator(
					Labels.getLabel("label_AgreementDialog_PlotUnitNumArabic.value"), null, true));
		}
		if (!this.collateralAuthority.isReadonly()) {
			this.collateralAuthority.setConstraint(new PTStringValidator(
					Labels.getLabel("label_AgreementDialog_CollateralAuthority.value"), null, true));
		}
		if (!this.custCityArabic.isReadonly()) {
			this.custCityArabic.setConstraint(
					new PTStringValidator(Labels.getLabel("label_AgreementDialog_CustCityArabic.value"), null, true));
		}
		if (!this.builtUpArea.isReadonly()) {
			this.builtUpArea.setConstraint(
					new PTStringValidator(Labels.getLabel("label_AgreementDialog_BuiltUpArea.value"), null, true));
		}
		if (!this.custPoBox.isReadonly()) {
			this.custPoBox.setConstraint(
					new PTStringValidator(Labels.getLabel("label_AgreementDialog_CustomerPoBox.value"), null, true));
		}
		if (!this.collaeral1.isReadonly()) {
			this.collaeral1.setConstraint(
					new PTStringValidator(Labels.getLabel("label_AgreementDialog_collaeral1.value"), null, true));
		}
		if (!this.area.isReadonly()) {
			this.area.setConstraint(
					new PTStringValidator(Labels.getLabel("label_AgreementDialog_collaeral1.value"), null, true));
		}
	...
	private boolean doSaveProcess(AuditHeader auditHeader, String method) {
		...
		AgreementFieldDetails aAgreementFieldDetails = (AgreementFieldDetails) auditHeader.getAuditDetail()
				.getModelData();
			...
			while (retValue == PennantConstants.porcessOVERIDE) {
				if (StringUtils.isBlank(method)) {
					...
					if (StringUtils.trimToEmpty(method).equalsIgnoreCase(PennantConstants.method_doApprove)) {
						...
						if (aAgreementFieldDetails.getRecordType().equals(PennantConstants.RECORD_TYPE_DEL)) {
							deleteNotes = true;
						}

					} else if (StringUtils.trimToEmpty(method).equalsIgnoreCase(PennantConstants.method_doReject)) {
						auditHeader = getAgreementFieldsDetailService().doReject(auditHeader);
						if (aAgreementFieldDetails.getRecordType().equals(PennantConstants.RECORD_TYPE_NEW)) {
							deleteNotes = true;
						}
						...
						retValue = ErrorControl.showErrorControl(this.window_AgreementFieldsDetailDialog, auditHeader);
						return processCompleted;
					}
				}

				auditHeader = ErrorControl.showErrorDetails(this.window_AgreementFieldsDetailDialog, auditHeader);
	 ...
	 * @return AuditHeader
	 */
	private AuditHeader getAuditHeader(AgreementFieldDetails aAgreementFieldDetails, String tranType) {
		AuditDetail auditDetail = new AuditDetail(tranType, 1, aAgreementFieldDetails.getBefImage(),
				aAgreementFieldDetails);
		return new AuditHeader(String.valueOf(aAgreementFieldDetails.getId()), null, null, null, auditDetail,
