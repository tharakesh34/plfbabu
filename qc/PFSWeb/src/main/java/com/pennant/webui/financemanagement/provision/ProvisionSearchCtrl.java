...
public class ProvisionSearchCtrl extends GFCBaseCtrl<Provision> {
	...
	public void onCreate$window_ProvisionSearch(Event event) throws Exception {
		...
		this.sortOperator_autoReleaseNFP
				.setModel(new ListModelList<SearchOperators>(new SearchOperators().getBooleanOperators()));
	...
	public void doSearch() {
		...
		if (StringUtils.isNotEmpty(this.finReference.getValue())) {
			...
			if (item_FinReference != null) {
				...
				if (searchOpId == Filter.OP_LIKE) {
					so.addFilter(new Filter("finReference", "%" + this.finReference.getValue().toUpperCase() + "%",
							searchOpId));
		...
		if (this.custID.getValue() != null) {
			...
			if (item_CustID != null) {
				...
				if (searchOpId == Filter.OP_LIKE) {
					so.addFilter(
							new Filter("lovDescCustCIF", "%" + this.custID.getValue().toUpperCase() + "%", searchOpId));
				} else if (searchOpId == -1) {
					// do nothing
				} else {
		...
		if (this.provisionCalDate.getValue() != null) {
			...
			if (item_ProvisionCalDate != null) {
				final int searchOpId = ((SearchOperators) item_ProvisionCalDate.getAttribute("data"))
						.getSearchOperatorId();
		...
		if (this.provisionedAmt.getValue() != null) {
			...
			if (item_ProvisionedAmt != null) {
				final int searchOpId = ((SearchOperators) item_ProvisionedAmt.getAttribute("data"))
						.getSearchOperatorId();
		...
		if (this.nonFormulaProv.getValue() != null) {
			...
			if (item_NonFormulaProv != null) {
				final int searchOpId = ((SearchOperators) item_NonFormulaProv.getAttribute("data"))
						.getSearchOperatorId();
		...
		this.label_ProvisionSearchResult.setValue(
				Labels.getLabel("label_ProvisionSearchResult.value") + " " + String.valueOf(paging.getTotalSize()));
