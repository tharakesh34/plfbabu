...
public class TakafulProviderServiceImpl extends GenericService<TakafulProvider> implements TakafulProviderService {
	...
	/**
	 * @param takafulProviderDAO
	 *            the takafulProviderDAO to set
	 */
	 ...
	 * TakafulProvider/TakafulProvider_Temp by using TakafulProviderDAO's save method b) Update the Record in the table.
	 * based on the module workFlow Configuration. by using TakafulProviderDAO's update method 3) Audit the record in to
	 * AuditHeader and AdtTakafulProvider by using auditHeaderDAO.addAudit(auditHeader)
	 * 
	 * @param AuditHeader
	 *            (auditHeader)
	 * @return auditHeader
	 ...
	 * TakafulProvider by using TakafulProviderDAO's delete method with type as Blank 3) Audit the record in to
	 * AuditHeader and AdtTakafulProvider by using auditHeaderDAO.addAudit(auditHeader)
	 * 
	 * @param AuditHeader
	 *            (auditHeader)
	 ...
	 * workFlow table by using getTakafulProviderDAO().delete with parameters takafulProvider,"_Temp" 3) Audit the
	 * record in to AuditHeader and AdtTakafulProvider by using auditHeaderDAO.addAudit(auditHeader) for Work flow
	 * 
	 * @param AuditHeader
	 *            (auditHeader)
	 * @return auditHeader
	 ...
	 * mismatch conditions Fetch the error details from getTakafulProviderDAO().getErrorDetail with Error ID and
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage, String method, boolean onlineRequest) {
		...
		if (takafulProvider.isNew()) { // for New record or new record into work flow
			...
			if (!takafulProvider.isWorkflow()) { // With out Work flow for update and delete
				...
				if (befTakafulProvider == null) { // if records not exists in the main table
					...
					if (oldTakafulProvider != null
							&& !oldTakafulProvider.getLastMntOn().equals(befTakafulProvider.getLastMntOn())) {
				...
				if (tempTakafulProvider != null && oldTakafulProvider != null
						&& !oldTakafulProvider.getLastMntOn().equals(tempTakafulProvider.getLastMntOn())) {
					auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(
							new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, valueParm), usrLanguage));
				}
			}
		...
		if (StringUtils.equals(takafulProvider.getRecordStatus(), PennantConstants.RCD_STATUS_APPROVED)
			...
			List<InsuranceTypeProvider> insTypeprovider = getInsuranceTypeDAO()
					.getInsuranceType(takafulProvider.getTakafulCode(), "_view");
