		...
		if (productAsset.isNew()) { // for New record or new record into work flow
			...
			if (!productAsset.isWorkflow()) { // With out Work flow for update and delete
				...
				if (befProductAsset == null) { // if records not exists in the main table
					...
					if (oldProductAsset != null
							&& !oldProductAsset.getLastMntOn().equals(befProductAsset.getLastMntOn())) {
				...
				if (tempProductAsset != null && oldProductAsset != null
						&& !oldProductAsset.getLastMntOn().equals(tempProductAsset.getLastMntOn())) {
					auditDetail.setErrorDetail(new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, null));
				}
			}
		}
