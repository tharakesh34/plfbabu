...
public class AccountTypeServiceImpl extends GenericService<AccountType> implements AccountTypeService {
	 ...
	 * RMTAccountTypes/RMTAccountTypes_Temp by using AccountTypeDAO's save method b) Update the Record in the table.
	 * based on the module workFlow Configuration. by using AccountTypeDAO's update method 3) Audit the record in to
	 * AuditHeader and AdtRMTAccountTypes by using auditHeaderDAO.addAudit(auditHeader)
	 * 
	 * @param AuditHeader
	 *            (auditHeader)
	 * @return auditHeader
	 ...
	 * RMTAccountTypes by using AccountTypeDAO's delete method with type as Blank 3) Audit the record in to AuditHeader
	 * and AdtRMTAccountTypes by using auditHeaderDAO.addAudit(auditHeader)
	 * 
	 * @param AuditHeader
	 *            (auditHeader)
	 * @return auditHeader
	...
	/**
	 * getAccountTypeById fetch the details by using AccountTypeDAO's getAccountTypeById method.
	 * 
	 * @param id
	 *            (String)
	 * @param type
	...
	/**
	 * getApprovedAccountTypeById fetch the details by using AccountTypeDAO's getAccountTypeById method . with parameter
	 * id and type as blank. it fetches the approved records from the RMTAccountTypes.
	 * 
	 * @param id
	 *            (String)
	 * @return AccountType
	...
	/**
	 * getApprovedAccountTypeById fetch the detail. It fetches the approved records from RMTAccountTypes.
	 * 
	 * @param String
	 *            acType
	 * @return AccountType
	 ...
	 * following actions a) DELETE Delete the record from the main table by using getAccountTypeDAO().delete with
	 * parameters accountType,"" b) NEW Add new record in to main table by using getAccountTypeDAO().save with
	 * parameters accountType,"" c) EDIT Update record in the main table by using getAccountTypeDAO().update with
	 * parameters accountType,"" 3) Delete the record from the workFlow table by using getAccountTypeDAO().delete with
	 * parameters accountType,"_Temp" 4) Audit the record in to AuditHeader and AdtRMTAccountTypes by using
	 * auditHeaderDAO.addAudit(auditHeader) for Work flow 5) Audit the record in to AuditHeader and AdtRMTAccountTypes
	 ...
	 * workFlow table by using getAccountTypeDAO().delete with parameters accountType,"_Temp" 3) Audit the record in to
	 * AuditHeader and AdtRMTAccountTypes by using auditHeaderDAO.addAudit(auditHeader) for Work flow
	 * 
	 * @param AuditHeader
	 *            (auditHeader)
	 * @return auditHeader
	 ...
	 * from getAccountTypeDAO().getErrorDetail with Error ID and language as parameters. if any error/Warnings then
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage, String method) {
		...
		if (accountType.isWorkflow()) {
			tempAccountType = getAccountTypeDAO().getAccountTypeById(accountType.getId(), "_Temp");
		}
		AccountType befAccountType = getAccountTypeDAO().getAccountTypeById(accountType.getId(), "");
			...
			if (!accountType.isWorkflow()) { // With out Work flow for update and delete
				...
				if (befAccountType == null) { // if records not exists in the main table
					...
					if (oldAccountType != null
							&& !oldAccountType.getLastMntOn().equals(befAccountType.getLastMntOn())) {
				...
				if (tempAccountType != null && oldAccountType != null
						&& !oldAccountType.getLastMntOn().equals(tempAccountType.getLastMntOn())) {
					auditDetail.setErrorDetail(new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, null));
				}
			}
		}
