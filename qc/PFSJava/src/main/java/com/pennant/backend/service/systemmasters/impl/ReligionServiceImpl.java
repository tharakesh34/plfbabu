...
public class ReligionServiceImpl extends GenericService<Religion> implements ReligionService {
	 ...
	 * Do Add or Update the Record a) Add new Record for the new record in the DB table Religion/Religion_Temp by using
	 * ReligionDAO's save method b) Update the Record in the table. based on the module workFlow Configuration. by using
	 ...
	 * following actions a) DELETE Delete the record from the main table by using getReligionDAO().delete with
	 * parameters religion,"" b) NEW Add new record in to main table by using getReligionDAO().save with parameters
	 * religion,"" c) EDIT Update record in the main table by using getReligionDAO().update with parameters religion,""
	 * 3) Delete the record from the workFlow table by using getReligionDAO().delete with parameters religion,"_Temp" 4)
	 * Audit the record in to AuditHeader and AdtReligion by using auditHeaderDAO.addAudit(auditHeader) for Work flow 5)
	...
	public AuditHeader doApprove(AuditHeader auditHeader) {
		...
		if (religion.getRecordType().equals(PennantConstants.RECORD_TYPE_DEL)) {
			...
			if (religion.getRecordType().equals(PennantConstants.RECORD_TYPE_NEW)) {
