...
public class CorpRelationCodeServiceImpl extends GenericService<CorpRelationCode> implements CorpRelationCodeService {
	 ...
	 * BMTCorpRelationCodes/BMTCorpRelationCodes_Temp by using CorpRelationCodeDAO's save method b) Update the Record in
	...
	public AuditHeader saveOrUpdate(AuditHeader auditHeader) {
		...
		String tableType = "";
		CorpRelationCode corpRelationCode = (CorpRelationCode) auditHeader.getAuditDetail().getModelData();
	 ...
	 * following actions a) DELETE Delete the record from the main table by using getCorpRelationCodeDAO().delete with
	 * parameters corpRelationCode,"" b) NEW Add new record in to main table by using getCorpRelationCodeDAO().save with
	 * parameters corpRelationCode,"" c) EDIT Update record in the main table by using getCorpRelationCodeDAO().update
	 * with parameters corpRelationCode,"" 3) Delete the record from the workFlow table by using
	 * getCorpRelationCodeDAO().delete with parameters corpRelationCode,"_Temp" 4) Audit the record in to AuditHeader
	 * and AdtBMTCorpRelationCodes by using auditHeaderDAO.addAudit(auditHeader) for Work flow 5) Audit the record in to
	...
	public AuditHeader doApprove(AuditHeader auditHeader) {
		...
		BeanUtils.copyProperties((CorpRelationCode) auditHeader.getAuditDetail().getModelData(), corpRelationCode);

		if (corpRelationCode.getRecordType().equals(PennantConstants.RECORD_TYPE_DEL)) {
			...
			if (corpRelationCode.getRecordType().equals(PennantConstants.RECORD_TYPE_NEW)) {
	 ...
	 * from getCorpRelationCodeDAO().getErrorDetail with Error ID and language as parameters. if any error/Warnings then
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage, String method) {
		...
		CorpRelationCode corpRelationCode = (CorpRelationCode) auditDetail.getModelData();
		CorpRelationCode tempCorpRelationCode = null;

		if (corpRelationCode.isWorkflow()) {
			tempCorpRelationCode = getCorpRelationCodeDAO().getCorpRelationCodeById(corpRelationCode.getId(), "_Temp");
		}
		...
		if (corpRelationCode.isNew()) { // for New record or new record into
			...
			if (!corpRelationCode.isWorkflow()) {// With out Work flow only new
				...
				if (corpRelationCode.getRecordType().equals(PennantConstants.RECORD_TYPE_NEW)) { // if records type
					...
					if (befCorpRelationCode == null || tempCorpRelationCode != null) {
			...
			if (!corpRelationCode.isWorkflow()) { // With out Work flow for
				...
				if (befCorpRelationCode == null) { // if records not exists in
					...
					if (oldCorpRelationCode != null
							&& !oldCorpRelationCode.getLastMntOn().equals(befCorpRelationCode.getLastMntOn())) {
				...
				if (tempCorpRelationCode != null && oldCorpRelationCode != null
						&& !oldCorpRelationCode.getLastMntOn().equals(tempCorpRelationCode.getLastMntOn())) {
					auditDetail.setErrorDetail(new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, null));
				}
			}
		}
		...
		if ("doApprove".equals(StringUtils.trimToEmpty(method)) || !corpRelationCode.isWorkflow()) {
