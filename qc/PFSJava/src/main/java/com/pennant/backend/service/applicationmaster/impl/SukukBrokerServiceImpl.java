...
public class SukukBrokerServiceImpl extends GenericService<SukukBroker> implements SukukBrokerService {
	...
	/**
	 * @param sukukBrokerDAO
	 *            the sukukBrokerDAO to set
	 */
	 ...
	 * SukukBrokers by using SukukBrokerDAO's delete method with type as Blank 3) Audit the record in to AuditHeader and
	 * AdtSukukBrokers by using auditHeaderDAO.addAudit(auditHeader)
	 * 
	 * @param AuditHeader
	 *            (auditHeader)
	 * @return auditHeader
	...
	public SukukBroker getSukukBrokerById(String id) {
		...
		sukukBroker.setSukukBrokerBonds(
				getSukukBrokerBondsDAO().getSukukBrokerBondsByCode(sukukBroker.getBrokerCode(), "_View"));
		return sukukBroker;
	}

	/**
	 * getApprovedSukukBrokerById fetch the details by using SukukBrokerDAO's getSukukBrokerById method . with parameter
	 * id and type as blank. it fetches the approved records from the SukukBrokers.
	 * 
	 * @param id
	 *            (String)
	 * @return SukukBroker
	...
	public SukukBroker getApprovedSukukBrokerById(String id) {
		...
		sukukBroker.setSukukBrokerBonds(
				getSukukBrokerBondsDAO().getSukukBrokerBondsByCode(sukukBroker.getBrokerCode(), "_AView"));
		return sukukBroker;
	}
	...
	public AuditHeader doApprove(AuditHeader auditHeader) {
		...
		auditHeader.setAuditDetail(
				new AuditDetail(auditHeader.getAuditTranType(), 1, sukukBroker.getBefImage(), sukukBroker));
		getAuditHeaderDAO().addAudit(auditHeader);
	 ...
	 * workFlow table by using getSukukBrokerDAO().delete with parameters sukukBroker,"_Temp" 3) Audit the record in to
	 * AuditHeader and AdtSukukBrokers by using auditHeaderDAO.addAudit(auditHeader) for Work flow
	 * 
	 * @param AuditHeader
	 *            (auditHeader)
	 * @return auditHeader
	 ...
	 * mismatch conditions Fetch the error details from getSukukBrokerDAO().getErrorDetail with Error ID and language as
		...
		if (sukukBroker.isNew()) { // for New record or new record into work flow
			...
			if (!sukukBroker.isWorkflow()) { // With out Work flow for update and delete
				...
				if (befSukukBroker == null) { // if records not exists in the main table
					...
					if (oldSukukBroker != null
							&& !oldSukukBroker.getLastMntOn().equals(befSukukBroker.getLastMntOn())) {
				...
				if (tempSukukBroker != null && oldSukukBroker != null
						&& !oldSukukBroker.getLastMntOn().equals(tempSukukBroker.getLastMntOn())) {
					auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(
							new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, valueParm), usrLanguage));
				}
			}
	...
	private List<AuditDetail> setBrokerbondAuditData(SukukBroker sukukBroker, String method) {
		...
		List<AuditDetail> auditDetails = new ArrayList<AuditDetail>();
		String[] fields = PennantJavaUtil.getFieldDetails(new SukukBrokerBonds(),
				new SukukBrokerBonds().getExcludeFields());
	...
	public List<AuditDetail> brokerbondListValidation(List<AuditDetail> auditDetails, String method,
			String usrLanguage) {
		...
		for (AuditDetail auditDetail : auditDetails) {
			...
			if (sukukBrokerBonds.isWorkflow()) {
				tempFeeTier = getSukukBrokerBondsDAO().getSukukBrokerBondsById(sukukBrokerBonds.getBrokerCode(),
						sukukBrokerBonds.getBondCode(), "_Temp");
			}

			SukukBrokerBonds beftransactionEntry = getSukukBrokerBondsDAO()
					.getSukukBrokerBondsById(sukukBrokerBonds.getBrokerCode(), sukukBrokerBonds.getBondCode(), "");
			...
			if (sukukBrokerBonds.isNew()) { // for New record or new record into work flow
					...
					if (sukukBrokerBonds.getRecordType().equals(PennantConstants.RECORD_TYPE_NEW)) { // if records type is new
						if (beftransactionEntry != null || tempFeeTier != null) { // if records already exists in the main table
							auditDetail.setErrorDetail(
									new ErrorDetail(PennantConstants.KEY_FIELD, "41014", errParm, null));
						}
					} else { // if records not exists in the Main flow table
				...
				if (!sukukBrokerBonds.isWorkflow()) { // With out Work flow for update and delete
					...
					if (beftransactionEntry == null) { // if records not exists in the main table
						...
						if (oldTransactionEntry != null
								&& !oldTransactionEntry.getLastMntOn().equals(beftransactionEntry.getLastMntOn())) {
							if (StringUtils.trimToEmpty(auditDetail.getAuditTranType())
									.equalsIgnoreCase(PennantConstants.TRAN_DEL)) {
								auditDetail.setErrorDetail(
					...
					if (tempFeeTier != null && oldTransactionEntry != null
							&& !oldTransactionEntry.getLastMntOn().equals(tempFeeTier.getLastMntOn())) {
						auditDetail.setErrorDetail(new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, null));
					}
