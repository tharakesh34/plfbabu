...
public class WIFFinanceMainServiceImpl extends GenericService<FinanceMain> implements WIFFinanceMainService {
	 ...
	 * WIFFinanceMain/WIFFinanceMain_Temp by using WIFFinanceMainDAO's save method b) Update the Record in the table.
		...
		if (wIFFinanceMain.isNew()) { // for New record or new record into work flow
			...
			if (!wIFFinanceMain.isWorkflow()) { // With out Work flow for update and delete
				...
				if (befWIFFinanceMain == null) { // if records not exists in the main table
					...
					if (oldWIFFinanceMain != null
							&& !oldWIFFinanceMain.getLastMntOn().equals(befWIFFinanceMain.getLastMntOn())) {
				...
				if (tempWIFFinanceMain != null && oldWIFFinanceMain != null
						&& !oldWIFFinanceMain.getLastMntOn().equals(tempWIFFinanceMain.getLastMntOn())) {
					auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(
							new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, valueParm), usrLanguage));
				}
			}
