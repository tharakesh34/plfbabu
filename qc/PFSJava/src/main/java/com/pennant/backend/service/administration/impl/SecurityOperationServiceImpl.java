 ...
 * 
 */
public class SecurityOperationServiceImpl extends GenericService<SecurityOperation>
		implements SecurityOperationService {
	 ...
	 * Do Add or Update the Record a) Add new Record for the new record in the DB table SecOperations/SecOperations_Temp
	 * by using SecurityOperationDAO's save method b) Update the Record in the table. based on the module workFlow
	 ...
	 * from getSecurityOperationDAO().getErrorDetail with Error ID and language as parameters. if any error/Warnings
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage, String method) {
		...
		if (securityOperation.isWorkflow()) {
			tempSecurityOperation = getSecurityOperationDAO().getSecurityOperationByCode(securityOperation.getOprCode(),
					"_Temp");
		}

		SecurityOperation befSecurityOperation = getSecurityOperationDAO()
				.getSecurityOperationByCode(securityOperation.getOprCode(), "");
		...
		if (securityOperation.isNew()) { // for New record or new record into work flow
			...
			if (!securityOperation.isWorkflow()) { // With out Work flow for update and delete
				...
				if (tempSecurityOperation != null && oldSecurityOperation != null
						&& !oldSecurityOperation.getLastMntOn().equals(tempSecurityOperation.getLastMntOn())) {
					auditDetail.setErrorDetail(new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, null));
				}
			}
		}
		if ((StringUtils.equals(securityOperation.getRecordStatus(), PennantConstants.RCD_STATUS_SUBMITTED)
				|| "Save".equals(securityOperation.getUserAction()))
	 ...
	 * following actions a) DELETE Delete the record from the main table by using getSecurityOperaionDAO().delete with
	 * parameters securityOperation,"" b) NEW Add new record in to main table by using getSecurityOperaionDAO().save
	 * with parameters securityOperation,"" c) EDIT Update record in the main table by using
	 * getSecurityOperaionDAO().update with parameters securityOperation,"" 3) Delete the record from the workFlow table
	 * by using getSecurityOperaionDAO().delete with parameters securityOperation,"_Temp" 4) Audit the record in to
	 * AuditHeader and AdtSecOperations by using auditHeaderDAO.addAudit(auditHeader) for Work flow 5) Audit the record
	...
	public SecurityOperation getSecurityOperationRoleById(long id, String type, boolean getRoles) {
		...
		if (securityOperation != null && getRoles) {
			...
			securityOperation.setSecurityOperationRolesList(
					getSecurityOperationRolesDAO().getSecOperationRolesByOprID(securityOperation, type));
		}
