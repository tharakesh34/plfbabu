...
public class VehicleVersionDAOImpl extends SequenceDao<VehicleVersion> implements VehicleVersionDAO {
	...
	public VehicleVersion getVehicleVersionById(final long id, String type) {
		...
		RowMapper<VehicleVersion> typeRowMapper = ParameterizedBeanPropertyRowMapper.newInstance(VehicleVersion.class);

		try {
			vehicleVersion = this.jdbcTemplate.queryForObject(selectSql.toString(), beanParameters, typeRowMapper);
	...
	/**
	 * This method Deletes the Record from the AMTVehicleVersion or AMTVehicleVersion_Temp. if Record not deleted then
	 * throws DataAccessException with error 41003. delete Vehicle Version Details by key VehicleVersionId
	 * 
	 * @param Vehicle
	...
	/**
	 * This method insert new Records into AMTVehicleVersion or AMTVehicleVersion_Temp. it fetches the available
	 * Sequence form SeqAMTVehicleVersion by using getNextidviewDAO().getNextId() method.
	 * 
	...
	public long save(VehicleVersion vehicleVersion, String type) {
		...
		insertSql.append(
				"(VehicleVersionId, VehicleModelId, VehicleVersionCode, VehicleDoors,VehicleCc, VehicleCategory, ");
	...
	/**
	 * This method updates the Record AMTVehicleVersion or AMTVehicleVersion_Temp. if Record not updated then throws
	 * DataAccessException with error 41004. update Vehicle Version Details by key VehicleVersionId and Version
	 * 
	 * @param Vehicle
	 *            Version Details (vehicleVersion)
	 * @param type
	...
	public void update(VehicleVersion vehicleVersion, String type) {
		...
		updateSql.append(
				" VehicleVersionCode = :VehicleVersionCode, VehicleDoors = :VehicleDoors, VehicleCc = :VehicleCc,");
		updateSql.append(
				" VehicleCategory = :VehicleCategory, Version = :Version , LastMntBy = :LastMntBy, LastMntOn = :LastMntOn,");
	...
	public VehicleVersion getVehicleVersionByType(VehicleVersion vehicleVersion, String type) {
		...
		selectSql.append(StringUtils.trimToEmpty(type));
		selectSql.append(
				" Where VehicleVersionId != :VehicleVersionId AND VehicleModelId =:VehicleModelId AND VehicleVersionCode =:VehicleVersionCode  ");
