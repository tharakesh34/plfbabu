...
public class CostCenterServiceImpl extends GenericService<CostCenter> implements CostCenterService {
	 ...
	 * Do Add or Update the Record a) Add new Record for the new record in the DB table CostCenters/CostCenters_Temp by
	 * using CostCentersDAO's save method b) Update the Record in the table. based on the module workFlow Configuration.
	 ...
	 * following actions a) DELETE Delete the record from the main table by using getCostCenterDAO().delete with
	 * parameters costCenter,"" b) NEW Add new record in to main table by using getCostCenterDAO().save with parameters
	 * costCenter,"" c) EDIT Update record in the main table by using getCostCenterDAO().update with parameters
	 * costCenter,"" 3) Delete the record from the workFlow table by using getCostCenterDAO().delete with parameters
	 * costCenter,"_Temp" 4) Audit the record in to AuditHeader and AdtCostCenters by using
	...
	public AuditHeader doApprove(AuditHeader auditHeader) {
		...
		if (costCenter.getRecordType().equals(PennantConstants.RECORD_TYPE_DEL)) {
			...
			if (costCenter.getRecordType().equals(PennantConstants.RECORD_TYPE_NEW)) {
	 ...
	 * from getCostCenterDAO().getErrorDetail with Error ID and language as parameters. if any error/Warnings then
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage) {
		...
		if (StringUtils.trimToEmpty(costCenter.getRecordType()).equals(PennantConstants.RECORD_TYPE_DEL)) {
			...
			if (count != 0) {
				...
				parameters[0] = PennantJavaUtil.getLabel("label_CostCenterCode") + ": "
