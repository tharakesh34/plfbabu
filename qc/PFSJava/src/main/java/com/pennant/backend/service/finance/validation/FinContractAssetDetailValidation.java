...
public class FinContractAssetDetailValidation {
	...
	public List<AuditDetail> finContractAssetDetailValidation(List<AuditDetail> auditDetails, String method,
			String usrLanguage) {
	...
	private AuditDetail validate(AuditDetail auditDetail, String usrLanguage, String method) {
		...
		if (contractorAssetDetail.isWorkflow()) {
			tempContractorAssetDetail = getContractorAssetDetailDAO().getContractorAssetDetailById(
					contractorAssetDetail.getFinReference(), contractorAssetDetail.getContractorId(), "_Temp");
		}
		ContractorAssetDetail befContractorAssetDetail = getContractorAssetDetailDAO().getContractorAssetDetailById(
				contractorAssetDetail.getFinReference(), contractorAssetDetail.getContractorId(), "");
		...
		if (contractorAssetDetail.isNew()) { // for New record or new record into work flow
			...
			if (!contractorAssetDetail.isWorkflow()) { // With out Work flow for update and delete
				...
				if (befContractorAssetDetail == null) { // if records not exists in the main table
					...
					if (oldContractorAssetDetail != null && !oldContractorAssetDetail.getLastMntOn()
							.equals(befContractorAssetDetail.getLastMntOn())) {
				...
				if (tempContractorAssetDetail != null && oldContractorAssetDetail != null
						&& !oldContractorAssetDetail.getLastMntOn().equals(tempContractorAssetDetail.getLastMntOn())) {
					auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(
							new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errParm, valueParm), usrLanguage));
				}
			}
