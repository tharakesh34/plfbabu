...
public class ScoringDetailServiceImpl extends GenericService<FinanceDetail> implements ScoringDetailService {
	 ...
	 */
	@Override
	public FinanceDetail fetchFinScoringDetails(FinanceDetail financeDetail,
			List<FinanceReferenceDetail> scoringGroupList, String ctgType) {
	 ...
	 */
	@Override
	public FinanceDetail setFinanceScoringDetails(FinanceDetail financeDetail, String finType, String userRole,
			String ctgType, String screenEvent) {
		...
		if (PennantConstants.PFF_CUSTCTG_INDIV.equals(ctgType)) {
			scoringGroupList = getFinanceReferenceDetailDAO().getFinRefDetByRoleAndFinType(finType, screenEvent,
					userRole, null, "_ASGView");
		} else {
			scoringGroupList = getFinanceReferenceDetailDAO().getFinRefDetByRoleAndFinType(finType, screenEvent,
					userRole, null, "_ACSGView");
		}
	 ...
	 */
	private FinanceDetail fetchScoringSlabMetrics(FinanceDetail financeDetail,
			List<FinanceReferenceDetail> scoringGroupList, String ctgType) {
	...
	private void setFinScoreHeaderList(FinanceDetail financeDetail) {
		...
		if (StringUtils.trimToNull(finReference) != null) {
			finScoreHeaderList = getFinanceScoreHeaderDAO()
					.getFinScoreHeaderList(financeDetail.getFinScheduleData().getFinReference(), "_View");
	 ...
	 */
	@Override
	public List<ScoringMetrics> executeScoringMetrics(List<ScoringMetrics> scoringMetricsList,
	...
	public List<Object> getFinScoreDetailList(String finReference) {
		...
		List<FinanceScoreHeader> financeScoreheaders = getFinanceScoreHeaderDAO().getFinScoreHeaderList(finReference,
				"_View");
		...
		if (financeScoreheaders != null && financeScoreheaders.size() > 0) {
			...
			List<FinanceScoreDetail> financeScoreDetails = getFinanceScoreHeaderDAO().getFinScoreDetailList(headerIds,
					"_View");
			for (FinanceScoreDetail scoreDetail : financeScoreDetails) {
	 ...
	 */
	@Override
	public void validate(FinanceDetail financeDetail, AuditDetail auditDetail, String[] errParm, String[] valueParm,
			String usrLanguage) {
		logger.debug("Entering");
		...
		if (isInputAllowed) {
			if (!financeDetail.isSufficientScore()) {
				auditDetail.setErrorDetail(ErrorUtil.getErrorDetail(
						new ErrorDetail(PennantConstants.KEY_FIELD, "30564", errParm, valueParm), usrLanguage));
			}
		}
		logger.debug("Leaving");
	}
		...
		if (financeDetail.getFinScoringMetricList() != null) {
			...
			for (ScoringMetrics scoringMetric : financeDetail.getFinScoringMetricList()) {
				if (financeDetail.getScoringMetrics().containsKey(scoringMetric.getScoringId())) {
					List<ScoringMetrics> subMetricList = financeDetail.getScoringMetrics()
							.get(scoringMetric.getScoringId());
