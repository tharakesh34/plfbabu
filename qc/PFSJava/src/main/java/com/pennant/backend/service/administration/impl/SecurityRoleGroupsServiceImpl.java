...
public class SecurityRoleGroupsServiceImpl extends GenericService<SecurityRoleGroups>
		implements SecurityRoleGroupsService {
	...
	public AuditHeader save(AuditHeader auditHeader) {
		...
		for (int i = 0; i < auditHeader.getAuditDetails().size(); i++) {
			...
			if (auditHeader.getAuditDetails().get(i).getAuditTranType().equals(PennantConstants.TRAN_ADD)) {
				...
				getSecurityRoleGroupsDAO()
						.save((SecurityRoleGroups) auditHeader.getAuditDetails().get(i).getModelData());
			...
			if (auditHeader.getAuditDetails().get(i).getAuditTranType().equals(PennantConstants.TRAN_DEL)) {
				SecurityRoleGroups tempSecRoleGroup = getRoleGroupsByRoleAndGrpId(aSecRoleGroup.getRoleID(),
						aSecRoleGroup.getGrpID());
	...
	/**
	 * This method validates list of AuditDetails by calling validation() method and adds error details of each audit
	 * detail into auditHeader
	 * 
	 * @param auditHeader's
	 *            ErrorList and gets nextProcess by calling nextProcess()
	 * @return
	 */
	 ...
	 * from getSecurityRoleGroupsDAO().getErrorDetail with Error ID and language as parameters. if any error/Warnings
	...
	private AuditDetail validation(AuditDetail auditDetail, String usrLanguage, String method) {
		...
		if (StringUtils.equals(auditDetail.getAuditTranType(), PennantConstants.TRAN_ADD)) {
			rolesGroups = getSecurityRoleGroupsDAO().getRoleGroupsByRoleAndGrpId(rolesGroups.getRoleID(),
					rolesGroups.getGrpID());
			if (rolesGroups != null) {
				auditDetail.setErrorDetail(
						new ErrorDetail(PennantConstants.KEY_FIELD, "41005", errPrmRecordExisted, null));
			}
		}
	...
	/**
	 * This method fetches {@link List} of {@link SecurityGroup} by calling SecurityRoleGroupsDAO's getGroupsByRoleId()
	 * method
	 * 
	 * @param roleId
	 *            (long)
