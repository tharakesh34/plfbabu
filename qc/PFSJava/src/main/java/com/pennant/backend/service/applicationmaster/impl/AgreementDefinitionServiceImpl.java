 ...
 * 
 */
public class AgreementDefinitionServiceImpl extends GenericService<AgreementDefinition>
		implements AgreementDefinitionService {
	 ...
	 * BMTAggrementDef/BMTAggrementDef_Temp by using AgreementDefinitionDAO's save method b) Update the Record in the
	 ...
	 * following actions a) DELETE Delete the record from the main table by using getAgreementDefinitionDAO().delete
	 * with parameters agreementDefinition,"" b) NEW Add new record in to main table by using
	 * getAgreementDefinitionDAO().save with parameters agreementDefinition,"" c) EDIT Update record in the main table
	 * by using getAgreementDefinitionDAO().update with parameters agreementDefinition,"" 3) Delete the record from the
	 * workFlow table by using getAgreementDefinitionDAO().delete with parameters agreementDefinition,"_Temp" 4) Audit
	...
	public AuditHeader doApprove(AuditHeader auditHeader) {
		...
		BeanUtils.copyProperties((AgreementDefinition) auditHeader.getAuditDetail().getModelData(),
				agreementDefinition);
		...
		if (!PennantConstants.RECORD_TYPE_NEW.equals(agreementDefinition.getRecordType())) {
			auditHeader.getAuditDetail().setBefImage(
					agreementDefinitionDAO.getAgreementDefinitionByCode(agreementDefinition.getAggCode(), ""));
		}
