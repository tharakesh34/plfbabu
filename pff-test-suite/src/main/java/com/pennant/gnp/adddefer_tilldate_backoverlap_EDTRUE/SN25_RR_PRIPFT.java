package com.pennant.gnp.adddefer_tilldate_backoverlap_EDTRUE;
...
public class SN25_RR_PRIPFT extends TestingUtil {

	private static boolean isSuccess = false;
	private static BigDecimal expectedResult = new BigDecimal(8038886);
	private static BigDecimal expectedTotPft = new BigDecimal(9606682);
	private static BigDecimal expectedDefPri = new BigDecimal(15969280);
	private static BigDecimal expectedDefPft = new BigDecimal(1058129);
	...
	private static Date defPayStartDate = DateUtility.getDate("31/03/2012");
	private static Date defPayEndDate = DateUtility.getDate("31/05/2012");


	private static String getFile() {
		return getFileLoc() + SN25_RR_PRIPFT.class.getName()
				+ ".xls";
	...
	public static boolean RunTestCase() {
		try {
			...
			sh.setScheduleMethod(CalculationConstants.PRI_PFT);
			...
			sh2.getFinanceMain().setRecalFromDate(DateUtility.getDate("31/03/2012"));
			sh2.getFinanceMain().setRecalToDate(DateUtility.getDate("31/05/2012"));
			sh2 = ScheduleCalculator.addDeferment(sh2);

			//Add Deferment on January till May AND exclude already defered schedules
			sh2.getFinanceMain().setEventFromDate(
			...
			sh2.getFinanceMain().setRecalFromDate(DateUtility.getDate("29/02/2012"));
			sh2.getFinanceMain().setRecalToDate(DateUtility.getDate("31/05/2012"));
			sh2.getFinanceMain().setExcludeDeferedDates(true);
			...
			for (int i = 0; i < sh2.getFinanceScheduleDetails().size(); i++) {
				...
				if (sd.getSchDate().compareTo(defPayStartDate) >= 0
						&& sd.getSchDate().compareTo(defPayEndDate) <= 0) {
