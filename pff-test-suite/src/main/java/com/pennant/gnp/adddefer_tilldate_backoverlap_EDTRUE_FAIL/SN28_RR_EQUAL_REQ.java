package com.pennant.gnp.adddefer_tilldate_backoverlap_EDTRUE_FAIL;
...
public class SN28_RR_EQUAL_REQ extends TestingUtil {
	private static boolean isSuccess = false;
	private static BigDecimal expectedResult = new BigDecimal(8348832);
	private static BigDecimal expectedTotPft = new BigDecimal(9648832);
	private static BigDecimal expectedDefPri = new BigDecimal(15540457);
	private static BigDecimal expectedDefPft = new BigDecimal(1059543);
	...
	private static String getFile() {
		return getFileLoc() + SN28_RR_EQUAL_REQ.class.getName() + ".xls";
	...
	public static boolean RunTestCase() {
		try {
			...
			FinScheduleData sh1 = ScheduleGenerator.getNewSchd(financeDetail
					.getFinScheduleData());
			...
			sh2.getFinanceMain().setRecalFromDate(
					DateUtility.getDate("31/03/2012"));
			sh2.getFinanceMain().setRecalToDate(
					DateUtility.getDate("31/05/2012"));
			sh2 = ScheduleCalculator.addDeferment(sh2);

			// Add Deferment on January till May AND exclude already defered
			// schedules
			sh2.getFinanceMain().setEventFromDate(
