package com.pennant.gmp.mdd_tillmdt;
...
public class SN09_RR_PRIPFT_REQ extends TestingUtil {

	private static boolean isSuccess = false;
	private static BigDecimal expectedResult = new BigDecimal(17446850);
	private static BigDecimal expectedTotPft = new BigDecimal(15167862);
	private static BigDecimal resultedTotPft = BigDecimal.ZERO;


	private static String getFile() {
		return getFileLoc() + SN09_RR_PRIPFT_REQ.class.getName()
				+ ".xls";
	...
	public static boolean RunTestCase() {
		try {
			...
			sh2 = ScheduleCalculator.addDisbursement(sh2, Amount, CalculationConstants.ADDTERM_AFTMDT,BigDecimal.ZERO);
			...
			sh2.getFinanceMain().setRecalToDate(DateUtility.getDate("31/12/2012"));
			sh2.getFinanceMain().setRecalFromDate(DateUtility.getDate("31/12/2013"));
			schdMethod = CalculationConstants.PRI_PFT;
			sh2 = ScheduleCalculator.changeRepay(sh2, Amount, schdMethod);
