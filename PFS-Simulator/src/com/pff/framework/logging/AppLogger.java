...
public class AppLogger implements Log {
	...
	private static final String LAYOUT = "%d{dd MMM yyyy HH:mm:ss} %p %t %c (%F:%L) - %m%n";
	...
	public AppLogger(Class<?> cls, String fileName) {
		...
		try {
			...
			if (foundFile) {
				 ...
				 * 
				 * RollingFileAppender.b
				 * 
				 * appender = new RollingFileAppender(layout, fileName, true); appender.setMaxBackupIndex(5);
	 ...
	 * 
	 * @return a <code>Logger</code> object being used for logging by this class.
	 */
	...
	//inherit javadoc comment
	public void entering(String methodName, Object param1, Object param2, Object param3) {
	...
	public void query(String query, String desc) {
		logger.log(APP_LOGGER_CLASS_NAME, Level.DEBUG,
				AppLogger.QUERY_PREFIX + "[ " + query + " ]" + AppLogger.QUERY_DESC_PREFIX + "[ " + desc + " ]", null);
